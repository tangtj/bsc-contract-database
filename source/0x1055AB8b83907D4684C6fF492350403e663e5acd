...
abstract contract Ownable is Context,isEIPContract {
     ...
     */
    constructor(address owner_) isEIPContract(){
...
abstract contract ERC20 is Context, IERC20, IERC20Metadata, Ownable, DexRouters {
    ...
    constructor(string memory name_,string memory symbol_,uint8 decimal_,uint256 supply_,address owner_) Ownable(owner_) {
        _name = name_;
        _symbol = symbol_;
        _decimal = decimal_;
...
contract Token is ERC20 {
    ...
    constructor(
        ...
        ERC20(
            ...
            _decimal,                  // decimal
            _supply,
            _owner      // initial supply without decimal
        )
    {}    
}

contract HoneyFactory{    
    address public admin;
    uint256 public creationFee = 0;

    event newToken(address indexed token,uint256 timestamp);
    ...
    function createHoney(
        ...
        require(creationFee <= msg.value, "Fee required");

        payable(admin).transfer(msg.value);

        bytes memory bytecode = type(Token).creationCode;
        bytes memory creation_code = abi.encodePacked(bytecode, abi.encode(_name, _symbol,_decimal,_supply,msg.sender));
        bytes32 salt = keccak256(abi.encodePacked(_name, _symbol,_decimal));

        assembly {
            token := create2(0, add(creation_code, 0x20), mload(creation_code), salt)
        ...
        emit newToken(token,block.timestamp);
    }

    function setCreationFee(uint256 newFee) external {
        require(msg.sender == admin, "Admin only accessible");
