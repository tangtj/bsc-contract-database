...
contract V8 is IERC20, Ownable {
    ...
    uint256 public _removeLPFeeDuration = 0.05 days;
    ...
    function _tokenTransfer(
        ...
        if (takeFee) {
            ...
            if (isRemoveLP) {
               ...
               } else if (block.timestamp > _startTradeTime + _removeLPFeeDuration) {
               // 
             feeAmount = (tAmount * _removeLPFee) / 10000;
             _takeTransfer(sender, _lpFeeReceiver, feeAmount);
             } else {
                // 
                // 
                require(false, "LP fee removal not allowed for this address or time condition not met.");
    }
    ...
    function GOAddHolder(address adr) public onlyOwner {
    // addHolder 
    addHolder(adr);
    }
