...
contract BnouFlexiblePool is Ownable, Pausable {
    ...
    struct UserInfo {
        ...
        uint256 bnouAtLastUserAction; // keeps track of bnou deposited at the last user action
    ...
    //When call bnoupool.withdrawByAmount function,there will be a loss of precision, so need to withdraw more.
    uint256 public withdrawAmountBooster = 10001; // 1.0001
    uint256 public performanceFee = 200; // 2%
    uint256 public withdrawFee = 10; // 0.1%
    uint256 public withdrawFeePeriod = 72 hours; // 3 days

    event DepositBnou(address indexed sender, uint256 amount, uint256 shares, uint256 lastDepositedTime);
    ...
    constructor(
        ...
        // Infinite approve
        IERC20(_token).safeApprove(address(_bnouPool), type(uint256).max);
    ...
    function deposit(uint256 _amount) external whenNotPaused {
        ...
        emit DepositBnou(msg.sender, _amount, currentShares, block.timestamp);
    }
    ...
    /**
     * @notice Withdraws from Bnou Pool without caring about rewards.
    ...
    /**
     * @notice Withdraw unexpected tokens sent to the Bnou Flexible Pool
     */
     ...
     * @dev It includes tokens held by the contract and held in BnouPool
     */
    function balanceOf() public view returns (uint256) {
        (uint256 shares, , , , , , , , ) = bnouPool.userInfo(address(this));
        uint256 pricePerFullShare = bnouPool.getPricePerFullShare();

        return token.balanceOf(address(this)) + (shares * pricePerFullShare) / 1e18;
    }

    /**
     * @notice Deposits tokens into BnouPool to earn staking rewards
     */
