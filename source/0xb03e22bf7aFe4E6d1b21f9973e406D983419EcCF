...
// This is a hybrid strategy that converts USDC into other stablecoins
...
contract zsbToken is ERC20("Stabilize Strategy BSC Hybrid USDC", "zsbh-USDC"), Ownable, ReentrancyGuard {
    ...
    function valueOfVaultAndStrategy() public view returns (uint256) { // The total value of the tokens
        ...
        if(currentStrategy != StabilizeStrategy(address(0))){
            balance += currentStrategy.balance(); // And tokens stored at the strategy in normalized units
        }
        return balance;
    }
