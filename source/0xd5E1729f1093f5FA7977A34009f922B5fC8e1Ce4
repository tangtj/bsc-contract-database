...
contract PreSaleManager is Ownable {
    ...
    function withdraw(uint percent) public onlyOwner {
        require(percent > 0 && percent <= 100 && address(this).balance > 0, "Invalid percent");
        uint value = address(this).balance/100*percent;
        require(value > 0, "Owner has not balance to withdraw");
