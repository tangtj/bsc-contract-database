...
contract LotteryNFT is ERC721, Ownable {
    ...
    constructor() public ERC721("PushProtocol Lottery Ticket", "PUSHLT") {}
...
// File: contracts\PUSHLottery.sol
...
contract Lottery is OwnableUpgradeable {
    ...
    function buy(uint256 _price, uint8[4] memory _numbers) external {
        ...
        push.safeTransferFrom(address(msg.sender), address(this), _price);
    ...
    function multiBuy(uint256 _price, uint8[4][] memory _numbers) external {
        ...
        push.safeTransferFrom(address(msg.sender), address(this), totalPrice);
    ...
    function claimReward(uint256 _tokenId) external {
        ...
        if(reward>0) {
            safepushTransfer(address(msg.sender), reward);
        }
    ...
    function multiClaim(uint256[] memory _tickets) external {
        ...
        if(totalReward>0) {
            safepushTransfer(address(msg.sender), totalReward);
        }
    ...
    function safepushTransfer(address _to, uint256 _amount) internal {
        uint256 pushBal = push.balanceOf(address(this));
        if (_amount > pushBal) {
            push.transfer(_to, pushBal);
        } else {
            push.transfer(_to, _amount);
        }
    }
    ...
    function adminWithdraw(uint256 _amount) external onlyAdmin {
        push.safeTransfer(address(msg.sender), _amount);
