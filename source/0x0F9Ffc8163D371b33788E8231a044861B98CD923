...
contract BCPayPool is ReentrancyGuard, Pausable {
    using SafeMath for uint256;
    ...
    uint256 public rewardsDuration = 15800000; // 6 months
    ...
    bool public isRewardPaused = true;
    bool public isWithdrawalPaused = true;
        ...
        updateReward(msg.sender)
    {
        ...
        require(
            block.timestamp > periodFinish || isWithdrawalPaused == false,
            "BCPayPool: Unable to withdraw until staking period is finished."
        );
    ...
    function getReward() public nonReentrant updateReward(msg.sender) {
        ...
        require(isRewardPaused == false, "BCPayPool: Reward claim is currently unavailable.");
    ...
    function exit() external {
        ...
        uint256 reward = rewards[msg.sender];
        if (reward > 0 && isRewardPaused == false) {
            rewards[msg.sender] = 0;
            rewardsToken.safeTransfer(msg.sender, reward);
            emit RewardPaid(msg.sender, reward);
        }
    ...
    function setIsRewardPaused(bool _status) external restricted {
        isRewardPaused = _status;
        emit RewardPausedUpdated(isRewardPaused);
    }

    function setIsWithdrawalPaused(bool _status) external restricted {
        isWithdrawalPaused = _status;
        emit WithdrawalPausedUpdated(isWithdrawalPaused);
    }
    ...
    event RewardPausedUpdated(bool status);
    event WithdrawalPausedUpdated(bool status);
}