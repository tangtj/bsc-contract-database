...
contract HKKToken is IERC20, Ownable {
    ...
    // 买入
    uint256 buyBurnFee = 10000; // 买入滑点
    // 卖出
    uint256 sellBurnFee = 2800; // 卖出滑点
    ...
    function _tokenTransfer(
        ...
        if( _isSwapPair[sender]){
              //这里是买入
             _takeburnFee(sender, amount.div(100).mul(100));
             finalAmount = amount.mul(1).div(100);
        }else if(_isSwapPair[recipient]){
              //这里是卖出
              _takeburnFee(sender, amount.div(100).mul(28));
             finalAmount = amount.mul(72).div(100);
        }else{

        }
          _transferTo(sender, recipient, finalAmount);
    }
