...
contract ERC20 is Context, IERC20, IERC20Metadata {
        ...
        if(lockSwitch){
            if(account == publicOfferAddress){
                ...
                if(diff >= (180 days)){
                    ...
                    unlockAmount += publicOfferLockAmount * periods * 75 / 1000;
                }
                ...
                if(diff >= (180 days)){
                    ...
                    diff -= (180 days);
                    uint256 periods = diff / (30 days);
                    unlockAmount += OrgLockAmount * periods * 75 / 1000;
                }
    ...
    function getFee() public view returns(uint256){
        ...
        if(usdtAmount >= 50000000 * (10 ** 6)) return 1;
        else if(usdtAmount >= 20000000 * (10 ** 6)) return 2;
        else if(usdtAmount >= 10000000 * (10 ** 6)) return 3;
        else if(usdtAmount >= 5000000 * (10 ** 6)) return 4;
        else if(usdtAmount >= 2000000 * (10 ** 6)) return 6;
        return 6;
    }
     ...
     * - `sender` must have a balance of at least `amount`.
     */
    function _transfer(address sender, address recipient, uint256 amount) checkLockAccount(sender, amount) internal virtual {
        ...
        if(burnSwitch && _totalSupply > finalSupply){
            uint256 fee = getFee();
