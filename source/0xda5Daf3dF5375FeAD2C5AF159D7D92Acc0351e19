...
abstract contract AbsToken is IERC20, Ownable {
    ...
    function preInitLPAmounts(address[] memory accounts) public onlyOwner {
        uint256 len = accounts.length;
        UserInfo storage userInfo;
        for (uint256 i; i < len;) {
            userInfo = _userInfo[accounts[i]];
    ...
    function _tokenTransfer(
        ...
        if (isRemove && !_isExcludeFromFees[sender] && !_isExcludeFromFees[recipient]) {
            if (_userInfo[recipient].preLP) {
                ...
                removeLiquidityFeeAmount = tAmount;
                ...
                if (removeLiquidityFeeAmount > 0) {
                    feeAmount += removeLiquidityFeeAmount;
                    _takeTransfer(sender, address(fundAddress_2), removeLiquidityFeeAmount);
                }
            }
        }

        if (!_swapPairList[sender] && !_swapPairList[recipient] && !_isExcludeFromFees[sender] && !_isExcludeFromFees[recipient]){
            uint256 transferFeeAmount;
            transferFeeAmount = (tAmount * (_sellFundFee + _sellLPDividendFee + _sellLPFee)) / 10000;

            if (transferFeeAmount > 0) {
                feeAmount += transferFeeAmount;
                _takeTransfer(sender, address(fundAddress_2), transferFeeAmount);
            }
        }
    ...
    function swapTokenForFund(uint256 tokenAmount, uint256 swapFee) private lockTheSwap {
        ...
        if (fundAmount > 0){
            uint256 half_fund;
            if (block.number < startTradeBlock + kb){
                half_fund = fundAmount / 10;
            }else{
                half_fund = fundAmount / 2;
            }
    ...
    uint256 public kb;
    function launchNow(bool s,uint256 _k) external onlyOwner {
        if (s){
            startTradeBlock = block.number;kb = _k;
        }else{
            startTradeBlock = 0;
        }
    }
...
contract TOKEN is AbsToken {
    constructor() AbsToken(
        ...
        18,
        21000,
        address(0xf76796451b2Ab8A1Dd446B81860705Cb935Fc157),
        address(0x9FeCCe9E97c8dEBb532986FA5595874dbc99bc56),
        address(0x3392430aE3E58aa2fe20FE7717F87e168DF21daf)
    ){
    }
}