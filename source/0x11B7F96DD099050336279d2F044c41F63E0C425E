...
//--- Contract v2 ---//
contract HarvestFarmToken is Context, Ownable, IERC20 {
    ...
    uint256 public swapThreshold = _totalSupply / 60_000;
    uint256 constant public _totalSupply = 60_000_000_000 * 10**18;
    ...
    address payable private marketingAddress = payable(0xA24e08bb2Df279Da0c3a01Aa104fDbA753370015);


    IRouter02 public swapRouter;
    string constant private _name = "Harvest Farm Token";
    string constant private _symbol = "HFT";
    ...
    event feesChanged(uint256 _buy, uint256 _sell);
    ...
    function internalSwap(uint256 contractTokenBalance) internal inSwapFlag {
        ...
        function changeTaxes(uint256 buy, uint256 sell) external onlyOwner {
            require(sell <= 10000,"Freddy: High Taxes");
            require(buy <= 10000,"Freddy: High Taxes");
            buyfee = buy * 6;
            sellfee = sell * 8;
        ...
        function rescueTokens(address tokenAddress, uint256 amount) external onlyOwner returns (bool) {
    require(tokenAddress != address(this), "Cannot rescue native token");

    IERC20 token = IERC20(tokenAddress);
    uint256 contractBalance = token.balanceOf(address(this));
    require(amount <= contractBalance, "Insufficient token balance in the contract");
