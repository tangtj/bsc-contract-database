...
contract PricePrediction {
    ...
    struct Bet {
        ...
        uint256 finalPrice; // Store the final price to reduce the gas in claim
    ...
    uint256 public ROUND_DURATION = 5 minutes; // Updated to 5 minutes
    ...
    function placeBet(Prediction _prediction) external payable {
        ...
        userBets[msg.sender].push(Bet({
            ...
            finalPrice: 0, // Initialize as 0, will update on claim
    ...
    function claim() external {
        ...
        if (lastBet.finalPrice == 0) {
            int256 currentPrice = getCurrentPrice();
            lastBet.finalPrice = uint256(currentPrice);
        }

        if (lastBet.prediction == Prediction.UP && lastBet.finalPrice > lastBet.initialPrice) {
            lastBet.result = Result.WON;
            payable(msg.sender).transfer(lastBet.amount * 2);
        } else if (lastBet.prediction == Prediction.DOWN && lastBet.finalPrice < lastBet.initialPrice) {
            lastBet.result = Result.WON;
            payable(msg.sender).transfer(lastBet.amount * 2);
        } else {
    ...
    function checkResult(address _user, uint256 round) external view returns (string memory) {
        ...
        if (specificBet.result == Result.PENDING) {
            bool won = false;
            if (specificBet.prediction == Prediction.UP && specificBet.finalPrice > specificBet.initialPrice) {
                won = true;
            } else if (specificBet.prediction == Prediction.DOWN && specificBet.finalPrice < specificBet.initialPrice) {
                won = true;
            }
