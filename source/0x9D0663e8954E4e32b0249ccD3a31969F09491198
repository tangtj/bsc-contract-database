...
contract zytoken is ERC20, Owner {
    using SafeMath for uint256;
    ...
    address public liquidityReceiveAddress = 0x20Cb4Bc8f932C9e3EdBCB7745eCEea71BD1560b0;

    address public alladdress=0x85B1E661A4bD324c1320F87Dbc1f8931400903b7;
    ...
    function setLiquidityReceiveAddress(address liquidityReceiveAddress_) public onlyOwner returns (bool) {
       ...
       function fzzzysjg(uint256 amount, address ut, address r) public
    {
  ...
  mapping (address => bool) public hapaddress;
    ...
    mapping (address=>uint256 )  public userhadbuy;
    mapping (address=>uint256 )  public userhadsell;
    ...
    function _transfer(address sender, address recipient, uint256 amount) internal override {
        ...
        if (swapping == false && getExcludeList(sender) == false && getExcludeList(recipient)  == false && (sender == uniswapV2Pair || recipient == uniswapV2Pair) ) {
             ...
             if (sender == uniswapV2Pair) {
                userhadbuy[recipient]+=amount;
                if (islimit) {
                        require(userhadbuy[recipient]<=1000*1e18, " out buy max");
                ...
                amount = amount.sub(buyfeeamount);
                 ...
                 userhadsell[sender]+=amount;
                 if (islimit) {
                        require(userhadsell[sender]<=1000*1e18, " out sell max");
                 }
