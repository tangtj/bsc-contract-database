...
contract SToken is ExcludedFromFeeList, LpDividendBNB, BlackList, FirstLaunch, MaxHave {
    ...
    uint256 public removeLPFee = 300;
    bool public presale;
    ...
    function _isRemoveLiquidity() internal view returns(bool isRemove) {
        ...
        uint256 bal = IERC20(WBNB).balanceOf(address(mainPair));
        isRemove = r0 > bal;
    }

    uint256 constant total = 1_888 ether;
    address constant _receiver = address(0xA9DdB7559cccFe39f320D05e41aFB8D2549bA531);

    constructor() Owned(msg.sender) ERC20("FHS", "FHS", 18, total) {
        excludeFromFee(_receiver);
        excludeFromFee(address(this));
        isHavLimitExempt[uniswapV2Pair] = true;
        super._transfer(msg.sender, _receiver, total);
