...
contract FarmProRata is Ownable {
    ...
    address public constant trustedStakingContractAddress = 0x52B638Ee4C0Db759234f792B0ad59A8ce95737f3;
    ...
    function addHolder(address account, uint amount) external onlyOwner {
        require(amount > 0, "Amount must not be 0!");
        require(account != address(0), "Invalid account!");
        IERC20(trustedDepositTokenAddress).transferFrom(msg.sender, address(this), amount);
        
        depositedTokens[account] = depositedTokens[account].add(amount);
        totalTokens = totalTokens.add(amount);

        holders.add(account);
        depositTime[account] = now;
    }
