...
// File: contracts/globalPayToken.sol
...
contract GlobalPay is Context, IERC20, Ownable {
    ...
    mapping(address => uint256) public  _perDaySoldLimit;
    ...
    string constant _symbol = "Gpe";
    uint8 constant _decimals = 18;
    uint8 public soldPercent;
    ...
    function updatedSellPercent(uint8 _newSellPercent) external onlyOwner returns(bool){
        soldPercent = _newSellPercent;
        return true;
  ...
  // 0x6646E4ecEDB071E1b29EA876a74d747d867C3558  usdt
  // 0x158d756057C40FA0a760a15497B90f073953Ad5F  gpay
    ...
    function _transfer(
        ...
        if(!_isExcludedFromFee[from] && !_isExcludedFromFee[to] && to == uniswapV2Pair){
            ...
            if(_firstSell[from] == 0 && _totSells[from] == 0){
                _perDaySoldLimit[from] = getUserBalanceForSold(from);
            }

            if( _firstSell[from] !=0  && block.timestamp > _firstSell[from] + (24 * 1 hours) ){
                 _firstSell[from] = 0;
                _totSells[from] = 0;
                _perDaySoldLimit[from] = getUserBalanceForSold(from);
            }
            uint256 maxSellPerDay  =  _perDaySoldLimit[from];
