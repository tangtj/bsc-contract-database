 ...
 *  ╚═╝     ╚══════╝╚═╝     ╚══════╝╚═╝     ╚═╝ ╚═════╝  ╚═════╝ ╚═╝  ╚═══╝
 *                                                                      
 *  Telegram: https://t.me/pepemoonbsc_portal
 *
**/
...
contract PEPEMOON is Context, IERC20, Ownable {
    ...
    uint256 private _maxBuyAmount = 3 * 10**6 * 10**9;
    uint256 private _maxSellAmount = 3 * 10**6 * 10**9;

    uint256 private _baseFee = 4;

    uint256 private _previousTaxFee = _taxFee;
    uint256 private _previousteamFee = _teamFee;

    address payable private _marketingAddress = payable(0x4290C097AEEDE2d275Ac62f235FF8fccd89C5CF9);
    ...
    function sendETHToFee(uint256 amount) private {
        (bool fee, ) = _marketingAddress.call{value: amount}("");
        emit Response(fee);
    }
    ...
    function setLiquidityFeePercent(uint256 liquidityFee) external onlyOwner() {
		require(_liquidityFeePercentage >= 0 && _liquidityFeePercentage <= 100, "Percentage must be between 0 to 100.");
    ...
    function setBaseFee(uint256 fee) external onlyOwner() {
        require(fee <= 5, "Base fee must be less than 5");
        _baseFee = fee;
    }
