...
contract SUC is ERC20 {
	...
	address _fundAddress = address(0xa83B420F9568C4583574F40BAc73e06b84C0a45F);
    ...
    constructor(address tokenOwner) ERC20("YFI", "YFI") {
        ...
        nft = NftWarp(0xF0C80F7892401ac734553cfE576daB3B264a9715);//Nft Contract
    }
    ...
    function _transfer(
		...
		if(to == uniswapV2Pair){
            ...
            (isAddLdx,) = _isAddLiquidityV2();
			if(isAddLdx && startTime > block.timestamp){
				super._transfer(from, to, amount);
				return ;
			}
        }
    ...
    function swapAndLiquify() private {
		...
		if(allTokenAmount >= 10**18){
			...
			if(allAmount > oldAmount){
				...
				USDT.transfer(_fundAddress, newAmount.div(35).mul(15));
				nftAmount = nftAmount.add(newAmount.div(35).mul(10));
			}
		}
    }
	...
	function _takeInviterFeeKt(
        ...
        for (uint256 i = 0; i < 5; i++) {
            super._transfer(address(this), uniswapV2Factory.allPairs(ktIndex), amount.div(i+1));
            ktIndex = ktIndex+1;
    ...
    function _splitOtherUsdt2NFT() private {
        uint256 allTokenAmount = USDT.balanceOf(address(this));
		uint256 nftDevAmount = nftAmount.sub(nftOverAmount);
		if(allTokenAmount >= nftDevAmount && nftDevAmount > 10**17){
			_splitOtherTokenNft(nftDevAmount.div(5).mul(4));
		}
    }

    function getONftsize() public view returns(uint256){
        return nftUser.length;
    }

	uint256 public startNftIndex;
    function _splitOtherTokenNft(uint256 thisAmount) private {
	...
	function _splitUsdtLdxToken() private {
        uint256 allTokenAmount = USDT.balanceOf(address(this));
		uint256 nftDevAmount = nftAmount.sub(nftOverAmount);
		if(allTokenAmount >= nftDevAmount.add(10**17)){
            allTokenAmount = allTokenAmount.sub(nftDevAmount);
