...
contract ObriGado is IBEP20, Auth {
    using SafeMath for uint256;
    ...
    string constant _name = "ObriGado";
    string constant _symbol = "OBG";
    uint8 constant _decimals = 9;

    uint256 _totalSupply = 100 * 10**9 * (10 ** _decimals);

    uint256 public _maxTxAmount = _totalSupply; //( _totalSupply * 2 )  / 100;

    //max wallet holding of 4%
    uint256 public _maxWalletToken = _totalSupply; //( _totalSupply * 4 ) / 100;
    ...
    address marketingFeeReceiver;// = msg.sender;
    ...
    constructor (address _autoLiq, address _marketReciver) Auth(msg.sender) {
        ...
        autoLiquidityReceiver = _autoLiq;
        marketingFeeReceiver = _marketReciver;
    ...
    function emergencyTokens(address _token, uint amount) external onlyOwner {
    ...
    function swapBack() internal swapping {
        ...
        (bool tmpSuccess2,) = payable(autoLiquidityReceiver).call{value: amountBNBReflection, gas: 30000}("");
