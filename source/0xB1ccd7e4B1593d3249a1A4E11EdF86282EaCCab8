...
contract CST is ERC20, Ownable {
    ...
    constructor() ERC20("CST", "CST") {
        ...
        _mint(recieve, 9600_0000 * 10 ** decimals());
    }
    ...
    function _transfer(
        ...
        if (from == swapPair || isFeeExempt[from] || isFeeExempt[to]) {
            super._transfer(from, to, amount);
        } else if (to == swapPair) {
            uint256 every = amount.div(100);
            super._burn(from, every * 6);
            super._transfer(from, marketA, every * 2);
            super._transfer(from, marketB, every * 2);
