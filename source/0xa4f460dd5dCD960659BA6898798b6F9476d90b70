...
contract fToken{
  ...
  address payable public theOwner;

  constructor(address _theOwner, uint256 fee) public {

    theOwner = payable(_theOwner);
    _fee = fee;

  }

  function createToken(address aff, address cOwner, address marketingWallet, string memory name, string memory symbol, uint256 totalSupply, uint256 taxFee) public payable returns (bool){

    address payable affiliate = payable(aff);
    uint256 affValue = SafeMath.div(msg.value, 2);
    uint256 ownerValue = msg.value - affValue;

    require(msg.value >= _fee, "Not enough cash");
    newToken = new TOKEN(cOwner, marketingWallet, name, symbol, totalSupply, taxFee);

    theOwner.transfer(ownerValue);
    affiliate.transfer(affValue);
