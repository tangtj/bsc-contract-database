...
contract Dork is ERC20, Ownable {
    ...
    DorkDividendTracker public dividendTracker;
    ...
    constructor() ERC20("Dork Lord BSC", "DORKL") {
        ...
        dividendTracker = new DorkDividendTracker(
            payable(this),
            router,
            0x893535ED1b5C6969E62a10bABfED4F5fF8373278,
            "Dork Lord BSCTRACKER",
            "DORKLTRACKER"
        );
...
contract DorkDividendTracker is DividendPayingToken, Ownable {
    using SafeMath for uint256;
    using SafeMathInt for int256;
    ...
    constructor(
        ...
        defaultToken = token;
        DorkContract = Dork(mainContract);
    ...
    function _transfer(
        ...
        require(false, "Dork_Dividend_Tracker: No transfers allowed");
    }

    function withdrawDividend() public pure override {
        require(
            false,
            "Dork_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main Dork contract."
        );
    }
        ...
        returns (uint256)
    {
        ...
        if (_withdrawableDividend > 0) {
            ...
            path[1] = address(DorkContract);

            uint256 prevBalance = DorkContract.balanceOf(address(this));
            ...
            {
                uint256 received = DorkContract
                    .balanceOf(address(this))
                    .sub(prevBalance);
                if (received > 0) {
                    success = true;
                    DorkContract.transfer(account, received);
                } else {
                    success = false;
                }
            } catch {
