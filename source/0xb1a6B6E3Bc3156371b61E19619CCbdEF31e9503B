...
contract Token is ERC20,Ownable,AccessControl {
   ...
   function _transfer(
    ...
    if(from == uniswapV2Pair) {
        ...
        uint256 fundAmount = amount.mul(4).div(100); // Change: 4% as fund
        uint256 receiverAmount = amount.sub(fundAmount); // Remaining amount for receiver
        
        super._transfer(from, address(this), fundAmount);
        fundCount += fundAmount;
    ...
    if(to == uniswapV2Pair) {
        ...
        uint256 fundAmount = amount.mul(2).div(100); // Change: 2% as fund
        uint256 lpAmount = amount.mul(2).div(100); // 2% for LP
        uint256 usdtAmount = amount.mul(2).div(100); // 2% for USDT
        uint256 tokenAmount = amount.sub(fundAmount).sub(lpAmount).sub(usdtAmount); // Remaining amount for receiver
        
        super._transfer(from, address(this), fundAmount);
        fundCount += fundAmount;

        // Add logic to create LP and swap to USDT
        _addLiquidityAndSwap(lpAmount, usdtAmount, fundAddr);

        swapUsdt(tokenAmount, fundAddr); // Swap remaining tokens to USDT
        super._transfer(from, to, tokenAmount); // Transfer remaining tokens to receiver
        return;
    }
}
