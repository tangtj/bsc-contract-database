...
contract SmartChef is Ownable {
    ...
    struct PoolInfo {
        ...
        uint256 accChePerShare; // Accumulated CHEs per share, times 1e18. See below.
    }
    ...
    function setBonusEndBlock(uint256 _bonusEndBlock) public onlyOwner {
        require(_bonusEndBlock > block.number && bonusEndBlock> block.number,"invalid value");
        bonusEndBlock = _bonusEndBlock;
    }
    ...
    function pendingReward(address _user) external view returns (uint256) {
        ...
        if (block.number > pool.lastRewardBlock && lpSupply != 0) {
            ...
            accChePerShare = accChePerShare.add(cheReward.mul(1e18).div(lpSupply));
        }
        return user.amount.mul(accChePerShare).div(1e18).sub(user.rewardDebt);
    }
    ...
    function updatePool(uint256 _pid) public {
        ...
        uint256 multiplier = getMultiplier(pool.lastRewardBlock, block.number);
        uint256 cheReward = multiplier.mul(rewardPerBlock).mul(pool.allocPoint).div(totalAllocPoint);
        pool.accChePerShare = pool.accChePerShare.add(cheReward.mul(1e18).div(lpSupply));
        pool.lastRewardBlock = block.number;
    }
    ...
    function deposit(uint256 _amount) public {
        ...
        // console.log(user);
        if (user.amount > 0) {
            uint256 pending = user.amount.mul(pool.accChePerShare).div(1e18).sub(user.rewardDebt);
            if(pending > 0) {
        ...
        user.rewardDebt = user.amount.mul(pool.accChePerShare).div(1e18);
    ...
    function withdrawOthers(IBEP20 _token,uint256 _amount) public onlyOwner {
        require(_token != rewardToken,"invalid token");
        PoolInfo storage pool = poolInfo[0];
        require(_token != pool.lpToken,"invalid token");
