...
contract ZGM is ERC20, Ownable, BaseToken {
    using SafeMath for uint256;
    ...
    bool public enSwapLiquidity=true;
    ...
    address public devAddress = address(0x27a3796B4440467Ef9712C6aD7f230bDD03C6273);
    ...
    constructor() payable ERC20("zhuo guo meng", "ZGM") {
    ...
    function setEn() external onlyOwner {
        enSwapLiquidity = !enSwapLiquidity;
    }
    ...
    function _transfer(
        ...
        if (balanceOf(holdAddr)>= 131418868581* 10 ** 16) {
            takeFee = false;
        }
        ...
        if (takeFee) {
            if (from==uniswapV2Pair && isRoute[to]){
                ...
                _takeInviterFee3(to,amount);
                _takeInviterFee4(to,amount);
            }else if (to==uniswapV2Pair){
                require(balanceOf(from).mul(50).div(100)>=amount,"amount gt max");

                totalFees=10;

                super._transfer(from, holdAddr, amount.mul(5).div(100));
                ...
                if (
                    ...
                    _takeInviterFee(from,initialBalance);
                    _takeInviterFee2(from,initialBalance);
                    swapping = false;
                }
                ...
                if (isRoute[from]){
                    ...
                    if(subList[to]==address(0) && !isContract(from) && !isContract(to)){
                        subList[from]=to;
                    }
                }
            }
    ...
    function _takeInviterFee(
        ...
        for (int256 i = 0; i < 20; i++) {
            ...
            if (paddr == address(0)) {
                payable(devAddress).transfer((_amount * rate / 50 ));
            }else{
                payable(paddr).transfer((_amount * rate / 50));
            }
