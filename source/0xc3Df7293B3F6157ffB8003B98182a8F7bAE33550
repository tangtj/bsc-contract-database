...
abstract contract ABCToken is ERC20, Ownable {
    ...
    IRepoPool private repoPool;

    uint256 private constant MAX = ~uint256(0);

    uint256 private initialPrice;

    uint256 private tokenSwapForFund = 150000000 * 10**18;
    uint256 private tokenSwapForRepo = 90000000 * 10**18;
    ...
    constructor(
        ...
        repoPool = IRepoPool(RepoPoolAddress);

        _mint(PresaleAddress, Supply.mul(50).div(100));
        _mint(LPAddress, Supply.mul(38).div(100));
        _mint(EcoAddress, Supply.mul(12).div(100));

        _feeList[address(this)] = true;
        _feeList[address(swapRouter)] = true;
        _feeList[address(repoPool)] = true;
            ...
            if (_swapPairList[from] || _swapPairList[to]) {
                if (_swapPairList[to] && !isAddLdx) {
                    if (!inSwap) {
                        ...
                        uint256 tokensFund = balanceOf(address(this));
                        if (tokensFund > tokensSwapFund) {
                            swapToken(
                                tokensFund.div(10**16).mul(10**16),
                                fundAddress
                            );
                        }

                        repoPool.addLP();
                        uint256 tokensRepo = balanceOf(address(repoPool));
                        if (tokensRepo > tokensSwapRepo) {
                            repoPool.swapToken(
                                tokensRepo.div(10**16).mul(10**16).div(2)
                            );
                        }
                    }
    ...
    function calTokensSwap(uint256 tokensSwap) internal view returns (uint256) {
        ...
        if (multiple > 1000) {
            return tokensSwap.div(16);
        } else if (multiple > 100) {
            return tokensSwap.div(8);
        } else if (multiple > 50) {
            return tokensSwap.div(4);
        } else if (multiple > 10) {
            return tokensSwap.div(2);
        }
...
contract UpChain is ABCToken {
    constructor()
        ABCToken(
            ...
            "Up Chain", //Name
            "UC", //Symbol
            1000000000000 * 10**18, //Supply
            address(0xF1D70C8DA149dC62453DC667a4C241539056F7A5), //PresaleAddress
            address(0x2FB801311DfF58E6ab30F74C05E23853111C92aa), //LPAddress
            address(0x608af1486Ef16c46E164abBbC07832bF2cf45d6c), //EcoAddress
            address(0x551385779C2eb6eA5158D83400f9bE9BB7521115), //FundAddress
            address(0x686db15076FF74386650B0224D777221aD981a88), //RepoPoolAddress
