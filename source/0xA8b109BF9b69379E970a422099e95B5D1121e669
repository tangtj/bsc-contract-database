...
contract WUKONGINU is ERC20, Ownable {
    ...
    address public devmarketWallet = 0x3F7Cd728725999154189266de642CA4346d81618;
    ...
    constructor() ERC20("WUKONG INU", "WUKONG") {
        ...
        exemptFee[devmarketWallet] = true;
    ...
    function _transfer(
       ...
       if (!exemptFee[sender] && !exemptFee[recipient]) {
        ...
        if (_interlock || exemptFee[sender] || exemptFee[recipient])
            ...
            feeswap = sellTaxes.liquidity + sellTaxes.devmarket;
            feesum = feeswap;
            currentTaxes = sellTaxes;
        } else if (!useLaunchFee) {
            feeswap = taxes.liquidity + taxes.devmarket;
            feesum = feeswap;
            currentTaxes = taxes;
        } else if (useLaunchFee) {
            feeswap = launchtax;
    ...
    function Liquify(uint256 feeswap, Taxes memory swapTaxes) private lockTheSwap {
        ...
        if (contractBalance >= tokenLiquidityThreshold) {
            ...
            uint256 devmarketAmt = unitBalance * 2 * swapTaxes.devmarket;
            if (devmarketAmt > 0) {
                payable(devmarketWallet).sendValue(devmarketAmt);
            }
        }
    }
    ...
    function updateDevmarketWallet(address newWallet) external onlyOwner {
        require(newWallet != address(0), "Fee Address cannot be zero address");
        devmarketWallet = newWallet;
    }
