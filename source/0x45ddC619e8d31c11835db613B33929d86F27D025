...
contract RouterV2 is Router,IRouterV2 {
    ...
    function swapExactTokensForTokensSupportingFeeOnTransferTokensMultiple(
        ...
        for(uint256 i=0; i < path.length; i++){
            _swapExactTokensForTokensSupportingFeeOnTransferTokens(amountIn[i],amountOutMin[i],path[i],feePath[i],to);
        }
    }

    function _swapExactNativeForTokensSupportingFeeOnTransferTokens(
        uint256 amountOutMin,
        address[] calldata path,
        uint32[] calldata feePath,
        address to
    ) internal {
        require(path[0] == WNative, "RouterV2: INVALID_PATH");
    ...
    function swapExactNativeForTokensSupportingFeeOnTransferTokens(
        ...
        return _swapExactNativeForTokensSupportingFeeOnTransferTokens(
            amountOutMin,
            path,
            feePath,
            to
    ...
    function swapExactNativeForTokensSupportingFeeOnTransferTokensMultiple(
        ...
        require(path.length == feePath.length, "path and feePath length dont match");
        for(uint256 i=0; i < path.length; i++){
            _swapExactNativeForTokensSupportingFeeOnTransferTokens(amountOutMin[i],path[i],feePath[i],to);
        }
    }
