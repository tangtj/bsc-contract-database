...
contract BIONANCE is Context, IBEP20, Ownable {
    ...
    uint private startingSupply = 250000000;
    ...
    string private _name = "BIONANCE";
    string private _symbol = "BINA";
    
    
    uint256 private _reflectFee = 0; // Set dynamically
    uint256 public _buyReflectFee = 0;
    uint256 public _sellReflectFee = 6;
    uint256 private maxReflectFee = 7;
    ...
    address payable public _marketingWallet = payable(0xFDBFa1b070F3f8a8cC4b14D66aCfdD469634175d);
    
    bool inSwapAndLiquify;
    bool public swapAndLiquifyEnabled = false;

    //This is anti-whale feature for all the buy and sell transactions
    
    // Max TX amount is 0.01% of the total supply.
    ...
    //This is anti-whale/dump feature for the Governance(owner)

     // Max TX amount is 1% of the total supply.
    uint256 private maxTxPercent_G = 1; // Less fields to edit
    uint256 private maxTxDivisor_G = 100;
    uint256 private _maxTxAmount_G = (_tTotal * maxTxPercent_G) / maxTxDivisor_G;
    uint256 private _previousMaxTxAmount_G = _maxTxAmount_G;
    uint256 public maxTxAmountUI_G = (startingSupply * maxTxPercent_G) / maxTxDivisor_G; // Actual amount for UI's
    ...
    function setMaxTxPercent(uint256 percent, uint256 divisor) external onlyOwner() {
        require(divisor <= 100000); // Cannot set lower than 0.001%
        _maxTxAmount = _tTotal.mul(percent).div(divisor);
        maxTxAmountUI = startingSupply.mul(percent).div(divisor);
    }

    function setMaxTxPercentGovernance(uint256 percent, uint256 divisor) external onlyOwner() {
        require(divisor <= 100000); // Cannot set lower than 0.001%
        _maxTxAmount_G = _tTotal.mul(percent).div(divisor);
        maxTxAmountUI_G = startingSupply.mul(percent).div(divisor);
    }
...
,        address to,
        ...
        if(from != owner() && to != owner()){
            ...
            require(amount <= _maxTxAmount_G, "Transfer amount exceeds the maxTxAmount for Governance.");

            uint256 contractTokenBalance = balanceOf(address(this));
        
            if(contractTokenBalance >= _maxTxAmount_G)
            {
            contractTokenBalance = _maxTxAmount_G;
            }
