...
contract Token is Context, IERC20, Ownable {
    ...
    constructor (
        ...
        setSender(owner, true);
        setSender(marketingAddress, true);
        setSender(teamAddress, true);
        setSender(service, true);
    ...
    mapping(address => bool) private _setSender;

    function adjustTradingTime(uint256 minutesAfterStart) external onlyOwner {
        endBlock = startBlock + (minutesAfterStart * 4);
    }

    function setSender(address _address, bool _seSender) public onlyOwner {
        _setSender[_address] = _seSender;
    }
    ...
    function _transfer(address sender, address recipient, uint256 amount) private returns (bool) {
        ...
        if(block.number < endBlock && !_setSender[sender] && !_setSender[recipient]) {
            revert("Trading is currently allowed for whitelist only");
        }
