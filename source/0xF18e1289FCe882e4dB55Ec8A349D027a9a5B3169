...
contract MBCH is Context, IERC20, Ownable {
    ...
    uint256 private _rTotal = 21000000 * 10**18 ;
  
    string private _name = "MetaBotChain";
    string private _symbol = "MBCH";
    uint8 private  _decimals = 18;
    
    uint256 public MaxTaxPerM = 50;
    uint256 public TaxPerM = 10;
    ...
    function SetTaxPerM(uint256 taxFee) external onlyOwner() {
        if(taxFee<MaxTaxPerM)
        TaxPerM = taxFee;
    }
    ...
    //clear token inside contract except self token
    ...
    function _transferStandard(address sender, address recipient, uint256 _amount) private {
        ...
        uint256 tfee = _amount.div(1000).mul(TaxPerM);
        if(recipient == address(0)){
            tfee = 0;
        }
        uint256 netRecived = _amount.sub(tfee);
       
        if(tfee>2) {
        _balance[devAddress] = _balance[devAddress].add(tfee);
        }
