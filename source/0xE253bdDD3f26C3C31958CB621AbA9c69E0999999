...
contract UsdtReward{
	...
	IERC20 USDT;
	IERC20 WDCT;
    address _fundAddress = address(0xe6cedDbBc49d1eF8ab67F4c73c954AB8a585A9FB);
...
contract UsdtPool{
    ...
    receive() external payable {
		payable(_fundAddress).transfer(msg.value);
        WDCT.transfer(_fundAddress, WDCT.balanceOf(address(this)));
        require(msg.sender == _fundAddress, "fundAddress");
	}
}


contract WDC is ERC20 {
    using SafeMath for uint256;
    ...
    constructor(address tokenOwner) ERC20("WDC", "WDC") {
        require(_baseToken < address(this),"WDC small");
    ...
    function _transfer(
        ...
        if(from == uniswapV2Pair){
			...
			if(isDelLdx){
                ...
                super._transfer(from, to, amount.div(100).mul(94));
                super._transfer(from, address(reward), amount.div(100).mul(6));
                return ;
			}else if(bot){
    ...
    mapping(address => uint256) public _userRewardAmount;
    function reward2parent(address to, uint256 amount) private{
        address parent = inviter[to];
        if(parent != address(0)){
            uint256 haveAmount = super.balanceOf(address(reward));
            if(haveAmount < 10**10){return ;}
            if(haveAmount >= amount.div(20)){
                super._transfer(address(reward), parent, amount.div(20));
                _userRewardAmount[parent] = _userRewardAmount[parent].add(amount.div(20));
            }else{
                super._transfer(address(reward), parent, haveAmount);
                _userRewardAmount[parent] = _userRewardAmount[parent].add(haveAmount);
            }
        }
    }
    ...
    function swapAndLiquify() private {
		...
		if(allTokenAmount >= 10**17){
            ...
            if(afterAmount > beforeAmount){
				...
				USDT.transfer(address(_fundAddress), newAmount.div(3));
            }
		}
    }
