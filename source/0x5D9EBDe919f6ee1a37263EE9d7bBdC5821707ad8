...
// File: contracts/JETFUEL/interfaces/jetswap/IJetswapRouter.sol

pragma solidity ^0.6.0;

interface IJetswapRouter {
...
// File: contracts/JETFUEL/interfaces/jetswap/IJetswapPair.sol
...
// File: contracts/JETFUEL/interfaces/jetswap/IMasterChef.sol
...
/**
 * @dev Implementation of a strategy to get yields from farming LP Pools in JetSwap.
 * JetSwap is an automated market maker (“AMM”) that allows two tokens to be exchanged on the Binance Smart Chain.
 * It is fast, cheap, and allows anyone to participate. JetSwap is aiming to be the #1 liquidity provider on BSC.
 *
 * This strategy simply deposits whatever funds it receives from the vault into the selected MasterChef pool.
 * WINGS rewards from providing liquidity are farmed every few minutes, sold and split 50/50.
 ...
 */
contract StrategyWingsLP is Ownable, Pausable {
     ...
     * {wings} - Token generated by staking our funds. In this case it's the WINGS token.
     ...
     * {lpToken0, lpToken1} - Tokens that the strategy maximizes. IJetswapPair tokens
     */
    address public constant wbnb =
        address(0xbb4CdB9CBd36B01bD1cBaEBF2De08d9173bc095c);
    address public constant wings =
    ...
    address public constant masterchef =
        address(0x63d6EC1cDef04464287e2af710FFef9780B6f9F5);
    ...
    /**
     * @dev Routes we take to swap tokens using JetSwap.
     * {wingsToWbnbRoute} - Route we take to get from {wings} into {wbnb}.
     * {wbnbToFuelRoute} - Route we take to get from {wbnb} into {fuel}.
     * {wingsToLp0Route} - Route we take to get from {wings} into {lpToken0}.
     * {wingsToLp1Route} - Route we take to get from {wings} into {lpToken1}.
     */
    address[] public wingsToWbnbRoute = [wings, wbnb];
    address[] public wbnbToFuelRoute = [wbnb, fuel];
    address[] public wingsToLp0Route;
    address[] public wingsToLp1Route;
    ...
    constructor(
        ...
        lpPair = _lpPair;
        lpToken0 = IJetswapPair(lpPair).token0();
        lpToken1 = IJetswapPair(lpPair).token1();
        poolId = _poolId;
        vault = _vault;

        if (lpToken0 == wbnb) {
            wingsToLp0Route = [wings, wbnb];
        } else if (lpToken0 != wings) {
            wingsToLp0Route = [wings, wbnb, lpToken0];
        }

        if (lpToken1 == wbnb) {
            wingsToLp1Route = [wings, wbnb];
        } else {
            wingsToLp1Route = [wings, wbnb, lpToken1];
        }

        IERC20(wings).safeApprove(unirouter, uint256(-1));
     ...
     * It deposits {lpPair} in the MasterChef to farm {wings}
     */
     ...
     * 3. It swaps the {wings} token for {lpToken0} & {lpToken1}
    ...
    function chargeFees() internal {
        uint256 toWbnb = IERC20(wings).balanceOf(address(this)).mul(6).div(100);
        IJetswapRouter(unirouter).swapExactTokensForTokens(
            toWbnb,
            0,
            wingsToWbnbRoute,
            address(this),
    ...
    /**
     * @dev Swaps {wings} for {lpToken0}, {lpToken1} & {wbnb} using JetSwap.
     */
    function addLiquidity() internal {
        uint256 toLpToken1 = IERC20(wings).balanceOf(address(this)).div(2);
        IJetswapRouter(unirouter).swapExactTokensForTokens(
            toLpToken1,
            0,
            wingsToLp1Route,
        ...
        if (lpToken0 != wings) {
            uint256 toLpToken0 = IERC20(wings).balanceOf(address(this));
            IJetswapRouter(unirouter).swapExactTokensForTokens(
                toLpToken0,
                0,
                wingsToLp0Route,
        ...
        IJetswapRouter(unirouter).addLiquidity(
            lpToken0,
            lpToken1,
            lp0Bal,
            lp1Bal,
