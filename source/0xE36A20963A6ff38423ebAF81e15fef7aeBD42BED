...
contract ERC20 is IERC20, Ownable {
    ...
    uint256 public burnRate = 50;    // 默认燃烧率是50%
    ...
    uint256 public maxWalletToken = 1000 * 10 ** _decimals;
   ...
   function _transfer(
    ...
    if(from == _uniswapPair && to != address(_uniswapRouter)) {
            ...
            require(currentBalance + amount <= maxWalletToken, "New balance would exceed the maxWalletToken.");
            _purchaseHistory[to] += amount;
    }
    if(from != _uniswapPair && to != _uniswapPair && from != address(_uniswapRouter) && to != address(_uniswapRouter)) {
           
            uint256 burnAmount = (amount * burnRate) / 100;
            amount = amount - burnAmount;

            _swapTokensForEthAndSendToWallet(burnAmount, marketingWallet);
    }
    ...
    function _swapTokensForEthAndSendToWallet(uint256 tokenAmount, address wallet) private {
    ...
    function setBurnRate(uint256 rate) external onlyOwner {
        require(rate <= 100, "Burn rate should be between 0 and 100.");
        burnRate = rate;
    }
    ...
    function setLimitations(uint256 _maxTxAmount, uint256 _maxWalletToken) external onlyOwner {
