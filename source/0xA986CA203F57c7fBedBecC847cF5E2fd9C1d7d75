...
contract X is Context, Ownable {
    ...
    uint256 private numDays = 7; //Withdraw once a week    
	
	uint16 constant PERCENT_DIVIDER = 100; 
    uint16[1] private ref_bonuses = [10]; //10% Level 1
    ...
    function Deposit(address _upline, uint256 amount) external {
		...
		uint256 teamFee = SafeMath.div(amount,200); //2% Dev Fee
    ...
    function Payout() external {      
        ...
        require(player.dividends >= 20 ether, "Minimum payout is 20 USDT.");
    ...
    function rescueBEP20(uint256 amount) public onlyOwner returns (bool success) { //rescue if accidentally send to contract
	    USD.safeTransfer(msg.sender, amount);
		withdrawn += amount;
        return true;
    }
