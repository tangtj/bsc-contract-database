// File: Lottery-N-Proxy\NFT\LotteryNFT.sol
...
contract LotteryNFT is ERC721, Ownable {
    ...
    constructor() public ERC721("TeamRocket Lottery Ticket", "TNT") {
        initial[msg.sender] = true;
    }

    modifier INIT () {
        require(initial[msg.sender],"not:admin");
        _;
    }
    function grant(address _new) external onlyOwner {
        initial[_new] = true;
    }
    function revoke(address _exist) external onlyOwner {
        initial[_exist] = false;
    }
    ...
    function claimReward(uint256 tokenId) external INIT {
        claimInfo[tokenId] = true;
    }
    function multiClaimReward(uint256[] memory _tokenIds) external INIT {
        for (uint i = 0; i < _tokenIds.length; i++) {
            claimInfo[_tokenIds[i]] = true;
        }
    }
    function burn(uint256 tokenId) external INIT {
        _burn(tokenId);
    }
