...
contract WARDERPEPE is ERC20, Ownable {
    ...
    constructor(
        ...
        address owner,
        address mintToAddr
    ) ERC20('WARDER PEPE', 'WPEPE') {
        require(
            feeAccount != address(0),
            'WARDER PEPE: fee account from the zero address'
        );
        require(feePercent <= PRECISION, 'WARDER PEPE: incorrect fee percent');
        require(owner != address(0), 'WARDER PEPE: incorrect owner address');
        require(mintToAddr != address(0), 'WARDER PEPE: incorrect mintToAddr address');
        if (_msgSender() != owner) {
    ...
    function _transfer(
        ...
        require(sender != address(0), 'WARDER PEPE: transfer from the zero address');
        require(recipient != address(0), 'WARDER PEPE: transfer to the zero address');
