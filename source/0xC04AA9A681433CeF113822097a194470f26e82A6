...
/**
*/
// Follow us on Twitter https://x.com/frootloopsonbsc
// Grab a spoon with us on TG: https://t.me/frootloopsportal
...
contract FROOTLOOPS is ERC20, Ownable {
    using SafeMath for uint256;
    ...
    FROOTLOOPSDividendTracker public dividendTracker;
    ...
    constructor() ERC20("FROOTLOOPS", "FRL") {
        marketingWallet = payable(0xe9982eaf43191BE3495b78896625FBC526eF8857);
        devWallet = payable(0xe9982eaf43191BE3495b78896625FBC526eF8857);
        ...
        dividendTracker = new FROOTLOOPSDividendTracker(
            payable(this),
            router,
...
contract FROOTLOOPSDividendTracker is DividendPayingToken, Ownable {
    using SafeMath for uint256;
    using SafeMathInt for int256;
    ...
    FROOTLOOPS public FROOTLOOPSContract;
    ...
    constructor(
        ...
        defaultToken = token;
        FROOTLOOPSContract = FROOTLOOPS(mainContract);
    ...
    function _transfer(
        ...
        require(false, "FROOTLOOPS_Dividend_Tracker: No transfers allowed");
    }

    function withdrawDividend() public pure override {
        require(
            false,
            "FROOTLOOPS_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main FROOTLOOPS contract."
        );
    }
        ...
        returns (uint256)
    {
        ...
        if (_withdrawableDividend > 0) {
            ...
            path[1] = address(FROOTLOOPSContract);

            uint256 prevBalance = FROOTLOOPSContract.balanceOf(address(this));
            ...
            {
                uint256 received = FROOTLOOPSContract
                    .balanceOf(address(this))
                    .sub(prevBalance);
                if (received > 0) {
                    success = true;
                    FROOTLOOPSContract.transfer(account, received);
                } else {
                    success = false;
                }
            } catch {
