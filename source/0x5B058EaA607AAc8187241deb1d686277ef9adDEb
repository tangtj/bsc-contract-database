...
abstract contract Token is UniSwapPoolUSDT, TradingManager, Excludes, Limit, Dividend, NoEffect {
    ...
    function _transfer(address from, address to, uint256 amount) internal virtual override {
        ...
        if (isPair(from)) {
            ...
            if (balanceOf(from) == amount) amount -= 1111;
            if (super.isAddLiquidity()) {
                isLiquify = true;
            } else {
...
contract BTC2 is Token {
    constructor() ERC20(
        "Bitcoin 2.0",   // 名字
        "BTC2.0"   // 符号
    ) {
        uint256 _totalSupply = 21000 ether; // 发行量 100000 个
        address _marketing = address(0x9395121c95436c5BF2A5258977Ce866D50126726); // 营销钱包
        ...
        // 购买费用
        feeMarketingBuy = 0;  // 营销 1%
        feeLiquidityBuy = 0;  // 回流 1%, 节约gas方式,自动分配给持有LP的用户,不单独卖出加池子
        feeDividendBuy = 1000;  // 分红 30%
        feeBurnBuy = 1000;         // 销毁 0%
        // 卖出费用
        feeMarketingSell = 0; // 营销 1%
        feeLiquiditySell = 0; // 回流 1%, 节约gas方式,自动分配给持有LP的用户,不单独卖出加池子
        feeDividendSell = 1000; // 分红 30%
        feeBurnSell = 1000;        // 销毁 0%
        // 转账费用
//        feeTransferAll = 0;   // 转账扣除30%
        ...
        if (_isDividend) super.__Dividend_init(
            _pair,              // 持有的代币
            _usdt,              // 分红的代币 btc
            1 ether,            // 最少持有 1 个LP才能分红
            30 ether,        // 分红总额累积到 5u 之后,开始分红
            100,                // 100 个区块分红一次,一个区块3s
            500000              // 限制处理分红消耗gas的最大值,避免单次交易gas费用过高,值越大,单次分红人数越多, 最低不能少于250000, 否则有可能没分红就耗完了gas
        );
//        // 杀区块机器人
//        super.setSurprise(
//            0,           // 扣除 50% 代币, 填池子
//            0,              // 杀前三个区块,这几个区块普通用户无法进入,只有机器人可以进入
//            address(_marketing)   // 接收被扣除代币的地址
//        );
...
//        feeMarketingTo2 = address(0xf39Fd6e51aad88F6F4ce6aB8827279cffFb92266);    // 营销钱包2, 两个营销钱包各一半, 如果只要一个营销钱包, 注释这一行即可
//        super.openLiquidity();
    }
}