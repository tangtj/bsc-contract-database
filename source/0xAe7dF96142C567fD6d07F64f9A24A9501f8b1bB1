...
contract CECCBUYNFT is Ownable {
	...
	function Deposit(address address_A, address address_B, uint256 amount_A, uint256 amount_B, uint256 type_id) lock public payable {
		...
		amount_B= UamountOutMin;
		
		
        buy_lists[msg.sender]=block.timestamp;
        ERC20(cecc_addr).transferFrom(msg.sender, address(this), amount_A);
		ERC20(USDT_ADDRESS).transferFrom(msg.sender, address(this), amount_B);
		...
		ERC20(USDT_ADDRESS).transfer(makerAdr_1, amount_B);
