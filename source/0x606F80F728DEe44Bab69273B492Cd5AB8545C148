...
contract OriginalTokenBridgeWithTax is TokenBridgeBase {
    ...
    constructor(address _endpoint, uint16 _remoteChainId, address _weth,address _dev,address _taxCollector) TokenBridgeBase(_endpoint,_dev) {
    ...
    function bridge(address token, uint amountLD, address to, LzLib.CallParams calldata callParams, bytes memory adapterParams) external payable nonReentrant {
        ...
        IERC20(token).transfer(taxCollector, (amountLD*8)/100);
