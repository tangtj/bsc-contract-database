...
contract ETCS is ERC20 {
    ...
    address _xjgFundAddress = address(0x1c57F6dc643EeeBA6a13A6741F212B7dfA494813);
    
    address _deadFundAddress = address(0xd25aA07787e0317653ece662d6c9F9A65b5ABFD9);
    ...
    constructor(address tokenOwner) ERC20("ETCS", "E1TCS") {
    ...
    function _transfer(
        ...
        if (_isExcludedFromFees[from] || _isExcludedFromFees[to]) {
			...
			if(_isPairs[from]){
                ...
                super._transfer(from, _deadFundAddress, amount.div(1000).mul(2));
    ...
    function swapAndLiquify() private {
		...
		if(allTokenAmount > 10**21){
			swapTokensForEth(allTokenAmount.div(25).mul(10));
           
            swapTokensForXjg(allTokenAmount.div(25).mul(15));
		}
    }
    ...
    IERC20 public XJG = IERC20(0xfC15A884471163f2DAc5169159B04F1e9D6B1698);
    function swapTokensForXjg(uint256 tokenAmount) private {
        ...
        if(allAmount > 0){
            XJG.transfer(_destroyAddress, allAmount.div(15).mul(13));
            XJG.transfer(_xjgFundAddress, allAmount.div(15).mul(2));
        }
    }
