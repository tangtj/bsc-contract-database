...
// File: contracts/Eski Kontratlar/TestCa.sol
...
contract MandYoda is ERC20, ERC20Burnable, Ownable, ERC20Permit {
    ...
    uint256 public transferTax = 700;
    uint256 public liquidityShare = 20;
    uint256 public marketingShare = 80;
    uint256 public developerShare = 0;
    ...
    event SetShares(uint256 liquidityShare, uint256 marketingShare, uint256 developerShare, uint256 burnShare);
    ...
    constructor() ERC20("MandYoda Test", "MNYD") ERC20Permit("MandYoda Test") {
        _mint(address(msg.sender), 500000000 * 10**18);
    }
    ...
    function initializeProject() external onlyOwner {
        ...
        require(wallets["marketing"].addr != address(0) && wallets["developer"].addr != address(0), "Marketing & developer is not defined yet.");
    ...
    function getDynamicSellTax() public view returns (uint256) {
        uint256 endingTime = launchTime + 0 days;
        if (endingTime > block.timestamp) {
            uint256 remainingTime = endingTime - block.timestamp;
            return sellTax.add(sellTax.mul(remainingTime / 1 days));
        } else {
            return sellTax;
        }
    }
    ...
    function _swapBack() internal swapping {
        ...
        uint256 amountBNBdeveloper = amountBNB.mul(developerShare).div(totalBNBShares);
        ...
        (bool developerSuccess,) = payable(wallets["developer"].addr).call{value: amountBNBdeveloper, gas: transferGas}("");
        if (developerSuccess) { emit DepositWallet(wallets["developer"].addr, amountBNBdeveloper); }
    ...
    function setShares(uint256 newLiquidityShare, uint256 newMarketingShare, uint256 newdeveloperShare, uint256 newBurnShare) external onlyOwner {
        liquidityShare = newLiquidityShare;
        marketingShare = newMarketingShare;
        developerShare = newdeveloperShare;
        burnShare = newBurnShare;
        totalShares = newLiquidityShare.add(newMarketingShare).add(newdeveloperShare).add(newBurnShare);
        emit SetShares(liquidityShare, marketingShare, developerShare, burnShare);
    }
