...
contract GameToken is Ownable, ERC20 {
    using SafeMath for uint256;
    
    uint256 constant public maxSupply = 50 * 10**6 * 10**18;
    ...
    constructor(address _taxAddress, address _router) ERC20("DracoVerse", "DRACO") {
    ...
    function setFeeRate(uint256 _sellRate, uint256 _buyRate) external onlyOwner {
        require(_sellRate < 8, "over sell");
        require(_buyRate < 5, "over buy");
