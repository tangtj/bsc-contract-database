...
Telegram - @SmileSatoshiNoticeBoard and @SmileSatoshiGlobal                                                                 
...
While Satoshi did release the cat among the penned hens, kind of disrupting the concept of money and their so
called guardians. We the crypto investors are fueling that fire. Uncertainties, diappointments, and euphoria,
all rule this space. And as any space we have a fair share of good and bad actors, its upto us to perhaps be
more aware, do appropriate research and careful to not get too carried away while investing and stay safe, help
each other to make the space more inclusive and attractive. Here is my attempt to do something about that.

I remember about when I started investing in crypto feels like yesterday, with no clue how to ride these waters.
Boy what a ride it has been, from loosing everything to rugpulls to making unimaginable gains and then loosing
that again. But I love this space even more, its like that song Hotel California “You can checkout anytime you
like, but you can never leave”

Introducing Smile Satoshi protocol.

Would like to invite you to embark on a journey with me Joy in my attempt to spread some decentralised happiness,
While if you are reading this it means the stage 1 has been activated our coin $JOY is already live. We will have
a charity component to the project as we grow where we want to contribute to TeamTrees / TeamSeas / EveryMotherCounts
and SaveTheChildren. More hands on ground work will happen as we grow.

I understand the power of the ETH network, Rayoshi keeps choosing that, but times have changed, for gas fee and
reaching more people, choosing BNB chain was a no brainer, keeping it more tradable and affordable for more people.

Launch liquidity = 0.05 BNB
Total Supply = 100 Billion Tokens
Max Wallet/Buy/Sell = 500 Million Tokens
Taxes will vary between 0 - 15 percent

Initial Security:-
1) Contract won't be renounced, so to give some security
    a) We have hard coded in the contract the buy/sell taxes can never go beyond 15%.
    b) We have hard coded that max sell can never be decreased beyond 100 Million Tokens.
2) LP is locked.
3) No Team Tokens.

House Rules:- 
1) Respect each other, and keep the content family friendly.
2) If you are a marketer/caller, buy a bag and call us.
3) If you are an investor, spread the word, shill, volunteer.

Target 1 - Building a strong community. 

Let's do this, build it brick by brick.
...
contract SmileSatoshi is Context, IERC20, IERC20Extented, Ownable {
    using SafeMath for uint256;
    string private constant _name = "Smile Satoshi";
    ...
    uint256 private constant _tTotal = 1 * 10**11 * 10**9; // 100 Billion
    uint256 private _firstBlock;
    uint256 private _botBlocks;
    uint256 public _maxWalletAmount;
    uint256 private _maxSell;
    uint256 private _maxBuy;
    uint256 public numTokensToSwap = 2 * 10**8 * 10**9; // 0.2% = 200 Million

    // buy fees
    uint256 public _buyLiquidityFee = 80; // divided by 1000
    ...
    uint256 public _buyTeamFee = 20; // divided by 1000
    uint256 private _previousBuyTeamFee = _buyTeamFee;

    // sell fees
    uint256 public _sellLiquidityFee = 80; //50; // divided by 1000
    uint256 private _previousSellLiquidityFee = _sellLiquidityFee;
    uint256 public _sellEcosystemFee = 0; //15; // divided by 1000
    uint256 private _previousSellEcosystemFee = _sellEcosystemFee;
    uint256 public _sellMarketingFee = 50; //25; // divided by 1000
    uint256 private _previousSellMarketingFee = _sellMarketingFee;
    uint256 public _sellTeamFee = 20; // divided by 1000
    ...
    address payable private _marketingAddress = payable(0xDe0113A010F520AabB4143b765A9f372aE5287Fc);
    address payable private _teamAddress = payable(0x211E346Cd9f972B83AF9c2d314fBb9afF1Edbd45);
    address payable private _ecosystemAddress = payable(0x6c068b2A0beF72Bd46195B74D5988687C88823eF);
    ...
    constructor() {
        ...
        _maxTxAmount = 5 * 10**8 * 10**9; // 0.5% = 500 Million
        _maxWalletAmount = 5 * 10**8 * 10**9; // 0.5% = 500 Million
        _maxBuy = 5 * 10**8 * 10**9; // 0.5% = 500 Million
        _maxSell = 5 * 10**8 * 10**9; // 0.5% = 500 Million
    ...
    function setMaxWalletAmount(uint256 maxWalletAmount) external onlyOwner() {
        require(maxWalletAmount >= _tTotal.div(1000), "Amount must be greater than or equal to 0.1% of supply");
    ...
    function setBuyTaxes(uint256 marketingFee, uint256 liquidityFee, uint256 teamFee, uint256 ecosystemFee) external onlyOwner() {
        ...
        require(totalFee <= 150, "Total Buy Tax can never be more than 15%");
    ...
    function setSellTaxes(uint256 marketingFee, uint256 liquidityFee, uint256 teamFee, uint256 ecosystemFee) external onlyOwner() {
        uint256 totalFee = marketingFee.add(liquidityFee).add(teamFee).add(ecosystemFee);
        require(totalFee <= 150, "Total Sell Tax can never be more than 15%");
    ...
    function updateMaxSell(uint256 maxSell) external onlyOwner() {
        require(maxSell >= _tTotal.mul(1).div(1000) , "cant make the limit lower than 0.1% of the supply");
        _maxSell = maxSell;
    }
    
    function updateMaxBuy(uint256 maxBuy) external onlyOwner() {
        require(maxBuy >= _tTotal.mul(1).div(1000) , "cant make the limit lower than 0.1% of the supply");
        _maxBuy = maxBuy;
    }
