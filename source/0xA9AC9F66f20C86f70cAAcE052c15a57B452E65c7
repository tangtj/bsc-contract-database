...
contract StableFinance is ERC20, Ownable {

    // Events in this contract
    event TransferFee(uint sellFee);
    event ProjectWalletUpdated(address newWallet, address oldWallet);

	mapping(address => bool) public blacklists;


    // Exclude From fee and limits
    ...
    constructor (address _projectAddress) ERC20 ("Stable Finance", "SWF"){
        
        swapRouter = 0x10ED43C718714eb63d5aA57B78B54704E256024E;
        // Project Address
        projectAddress = _projectAddress;
        // 3 percentage for Project
        projectFee = 3;
        maxAmountForWallet = 50000000 * 10 ** decimals();
        maxAmountForTx = 30000000 * 10 ** decimals();

        address swapFactory = IUniswapV2Router02(swapRouter).factory();
		...
		isExcludeFromFee[projectAddress] = true;

        isExcludeFromTxLimit[address(this)] = true;
        isExcludeFromTxLimit[_msgSender()] = true;
        isExcludeFromTxLimit[projectAddress] = true;
        isExcludeFromTxLimit[swapRouter] = true;

        isExcludeFromWalletLimit[address(this)] = true;
        isExcludeFromWalletLimit[_msgSender()] = true;
        isExcludeFromWalletLimit[projectAddress] = true;
        isExcludeFromWalletLimit[swapRouter] = true;
        isExcludeFromWalletLimit[swapPair] = true;

        super._mint(msg.sender, 124000000 * 10 ** decimals());
    ...
    function updateProjectWallet(address newWallet) external onlyOwner {
		address oldWallet = projectAddress;
		projectAddress = newWallet;

		emit ProjectWalletUpdated(newWallet, oldWallet);
	}

	function updateMaxWalletAmount(uint256 maxWallet) external onlyOwner {
		require(maxWallet <= 124000000 ether && maxWallet >= 1, "Enter Correct Value");
		maxAmountForWallet = maxWallet;
	}

	function updateMaxTransactionAmount(uint256 maxTx) external onlyOwner {
		require(maxTx <= 124000000 ether && maxTx >= 1, "Enter Correct Value");
		maxAmountForTx = maxTx;
	}
    ...
    function _swapWithSellFee() internal onlySwapping {
		uint256 totalFees = tokensForProject;
        ...
        if (ethBalance > 0) {
			payable(projectAddress).transfer(ethBalance);
		}

        tokensForProject = 0;
        emit TransferFee(totalFees);
    }

    function _transfer(address from, address to, uint256 amount) internal override {
		require(!blacklists[to] && !blacklists[from], "Blacklisted");
		if (!isExcludeFromTxLimit[from] && !isExcludeFromTxLimit[to])
			require(maxAmountForTx >= amount, "Greater than max amount for Trx");
		if (!isExcludeFromWalletLimit[to])
			require((balanceOf(to) + amount) <= maxAmountForWallet, "Greater than max amount for Wallet");
        ...
        if (
			...
			uint256 uSellFee = projectFee;
            if (to == swapPair && uSellFee > 0) {
				fees = (amount * uSellFee) / 100;
