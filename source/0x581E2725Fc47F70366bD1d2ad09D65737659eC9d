...
contract RandomWinner {
    ...
    modifier gameActive() {
        require(_isGameActive, "Game is not active");
        _;
    }
    ...
    receive() external payable gameActive noReentrancy {
    ...
    function withdraw() external {
        require(!_isGameActive, "Withdrawals are not allowed while the game is active");
    ...
    function clearContract() external onlyOwner {
        require(!_isGameActive, "Clearing contract is not allowed while the game is active");
        payable(owner).transfer(address(this).balance);
    }

    function pickWinner() internal gameActive noReentrancy {
