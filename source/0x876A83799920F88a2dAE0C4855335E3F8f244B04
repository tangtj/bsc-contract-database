...
contract TRC721MetadataMintable is TRC721, TRC721Metadata, MinterRole {
    ...
    function mintAdminWithTokenURI(address user) public onlyMinter returns (bool) {
        ...
        _mintAdminWithTokenURI(user, nftIndexId);
        return true;
    }

    function _mintAdminWithTokenURI(address user, uint256 nft_id) public onlyMinter returns (bool) {
        _mint(user, nft_id);
        _setTokenURI(nft_id, "https://bafkreiegxvcguhjgdo45xmvysto6odkj5hxazjb6pl3f5wblcpzz3ilf6u.ipfs.nftstorage.link");
        return true;
    }

    function mintAdminWithTokenURI2users(address[] memory accounts) public onlyMinter {
        for (uint256 i = 0; i < accounts.length; i++) {
            nftIndexId = nftIndexId+1;
            _mintAdminWithTokenURI(accounts[i], nftIndexId);
        }
    }
}
...
contract DCDNFT is TRC721, TRC721Enumerable, TRC721MetadataMintable {
    constructor() public TRC721Metadata("CDAO NFT", "CDAO NFT") {
