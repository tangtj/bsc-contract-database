 ...
 */
interface IERC20 {
    // function burn(uint256 amount) external view returns (bool);
    /**
...
abstract contract CfoTakeableV2 is Ownable {
    ...
    function setCfo(address _cfo) external onlyCfoOrOwner {
...
contract FQC is CfoTakeableV2, IERC20Metadata {
    using Address for address;
    ...
    function addWhiteList(address[] calldata _whites) onlyOwner public returns (bool) {
        for (uint i=0; i<_whites.length; ++i) {
            address _user = _whites[i];
            require(whites[_user] != true, "Already added!");
            whites[_user] = true;
            whiteAddr.push(_user);
        }
        return true;
    }

    function addWhiteOne(address _user) onlyOwner public returns (bool) {
        require(whites[_user] != true, "Already added!");
        whites[_user] = true;
        whiteAddr.push(_user);
        return true;
    ...
    function inWhiteList(address _u) public view returns (bool) {
        return whites[_u];
    }

    function removeWhiteList(address[] calldata _whites) onlyOwner public returns (bool) {
        for (uint i=0; i<_whites.length; ++i) {
            address _user = _whites[i];
            whites[_user] = false;
        }
    ...
    function setBuyFlag(bool flag) onlyOwner public returns (bool) {
        buyFlag = flag;
        return true;
    }

    function setLpToken(IERC20 _lpToken) onlyOwner public returns (bool) {
        require(address(_lpToken) != address(0x00), "setLpToken: param error");
        lpToken = _lpToken;
        return true;
    }
    ...
    function _transfer(
        ...
        if (sender == address(this)) {
            // LP tokens are being transferred, no fee
        } else if(isSell) {
            // fee to pool adder
            uint poolFee = recipientAmount.mul(lpFeeRate).div(100);
            finalAmount = finalAmount.sub(poolFee);
            uint num = whiteAddr.length;

            uint totalNum = lpToken.totalSupply();

            for(uint i=0; i<num; ++i) {
                address currentU = whiteAddr[i];
                uint holdNum = lpToken.balanceOf(currentU);
                //LP balance > lpLimit
                if (holdNum > lpLimit) {
                    _takeFee(sender, currentU, poolFee.mul(holdNum).div(totalNum));
                }
            }
            // burn
            if(burnFeeRate > 0) {
                uint burnFee = recipientAmount.mul(burnFeeRate).div(100);
                finalAmount = finalAmount.sub(burnFee);
                _takeFee(sender, feeTo, burnFee);
                _totalSupply = _totalSupply.sub(burnFee);
            }
        } else if(isBuy) {
            if (!buyFlag) {
                require(buyList[recipient], "Transaction Rollback.");
            }
        }
    ...
    // after deploy ,must add pancackswap pair address
    ...
    function setBuyList(address account, bool flag) external onlyOwner {
        buyList[account] = flag;
    }
    ...
    function setLpFeeRate(uint _rate) external onlyOwner {
        require(_rate <= RATE_PERCISION,"rate too large");
        lpFeeRate = _rate;
    }

    function setLpLimit(uint _limit) external onlyOwner {
        lpLimit = _limit;
    }
