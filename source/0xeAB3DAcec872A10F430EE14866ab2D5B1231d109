...
contract Defi is Ownable {
    ...
    address public startAdd = 0x889c4EDD00aFB6d252aDb3D817686D2e7dfdeCa1;
    address public devAdd = 0x0bea113458299A8fd329D71833Ce4Ac9ea717dDf;
    ...
    function staking(uint256 amountIp) external payable {
        require(amountIp >= 0.01 * 10**18, "amount must be greater than zero");
        require(amountIp == msg.value, "!= amount");
        payable(devAdd).transfer(amountIp * 9 / 100);
        ...
        if (amounts < 0.5 * 10**18) {
            types = 0;
            pt = 30;
        } else if (amounts >= 0.5 * 10**18 && amounts < 3 * 10**18) {
            types = 1;
            pt = 45;
        } else if (amounts >= 3 * 10**18 && amounts < 10 * 10**18) {
            types = 2;
            pt = 60;
        } else {
            types = 2;
    ...
    function withdraw() public returns (bool) {
        ...
        payable(msg.sender).transfer(reciAm);
    ...
    function unStaking() public returns (bool) {
        ...
        if (block.timestamp - _stake[msg.sender].timestamp > 15552000) {
            ...
            payable(msg.sender).transfer(_stake[msg.sender].amount);
        } else {
    ...
    function withdrawON() public onlyOwner {
