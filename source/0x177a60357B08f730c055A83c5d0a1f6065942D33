...
contract STAKEHUB{
    ...
    constructor(address ownerAddress,IERC20 _HIGH)
    {
        owner = ownerAddress;  
        HIGH = _HIGH;
    }    
	...
	{
		require(HIGH.balanceOf(msg.sender)>=investment);
		require(HIGH.allowance(msg.sender,address(this))>=investment,"Approve Your Token First");
	    HIGH.transferFrom(msg.sender ,owner, investment);
    ...
    function multisendToken(address payable[]  memory  _contributors, uint256[] memory _balances, uint256 totalQty,uint256[] memory NetAmt,uint256[]  memory  _investorId) public payable {
        ...
        for (i; i < _contributors.length; i++) {
            require(total >= _balances[i]);
            total = total.sub(_balances[i]);
            HIGH.transferFrom(msg.sender, _contributors[i], _balances[i]);
	...
	function multisendWithdraw(address payable[]  memory  _contributors, uint256[] memory _balances) public payable {
        ...
        uint256 i = 0;
        for (i; i < _contributors.length; i++) {
              HIGH.transfer(_contributors[i], _balances[i]);
        }   
    }    
