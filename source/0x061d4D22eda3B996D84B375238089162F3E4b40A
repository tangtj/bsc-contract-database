...
contract StratManager is Ownable, Pausable {
    ...
    address public vgatorFeeRecipient;
     ...
     * @param _vgatorFeeRecipient address where to send VGator's fees.
     */
    constructor(
        address _keeper,
        address _strategist,
        address _unirouter,
        address _vault,
        address _vgatorFeeRecipient
    ) public {
        keeper = _keeper;
        strategist = _strategist;
        unirouter = _unirouter;
        vault = _vault;
        vgatorFeeRecipient = _vgatorFeeRecipient;
    }
    ...
    /**
     * @dev Updates vgator fee recipient.
     * @param _vgatorFeeRecipient new vgator fee recipient address.
     */
    function setVGatorFeeRecipient(address _vgatorFeeRecipient) external onlyOwner {
        vgatorFeeRecipient = _vgatorFeeRecipient;
    }
...
abstract contract FeeManager is StratManager {
    ...
    uint public vgatorFee = MAX_FEE - STRATEGIST_FEE - callFee;

    function setCallFee(uint256 _fee) external onlyManager {
        require(_fee <= MAX_CALL_FEE, "!cap");
        
        callFee = _fee;
        vgatorFee = MAX_FEE - STRATEGIST_FEE - callFee;
    }
}

// File: contracts/BIFI/strategies/Auto/StrategyAuto4Belt.sol
...
contract StrategyAuto4Belt is StratManager, FeeManager, GasThrottler {
    ...
    constructor(
        ...
        address _strategist,
        address _vgatorFeeRecipient
    ) StratManager(_keeper, _strategist, _unirouter, _vault, _vgatorFeeRecipient) public {
        _giveAllowances();
    }
    ...
    function chargeFees() internal {
        ...
        uint256 vgatorFeeAmount = wbnbBal.mul(vgatorFee).div(MAX_FEE);
        IERC20(wbnb).safeTransfer(vgatorFeeRecipient, vgatorFeeAmount);
