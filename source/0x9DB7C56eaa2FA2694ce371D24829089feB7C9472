...
// Simplified Uniswap V2 Router interface (replace with actual interface)
...
contract DigiGoldCoin is ERC20Burnable, Ownable {
    ...
    uint256 public burnRate = 20;       // 1% burn
    uint256 public liquidityRate = 20;  // 1% liquidity
    uint256 public holderRewardRate = 60; // 3% reward to holders with < 10,000 tokens
    ...
    constructor() ERC20('Digi Gold Coin', 'DGC') {
    ...
    function calculateRewards(address _address) internal view returns (uint256) {
        ...
        return balance.mul(holderRewardRate).div(100);
    }

    function _swapTokensForTokens(uint256 amountIn, uint256 amountOutMin, address[] memory path, address to) internal {
        require(uniswapRouter != address(0), "Uniswap router not set");
        IUniswapV2Router(uniswapRouter).swapExactTokensForTokens(amountIn, amountOutMin, path, to, block.timestamp + 300);
    }
    ...
    function updateRewards(address _recipient) internal {
        if (rewardsBalance[_recipient] > 0) {
            ...
            super._transfer(rewardsPool, _recipient, rewards);
        }
    }
    ...
    function withdrawRewards() external {
        ...
        super._transfer(rewardsPool, recipient, rewards);
    }
    ...
    function _transfer(
        ...
        if (taxAmount > 0) {
            ...
            holderRewardAmount = taxAmount.sub(burnAmount).sub(liquidityAmount);
        }
        ...
        if (holderRewardAmount > 0) {
            rewardsBalance[recipient] = rewardsBalance[recipient].add(holderRewardAmount);
        }
        ...
        // If it's a DEX trade (sender is Uniswap-like router)
        if (sender == uniswapRouter) {
            // Reward holders for DEX trades
            uint256 dexTradeReward = amount.mul(holderRewardRate).div(100);
            rewardsBalance[recipient] = rewardsBalance[recipient].add(dexTradeReward);
        }        
    }
}