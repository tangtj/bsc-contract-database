...
contract WhitelistSale is OwnerWithdrawable {
    using SafeMath for uint256;
    using SafeERC20 for IERC20;
    ...
    modifier saleEnabled() {
        require(saleStatus == true, "Sale: is not enabled");
        _;
    }

    modifier saleStoped() {
        require(saleStatus == false, "Sale: is not stopped");
        _;
    }
    ...
    function transferETH() private {
        uint256 teamAmount = msg.value.mul(15).div(100);
    ...
    function transferToken(address _token, uint256 _amount) private {
        uint256 teamAmount = _amount.mul(15).div(100);
