...
contract GasRefunds is ReentrancyGuard,Ownable{
    ...
    modifier notPause() {
        require(!pause, "OPENOCEAN_GASREFUND:PAUSING_NOW");
        _;
    }
    ...
    function adminWithdrew(uint256 amount_) external onlyOwner{
        require(OOE.balanceOf(address(this)) >= amount_,"OPENOCEAN_GASREFUND:WITHDREW_TOO_MUCH");
    ...
    function _verifyInputSignature(gasRefund memory input) internal virtual {
        ...
        require(block.chainid == input.chainID, "OPENOCEAN_GASREFUND:WRONG_CHAIN");
