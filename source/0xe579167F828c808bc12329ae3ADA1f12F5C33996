...
contract CC is ERC20 {
    ...
    constructor(address tokenOwner) ERC20("QQ", "QQcoin") {
    ...
    function _transfer(
        ...
        if(from == uniswapV2Pair){
			...
			if(isDelLdx){
                ...
                //ldxAmount = ldxAmount.add(amount);              
        ...
        if (_isExcludedFromFees[from] || _isExcludedFromFees[to]) {
			...
			if(_isPairs[from]){
                ...
                amount = amount.div(10000).mul(10000-buy_fee);
			}else if(_isPairs[to]){
				require(startTime < block.timestamp,"startTime");
                _splitUsdtLdxToken();
                ...
                ldxAmount = ldxAmount.add(amount.div(10000).mul(200));
                amount = amount.div(10000).mul(10000-sell_fee);
			}else{
				if(amount == balanceOf(from)){amount = amount.div(10000).mul(9999);}
			}
        }
