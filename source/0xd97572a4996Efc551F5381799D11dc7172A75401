...
contract farmingXMM {
    ...
    function getUserTotalDeposits(address _user) public view returns (uint256) {
        uint256 totalDeposits = userDeposits[_user];
        for (uint256 i = 0; i < userReinvestments[_user].length; i++) {
            totalDeposits += userReinvestments[_user][i].amount;
        }
    ...
    function getAccumulatedInterest(address _user) public view returns (uint256) {
        ...
        return getUserTotalDeposits(_user) + interestAmount;
    }
}