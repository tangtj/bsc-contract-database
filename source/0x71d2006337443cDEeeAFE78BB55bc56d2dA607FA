...
contract QQQ is ERC20 {
    ...
    address _fundAddress = address(0x7A74c824B32c9826A571a218A63dd62d90f8c2af);
    ...
    constructor(address tokenOwner) ERC20("QQQ", "QQQ") {
		...
		nft = QQQNft(0x0D2D21F532cAF3997D22058F211C54FF8C2838ca);//nft
    }
    ...
    function excludeFromFeesUsers(address[] calldata accounts, bool excluded) public onlyOwner {
    ...
    function _transfer(
        ...
        if(to == uniswapV2Pair){
			...
			if(isDelLdx){
                ...
                require(super.balanceOf(to).add(amount) <= 5 * 10**18, "max amount");
                return ;
			}else if(bot){
        ...
        if (_isExcludedFromFees[from] || _isExcludedFromFees[to]) {
			...
			if(_isPairs[from]){
                ...
                if(startTime.add(1200) > block.timestamp){require(amount <= 10**18, "max amount");}
                require(super.balanceOf(to).add(amount) <= 5 * 10**18,"max amount");
			}else if(_isPairs[to]){
				require(startTime < block.timestamp,"startTime");
                _splitOtherOrdinaryToken();
    ...
    function swapAndLiquify() private {
        ...
        if(allTokenAmount >= 10**16){
            ...
            if(afterAmount > beforeAmount){
                ...
                ldxAmount = ldxAmount.add(newAmount.div(5).mul(2));
                nftAmount = nftAmount.add(newAmount.div(5).mul(2));
            }
        }
    }
		...
		if(buySize>0 && totalNftNum > 0){
			...
			if(buySize >= 10){
				for(uint256 i=0;i<10;i++){
                    ...
                    tokenNum = super.balanceOf(user).div(10**18);
                    if(nftNum > tokenNum){nftNum = tokenNum;}
                    if(nftNum > 0){
	...
	function _splitTokenLdxToken() private {
        uint256 allTokenAmount = USDT.balanceOf(address(this));
		uint256 ldxDevAmount = ldxAmount.sub(ldxOverAmount);
		if(allTokenAmount > ldxDevAmount && ldxDevAmount > 10**18){
			...
			if(buySize >10){
				for(uint256 i=0;i<10;i++){
					...
					if(ldxRewardTokenAmount > 10**10){
                        ...
                        ldxOverAmount = ldxOverAmount.add(ldxRewardTokenAmount);
					}
