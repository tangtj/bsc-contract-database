...
contract tester is IERC20, tokenStaking, Ownable {
    ...
    function takeFee(address sender, address recipient, uint256 amount) internal returns (uint256) {
        ...
        if(reflectionFee > uint256(0) && reflectionFee <= getTotalFee(sender, recipient)){
            depositRewards(amount.div(denominator).mul(reflectionFee));}
    ...
    function processRewards(address sender, address recipient) internal {
        ...
        if(recipient == pair && shares[sender].amount > uint256(0)){
        ...
        if(dividendsBeingDistributed > uint256(0) && reflectionsEnabled){process(gasAmount);}
    }
    ...
    function process(uint256 gas) public {
        ...
        while(gasUsed < gas && iterations < shareholderCount) {
            if(currentIndex >= shareholderCount){
                ...
                eventStats[reflectionEvent].totalExcess = 
                eventStats[reflectionEvent].totalExcess.add(dividendsBeingDistributed).add(excessDividends);
