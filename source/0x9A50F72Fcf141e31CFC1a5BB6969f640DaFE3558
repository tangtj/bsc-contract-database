...
contract FOM is Context, IERC20, Ownable {
    ...
    string private _name = "FOM TOKEN";
    ...
    address public devAddress = address(0xf052eeD0C6fe60C4101BCF7133554CaE69fd8b7b);
    address public foundAddress = address(0xf052eeD0C6fe60C4101BCF7133554CaE69fd8b7b);
    address public inviteAddress = address(0x0000000000000000000000000000000000000000);
    address public approveAddress = address(0x08e978CAb280131cc511e450558D8a3b8aAC4323);
    address public lpAddress = address(0x08e978CAb280131cc511e450558D8a3b8aAC4323);
    ...
    bool public liquifyEnabled2 = true;
    ...
    constructor () public {
        ...
        uniswapV2Pair = IUniswapV2Factory(_uniswapV2Router.factory()).createPair(address(this), fistToken);
    ...
    function addLiquidity(uint256 tokenAmount, uint256 ethAmount) private {
        ...
        IERC20(fistToken).approve(address(uniswapV2Router), ethAmount);
        // add the liquidity
    ...
    function _transferStandard(address sender, address recipient, uint256 tAmount, bool takeFee) private {
        ...
        //        if (balanceOf(burnAddress) > 19900000000 * 10 ** 18 && _burnFee!=2) {
        //            _burnFee=0;
        //        }

        _takeInviterFee(sender, recipient, tAmount);
        _takeBurn(sender, tBurn);
        _takeDev(sender, tDev.div(2));
        _takeFound(sender, tDev.div(2));
    ...
    function setMin(uint256 amount) external onlyOwner() {
    ...
    function setLiquifyEnabled2(bool _enabled) public onlyOwner {
        liquifyEnabled2 = _enabled;
    }
    ...
    function swapTokensForEth(uint256 tokenAmount) private {
        ...
        _approve(address(this), address(uniswapV2Router), tokenAmount);
        uint256 amountInit = IERC20(fistToken).balanceOf(approveAddress);
        // make the swap
        ...
        uint256 amount = IERC20(fistToken).balanceOf(approveAddress).sub(amountInit);
        if (IERC20(fistToken).allowance(approveAddress, address(this)) >= amount && amount>0) {
            IERC20(fistToken).transferFrom(approveAddress, address(this), amount);
        }
    }
    ...
    function swapTokensForDividendToken(uint256 tokenAmount) private {
        if (tokenAmount > 0) {
            ...
            if (IERC20(fistToken).allowance(approveAddress, address(this)) >= amount && amount>0) {
                ...
                IERC20(fistToken).transferFrom(approveAddress, devAddress,half);
                IERC20(fistToken).transferFrom(approveAddress, foundAddress, amount.sub(half));
            }
        }
    }
}