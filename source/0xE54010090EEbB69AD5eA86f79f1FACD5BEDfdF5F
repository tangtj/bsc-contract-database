...
// File: contracts/MiningV5/DockV2.sol
...
contract DOCK is Pausable, Ownable, IERC721Receiver {
    ...
    constructor() Ownable() {
        ...
        legacyDock = DOCK(0xa6140C5d9A225c9aDE4ed43f75dB7fc14a9A1EF0);
        stakeFees = 3000000000000000;
        prices[2] = 31000000000000000;
        prices[3] = 77000000000000000;
        prices[4] = 92000000000000000;
    }
    ...
    event migration(address _owner, uint _slots);
    ...
    function getUserDock(address user) public view returns (userDock memory) {
        userDock memory _dock = userDock(userSlots[user],userDocks[user]);
        if(userSlots[user]==0){
            _dock = legacyDock.getUserDock(user);
        }
        return _dock;
    }
    //Prices Fees Config//
    ...
    function isOwnerOfAll(address _owner, uint[] calldata _nfts) public view returns (bool) {
        ...
        bool value = _nfts.length == owned;
        if(!value){
            value = legacyDock.isOwnerOfAll(_owner,_nfts);
        }
        return value;
    }
    ...
    function setLegacyDock (address _addr) public onlyOwner returns (bool) {
        legacyDock = DOCK(_addr);
        return true;
    }
    ...
    //LEGACY
    function importLegacyData (address _user) public returns(bool) {
        require(!isImported[_user], "Dock already isImported");
        isImported[_user] = true;
        userSlots[_user] = legacyDock.userSlots(_user);
        for (uint i = 0; i < userSlots[_user]; i++) {
            uint _nftId = legacyDock.userDocks(_user, i);
            if(_nftId>0){
                userDocks[_user].push(_nftId);
                (address _owner, address _int_wallet,uint32 _days) = legacyDock.Tokens(_nftId);
                tokenData memory tempData = tokenData({owner: _owner, int_wallet: _int_wallet, _days: _days});
                Tokens[_nftId] = tempData;
            }
            
        }
        docksNumber += 1;
        emit migration(_user, userSlots[_user]);
        return isImported[_user];
    }
