/**
FVOL - The largest photovoltaic project in Europe in Murcia

FVOL is the pillar project that will keep the largest desalination plant in the world located in Murcia,
Spain, operational. The desalination plant is minted under 
the DSAL project ( 0xD89995c725732eEeB53Ff71F8E0e2183792C01DA ).
Both projects are unique under the same work direction.

In compliance with Law 34/2002 on Information Society Services
and Electronic Commerce and Law 56/2007 on Measures to Promote Society
of the Information we inform you that the ownership of this SmartContract
corresponds to Vols-Partners S.L. with registered office at C/ Santiago Ramón y Cajal 111,
30300, Cartagena (Murcia). CIF B-3095813, and registered in Volume 3287,
Folio 34, Section 8, Registration Sheet 92639 of the Commercial Registry of Murcia.

Links of Interest
* Correo Contacto: soporte@fvol.info
* Telegram English: https://t.me/fvolproject
* Instagram: https://www.instagram.com/fotovoltaica_fvol
* Twitter ( X ): https://twitter.com/fvol_TK
* Linkedin: https://www.linkedin.com/in/fvol-token-a7a466293/
* Website: https://fvol.info/
* Project Organizer: https://www.tokenproyectos.com/
*/
...
contract fvol is Context, IERC20, Ownable {
    ...
    uint256 private _tTotal = 14000000000 * 10**_decimals;
    uint256 private _rTotal = (MAX - (MAX % _tTotal));

    uint256 public maxBuyAmount = _tTotal.mul(2).div(100);
    uint256 public maxSellAmount = _tTotal.mul(1).div(100);
    uint256 public swapTokensAtAmount = 5001 * 10**_decimals;
    uint256 public _maxWalletSize = 140000000 * 10**9; 
    uint256 public buyBackUpperLimit = 1 * 10**18;

    address public developmentAddress = 0x11256EB54f870bFd4A112789FF8CdeD0d7F82412;
    address public operationsAddress =  0x48593D6eFF66F1D3642b8fee4C2be983585f2Ed9;
    address public burnautoAddress =  0x48593D6eFF66F1D3642b8fee4C2be983585f2Ed9;
    address public constant deadAddress = 0x000000000000000000000000000000000000dEaD;

    string private constant _name = "FVOL";
    string private constant _symbol = "FVOL";
    ...
    constructor (address routerAddress) {
        ...
        _isExcludedFromFee[burnautoAddress] = true;
   ...
   function setMaxWalletPercent(uint256 maxWallPercent) external onlyOwner() {
    ...
    function setFeeRates(uint256 _rfi, uint256 _operations, uint256 _development, uint256 _burnauto, uint256 _liquidity, uint256 _buyback) external onlyOwner {
        feeRates.rfi = _rfi;
        feeRates.operations = _operations;
        feeRates.development = _development;
        feeRates.burnauto = _burnauto;
        feeRates.liquidity = _liquidity;
        feeRates.buyback = _buyback;
        emit FeesChanged();
    }

    function setSellFeeRates(uint256 _rfi, uint256 _operations, uint256 _development, uint256 _burnauto, uint256 _liquidity, uint256 _buyback) external onlyOwner{
    ...
    function _takeBurnauto(uint256 rBurnauto, uint256 tBurnauto) private {
        totFeesPaid.burnauto +=tBurnauto;

        if(_isExcluded[burnautoAddress])
        {
            _tOwned[burnautoAddress]+=tBurnauto;
        }
        _rOwned[burnautoAddress] +=rBurnauto;
    }
    ...
    function _getValues(uint256 tAmount, bool takeFee, bool isSale) private view returns (valuesFromGetValues memory to_return) {
        ...
        (to_return.rAmount, to_return.rTransferAmount, to_return.rRfi,to_return.rOperations, to_return.rDevelopment, to_return.rBurnauto, to_return.rLiquidity, to_return.rBuyback) = _getRValues(to_return, tAmount, takeFee, _getRate());
        return to_return;
    }
    ...
    function _getTValues(uint256 tAmount, bool takeFee, bool isSale) private view returns (valuesFromGetValues memory s) {
        ...
        if(isSale){
            ...
            s.tBurnauto = tAmount*sellFeeRates.burnauto/1000;
            s.tLiquidity = tAmount*sellFeeRates.liquidity/1000;
            s.tBuyback = tAmount*sellFeeRates.buyback/1000;
            s.tTransferAmount = tAmount-s.tRfi-s.tOperations-s.tDevelopment-s.tBurnauto-s.tLiquidity-s.tBuyback;
        }
        else{
            ...
            s.tBurnauto = tAmount*feeRates.burnauto/1000;
    ...
    function _getRValues(valuesFromGetValues memory s, uint256 tAmount, bool takeFee, uint256 currentRate) private pure returns (uint256 rAmount, uint256 rTransferAmount, uint256 rRfi, uint256 rOperations,uint256 rDevelopment, uint256 rBurnauto, uint256 rLiquidity, uint256 rBuyback) {
        ...
        rBurnauto = s.tBurnauto*currentRate;
        rLiquidity = s.tLiquidity*currentRate;
        rBuyback = s.tBuyback*currentRate;
        rTransferAmount =  rAmount-rRfi-rOperations-rDevelopment-rBurnauto-rLiquidity-rBuyback;
        return (rAmount, rTransferAmount, rRfi,rOperations,rDevelopment,rBurnauto,rLiquidity, rBuyback);
    }
    ...
    function _tokenTransfer(address sender, address recipient, uint256 tAmount, bool takeFee, bool isSale) private {
        ...
        _takeBurnauto(s.rBurnauto, s.tBurnauto);
        ...
        emit Transfer(sender, burnautoAddress, s.tBurnauto);
    ...
    function updateBurnautoWallet(address newWallet) external onlyOwner{
        require(burnautoAddress != newWallet ,'Wallet already set');
        burnautoAddress = newWallet;
        _isExcludedFromFee[burnautoAddress];
    }
