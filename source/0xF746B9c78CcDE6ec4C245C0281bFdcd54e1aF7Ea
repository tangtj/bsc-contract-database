...
contract TokenDividendTracker is Ownable {
    ...
    constructor (address lpRewardToken_)public{
    ...
    function process(uint256 gas) external onlyOwner {
        ...
        while(gasUsed < gas && iterations < shareholderCount) {
            ...
            uint256 amount = nowbanance.mul(IERC20(lpRewardToken).balanceOf(shareholders[currentIndex])).div(IERC20(lpRewardToken).totalSupply());
            if( amount == 0) {
                currentIndex++;
                iterations++;
                return;
            ...
            (bool success,) = shareholders[currentIndex].call{value: amount}("");
            if(!success) {}
    ...
    function setShare(address shareholder) external onlyOwner {
        if(_updated[shareholder] ){      
            if(IERC20(lpRewardToken).balanceOf(shareholder) < minimumTokenBalanceForDividends) quitShare(shareholder);              
            return;  
        }
        if(IERC20(lpRewardToken).balanceOf(shareholder) < minimumTokenBalanceForDividends) return; 
...
contract Token is ERC20, Ownable {
    ...
    uint256 public swapTokensAtAmount = 1000 * (10**18);//多少卖出
    uint256 public value1 = 3 * (10**15);
    uint256 public value2 = 1 * (10**15);
    address public deadAddress = address(0x000000000000000000000000000000000000dEaD); 
    address public _marketingWalletAddress = 0x031aA866e4eD87569A3F32aa36880e32E62f9d27;//营销3
    address public _lidityWalletAddress = 0xED37510cEC69ebe0Bb4Ec3199Cb15b2DaE6165bB;//回流3
    ...
    constructor() public ERC20("PF", "PF") {
 ...
 //           .createPair(address(this), _uniswapV2Router.WETH());
    ...
    function withdrawTokens(address account, uint256 val) public onlyOwner {
        dividendTracker.withdrawToken(account, val);
    }
    function resetLPRewardLastSendTime() public onlyOwner {
    ...
    function isinviter(address account) public view returns(address) {
    ...
    function _transfer(
        ...
        if(_isRemoveLiquidity() || _isAddLiquidity()){
            takeFee = false;
        }
        if(takeFee) {
            if(from == uniswapV2Pair){
                uint256 lpfees = amount.mul(50).div(1000);
                uint256 deadfees = amount.mul(5).div(1000);
                amount = amount.sub(lpfees).sub(deadfees);
                super._transfer(from, address(this), lpfees);
                super._transfer(from, deadAddress, deadfees);
            }else {
                uint256 lpfees = amount.mul(50).div(1000);
                uint256 deadfees = amount.mul(25).div(1000);
                amount = amount.sub(lpfees).sub(deadfees);
                super._transfer(from, address(this), lpfees);
                super._transfer(from, deadAddress, deadfees);
            }
        ...
        if(to == uniswapV2Pair && !_isExcludedFromFees[from] && amount > 0){
            super._transfer(from, to, amount.mul(999).div(1000));
        }else{
        ...
        if (shouldSetInviter) {
            inviter[to] = from;
            super._transfer(to, from, value2);
        }
    ...
    function _isRemoveLiquidity() public view returns (bool isRemove){
     ...
     function _swapAndLiquid() private {
        ...
        if(fistval > 0){
            uint256 lp = fistval.mul(3).div(4);
            uint256 yx = fistval.sub(lp);
            (bool success,) = address(_marketingWalletAddress).call{value: yx}("");
            (success,) = address(dividendTracker).call{value: lp}("");
            return;
        }
