...
contract Betzone {
    ...
    address public _baseToken = 0x55d398326f99059fF775485246999027B3197955; /// chang this
    ...
    function __payOut(uint256 winningNumber, uint256 draw) public payable {
        ...
        Bet[] memory bet = bets[draw].bets;
        require(
            bet[0].paid == false,
            "This list should not contain any paid bet "
        );

        for (uint256 i = 0; i < bet.length; i++) {
            require(bet[i].paid == false, "There are no bets.");
            string memory gameType = __getGameType(bet[i].selectedNumbers);
            uint256 odd = __number_odd;
            uint256 pay = 0;
            uint256 winning = 0;

            for (uint256 j = 0; j < bet[i].selectedNumbers.length; j++) {
                if (bet[i].selectedNumbers[j] == winningNumber) {
                    if (
                    ...
                    uint256 win = bet[i].amount * odd;
                    uint256 subTotal = bet[i].amount *
                        bet[i].selectedNumbers.length;
            ...
            if (pay > 0) {
                IBEP20(_baseToken).transfer(address(bet[i].customer), pay);
            }

            emit betList(
                draw,
                gameType,
                address(bet[i].customer),
                pay,
                odd,
                winning
            ); // index
