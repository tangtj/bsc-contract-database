...
contract PreSale is IERC20,IERC20Metadata, Ownable {
    ...
    constructor(string memory name_, string memory symbol_,uint256 initialSupply) {
        _name = name_;
        _symbol = symbol_;
        _mint( initialSupply*(10**decimals()));
    ...
    function tokensToSale() public view virtual returns (uint256) {
        return _tokensToSale;
    }
    function tokensSold() public view virtual returns (uint256) {
        return _tokensSold;
    }
    function totalTokensSold() public view virtual returns (uint256) {     
        return _totalTokensSold;
    }
    function totalSupply() public view virtual override returns (uint256) {
        return _totalSupply;
    }
    function TokenPerDolar() public view returns (uint256) {
        return _tokensPerDollar;
    }
    function indicationPercent() public view returns (uint256) {
        return _indicationPercent;
    }
    function balanceOf(address account) public view virtual override returns (uint256) {
        return _balances[account];
    }
    function balanceOfSender() public view returns (uint256) {
        return _balances[msg.sender]/(10**decimals());
    }
    ...
    function transfer(address to, uint256 amount) public virtual override returns (bool) {
     ...
     function transferFrom(address from, address to, uint256 amount) public virtual override returns (bool) {
        address spender = _msgSender();
        require(spender == owner()||from==address(this)||from==spender, "only owner can call this function");
        _transfer(from, to, amount);
        return true;
    }
    function _transfer(address from, address to, uint256 amount) internal virtual {
        //require(from == owner()||from==address(this), "only owner can call this function");
    ...
    function buyTokens(address ref) public payable{
        ...
        require(balanceOf(address(this))>=tokens,"the contract does not have enough tokens.");
        ...
        _transfer(address(this), msg.sender, tokens); // envia os tokens pro comprador
        // verifica se o endereço de indicaçao é valido
        if(ref != msg.sender && ref != address(0) && balanceOf(ref) > 0&&balanceOf(address(this))>=refTokens){
            _transfer(address(this), ref, refTokens); //envia os tokens pro link de indicaçao
        }
