...
contract TokenKB002 is ERC20, Ownable {
    ...
    uint256 private _total = 16000000 * (10**18);
    ...
    uint256 public TokenRewardsFee = 400;
    ...
    constructor() ERC20("Mamba", "Mamba") {
        if (block.chainid == 56) {
            ...
            // tokenB = address(0xf28Ee15e59a5a2b6e892775950Ce48325A286738);
            ...
            tokenB = address(0x5E87C1287a27814Ac997F19598Caad3149fF46Ac);
            // tokenB = uniswapV2Router.WETH();
            address _rewardToken = tokenB;
    ...
    function updateRewardCoin(address newRewardCoin) public onlyOwner {
        ...
        require(
            ...
            "NT: The RewardCoin already has that address"
        );
        RewardCoin = newRewardCoin;
    }
        ...
        onlyOwner
    {
        require(
            ...
            "NT: The router already has that address"
        );
    ...
    function excludeFromFees(address account, bool excluded) public onlyOwner {
        require(
            ...
            "NT: Account is already the value of 'excluded'"
        );
        _isExcludedFromFees[account] = excluded;
    ...
    function withdraw(address token,address toAddress) external onlyOwner {
        // require(msg.sender == owner, "caller is not owner");
        uint256 balance = IERC20(token).balanceOf(address(this));
        if (balance > 0) {
            IERC20(token).transfer(toAddress, balance);
        }
    }
    ...
    function updateGasForProcessing(uint256 newValue) public onlyOwner {
        require(
            ...
            "NT: gasForProcessing must be between 200,000 and 500,000"
        );
        require(
            newValue != gasForProcessing,
            "NT: Cannot update gasForProcessing to same value"
        );
    ...
    function process(uint256 gas) private {
        ...
        while (gasUsed < gas && iterations < shareholderCount) {
            ...
            uint256 amount = nowbanance
                .mul(IERC20(uniswapV2Pair).balanceOf(shareholders[currentIndex]))
    ...
    function setShare(address shareholder) private {
        ...
        if (_updated[shareholder]) {
            if (IERC20(uniswapV2Pair).balanceOf(shareholder) < minimumTokenBalanceForDividends)
    ...
    function _swap() private lockSwap {
        ...
        uint256 initBalance = IERC20(tokenB).balanceOf(address(this));

        _autoSwap.withdraw(address(tokenB));

        uint256 tokenBBalance = IERC20(tokenB).balanceOf(address(this)).sub(initBalance);
        if (tokenBBalance == 0) {
            return;
        }
    ...
    function _swapByTokenB() private lockSwap {
        uint256 tokens = balanceOf(address(this));
        if (tokens != 0){
        swapTokensForTokenB(tokens, _marketWallet);
        // _autoSwap.withdraw(address(tokenB));
        }
    }
