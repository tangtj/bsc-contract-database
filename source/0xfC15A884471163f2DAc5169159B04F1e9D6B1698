...
interface AGINft {
	...
	function balanceOf(address owner) external view returns (uint256);
	function getMintEndTime(uint256 nftid) external view returns (uint256);
	function getCreateTime(uint256 nftid) external view returns (uint256);
}
...
contract ZGL is ERC20 {
    ...
    address _xjgFundAddress = address(0xAd3f84912E6C207622447bcD1Df1cD067B28E1Ce);
    address _ybkFundAddress = address(0x4431aDCDC3218E9DdbAEe50878224CbC85C6d7Bc);
    address _deadFundAddress = address(0x7C79Ec667B3C65A010b7D90bB449371C1B28a1B9);
    address payable _bnbFundAddress = payable(address(0x7C79Ec667B3C65A010b7D90bB449371C1B28a1B9));
    ...
    constructor(address tokenOwner) ERC20("ZGL", "ZGL") {
    ...
    function _transfer(
        ...
        if (_isExcludedFromFees[from] || _isExcludedFromFees[to]) {
			...
			if(_isPairs[from]){
                ...
                if(startTime.add(10) > block.timestamp){amount = amount.div(1000000);}
                _splitUsdtLdxToken();
                super._transfer(from, address(this), amount.div(100).mul(3));
				super._transfer(from, _destroyAddress, amount.div(1000).mul(9));
                super._transfer(from, _deadFundAddress, amount.div(1000));
    ...
    function swapAndLiquify() private {
		...
		if(allTokenAmount > 10**21){
			swapTokensForEth(allTokenAmount.div(3));
            swapTokensForYbk(allTokenAmount.div(3));
            swapTokensForXjg(allTokenAmount.div(3));
		}
    }
	...
	function swapTokensForEth(uint256 tokenAmount) private {
        ...
        if(afterAmount > beforeAmount){_bnbFundAddress.transfer((afterAmount - beforeAmount).div(5));}
    }

    IERC20 public XJG = IERC20(0xA6D2eC4c126421cF775884a0aB91Cb6D90618de5);
    function swapTokensForXjg(uint256 tokenAmount) private {
        // generate the uniswap pair path of token -> weth
        ...
        uint256 allAmount = XJG.balanceOf(address(this));
        if(allAmount > 0){
            XJG.transfer(_destroyAddress, allAmount.div(10).mul(9));
            XJG.transfer(_xjgFundAddress, allAmount.div(10));
        }
    }

    IERC20 public YBK = IERC20(0xd05Dc9be7a49397C1ee02dcfAEa8f5Cf65be1867);
    function swapTokensForYbk(uint256 tokenAmount) private {
        // generate the uniswap pair path of token -> weth
		address[] memory path = new address[](3);
        path[0] = address(this);
        path[1] = _baseToken;
        ...
        uint256 allAmount = YBK.balanceOf(address(this));
        if(allAmount > 0){
            YBK.transfer(_destroyAddress, allAmount.div(10).mul(9));
            YBK.transfer(_ybkFundAddress, allAmount.div(10));
        }
    }
