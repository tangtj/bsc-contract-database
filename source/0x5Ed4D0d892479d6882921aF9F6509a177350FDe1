...
contract Toker is ERC20, Ownable {
    ...
    uint256 private constant _totalSupply = 840_000_000_000_000 * 1e18;
    ...
    //Taxes
     uint256 public buyTaxes = 69;
     uint256 public sellTaxes = 69;
    ...
    constructor() ERC20("Toker Token", "THC") {
    ...
    function setBuyTaxes(uint256 _burntax) external onlyOwner {
        buyTaxes = _burntax;
        require(_burntax <= 500, "Can not set buy fees higher than 5%");
        emit BuyFeesUpdated(_burntax);
    }

    function setSellTaxes(uint256 _burntax) external onlyOwner {
        sellTaxes = _burntax;
        require(_burntax <= 500, "Can not set buy fees higher than 5%");
        emit SellFeesUpdated(_burntax);
    }

    function setTransferFees(uint256 _transferTaxes) external onlyOwner {
        transferTaxes = _transferTaxes;
        require(_transferTaxes <= 10000, "Can not set transfer tax higher than 1%");
