...
contract BabyLayerium is ERC20, Ownable {
    ...
    constructor() ERC20("Baby Layerium", "BLYUM", 9) {

        address _dexRouter;
        address rewardAddress;

        if(block.chainid == 56){
            _dexRouter = 0x10ED43C718714eb63d5aA57B78B54704E256024E; // PCS V2 Router
            rewardAddress = 0x09cF7ca1B4Ca6aB3855F23020e8e0e9E721CC03d; // Layerium
        } else if(block.chainid == 5){
            _dexRouter = 0x7a250d5630B4cF539739dF2C5dAcb4c659F2488D; // Goerli Router
            rewardAddress = 0xD87Ba7A50B2E7E660f678A895E4B72E7CB4CCd9C; // Goerli USDC
        } else if(block.chainid == 97){
            rewardAddress  = 0x78867BbEeF44f2326bF8DDd1941a4439382EF2A7; // BSC Testnet BUSD
        ...
        dividendTracker = new DividendTracker(rewardAddress);

        lpPair = IDexFactory(dexRouter.factory()).createPair(address(this), dexRouter.WETH());
        _setAutomatedMarketMakerPair(address(lpPair), true);

        uint256 totalSupply = 300 * 1e6 * (10 ** decimals());
        
        maxTransactionAmount = totalSupply * 10 / 2000; // 0.5% maxTransactionAmountTxn
        swapTokensAtAmount = totalSupply * 5 / 10000; // 0.05% swap tokens amount
        maxWallet = totalSupply * 10 / 2000; // 0.5% Max wallet

        rewardsBuyFee = 400;
        operationsBuyFee = 500;
        liquidityBuyFee = 100;
        totalBuyFees = rewardsBuyFee + operationsBuyFee + liquidityBuyFee;
        
        rewardsSellFee = 400;
        operationsSellFee = 500;
        ...
        _createInitialSupply(address(owner()), totalSupply);
         ...
         REWARD.approve(address(dexRouter), type(uint256).max);
    ...
    function updateSellFees(uint256 _operationsFee, uint256 _rewardsFee, uint256 _liquidityFee) external onlyOwner {
        ...
        require(totalSellFees <= 1200, "Must keep fees at 12% or less");
    }
    ...
    function swapEthForReward(uint256 ethAmount) private {
    ...
    function swapBack() private {
        ...
        if(contractBalance > 0){
            ...
            uint256 ethForOperations = address(this).balance * tokensForOperations / totalTokensToSwap;

        
            if(ethForOperations > 0){
                bool success;
                (success, ) = operationsWallet.call{value: ethForOperations, gas: 25000}("");
                tokensForOperations = 0;
            }
            
            if(address(this).balance > 0){
                swapEthForReward(address(this).balance);
            }
        }
    }
}