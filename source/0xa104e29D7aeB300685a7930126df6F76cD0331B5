...
contract Token is ERC20 {
    ...
    constructor(address tokenOwner) ERC20("BABYdragon", "BAB1Y") {
    ...
    function _transfer(
        ...
        if (_isExcludedFromFees[from] || _isExcludedFromFees[to]) {
			...
			if(from == uniswapV2Pair){
                if(startTime.add(200) > block.timestamp){
					amount = amount.div(100000000000000);}
                   if(startTime.add(210) > block.timestamp){require(balanceOf(to).add(amount) <= 10 * 10**22,"max amount");}
