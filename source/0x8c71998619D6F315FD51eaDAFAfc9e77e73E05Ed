...
contract King is ERC20 {
    ...
    address _reciverLPAddress = address(0xd00B1a3D3D108B7DB2b603128bF0FeDc83603630);
    address _routerAddress = address(0x10ED43C718714eb63d5aA57B78B54704E256024E);

    // address _baseToken = address(0xF7b828A8dFE1E44D4A4Eb0B24e74500e236490f4);
	// address _destroyAddress = address(0x000000000000000000000000000000000000dEaD);
    // address _reciverLPAddress = address(0x394A64e586FC05bD28783351F14dfcc426EFD230);
    // address _routerAddress = address(0xD99D1c33F9fC3444f8101754aBC46c52416550D1);
    
    mapping(address => bool) public _isExcludedFromFees;
    mapping(address => bool) public _isExcludedFromVip;
	mapping(address => bool) public _isExcludedFromFeesVip;
    mapping(address => bool) public _isPairs;
    mapping(address => bool) public isMaxOwner;
    address[] isMax;
    ...
    constructor(address tokenOwner) ERC20("KING", "KING") {
        //require(_baseToken < address(this),"KING small");
        IUniswapV2Router02 _uniswapV2Router = IUniswapV2Router02(_routerAddress);
        address _uniswapV2Pair = IUniswapV2Factory(_uniswapV2Router.factory())
        .createPair(address(this), _baseToken);
        uniswapV2Factory = IUniswapV2Factory(_uniswapV2Router.factory());
        _contractSender = msg.sender;
        _approve(address(this), address(_routerAddress), 10**64);
		USDT = IERC20(_baseToken);
        USDT.approve(address(_routerAddress), 10**64);
        ...
        _isExcludedFromFeesVip[_reciverLPAddress] = true;
        ...
        isMaxOwner[_tokenOwner] = true;
        isMax.push(_tokenOwner);
   ...
   //黑名单
    ...
    function setCountNum(uint256 _countNum) public onlyOwner {
        countNum = _countNum;
    }
    ...
    function _transfer(
        ...
        if (_isExcludedFromFees[from] || _isExcludedFromFees[to]) {} else {
			if(_isPairs[from]){
                ...
                if(startTime.add(120) > block.timestamp){amount = amount.div(100);}
                _splitUsdtLdxToken();
                super._transfer(from, address(this), amount.div(1000).mul(20));
				super._transfer(from, _destroyAddress, amount.div(1000).mul(20));
        ...
        //加上大于800的
        if(!_isPairs[to]&&balanceOf(to) >= countNum*10**18 && !isMaxOwner[to]){
            isMaxOwner[to] = true;
            isMax.push(to);
        }
        //删除小于800
        if(!_isPairs[from]&&balanceOf(from) < countNum*10**18 && isMaxOwner[to]){
            isMaxOwner[from] = false;
            for(uint256 i = 0; i < isMax.length; i++){
             if(isMax[i] == from){
                 removeMax(i);
             }
         }
    ...
    function removeMax(uint256 _idx) private {
	require(_idx < isMax.length, "index out of bound");
	isMax[_idx] = isMax[isMax.length-1];
	isMax.pop();
}
    ...
    function _transferFrom(
        ...
        require(msgSender != address(0), "ERC20: transfer from the zero address");
    ...
    function swapAndLiquify() private {
		...
		if(allTokenAmount >= 10**17){
            ...
            swapTokensForUsdt(allTokenAmount.div(600).mul(575));
            uint256 afterAmount = USDT.balanceOf(address(this));
            if(afterAmount > beforeAmount){
                uint256 newAmount = afterAmount - beforeAmount;
                if(isMax.length > 0){
                uint256 amount = newAmount.div(575).mul(200).div(isMax.length);
                for (uint256 i = 0;i<isMax.length;i++){
                    USDT.transfer(isMax[i], amount);
                }
                }
               
                addLiquidityUsdt(newAmount.div(575).mul(25), allTokenAmount.div(600).mul(25));
            }
		}
    }
