...
contract Presale {
    ...
    IERC20 TOKEN = _token;
    require(USDT.transferFrom(msg.sender, address(this), usdAmount), "USD transfer failed");
    uint256 tokensToTransfer = usdAmount.mul(TokenPer_USD);
    require(tokensToTransfer > 0, "Amount must be greater than 0");
    require(TOKEN.balanceOf(address(this)) >= tokensToTransfer, "Insufficient token balance");
    uint256 referralBonus = usdAmount.mul(10).div(100);
    uint256 devToken = usdAmount.mul(90).div(100);
    TOKEN.transfer(msg.sender, tokensToTransfer);
    if (_referral != address(0) && referralBonus > 0) {
