...
contract Sale_Contract {
    ...
    function buy(address _referrer) public payable {
        if (_referrer == address(0)) {
            ...
            // Handle the case when _referrer is provided
        }
    ...
    function _calculateReferralBonus(uint256 _weiAmount) internal view returns (uint256) {
        uint256 level = referralCount[msg.sender] / 3;
        uint256 bonusRate;
    
        if (level >= 3 && level <= 42) {
            bonusRate = level.mul(5).add(5); // Starts from 10% and increases by 5% for each level
            return _weiAmount.mul(bonusRate).div(100);
        } else if (level > 42) {
