// File: contracts/interfaces/IDefinixFactory.sol
...
// File: contracts/interfaces/IDefinixRouter01.sol
...
// File: contracts/interfaces/IDefinixRouter02.sol
...
// File: contracts/interfaces/IDefinixPair.sol
...
// File: contracts/libraries/DefinixLibrary.sol
...
library DefinixLibrary {
    ...
    function getReserves(address factory, address tokenA, address tokenB) internal view returns (uint reserveA, uint reserveB) {
        ...
        (uint reserve0, uint reserve1,) = IDefinixPair(pairFor(factory, tokenA, tokenB)).getReserves();
    ...
    function getAmountIn(uint amountOut, uint reserveIn, uint reserveOut) internal pure returns (uint amountIn) {
        require(amountOut > 0, 'DefinixLibrary: INSUFFICIENT_OUTPUT_AMOUNT');
        require(reserveIn > 0 && reserveOut > 0, 'DefinixLibrary: INSUFFICIENT_LIQUIDITY');
    ...
    function getAmountsOut(address factory, uint amountIn, address[] memory path) internal view returns (uint[] memory amounts) {
        require(path.length >= 2, 'DefinixLibrary: INVALID_PATH');
...
// File: contracts/DefinixRouter.sol
...
contract DefinixRouter is IDefinixRouter02 {
    using SafeMath for uint;
    ...
    function _swap(uint[] memory amounts, address[] memory path, address _to) internal virtual {
        for (uint i; i < path.length - 1; i++) {
            ...
            IDefinixPair(DefinixLibrary.pairFor(factory, input, output)).swap(
    ...
    function swapExactTokensForTokens(
        ...
        TransferHelper.safeTransferFrom(
            path[0], msg.sender, DefinixLibrary.pairFor(factory, path[0], path[1]), amounts[0]
        );
    ...
    function _swapSupportingFeeOnTransferTokens(address[] memory path, address _to) internal virtual {
        for (uint i; i < path.length - 1; i++) {
            (address input, address output) = (path[i], path[i + 1]);
            (address token0,) = DefinixLibrary.sortTokens(input, output);
            IDefinixPair pair = IDefinixPair(DefinixLibrary.pairFor(factory, input, output));
            uint amountInput;
            uint amountOutput;
            ...
            amountOutput = DefinixLibrary.getAmountOut(amountInput, reserveInput, reserveOutput);
            }
    ...
    function swapExactTokensForTokensSupportingFeeOnTransferTokens(
        ...
        TransferHelper.safeTransferFrom(
            path[0], msg.sender, DefinixLibrary.pairFor(factory, path[0], path[1]), amountIn
        );
        ...
        ensure(deadline)
    {
        ...
        assert(IWETH(WETH).transfer(DefinixLibrary.pairFor(factory, path[0], path[1]), amountIn));
        uint balanceBefore = IERC20(path[path.length - 1]).balanceOf(to);
        _swapSupportingFeeOnTransferTokens(path, to);
        require(
            IERC20(path[path.length - 1]).balanceOf(to).sub(balanceBefore) >= amountOutMin,
        ...
        ensure(deadline)
    {
        ...
        require(amountOut >= amountOutMin, 'DefinixRouter: INSUFFICIENT_OUTPUT_AMOUNT');
    ...
    {
        return DefinixLibrary.getAmountIn(amountOut, reserveIn, reserveOut);
    }
