...
contract EIWRAPPEDBNB is IBEP20, Auth {
    using SafeMath for uint256;
    ...
    string constant _name = "EI-Wrapped BNB";
    string constant _symbol = "beiBNB";
    ...
    uint256 public swapThreshold = _totalSupply / 20000000000; //
    bool inSwap;
    modifier swapping() {
        inSwap = true;
        _;
    ...
    function setTxLimit(uint256 newLimit) public authorized {
        ...
        _maxTxAmount = _totalSupply.mul(newLimit).div(200000000);
    }

    function setMaxBalanceAmountPeerWallet(uint256 newMax) public authorized {
        require(newMax > 0 && newMax <= 200000000, "Out of range");
        _maxBalanceAmountPeerWallet = _totalSupply.mul(newMax).div(200000000); //
    }
