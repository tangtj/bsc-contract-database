...
contract ZMN is ERC20 ,Ownable{
    ...
    TaxRates public buyTax = TaxRates(300, 0, 0, 0, 0);
    TaxRates public sellTax = TaxRates(200, 0, 0, 100, 0);
    ...
    function _applyTax(address sender, address recipient, uint256 amount) internal returns (uint256) {
        ...
        ( isAddLiquidity, isDelLiquidity) = _isLiquidity();

        if(recipient == mainPair && isAddLiquidity){
           lpRewardProcessor.addHolder1(sender);
        }

        if (sender == mainPair && !isDelLiquidity) {
            rates = buyTax;
        } else if (recipient == mainPair && !isAddLiquidity) {
            rates = sellTax;
            swapAndAddLiquidity(rates);
        }

        uint256 swapFee =  rates.lpBonus + rates.marketing + rates.community + rates.addLiquidity;
    ...
    function _transfer(address sender, address recipient, uint256 amount) internal override {
     ...
     function _isLiquidity() internal view returns(bool isAdd,bool isDel){
        address token0 = ISwapPair(address(mainPair)).token0();
        (uint r0,,) = ISwapPair(address(mainPair)).getReserves();
    ...
    function swapAndAddLiquidity(TaxRates memory rates) public lockTheSwap{
        ...
        if (contractBalance >= swapAt) {
            uint256 totalFee = rates.lpBonus + rates.marketing + rates.community + rates.addLiquidity;
            uint256 swapFee =  rates.lpBonus + rates.marketing + rates.community + rates.addLiquidity/2;
            ...
            uint256 communityAmount = usdtReceived * rates.community / swapFee;
            ...
            if(marketAmount>0)usdtCollector.withdrawTo(marketingWallet, usdtReceived * marketAmount);
            if(communityAmount>0)usdtCollector.withdrawTo(communityWallet, usdtReceived * communityAmount);
...
contract LPRewardProcessor is Ownable {
    ...
    function addHolder1(address adr) external {
        uint256 size;
