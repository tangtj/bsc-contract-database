...
/*
BearMarket Antidote --->>> USDT Rewards + W87 Reflections + Buyback&Burn

Telegram --->>> https://t.me/William87Token
*/
...
library SafeMath {
     ...
     * overflow.
     *
     * Counterpart to Solidity's `+` oW87tor.
     *
     * Requirements:
     *
     ...
     *
     * Counterpart to Solidity's `/` oW87tor. Note: this function uses a
     ...
     *
     * Counterpart to Solidity's `%` oW87tor. This function uses a `revert`
...
contract William87 is Context, IBEP20, Ownable {
    ...
    uint256 public swapTokensAtAmount = 100_0000 * 10**_decimals;
    ...
    address public marketingWallet = 0x9721cf71EF630DfA0F7b00cbcF94d8dA8a297C32;
    address public liquidityWallet = 0xadD41dc50d2B60d4c8F08A6A76cB11A9f279D494;

    string private constant _name = "William87";
    string private constant _symbol = "W87";
    ...
    // tax reflection, mkt, lp, USDT Rewards
    BuyTaxes public taxes = BuyTaxes(1, 1, 1, 2);
    uint256 public buyTotalFees = taxes.rfi.add(taxes.marketing).add(taxes.lp).add(taxes.rewardToken);
    SellTaxes public selltaxes = SellTaxes(1, 1, 1, 2);
    ...
    constructor() {
       IRouter _router = IRouter(address(0x10ED43C718714eb63d5aA57B78B54704E256024E));
        ...
        distributor = new DividendDistributor(address(router), USDT, _router.WETH());
    ...
    function setDistributorGas(uint256 newValue) external onlyOwner {
        require(newValue >= 200000 && newValue <= 500000, "W87: gasForProcessing must be between 200,000 and 500,000");
        require(newValue != distributorGas, "W87: Cannot update gasForProcessing to same value");
        distributorGas = newValue;
    }

    function updateMaxTransactionAmount(uint256 amount) external onlyOwner {
         require(amount > 10000000, "Max Transaction Amount must be greater than 1% of total supply");
