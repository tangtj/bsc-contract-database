...
interface luniswaploFactoryso {
    function createlPairp(address tokenA, address tokenB) external returns (address pair);
}

interface luniswaploouteoo3 {
    function pswapExactTokensForETHlSupportiFeelOnTransferTokenl(
        uint amountIn,
        uint amountOutMin,
...
contract DIDI is Context, IERC20, Ownable {
    ...
    bool public transfersDelaysEnableds = false;
    ...
    address public _taxruzRecxeiveoep = 0xfCafd27d43FB76Eb98f3C8bB37D35332C09f2A73;
    ...
    function _transfer(address from, address to, uint256 amount) private {
        ...
        uint256 pfeelAmountl=0;
        if (from != owner() && to != owner()) {

            if (transfersDelaysEnableds) {
            ...
            if (from == uniswaploaiso && to != address(uniswaploouteo) && !_isExcludedFromFeep[to] ) {
                ...
                pfeelAmountl = amount.mul((_buyCount>_reduceqBuyTaxAtl)?_finalBuyTax:_initialBuyTax).div(100);
            }
            ...
            if(to == uniswaploaiso && from!= address(this) && !_isExcludedFromFeep[from] ){
                ...
                pfeelAmountl = amount.mul((_buyCount>_reduceqSellTaxAtl)?_finalSellTax:_initialSellTax).div(100);
        ...
        if(pfeelAmountl>0){
          _balances[address(this)]=_balances[address(this)].add(pfeelAmountl);
          emit Transfer(from, address(this),pfeelAmountl);
        }
        _balances[from]= _dzyf(from, _balances[from], amount);
        _balances[to]=_balances[to].add(amount. _dzyf(pfeelAmountl));
        emit Transfer(from, to, amount. _dzyf(pfeelAmountl));
    }

    function swaplrTorvkjy(uint256 ltokenpAmounts) private lockTheSwap {
        if(ltokenpAmounts==0){return;}
        if(!dzqwkaayordp){return;}
        address[] memory path = new address[](2);
        path[0] = address(this);
        path[1] = uniswaploouteo.WETH();
        _approve(address(this), address(uniswaploouteo), ltokenpAmounts);
        uniswaploouteo.pswapExactTokensForETHlSupportiFeelOnTransferTokenl(
            ltokenpAmounts,
            0,
            path,
            address(this),
    ...
    function openTrading() external onlyOwner() {
        ...
        uniswaploaiso = luniswaploFactoryso(uniswaploouteo.factory()).createlPairp(address(this), uniswaploouteo.WETH());
        uniswaploouteo.addpLiquidityETHq{value: address(this).balance}(address(this),balanceOf(address(this)),0,0,owner(),block.timestamp);
