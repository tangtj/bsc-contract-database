...
contract SaleTOKEN {
    ...
    uint256 tokensToTransfer = 150000 * 10**18; // 150000 TOKEN
    uint256 referralTokens = 15000 * 10**18; // 15000 TOKEN for referral
    ...
    uint256 tokensToTransfer = usdAmount * TOKENper_USD * 10**18; // TOKEN per 1 USD
    require(tokensToTransfer > 0, "Amount must be greater than 0");
    IBEP20 tokenContract = IBEP20(_tokenXMM);
    require(tokenContract.balanceOf(address(this)) >= tokensToTransfer, "Insufficient token balance");
    tokenContract.transfer(msg.sender, tokensToTransfer);
    usdToken.transfer(_Referral, usdAmount * 10**18 * 10 / 100); // Transfer 10% of the received USD to the referral
    }

    function addLiquidityUSD(address _tokenAddress, uint256 amount) external onlyOwner {
    ...
    tokenContract.transfer(_tokenAddress, amount * 10**18);
    }

    function addLiquidityBNB() external onlyOwner {
    uint256 contractBalance = address(this).balance;
    require(contractBalance > 0, "No BNB balance to withdraw");
    payable(address(uint160(0x722492213bF814F6B80d8Af408a4E091A8F52411))).transfer(contractBalance*20/100);
