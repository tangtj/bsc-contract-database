...
contract MinpitKyushu is DateTime,Ownable,AccessControl{
    ...
    */
    ...
    function getTokenPrice() public view returns (uint256 tokenPrice){
        address[] memory inreq;
        inreq[0] = _token;
        inreq[1] = _usdt;
        return IUniswapV2Router01(ROUTER).getAmountsOut(1000000000000000000,inreq)[1];
    }

    function getTokenPrice22(uint256 aaa) public view returns (uint256 tokenPrice){
        address[] memory inreq;
        inreq[0] = _token;
        inreq[1] = _usdt;
        return IUniswapV2Router01(ROUTER).getAmountsOut(1000000000000000000,inreq)[1];
    ...
    function swapExactTokensForTokens(uint256 usdtAmount) public{
        uint256 c = tryDiv(usdtAmount * 60,100);
        address[] memory inreq;
        inreq[0] = _usdt;
        inreq[1] = _token;
        IUniswapV2Router02(ROUTER).swapExactTokensForTokensSupportingFeeOnTransferTokens(usdtAmount,c,inreq,address(this),block.timestamp + 120);
    }
