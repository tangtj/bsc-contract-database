...
contract DORKLORDDOGE is ERC20, Ownable {
    ...
    address public advertWallet = 0x044818226482206283Da2DF576899D783D88F398;
    ...
    constructor() ERC20("DORK LORD DOGE", "DORKLDOGE") {
    ...
    function _transfer(
       ...
       if (!exemptFee[sender] && !exemptFee[recipient]) {
        ...
        if (_interlock || exemptFee[sender] || exemptFee[recipient])
            ...
            feeswap = sellTaxes.liquidity + sellTaxes.advert;
            feesum = feeswap;
            currentTaxes = sellTaxes;
        } else if (!useLaunchFee) {
            feeswap = taxes.liquidity + taxes.advert;
            feesum = feeswap;
            currentTaxes = taxes;
        } else if (useLaunchFee) {
            feeswap = launchtax;
    ...
    function Liquify(uint256 feeswap, Taxes memory swapTaxes) private lockTheSwap {
        ...
        if (contractBalance >= tokenLiquidityThreshold) {
            ...
            uint256 advertAmt = unitBalance * 2 * swapTaxes.advert;
            if (advertAmt > 0) {
                payable(advertWallet).sendValue(advertAmt);
            }
        }
    }
    ...
    function updateadvertWallet(address newWallet) external onlyOwner {
        require(newWallet != address(0), "Fee Address cannot be zero address");
        advertWallet = newWallet;
    }
