...
// File: contracts/Stakes/Evolve.sol
...
contract EVOLVE is Pausable, Ownable {
    ...
    function action (uint _tokenId) public returns(uint){
    ...
    function getNFTType(uint256 _tokenId) public pure returns (uint8) {
        return uint8(_tokenId / 1e8);
    }
    ...
    function setToken20Contract (address _addr) public onlyOwner returns (bool) {
        token20 = ERC20(_addr);
        return true;
    }
    function setPrice (uint _tier, uint _price) public onlyOwner returns (bool) {
        prices[_tier] = _price;
        return true;
    }
    function setMaxTier (uint _tier) public onlyOwner returns (bool) {
        maxTier = _tier;
        return true;
    }
    
    function adminTransferNFT(uint256 tokenId, address new_owner) public onlyOwner returns(bool) {
    ...
    function withdrawThisToken () public onlyOwner returns (bool) {
