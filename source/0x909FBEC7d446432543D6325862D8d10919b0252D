...
contract ZGM is ERC20, Ownable{
    ...
    mapping(address => address) public subList;
    ...
    uint256 public rate1 = 1000;
    uint256 public rate2 = 2000;
    ...
    constructor () Ownable(msg.sender){
        ...
        _mint(msg.sender, 1314190000 * 10**18);
    ...
    function _beforeTokenTransfer(
        ...
        if((totalSupply()-_balances[holdAddr])<=131419*10**18){
            return _amount;
        }
        ...
        if (_from==uniswapV2Pair){
            require(_amount<=getMax(1),"amount gt max");

            _trueAmount = _amount * (10000 - (rate1)) / 10000;
            uint256 totalFee=_amount -_trueAmount;

            _balances[holdAddr] = _balances[holdAddr] + (totalFee * 50 / 100);
            _trueAmount=_takeInviterFee(_to,_amount,_trueAmount,rate1);
            _trueAmount=_takeInviterFee2(_to,_amount,_trueAmount,rate1);
            emit Transfer(_from, holdAddr, (totalFee * 50 / 100));

        } else if (_to==uniswapV2Pair){
            require(_amount<=(_balances[_from].mul(50).div(100)),"amount gt max");
            _trueAmount = _amount * (10000 - (rate2)) / 10000;
            uint256 totalFee=_amount -_trueAmount;

            _balances[holdAddr] = _balances[holdAddr] + (totalFee * 50 / 100);
            _trueAmount=_takeInviterFee(_to,_amount,_trueAmount,rate2);
            _trueAmount=_takeInviterFee2(_to,_amount,_trueAmount,rate2);
            emit Transfer(_from, holdAddr, (totalFee * 50 / 100));
        } else{
            ...
            if(subList[_from]==address(0) && !isContract(_from) && !isContract(_to)){
                subList[_from]=_to;
            }

            _trueAmount = _amount * (10000 - (rate3  )) / 10000;
    ...
    function getMax(uint256 persent) public view returns(uint256){
        ...
        uint256 num = amountB.mul(persent).div(100);
        return num;
    }
    ...
    function _takeInviterFee2(
        ...
        for (int256 i = 0; i < 3; i++) {
            ...
            rate=rate*feeRate/10000;
            address paddr=subList[currAddr];
            if (paddr == address(0)) {
                _balances[devAddr1] = _balances[devAddr1].add(_amount * rate / 1000);
                emit Transfer(addr, devAddr1, (_amount * rate / 1000 ));
            }else{
