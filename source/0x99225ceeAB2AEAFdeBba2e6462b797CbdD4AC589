...
contract Astrobirdz is IERC20, IERC20Metadata, Pausable, ReentrancyGuard {
  ...
  mapping(address => bool) private _isIncludedFromFees;
  ...
  event SwapBackTaxSuccess(uint256 tokenAmount, uint256 ethAmountReceived, bool success);
  ...
  constructor() {
    ...
    _isIncludedFromFees[address(pancakePair)] = true;
  ...
  function setTax(uint256 _newTeamTax, uint256 _newMarketingTax) public whenNotPaused onlyOwner {
    _teamTax = _newTeamTax;
    _marketingTax = _newMarketingTax;
    _totalTax = _teamTax + _marketingTax;
  }

  function setTeamAddress(address _newAddress) public whenNotPaused onlyOwner {
    require(_newAddress != address(0), "Zero Address Given");
    _teamAddress = _newAddress;
  }

  function setMarketingAddress(address _newAddress) public whenNotPaused onlyOwner {
    require(_newAddress != address(0), "Zero Address Given");
  ...
  function setExcludedFromLimits(address account, bool enabled) public whenNotPaused onlyOwner {
    require(account != address(0), "Zero Address Given");
    _isExcludedFromLimits[account] = enabled;
  }

  function setMaxTxPercent(uint256 newMaxTxPercent) external whenNotPaused onlyOwner {
    require(
      (newMaxTxPercent > 0) && (newMaxTxPercent < 1000),
      "Max Tx Percent should be  greater than 0 and less than 1000"
    );
  ...
  function setExcludedFromFees(address account, bool enabled) public whenNotPaused onlyOwner {
    require(account != address(0), "Zero Address Given");
  ...
  function setAutomatedMarketMakerPair(address pair, bool value) public onlyOwner {
    require(pair != pancakePair, "The pair cannot be removed from automatedMarketMakerPairs");
  ...
  function _transfer(address from, address to, uint256 amount) internal returns (bool) {
    ...
    bool takeFee;
    if (_isIncludedFromFees[from] && _isIncludedFromFees[to]) {
      takeFee = true; // Default to taking fees
    }
  ...
  function _finalizeTransfer(
    ...
    if (swapEnabled && !swapping && takeFee) {
      swapping = true;
