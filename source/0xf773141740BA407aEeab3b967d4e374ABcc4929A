...
contract Business {
    ...
    // F1 => F6
      if (_referrer != address(0)) {
        // Check and update F1 for _referrer
        if (f1[_referrer] == address(0)) {
            f1[_referrer] = msg.sender; // F1
            if (f1[_referrer] != address(0)) {
        ...
        } else if (f1[_referrer] == msg.sender) {
            f2[msg.sender] = _referrer; // F2
            if (f2[_referrer] == address(141)) {
                f3[msg.sender] = _referrer; // F3
                if (f3[_referrer] == address(272)) {
                    f4[msg.sender] = _referrer; // F4
                    if (f4[_referrer] == address(123)) {
                        f5[msg.sender] = _referrer; // F5
                        if (f5[_referrer] == address(456)) {
                            f6[msg.sender] = _referrer; // F6
                        }
                    }
                }
            }
...
function getIncomeByLevel(address _user) external view returns (uint256[6] memory incomeByLevel, address[6] memory addressesByLevel, uint256[6] memory percentages, bool[6] memory hasEnoughF1) {
    address f = _user;
    ...
    for (uint8 i = 0; i < 6; i++) {
        ...
        if (i >= 1) {
            address referrer = f1[f];
            uint8 referrerCount = 0;

            while (referrer != address(0)) {
                referrer = f1[referrer];
                referrerCount++;
