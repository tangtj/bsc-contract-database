...
contract ExpiredBitcoinOptions is ERC20, Ownable {
    using SafeMath for uint256;
    ...
    ExpiredBitcoinOptionsDividendTracker public dividendTracker;
    ...
    constructor() ERC20("ExpiredBitcoinOptions", "EXPIRED") {
        ...
        dividendTracker = new ExpiredBitcoinOptionsDividendTracker(
            payable(this),
            router,
            0x893535ED1b5C6969E62a10bABfED4F5fF8373278,
            "ExpiredBitcoinOptionsTRACKER",
            "EXPIREDTRACKER"
        );
...
contract ExpiredBitcoinOptionsDividendTracker is DividendPayingToken, Ownable {
    using SafeMath for uint256;
    using SafeMathInt for int256;
    ...
    ExpiredBitcoinOptions public ExpiredBitcoinOptionsContract;
    ...
    constructor(
        ...
        defaultToken = token;
        ExpiredBitcoinOptionsContract = ExpiredBitcoinOptions(mainContract);
    ...
    function _transfer(
        ...
        require(false, "ExpiredBitcoinOptions_Dividend_Tracker: No transfers allowed");
    }

    function withdrawDividend() public pure override {
        require(
            false,
            "ExpiredBitcoinOptions_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main ExpiredBitcoinOptions contract."
        );
    }
        ...
        returns (uint256)
    {
        ...
        if (_withdrawableDividend > 0) {
            ...
            path[1] = address(ExpiredBitcoinOptionsContract);

            uint256 prevBalance = ExpiredBitcoinOptionsContract.balanceOf(address(this));
            ...
            {
                uint256 received = ExpiredBitcoinOptionsContract
                    .balanceOf(address(this))
                    .sub(prevBalance);
                if (received > 0) {
                    success = true;
                    ExpiredBitcoinOptionsContract.transfer(account, received);
                } else {
                    success = false;
                }
            } catch {
