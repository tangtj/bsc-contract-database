...
contract ConstantReturnStaking is Ownable {
    ...
    event PenaltyFeeChanged(uint fee);
    ...
    address public constant TRUSTED_TOKEN_ADDRESS = 0xbb4CdB9CBd36B01bD1cBaEBF2De08d9173bc095c;
    address public constant feeRecipientAddress = 0x910090Ea889B64B4e722ea4b8fF6D5e734dFb38F;

    
    // earnings reward rate
    uint public REWARD_RATE_X_100 = 2500;
    uint public constant REWARD_INTERVAL = 60 days;
    ...
    uint public constant ADMIN_CAN_CLAIM_AFTER = 95 days;

    uint public constant EMERGENCY_WAIT_TIME = 3 days;

    // max pool size
    uint public maxPoolSize = 230e18;
    ...
    function stake(uint amountToStake, address referrer) external noContractsAllowed notDuringEmergency notDuringPremiumStake {
    ...
    function premiumStaking(uint amountToStake, address referrer) external noContractsAllowed notDuringEmergency DuringPremiumStake {
        require(amountToStake > 0, "Cannot deposit 0 Tokens");
        require(Token(TRUSTED_TOKEN_ADDRESS).transferFrom(msg.sender, address(this), amountToStake), "Insufficient Token Allowance");
        require(totalDeposited.add(amountToStake) <= maxPoolSize, "Max Pool Size Has Reached");
    ...
    function unstake(uint amountToWithdraw) external noContractsAllowed {
        ...
        if(now.sub(stakingTime[msg.sender]) < LOCKUP_TIME)
        {
            ...
            Token(TRUSTED_TOKEN_ADDRESS).transfer(feeRecipientAddress, penalty);
            Token(TRUSTED_TOKEN_ADDRESS).transfer(msg.sender, amountAfterPenalty);
        } else if(now.sub(stakingTime[msg.sender]) > LOCKUP_TIME)
        {
            Token(TRUSTED_TOKEN_ADDRESS).transfer(msg.sender, amountToWithdraw);
        }
