...
contract SmartChefInitializable is Ownable, ReentrancyGuard {
    ...
    // Boosters
    uint256 public booster1Time;
    uint256 public booster2Time;
    uint256 public booster3Time;
    uint256 public booster1Power;
    uint256 public booster2Power;
    uint256 public booster3Power;
    ...
    // Temporary Decimals Reward Token

    uint256 public xDecimalsRewardToken;
    ...
    struct UserInfo {
        ...
        uint256 boosterTier; // booster tier
    ...
    constructor(
        ...
        // Deposit Fee Address
        depositFeeAddress = 0x1A6eeABCFf0cC8567a3015CBCfC5666d16d69ED3;
        ...
        // Booster 1 Setting
        booster1Time = 300; // 5 minutes
        booster1Power = 50; // 50% (x1.5)

        // Booster 2 Setting
        booster2Time = 600; // 10 minutes
        booster2Power = 100; // 100% (x2.0)

        // Booster 3 Setting
        booster3Time = 1200; // 20 minutes
        booster3Power = 200; // 200% (x3.0)
    }
    ...
    function initialize(
        ...
        xDecimalsRewardToken = decimalsRewardToken;
     ...
     */
    function deposit(uint256 _amount, uint256 _boosterTier) external nonReentrant {
        UserInfo storage user = userInfo[msg.sender];
        require(_boosterTier <= boosterCount, "Invalid booster selection");
        ...
        require(user.boosterTier <= _boosterTier || user.locked == false, "Cannot downgrade booster while locked");

        bool newBooster;
        if (user.boosterTier != _boosterTier) {
            newBooster = true;
        }

        uint256 boosterTime;
        uint256 boosterPower;
        if (_boosterTier == 0) {
            boosterTime = 0;
            boosterPower = 0;
        } else if (_boosterTier == 1) {
            boosterTime = booster1Time;
            boosterPower = booster1Power;
        } else if (_boosterTier == 2) {
            boosterTime = booster2Time;
            boosterPower = booster2Power;
        } else if (_boosterTier == 3) {
            boosterTime = booster3Time;
            boosterPower = booster3Power;
        }

        // Depositing while already boosted will re-lock the position
        // Changing booster tier without depositing additional tokens will also lock the position
        if (_boosterTier > 0) {
            if (_amount > 0 || newBooster) {
                user.lockStartTime = block.timestamp;
                user.lockEndTime = block.timestamp + boosterTime;
                user.locked = true;
                user.boosterTier = _boosterTier;
            }
        } else if (_boosterTier == 0) {
            if (newBooster) {
        ...
        if (user.amount > 0) {
            ...
            uint256 stakeCount = 1;
            totalStakedUsers = totalStakedUsers + stakeCount;
        }

        if (_amount > 0) {
            uint256 depositFee;
            if (depositFeeBP > 0) {
                depositFee = (_amount * depositFeeBP) / 10000;
            } else {
                depositFee = 0;
            }
            uint256 stakedTokenSupply = stakedToken.balanceOf(address(this));
            uint256 oldUserShares = user.shares;
            uint256 newUserAmount = user.amount + _amount - depositFee;
            uint256 newUserShares = (newUserAmount * boosterPower *10**10) / PRECISION_FACTOR + newUserAmount;
            totalStakedTokens = stakedTokenSupply + _amount - depositFee;
            totalStakedShares = totalStakedShares - oldUserShares + newUserShares;
            user.amount = newUserAmount;
            user.shares = newUserShares;
            uint256 totalTransferAmount = _amount - depositFee;
            stakedToken.safeTransferFrom(address(msg.sender), address(this), totalTransferAmount);
            if (depositFee > 0) {
                stakedToken.safeTransfer(depositFeeAddress, depositFee);
            }
        } else if (_amount == 0 && newBooster) {
            uint256 oldUserShares = user.shares;
            uint256 newUserShares = (user.amount * boosterPower *10**10) / PRECISION_FACTOR + user.amount;
            totalStakedShares = totalStakedShares - oldUserShares + newUserShares;
            user.shares = newUserShares;
        }
    ...
    function withdraw(uint256 _amount) external nonReentrant {
        UserInfo storage user = userInfo[msg.sender];
        require(user.lockEndTime < block.timestamp || unlockAll == true, "Still in lock");
        ...
        uint256 boosterPower;
        if (user.boosterTier == 0) {
            boosterPower = 0;
        } else if (user.boosterTier == 1) {
            boosterPower = booster1Power;
        } else if (user.boosterTier == 2) {
            boosterPower = booster2Power;
        } else if (user.boosterTier == 3) {
            boosterPower = booster3Power;
        }
        ...
        if (_amount > 0) {
            uint256 stakedTokenSupply = stakedToken.balanceOf(address(this));
            uint256 oldUserShares = user.shares;
            uint256 newUserAmount = user.amount - _amount;
            uint256 newUserShares = (newUserAmount * boosterPower *10**10) / PRECISION_FACTOR + newUserAmount;
            totalStakedTokens = stakedTokenSupply - _amount;
            totalStakedShares = totalStakedShares - oldUserShares + newUserShares;
            user.amount = newUserAmount;
            user.shares = newUserShares;
            if (user.amount == 0) {
    ...
    function unlockAllUsers(bool _unlockAll) external onlyOwner {
        unlockAll = _unlockAll;
    }

    function setDepositfee(uint16 _depositFeeBP) external onlyOwner {
    ...
    function pendingReward(address _user) external view returns (uint256) {
        ...
        if (block.number > lastRewardBlock && stakedTokenSupply != 0) {
            ...
            uint256 adjustedTokenPerShare = accTokenPerShare + (cakeReward * PRECISION_FACTOR) / totalStakedShares;
    ...
    function _updatePool() internal {
        ...
        uint256 multiplier = _getMultiplier(lastRewardBlock, block.number);
        uint256 cakeReward = multiplier * rewardPerBlock;
        accTokenPerShare = accTokenPerShare + (cakeReward * PRECISION_FACTOR) / totalStakedShares;
        lastRewardBlock = block.number;
    }
