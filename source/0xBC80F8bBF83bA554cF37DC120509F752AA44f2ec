...
contract Token is ERC20,Ownable,AccessControl {
    ...
    uint256 private maxAmount = 500*10**decimals();
    uint256 public lpAtAmount = 20*10**18; 
    ...
    constructor()ERC20("ccgds", "ccgds") {
        admin=0xeE7FA519133052e1668EFe8cDDD9627E752f03ca;
        address sentTo=0xc70E46C88EeC2E852A1ebAE35B7c944Dd57C8F41;
        //admin=msg.sender;
        fundAddr=0x71879fd0AcD085dFA22559F814486B0A37a54dE7;
        uint256 total=15*10**7*10**decimals();
        _mint(sentTo, total);
    ...
    function _transfer(
        ...
        if(from == uniswapV2Pair) {
            ...
            require(startTradeBlock>0, "not open");
            if(limited) require(amount <= maxAmount,"amount too max");
            uint256 lpBalance = IERC20(uniswapV2Pair).balanceOf(to);
            if(lpBalance>0){
                if(limited) require(super.balanceOf(to).add(amount) <= maxAmount,"balance of to address too max");
                super._transfer(from, to, amount);
                return;
            }
            
            uint256 fees = amount.mul(buyFee).div(100);
            uint256 lpfee = fees.div(7);
            super._transfer(from, address(this), lpfee);
            super._transfer(from, fundAddr, fees.sub(lpfee));
            fundCount += lpfee;
            //airdrop(address(this));
            if(limited) require(super.balanceOf(to).add(amount).sub(fees) <= maxAmount,"balance of to address too max");
        ...
        if(to == uniswapV2Pair) {
            ...
            require(startTradeBlock>0, "not open");
            uint256 lpBalance = IERC20(uniswapV2Pair).balanceOf(from);
            if(lpBalance>0){
                super._transfer(from, to, amount);
                return;
            }
            ...
            fundCount += lpfee;
            //airdrop(address(this));
            uint256 swapAmount = fundCount.div(2);
            swapUsdt(swapAmount,address(usdtHelper));
            
            fundCount = 0;
            uint256 BalanceofLp = IERC20(usdt).balanceOf(address(this));
            if(BalanceofLp >= lpAtAmount){
                addLiquidity(super.balanceOf(address(this)),BalanceofLp);
            }
            super._transfer(from, to, amount.sub(fees));
    ...
    function swapUsdt(uint256 tokenAmount,address to) private lockTheSwap {
        ...
        uint256 usdtAmount = IERC20(usdt).balanceOf(to);
        //uint256 lpfees = usdtAmount.mul(1).div(13);
        usdtHelper.withdrawToken(usdt, address(this),usdtAmount);
        //usdtHelper.withdrawToken(usdt, fundAddr,usdtAmount.sub(lpfees));
    }
    ...
    function setLimit(bool _isLimit) public onlyOwner{
        limited = _isLimit;
    }



    function setMinLpUsdt(uint256 amountUsdt) public onlyOwner{
        lpAtAmount = amountUsdt * 10**18;
    }
