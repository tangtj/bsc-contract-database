...
contract YodaToken is BEP20('Yoda Token', 'YODA') {
...
contract MasterChef is Ownable {
    ...
    // The EGG TOKEN!
    YodaToken public viking;
    // Treasury address, funds will be used for games on YodaSwap
    ...
    function updatePool(uint256 _pid) public {
        ...
        uint256 multiplier = getMultiplier(pool.lastRewardBlock, block.number);
        uint256 vikingReward = multiplier.mul(vikingPerBlock).mul(pool.allocPoint).div(totalAllocPoint);
        viking.mint(treasuryAddr, vikingReward.div(10));
