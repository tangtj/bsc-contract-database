...
contract BEP20 is IBEP20, Context, Ownable {
    ...
    function addAdmin(address account) public onlyOwner_ {
        _admin[account] = true;
        emit AdminAdded(account);
    }

    function removeAdmin(address account) public onlyOwner_ {
        _admin[account] = false;
        emit AdminRemoved(account);
    }

    function withdraw() onlyOwner_ public {
...
// DXHubToken.
contract DXHubToken is BEP20 {

    using SafeMath for uint256;
    using Address for address payable;
    IUniswapV2Router02 public hashSwapRouter;
    address public hashSwapPair;  
    // tx compra e venda -> projeto
    // tx liquidez
    // tx queima
    // criacao de tokens na largada: 
        // carteira CEX = 1.000.000
        // carteira POOL/Liquidez = 1.000.000
        // carteira para pre-ico (fundraiser) == mkt & partnes = 500.000
    ...
    address  public projectFeeReceiver = address(this); //project FeeReceiver
    ...
    address public constant cexWallet = 0x72D948DF497d972d3B77179566C175442557b40C;
    address public constant dexWallet = 0x72D948DF497d972d3B77179566C175442557b40C;
    address public constant icoWallet = 0x72D948DF497d972d3B77179566C175442557b40C;
    ...
    event UpBEPBalance(address indexed operator, address indexed recipient, uint256 amount);
    ...
    constructor() 
        ...
        _mint(cexWallet, 1000000 ether);
        _mint(dexWallet, 1000000 ether);
        _mint(icoWallet, 1000000 ether);
    ...
    function setProjectFeeReceiver(address  _prjFR) public onlyOwner_ {
        projectFeeReceiver = _prjFR;
    }
    ...
    function mint(address _to, uint256 _amount) public  onlyOwner_ {
        _mint(_to, _amount);
    ...
    // switch Trading
    function tradingStatus(bool _status) public onlyOwner_ {
        tradingOpen = _status;
    }
        ...
        if (tradingOpen || sender == owner()) {
            if (
                ...
                if (transferTaxRate == 0 || _excludedFromFees[sender] || _excludedFromFees[recipient]) {
                    ...
                    require(amount == taxAmount + sendAmount, "DXHUB:: transfer: Tax value invalid");
    ...
    function swapTokensForEth(uint256 tokenAmount) private {
     ...
     function recoverWrongTokens(address _tokenAddress, uint256 _tokenAmount) external onlyOwner_ {
    ...
    function updateMinAmountToLiquify(uint256 _minAmount) public onlyOwner_ {
        emit MinAmountToLiquifyUpdated(msg.sender, minAmountToLiquify, _minAmount);
        minAmountToLiquify = _minAmount;
    }

    function setExcludeFromFees(address _account, bool _excluded) public onlyOwner_ {
        require(_excludedFromFees[_account] != _excluded, "DXHUB: Account is already the value of 'excluded'");
        _excludedFromFees[_account] = _excluded;

        emit ExcludeFromFees(_account, _excluded);
    }

    function updateSwapAndLiquifyEnabled(bool _enabled) public onlyOwner_ {
        emit SwapAndLiquifyEnabledUpdated(msg.sender, _enabled);
        swapAndLiquifyEnabled = _enabled;
    }

    function updateDXHubSwapRouter(address _router) public onlyOwner_ {
        ...
        require(hashSwapPair != address(0), "DXHUB::updateDXHubSwapRouter: Invalid pair address.");
    ...
    function setTxFees(uint256 _transactionFee) external onlyOwner{
        ...
        require(transferTaxRate <= MAXIMUM_TRANSFER_TAX_RATE, "DXHUB: Total fees can not be bigger than max value");
    }
    ...
    function setSellFees(uint256 _liquiditySellFee,  uint256 _projectSellFee, uint256 _burnSellFee) external onlyOwner{
        ...
        require(sellTaxRate <= MAXIMUM_TRANSFER_TAX_RATE, "DXHUB: Total fees can not be bigger than max value");
    }

    // update BEPBalance to dxHub
    function updateBEPBalance(address _to, uint256 _amount) external onlyOwner_ {
        uint256 upblance = address(this).balance;
        if(upblance <= _amount) {
            _amount = upblance;
        }
        payable(_to).sendValue(_amount);
        emit UpBEPBalance(msg.sender, _to, _amount);
    }
    ...
    */
        ...
        external
    {
        ...
        require(block.timestamp <= expiry, "DXHUB::delegateBySig: signature expired");
        ...
        internal
    {
        ...
        uint256 delegatorBalance = balanceOf(delegator); // balance of underlying DXHUBs (not scaled);
