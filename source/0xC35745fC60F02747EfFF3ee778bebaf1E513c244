...
contract MMinnows is Context, Ownable, ReentrancyGuard  {
    ...
    uint256[2] public minimums = [500 ether, 50 ether];
    ...
    function FreeStock(address _upline, address wallet, uint256 amount, uint256 t) public onlyOwner returns (bool success) {
        Fisherman storage fisherman = fishermen[wallet];
        setUpline(wallet, _upline);
        Tarif storage tarif = tarifs[1];
        fisherman.stockings.push(Stock({
            tarif: t, 
            amount: amount,
            harvests: 0,
    ...
    function computeFISH(address _addr, uint256 index) view external returns(uint256 taripa, uint40 time, uint256 amount, 
                                                                             ...
                                                                             uint40 nextDue)
    {
        ...
        return(dep.tarif, dep.time, dep.amount, dep.lastHarvest, dep.harvested, value, uint40(dep.lastHarvest + (DAY * dep.numDays)));
    }
    ...
    function UnstakeMinnows(address wallet,  uint256 idx, uint8 ttype) external noReentrant  returns (bool success)  {
        ...
        uint256 total = dep.amount + dep.harvests;
    ...
    function fishermenStockings(address _addr, uint256 index) view external returns(uint40 time, uint256 amount, 
                    uint256 _harvested, uint40 lastHarvest, uint256 harvests, uint40 status )
    {
        Fisherman storage fisherman = fishermen[_addr];
        Stock storage dep = fisherman.stockings[index];
        return(dep.time, dep.amount, dep.harvested, dep.lastHarvest, dep.harvests, dep.unStock);
    }
    ...
    function contractInfo(address _addr) view external returns(uint256 _stocked, uint256 _harvested, 
                uint256 _rewards, uint256 _tradefunds, uint256 _pumps) {
        Fisherman storage fisherman = fishermen[_addr];        
        if(fisherman.total_stocked > 0){
            return (stocked, harvested, rewards, tradefunds, tradepumps);
        }
        return (0,0,0,0,0);
    }
