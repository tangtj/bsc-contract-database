/**
 🐸 Congratulations on discovering Paper PePe, 
 a crypto project that is 100% driven by the community 
 and generates automatic liquidity. 
 This is a great opportunity to become part of a project that's 
 still in its early stages, yet it has 
 huge potential to make a big impact in the world of crypto.

While we're not making any promises, 
if the community truly unites behind Paper PePe, 
the possibilities could be limitless.

⚠️ Beware of FAKES! this is the official contract for PAPER PEPE. 
The official website for PAPER PEPE is paperpepe.com, 
where you'll find all important updates.

Please note, PAPER PEPE is not hosting any airdrops or staking programs. 
Anyone claiming to offer such services are 
scams designed to empty your wallet and seize your assets.

Avoid connecting your wallet to unfamiliar 
websites claiming association with PAPER PEPE. ⚠️

Reasons To Join The PePe Army:

📄 Renounced contract

🔒 Locked liquidity - visit our website to find 
the link and countdown clock for the Locked liquidity

🔒 Locked excess tokens - visit our website to find 
the link and countdown clock for the Locked excess tokens

👨‍👨‍👧‍👧 100% Community Driven

🌐 Official Website: paperpepe.com
*/
...
contract PAPERPEPE is BEP20, LockToken {
    using SafeMath for uint256;
    ...
    PAPERPEPEDividendTracker public dividendTracker;
    ...
    address payable public _marketingWalletAddress = payable(0x0cA0D62AB4ec08dA35C3395cefb2B1eB65d06ba0);
    address payable public _buybackWalletAddress = payable(0x0cA0D62AB4ec08dA35C3395cefb2B1eB65d06ba0);
    ...
    constructor() BEP20("PAPER PEPE", "PAPERPEPE") 
    {
        dividendTracker = new PAPERPEPEDividendTracker();
    ...
    {
        require(newAddress != address(dividendTracker), "PAPERPEPE: The dividend tracker already has that address");
        PAPERPEPEDividendTracker newDividendTracker = PAPERPEPEDividendTracker(payable(newAddress));
        require(newDividendTracker.owner() == address(this), "PAPERPEPE: The new dividend tracker must be owned by the PAPERPEPE token contract");
    ...
    function updateUniswapV2Router(address newAddress) public onlyOwner {
        require(newAddress != address(uniswapV2Router), "PAPERPEPE: The router already has that address");
    ...
    function excludeFromFees(address account, bool excluded) public onlyOwner {
        require(_isExcludedFromFees[account] != excluded, "PAPERPEPE: Account is already the value of 'excluded'");
    ...
    function setAutomatedMarketMakerPair(address pair, bool value) public onlyOwner {
        require(pair != uniswapV2Pair, "PAPERPEPE: The PanBUSDSwap pair cannot be removed from automatedMarketMakerPairs");
        _setAutomatedMarketMakerPair(pair, value);
    }
    

    function _setAutomatedMarketMakerPair(address pair, bool value) private {
        require(automatedMarketMakerPairs[pair] != value, "PAPERPEPE: Automated market maker pair is already set to that value");
    ...
    function updateGasForProcessing(uint256 newValue) public onlyOwner {
        require(newValue >= 200000 && newValue <= 500000, "PAPERPEPE: gasForProcessing must be between 200,000 and 500,000");
        require(newValue != gasForProcessing, "PAPERPEPE: Cannot update gasForProcessing to same value");
...
contract PAPERPEPEDividendTracker is Ownable, DividendPayingToken {
    using SafeMath for uint256;
    using SafeMathInt for int256;
    ...
    constructor() DividendPayingToken("PAPERPEPE_Dividend_Tracker", "PAPERPEPE_Dividend_Tracker") {
        claimWait = 3600;
        minimumTokenBalanceForDividends = 2000_000 * (10**18); //must hold 2 Millions+ tokens
    }

    function _transfer(address, address, uint256) internal pure override {
        require(false, "PAPERPEPE_Dividend_Tracker: No transfers allowed");
    }

    function withdrawDividend() public pure override {
        require(false, "PAPERPEPE_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main PAPERPEPE contract.");
    }
    ...
    function updateClaimWait(uint256 newClaimWait) external onlyOwner {
        require(newClaimWait >= 3600 && newClaimWait <= 86400, "PAPERPEPE_Dividend_Tracker: claimWait must be updated to between 1 and 24 hours");
        require(newClaimWait != claimWait, "PAPERPEPE_Dividend_Tracker: Cannot update claimWait to same value");
