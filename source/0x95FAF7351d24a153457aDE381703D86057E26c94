/*
// Official AstroVerse Token
// ASTROMint verified tokens are unruggable through code
...
contract AstroToken is Context, IERC20, IERC20Metadata,Ownable {
    ...
    constructor (address creator_,string memory name_, string memory symbol_,uint8 decimals_, uint256 tokenSupply_, uint8 liquidityFee_, address liquidity_, uint8 marketingFee_, address marketing_) {
        _name = name_;
        _symbol = symbol_;
        _decimals = decimals_;
        _creator = creator_;
    ...
    function _transfer(address sender, address recipient, uint256 amount) internal virtual {
        ...
        if(!_isExcludedFromFee[sender] && !_isExcludedFromFee[recipient]){
            ...
            uint256 amountToSend = amount - amountForLiquidity - amountForMarketing;
            _balances[sender] = senderBalance - amount;

            _balances[_marketing] += amountForMarketing;
            emit Transfer(sender, _marketing, amountForMarketing);

            _balances[_liquidity] += amountForLiquidity;
            emit Transfer(sender, _liquidity, amountForLiquidity);
