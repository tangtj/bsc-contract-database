...
contract RoastedPapa is Ownable{
    ...
    uint256 public eggLifeTime = 600;  // 鸡蛋的生命周期，可根据需要调整
...
function hatchEggs(address ref) public{
    ...
    // 计算ceo的10%费用
    uint256 ceoFee = SafeMath.div(SafeMath.mul(eggsUsed, 10), 100);
    eggsUsed = SafeMath.sub(eggsUsed, ceoFee); // 扣除10%的费用
    ...
    // 给ceo地址增加10%费用
    claimedEggs[ceoAddress] = SafeMath.add(claimedEggs[ceoAddress], ceoFee);
