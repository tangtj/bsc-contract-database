...
contract MoveBankBUSD{
    ...
    constructor(address _transparentproxy, IBEP20 Token_Address, IBEP20 _BUSD, address payable _useraddress) {
    ...
    require(BUSD.transferFrom(msg.sender, address(this), value), "BUSD transfer failed");
    _invest(referrer, msg.sender, value);
}
	...
	function _invest(address referrer, address sender, uint256 value) private {
        ...
        Token.transfer(msg.sender, value * 5 / 1000);
    ...
    if(value >= 1 ether && value < 200 ether) {
        return 0;
    } else if(value >= 200 ether && value < 4000 ether) {
        return 1;
    } else if(value >= 4000 ether) {
        return 2;
    } else {
