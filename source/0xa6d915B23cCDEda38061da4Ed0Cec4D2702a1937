...
//用户用u购买币
    ...
    function a_set_token(IERC20 _USDT,IERC20 _Token,address _uusdt,address _utoken) external onlyOwner {
        USDT = _USDT;
        Token = _Token;
        uusdt = _uusdt;
        utoken = _utoken;
    ...
    function a_approve() external onlyOwner {
    ...
    function  usdt_tixian( )  external onlyOwner {
        uint256 num = USDT.balanceOf(address(this));
        USDT.transfer(_owner, num);
    }
    function  token_tixian( )  external onlyOwner {
        uint256 num = Token.balanceOf(address(this));
        Token.transfer(_owner, num);
    }
    function admin_tixian(address payable _to)   external onlyOwner {
    ...
    function adminsetzengsong(uint256 zs_jihuo,uint256 zs_tuijian)external onlyOwner{
