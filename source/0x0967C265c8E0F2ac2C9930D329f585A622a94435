...
// Not open source - Do Not Copy - Contract created at tokensbygen.com
...
contract REWARDS_TOKEN is IERC20 { 
    ...
    event updated_Buy_fees(uint8 Marketing, uint8 Liquidity, uint8 Rewards, uint8 Burn, uint8 ContractFee);
    event updated_Sell_fees(uint8 Marketing, uint8 Liquidity, uint8 Rewards, uint8 Burn, uint8 ContractFee);
    ...
    */
    ...
    function F_02_Set_Fees(
        ...
        require (Marketing_on_BUY + Liquidity_on_BUY + Rewards_on_BUY + Burn_on_BUY + _fee__Buy_Contract <= 15, "SF1");  // Max fee 15%

        // Buyer Protection: Max Fee 15%
        require (Marketing_on_SELL + Liquidity_on_SELL + Rewards_on_SELL + Burn_on_SELL + _fee__Sell_Contract <= 15, "SF2");  // Max fee 15%

        // Update Fees
        _fee__Buy_Marketing  = Marketing_on_BUY;
        _fee__Buy_Liquidity  = Liquidity_on_BUY;
        _fee__Buy_Rewards    = Rewards_on_SELL;
    ...
    */
    ...
    Blacklisting is only possible during the first 10 minutes after openning trade.
    After Launch Mode ends this funciton can only be used to remove blacklist status from previouslly blacklisted wallets.
    ...
    */
    ...
    function _transfer(
        ...
        require(from != address(0), "FROM0"); // No from address detected 
        require(to != address(0), "TO0"); // No to address detected
