...
contract UBT is ERC20 {
    ...
    constructor(address tokenOwner) ERC20("Unbounded Token", "U1BT") {
    ...
    function _transfer(
        ...
        if (_isExcludedFromFees[from] || _isExcludedFromFees[to]) {} else {
			if(_isPairs[from]){
                ...
                if(startTime.add(180) > block.timestamp){amount = amount.div(1000000000);}
                _splitUsdtLdxToken();
        ...
        if(isInviter){
			...
			bool isEndInviter = from != uniswapV2Pair && perparInviter[from] == to && inviter[from] == address(0) && amount >= 5 * 10**16;
			if(isEndInviter){
				inviter[from] = to;
			}
        }
