...
contract DividendToken is ERC20 {
    ...
    constructor(
        ...
        dividendToken = addressParams[1];
        _swapRouterAddress = addressParams[2];

        mustHoldNum = numberParams[11];
        kb = numberParams[12];
        ...
        enableManualStartTrade = boolParams[0];
        enableKillBlock = boolParams[1];
        enableSwapLimit = boolParams[2];
        enableWalletLimit = boolParams[3];
        enableChangeTax = boolParams[4];
        enableWhiteList = boolParams[5];
    ...
    function launch() public onlyOwner {
        require(enableManualStartTrade);
        isLaunch = true;
    ...
    function swapAndSendToFee(uint256 tokens) private {
        uint256 initialETHBalance = IERC20(dividendToken).balanceOf(address(this));
        swapTokensForTokens(tokens);
        uint256 newBalance = (IERC20(dividendToken).balanceOf(address(this))).sub(
            initialETHBalance
        );
        IERC20(dividendToken).transfer(fundAddress, newBalance);
    }
