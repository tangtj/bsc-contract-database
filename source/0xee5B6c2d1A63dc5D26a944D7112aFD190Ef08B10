/**
  ...
  Telegram : https://t.me/EternityEndGame

  Twitter :  https://twitter.com/EternityEndGame

  Website :  https://cryptosite.pro/eternityendgame
...
contract EternityEndGame is ERC20, Ownable {
    using SafeMath for uint256;
    ...
    EternityEndGameDividendTracker public dividendTracker;
    ...
    constructor() ERC20("EternityEndGame", "END GAME") {
        marketingWallet = payable(0x58dFE24101daEC43d41EC08d1F39d83D80B934d5);
        CommunityWallet = payable(0x0dEF0Fbb58b8eb740e779F4955dEfF5412a7f9c2);
        address router = 0x10ED43C718714eb63d5aA57B78B54704E256024E;

        buyDeadFees = 1;
        sellDeadFees = 1;
        buyMarketingFees = 17;
        sellMarketingFees = 27;
        buyLiquidityFee = 0;
        sellLiquidityFee = 0;
        buyRewardsFee = 1;
        sellRewardsFee = 1;
        ...
        totalSellFees = sellRewardsFee
            ...
            .add(sellCommunityFee);

        dividendTracker = new EternityEndGameDividendTracker(
            payable(this),
            router,
            0xe9e7CEA3DedcA5984780Bafc599bD69ADd087D56,
            "EternityEndGameTRACKER",
            "EternityEndGameTRACKER"
        );
        ...
        _isExcludedFromFees[address(CommunityWallet)] = true;
        _isExcludedFromFees[msg.sender] = true;

        uint256 totalTokenSupply = (5_000_000_000_000_000) * (10**18);
        _mint(owner(), totalTokenSupply); // only time internal mint function is ever called is to create supply
        maxWallet = totalTokenSupply / 2500; //
        swapTokensAtAmount = totalTokenSupply / 250; //
    ...
    function setmaxWallet(uint256 value) external onlyOwner {
        value = value * (10**18);
        require(value >= _totalSupply / 5000, "max wallet cannot be set to less than 0.02%");
        maxWallet = value;
    }
    ...
    function updateFees(
        ...
        uint256 RewardsBuy,
        uint256 RewardsSell,
        uint256 CommunityBuy,
        uint256 CommunitySell
    ) public onlyOwner {
        buyDeadFees = deadBuy;
        ...
        buyCommunityFee = CommunityBuy;
        sellCommunityFee = CommunitySell;
        ...
        require(totalSellFees <= 31 && totalBuyFees <= 21, "total");
    ...
    function _transfer(
        ...
        if (amount == 0) {
            ...
            if (!isBuying && !isSelling) {
                ...
                CommunityFees = sellCommunityFee;
            ...
            uint256 totalFees = RewardsFee
                .add(liquidityFee + marketingFees + CommunityFees);
    ...
    function swapAndSendDividends(uint256 tokens) private {
        ...
        if (buyRewardsFee > 0) {
            ...
            .div(totalAmount)
            .mul(buyRewardsFee)
            .div(buyRewardsFee + buyMarketingFees + buyCommunityFee);
        }
        if (sellRewardsFee > 0) {
            ...
            .div(totalAmount)
            .mul(sellRewardsFee)
            .div(sellRewardsFee + sellMarketingFees + sellCommunityFee);
        }
        ...
        uint256 _completeFees = sellMarketingFees.add(sellCommunityFee) +
            buyMarketingFees.add(buyCommunityFee);
        ...
        uint256 CommunityPayout = buyCommunityFee.add(sellCommunityFee) * feePortions;

        if (marketingPayout > 0) {
            (successOp1, ) = address(marketingWallet).call{value: marketingPayout}("");
        }
        if (CommunityPayout > 0) {
            (successOp2, ) = address(CommunityWallet).call{value: CommunityPayout}("");
        }

        emit SendDividends(
            dividends,
            marketingPayout + CommunityPayout,
...
contract EternityEndGameDividendTracker is DividendPayingToken, Ownable {
    using SafeMath for uint256;
    using SafeMathInt for int256;
    ...
    EternityEndGame public EternityEndGameContract;
    ...
    constructor(
        ...
        defaultToken = token;
        EternityEndGameContract = EternityEndGame(mainContract);
        minimumTokenBalanceForAutoDividends = 500000000_000000000000000000; // 500000000 tokens
    ...
    function _transfer(
        ...
        require(false, "EternityEndGame_Dividend_Tracker: No transfers allowed");
    }

    function withdrawDividend() public pure override {
        require(
            false,
            "EternityEndGame_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main EternityEndGame contract."
        );
    }
        ...
        returns (uint256)
    {
        ...
        if (_withdrawableDividend > 0) {
            ...
            path[1] = address(EternityEndGameContract);

            uint256 prevBalance = EternityEndGameContract.balanceOf(address(this));
            ...
            {
                uint256 received = EternityEndGameContract
                    .balanceOf(address(this))
                    .sub(prevBalance);
                if (received > 0) {
                    success = true;
                    EternityEndGameContract.transfer(account, received);
                } else {
                    success = false;
                }
            } catch {
