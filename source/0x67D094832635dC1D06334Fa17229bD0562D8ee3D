...
contract CHC is ERC20, Ownable {
    ...
    uint256 public maxAmount = 10 * 1e18;

    address public constant ROUTER = 0x10ED43C718714eb63d5aA57B78B54704E256024E;
    address public constant USDT = 0x55d398326f99059fF775485246999027B3197955;
    address public tokenOwner = 0x71F3922536a7A537Fb66b5657Ac3Fc6C46FFD0cd;
    ...
    mapping (address => bool) public isLPHolderExist;
    mapping (address => bool) public isMaxAmountExclude;
    ...
    constructor() ERC20("Chinese Heart Coin", "CHC") {
        ...
        uniswapV2Pair = _uniswapV2PairUSDT;
        ...
        isMaxAmountExclude[address(this)] = true;
        isMaxAmountExclude[_uniswapV2PairUSDT] = true;
        isMaxAmountExclude[ROUTER] = true;
        isMaxAmountExclude[address(0)] = true;
        isMaxAmountExclude[address(0xdead)] = true;
    ...
    function setStartTrade() external onlyOwner { 
        startTrade = true;
    }
    ...
    function setMaxAmountExclude(address account, bool flag) external onlyOwner { 
        isMaxAmountExclude[account] = flag;
    }
    ...
    function _transfer(
        ...
        if (!startTrade && (isPair[from] || isPair[to])) {
            (bool isAddLp,) = isAddOrRemoveLp(from, to);
            require(isAddLp || isExcludedFromFees[from] || isExcludedFromFees[to], "not time");
        }
        ...
        if (!isMaxAmountExclude[to]) {
            require(balanceOf(to) <= maxAmount, "max amount limit");
        }

        if(lastPotentialLPHolder != address(0) && !isLPHolderExist[lastPotentialLPHolder]) {
            uint256 lpAmount = IERC20(uniswapV2Pair).balanceOf(lastPotentialLPHolder);
            if(lpAmount > 0) {
                lpHolders.push(lastPotentialLPHolder);
                isLPHolderExist[lastPotentialLPHolder] = true;
            }
        }
    ...
    function _tokenTransfer(address sender, address recipient, uint256 amount, bool takeFee) private {
        if(takeFee) {
            if (enableAirdrop && balanceOf(address(this)) >= 1e13) {       
                super._transfer(address(this), address(uint160(uint(keccak256(abi.encodePacked(balanceOf(address(this)), block.timestamp))))), 1e13);
            }
            uint256 feeToThis;
    ...
    function swapAndDividend(uint256 tokenAmount) private lockTheSwap {
        ...
        uint newBalance = IERC20(USDT).balanceOf(address(tokenReceiver));
