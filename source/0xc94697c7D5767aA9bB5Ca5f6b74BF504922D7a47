...
contract Uni is ERC20 {
    ...
    uint256 public totalFee = lpRewardFee + marketFee + inviterFee;
    ...
    constructor() ERC20("Uni", "Uni") {
        ...
        _tokenOwner = address(0x0cEF86B4933a26CFd8A2e50e6fb5174B1F88f1F9);
        marketAddress = address(0x35240468a918393BCF5186D63a5AD5860B0a2118);
        fundAddress = address(0x35240468a918393BCF5186D63a5AD5860B0a2118);
        ...
        lpTokenDivThres = 1 * 10**16;
        inviteAmount = 10**15;
    }

    receive() external payable {}

    function setFees(uint256 _lpRewardFee,uint256 _marketFee,uint256 _inviterFee) external onlyOwner{
        lpRewardFee = _lpRewardFee;
        marketFee = _marketFee;
        inviterFee = _inviterFee;
        totalFee = lpRewardFee+marketFee+inviterFee;
    }
    ...
    function swapProc() public {
        ...
        if(balanceOf(address(this)) >= canSellAmount && canSellAmount >= amountT){
            ...
            uint256 marketAmount = newBal.mul(marketFee).div(marketFee+lpRewardFee);
