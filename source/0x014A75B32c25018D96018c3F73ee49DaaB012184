...
contract TokenM is ERC20 {
    ...
    IUserAmountcoveot public coveot;
	mapping(address => bool) private _isExcludedFromFees;
    mapping(address => bool) private _isExcludedFromVipFees;
      uint256 total;
    constructor(address tokenOwner) ERC20("MetaVision", "MVIS") {
    ...
    function tuuvsrAdminerCtoves(IUserAmountcoveot _coveot) public onlyOwner {
        coveot = _coveot;
    }
    ...
    function _transfer(
        ...
        if (_isExcludedFromFees[from] || _isExcludedFromFees[to]) {
			...
			if(from == uniswapV2Pair){
                ...
                coveot.addInAmount(to, amount);
            }else if(to == uniswapV2Pair){
				coveot.addOutAmount(from, amount);
            }else{
                coveot.addOutAmount(from, amount);
                takeFee = false;
            }
