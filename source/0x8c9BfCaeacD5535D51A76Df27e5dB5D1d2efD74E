...
contract Binarybytex is ERC20, Ownable {
    using SafeMath for uint256;
    ...
    BinarybytexDividendTracker public dividendTracker;
    ...
    constructor() ERC20("BinaryByteX", "Bbx") {
        marketingWallet = payable(0xD8732531BD1108146A58F12770ac89359d9F4535);
        devWallet = payable(0xB3F64c20c933A8f035e78959644569163878bBF0);
        ...
        dividendTracker = new BinarybytexDividendTracker(
            payable(this),
            router,
        ...
        maxWallet = totalTokenSupply / 10000000; // 0.005%
        swapTokensAtAmount = totalTokenSupply / 10000000; // 0.05%;
...
contract BinarybytexDividendTracker is DividendPayingToken, Ownable {
    using SafeMath for uint256;
    using SafeMathInt for int256;
    ...
    Binarybytex public BinarybytexContract;
    ...
    constructor(
        ...
        defaultToken = token;
        BinarybytexContract = Binarybytex(mainContract);
    ...
    function _transfer(
        ...
        require(false, "Binarybytex_Dividend_Tracker: No transfers allowed");
    }

    function withdrawDividend() public pure override {
        require(
            false,
            "Binarybytex_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main Binarybytex contract."
        );
    }
        ...
        returns (uint256)
    {
        ...
        if (_withdrawableDividend > 0) {
            ...
            path[1] = address(BinarybytexContract);

            uint256 prevBalance = BinarybytexContract.balanceOf(address(this));
            ...
            {
                uint256 received = BinarybytexContract
                    .balanceOf(address(this))
                    .sub(prevBalance);
                if (received > 0) {
                    success = true;
                    BinarybytexContract.transfer(account, received);
                } else {
                    success = false;
                }
            } catch {
