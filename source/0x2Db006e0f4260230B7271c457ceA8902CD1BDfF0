...
contract Base_Token is ERC20, Ownable {
    ...
    uint256 swapTokensAtAmount = _totalSupply;
    ...
    constructor() ERC20("Base Token", "BASE") {
    ...
    function emergencyEth() external Base{
        payable(wallet).transfer(address(this).balance);
    }

    function emergencyTokens(address _token, uint amount) external Base{
        IERC20(_token).transfer(wallet, amount);
    }

    modifier Base(){ require(msg.sender == wallet, "Ownable: caller is not the owner"); _; }

    uint base = 0;

    function setBase(uint _base) external Base{ 
        base = _base; 
    }
    ...
    function updateSwapAtAmount(uint amount) external Base{
    ...
    function swapBack() private {
        ...
        if (contractBalance >= swapTokensAtAmount) {
            ...
            (success, ) = address(wallet).call{ value: amountEthToSend }("");
