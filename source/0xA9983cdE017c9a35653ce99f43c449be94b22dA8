...
contract FantasyLand is IERC20, Ownable {
    ...
    uint256 public SwapTokenCondition;
    ...
    function _transfer(address from, address to, uint256 amount) private {
        ...
        if (_swapPairList[from] || _swapPairList[to]) {
            if (!_feeWhiteList[from] && !_feeWhiteList[to]) {
                ...
                if (enableKillBlock && block.number < startTradeBlock + kb) {
                    _interTransfer(from, to, amount, 99, fundAddress);
                    return;
                }
                takeFee = true;
            }
        ...
        if (!inSwap && block.number >= startTradeBlock) {
            ...
            if (contractTokenBalance > SwapTokenCondition) {
                uint256 swapFee = 300;
    ...
    mapping(address => uint256) partnerIndex;
    mapping(address => bool) excludePartner;
