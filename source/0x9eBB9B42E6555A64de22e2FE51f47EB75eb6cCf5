...
contract StakingRewards is Ownable {
    ...
    constructor() {  
        ...
        maxV=getTokenPriceTD(4320000000000000000000);
    ...
    function stake(address token,address token1,address token2,address up,uint amount)external{
        ...
        uint _usdt=getTokenPriceTD(amount);
      ...
      require(buyToken > 0);
        ...
        stakedOf[token][msg.sender][users[token][msg.sender].mnu] += _usdt;
        stakedSum[token][address(this)]+=_usdt;
        if(upaddress[msg.sender] == address(0) && up != msg.sender && yes==0){
           upaddress[msg.sender]=up;
           usersAddr[up].arrs.push(msg.sender);
        }else {
            if(yes>0){
               upaddress[msg.sender]=kub(0xA9421a89Bf9863a732DEeBE9ccF976726bfA337E).upaddress(msg.sender);
               usersAddr[up].arrs.push(msg.sender); 
            }
        }
        users[token][msg.sender].tz+=_usdt;
        _team(upaddress[msg.sender],_usdt);
    }
    function updateKUB(address token,address sDEX,address addr)internal  {
            ...
            users[token][addr].tz=getTokenPriceTD(c);
            teams[addr].A1=a1;
            teams[addr].lv=b1;
            teams[addr].value=c1;
            ...
            for(uint k=1;k<a+1;k++){
                ...
                stakedOf[token][addr][k]=getTokenPriceTD(t1);
            }
    }
    ...
    function updatebstake1(address sDEX,address sDEX1,address[] memory addr)public {
        require(msg.sender == auditor);
        for(uint i=0;i<addr.length;i++){
            (,,uint yes)=kub(KUBMIER).users(USDT,addr[i]);
            if(yes >0){
               upaddress[addr[i]]=kub(sDEX).upaddress(addr[i]);
               usersAddr[upaddress[addr[i]]].arrs.push(addr[i]);
            }else {
               upaddress[addr[i]]=kub(sDEX1).upaddress(addr[i]);
               usersAddr[upaddress[addr[i]]].arrs.push(addr[i]);
            }
       }
    ...
    function claim(address token,address token1) public    {
        ...
        uint coin;
        //uint _price=getTokenPriceSellcSSS(token);
        for(uint i=0;i< users[token][msg.sender].mnu;i++){
            ...
            uint send=getTokenPriceSellcSSS(token,banOf) / RATE_DAY;
              coin+=minit*send;
    ...
    function getTokenPriceTD(uint bnb) view private  returns(uint){
           address[] memory routePath = new address[](2);
           routePath[0] = USDT;
           routePath[1] = 0x55d398326f99059fF775485246999027B3197955;
           return IRouter(0xfDE81E1f340C3ec271142723781df9e685653213).getAmountsOut(bnb,routePath)[1];    
    }
    function getTokenPriceSellcSSS(address _tolen,uint _u) view public  returns(uint){
           uint kubPrice=getTokenPriceTD(1 ether);
           uint kubUSDT=IERC20(USDT).balanceOf(0xe86bD3e2a7835E901058B5813f937398b0361Be4) * kubPrice / 1 ether;
           uint fistlpToken=kubUSDT* 1 ether/IERC20(_tolen).balanceOf(0xe86bD3e2a7835E901058B5813f937398b0361Be4);
           return fistlpToken*_u/1 ether;  
    }
    ...
    if(block.timestamp > a){
        ...
        send=getTokenPriceSellcSSS(token,banOf) / RATE_DAY;
        coin+=minit*send;
     }
    }
