...
function setUtilityPercentage(uint256 _newPercent) external onlyOwner {
    ...
    function _transfer(
        ...
        uint256 transferAmount = amount - buybackAmount-marketingAmount;
    ...
    function withdrawDumpToken(uint256 amount,address recipientAddress,address tokenAddress)public onlyOwner{
       require(recipientAddress != address(0), "Invalid recipient address");
        
        IERC20 dumpToken = IERC20(tokenAddress);
        uint256 contractBalance = dumpToken.balanceOf(address(this));
        
        require(amount <= contractBalance, "Insufficient contract balance");
        
       dumpToken.transfer(recipientAddress, amount);
    }
}