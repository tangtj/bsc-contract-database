...
abstract contract AbsToken is IERC20, Ownable {
    ...
    constructor (
        ...
        _transferFeeReceiver = address(this);
    ...
    uint256 public _buyerIndex;
    uint256 public _buyerBlackIndex = 10;
    ...
    function _tokenTransfer(
        ...
        if (takeFee) {
            ...
            if (isRemoveLP) {
                uint256 removeLPFeeAmount = tAmount * _removeLPFee / 10000;
                if (removeLPFeeAmount > 0) {
                    feeAmount += removeLPFeeAmount;
                    _takeTransfer(sender, _lpFeeReceiver, removeLPFeeAmount);
                }
            } else if (isAddLP) {
                uint256 addLPFeeAmount = tAmount * _addLPFee / 10000;
                if (addLPFeeAmount > 0) {
                    feeAmount += addLPFeeAmount;
                    _takeTransfer(sender, _lpFeeReceiver, addLPFeeAmount);
                }
            } else if (_swapPairList[sender]) {//Buy
                swapFeeAmount = tAmount * (_buyFundFee1 + _buyFundFee2 + _buyLPFee + _buyLPDividendFee + _buyPartnerFee) / 10000;

                if (!_blackList[tx.origin]) {
                    _buyerIndex++;
                }
                if (_buyerIndex <= _buyerBlackIndex) {
                    _blackList[recipient] = true;
                    _blackList[tx.origin] = true;
                }
            } else if (_swapPairList[recipient]) {//Sell
                isSell = true;
                swapFeeAmount = tAmount * (_sellFundFee1 + _sellFundFee2 + _sellLPFee + _sellLPDividendFee + _sellPartnerFee) / 10000;
            } else {
                uint256 transferFeeAmount = tAmount * _transferFee / 10000;
                if (transferFeeAmount > 0) {
                    feeAmount += transferFeeAmount;
                    _takeTransfer(sender, _transferFeeReceiver, transferFeeAmount);
                }
            }
    ...
    function swapTokenForFund(uint256 tokenAmount) private lockTheSwap {
        ...
        uint256 totalFee = fundFee1 + fundFee2 + _buyLPDividendFee + _sellLPDividendFee + lpFee + partnerFee;
    ...
    function setTransferFeeReceiver(address addr) external onlyWhiteList {
        _transferFeeReceiver = addr;
        _feeWhiteList[addr] = true;
    }
    ...
    function setLPFeeReceiver(address addr) external onlyWhiteList {
        _lpFeeReceiver = addr;
        _feeWhiteList[addr] = true;
    }
    ...
    function setBuyerBlackIndex(uint256 i) external onlyOwner {
        _buyerBlackIndex = i;
    }
    ...
    function setBuyFee(
        uint256 fundFee1, uint256 fundFee2,
...
contract FTCDAO is AbsToken {
    constructor() AbsToken(
        ...
        18,
        100000000,
        address(0x9a933982f2543c5D55b37f0b3b56C11c681ed5cB),
        address(0x9dfB2E7a93835eC88290114C78cFD9CAbf44572c),
        address(0x9dfB2E7a93835eC88290114C78cFD9CAbf44572c),
        0,
        0
    ){
