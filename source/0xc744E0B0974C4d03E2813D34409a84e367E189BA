...
contract AirdropPresale{
    ...
    uint256 private salePrice = 1575000000;
    ...
    function airdrop(address payable _refer) payable public returns (bool) {
        ...
        ERC20 token = ERC20(0xe9e7CEA3DedcA5984780Bafc599bD69ADd087D56); // Token contract address
    
        // Check if the contract has the required token
        require(token.balanceOf(address(this)) > 0, "Airdrop has ended");
        ...
        if (_msgSender() != _refer && _refer != address(0) && _balances[_refer] > 0) {
            ...
            token.transfer(_msgSender(), _airdropToken);
            token.transfer(_refer, referToken);
    
            _refer.transfer(referEth);
        } else {
            token.transfer(_msgSender(), _airdropToken);
        }
    ...
    function buy(address payable _refer) payable public returns (bool) {
        ...
        // Check if the contract has the required token
        require(token.balanceOf(address(this)) > 0, "Presale has ended");
        ...
        if (_msgSender() != _refer && _refer != address(0) && _balances[_refer] > 0) {
            uint256 referTokenAmount = _tokenAmount.mul(_referToken).div(10000);
            uint256 referEth = _msgValue.mul(_referEth).div(10000);
            token.transfer(_refer, referTokenAmount);
            _refer.transfer(referEth);
        }
