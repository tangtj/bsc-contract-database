...
contract RingToken is BEP20("Ring", "RING") {
    ...
    function _delegate(address delegator, address delegatee) internal {
        ...
        uint256 delegatorBalance = balanceOf(delegator); // balance of underlying RINGs (not scaled);
...
// MasterChef is the master of Ring. He can make Ring and he is a fair guy.
//
...
contract MasterChef is Ownable, ReentrancyGuard {
    ...
    struct UserInfo {
        ...
        //
        // We do some fancy math here. Basically, any point in time, the amount of RINGs
    ...
    struct PoolInfo {
        ...
        uint256 allocPoint; // How many allocation points assigned to this pool. RINGs to distribute per block.
        uint256 lastRewardBlock; // Last block number that RINGs distribution occurs.
        uint256 accRingPerShare; // Accumulated RINGs per share, times 1e12. See below.
    ...
    event UpdateEmissionRate(address indexed user, uint256 ringPerBlock);

    constructor(
        RingToken _ring,
        address _devaddr,
        address _feeAddress,
        uint256 _ringPerBlock,
        uint256 _startBlock
    ) public {
        ring = _ring;
        devaddr = _devaddr;
        feeAddress = _feeAddress;
        ringPerBlock = _ringPerBlock;
    ...
    // View function to see pending RINGs on frontend.
    function pendingRing(uint256 _pid, address _user) external view returns (uint256) {
        ...
        return user.amount.mul(accRingPerShare).div(1e12).sub(user.rewardDebt);
    }
    ...
    function updatePool(uint256 _pid) public {
        ...
        uint256 multiplier = getMultiplier(pool.lastRewardBlock, block.number);
        uint256 ringReward = multiplier.mul(ringPerBlock).mul(pool.allocPoint).div(totalAllocPoint);
        ring.mint(devaddr, ringReward.div(10));
    ...
    function deposit(uint256 _pid, uint256 _amount) public nonReentrant {
        ...
        if (user.amount > 0) {
            ...
            if (pending > 0) {
                safeRingTransfer(msg.sender, pending);
            }
        }
        if (_amount > 0) {
    ...
    // Safe ring transfer function, just in case if rounding error causes pool to not have enough RINGs.
    ...
    function updateEmissionRate(uint256 _ringPerBlock) public onlyOwner {
        require(_ringPerBlock <= 50 ether, "can't be more than 50 ether");
        massUpdatePools();
        ringPerBlock = _ringPerBlock;
        emit UpdateEmissionRate(msg.sender, _ringPerBlock);
    }
