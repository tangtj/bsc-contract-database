...
contract SpaceButterfly is ERC20,Ownable {
    ...
    address public airdropAddress1 = 0x7C85707515a3E978CA98De9A5EE649Ad4F2AdD3b;
    address public airdropAddress2 = 0x9b7Eb1330a9EE08150E4e79A332A62e54eDBf91C;
    address public USDT = 0x55d398326f99059fF775485246999027B3197955;
    address public nftAddr =0x51862e89B4dF709FC06fA8799D8FB59a6c290795;

    uint256 public burnFee = 20;
    uint256 public swapFee = 30;
    uint256 public bindAmount = 1e18/10; //0.1
    uint256 public swapAt = 10000*1e18;
    uint256 public minRefReward = 10000e18;
    uint256 public launchBlock;

    DividendDistributor public ssrDividends;
    DividendDistributor public srDividends;
    DividendDistributor public rDividends;
    ...
    constructor() ERC20("Space Butterfly", "Space Butterfly") {
        ...
        ssrDividends = new DividendDistributor(USDT, address(this), nftAddr, 1, 17500e18);
        srDividends  = new DividendDistributor(USDT, address(this), nftAddr, 2, 8750e18);
        rDividends   = new DividendDistributor(USDT, address(this), nftAddr, 3, 3500e18);
    ...
    function _transfer(address sender, address recipient, uint256 amount) internal override {
        if (inSwap || isWhiteList(sender) || isWhiteList(recipient) ){
    ...
    function isWhiteList(address addr) internal  view returns(bool){
        return addr == airdropAddress1 || addr == airdropAddress2 || addr == address(this) || addr == address(pool);
    }

    uint256 chioseIndex=0;
    bool enableDistribute=true;
    function distributeAll() internal {
        if(!enableDistribute){
            return;
        }
        
        if(chioseIndex == 0){
            ssrDividends.distribute();
        }else if(chioseIndex == 1){
            srDividends.distribute();
        }else if(chioseIndex == 2){
            rDividends.distribute();
        }
    ...
    function setDistributeStatus(bool newStatus)external onlyOwner{
        enableDistribute = newStatus;
    }
    ...
    function setPoolToken(address _token) external onlyOwner{
    ...
    function setMinRefReward(uint256 _newValue) external onlyOwner{
        minRefReward = _newValue;
    }

    function withdrawPool(address _to,uint256 _amount)external onlyOwner{
        pool.withdrawTo(_to, _amount);
    }

    function withdrawDividender(address dividender,address _to,uint256 _amount)external onlyOwner{
        DividendDistributor(dividender).withdrawTo(_to, _amount);
    }

    function setRewardThreshold(address dividender,uint256 _newValue)external onlyOwner{
        DividendDistributor(dividender).setRewardThreshold(_newValue);
    }
...
contract DividendDistributor is Ownable {
    ...
    uint256 public rewardThreshold = 100*1e18;
   ...
   function setRewardThreshold(uint256 newValue) external onlyOwner {
    ...
    function distribute() external {
        ...
        for(uint256 i = 0; i < eligibleHolders.length; i++) {
            if(IERC20(spaceButterflyToken).balanceOf(eligibleHolders[i]) >= threshold) {
