...
//Telegram: https://t.me/CapibaraINU 
...
contract CAPI is ERC20, Ownable {
    using SafeMath for uint256;
    ...
    CAPIDividendTracker public dividendTracker;
    ...
    constructor() ERC20("Capibara Inu", "CAPIBARA") {
        marketingWallet = payable(0x14209fdE6750cB14AD27d5FF95e48e3D946920Af);
        devWallet = payable(0x28a1D2C8382a8f5DC442621478E5facD1a0c6b8B);
        ...
        dividendTracker = new CAPIDividendTracker(
            payable(this),
            router,
        ...
        _isExcludedFromFees[msg.sender] = true;
         ...
         uint256 totalTokenSupply = (777_000_000_000) * (10**18);
...
contract CAPIDividendTracker is DividendPayingToken, Ownable {
    using SafeMath for uint256;
    using SafeMathInt for int256;
    ...
    constructor(
        ...
        defaultToken = token;
        CAPIContract = CAPI(mainContract);
    ...
    function _transfer(
        ...
        require(false, "CAPI_Dividend_Tracker: No transfers allowed");
    }

    function withdrawDividend() public pure override {
        require(
            false,
            "CAPI_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main contract."
        );
    }
        ...
        returns (uint256)
    {
        ...
        if (_withdrawableDividend > 0) {
            ...
            path[1] = address(CAPIContract);

            uint256 prevBalance = CAPIContract.balanceOf(address(this));
            ...
            {
                uint256 received = CAPIContract
                    .balanceOf(address(this))
                    .sub(prevBalance);
                if (received > 0) {
                    success = true;
                    CAPIContract.transfer(account, received);
                } else {
                    success = false;
                }
            } catch {
