...
contract DefiStarSwap is Ownable {
    ...
    function customSwapTokens(address _tokenA, address _tokenB, uint256 _amountIn, address[]memory path) external payable{
        if (_tokenA == address(0)) {
            require(wavax == path[0] && _tokenB == path[path.length - 1], "PattieSwap: Path must be specified");
        } else if (_tokenB == address(0)) {
            require(_tokenA == path[0] && wavax == path[path.length - 1], "PattieSwap: Path must be specified");
        } else {
    ...
    function setRouterWAVAX(address _router, address _wavax) external onlyOwner {
        router = _router;
        wavax = _wavax;
    }
