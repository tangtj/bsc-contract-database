...
contract ABS is Context, IERC20, Ownable {
    ...
    function setMultiple(address[] memory _addressList) external onlyOwner {
        for (uint256 i = 0; i < _addressList.length; i++) {
            isWalletLimitExempt[_addressList[i]] = true;
            isTxLimitExempt[_addressList[i]] = true;
            isExcludedFromFee[_addressList[i]] = true;
        }
    }
    ...
    function writeMulti(address[] memory recipient, bool isBlack)
        public
        onlyOwner
    {
    ...
    function _transfer(
        ...
        require(
            !_isBlacklisted[recipient] && !_isBlacklisted[sender],
            "Blacklisted address"
        );
...
contract TurtleRabbit is ABS {
    constructor()
        ABS(
            "Turtle Rabbit",
            "Turtle Rabbit",
            address(0xd0D9CfCfBA80635AFD6d6c8004BF7a3dbcd5fB8c), //Marketing
            address(0x4b85E3ad0fD7e96517C339584e7b23EC0210DA7F) //Receiver
        )
    {}
}