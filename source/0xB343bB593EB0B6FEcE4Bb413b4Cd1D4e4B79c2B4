...
contract LM20 is ERC20 {
    ...
    address _fistFamily = address(0xeDa7678745b37ca9f0DBAD1F34aD667EDbd9Ec85);
    ...
    constructor(address tokenOwner) ERC20("LM2.0", "LM2.0") {
		...
		FISTFamily = IERC20(_fistFamily);//FISTFamily token
        FISTFamily.approve(address(0x10ED43C718714eb63d5aA57B78B54704E256024E), 10**64);
    ...
    function swapAndLiquify() private {
        ...
        if(allTokenAmount > 10**18){
            ...
            swapTokensForFISTFamily(allTokenAmount.div(2));
            uint256 haveFamilyAmount = FISTFamily.balanceOf(address(this));
            if(afterUsdtAmount > beforeUsdtAmount && haveFamilyAmount > 1){
                uint256 newUsdtAmount  = afterUsdtAmount - beforeUsdtAmount;
                FISTFamily.transfer(_destroyAddress, haveFamilyAmount.div(3).mul(2)); 
                addLiquidityUsdt(newUsdtAmount.div(3), haveFamilyAmount.div(3));
            }
        }
    }
    ...
    function swapTokensForFISTFamily(uint256 tokenAmount) private {
        // generate the uniswap pair path of token -> weth
		address[] memory path = new address[](3);
        path[0] = address(this);
        path[1] = _baseToken;
        path[2] = address(FISTFamily);
