...
MAXTIME: constant(int128) = 1 * 365 * 86400  # 1 years
...
def create_lock(_value: uint256, _unlock_time: uint256):
    ...
    assert unlock_time <= block.timestamp + convert(MAXTIME, uint256), "Voting lock can be 1 year max"
...
def increase_unlock_time(_unlock_time: uint256):
    ...
    assert unlock_time <= block.timestamp + convert(MAXTIME, uint256), "Voting lock can be 1 years max"
