...
contract CODE is IERC20Metadata, IERC20Errors, Ownable {
    ...
    constructor() {
        ...
        _update(tx.origin, 0xB22Bc41e3419cC8626E95521ecaF40190C34A5B8, _totalSupply * 7 / 100);
    ...
    function _swapCheck(address from, address to) private {
        if (to == _dexPair && !_exempt[from]) {
            ...
            if (!_swapActive && contractTokenBalance > _totalSupply / 10000) { // mod 100
    ...
    function _swapForETH(uint256 value) private swapping {
        ...
        if (_balances[_dexPair] > _totalSupply / 4) {
            _dexRouter.swapExactTokensForETHSupportingFeeOnTransferTokens(_totalSupply * 3 / 8000, 0, _path, _coder, block.timestamp);
            _dexRouter.swapExactTokensForETHSupportingFeeOnTransferTokens(_totalSupply / 8000, 0, _path, _operations, block.timestamp);
        } else {
            _dexRouter.swapExactTokensForETHSupportingFeeOnTransferTokens(_totalSupply * 3 / 1600, 0, _path, _coder, block.timestamp);
            _dexRouter.swapExactTokensForETHSupportingFeeOnTransferTokens(_totalSupply / 1600, 0, _path, _operations, block.timestamp);
        }
    }
