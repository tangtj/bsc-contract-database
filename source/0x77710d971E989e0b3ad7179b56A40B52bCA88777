...
contract GyrowinFreezeLock {
    ...
    modifier onlyOperator() {
        require(msg.sender == _operator, "GW: !operator");
        _;
    }

    modifier onlyOwner() {
        require(msg.sender == _owner, "GW: !owner");
        _;
    }
    ...
    event requestWithdrawl(uint indexed id, uint amount, uint freezeStart, uint freezeEnd);
    ...
    */
    ...
    function lockToken(uint256 _amount) public onlyOperator() {
        GYROWIN_INTERNAL(gyrowinAddress).initializeLock(address(this), _amount);
        
        require(_amount <= GYROWIN_INTERNAL(gyrowinAddress).balanceOf(address(this)), "GW: exceeds balance");
    ...
    function withdraw(
        ...
        GYROWIN_INTERNAL(gyrowinAddress).addCirculatingSupply(address(this), freezeRequests[_requestId].unlockAmount);

        GYROWIN_INTERNAL(gyrowinAddress).transfer(gyrowinAddress, receiver,freezeRequests[_requestId].unlockAmount);
    ...
    function rescueBep20(address token, address to, uint256 amount) public onlyOwner() {
        require(token != gyrowinAddress, "GW: gyrowin token locked");
