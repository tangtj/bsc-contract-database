...
contract TokenSale is ReentrancyGuard {
    ...
    IERC20 public token; 
    uint256 public tokenPrice = 90 * 10**18;
    uint256 public referralBNBRewardPercentage = 20;
    ...
    function buyTokens(address _referralAddress) external payable nonReentrant {
        ...
        if (referralAddress != address(0)) {
            payable(referralAddress).transfer((bnbAmount * referralBNBRewardPercentage) / 100);
            referralBNBEarnings[referralAddress] += (bnbAmount * referralBNBRewardPercentage) / 100;
        }
    }
