...
pragma solidity ^0.8.17;
contract Zap is IZap, ZapBase {
    mapping(address => VaultInfo) public vaultInfos;
    mapping(address => bool) public liquidityImplementations;

    function zapIn(ZapInInput memory zapInInput) external payable {
        ...
        require(
            liquidityImplementations[address(liquidityImplementation)],
            "Liquidity implementation not provided"
        );

        address token0 = liquidityImplementation.token0(vaultInfo.lpToken);
        address token1 = liquidityImplementation.token1(vaultInfo.lpToken);

        uint256 amountToken0 = _fillQuote(
            zapInInput.fromTokenAddress,
            token0,
            zapInInput.token0Info.tokenShare,
            zapInInput.token0Info.swapTarget,
            zapInInput.token0Info.swapData
        );

        uint256 amountToken1 = _fillQuote(
            zapInInput.fromTokenAddress,
            token1,
            zapInInput.token1Info.tokenShare,
            zapInInput.token1Info.swapTarget,
            zapInInput.token1Info.swapData
        );
    }

    function setLiquidityImplementation(
        address liquidityImplementation,
        bool enabled
    ) external onlyOwner {
        liquidityImplementations[liquidityImplementation] = enabled;
    }
