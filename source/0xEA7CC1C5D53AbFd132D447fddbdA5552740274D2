...
abstract contract NewTradeFeeableToken is Ownable, ERC20 {
    ...
    function _setFeeRatios(AccountRatio[] memory source, AccountRatio[] storage target) internal onlyOwner {
        ...
        if (source.length > 0) {
            for (uint256 i = 0; i < source.length; i++) {
                require(source[i].ratio <= PRECISION, "TOKEN: fee too high");
...
contract CTX is Ownable, NewTradeFeeableToken {
    constructor(string memory symbol, uint256 supply) ERC20(symbol, symbol) {
