...
contract RuneReward is IPool,Governance {
    using SafeERC20 for IERC20;
    using SafeMath for uint256;

    IERC20 public _sato = IERC20(0x0);
    ...
    uint256 public _initReward = 280 * 1e18;
    ...
    uint256 public _totalBalance = 0;
    ...
    // types 5 for increase max buffrate
    struct CardInfo {
        uint256 tokenIndex;
        uint256 powers;
        uint256 timeStamp;
    ...
    uint256 public _maxBuffRate = 3000;
    uint256 public _growthBuffRate = 50;

    mapping(address => uint256[]) public _playerId;
    mapping(uint256 => uint256) public _runeMapIndex;
    
    event RewardAdded(uint256 reward);
    event StakedRune(address indexed user, uint256 amount);
    event WithdrawnRune(address indexed user, uint256 amount);
    ...
    constructor(address sato, address narcard, address narcardFactory) public {
        _sato = IERC20(sato);
        _narcardToken = INarCardToken(narcard);
        _narcardFactory = INarCardFactory(narcardFactory);
    }
    ...
    function updateBuffRate(address account) updateReward(account)  public {
        ...
        uint256 maxBuffRate = getRuneRate(account,5);
    ...
    {
        require(token != _narcardToken, "narcard token");
    ...
    function setRune(uint256 nftId) checkStart public {
        uint8 types;
        uint256 power;
        (types,power) = _narcardFactory.getNarCard(nftId);
        
        require(types == 1 || types == 2 || types == 5, "rune not match");
    ...
    function removeRune(uint256 nftId) checkStart public {
        uint8 types;
        uint256 power;
        (types, power) = _narcardFactory.getNarCard(nftId);

        require(types == 1 || types == 2 || types == 5, "rune not match");
        require(rune[msg.sender][types].tokenIndex == nftId,"not rune owner");
    ...
    // stake Rune
    function stakeRune(uint256 nftId)
        public
        ...
        checkStart
    {
        uint8 types;
        uint256 amount;
        (types,amount) = _narcardFactory.getNarCard(nftId);

        require(types == 6, "rune not match");

        uint256 oldBalance = balanceOf(msg.sender);

        uint256 stakingAmount = amount.mul(1e18);

        _balances[msg.sender] = _balances[msg.sender].add(stakingAmount);
            
        _totalBalance = _totalBalance.add(stakingAmount);

        uint256[] storage nftIds = _playerId[msg.sender];

        nftIds.push(nftId);

        _runeMapIndex[nftId] = nftIds.length - 1;
        ...
        if(oldBalance == 0){
            uint256 power =  stakingAmount.mul(initBuffRate);
            ...
            uint256 averageBuffRate = (userInfo[msg.sender].currentBuffrate.mul(oldBalance)).add((stakingAmount).mul(initBuffRate));
        ...
        emit StakedRune(msg.sender, nftId);
    ...
    function withdrawRune(uint256 nftId)
        public
        updateReward(msg.sender)
        checkHalve
        checkStart
    {        
        uint256[] memory nftIds = _playerId[msg.sender];
        uint256 nftIndex = _runeMapIndex[nftId];
        
        require(nftIds[nftIndex] == nftId, "not rune owner");
        uint256 types;
        uint256 amount;
        (types,amount) = _narcardFactory.getNarCard(nftId);

        uint256 nftLength = nftIds.length;
        uint256 tailId = nftIds[nftLength-1];

        _playerId[msg.sender][nftIndex] = tailId;
        _playerId[msg.sender][nftLength-1] = 0;
        _playerId[msg.sender].pop();
        _runeMapIndex[tailId] = nftIndex;
        _runeMapIndex[nftId] = 0;

        uint256 stakingAmount = amount.mul(1e18);

        _balances[msg.sender] = _balances[msg.sender].sub(stakingAmount);
            
        _totalBalance = _totalBalance.sub(stakingAmount);
        ...
        emit WithdrawnRune(msg.sender, nftId);
    }
        ...
        checkStart
    {
        ...
        for (uint8 index = 0; index < nftIds.length; index++) {
                withdrawRune(nftIds[index]);
        }
    }

    function getPlayerIds( address account ) public view returns( uint256[] memory nftIds )
    {
        nftIds = _playerId[account];
    }
    ...
    function getReward() public updateReward(msg.sender) checkHalve checkStart {
        ...
        if (reward > 0) {
            ...
            _sato.safeTransfer(msg.sender, reward );
    ...
    modifier checkHalve() {
        if (block.timestamp >= _periodFinish) {
            ...
            _sato.mint(address(this), _initReward);
