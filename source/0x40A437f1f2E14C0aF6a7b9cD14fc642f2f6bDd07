...
contract GamePet is ERC721Enumerable, AccessControl {
    ...
    mapping(address => uint256) private _maxHoldings;

    Counters.Counter private _lastId;

    constructor() ERC721("Memeworld Pets", "PET") {
        _grantRole(DEFAULT_ADMIN_ROLE, _msgSender());

        _baseTokenURI = "https://app.memeworld.world/v1/tokens/meta/";
        _maxHolding = 16;
    }
    ...
    function setMaxHolds(
        address user,
        uint256 max
    ) public onlyRole(DEFAULT_ADMIN_ROLE) {
        _maxHoldings[user] = max;
    }
    ...
    function maxHoldingOf(address user) public view returns (uint256) {
        if (_maxHoldings[user] > 0) {
            return _maxHoldings[user];
        }
    ...
    function _beforeTokenTransfer(
        ...
        if (to != address(0)) {
            require(
                balanceOf(to) + batchSize <= maxHoldingOf(to),
                "GamePet: Exceeded the maximum number of holdings"
            );
        }
    }
