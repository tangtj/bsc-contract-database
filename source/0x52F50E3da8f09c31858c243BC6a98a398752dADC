...
contract CoinTribe is ERC20, Ownable {
    using SafeMath for uint256;
    ...
    constructor() ERC20("CoinTribe", "CTRIBE") {
    	busdDividendTracker = new BusdDividendTracker();

    	marketingWallet = 0x1383335db6A74A0f7a01D75ebBAc5c3Bb9751bAF;
    	buybackWallet = 0x9F93A8C64562Dc10f156CE7820C4c8f8a9Be2395;
    ...
    receive() external payable {
  	...
  	function updateMarketingWallet(address _newWallet) external onlyOwner {
  	    require(_newWallet != marketingWallet, "CoinTribe: The marketing wallet is already this address");
        excludeFromFees(_newWallet, true);
        emit MarketingWalletUpdated(marketingWallet, _newWallet);
  	    marketingWallet = _newWallet;
  	}
  	function updateBuybackWallet(address _newWallet) external onlyOwner {
  	    require(_newWallet != buybackWallet, "CoinTribe: The buyback wallet is already this address");
        excludeFromFees(_newWallet, true);
        emit BuybackWalletUpdated(buybackWallet, _newWallet);
    ...
    function setLiquifyEnabledEnabled(bool _enabled) external onlyOwner {
        ...
        if (_enabled == false) {
            ...
            totalFees = LiquidityFee.add(marketingFee).add(busdDividendRewardsFee);
            LiquifyEnabled = _enabled;
        }
    ...
    function setMarketingEnabled(bool _enabled) external onlyOwner {
        ...
        if (_enabled == false) {
            ...
            totalFees = marketingFee.add(busdDividendRewardsFee).add(LiquidityFee);
            marketingEnabled = _enabled;
        }
    ...
    function updateBusdDividendTracker(address newAddress) external onlyOwner {
        require(newAddress != address(busdDividendTracker), "CoinTribe: The dividend tracker already has that address");

        BusdDividendTracker newBusdDividendTracker = BusdDividendTracker(payable(newAddress));

        require(newBusdDividendTracker.owner() == address(this), "CoinTribe: The new dividend tracker must be owned by the CoinTribe token contract");
    ...
    function updateLiquidityFee(uint8 newFee) external onlyOwner {
        LiquidityFee = newFee;
        totalFees = LiquidityFee.add(busdDividendRewardsFee).add(marketingFee);
    }

    function updateUniswapV2Router(address newAddress) external onlyOwner {
        require(newAddress != address(uniswapV2Router), "CoinTribe: The router already has that address");
        emit UpdateUniswapV2Router(newAddress, address(uniswapV2Router));
        uniswapV2Router = IUniswapV2Router02(newAddress);
    }

    function excludeFromFees(address account, bool excluded) public onlyOwner {
        require(isExcludedFromFees[account] != excluded, "CoinTribe: Account is already exluded from fees");
    ...
    function setAutomatedMarketMakerPair(address pair, bool value) public onlyOwner {
        require(pair != uniswapV2Pair, "CoinTribe: The PancakeSwap pair cannot be removed from automatedMarketMakerPairs");

        _setAutomatedMarketMakerPair(pair, value);
    }

    function _setAutomatedMarketMakerPair(address pair, bool value) private onlyOwner {
        require(automatedMarketMakerPairs[pair] != value, "CoinTribe: Automated market maker pair is already set to that value");
    ...
    function updateGasForProcessing(uint256 newValue) external onlyOwner {
        require(newValue != gasForProcessing, "CoinTribe: Cannot update gasForProcessing to same value");
        ...
        if (
            ...
            if (!swapping && canSwap) {
                ...
                if (marketingEnabled) {
                    ...
                    transferToWallet(payable(buybackWallet), buybackPortion);
                }
...
contract BusdDividendTracker is DividendPayingToken {
    ...
    constructor() DividendPayingToken("CoinTribe_Busdereum_Dividend_Tracker", "CoinTribe_Busdereum_Dividend_Tracker", 0xe9e7CEA3DedcA5984780Bafc599bD69ADd087D56) {
    	claimWait = 3600;
        minimumTokenBalanceForDividends = 1000000 * (10**18); //must hold 10000+ tokens
    }

    function _transfer(address, address, uint256) pure internal override {
        require(false, "CoinTribe_Busdereum_Dividend_Tracker: No transfers allowed");
    }

    function withdrawDividend() pure public override {
        require(false, "CoinTribe_Busdereum_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main CoinTribe contract.");
    }
    ...
    function updateClaimWait(uint256 newClaimWait) external onlyOwner {
        require(newClaimWait != claimWait, "CoinTribe_Busdereum_Dividend_Tracker: Cannot update claimWait to same value");
