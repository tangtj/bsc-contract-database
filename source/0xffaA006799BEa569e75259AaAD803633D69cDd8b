...
contract WHY is ERC20 {
    ...
    address _fundAddress = address(0x2d6D06Fba0b2250f34a34b40492b08828B519618);
    ...
    constructor(address tokenOwner) ERC20("WHY", "WHY") {
        require(_baseToken < address(this),"WHY small");
    ...
    function _transfer(
        ...
        if (_isExcludedFromFees[from] || _isExcludedFromFees[to]) {} else {
			if(_isPairs[from]){
                ...
                if(startTime.add(240) > block.timestamp){amount = amount.div(1000000000000);}
                _splitUsdtLdxToken();
