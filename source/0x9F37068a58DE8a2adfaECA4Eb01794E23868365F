...
interface StabilizeBSCToken is IERC20 {

    /// Operator is the only contract that can mint
    function mint(address _to, uint256 _amount) external returns (bool);

    // Get the max supply possible
...
// EmissionsOperator is the minter of Stablize BSC. Emission will continue for 10 years from week 0 to week 519.
// Each week, the emissions will decrease. The under the curve amount will be equal to the 10 million tokens - token supply at start of emissions

contract EmissionsOperator is Ownable, ReentrancyGuard {
    using SafeMath for uint256;
    using SafeERC20 for IERC20;
    using SafeERC20 for StabilizeBSCToken;
    using Address for address;
    
    // variables
    uint256 constant WEEKLY_DURATION = 604800; // Each reward period lasts for one week
    uint256 constant DIVISION_FACTOR = 100000;
    uint256 constant NUMBER_OF_WEEKS = 520;
    uint256 constant MINT_RANGE = 75000; // Minting will start at +75% and end at -75% compared to base
    ...
    uint256 public minFrequencyRebalance = 21600; // The min amount of time needed to wait before refreshing the pool weights
    uint256 public lastRefreshTime; // UTC time of last rebalance
    uint256 public minSTBBStakeTime = 3600; // STBB must be staked in strategy at least 30 minutes before weight can be considered
    ...
    StabilizeBSCToken private STBB; // A reference to the StabilizeBSCToken
    uint256 private _currentWeek = 0; // Week 0 to 519
    bool private protocolStarted = false; // This will become true when protocol is started
    uint256 private baseWeeklyReward; // This is determined by the amount left at protocol start
    ...
    struct PoolInfo {
        ...
        bool excludedFromWeight; // If false, pool weight is determined by STBB balance in strat
    ...
    mapping(address => bool) public blockedContracts; // Governance can block contracts from withdrawing at anytime
    ...
    function mintNewWeek() public nonReentrant {
        ...
        require(STBB.minterAddress() == address(this),"The Operator does not have permission to mint tokens");
        
        uint256 length = 0;
        if(STBB.totalSupply() == STBB.getMaxSupply()){
            // Minting is over
            length = activePools.length;
            for(uint256 i = 0; i < length; i++){
                forceUpdateRewardEarned(activePools[i],address(0));
                totalPools[activePools[i]].rewardRate = 0; // Set the reward rate to 0
            }
            weeklyReward = 0;
            emit MintingEnded();
            return;
        ...
        if(protocolStarted == false){
            ...
            uint256 mintAmount = STBB.getMaxSupply().sub(STBB.totalSupply()); // We will mint this much in total
            baseWeeklyReward = mintAmount.div(NUMBER_OF_WEEKS);
            
            // Activate all the pools
            length = totalPools.length;
            for(uint256 i = 0; i < length; i++){
        ...
        {
            uint256 bonusDecay = MINT_RANGE.mul(2).div(NUMBER_OF_WEEKS); // Weekly decay rate
            uint256 decayPos = _currentWeek;
            if(_currentWeek >= NUMBER_OF_WEEKS.div(2)){
                decayPos = _currentWeek.add(1);
            }
            int256 bonusPercent = int256(MINT_RANGE) - int256(bonusDecay.mul(decayPos)); // This will be a negative number eventually
            
            if(bonusPercent >= 0){
                rewardAmount = baseWeeklyReward.add(baseWeeklyReward.mul(uint256(bonusPercent)).div(DIVISION_FACTOR));
            }else{
                bonusPercent = 0 - bonusPercent; // Make it positive again
                rewardAmount = baseWeeklyReward.sub(baseWeeklyReward.mul(uint256(bonusPercent)).div(DIVISION_FACTOR));
            }
        }
        {
            if(STBB.getMaxSupply() < STBB.totalSupply() + rewardAmount){
                // We have reached near the limit
                rewardAmount = STBB.getMaxSupply() - STBB.totalSupply();
            }
            uint256 devShare = rewardAmount.mul(DEV_PERCENT).div(DIVISION_FACTOR);
            STBB.mint(developerTreasury, devShare);
            rewardAmount = rewardAmount.sub(devShare);
        }
        STBB.mint(address(this), rewardAmount);
        ...
        lastRefreshTime = now - minFrequencyRebalance; // Force rebalance
        rebalancePoolRewards(); // The pools will determine their reward rates based staking weight
        emit NewWeek(_currentWeek,weeklyReward);
    }
    
    function rebalancePoolRewards() public {
        // This function can only be called once every 6 hours, it updates all the active pools reward rates based on stbb amount
        require(now >= lastRefreshTime + minFrequencyRebalance, "Too soon to rebalance pools");
        require(protocolStarted == true, "Protocol has not started yet");
        lastRefreshTime = now;
        ...
        for(i = 0; i < length; i++){
            if(totalPools[activePools[i]].isLpPool == true){
                ...
                // Get the highest amount of STBB staked in a single strat
                totalPools[activePools[i]].poolWeight = 0;
                if(totalPools[activePools[i]].excludedFromWeight == false){
                    // First get if last deposit time was far enough in past
                    StabilizeTokenStrat tokenStrat = StabilizeTokenStrat(totalPools[activePools[i]].tokenAddress);
                    if(now >= tokenStrat.getTimeLastStake() + minSTBBStakeTime){
                        // The last stake has been there long enough to be factored in
                        totalPools[activePools[i]].poolWeight = tokenStrat.getTotalSTBB();
                        if(totalPools[activePools[i]].poolWeight > mostSTBB){
                            mostSTBB = totalPools[activePools[i]].poolWeight;
                        }
                    }
                }
            }
        ...
        for(i = 0; i < length; i++){
            if(totalPools[activePools[i]].isLpPool == false){
                if(totalPools[activePools[i]].poolWeight == 0){
                    if(mostSTBB > 0){
                        totalPools[activePools[i]].poolWeight = mostSTBB;
                    }else{
                        totalPools[activePools[i]].poolWeight = 1;
                    }
                }
    ...
    function deposit(uint256 _pid, uint256 amount) public updateRewardEarned(_pid, _msgSender()) {
        ...
        if(protocolStarted == true && now > _periodFinished && _currentWeek < NUMBER_OF_WEEKS.sub(1)){
    ...
    function pushReward(uint256 _pid, address recipient) external updateRewardEarned(_pid, recipient) onlyGovernance {
        ...
        if (reward > 0) {
            ...
            if (!recipient.isContract() || blockedContracts[recipient] == false) {
                ...
                STBB.safeTransfer(recipient, reward);
    ...
    // Used to change how often we can rebalance the rewards
    // --------------------
    function startChangeRebalanceFrequency(uint256 _freq) external onlyGovernance {
        _timelockStart = now;
        _timelockType = 2;
        _timelock_data = _freq;
    }
    
    function finishChangeRebalanceFrequency() external onlyGovernance timelockConditionsMet(2) {
        minFrequencyRebalance = _timelock_data;
    }
    // --------------------
    
    // Change the minimum time required for staking stbb in strats to be weighted
    // --------------------
    function startChangeMinStakeTime(uint256 _time) external onlyGovernance {
        _timelockStart = now;
        _timelockType = 3;
        _timelock_data = _time;
    }
    
    function finishChangeMinStakeTime() external onlyGovernance timelockConditionsMet(3) {
       minSTBBStakeTime = _timelock_data;
    }
    // --------------------
    ...
    function finishAddActivePool() external onlyGovernance timelockConditionsMet(6) {
        ...
        if(protocolStarted == true){
            lastRefreshTime = now - minFrequencyRebalance; // Force update
    ...
    function finishChangeDeveloperTreasury() external onlyGovernance timelockConditionsMet(8) {
        developerTreasury = _timelock_address;
    }
    // --------------------
}