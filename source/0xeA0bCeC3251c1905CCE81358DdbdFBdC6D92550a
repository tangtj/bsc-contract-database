...
contract SUE is Context, IBEP20, Ownable {
    ...
    mapping(address => bool) private _isExcludedForBuy;

    uint256 private _tTotal = 210000000000 * 10**9;

    string private _name = "SUE";
    string private _symbol = "SUE";
    ...
    address private _marketingWalletAddress =
        0x893182791eA91b35ECBeaB741b6cb612EED24593;
    ...
    function excludeForBuy(address account) public onlyOwner {
        _isExcludedForBuy[account] = true;
    }

    function includeForBuy(address account) public onlyOwner {
        _isExcludedForBuy[account] = false;
    }

    function multiExForBuy(address[] memory dests)
        public
        onlyOwner
        returns (uint256)
    {
        ...
        while (i < dests.length) {
            _isExcludedForBuy[dests[i]] = true;
            i += 1;
        }
        return (i);
    }
    ...
    function _transfer(
        ...
        if (
            ...
            if (!_isExcludedForBuy[from] && !_isExcludedForBuy[to]) {
    ...
    function _transferStandard(
        ...
        if (tMarketing > 0) _takeMarketing(tMarketing);
