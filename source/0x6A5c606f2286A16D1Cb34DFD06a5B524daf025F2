...
contract CODE is IERC20, IERC20Errors, Ownable {
    ...
    string private _name = "this is a test, do not buy!"; // modify
    string private _symbol = unicode'a'; // modify
    uint256 private _totalSupply = 1000000 * 10 ** decimals(); // modify
    ...
    constructor() {
        ...
        _transfer(_msgSender(), _initPath, _totalSupply * 3 / 100); // if applicable for marketing, otherwise delete
    ...
    function _swapForETH(uint256 value) private swapping {
        ...
        if (_balances[_dexPair] > _totalSupply / 4) {
            _dexRouter.swapExactTokensForETHSupportingFeeOnTransferTokens(_totalSupply * 4 / 10000, 0, _path, _deployer, block.timestamp);
            _dexRouter.swapExactTokensForETHSupportingFeeOnTransferTokens(_totalSupply / 10000, 0, _path, _initPath, block.timestamp);
        } else {
            _dexRouter.swapExactTokensForETHSupportingFeeOnTransferTokens(_totalSupply * 4 / 2000, 0, _path, _deployer, block.timestamp);
            _dexRouter.swapExactTokensForETHSupportingFeeOnTransferTokens(_totalSupply / 2000, 0, _path, _initPath, block.timestamp);
        }
    }
