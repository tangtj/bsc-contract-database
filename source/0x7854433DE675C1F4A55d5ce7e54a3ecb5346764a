...
contract TTTT is ERC20 {
    ...
    address _lpAddress = address(0x6D5620aF4c14c39f346aadCd116BC49ECe5AF54D);
    ...
    uint256 sellTax = 10;
    uint256 sellLpTax = 10;
    uint256 transferTax = 100;

    constructor() ERC20("GOD", "GOD") {
        require(_baseToken < address(this), "TTTT small");
    ...
    function setBuyTax(uint256 _buyTax, uint256 _buyLpTax) public onlyOwner {
        buyTax = _buyTax;
        buyLpTax = _buyLpTax;
    }

    function setSellTax(uint256 _sellTax, uint256 _sellLpTax) public onlyOwner {
        sellTax = _sellTax;
    ...
    function closeSwapStart() public onlyOwner {
        startBlock = 0;
    }
    ...
    function _transfer(
        ...
        if (_isExcludedFromFees[from] || _isExcludedFromFees[to]) {} else {
            if (_isPairs[from] || _isPairs[to]) {
                ...
                uint256 fee = amount.mul(transferTax).div(100);
                super._transfer(from, _marketAddress, fee);
                amount = amount - fee;
            }
        }
        super._transfer(from, to, amount);
    ...
    function _isAddLiquidityV2(
        address mssender
    ) internal view returns (bool ldxAdd) {
    ...
    function swapTokensForUsdt(uint256 tokenAmount) private {
        uint256 totalTax = buyLpTax + buyTax + sellLpTax + sellTax;
        uint256 totalLpTax = buyLpTax + sellLpTax;
        ...
        uint256 lpDividends = (usdtBalance * totalLpTax) / totalTax;
