...
// File: contracts/supra/interfaces/beez/IShareRewardPool.sol
...
// File: contracts/supra/strategies/beez/StrategybeezLP.sol
 ...
 * beez rewards from providing liquidity are farmed every few minutes, sold and split 50/50.
 ...
 */
contract StrategybeezLPV2 is Ownable, Pausable {
    ...
    address public constant beez =
        address(0xD0e924FC4783d2C66932475E9706dbf357017539);
    ...
    address public constant shareRewardPool =
        address(0x8fD826fca3272eC1E87a984fc9e3DAA2a810C13d);
    uint8 public poolId;
    ...
    address[] public beezToWbnbRoute = [beez, wbnb];
    address[] public beezToLp0Route;
    address[] public beezToLp1Route;
    address[] public Lp0ToLp1Route = [lpToken0, wbnb, lpToken1];
    address[] public Lp1ToLp0Route = [lpToken1, wbnb, lpToken0];
    ...
    constructor(
        ...
        if (lpToken0 == wbnb) {
            beezToLp0Route = [beez, wbnb];
        } else if (lpToken0 != beez) {
            beezToLp0Route = [beez, wbnb, lpToken0];
        }

        if (lpToken1 == wbnb) {
            beezToLp1Route = [beez, wbnb];
        } else if (lpToken1 != beez) {
            beezToLp1Route = [beez, wbnb, lpToken1];
        }

        IERC20(lpPair).safeApprove(shareRewardPool, uint256(-1));
        IERC20(beez).safeApprove(unirouter, uint256(-1));
     ...
     * It deposits {lpPair} in the shareRewardPool to farm {beez}
     */
     ...
     * 3. It swaps the {beez} token for {lpToken0} & {lpToken1}
    ...
    function chargeFees() internal {
        uint256 toWbnb = IERC20(beez).balanceOf(address(this)).mul(40).div(1000);
        IPancakeRouter(unirouter).swapExactTokensForTokensSupportingFeeOnTransferTokens(
            toWbnb,
            0,
            beezToWbnbRoute,
            address(this),
    ...
    /**
     * @dev Swaps {beez} for {lpToken0}, {lpToken1} & {wbnb} using PancakeSwap.
     */
    function addLiquidity() internal {
        uint256 beezHalf = IERC20(beez).balanceOf(address(this)).div(2);
    ...
    function pause() public onlyOwner {
        ...
        IERC20(beez).safeApprove(unirouter, 0);
