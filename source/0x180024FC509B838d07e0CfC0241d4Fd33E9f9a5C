...
contract AlphanetStaking is Context, Ownable,ReentrancyGuard {
    ...
    function initLevel() internal {
        _ratesLevel[levels[0]] = RateLevel({min:0,max:1000,weight:1});
        _ratesLevel[levels[1]] = RateLevel({min:1000,max:10000,weight:2});
        _ratesLevel[levels[2]] = RateLevel({min:10000,max:50000,weight:3});
        _ratesLevel[levels[3]] = RateLevel({min:50000,max:100000,weight:4});
        _ratesLevel[levels[4]] = RateLevel({min:100000,max:999999999,weight:5});
    }
    ...
    function getAllLevelDetails() external view returns(RateLevel[] memory){
        uint length = levels.length;
        RateLevel[] memory rateLevels=new RateLevel[](length);
        for (uint8 index = 0 ;index < levels.length; index++){
            RateLevel memory tmp = _ratesLevel[levels[index]];
            rateLevels[index]=tmp;
        }
        return rateLevels;
    }
    ...
    function applyWithdraw(uint256 amount) external nonReentrant{
        ...
        withdrawApplies.applications[block.timestamp] = amount;
        withdrawApplies.applyTimes.push(block.timestamp);
        ...
        emit ApplyWithdraw(msg.sender,amount,block.timestamp);
    }
    ...
    function withdrawableAmount(address account)public view returns(uint256){
        ...
        for (uint8 index = 0; index < withdrawApplies.applyTimes.length; index++) {
            ...
            if (block.timestamp.sub(key) > lockDownDuration){
