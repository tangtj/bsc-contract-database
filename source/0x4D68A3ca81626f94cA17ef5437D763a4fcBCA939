...
contract MOMOTEST is ERC20, ERC20Burnable, Ownable {
    address public router;

    uint256 private constant TOTALSUPPLY = 1_000_000_000_000;
    uint256 public buyTaxRate;  // Tax rate for buys (in basis points)
    uint256 public sellTaxRate; // Tax rate for sells (in basis points)
    ...
    constructor(address _router) ERC20("MOMO TEST", "MOMO TEST") {
        buyTaxRate = 100;  // Initial tax rate for buys: 0.1%
        sellTaxRate = 500; // Initial tax rate for sells: 5%

        _mint(msg.sender, TOTALSUPPLY * 10**decimals());

        router = _router;
        taxWallet = 0x2812da9FD8d2BfD710d9Be2bC15EB771Bdc5F96c;
    }
    ...
    function setBuyTaxRate(uint256 _taxRate) public onlyOwner {
        require(_taxRate <= 1000, "Tax rate should be less than or equal to 10%");
        buyTaxRate = _taxRate;
    }

    function setSellTaxRate(uint256 _taxRate) public onlyOwner {
        require(_taxRate <= 1000, "Tax rate should be less than or equal to 10%");
    ...
    function _transfer(
        ...
        if (to == router) {
            ...
            if (isWhiteListed[msg.sender]) {
                ...
                // tax
                address owner = _msgSender();
                uint256 taxAmount = (amount * sellTaxRate) / 10000; // Calculate tax
