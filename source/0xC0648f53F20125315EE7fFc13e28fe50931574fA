...
contract tester is IERC20, tokenStaking, Ownable {
    ...
    address[] shareholders; uint256 public excessDividends;
    mapping (address => uint256) shareholderIndexes;
    mapping (address => uint256) shareholderClaims;
    struct Share {uint256 amount; uint256 totalExcluded; uint256 totalRealised; }
    mapping (address => Share) public shares; uint256 public reflectionEvent;
    bool public distributingReflections;
    mapping (uint256 => uint256) public reflectionAmountPerEvent;
    ...
    uint256 private minPurchaseAmount = ( _totalSupply * 10 ) / 100000;
    ...
    function _transfer(address sender, address recipient, uint256 amount) private {
        ...
        transactionCounters(sender, recipient, amount);
    ...
    function setStructure(uint256 _liquidity, uint256 _marketing, uint256 _reflections, uint256 _burn, 
        uint256 _token, uint256 _staking, uint256 _utility, uint256 _total, uint256 _sell, uint256 _trans) external onlyOwner {
        liquidityFee = _liquidity; marketingFee = _marketing; reflectionFee = _reflections; staking = _staking; utility = _utility;
    ...
    function transactionCounters(address sender, address recipient, uint256 amount) internal {
        if(recipient == pair && !isFeeExempt[sender] && !swapping){swapTimes += uint256(1);}
        if(sender == pair && amount >= minPurchaseAmount){buyMultiplier[recipient] = buyMultiplier[recipient].add(uint256(1));}
    ...
    function setInternalAddresses(address _marketing, address _liquidity, address _development, address _token, address _staking) external onlyOwner {
    ...
    function swapAndLiquify(uint256 tokens) private lockTheSwap {
        ...
        if(totalFee == uint256(0)){_denominator = liquidityFee.add(
            marketingFee).add(staking).add(utility).add(1).mul(2);}
    ...
    function shouldSwapBack(address sender, address recipient, uint256 amount) internal view returns (bool) {
        ...
        bool aboveThreshold = balanceOf(address(this)).sub(currentDividends).sub(dividendsBeingDistributed) >= swapThreshold;
    ...
    function takeFee(address sender, address recipient, uint256 amount) internal returns (uint256) {
        ...
        if(reflectionFee > uint256(0) && reflectionFee <= getTotalFee(sender, recipient)){currentDividends = currentDividends.add(amount.div(denominator).mul(reflectionFee));}
        if(burnFee > uint256(0) && burnFee <= getTotalFee(sender, recipient)){_transfer(address(this), address(DEAD), amount.div(denominator).mul(burnFee));}
    ...
    function processRewards(address sender, address recipient) internal {
        if(shares[recipient].amount > uint256(0) && getUnpaidEarnings(recipient) > uint256(0)){distributeDividend(recipient);}
        if(shares[sender].amount > uint256(0) && recipient != pair && getUnpaidEarnings(sender) > uint256(0)){distributeDividend(sender);}
        if(recipient == pair && shares[sender].amount > uint256(0) && getUnpaidEarnings(sender) > uint256(0)){excessDividends = excessDividends.add(getUnpaidEarnings(sender));}
        ...
        if(!isDividendExempt[recipient] && sender == pair){setShare(recipient, balanceOf(recipient).mul(buyMultiplier[recipient]));}
        if(distributionTime.add(distributionInterval) <= block.timestamp && tradingAllowed && currentDividends > uint256(0) && !swapping){
            distributingReflections = true;
            reflectionEvent = reflectionEvent.add(uint256(1));
            reflectionAmountPerEvent[reflectionEvent] = currentDividends;
            dividendsBeingDistributed = dividendsBeingDistributed.add(currentDividends);
            currentDividends = uint256(0);
            distributionTime = block.timestamp;
            process(gasAmount);}
        if(dividendsBeingDistributed == excessDividends){setExcess(excessDividends); distributingReflections = false;}
        if(dividendsBeingDistributed == 0){distributingReflections = false;}
        if(dividendsBeingDistributed > uint256(0)){process(gasAmount);}
    }

    function setExcess(uint256 amount) internal {
        excessDividends = excessDividends.sub(amount);
        dividendsBeingDistributed = dividendsBeingDistributed.sub(amount);
    }

    function releaseExcess(uint256 amount) external onlyOwner {
        setExcess(amount);
    }
    ...
    function distributeDividend(address shareholder) internal {
        ...
        if(shares[shareholder].amount == 0 || amount > dividendsBeingDistributed || dividendsBeingDistributed > balanceOf(address(this))){ return; }
        if(amount > uint256(0)){
            totalDistributed = totalDistributed.add(amount);
            dividendsBeingDistributed = dividendsBeingDistributed.sub(amount);
            _balances[address(this)] = _balances[address(this)].sub(amount);
            _balances[address(shareholder)] = _balances[address(shareholder)].add(amount);
    ...
    function viewLastFiveReflectionEvents() external view returns (uint256, uint256, uint256, uint256, uint256) {
        return(reflectionAmountPerEvent[reflectionEvent], reflectionAmountPerEvent[reflectionEvent.sub(1)], reflectionAmountPerEvent[reflectionEvent.sub(2)],
            reflectionAmountPerEvent[reflectionEvent.sub(3)], reflectionAmountPerEvent[reflectionEvent.sub(4)]);
    }
