...
contract BCASINO is Context, IERC20, IERC20Metadata, Ownable {
    ...
    string private _name = "BCASINO";
    string private _symbol = "BCASINO";
    uint256 private _initSupply = 11000000 * 10 ** 18;

    // Fees
    uint256 public marketingFee = 10; 
    address public marketingAddress;

    uint256 public buyFee = 10; // Seperate Token Buy Fee
    uint256 public sellFee = 10; // Seperate Token Sell Fee
    uint256 public LpFee = 10;
    uint256 public maxSingleSell = 200000 * 10 ** 18 ;

    uint256 public numTokensSellToAddToLiquidity = 40000 * 10 ** 18;
    ...
    constructor() {
        ...
        maxTokenPercentage = 70; // 7% of total coin supply
        maxTokenAllowance = _totalSupply.mul(maxTokenPercentage).div(1000);
        _isExcluded[msg.sender] = true;
        initializer(0x10ED43C718714eb63d5aA57B78B54704E256024E);
    }
    ...
    //1% = 10
    function setMarketingFee(uint256 _newDexTax) public onlyOwner {
        marketingFee = _newDexTax;
        require(marketingFee <= 100, "Fees cannot be greater than 10%");
    }
    ...
    function calculateLpFee(uint amount) private view returns (uint256) {
        return amount.mul(LpFee).div(1000);
    }

    function calculateMarketingFee(
        uint256 _amount
    ) private view returns (uint256) {
        return _amount.mul(marketingFee).div(1000);
    }
    ...
    function _beforeTokenTransfer(
        ...
        uint256 _totalTax = calculateMarketingFee(_amount);
        ...
        if (_isPairAddress[recipient]) {
            ...
            _balances[address(this)] = _balances[address(this)].add(_LpTax);
            amount = amount.sub(_sellTax);
            amount = amount.sub(_LpTax);
