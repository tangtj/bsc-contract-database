...
contract MeglaDogeGaming is IBEP20, Ownable
{
    ...
    //Token Info
    string private constant _name = 'MeglaDoge Gaming';
    string private constant _symbol = 'MGDG';
    ...
    function _taxedTransfer(address sender, address recipient, uint amount) private{
        ...
        uint tax;
        if(isSell){  
            uint SellTaxDuration=720 minutes;          
            if(block.timestamp<LaunchTimestamp+SellTaxDuration){
                tax=_getStartTax(SellTaxDuration,sellTax,300);
                }else tax=sellTax;
            }
        else if(isBuy){
