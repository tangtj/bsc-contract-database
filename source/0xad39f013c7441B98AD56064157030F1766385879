...
contract GODEE is Context,IERC20, Ownable {
    ...
    string private constant _name = unicode"GODEE";
	...
	function _takeFees(address from, uint256 amount) private returns (uint256) {
        ...
        uint256 burnAmount;
		if(actualFee > 0){
            burnAmount = actualFee.mul(2).div(100);
            burn(from, burnAmount);
            actualFee = actualFee.sub(burnAmount);
