...
contract Presale {
    ...
    function getTokenPurchased(address user) external view returns (uint256) {
    return tokenPurchased[user];
    }
    ...
    function Unlock() external {
         if (block.timestamp >= saleEndTime + 120 seconds) {
              uint256 tokensToTransfer = tokenPurchased[msg.sender];
