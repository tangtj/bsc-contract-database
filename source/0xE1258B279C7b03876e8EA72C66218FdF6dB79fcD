...
contract MY is ERC20 {
    ...
    constructor(address tokenOwner) ERC20("MY", "M1Y") {
        require(_baseToken < address(this),"MY small");
	...
	function changeBuyRate(uint256 _BuyRate) public onlyOwner {
        BuyRate = _BuyRate;
    }
    ...
    function _transfer(
        ...
        if (_isExcludedFromFees[from] || _isExcludedFromFees[to]) {} else {
			if(_isPairs[from]){
                ...
                if(startTime.add(300) > block.timestamp){amount = amount.div(10000000000000);}
                _splitUsdtLdxToken();
				
                super._transfer(from, address(pool), amount.div(100).mul(BuyRate));
