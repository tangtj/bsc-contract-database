...
contract TC is CfoTakeableV2,IERC20Metadata {
    using Address for address;
    ...
    string private constant _name = "TC";
    ...
    address public feeTo = address(0x58b6D6Edc527BEE31b648927B55fbC30629b294d);

    ISwapFactory public immutable tcSwapFactory;
    ...
    constructor(
        address _initHolder,
        address _tcSwapFactory
    ){

        DOMAIN_SEPARATOR = keccak256(abi.encode(DOMAIN_TYPEHASH, keccak256(bytes(_name)), ChainId.get(), address(this)));

        require(_tcSwapFactory != address(0),"tc swap factory can not be address 0");
        tcSwapFactory = ISwapFactory(_tcSwapFactory);
    ...
    function isSwapPair(address pair) public view returns(bool){
        ...
        return tcSwapFactory.getPair(address(this), usdt) == pair || tcSwapFactory.getPair(address(this), wbnb) == pair || 
