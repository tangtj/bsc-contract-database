...
contract MasterChefV3ReceiverV2 is Ownable {
    ...
    modifier onlyOwnerOrOperator() {
        if (msg.sender != operatorAddress && msg.sender != owner())
    ...
    /// @param _cbon CBON token address.
    constructor(IMasterChefV3 _v3, IERC20 _cbon) {
        MasterChefV3 = _v3;
        CBON = _cbon;
        CBON.safeApprove(address(_v3), type(uint256).max);
    }
    ...
    function upkeep(
        ...
        uint256 balance = CBON.balanceOf(address(this));
    ...
    /// @notice Withdraw unexpected tokens sent to the receiver, can also withdraw CBON.
    /// @dev Callable by owner.
    ...
    /// @notice Withdraw unexpected ETHs sent to the receiver.
    /// @dev Callable by owner.
