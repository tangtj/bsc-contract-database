...
contract TraderAI is ERC20, Ownable {
    ...
    bool public isSwapbackEnabled = false;
    ...
    event SellFeeUpdated(uint256 totalSellFee);
    event BuyFeeUpdated(uint256 totalBuyFee);

    constructor() ERC20("Trader AI", "TRAI") {
        
        projectWallet = address(0xF9864eA2eB2C604443964aFB3726ABd994D08EA9);
        ...
        liquidityPair = IUniswapV2Factory(_dexRouter.factory()).createPair(
            address(this),
            _dexRouter.WETH()
        );
        _setAMMPair(address(liquidityPair), true);

        uint256 _buyFee = 2;
        uint256 _sellFee = 2;

        uint256 totalSupply = 100_000_000 * 1e18;
        swapLimit = (totalSupply * 2) / 10000;
    ...
    function updateSwapLimit(uint256 newAmount)
        external
        onlyOwner
        returns (bool)
    {
        require(
            ...
            "Swap limit cannot be lower than 0.001% total supply."
        );
        require(
    ...
    function updateBuyFees(uint256 _newFee) external onlyOwner {
        totalBuyFee = _newFee;
        emit BuyFeeUpdated(totalBuyFee);
        require(totalBuyFee <= 4, "Must keep fees at 4% or less");
    }

    function updateSellFees(uint256 _newFee) external onlyOwner {
        totalSellFee = _newFee;
        emit SellFeeUpdated(totalSellFee);
        require(totalSellFee <= 4, "Must keep fees at 4% or less");
    }
    ...
    function swapBack() private {
        ...
        if (contractBalance > swapLimit * 20) {
            contractBalance = swapLimit * 20;
        }
