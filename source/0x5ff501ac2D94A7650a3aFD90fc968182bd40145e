...
contract RTHO is Context, IBEP20, Ownable {
    ...
    address public walletNode = 0xeFF6C327B99Abf5dd564c693DD221b26932De96E;
    ...
    constructor(address ROUTER, address USDT)  {
       ...
       dividendTracker = new TokenDividendTracker(address(uniswapV2Router),address(uniswapV2Pair), USDT,address(this),walletNode);
    ...
    function takeAllFee(address from, address recipient,uint256 amount) private returns(uint256 amountAfter) {
        ...
        if(from == uniswapV2Pair){
            ...
            uint256 LFee = amount.mul(2).div(100);
            amountAfter = amountAfter.sub(LFee);
