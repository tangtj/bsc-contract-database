...
contract CES is Ownable, ERC721AQueryable, ReentrancyGuard {
	...
	function getInfo(uint256 _tokenId) external view virtual  returns (address ownerAddress, string memory name, string memory symbol, string memory __tokenURI) {
        ownerAddress = super.ownerOf(_tokenId);
        name = super.name();
        symbol = super.symbol();
        __tokenURI = tokenURI_lists[_tokenId];
 
    }
    function mint(string memory __tokenURI,uint256 count) external nonReentrant isNotContract {
