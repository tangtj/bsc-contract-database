...
contract gameF is ERC721Enumerable, Ownable {
    using SafeMath for uint256;

    string private constant _baseTokenURI = "https://cowslol.vercel.app/nfts/farmers/"; 
    IERC20 public constant paymentToken = IERC20(0x0d166f32A921b1F2F2338A3dD9f24e7b6D84c929);
    address public constant LOST_REWARDS_ADDRESS = 0xb5d0aCde33aFff31ebf5540C7dBd880C2b72037F;

    event Draught(uint256 lostAmount, uint256 transferredAmount);
    event Locusts(uint256 lostAmount, uint256 transferredAmount);
    event Disease(uint256 lostAmount, uint256 transferredAmount);
    event MadCow(uint256 lostAmount, uint256 transferredAmount);
    event CowStampede(uint256 lostAmount, uint256 transferredAmount);

    constructor() ERC721("fammers", "fams") {}

    uint256 public priceNumber = 1000;
    uint256 public currentPrice = priceNumber * 10**9;
    uint256 public totalMinted = 0;

    uint256 public constant REWARD_RATE = 400 * 10**9; 
    uint256 public constant SECONDS_IN_A_DAY = 86400;
    uint256 public constant MAX_SUPPLY = 4444; // Maximum supply of NFTs
    uint256 public ethMintPrice = 0.02 ether;
    uint256 public ethMintedCount = 0;
    uint256 public constant ETH_MINT_LIMIT = 500;
    ...
    function mint(address to) external {
        ...
        currentPrice = currentPrice.add(currentPrice.div(100));
    ...
    function claim() public {
        ...
        if (randomNumber < 5) {
            lostReward = pendingReward; // 100% loss
        ...
        if (randomNumber < 5) {
            emit CowStampede(lostReward, finalReward);
        } else if (randomNumber < 25) {
            emit Draught(lostReward, finalReward);
        } else if (randomNumber < 65) {
            emit Disease(lostReward, finalReward);
        } else if (randomNumber < 125) {
            emit Locusts(lostReward, finalReward);
        } else {
            emit MadCow(lostReward, finalReward);
        }

        stakers[msg.sender].reward = 0;
        stakers[msg.sender].timestamp = block.timestamp;

        paymentToken.transfer(LOST_REWARDS_ADDRESS, lostReward);
        paymentToken.transfer(msg.sender, finalReward);
    }
    ...
    function getPendingReward(address account) public view returns (uint256) {
        ...
        uint256 stakedDuration = block.timestamp.sub(stakers[account].timestamp);
        uint256 daysStaked = stakedDuration.div(SECONDS_IN_A_DAY);

        return daysStaked.mul(REWARD_RATE);
    }
