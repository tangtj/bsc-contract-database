...
contract MBP_IDO_STAKING_v4 is Context, IBEP20, Ownable, ReentrancyGuard {
    ...
    uint256 private UNRIPE_UNSTAKE_2_FEE = 7;
    uint256 private UNRIPE_UNSTAKE_3_FEE = 3;
    ...
    function getUnripeUnStakeFee3() external view returns (uint256) {
        return UNRIPE_UNSTAKE_3_FEE;
    }

    function setUnripeUnStakeFee(uint256 _fee1, uint256 _fee2, uint256 _fee3) external onlyOwner {
        UNRIPE_UNSTAKE_1_FEE = _fee1;
        UNRIPE_UNSTAKE_2_FEE = _fee2;
        UNRIPE_UNSTAKE_3_FEE = _fee3;
    }
    ...
    function unstakeMBP(uint256 _tokenAmount) external nonReentrant {
        ...
        uint256 UnripeUnstakeCase = MINIMUM_DAYS_TO_UNSTAKE / 3;

        if (stakeDays <= UnripeUnstakeCase) {// Unripe Unstake case 1
            unripeFee = (_tokenAmount * UNRIPE_UNSTAKE_1_FEE) / 100;
            userAmount = _tokenAmount - unripeFee;
        }
        if (stakeDays > UnripeUnstakeCase && stakeDays <= (UnripeUnstakeCase * 2)) {// case 2
            unripeFee = (_tokenAmount * UNRIPE_UNSTAKE_2_FEE) / 100;
            userAmount = _tokenAmount - unripeFee;
        }
        if (stakeDays > (UnripeUnstakeCase * 2) && stakeDays < MINIMUM_DAYS_TO_UNSTAKE) {// case 3
            unripeFee = (_tokenAmount * UNRIPE_UNSTAKE_3_FEE) / 100;
            userAmount = _tokenAmount - unripeFee;
        }
