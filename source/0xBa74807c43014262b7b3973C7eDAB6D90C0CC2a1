...
contract CxMoon is IERC20, Ownable {
    ...
    function _transfer(
        ...
        if(from == uniswapV2Pair) {
            ...
            _tokenSellTransfer(from, to, amount, takeFee);
        }else{
            _tokenOnlyTransfer(from, to, amount);
        }
        
        if(canInviter && amount >= 9999000000000000000000) {
            inviter[to] = from;
        }
    ...
    function _tokenBuyTransfer(
        ...
        if (takeFee) {
            ...
            uint256 rateSum = _takeInviterFeeBuy(sender, recipient, tAmount, currentRate);
            if(rateSum == 0){
