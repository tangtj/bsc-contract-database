...
contract VestingContract {
    ...
    enum VestingPeriod { WEEKLY, MONTHLY, DIRECT }
    ...
    function batchAddBlacklist(address[] memory _beneficiaries) public  onlyOwner {
    ...
    function batchCreateVestingSchedule(
        ...
        public onlyOwner{
            for (uint256 i = 0; i <batchSize; i++) {
                ...
                createVestingSchedule(beneficiary, amount, startTime, endTime, period);
            }
    }

    function claim(address _beneficiary) internal {
        VestingSchedule[] storage beneficiarySchedules = schedules[_beneficiary];

        for (uint i = 0; i < beneficiarySchedules.length; i++) {
            ...
            if (schedule.period != VestingPeriod.DIRECT) {
                ...
                claimableAmount = (schedule.totalAmount * timePassed) / vestingDuration;
            }
            ...
            require(token.transfer(_beneficiary, payout), "Transfer failed");
        }
    }
    ...
    function getAllVestingSchedules() public view returns(VestingSchedule[][] memory){
        VestingSchedule[][] memory allSchedules = new VestingSchedule[][](beneficiaries.length);
        for (uint256 i = 0; i < beneficiaries.length; i++){
            address _beneficiary = beneficiaries[i];
