...
interface IMigratorToPOKESwap {
    // Perform LP token migration from legacy UniswapV2 to POKESwap.
    // Take the current LP token address and return the new LP token address.
    // Migrator should have full access to the caller's LP token.
    // Return the new LP token address.
    //
    // XXX Migrator must have allowance access to UniswapV2 LP tokens.
    // POKESwap must mint EXACTLY the same amount of POKESwap LP tokens or
    // else something bad will happen. Traditional UniswapV2 does not
    // do that so be careful!
    function migrate(IERC20 token) external returns (IERC20);
}

// POKEMasterFarmer is the master of POKE. He can make POKE and he is a fair guy.
//
// Note that it's ownable and the owner wields tremendous power. The ownership
// will be transferred to a governance smart contract once POKE is sufficiently
// distributed and the community can show to govern itself.
//
// Have fun reading it. Hopefully it's bug-free. God bless.
contract POKEMasterFarmer is Ownable {
    using SafeMath for uint256;
    ...
    struct UserInfo {
        ...
        //
        // We do some fancy math here. Basically, any point in time, the amount of POKEs
        // entitled to a user but is pending to be distributed is:
        //
        //   pending reward = (user.amount * pool.accPOKEPerShare) - user.rewardDebt
        //
        // Whenever a user deposits or withdraws LP tokens to a pool. Here's what happens:
        //   1. The pool's `accPOKEPerShare` (and `lastRewardBlock`) gets updated.
    ...
    struct PoolInfo {
        ...
        uint256 allocPoint;       // How many allocation points assigned to this pool. POKEs to distribute per block.
        uint256 lastRewardBlock;  // Last block number that POKEs distribution occurs.
        uint256 accPOKEPerShare; // Accumulated POKEs per share, times 1e12. See below.
    }
    ...
    // The POKE TOKEN!
    POKEBALLToken public poke;
    // Dev address.
    address public devaddr;
    // POKE tokens created per block.
    uint256 public REWARD_PER_BLOCK; // 1000000000000000000
    // Bonus muliplier for early POKE makers.
    ...
    // The block number when POKE mining starts.
    ...
    uint256 public burnPercent = 0; // init 0% burn poke

    uint256 public lockFromBlock;
    uint256 public lockToBlock;

    // The migrator contract. It has a lot of power. Can only be set through governance (owner).
    IMigratorToPOKESwap public migrator;
    ...
    event SendPOKEReward(address indexed user, uint256 indexed pid, uint256 amount, uint256 lockAmount);
    event Lock(address indexed to, uint256 value);
    ...
    function add(uint256 _allocPoint, IERC20 _lpToken, bool _withUpdate) public onlyOwner {
        require(poolId1[address(_lpToken)] == 0, "POKEMasterFarmer::add: lp is already in pool");
        if (_withUpdate) {
            massUpdatePools();
        }
    ...
    // Update the given pool's POKE allocation point. Can only be called by the owner.
    ...
    function setMigrator(IMigratorToPOKESwap _migrator) public onlyOwner {
        migrator = _migrator;
    }
    ...
    function updatePool(uint256 _pid) public {
        ...
        uint256 pokeForBurn;
        uint256 pokeForDev;
        uint256 pokeForFarmer;
        (pokeForBurn, pokeForDev, pokeForFarmer) = getPoolReward(pool.lastRewardBlock, block.number, pool.allocPoint);

        if (pokeForBurn > 0) {
            poke.mint(address(this), pokeForBurn);
            poke.burn(pokeForBurn);
        }

        if (pokeForDev > 0) {
            // Mint unlocked amount for Dev
            poke.mint(devaddr, pokeForDev.mul(100 - PERCENT_LOCK_BONUS_REWARD).div(100));
            // Mint locked amount for Dev
            //For more simple, I lock reward for dev if mint reward in bonus time
            poke.mint(address(this), pokeForDev.mul(PERCENT_LOCK_BONUS_REWARD).div(100));
            farmLock(devaddr, pokeForDev.mul(PERCENT_LOCK_BONUS_REWARD).div(100));
        }
        poke.mint(address(this), pokeForFarmer);
        pool.accPOKEPerShare = pool.accPOKEPerShare.add(pokeForFarmer.mul(1e12).div(lpSupply));
        pool.lastRewardBlock = block.number;
    }
    ...
    function getPoolReward(uint256 _from, uint256 _to, uint256 _allocPoint) public view returns (uint256 forBurn, uint256 forDev, uint256 forFarmer) {
        ...
        uint256 pokeCanMint = poke.cap().sub(poke.totalSupply());

        if (pokeCanMint < amount) {
            forBurn = 0;
            forDev = 0;
            forFarmer = pokeCanMint;
        }
        else {
    ...
    // View function to see pending POKEs on frontend.
    function pendingReward(uint256 _pid, address _user) external view returns (uint256) {
        PoolInfo storage pool = poolInfo[_pid];
        UserInfo storage user = userInfo[_pid][_user];
        uint256 accPOKEPerShare = pool.accPOKEPerShare;
        uint256 lpSupply = pool.lpToken.balanceOf(address(this));
        if (block.number > pool.lastRewardBlock && lpSupply > 0) {
            uint256 pokeForFarmer;
            (, , pokeForFarmer) = getPoolReward(pool.lastRewardBlock, block.number, pool.allocPoint);
            accPOKEPerShare = accPOKEPerShare.add(pokeForFarmer.mul(1e12).div(lpSupply));

        }
        return user.amount.mul(accPOKEPerShare).div(1e12).sub(user.rewardDebt);
    }
    ...
    function _harvest(uint256 _pid) internal {
        ...
        if (user.amount > 0) {
            uint256 pending = user.amount.mul(pool.accPOKEPerShare).div(1e12).sub(user.rewardDebt);
            uint256 masterBal = poke.balanceOf(address(this));
            ...
            if(pending > 0) {
                poke.transfer(msg.sender, pending.mul(100 - PERCENT_LOCK_BONUS_REWARD).div(100));
                ...
                emit SendPOKEReward(msg.sender, _pid, pending, lockAmount);
            }

            user.rewardDebt = user.amount.mul(pool.accPOKEPerShare).div(1e12);
        }
    }

    // Deposit LP tokens to POKEMasterFarmer for POKE allocation.
    function deposit(uint256 _pid, uint256 _amount) public {
        require(_amount > 0, "POKEMasterFarmer::deposit: amount must be greater than 0");
    ...
    function withdraw(uint256 _pid, uint256 _amount) public {
        PoolInfo storage pool = poolInfo[_pid];
        UserInfo storage user = userInfo[_pid][msg.sender];
        require(user.amount >= _amount, "POKEMasterFarmer::withdraw: not good");
    ...
    // Safe poke transfer function, just in case if rounding error causes pool to not have enough POKEs.
    function safePOKETransfer(address _to, uint256 _amount) internal {
        uint256 pokeBal = poke.balanceOf(address(this));
        if (_amount > pokeBal) {
            poke.transfer(_to, pokeBal);
        } else {
            poke.transfer(_to, _amount);
        }
    }
    ...
    function farmLock(address _holder, uint256 _amount) internal {
        require(_holder != address(0), "ERC20: lock to the zero address");
        require(_amount <= poke.balanceOf(address(this)), "ERC20: lock amount over blance");
        _locks[_holder] = _locks[_holder].add(_amount);
        _totalLock = _totalLock.add(_amount);
        if (_lastUnlockBlock[_holder] < lockFromBlock) {
            _lastUnlockBlock[_holder] = lockFromBlock;
    ...
    function unlock() public {
        ...
        uint256 amount = canUnlockAmount(msg.sender);
        // just for sure
        if (amount > poke.balanceOf(address(this))) {
            amount = poke.balanceOf(address(this));
        }
        poke.transfer(msg.sender, amount);
        _locks[msg.sender] = _locks[msg.sender].sub(amount);
        _lastUnlockBlock[msg.sender] = block.number;
        _totalLock = _totalLock.sub(amount);
    }
    ...
    function setTransferBurnRate(uint256 _tranferBurnRate) public onlyOwner {
        poke.setTransferBurnRate(_tranferBurnRate);
    }

    // In some circumstance, we should not burn POKE on transfer, eg: Transfer from owner to distribute bounty, from depositing to swap for liquidity
    function addTransferBurnExceptAddress(address _transferBurnExceptAddress) public onlyOwner {
        poke.addTransferBurnExceptAddress(_transferBurnExceptAddress);
    }

    function removeTransferBurnExceptAddress(address _transferBurnExceptAddress) public onlyOwner {
        poke.removeTransferBurnExceptAddress(_transferBurnExceptAddress);
    }
}