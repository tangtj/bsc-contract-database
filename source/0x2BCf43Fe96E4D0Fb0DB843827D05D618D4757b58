...
contract PreIDO is Ownable {
    ...
    uint256 internal idoRatio = 1; // 1U => 1
    uint256 internal inviterRewardFee = 5; //5%
    uint256 internal inviterRewardNftRequireNum = 10;

    uint256 internal idoMinRequireAmount = 100 * 10 ** 18;
    uint256 internal idoMaxRequireAmount = 1000 * 10 ** 18;
    ...
    function setIdoRequireAmount(uint256 _idoMinRequireAmount, uint256 _idoMaxRequireAmount) external onlyOwner {
       idoMinRequireAmount = _idoMinRequireAmount;
       idoMaxRequireAmount = _idoMaxRequireAmount;
    }

    function setInviterRewardNftRequireNum(uint256 _inviterRewardNftRequireNum) external onlyOwner {
    ...
    function setConfigAddress(
        address _QCTokenAddress,
        address _QCNFTAddress, 
        address _inviterAddress,
        address _managerAddress
    ) external onlyOwner {
        QCToken = IERC20(_QCTokenAddress);
        QCNFTAddress = _QCNFTAddress;
    ...
    function applyIDO(uint256 amount) public {
        ...
        require(amount >= idoMinRequireAmount && amount <= idoMaxRequireAmount, "Require USDT amount error");

        uint256 qcAmount = idoRatio.mul(amount);
        accountIDOMaps[msg.sender] = accountIDOMaps[msg.sender].add(qcAmount);
        accountIDOAmount[msg.sender] = amount;
        addAccountLogs(msg.sender, 1, qcAmount, "IDO Amount");

        //inviter
        address inviter = IInviter(inviterAddress).getInviter(msg.sender);
        uint256 inviterReward;
        if(inviter != address(0) && accountIDOAmount[inviter] > 0) {
            inviterReward = qcAmount.mul(inviterRewardFee).div(100);
            //accountIDOMaps[inviter] = accountIDOMaps[inviter].add(inviterReward);
            usdtToken.transferFrom(msg.sender, inviter, inviterReward);
            //addAccountLogs(inviter, 1, inviterReward, "IDO Reward");

            teamAchievement[inviter] += 1;
            accountAchievement[inviter] += amount;
            if(teamAchievement[inviter] >= inviterRewardNftRequireNum) {
                uint256 rewardCount = teamAchievement[inviter].div(inviterRewardNftRequireNum);
                uint256 sendNFTCount = rewardCount.sub(inviterNFTReward[inviter]);
                if(sendNFTCount >= 1 && QCNFTAddress != address(0)) {
                    IMetaNFT(QCNFTAddress).grantIDO(inviter);
                    addAccountLogs(inviter, 2, 1, "IDO Reward");
                    inviterNFTReward[inviter] = inviterNFTReward[inviter].add(1);
                }    
            }
        }
        usdtToken.transferFrom(msg.sender, managerAddress, amount.sub(inviterReward));
    ...
    function claim() public {
        ...
        accountTokenPaid[msg.sender] = claimAmount;
        QCToken.transfer(msg.sender, claimAmount);
    ...
    returns(
        ...
        qcAmount = accountIDOMaps[account];
