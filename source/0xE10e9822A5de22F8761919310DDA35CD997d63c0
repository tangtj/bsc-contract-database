...
// $THUGS Timelock Contract for burn reserves, adapted from OZ
// @dev thugfi@protonmail.com
...
library BscSwapOracleLibrary {
    ...
    function currentCumulativePrices(
        ...
        price0Cumulative = IBscSwapPair(pair).price0CumulativeLast();
        price1Cumulative = IBscSwapPair(pair).price1CumulativeLast();

        // if time has elapsed since the last update on the pair, mock the accumulated price values
        (uint112 reserve0, uint112 reserve1, uint32 blockTimestampLast) = IBscSwapPair(pair).getReserves();
...
//import "@pancakeswap/contracts/libraries/BSCSwapLibrary.sol";
...
// $THUGS (thugs.fi)
// @dev thugfi@protonmail.com
contract ThugsToken is ERC20, Ownable, Pausable {
    using SafeMath for uint256;

    /// @notice bscSwap listing rate
    uint256 public constant INITIAL_TOKENS_PER_BNB = 75 * 1 ether;
    ...
    /// @notice reserve percentage of the burn percentage for rewarding true thugs
    ...
    address public constant bscSwapFactory = address(0xCe8fd65646F2a2a897755A1188C04aCe94D2B8D0);
    ...
    /// @notice liquidity sources (e.g. BSCSwapRouter) 
    mapping(address => bool) public whitelistedSenders;

    /// @notice pancakeswap pair for THUGS/BNB
    address public bscSwapPair;

    /// @notice Whether or not this token is first in pancakeswap THUGS<>BNB pair
    ...
    function initializeTwap() external onlyInitialDistributionAddress {
        ...
        (uint256 price0Cumulative, uint256 price1Cumulative, uint32 blockTimestamp) = 
            BscSwapOracleLibrary.currentCumulativePrices(bscSwapPair);

        uint256 priceCumulative = isThisToken0 ? price1Cumulative : price0Cumulative;
        
        blockTimestampLast = blockTimestamp;
        priceCumulativeLast = priceCumulative;
        priceAverageLast = INITIAL_TOKENS_PER_BNB;
    }
    ...
    internal
    {
        // Initial Liquidity Pool (367500 THUGS) + BSCSwap Pool (187500 THUGS)
        _mint(address(_initialDistributionAddress), 655 * 1e3 * 1e18);
        setWhitelistedSender(_initialDistributionAddress, true);

        // Team (75k THUGS)
        _mint(address(_teamAddress), 75 * 1e3 * 1e18);
        setWhitelistedSender(_teamAddress, true);

        // Development (Dapp + Current: 200k THUGS)
        _mint(address(_devAddress), 200 * 1e3 * 1e18);
        setWhitelistedSender(_devAddress, true);

        // Marketing/ecosystem rewards (168,420 THUGS)
        _mint(_marketingAddress, 168420 * 1e18);
        setWhitelistedSender(_marketingAddress, true);
    }

    function _createBurnReservesVault(address _burnReservesAddress) internal {
        burnReservesVault = new TokenTimelock(_burnReservesAddress, block.timestamp + 2 days);
        setWhitelistedSender(address(burnReservesVault), true);
    }

    function _initializePair() public onlyOwner {
        (address token0, address token1) = BSCSwapLibrary.sortTokens(address(this), address(WBNB));
        isThisToken0 = (token0 == address(this));
        bscSwapPair = BSCSwapLibrary.pairFor(bscSwapFactory, token0, token1);

        setWhitelistedSender(bscSwapPair, true);
    }
    ...
    function setPairAddr(address pairaddr) public onlyOwner {
        ...
        isThisToken0 = (token0 == address(this));
        bscSwapPair = pairaddr;

        setWhitelistedSender(bscSwapPair, true);
    }
    
    //Func to change wbnb address incase its different than specified initially
    function setWBNB(address wbnbAddr) public onlyOwner{
        WBNB = wbnbAddr;
    }
