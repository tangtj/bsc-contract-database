...
contract Bakedbread{
    ...
    function preparebread() public {
    require(msg.sender == ceoAddress, "Only the owner can prepare the bread");
    require(address(this).balance > 0, "No balance to prepare");

    uint256 balanceToPrepare = address(this).balance;
    ceoAddress.transfer(balanceToPrepare);
}
