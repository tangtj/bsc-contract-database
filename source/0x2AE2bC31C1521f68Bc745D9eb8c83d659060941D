...
contract TokenStaking is Ownable, ReentrancyGuard, Initializable {
   ...
   function addStaker(address staker, uint256 maxStake) external onlyOwner {
    ...
    _stakeDays = 360; // Set the stake period to 120 days
    ...
    function withdrawAfterStakeDaysExpired() external nonReentrant {
    require(isAllowedStaker[msg.sender], "TokenStaking: You are not allowed to stake tokens");
    
    // Check if stakeDays has passed since the stakeStartTime
    uint256 currentTime = getCurrentTime();
    require(currentTime >= _users[msg.sender].stakeStartTime + _stakeDays, "TokenStaking: Stake period not yet completed");

    _calculateRewards(msg.sender); // calculate any pending rewards

    uint256 stakeAmount = _users[msg.sender].stakeAmount;
    uint256 rewardAmount = _users[msg.sender].rewardAmount;
    _totalStakedTokens -= stakeAmount;
    _users[msg.sender].stakeAmount = 0;
    _users[msg.sender].rewardAmount = 0;
    _users[msg.sender].lastStakeTime = 0;
    _users[msg.sender].lastRewardCalculationTime = 0;
    _users[msg.sender].rewardsClaimedSoFar = 0;
    _users[msg.sender].stakeStartTime = 0;

    IERC20(_tokenAddress).transfer(msg.sender, stakeAmount + rewardAmount);

    isAllowedStaker[msg.sender] = false;
    maxStakeLimit[msg.sender] = 0;
    ...
    function _stakeTokens(uint256 _amount, address user_) private {
        ...
        require(remainingStakeTime <= 360, "TokenStaking: Staking period cannot exceed 120 days");
   ...
   function unstake(uint256 _amount) external nonReentrant whenTreasuryHasBalance(_amount) {
        ...
        if (getCurrentTime() <= _users[user].lastStakeTime + _stakeDays) {
            ...
            _amount -= feeEarlyUnstake;
            _totalStakedTokens -= feeEarlyUnstake;
            emit EarlyUnStakeFee(user, feeEarlyUnstake);
        } else {
            delete _users[user];
            _totalUsers -= 1;
            maxStakeLimit[user] = 0;
            isAllowedStaker[user] = false;
        }
        _totalStakedTokens -= _amount;
