...
contract Business {
    ...
    userDeposits[msg.sender].depositTime = block.timestamp;// Update time

    IERC20 token = IERC20(tokenAddress);
    require(token.transfer(msg.sender, earnedUSD), "Withdrawal failed");

    emit TokensWithdrawn(msg.sender, earnedUSD, tokenAddress);
