...
// File: ICadinuProfile.sol
/**
 * @title ICadinuProfile
 */
...
contract SmartChefInitializable is Ownable, ReentrancyGuard {
    ...
    // The block number when CBON mining ends.
    uint256 public bonusEndBlock;

    // The block number when CBON mining starts.
    ...
    // Cadinu profile
    ICadinuProfile public immutable cadinuProfile;

    // Cadinu Profile is requested
    bool public cadinuProfileIsRequested;

    // Cadinu Profile points threshold
    uint256 public cadinuProfileThresholdPoints;

    // CBON tokens created per block.
    ...
    /**
     * @notice Constructor
     * @param _cadinuProfile: Cadinu Profile address
     * @param _cadinuProfileIsRequested: Cadinu Profile is requested
     * @param _cadinuProfileThresholdPoints: Cadinu Profile need threshold points
     */
    constructor(
        address _cadinuProfile,
        bool _cadinuProfileIsRequested,
        uint256 _cadinuProfileThresholdPoints
    ) {
        SMART_CHEF_FACTORY = msg.sender;

        // Call to verify the address is correct
        ICadinuProfile(_cadinuProfile).getTeamProfile(1);
        cadinuProfile = ICadinuProfile(_cadinuProfile);

        // if cadinuProfile is requested
        cadinuProfileIsRequested = _cadinuProfileIsRequested;

        // cadinuProfile threshold points when profile & points are requested
        cadinuProfileThresholdPoints = _cadinuProfileThresholdPoints;
    }
    ...
    function deposit(uint256 _amount) external nonReentrant {
        ...
        require(
            (!cadinuProfileIsRequested && cadinuProfileThresholdPoints == 0) ||
            cadinuProfile.getUserStatus(msg.sender),
            "Deposit: Must have an active profile"
        );

        uint256 numberUserPoints = 0;

        if (cadinuProfileThresholdPoints > 0) {
            (, numberUserPoints, , , , ) = cadinuProfile.getUserProfile(msg.sender);
        }

        require(
            cadinuProfileThresholdPoints == 0 || numberUserPoints >= cadinuProfileThresholdPoints,
    ...
    function updateProfileAndThresholdPointsRequirement(bool _isRequested, uint256 _thresholdPoints) external onlyOwner {
        ...
        cadinuProfileIsRequested = _isRequested;
        cadinuProfileThresholdPoints = _thresholdPoints;
    ...
    function pendingReward(address _user) external view returns (uint256) {
        ...
        if (block.number > lastRewardBlock && stakedTokenSupply != 0) {
            ...
            uint256 CBONReward = multiplier * rewardPerBlock;
            uint256 adjustedTokenPerShare = accTokenPerShare + (CBONReward * PRECISION_FACTOR) / stakedTokenSupply;
    ...
    function _updatePool() internal {
        ...
        uint256 multiplier = _getMultiplier(lastRewardBlock, block.number);
        uint256 CBONReward = multiplier * rewardPerBlock;
        accTokenPerShare = accTokenPerShare + (CBONReward * PRECISION_FACTOR) / stakedTokenSupply;
        lastRewardBlock = block.number;
    }
...
contract SmartChefFactory is Ownable {
    ...
    function deployPool(
        ...
        bytecode = abi.encodePacked(
            bytecode,
            abi.encode(_cadinuProfile, _cadinuProfileIsRequested, _cadinuProfileThresholdPoints)
        );
