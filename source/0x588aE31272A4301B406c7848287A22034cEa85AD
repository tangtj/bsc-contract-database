...
contract ElonMarkCoin is Context, IBEP20, Ownable {
    ...
    constructor () public {
        ...
        _maxTxAmount = 1000000000000000 * 10 ** _decimals;
        _maxHoldPercent = 100;
    ...
    function _tokenTransfer(address sender, address recipient, uint256 amount, bool takeFee) private {
        removeAllFee();
        if(takeFee) {
            if (recipient == pairAddress) { //продажа
                _taxFee = _previousTaxFee;
                _charityFee = _previousCharityFee;
                _burnFee = _previousBurnFee;
            } else if (sender == pairAddress) { // покупка
                _taxFee = _previousTaxFee;
                _burnFee = _previousBurnFee;
            }
        }
