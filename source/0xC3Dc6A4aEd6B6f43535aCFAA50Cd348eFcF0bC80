...
contract CrepeToken is Consts, FreezableMintableToken, BurnableToken, Pausable {
    ...
    WhitelistedCrowdsale
{
    ...
    function init() public onlyOwner {
        ...
        for (uint256 i = 0; i < addresses.length; i++) {
            if (freezes[i] == 0) {
                CrepeToken(token).mint(addresses[i], amounts[i]);
            } else {
                CrepeToken(token).mintAndFreeze(
                    addresses[i],
                    amounts[i],
                    freezes[i]
                );
