...
// File: contracts/farm/SafeBEP20.sol
...
// File: contracts/farm/MasterChefV2.sol
...
contract MasterChefV2 is AccessControl, ReentrancyGuard {
    using SafeMath for uint256;
    ...
    uint256 public constant MASTERCHEF_CAKE_PER_BLOCK = 0;
    uint256 public constant ACC_CAKE_PRECISION = 1e18;

    uint256 public constant BOOST_PRECISION = 100 * 1e10;
    uint256 public constant MAX_BOOST_PRECISION = 0;
    uint256 public constant CAKE_RATE_TOTAL_PRECISION = 0;
    ...
    function add(IBEP20 lp, IBEP20 rewardToken, uint256 _rewardPerBlock, uint256 blockNumber) public onlyRole(MANAGER_ROLE) {
        lpToken.push(lp);
        ...
        poolBlocks.push(PoolBlock({
        startBlock : block.number,
        endBlock : block.number + blockNumber,
        ...
        emit AddPool(lpToken.length - 1, 0, lp, true);
    }
    ...
    function pendingCake(uint256 _pid, address _user) external view returns (uint256) {
        ...
        PoolBlock memory poolBlock = poolBlocks[_pid];
        uint256 currentBlock;
        if (block.number > poolBlock.endBlock) {
            currentBlock = poolBlock.endBlock;
        } else {
        ...
        if (currentBlock > pool.lastRewardBlock && lpSupply != 0) {
            ...
            uint256 cakeReward = multiplier.mul(cakePerBlock(_pid));
    ...
    function cakePerBlock(uint256 pid) public view returns (uint256 amount) {
        return poolBlocks[pid].rewardPerBlock;
    }

    function deposit(uint256 _pid, uint256 _amount) external nonReentrant {
        PoolBlock memory poolBlock = poolBlocks[_pid];
        require(block.number < poolBlock.endBlock, "has end");
    ...
    function settlePendingCake(
        ...
        // SafeTransfer CAKE
        _safeTransfer(_user, pending, _pid);
    }

    function _safeTransfer(address _to, uint256 _amount, uint256 pid) internal {
        if (_amount > 0) {

            uint256 balance = rewardTokens[pid].balanceOf(address(this));
            if (balance < _amount) {
                _amount = balance;
            }
            rewardTokens[pid].safeTransfer(_to, _amount);
        }
    }

    function remove(address token, uint256 amount) public onlyRole(MANAGER_ROLE) {
