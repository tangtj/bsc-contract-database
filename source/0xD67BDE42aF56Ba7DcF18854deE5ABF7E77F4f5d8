...
contract TokenStaking is Ownable, ReentrancyGuard, Initializable {
   ...
   function addStaker(address staker, uint256 maxStake) external onlyOwner {
    ...
    function _stakeTokens(uint256 _amount, address user_) private {
        ...
        require(remainingStakeTime <= 120, "TokenStaking: Staking period cannot exceed 120 days");
  ...
  function unstake(uint256 _amount) external nonReentrant {
    ...
    if (stakePeriodOver) {
        ...
        // Subtract the early unstake fee from the amount to be transferred
        amountToTransfer -= earlyUnstakeFee;
    ...
    if (!stakePeriodOver) {
        _users[user].stakeAmount -= (_amount - amountToTransfer); // Deduct the earlyUnstakeFee
    }
    _totalStakedTokens -= _amount;
    IERC20(_tokenAddress).transfer(user, amountToTransfer);
    emit UnStake(user, amountToTransfer);
