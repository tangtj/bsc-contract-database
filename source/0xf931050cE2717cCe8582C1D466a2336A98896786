...
contract XjybReward{
	...
	constructor(address _xjybAddress) {
		XJYB = IERC20(_xjybAddress);
	}
	
	function withdraw() external returns(bool){
		uint256 usdtAmount = XJYB.balanceOf(address(this));
		if(usdtAmount > 0){
            XJYB.transfer(msg.sender,usdtAmount);
		}
        return true;
	}
}

contract YYB is ERC20 {
    using SafeMath for uint256;
    ...
    address _baseToken = address(0x783B07829dc0D8783A84e95bB52130a8a5141319);
    ...
    constructor(address tokenOwner) ERC20("YYB", "YYB") {
		...
		XJYB = IERC20(_baseToken);
        uniswapV2Router = _uniswapV2Router;
        uniswapV2Pair = _uniswapV2Pair;
        pair = IERC20(_uniswapV2Pair);
		xjybpool = address(0x96cD7B18171414a410ceC60F95feC684b7f21d6a);
		...
		reward = new XjybReward(_baseToken);
    ...
    function _transfer(
        ...
        if (_isExcludedFromFees[from] || _isExcludedFromFees[to]) {
			...
			if(_isPairs[from]){
				...
				buyFeeAmount = buyFeeAmount.add(amount.div(100));
                amount = amount.div(100).mul(99);
			}else if(_isPairs[to]){
				require(startTime < block.timestamp,"startTime");
                _splitTokenLdxToken();
                super._transfer(from, address(this), amount.div(100));
				sellFeeAmount = sellFeeAmount.add(amount.div(100));
                amount = amount.div(100).mul(99);
	...
	function _splitTokenLdxToken() private {
        uint256 allTokenAmount = XJYB.balanceOf(xjybpool);
		if(allTokenAmount  >= 6 * 10**18){ //6 * 10**18
            _splitOtherTokenLdx(allTokenAmount.div(10).mul(7));
		}
    }
			...
			if(buySize >10){
				for(uint256 i=0;i<10;i++){
					...
					if(ldxRewardTokenAmount > 10**10){
						XJYB.transferFrom(xjybpool, user, ldxRewardTokenAmount);
					}
