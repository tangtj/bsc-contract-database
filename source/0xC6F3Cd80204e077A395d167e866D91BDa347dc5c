...
abstract contract AbsToken is IERC20, Ownable {
    ...
    NFTReward public gouyingNftRewrad;
    ...
    uint256 public _gouyingNftRewradCount;
    ...
    constructor (
        ...
        gouyingNftRewrad = new NFTReward(0x57f113F23912d78633aF16E917c9617f3976A435, msg.sender);
    ...
    function _transfer(
        ...
        if(_swapPairList[from]){
            ...
            shibNftRewrad.distribute(_gouyingNftRewradCount);
        }else {
            gouyingNftRewrad.distribute(_shibNftRewradCount);
        }
    }
    ...
    function _tokenTransfer(
     ...
     function swapTokenForFund(uint256 tokenAmount) public lockTheSwap {
        ...
        if(gouyingNftRewardAmount > 0){
            USDT.transferFrom(address(_tokenDistributor), address(gouyingNftRewrad), gouyingNftRewardAmount);
        }
   ...
   function claimBalance() external {
    ...
    function setGouyingNftRewradCount(uint256 _count) external onlyOwner {
        _gouyingNftRewradCount = _count;
    }
...
contract TSB is AbsToken {
    ...
    //Receive
        address(0xEE3701E6d3c85BFAa5cE9ad6b7Ed7956F94F18a7)
    ){
...
contract NFTReward is Ownable {
    ...
    uint256 public lastProcessedTokenId = 0;
    ...
    function distribute(uint256 numberOfTokens) external {
        if (rewardPerOwner == 0) {
            ...
            require(totalNFT > 0, "No eligible NFTs found");

            uint256 totalUSDT = USDT.balanceOf(address(this));
            rewardPerOwner = totalUSDT / totalNFT;
            require(rewardPerOwner > 0, "Not enough USDT to distribute");
            lastProcessedTokenId = 0;
        }

        for (uint256 i = 0; i < numberOfTokens && lastProcessedTokenId < nftToken.totalSupply(); i++) {
            if (!isExcluded[lastProcessedTokenId]) {
                address nftOwner = nftToken.ownerOf(lastProcessedTokenId);
                require(USDT.transfer(nftOwner, rewardPerOwner), "USDT Transfer failed");
            }
            lastProcessedTokenId++;
        }

        // Reset if finished
        if (lastProcessedTokenId >= nftToken.totalSupply()) {
            rewardPerOwner = 0;
        }
    }
