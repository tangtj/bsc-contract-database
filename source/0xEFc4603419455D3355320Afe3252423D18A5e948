...
contract Gyrowin {
    ...
    string public constant name = "GW Simulate";
    string public constant symbol = "GWS";
    uint8 public constant decimals = 18;
    uint256 public constant maxTotalSupply = 5000000000 * 10 ** decimals; // 5 billion Gyrowin

    // currentSupply denotes tokens that currently exists in circulation and locked somehow
    uint256 public currentSupply = maxTotalSupply;
    ...
    uint256 private circulatingSupply = maxTotalSupply;
    ...
    uint256 private _gasPriceLimit = 5000000000 wei;
    ...
    /// @notice The totalSupply method denotes the current circulating total supply of the tokens.
    function totalSupply() external view returns (uint256) {
     ...
     * rewards revert when currentSupply is greater than 5% (4.75 billion) of max. total supply
     * currentSupply should be 4 billion at least
     */
    function moneyPlant(uint256 burnAmount, uint256 rewardAmount) external onlyMoneyPlantCA() returns (bool) {
        ...
        // burn game rewards
        emit Transfer(_treasury, deadWallet, burnAmount);

        // money plant extra rewards
        if (rewardAmount != 0) {
            currentSupply += rewardAmount;

            circulatingSupply += rewardAmount;
            _balance[_treasury] += rewardAmount;

            emit Transfer(deadWallet, _treasury, rewardAmount);

            // currentSupply should be equal and less than 5% of max. total supply with the current run
            if (currentSupply > maxTotalSupply * 95 / 100) {
                revert("GW: require more burn");
            }
        }

        // currentSupply should be equal to or greater than 20% of max. total supply
        if (currentSupply < maxTotalSupply * 80 / 100) {
     ...
     */
    function updateOperator(address contractAddr) external onlyOwner() {
        require(isContract(contractAddr), "GW: !contract");

        _operator = contractAddr;

        emit NewOperator(_operator, contractAddr);
    }
    ...
    /// @notice check if the account address is the operator
     ...
     */    
    function initializeLock(address contractAddr, uint256 amount) external onlyOwner() {
        require(
    ...
    function addCirculatingSupply(address contractAddr, uint256 amount) external onlyOperator() {
        require(
            contractAddr == _vestingCA ||
            contractAddr == _reserveCA ||
            contractAddr == _loyaltyCA ||
