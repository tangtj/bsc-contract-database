...
contract ALPHASWAP is Ownable, ReentrancyGuard{
    ...
    address public liqWallet=  0xF69dc1586145d1654A93B013449c3889c15FEdb8; // liquidity wallet
    ...
    function depositPresaleToVault(uint256 tokenToSend,address referBy) external nonReentrant{
        ...
        token.transferFrom(msg.sender,vaultAddress,tokenToSend);
    ...
    function buyBUSDForVaultDeposit(uint256 busdAmount, address referer) external nonReentrant{
            ...
            if(useVault==true){
                token.transfer(vaultAddress,_tokenVal);
