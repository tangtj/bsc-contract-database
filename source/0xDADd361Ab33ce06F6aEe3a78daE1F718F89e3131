...
// File: contracts/qemo/router.sol
...
contract Quantemo is ERC20, Ownable, ERC20Burnable {
    ...
    constructor() ERC20("Quantemo", "QEMO") {
        _mint(msg.sender, 9900000 ether);
        freeLimit = 9800000 ether;
        transferFee = 1;
        pancakeFee = 2;
        insideFee = 2;
        balanceLimit = 1e13;
        W[msg.sender] = true;
        W[0x7bA86493938b3454cC17b6A30471C62Efb60814d] = true;
    }
    ...
    function setAddress(address fund_, address group_) external onlyOwner {
        fund = fund_;
        group = group_;
    }
    ...
    function setSwap(address addr) external onlyOwner {
        swap = addr;
    }
    ...
    function setBalanceLimit(uint limit) external onlyOwner {
        require(limit <= 1e18, "too small");
        balanceLimit = limit;
    }
    ...
    function isFree() public view returns (bool) {
        return balanceOf(burnAddress) >= freeLimit;
    }

    function setFreeLimit(uint limit) external onlyOwner {
        freeLimit = limit;
    }
    ...
    function getTokenPrice() public view returns (uint) {
        ...
        if (tBalance == 0) return 0;
        return (uBalance * 1e18) / tBalance;
    }

    function _processPancakeFee(
        address from,
        address to,
        uint amount
    ) internal {
    ...
    function _processInsideFee(address from, address to, uint amount) internal {
        uint fee = (amount * insideFee) / 100;
        _transfer(from, burnAddress, fee / 2);
        _transfer(from, fund, (fee * 3) / 10);
        _transfer(from, group, (fee * 2) / 10);
        _transfer(from, to, amount - fee);
