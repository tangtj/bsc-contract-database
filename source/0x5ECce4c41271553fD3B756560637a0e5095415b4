...
contract BEP20PayToken is Ownable{
    ...
    function payBnb(uint orderid) external payable returns(bool){

        require(0 < msg.value, 'Amount: must be > 0');
        ...
        emit PayToken(bnb, sender, amount, orderid);
    ...
    function withDrawalBnb(address _address, uint amount) external onlyOwner returns(bool){
        ...
        emit WithDrawalToken(bnb, _address, amount);
