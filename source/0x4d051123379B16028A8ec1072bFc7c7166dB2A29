...
contract CompPlan is Pausable, Ownable {
    ...
    function purchaseWithToken(address _user, address _token, uint _type, uint _tier, address _ref) public whenNotPaused {
        ...
        uint _price = pricesTokens[_type * 10 + _tier][_token];
    ...
    function getCoinPrice() public view returns(uint256) {
        //WBNB-BUSD
        address gfamBUSDPair = pancakeSwapFactory.getPair(0xbb4CdB9CBd36B01bD1cBaEBF2De08d9173bc095c, 0xe9e7CEA3DedcA5984780Bafc599bD69ADd087D56);
        (uint112 reserve0, uint112 reserve1, ) = IPancakeSwapPair(gfamBUSDPair).getReserves();        
        uint256 k = uint256(reserve0) * uint256(reserve1); // Constant product
        uint256 newReserve0 = uint256(reserve0);
        uint256 newReserve1 = k / newReserve0; // New reserve after the trade        
        if (newReserve1 > reserve1) {
            return (newReserve1 - reserve1) * 10**18; // Adjusted price with slippage, scaled to 18 decimals
        } else {
            return (reserve1 - newReserve1) * 10**18; // Adjusted price with slippage, scaled to 18 decimals
        }
    }
