...
// mining v9
// tpis: eteUserToken=9-1MOS, eteCardToken=9-2MOS.
// new..
contract MiningV9 is Ownable, ReentrancyGuard {
    ...
    uint256 public canRatio = 250;         // account can deposit one issue is ratio. default 250%%.
    ...
    uint256 public userPoolRatio = 5000;                   // user pool ratio, remaining is card pool.
    ...
    constructor(
        ...
        oneIssueTime = _dayTime * 11;  // default is 11 day.
    ...
    function setUserPoolRatio(uint256 _userPoolRatio) public onlyOwner {
        require(_userPoolRatio > 0 && _userPoolRatio < 10000, "userPoolRatio error");
        userPoolRatio = _userPoolRatio;
    }
    ...
    function _startIssue(uint256 _turn, uint256 _issue, uint256 _totalUsdt) private {
        uint256 _third = _totalUsdt.mul(userPoolRatio).div(10000);
