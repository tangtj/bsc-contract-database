...
contract HIPPO is Context, Ownable, IBEP20, BEP20Detailed {
  ...
  using SafeMath for uint256;
    ...
    address payable public developmentAddress = payable(0xF4b8BE4EBF3a28C88EfE95951bfBE5f3F0F6AC23);
  
    bool inSwapAndLiquify;
    bool private swapAndLiquifyEnabled = true;

    uint256 public numTokensSellToFee = 2000000 * 10**18; // 0.001% of Total Supply
    ...
    constructor () BEP20Detailed("Crazy Hippo", "HIPPO", 18) {
      _owner = msg.sender ;
    ...
    function setBuyDevelopmentFeePercent(uint256 updatedBuyDevelopmentFee) external onlyOwner {
        require(updatedBuyDevelopmentFee <= 3, "Buy fee should be less than 3%");
        buyDevelopmentFee = updatedBuyDevelopmentFee;
    }

    function setSellDevelopmentFeePercent(uint256 updatedSellDevelopmentFee) external onlyOwner {
        require(updatedSellDevelopmentFee <= 3, "Sell fee should be less than 3%");
        sellDevelopmentFee = updatedSellDevelopmentFee;
    }

    function setDevelopmentAddress(address payable wallet) external onlyOwner {
        require(wallet != developmentAddress,"Development address is already that address");
        require(wallet != address(0), "Development address cannot be dead address");
        require(!isContract(wallet), "Development address cannot be contract");
    ...
    function changeNumTokensSellToFee(uint256 _numTokensSellToFee) external onlyOwner {
        require(_numTokensSellToFee >= 200000 * 10**18 && _numTokensSellToFee <= 2000000000 * 10**18, "Swap to fee threshold must be set within 200,000 to 2,000,000,000 tokens");
    ...
    function _transfer(address sender, address recipient, uint amount) internal{
        ...
        if(takeFee)
        {
            if(sender == uniswapV2Pair){developmentFee = buyDevelopmentFee;}
            else{developmentFee = sellDevelopmentFee;}
            uint256 taxAmount = amount.mul(developmentFee).div(100);
     ...
     function swapAndLiquify(uint256 contractTokenBalance) private lockTheSwap {
        ...
        // swap tokens for ETH
        swapTokensForEth(contractTokenBalance); // <- this breaks the ETH -> HIPPO swap when swap+liquify is triggered
