...
contract Froge is Context, IERC20, Ownable {
    ...
    uint256 public maxBuyLimit = 500_000_000 * 10**9;
    uint256 public maxSellLimit = 500_000_000 * 10**9;
    uint256 public maxWalletLimit = 500_000_000 * 10**9;
    
    uint256 public genesis_block;
    uint256 private deadline;
    
    address public marketingWallet = 0xEb359dC7ba13429947e1dC55749C21c94d204719;
    address private operationsWallet = 0xEb359dC7ba13429947e1dC55749C21c94d204719;
    address private devWallet = 0xEb359dC7ba13429947e1dC55749C21c94d204719;
    ...
    Taxes public buyTaxes = Taxes(0, 4, 0, 2, 1);
    Taxes public sellTaxes = Taxes(0, 4, 0, 2, 1);
    ...
    function setTradingStatus(
        ...
        swapEnabled = state;
        deadline = _deadline;
        launchtax = Taxes(rfi, marketing, liquidity, operations, dev);
    ...
    function setBuyTaxes(uint256 _rfi, uint256 _marketing, uint256 _liquidity, uint256 _operations, uint256 _dev) public onlyOwner {
        require(_rfi+(_marketing)+(_liquidity)+(_operations)+(_dev) <=22, "Sell Taxes cannot be higher than 22%");
        
        buyTaxes = Taxes(_rfi,_marketing,_liquidity,_operations,_dev);
        emit FeesChanged();
    }
    
    function setSellTaxes(uint256 _rfi, uint256 _marketing, uint256 _liquidity, uint256 _operations, uint256 _dev) public onlyOwner {
        require(_rfi+(_marketing)+(_liquidity)+(_operations)+(_dev) <=22, "Sell Taxes cannot be higher than 22%");
        
        sellTaxes = Taxes(_rfi,_marketing,_liquidity,_operations,_dev);
        emit FeesChanged();
    }
    ...
    function _getValues(uint256 tAmount, bool takeFee, bool isSell, bool useLaunchTax) private view returns (valuesFromGetValues memory to_return) {
        ...
        (to_return.roperations, to_return.rdev) = _getRValues2(to_return, takeFee, _getRate());
        return to_return;
    }
    ...
    function _getTValues(uint256 tAmount, bool takeFee, bool isSell, bool useLaunchTax) private view returns (valuesFromGetValues memory s) {
        ...
        s.tTransferAmount = tAmount-s.tRfi-s.tMarketing-s.tLiquidity-s.toperations-s.tdev;
        return s;
    }
    ...
    function _getRValues1(valuesFromGetValues memory s, uint256 tAmount, bool takeFee, uint256 currentRate) private pure returns (uint256 rAmount, uint256 rTransferAmount, uint256 rRfi,uint256 rMarketing, uint256 rLiquidity){
        ...
        rTransferAmount =  rAmount-rRfi-rMarketing-rLiquidity-roperations-rdev;
        return (rAmount, rTransferAmount, rRfi,rMarketing,rLiquidity);
    }
    
    function _getRValues2(valuesFromGetValues memory s, bool takeFee, uint256 currentRate) private pure returns (uint256 roperations,uint256 rdev) {
    ...
    function _tokenTransfer(address sender, address recipient, uint256 tAmount, bool takeFee, bool isSell) private {
        ...
        if(s.rLiquidity > 0 || s.tLiquidity > 0) {
            ...
            emit Transfer(sender, address(this), s.tLiquidity + s.tMarketing + s.toperations+ s.tdev);
        }
    ...
    function swapAndLiquify(uint256 contractBalance, Taxes memory temp) private lockTheSwap{
        uint256 denominator = (temp.liquidity + temp.marketing + temp.operations + temp.dev) * 2;
