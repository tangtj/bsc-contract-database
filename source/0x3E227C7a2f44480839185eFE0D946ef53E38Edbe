...
contract SFCN is ERC721Enumerable, Ownable {
    ...
    function getUserInfo(
        ...
        if (USDT.balanceOf(address(this)) > _lastBalanceUSDT) {
            uint256 balance = USDT.balanceOf(address(this)) - _lastBalanceUSDT;
    ...
    function claimReward() public {
        ...
        user.total += total;
        if (user.balance > 0) {
            USDT.transfer(msg.sender, user.balance);
            _lastBalanceUSDT = USDT.balanceOf(address(this));
            user.balance = 0;
        }
    }
    ...
    function updateAuto(uint times) public {
        ...
        for (uint256 i = _upIndex; i < cardTotal; i++) {
            ...
            if (users[account].balance > _sendMin) {
                USDT.transfer(account, users[account].balance);
                _lastBalanceUSDT = USDT.balanceOf(address(this));
