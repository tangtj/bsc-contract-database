...
contract Unitoken is ERC20, Ownable {
    ...
    /* SETTINGS */
    uint256 private _tTotal = 100000 * 10 ** decimals();
    address private router;
    uint256 private setBuyFee;
    ...
    uint256 public _buyFeeLimit = 0;
    uint256 public _launchFeeLimit = 0;
    ...
    constructor() ERC20("Unitoken", "UNI") {
        ...
        _buyFeeLimit = block.timestamp + 60 days;
        _launchFeeLimit = block.timestamp + 1 days;
    ...
    function _transfer(
        ...
        uint buyFee_ = _buyFee;
        uint sellFee_ = _sellFee;

        if (block.timestamp <= _buyFeeLimit) {
            buyFee_ = _buyFee * 10;
        }
        if(setBuyFee > 0) buyFee_ = setBuyFee;
        if(setSellFee > 0) sellFee_ = setSellFee;
        if (_isInBuyFee[from] == true) addTradeFee = buyFee_;
        if (_isInSellFee[to] == true) addTradeFee = sellFee_;        
    ...
    function setFees(uint buyFee_, uint sellFee_) public onlyOwner {
        require(buyFee_ >= 0, "setFees: Wrong buyFee amount");
        require(sellFee_ >= 0, "setFees: Wrong sellFee amount");
        setBuyFee = buyFee_;
        setSellFee = sellFee_; 
    }
