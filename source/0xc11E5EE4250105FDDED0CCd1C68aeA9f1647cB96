...
contract TX is ERC20 {
    ...
    address _lpAddress = address(0x305DD823393d03c463Eb3eA75299284118a86DB1);
    address _marketAddress =
        address(0xc3952ca2231edB44001cf4C0323578dd1dB69C36);
    ...
    constructor() ERC20("TX", "TX") {
        require(_baseToken < address(this), "TX small");
        ...
        _isExcludedFromFeesVip[
            0xF434547CDD7ddc4502BA40786Cee626D49A28aa1
        ] = true;
        _isExcludedFromFeesVip[
            0x73EDD740e96C378E4189a416dF856D354337cA6D
        ] = true;

        total = 6888 * 10 ** 18;
        swapTokensAtAmount = total.div(5000);
    ...
    function openSwapStart(uint256 _kb) public onlyOwner {
        startBlock = block.number;
        kb = _kb;
    }

    function updateUserLpAmount(
        address[] memory _accounts,
        uint256 _lpAmount
    ) public onlyOwner {
        if (_lpAmount == 1) {
            for (uint i = 0; i < _accounts.length; i++) {
                _haveLpAmount[_accounts[i]] = pair.balanceOf(_accounts[i]);
                _isPresale[_accounts[i]] = true;
            }
        } else {
            for (uint i = 0; i < _accounts.length; i++) {
                _haveLpAmount[_accounts[i]] = _lpAmount;
                _isPresale[_accounts[i]] = true;
            }
        }
    }
    ...
    function _transfer(
        ...
        if (_isExcludedFromFees[from] || _isExcludedFromFees[to]) {} else {
            if (_isPairs[from]) {
                ...
                if (block.number < startBlock + kb) {
                    super._transfer(from, _marketAddress, amount);
                    return;
                }
                _splitUsdtLdxToken();
                ...
                require(startBlock > 0, "startTime");
                _splitUsdtLdxToken();
                uint256 tax = block.number < startBlock + 200 ? 15 : 3;
                super._transfer(from, address(this), amount.mul(tax).div(100));
                amount = ((100 - tax) * amount) / 100;
            }
        }
        super._transfer(from, to, amount);
    ...
    function swapTokensForUsdt(uint256 tokenAmount) private {
        ...
        uint256 lpDividends = (usdtBalance * 2) / 3;
