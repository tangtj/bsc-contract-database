...
contract Business {
    ...
    function Reinvest() external {
        ...
        if (DepositUSD < 2) {// reinvest50 - withdrawal50
         require(token.transfer(msg.sender, (EarnedTOKEN * 5) / 10), "Withdrawal failed");
         emit TokensWithdrawn(msg.sender, (EarnedTOKEN * 5) / 10, tokenAddress);  
        }
        else if (DepositUSD >= 2 && DepositUSD < 3) {// reinvest40 - withdrawal60
         require(token.transfer(msg.sender, (EarnedTOKEN * 6) / 10), "Withdrawal failed");
         emit TokensWithdrawn(msg.sender, (EarnedTOKEN * 6) / 10, tokenAddress);  
        } 
        else {// reinvest60 - withdrawal40
         require(token.transfer(msg.sender, (EarnedTOKEN * 4) / 10), "Withdrawal failed");
         emit TokensWithdrawn(msg.sender, (EarnedTOKEN * 4) / 10, tokenAddress);  
        }
