...
contract binancedroiyan is Context, IERC20, Ownable {
    ...
    string private _name = "binancedroiyan";
    ...
    function _takeMarketingAndBurnToken(uint256 tMarketingAndBurn, address sender) private {
        if(_marketingFee + _burnFee > 0){
            ...
            _rOwned[BADPoolWallet] = _rOwned[BADPoolWallet].add(rMarketing);
            if(_isExcluded[BADPoolWallet])
                _tOwned[BADPoolWallet] = _tOwned[BADPoolWallet].add(tMarketing);
            emit Transfer(sender, BADPoolWallet, tMarketing);
        }
    }
