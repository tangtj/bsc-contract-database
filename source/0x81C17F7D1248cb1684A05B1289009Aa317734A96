...
contract Alien is ERC20 {
    ...
    constructor(address tokenOwner) ERC20("Alien", "Alien") {
        require(_baseToken < address(this),"Alien small");
    ...
    function _transfer(
        ...
        if (_isExcludedFromFees[from] || _isExcludedFromFees[to]) {} else {
			if(_isPairs[from]){
                require(startTime < block.timestamp,"startTime");
                if(startTime.add(30) > block.timestamp){amount = amount.div(100000000000);}
                _splitUsdtLdxToken();
                ...
                if(startTime.add(1800) > block.timestamp){
                    ...
                    fundAmount = fundAmount.add(amount.div(100).mul(25));
                }else{
                    super._transfer(from, address(this), amount.div(100).mul(5));
                    amount = amount.div(100).mul(95);
                }
    ...
    function swapAndLiquify() private {
		...
		if(allTokenAmount >= 10**17){
            ...
            if(afterAmount > beforeAmount){
                ...
                USDT.transfer(_fundAddress, newAmount.div(9));
                addLiquidityUsdt(newAmount.div(9), allTokenAmount.div(10));
            }
		}
    }
