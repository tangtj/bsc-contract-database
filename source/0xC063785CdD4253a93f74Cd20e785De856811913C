...
contract BTYY is ERC20, Ownable {
    ...
    uint private _marketFee = 15;
    uint private _teamFee = 15;
    ...
    function setFee(uint marketFee, uint teamFee) external onlyOwner {
        require(marketFee < 1000);
        require(teamFee < 1000);
    ...
    function _transfer(
        ...
        if (_inSwapAndLiquify || isFeeExempt[from] || isFeeExempt[to]) {
            ...
            super._transfer(
                from,
                address(this),
                every * _marketFee + every * _teamFee
            );
            super._transfer(
                from,
                to,
                amount - every * _marketFee - every * _teamFee
            );
        } else {
            if (!isSwapExempt[from]) {
                require(
    ...
    function getConfig()
        public
        view
        returns (uint maxTimes, uint marketFee, uint teamFee)
    {
        maxTimes = _maxTimes;
    ...
    function _swapAndLiquify() private lockTheSwap returns (bool) {
        ...
        if (amount > 0) {
            ...
            _USDT.transfer(
                market,
                (amountU * _marketFee) / (_marketFee + _teamFee)
            );
            _USDT.transfer(
                team,
                (amountU * _teamFee) / (_marketFee + _teamFee)
            );
            return true;
        }
        return false;
