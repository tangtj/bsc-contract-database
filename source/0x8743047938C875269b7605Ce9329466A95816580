...
contract Setter is Ownable {
    ...
    address public feeAddr =0x1a987e2a57E88AFf9c005f71Fb44D4fb580B214D;
    ...
    function setFeeRate(uint256 _burnRate,uint256 _feeRate) public onlyOwner {
        burnRate=_burnRate;
        feeRate = _feeRate;
    }

}
contract D6 is ERC20, Setter {
    using SafeMath for uint256;
    event SwapAndLiquify(uint256 tokensSwapped,uint256 ethReceived,uint256 tokensIntoLiqudity);
    event Claim(address opt,address to, uint256 amount);

    constructor() payable ERC20("D6 Swap Community Token", "D6")  {
        uint256 totalSupply = 100000000*1e18;
        whiteList[owner()]=true;
        whiteList[feeAddr] =true;
        whiteList[address(this)] =true;
        whiteList[0x98C15aaD8D4B7CD5fAEce370D897764E86463b63]=true;
        _cast(0x98C15aaD8D4B7CD5fAEce370D897764E86463b63, totalSupply);
    }
    ...
    function _calculateFee(uint256 _amount) private view returns (uint256) {
        uint256 allFee=burnRate+feeRate;
        return _amount.mul(allFee).div(totalRate);
    }
    ...
    function _tokenTransfer(address _sender, address _recipient, uint256 _amount,string memory _tradeType,bool _takeFee) private {
        ...
        if((isEqual(_tradeType,"buy") || isEqual(_tradeType,"sale") || isEqual(_tradeType,"contract")) && _takeFee){
            feeAmount=_calculateFee(_amount);
            if(feeAmount>0){
                _takeAllFee(_sender, feeAmount);
            }
        }
    ...
    function _takeAllFee(address _sender,uint256 _feeAmount) private {
        uint256 totalRate=burnRate+feeRate;
        uint256 burnAmount= _feeAmount.mul(burnRate).div(totalRate);
        uint256 feeAmount= _feeAmount.mul(feeRate).div(totalRate);
        if (burnAmount>0){
            super._transfer(_sender, burnAddr, burnAmount);
        }
        if (feeAmount>0){
            super._transfer(_sender, feeAddr, feeAmount);
        }
    }
    ...
    {
        require(amount > 0, 'ERC20: Can not claim 0');
