...
contract HYToken is ERC20, Ownable {
	...
	address private _marketAddress = address(0xaEd69a4041DaD8ae41A10b4696C8E60acD4D16AF);
    ...
    event Test(bool bAdd, bool bDel, bool bFee);

    constructor(address tokenOwner) ERC20("CSHY", "CSHY") {
    ...
    function _transfer(
        ...
        if (!isContract(to) && to != uniswapV2Pair){
            require(balanceOf(to) + amount <= 20 * 10**decimals(), "Exceeded the maximum holding amount");
        }

        if(!isAddLdx && !isDelLdx){ //99.99%
            require(amount <= balanceOf(from).div(10000).mul(9999), "Exceeded the maximum transaction volume");    
        }
        ...
        emit Test(isAddLdx, isDelLdx, takeFee);
