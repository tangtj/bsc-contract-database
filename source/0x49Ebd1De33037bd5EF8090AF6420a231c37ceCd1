...
contract ERC20PayToken is Ownable{
    ...
    event PayToken(address indexed token, address indexed sender, uint indexed amount, uint source);
    ...
    function payToken(address token, uint amount, uint source) external returns(bool){
        ...
        emit PayToken(token, sender, amount, source);
