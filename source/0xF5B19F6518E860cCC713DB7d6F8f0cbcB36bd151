...
contract Wrap {
    ...
    function transfer(address token, address marketAddress) external{
        uint allAmount = IERC20(token).balanceOf(address(this));
        IERC20(token).safeTransfer(marketAddress, allAmount);
    }
...
contract zjz is Context, IERC20, Ownable {
    using SafeMath for uint256;
    using SafeERC20 for IERC20;
    ...
    address marketAddress = 0x8230DAa4411792cd7Bb336D40cc00C9d4419009f;
    
    uint256 private constant MAX = ~uint256(0); 
    uint256 private _tTotal = 10000 * 10**18;
	
    string private _name = "zjz";
    string private _symbol = "zjz";
    ...
    uint256 public minSwappNum = 1e16;
	
	address public ownerAddress = address(0x4a61FFDb31735702b786C9e25f3A5013C62e445B);
	...
	function _transferStandard(address sender, address recipient, uint256 tAmount, bool takeFee) private {
        ...
        if (takeFee) {
            (uint256 feeAmount, uint256 sendAmount,)
                = _getTValues(tAmount);
            if (feeAmount != 0){
                uint burnAmount = feeAmount.div(6);
               _rOwned[address(this)] = _rOwned[address(this)].add(feeAmount.sub(burnAmount));
			    emit Transfer(sender, address(this), feeAmount.sub(burnAmount));

               _rOwned[address(0)] = _rOwned[address(0)].add(burnAmount);
    ...
    function _getTValues(uint256 tAmount) private view returns (uint256, uint256, uint) {
        uint doType = 0;
		uint feeAmount = tAmount.mul(_fee).div(1000);
        uint sendAmount = tAmount.sub(feeAmount);
    ...
    function swapTokensForToken(uint256 tokenAmount) private {
        ...
        wrap.transfer(usdtAddress, marketAddress);
    }
}