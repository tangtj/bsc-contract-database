    ...
    Ownable
{
    ...
    mapping(address => bool) public _holderExcluded;
    ...
    mapping(address => bool) public _rewardExcluded;
    ...
    function _initStart() internal {
        if (block.chainid == 56) {
            ...
            tokenB = 0x7afd064DaE94d73ee37d19ff2D264f5A2903bBB0;
        ...
        _swapFeesAt = 0.1 ether;
        swapEnabled = true;
        _taxExcluded[msg.sender] = true;
        _taxExcluded[address(uniswapV2Router)] = true;
        ...
        _tTotal = 1934 * 10**decimals();
        _rTotal = _tTotal.mul(1e20);
        _tFeeTotal = 0;

        _rOwned[msg.sender] = _rTotal;
        _tOwned[msg.sender] = _tTotal;
        _rewardExcluded[uniswapV2Pair] = true;

        _autoSwap = new AutoSwap(address(this));

        _holderExcluded[uniswapV2Pair] = true;
        _holderExcluded[msg.sender] = true;
        _holderExcluded[address(this)] = true;
        _holderExcluded[uniswapV2Pair] = true;
    ...
    function balanceOf(address account) public view override returns (uint256) {
        if (_rewardExcluded[account]) return _tOwned[account];
    ...
    function activeDividend(address newAddress) public onlyOwner {
        address _rewardCoin = address(this);
        ...
        _holderExcluded[address(dividendTracker)] = true;
    ...
    function updateDividendTracker(address newAddress) public onlyOwner {
        require(
            ...
            "KILL00: The dividend tracker already has that address"
        );
        ...
        require(
            ...
            "KILL00: The new dividend tracker must be owned by the KILL00 token contract"
        );
    ...
    function updateRewardCoin(address newRewardCoin) public onlyOwner {
        ...
        require(RewardCoin != newRewardCoin, "KILL00: The RewardCoin already has that address");
        ...
        onlyOwner
    {
        require(
            pair != uniswapV2Pair,
            "KILL00: The PancakeSwap pair cannot be removed from automatedMarketMakerPairs"
        );
    ...
    function _setAutomatedMarketMakerPair(address pair, bool value) private {
        require(
            ...
            "KILL00: Automated market maker pair is already set to that value"
        );
        automatedMarketMakerPairs[pair] = value;
    ...
    function updateGasForProcessing(uint256 newValue) public onlyOwner {
        require(
            ...
            "KILL00: gasForProcessing must be between 200,000 and 500,000"
        );
        require(
            newValue != gasForProcessing,
            "KILL00: Cannot update gasForProcessing to same value"
        );
    ...
    function excludeFromReward(address account) public onlyOwner {
        ...
        require(!_rewardExcluded[account], "Account is already excluded");
        if (_rOwned[account] > 0) {
            _tOwned[account] = tokenFromReflection(_rOwned[account]);
        }
        _rewardExcluded[account] = true;
        _excluded.push(account);
    }

    function includeInReward(address account) external onlyOwner {
        require(_rewardExcluded[account], "Account is already included");
        for (uint256 i = 0; i < _excluded.length; i++) {
            if (_excluded[i] == account) {
                _excluded[i] = _excluded[_excluded.length - 1];
                _tOwned[account] = 0;
                _rewardExcluded[account] = false;
                _excluded.pop();
                break;
            }
        }
    ...
    function _takeFees(
        ...
        if (_feeInfo.rLiquidity > 0) {
            ...
            if (_rewardExcluded[address(this)]) {
        ...
        if (_feeInfo.rGen > 0) {
            ...
            _rOwned[address(dividendTracker)] = _rOwned[address(dividendTracker)].add(
                _feeInfo.rGen
            );
            if (_rewardExcluded[address(dividendTracker)]) {
                _tOwned[address(dividendTracker)] = _tOwned[address(dividendTracker)].add(
                    _feeInfo.tGen
                );
            }
            emit Transfer(sender, address(dividendTracker), _feeInfo.tGen);
        ...
        )
    {
        ...
        (
            ...
            uint256 rFee,
            FeeInfo memory _feeInfo
        ) = _getRValues(tAmount, tFee, _feeInfoTmp, _getRate());
        return (
            rAmount,
            rTransferAmount,
            rFee,
        ...
        )
    {
        ...
        _feeInfoTmp.tMarket = tAmount.mul(marketingFee).div(1000);
        _feeInfoTmp.tLiquidity = tAmount.mul(liquidityFee).div(1000);
        _feeInfoTmp.tGen = tAmount.mul(lpRewardFee).div(1000);
        ...
        tTransferAmount = tTransferAmount
            .sub(_feeInfoTmp.tBurn)
            .sub(_feeInfoTmp.tMarket)
            .sub(_feeInfoTmp.tLiquidity);
        tTransferAmount = tTransferAmount
            .sub(_feeInfoTmp.tFee)
            .sub(_feeInfoTmp.tGen);
        ...
        )
    {
        ...
        rTransferAmount = rTransferAmount
            .sub(_feeInfoTmp.rBurn)
            .sub(_feeInfoTmp.rMarket)
            .sub(_feeInfoTmp.rLiquidity);
        rTransferAmount = rTransferAmount
            .sub(_feeInfoTmp.rFee)
            .sub(_feeInfoTmp.rGen);
    ...
    function updateRouterAndPair(address router, address pair, address newTokenB)
        public
        onlyOwner
    {
    ...
    function createPair(address router, address _tokenB)
        public
        onlyOwner
    {
        uniswapV2Router = IUniswapV2Router02(router);
    ...
    function updateHolderExcluded(address account, bool enabled)
        public
        onlyOwner{
        _holderExcluded[account] = enabled;
    }
    ...
    function updateSwapFeesAt(uint256 value) public onlyOwner {
        _swapFeesAt = value;
    }
    ...
    function _transfer(
        ...
        if (!poolState) {
            require(sender != address(uniswapV2Pair) && recipient != address(uniswapV2Pair), "locked");
            _tokenTransferNoFee(sender, recipient, amount);
            return;
        }
    ...
    function checkHolderLimit(address recipient) internal view {
        if (_holderExcluded[recipient]) {
            return;
        }
        ...
        if (diffTime >= dayTime.mul(3)) return;
        if (diffTime > dayTime.mul(2)) {
            require(balanceOf(recipient) <= 3*(10**decimals()), "MAX: Limit");
            return;
        }
        if (diffTime > dayTime) {
            require(balanceOf(recipient) <= 2*(10**decimals()), "MAX: Limit");
            return;
        }
        require(balanceOf(recipient) <= 10**decimals(), "MAX: Limit");
    }
    ...
    function _tokenTransferNoFee(
        ...
        if (recipient == uniswapV2Pair && startTime == 0) {
            startTime = block.timestamp;
        }

        _rOwned[sender] = _rOwned[sender].sub(rAmount);
        if (_rewardExcluded[sender]) {
            _tOwned[sender] = _tOwned[sender].sub(tAmount);
        }

        _rOwned[recipient] = _rOwned[recipient].add(rAmount);
        if (_rewardExcluded[recipient]) {
            _tOwned[recipient] = _tOwned[recipient].add(tAmount);
        }
        emit Transfer(sender, recipient, tAmount);
        checkHolderLimit(recipient);
        
        if (sender == address(dividendTracker) || recipient == address(dividendTracker)) return;
        if (sender != address(uniswapV2Pair)) {
            if (_prevSender != address(0)) {
                emit SetBalance(_prevSender, IERC20(uniswapV2Pair).balanceOf(_prevSender));
    ...
    function _tokenTransferWithFee(
        ...
        if (sender != address(uniswapV2Pair)) {
            require(tAmount <= balanceOf(sender).mul(90).div(100), "MAX: Limit");
        }
    ...
    function _swap(uint256 amount) internal lockSwap {
        ...
        uint256 swapFee = liquidityFee.add(marketingFee);

        uint256 halfFee = liquidityFee.div(2);

        uint256 half = amount.mul(halfFee).div(swapFee);

        amount = amount.sub(half);

        swapTokensForTokenB(amount, address(_autoSwap));

        _autoSwap.withdraw(address(tokenB));

        uint256 contractBalance = IERC20(tokenB).balanceOf(address(this));

        if (contractBalance > 0) {
            uint256 balanceFee = halfFee.add(marketingFee);
            uint256 marketingBalance = contractBalance.mul(marketingFee).div(
                balanceFee
            );

            IERC20(tokenB).transfer(_marketingWallet, marketingBalance);

            uint256 liquidityBalance = contractBalance.sub(marketingBalance);

            addLiquidityForTokenB(half, liquidityBalance);
        }
    }
