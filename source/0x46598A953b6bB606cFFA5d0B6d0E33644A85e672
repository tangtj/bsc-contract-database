...
contract RoastedPapa is Ownable{
    ...
    mapping(address => uint256) public eggExpiryTime;
    uint256 public eggLifeTime = 1728000;  // 鸡蛋的生命周期，可根据需要调整
    ...
    function hatchEggs(address ref) public{
        ...
        //send referral eggs
        claimedEggs[referrals[msg.sender]]=SafeMath.add(claimedEggs[referrals[msg.sender]],SafeMath.div(SafeMath.mul(eggsUsed,15),100));//推荐奖励百分之10

        //boost market to nerf miners hoarding
        marketEggs=SafeMath.add(marketEggs,SafeMath.div(eggsUsed,15));
    }
    ...
    function buyEggs(uint256 amount, address ref) public {
        ...
        // 设置鸡蛋的到期时间
        eggExpiryTime[msg.sender] = now + eggLifeTime;
    ...
    function getMyEggs() public view returns(uint256){
        if (now >= eggExpiryTime[msg.sender]) {
            return 0;  // 如果鸡蛋已经过期，返回0
        }
