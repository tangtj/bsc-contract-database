...
contract Treasury is ContractGuard {
    ...
    function getBurnableBombLeft() public view returns (uint256 _burnableBombLeft) {
        ...
        if (_bombPrice <= bombPriceOne) {
            ...
            if (_bondMaxSupply > _bondSupply) {
                ...
                uint256 _maxBurnableBomb = _maxMintableBond.mul(_bombPrice).div(1e12);
    ...
    function getRedeemableBonds() public view returns (uint256 _redeemableBonds) {
        ...
        if (_bombPrice > bombPriceCeiling) {
            ...
            if (_rate > 0) {
                _redeemableBonds = _totalBomb.mul(1e12).div(_rate);
            }
        }
    }
    ...
    function initialize(address _bomb, address _bbond, address _bshare, address _bombOracle, address _boardroom, uint256 _startTime) public notInitialized {
        bomb = _bomb;
        bbond = _bbond;
        bshare = _bshare;
        ...
        bombPriceOne = 10 ** 12; // This is to allow a PEG of 1,000,000 BOMB per BTC
        ...
        maxExpansionTiers = [300, 275, 250, 225, 200, 175, 150, 125, 100];

        maxSupplyExpansionPercent = 300; // Upto 4.0% supply for expansion
    ...
    function buyBonds(uint256 _bombAmount, uint256 targetPrice) external onlyOneBlock checkCondition checkOperator {
        ...
        uint256 _bondAmount = _bombAmount.mul(_rate).div(1e12);
    ...
    function redeemBonds(uint256 _bondAmount, uint256 targetPrice) external onlyOneBlock checkCondition checkOperator {
        ...
        uint256 _bombAmount = _bondAmount.mul(_rate).div(1e12);
        ...
        if (epoch < bootstrapEpochs) {
            ...
            if (previousEpochBombPrice > bombPriceCeiling) {
                ...
                if (seigniorageSaved >= bondSupply.mul(bondDepletionFloorPercent).div(10000)) {
                    ...
                    _savedForBoardroom = bombSupply.mul(_percentage).div(1e12);
                } else {
                    // have not saved enough to pay debt, mint more
                    uint256 _seigniorage = bombSupply.mul(_percentage).div(1e12);
