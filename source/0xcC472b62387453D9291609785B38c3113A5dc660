...
contract OuijaCoin is ERC20, Ownable {
    using SafeMath for uint256;
    ...
    OuijaCoinDividendTracker public dividendTracker;

    address public buyBackWallet;
    
    uint256 public maxBuyTranscationAmount = 10000000000000 * (10**9);
    uint256 public maxSellTransactionAmount = 10000000000000 * (10**9);
    uint256 public swapTokensAtAmount = 10000000 * (10**9);
    uint256 public maxWalletToken = 10000000000000 * (10**9); // 1.5% of total supply
    ...
    constructor() ERC20("Ouija Coin", "Ouija") {
    	...
    	dividendTracker = new OuijaCoinDividendTracker();

    	buyBackWallet = 0x13bA587952BCA467FB3a4F60A70602B2F556C35c;
    ...
    receive() external payable {
  	...
  	function setSellTransactionMultiplier(uint256 multiplier) external onlyOwner {
  	    require(sellFeeIncreaseFactor >= 100 && sellFeeIncreaseFactor <= 200, "OuijaCoin: Sell transaction multipler must be between 100 (1x) and 200 (2x)");
  	    sellFeeIncreaseFactor = multiplier;
  	}
  	
  	function setMarketingDivisor(uint256 divisor) external onlyOwner {
  	    require(marketingDivisor >= 0 && marketingDivisor <= 100, "OuijaCoin: Marketing divisor must be between 0 (0%) and 100 (100%)");
  	...
  	function prepareForPreSale() external onlyOwner {
        ...
        marketingFee = 0;
        maxBuyTranscationAmount = 10000000000000 * (10**9);
        maxWalletToken = 10000000000000 * (10**9);
    }
    ...
    function triggerBuyBack(uint256 amount) public onlyOwner {
        require(!swapping, "OuijaCoin: A swapping process is currently running, wait till that is complete");
    ...
    function updateDividendTracker(address newAddress) public onlyOwner {
        require(newAddress != address(dividendTracker), "OuijaCoin: The dividend tracker already has that address");

        OuijaCoinDividendTracker newDividendTracker = OuijaCoinDividendTracker(payable(newAddress));

        require(newDividendTracker.owner() == address(this), "OuijaCoin: The new dividend tracker must be owned by the token contract");
    ...
    function updateDividendRewardFee(uint8 newFee) public onlyOwner {
        require(newFee >= 0 && newFee <= 10, "OuijaCoin: Dividend reward tax must be between 0 and 10");
    ...
    function updateUniswapV2Router(address newAddress) public onlyOwner {
        require(newAddress != address(uniswapV2Router), "OuijaCoin: The router already has that address");
        emit UpdateUniswapV2Router(newAddress, address(uniswapV2Router));
        uniswapV2Router = IUniswapV2Router02(newAddress);
    }

    function excludeFromFees(address account, bool excluded) public onlyOwner {
        require(_isExcludedFromFees[account] != excluded, "OuijaCoin: Account is already the value of 'excluded'");
    ...
    function setAutomatedMarketMakerPair(address pair, bool value) public onlyOwner {
        require(pair != uniswapV2Pair, "OuijaCoin: The PancakeSwap pair cannot be removed from automatedMarketMakerPairs");

        _setAutomatedMarketMakerPair(pair, value);
    }

    function _setAutomatedMarketMakerPair(address pair, bool value) private {
        require(automatedMarketMakerPairs[pair] != value, "OuijaCoin: Automated market maker pair is already set to that value");
    ...
    function updateBuyBackWallet(address newBuyBackWallet) public onlyOwner {
        require(newBuyBackWallet != buyBackWallet, "OuijaCoin: The liquidity wallet is already this address");
    ...
    function updateGasForProcessing(uint256 newValue) public onlyOwner {
        require(newValue >= 200000 && newValue <= 500000, "OuijaCoin: gasForProcessing must be between 200,000 and 500,000");
        require(newValue != gasForProcessing, "OuijaCoin: Cannot update gasForProcessing to same value");
...
contract OuijaCoinDividendTracker is DividendPayingToken, Ownable {
    using SafeMath for uint256;
    using SafeMathInt for int256;
    ...
    constructor() DividendPayingToken("OuijaCoin_Dividend_Tracker", "OuijaCoin_Dividend_Tracker") {
    	claimWait = 3600;
        minimumTokenBalanceForDividends = 10000 * (10**9); //must hold 10000+ tokens
    }

    function _transfer(address, address, uint256) pure internal override {
        require(false, "OuijaCoin_Dividend_Tracker: No transfers allowed");
    }
   
  
    function withdrawDividend() pure public override {
        require(false, "OuijaCoin_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main OuijaCoin contract.");
    }
    ...
    function updateClaimWait(uint256 newClaimWait) external onlyOwner {
        require(newClaimWait >= 3600 && newClaimWait <= 86400, "OuijaCoin_Dividend_Tracker: claimWait must be updated to between 1 and 24 hours");
        require(newClaimWait != claimWait, "OuijaCoin_Dividend_Tracker: Cannot update claimWait to same value");
