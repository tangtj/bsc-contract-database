...
contract farmingXMM {
    ...
    function calculateAccumulatedRewards(address _user, uint256 _currentTime) internal view returns (uint256 dogeRewards, uint256 shibRewards, uint256 flokiRewards) {
        ...
        dogeRewards = (dailyRewards * timeSinceLastClaim * 69) / secondsInDay / (10 ** decimalsDOGE);
        shibRewards = (dailyRewards * timeSinceLastClaim * 500000) / secondsInDay / (10 ** decimalsSHIB);
        flokiRewards = (dailyRewards * timeSinceLastClaim * 200000) / secondsInDay / (10 ** decimalsFLOKI);
    }
