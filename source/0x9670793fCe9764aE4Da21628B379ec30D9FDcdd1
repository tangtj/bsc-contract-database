...
contract test is Context, IERC20, Ownable {
    ...
    uint256 public _walletMax = 1 * 10**8 * 1**1 * 10**_decimals;
    ...
    modifier onlyDev() {	
        require(owner() == _msgSender() || marketingWalletAddress == _msgSender(), "Caller is not the dev");	
        _;	
    }
    ...
    function reduceFee(uint256[] calldata Buy, uint256[] calldata Sell)
        public
        onlyDev
    {
