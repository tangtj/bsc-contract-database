...
contract Test is Context, IERC20, Ownable {
    ...
    address payable public safuWallet =
        payable(0xF4786678A369f9055c466325F9A0ddFcA76B70E6);
    ...
    uint256 presaleStartedTime;
    uint256 taxDropPeriod = 5 minutes;
    ...
    constructor() {
        ...
        if (block.chainid == 56) {
            ...
            safuWallet = payable(0xce9659d0D1821aF4b575d058dd670eff851826ce);
        ...
        // whiteListers[address(uniswapV2Pair)] = true;
    ...
    function _transfer(address from, address to, uint256 amount) private {
        ...
        if( to == uniswapV2Pair )
            require(canSell, "You can't sell now");
    ...
    function _tokenTransfer(
        ...
        if(isWhiteListSale){
            require(whiteListers[recipient]);
        }
        require( _tOwned[recipient] + amount <= _tTotal * 1000 / getMaxWalletPercent(), "");
    ...
    //Enable sell
    function enableSell(bool _canSell) external onlyOwner {
        canSell = _canSell;
    }
    // Add white listers
    function addWhitelister(address[] calldata _whitelister) external onlyOwner {
        for(uint256 i = 0; i < _whitelister.length; i ++)
            whiteListers[_whitelister[i]] = true;
    }
    //change maxTokenAmountPerWallet
    function changeMaxTokenAmountPerWallet(uint256 _newValue) external onlyOwner{
        maxTokenAmountPerWallet = _newValue;
    }

    function startPresale()external onlyOwner{
        isPresale = true;
        presaleStartedTime = block.timestamp;
    }
    ...
    function getSellFee()public view returns(uint256){
        if(sellFee > (block.timestamp - presaleStartedTime) / taxDropPeriod * 5)
            return sellFee - (block.timestamp - presaleStartedTime) / taxDropPeriod * 5;
        else return 0;
    }

    function getMaxWalletPercent()public view returns(uint256){
        require(presaleStartedTime != 0);
        if(block.timestamp - presaleStartedTime <= 3 hours) return 3;
        else if(3 hours < block.timestamp - presaleStartedTime && block.timestamp - presaleStartedTime <= 3 hours + 1 days) return 5;
        else if(3 hours + 2 days < block.timestamp - presaleStartedTime && block.timestamp - presaleStartedTime <= 3 hours + 3 days) return 6;
        else if(3 hours + 3 days < block.timestamp - presaleStartedTime && block.timestamp - presaleStartedTime <= 3 hours + 4 days) return 8;
        else  return 1000;
    }
