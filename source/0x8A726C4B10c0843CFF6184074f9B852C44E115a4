...
contract XAI is Context, IERC20, Ownable {
    ...
    string private _name = 'X AI';
    string private _symbol = 'XAI';
    uint8 private _decimals = 9;

    uint8 public transferTimeout = 18;
    ...
    function _transfer(address sender, address recipient, uint256 amount) private {
        ...
        if (recipient == uniswapPair && !_timeoutWhitelist[sender]) {
            require(block.timestamp >= lastBuy[sender] + transferTimeout, "lock 18s for prevent front runner bot.");
        }
    ...
    function _getTValues(uint256 tAmount) private pure returns (uint256, uint256) {
        uint256 tFee = tAmount.div(10000).mul(8);
