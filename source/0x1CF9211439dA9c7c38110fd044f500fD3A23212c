...
contract EnrichPoolCollections is Initializable, ERC721Upgradeable, ERC721URIStorageUpgradeable, OwnableUpgradeable {

    // Operator
     address public Operator;
     mapping(uint256 =>uint256)private getPool;
    // totalSupply
    ...
    // Evens 
     ...
     event SetURI(string _uri, uint256 _poolId);


    function initialize(address _operator) initializer external {
        __ERC721_init("EnrichCollections", "EnrichPools");
        __ERC721URIStorage_init();
        Operator=_operator;
        __Ownable_init();
    }

    function mintPoolToken(address to,uint256 tokenID, string memory uri, uint256 _pool)
      external
    {
        require(msg.sender==Operator,"No Permission");
        _totalSupply++;
        getPool[tokenID]=_pool;
        _safeMint(to, tokenID);
        _setTokenURI(tokenID, uri);
    }

    function setURI(string memory _URI, uint256 poolId)external onlyOwner{
        URI[poolId]=_URI;
        emit SetURI(_URI,poolId);
    }
    ...
    {
        string memory _uri=URI[getPool[tokenId]];
        return _uri;
    }
