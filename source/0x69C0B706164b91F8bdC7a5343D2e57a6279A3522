...
contract PEEWEE is ERC20, Ownable {
    using SafeMath for uint256;
    ...
    PEEWEEDividendTracker public dividendTracker;
    ...
    constructor() ERC20("PeeWee", "PeeWee") {
        marketingWallet = payable(0x95E00B7279B4a5CA9CD409200D486AdbdFf9b7a3);
        ...
        dividendTracker = new PEEWEEDividendTracker(payable(this), router, address(this),
                                                   "PEEWEETRACKER", "PEEWEETRACKER");
...
contract PEEWEEDividendTracker is DividendPayingToken, Ownable {
    using SafeMath for uint256;
    using SafeMathInt for int256;
    ...
    constructor(address payable mainContract, address router, address token, string memory _name,
        ...
        defaultToken = token;
        PEEWEEContract = PEEWEE(mainContract);
    ...
    function withdrawDividend() public pure override {
        require(false, "withdrawDividend disabled. Use the 'claim' function on the main PEEWEE contract.");
    }
    ...
    function _reinvestDividendOfUser(address account) private returns (uint256) {
        ...
        if (_withdrawableDividend > 0) {
            ...
            path[1] = address(PEEWEEContract);

            uint256 prevBalance = PEEWEEContract.balanceOf(address(this));
            ...
            {
                uint256 received = PEEWEEContract.balanceOf(address(this)).sub(prevBalance);
                if (received > 0) {
                    success = true;
                    PEEWEEContract.transfer(account, received);
                } 
                else {
                    success = false;
                }
