...
contract MiningMasterChef is Ownable, ReentrancyGuard {
    using SafeMath for uint256;
  ...
  mapping(IERC20 => uint256) public pricingTable;
  mapping(uint256 => uint256) public usedQuotaForNfa;
  mapping(uint256 => uint256) public availableQuotaForNfa;
 

    mapping(uint256 => bool) public stakeExistence;
    ...
    mapping(uint256 => mapping(uint256 => NfaStakeInfo)) public nftStakeInfo;
    // Info of each user that stakes LP tokens.
   
    address public nfaAddress;
   ...
   function setQuotaForNfa(uint256[] calldata tokenIds, uint256[] calldata quotas) external onlyOwner {
        for (uint256 i=0;i<tokenIds.length;i++) {
            uint256 quota = quotas[i];
            availableQuotaForNfa[tokenIds[i]] = quota;
        }
   }

 function setStakedToken(address _tokenAddr) external onlyOwner {
        stakedToken = IERC20(_tokenAddr);
    }
    ...
    function pendingStakedToken(uint256 _tokenId) external view returns (uint256) {
        if (!stakeExistence[_tokenId]) {
            return 0;
        }

         uint256 used = usedQuotaForNfa[_tokenId]; 
         uint256 totalPending = 0;
        for (uint256 i=0;i<used;i++) {
            uint256 lastClaimed = nftStakeInfo[_tokenId][i].lastClaimed;
            uint256 currentBlock = block.number;

            if (currentBlock > nftStakeInfo[_tokenId][i].endBlock) {
                currentBlock = nftStakeInfo[_tokenId][i].endBlock;
            }

            if (lastClaimed > nftStakeInfo[_tokenId][i].endBlock) {
                lastClaimed = nftStakeInfo[_tokenId][i].endBlock;
            }

            uint256 pending = (currentBlock.sub(lastClaimed)).mul(rewardsPerBlock);
    ...
    function setPricePerUnit(IERC20 _currency, uint256 _price) external onlyOwner {
        pricingTable[_currency] = _price;
    }

    function activateNfa(uint256 _tokenId, uint256 _quantity, IERC20 _currency) external 
    {

     //   require(!stakeExistence[_tokenId], "Nfa already activated");
        require(_quantity > 0, "Must not be negative");

        require(Membership(nfaAddress).ownerOf(_tokenId) == msg.sender, "Has to be owner");
    // set mking reward
    // update pool variables
        ...
        // check rewards end
        uint256 used = usedQuotaForNfa[_tokenId]; 
        uint256 available = availableQuotaForNfa[_tokenId];
        uint256 until = used+_quantity;

        require(until <= available, "Exceeded quota");

        uint256 totalPayable = pricingTable[_currency].mul(_quantity);

        require(totalPayable > 0, "No free meal");

        _currency.transferFrom(msg.sender,address(this), totalPayable);
       
        for (uint256 i=used;i<until;i++) 
        {
            nftStakeInfo[_tokenId][i] = NfaStakeInfo({
                tokenId: _tokenId,
                startBlock: block.number,
                endBlock: block.number + rewardPeriod,
                lastClaimed: block.number,
                slot: i
            });
        }

        usedQuotaForNfa[_tokenId] = usedQuotaForNfa[_tokenId] + _quantity;

        stakeExistence[_tokenId] = true;
    ...
    function claim(uint256 _tokenId) external {
        require(stakeExistence[_tokenId], "Invalid nfa");

        require(Membership(nfaAddress).ownerOf(_tokenId) ==  msg.sender, "Has to be owner");


        uint256 amount = this.pendingStakedToken(_tokenId);

        require(amount > 0, "Nothing to claim");

         uint256 used = usedQuotaForNfa[_tokenId]; 
         for (uint256 i=0;i<used;i++) {
            nftStakeInfo[_tokenId][i].lastClaimed = block.number;
         }
