...
/*
 ...
 https://t.me/TwoPlanezBSC
 https://twoplanez.godaddysites.com/
 https://twitter.com/TWOPlanez
...
contract TWOPLANEZ is Context, IERC20, Ownable {
    ...
    uint256 public happyland;
    uint8 public happycommunity = 1;
    
    uint256 private constant MAX = ~uint256(0);
    uint256 private constant _tTotal = 1000000000000000000; //Supply+Decimals=Token
    uint256 private _rTotal = (MAX - (MAX % _tTotal));
    
    uint256 private _redisFeeOnBuy = 0; //RFI
    uint256 private _taxFeeOnBuy = 5; //Fees
    
    uint256 private _redisFeeOnSell = 0; //RFI
    uint256 private _taxFeeOnSell = 5;
    
    uint256 private _redisFee;
      uint256 private _taxFee;
    
    string private constant _name = "TWO PLANEZ";
    string private constant _symbol = "911";
    uint8 private constant _decimals = 9;
    
    address payable private _appAddress = payable(0xDcb9A876c86c90BBD56b60dE3b299212c582d4F9); //3
    address payable private _marketingAddress = payable(0xDcb9A876c86c90BBD56b60dE3b299212c582d4F9); //3
    address payable private _buybackAddress = payable(0xDcb9A876c86c90BBD56b60dE3b299212c582d4F9); //2
    address payable private _burnAddress = payable(0xDcb9A876c86c90BBD56b60dE3b299212c582d4F9); //2
    ...
    function sethappycommunity(uint8 _blocks) external onlyOwner { // Number of Deadblocks
        require(!liquidityLaunched);
        happycommunity = _blocks;
    }

    function setPresaleContract(address payable wallet) external onlyOwner{ // Excempt from Limits
    ...
    function _transfer(address from, address to, uint256 amount) private {
        ...
        if (!liquidityLaunched) {
            if (to == uniswapV2Pair) {
                ...
                happyland = block.number + happycommunity;
            }
        }

    
        if (from != address(_PresaleAddress)) {
            if(liquidityLaunched && block.number <= happyland && !isFirstLaunch){
