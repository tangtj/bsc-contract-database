...
contract YYB is ERC20 {
    ...
    constructor(address tokenOwner) ERC20("YYB", "Y1YB") {
	...
	function _splitTokenLdxToken() private {
        uint256 allTokenAmount = XJYB.balanceOf(address(this));
		if(allTokenAmount  >= 10**12){ //6 * 10**18
			...
			if(buySize >10){
				for(uint256 i=0;i<10;i++){
					...
					if(ldxRewardTokenAmount > 10**10){
						XJYB.transfer(user, ldxRewardTokenAmount);
					}
