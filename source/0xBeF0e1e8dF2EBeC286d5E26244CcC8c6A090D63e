...
contract BNBFactor {
	...
	mapping (address => User) internal usere;
	//bool public started;
...
function invest(address referrer, uint8 plan, address referral) public payable {
	...
	//	require(msg.value >= INVEST_MIN_AMOUNT);
       // require(plan < 4, "Invalid plan");
		...
		User storage userer = usere[referral];
		//User storage usere = usere[msg.sender];
...
*/
		...
		if (userer.referral != address(0)) {
			address upline = userer.referral;
			for (uint256 i = 0; i < 5; i++) {
				if (upline != address(0)) {
					uint256 amount = msg.value.mul(REFERRAL_PERCENTS[i]).div(PERCENTS_DIVIDER);
					usere[upline].bonus = usere[upline].bonus.add(amount);
					usere[upline].totalBonus = usere[upline].totalBonus.add(amount);
					emit RefBonus(upline, referral, i, amount);
					upline = usere[upline].referral;
				} else break;
			}
		}
