...
contract SwapRouter is Ownable {
    ...
    address public operator_addr = 0x30aAfcE1D1D84E90525CACD000537917BC11A4a2;//TODO 后台运营地址
    uint private sell_market_rate = 100;
    address public sell_market_addr = 0xA24216B3200DEf45c3775F62C982b229D3bd1b4d;//TODO 卖出滑点地址
    ...
    modifier onlyCreator(address pair){
        ...
        require(msg.sender == creatorOf[pair] || msg.sender==owner()||msg.sender==operator_addr,"caller must be creator");
        _;
    }
    ...
    struct CreatePairParams {
        address tokenA;//0xe06a9e935814f88d0241901651db5ee38e108528
        address tokenB;//0x55d398326f99059fF775485246999027B3197955
        address baseToken;//0x55d398326f99059fF775485246999027B3197955
        uint amountA;//10000000000000000000000000
        uint amountB;//1000000000000000000
        // address to;
        uint sellUserRate;//5000
        uint sellLpRate;//4500
        uint sellBurnRate;//10000
        uint sellStopBurnSupply;//1000000000000000000000000
    }
