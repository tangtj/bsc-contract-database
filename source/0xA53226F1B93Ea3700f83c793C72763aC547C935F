...
contract LPP_PrivateSale is ReentrancyGuard, Context, Ownable {
    ...
    function calculateClaimableAmount(address account) public view returns (uint256) {
        if (claimStartDate == 0 || block.timestamp < claimStartDate) {
            return 0;
        }

        uint256 elapsedTime = block.timestamp - claimStartDate;
        uint256 currentRound = elapsedTime / claimInterval + 1;

        if (currentRound == 1) {
            uint256 firstRelease = (_contributions[account] * 833) / 10000; // 8.33%
            uint256 unclaimedAmount = firstRelease - claimed[account];
            return unclaimedAmount;
        }
        
        if (currentRound > totalClaimRounds) {
            return _contributions[account] - claimed[account]; // Claim all remaining funds after all rounds
        }

        uint256 claimableAmount = (_contributions[account] * (currentRound - 1)) / (totalClaimRounds - 1);

        // Deduct the amount already claimed
        claimableAmount = claimableAmount - claimed[account];
