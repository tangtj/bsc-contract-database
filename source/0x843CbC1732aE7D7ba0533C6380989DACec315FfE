...
contract TransactionThrottler is Ownable {
    ...
    function setTradingStart(uint256 _time) external onlyOwner() {
        require(_tradingStart > block.timestamp, "Protection: Too late");
        _tradingStart = _time;
...
contract Yarloo is Context, IBEP20, Ownable, TransactionThrottler {
    ...
    function _transfer(
        ...
        address to,
        uint256 amount
    ) private transactionThrottler(from, to, amount) {
