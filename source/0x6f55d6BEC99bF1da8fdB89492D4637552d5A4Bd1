...
contract Sale_Contract {
    ...
    uint256 currentBonusTier = (currentReferralCount / 8) + 1; // Calculate the current bonus tier

    if (currentBonusTier > 10) {
        currentBonusTier = 10; // Cap the bonus tier at 10 (80%)
    }

    uint256 baseReferralBonus = _weiAmount.mul(5).mul(currentBonusTier).div(100);
    uint256 totalBonus = baseReferralBonus;
