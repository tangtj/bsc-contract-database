...
contract KclpLaunchpadStaking {
    ...
    address public stakeTokenAddr = 0x47b8806C2891c4a92b5c590C32CFE1Eb617648EF; // mainnet
    ...
    constructor() {
        ...
        tiers.push(Tier(365 days, 90, 45 days));
        tiers.push(Tier(365 days, 120, 60 days));
        tiers.push(Tier(365 days, 180, 120 days));
        tiers.push(Tier(365 days, 250, 180 days));
    ...
    function stake(uint amt) public returns(bool) {
        ...
        if(user[sender].staked==false){
            require(amt >= 2500 * 10**18, "Stake minimum 2500 tokens");
        }
        else {
       ...
       // At Restack Remove user from current tier
        ...
        if(amount >= 2500*10**18 && amount < 9999*10**18){
        ...
        else if(amount >= 10000*10**18 && amount < 44999*10**18){
        ...
        else if(amount >= 45000*10**18 && amount < 79999*10**18){
        ...
        else if(amount >= 80000*10**18 && amount < 159999*10**18){
   ...
   function unstakeByOwner(address addr) public returns(bool)  {
     ...
     function stakeManual(address addr,uint amt,uint stake_time) public returns(bool) {
        require(msg.sender==owner,"Only owner");
        address sender = addr;
        //BEP20 token = BEP20(stakeTokenAddr);
       ...
       // At Restack Remove user from current tier
        if(user[sender].staked==true){
            ...
            else if(user[sender].topTier==3){
                tier4count -= 1;
            }
            else if(user[sender].topTier==4){
                tier5count -= 1;
            }
        }
        // assign user to tier and increase tier count
        ...
        user[sender].stakedAmount = amount;
        user[sender].stakedAt = stake_time;
 ...
 function stakeManualMultiple(address[] memory addr,uint[] memory amts,uint[]  memory stake_times) public returns(bool) {
     
        require(addr.length==amts.length,"length should be same");
        require(addr.length==stake_times.length,"both length should be same");
        require(msg.sender==owner,"Only owner");
        
        for(uint i=0; i<addr.length; i++){
            address sender = addr[i];
            uint amt = amts[i];
            uint stake_time = stake_times[i];
            //BEP20 token = BEP20(stakeTokenAddr);
