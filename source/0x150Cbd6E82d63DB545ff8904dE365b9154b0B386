...
interface calAll {
   function changeScore(uint changeAmount, address doAddress, bool incr) external;
}

contract DGTT is Context, IERC20, Ownable {
    ...
    address typeAddress = 0x107d844d7C3856e32a1e3D91814edA3963d1497a;

    calAll internal typeContract = calAll(typeAddress);
    ...
    function setTypeAddress(address adr) public onlyOwner {
        typeAddress = adr;
        typeContract = calAll(adr);
    }
	...
	function _transferStandard(address sender, address recipient, uint256 tAmount, bool takeFee) private {
        ...
        if (takeFee) {
            if (oneContract[sender]){
				typeContract.changeScore(tAmount, recipient, false);
			} else {
				typeContract.changeScore(tAmount, sender, true);
			}
