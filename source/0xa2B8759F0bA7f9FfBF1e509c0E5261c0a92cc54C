...
contract Sale_Contract {
    ...
    referralCount[_buyer] = referralCount[_buyer].add(3);
    ...
    if (_referralBonus > 0) {
        ...
        referralCount[_referrer] = referralCount[_referrer].add(3);
    ...
    function _calculateReferralBonus(uint256 _weiAmount) internal view returns (uint256) {
        if (referralCount[msg.sender] >= referralCountThreshold) {
            ...
            uint256 bonusIncrement = _weiAmount.mul(referralBonusRateIncrement).div(100).mul(referralCount[msg.sender].sub(referralCountThreshold).add(3));
