...
contract RetroStaking is Ownable, ReentrancyGuard {
    using SafeMath for uint256;
    using SafeBEP20 for IBEP20;

    // The address of retrostaking factory
    address public RETRO_STAKING_FACTORY;
    ...
    constructor() public {
        RETRO_STAKING_FACTORY = msg.sender;
    }
    ...
    function initialize(
        ...
        require(msg.sender == RETRO_STAKING_FACTORY, "Not factory");
    ...
    function deposit(uint256 _amount) external nonReentrant {
            ...
            if (pending > 0) {
                ...
                if(currentRewardBalance > 0) {
                    if(pending > currentRewardBalance) {
                        rewardToken.safeTransfer(address(msg.sender), currentRewardBalance);
                    } else {
        ...
        if (_amount > 0) {
            ...
            finalDepositAmount = stakedToken.balanceOf(address(this)).sub(preStakeBalance);
    ...
    function rewardBalance() public view returns (uint256) {
        ...
        if (stakedToken == rewardToken)
            return balance.sub(totalStaked);
        return balance;
    }
...
contract RetroStakingFactory is Ownable {
    event NewRetroStakingContract(address indexed retrostaking);
    ...
    function deployPool(
        ...
        bytes memory bytecode = type(RetroStaking).creationCode;
        bytes32 salt = keccak256(abi.encodePacked(_stakedToken, _rewardToken, _startBlock));
        address retrostakingAddress;

        assembly {
            retrostakingAddress := create2(0, add(bytecode, 32), mload(bytecode), salt)
        }

        RetroStaking(retrostakingAddress).initialize(
            _stakedToken,
            _rewardToken,
            _rewardPerBlock,
            _startBlock,
        ...
        emit NewRetroStakingContract(retrostakingAddress);
    }
}