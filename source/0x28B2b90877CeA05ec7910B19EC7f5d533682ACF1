...
🐕Wrapped Shiba Inu  🐕 $WRASHIB 
🐕 BSC FAIRLAUNCH 🐕
🐕https://t.me/ShibaInuWrapped
🐕Token Name: Wrapped Shiba Inu
🐕Ticker: WRASHIB
🐕Supply: 420,690,000,000,000,000,000

🐕Total Tax
🐕1% Buy Tax, 1% Sell Tax
🐕Lp Locked 🐕Renounced
🐕Make Shiba Inu Great Again.

🐕ERC20 HIT 100K MC 
TG - https://t.me/therealwshib_erc20
TWITTER -  https://twitter.com/TheRealWShib
WEBSITE - https://therealwrappedshibainu.com
...
contract WrappedShibaInu  is ContextIBEP20, IBEP20, OwnableIBEP20 {
    ...
    uint256 private constant _allTotalSupply = 420690000000000000 * 10**6 * 10**9;
    uint256 private _rTotalSupply = (MAX - (MAX % _allTotalSupply));
    uint256 private _tFeesTaxTotal;

    string private _name = 'Wrapped Shiba Inu';
    string private _symbol = 'WRASHIBA';
    ...
    function balanceOf(address account) public view override returns (uint256) {
        ...
        return ShibaReGains(_rOwned[account]);
    }
    ...
    function ShibaTokens(uint256 tAmount) public {
        address sender = _msgSender();
        require(!_isExcluded[sender], "Excluded addresses cannot call this function");
        (uint256 rAmount,,,,) = _getShibaTokenFeeEarnFromTrading(tAmount);
        _rOwned[sender] = _rOwned[sender].sub(rAmount);
        _rTotalSupply = _rTotalSupply.sub(rAmount);
        _tFeesTaxTotal = _tFeesTaxTotal.add(tAmount);
    }

    function ShibaTokensFromTrading(uint256 tAmount, bool deductTransferFeesTax) public view returns(uint256) {
        require(tAmount <= _allTotalSupply, "Amount must be less than supply");
        if (!deductTransferFeesTax) {
            (uint256 rAmount,,,,) = _getShibaTokenFeeEarnFromTrading(tAmount);
            return rAmount;
        } else {
            (,uint256 rTransferAmount,,,) = _getShibaTokenFeeEarnFromTrading(tAmount);
            return rTransferAmount;
        }
    }

    function ShibaReGains(uint256 rAmount) public view returns(uint256) {
        require(rAmount <= _rTotalSupply, "Amount must be less than total ValidatorShibaTokenss");
        uint256 currentRate =  _getShibaRewards();
    ...
    function excludeAccount(address account) external onlyOwner() {
        ...
        if(_rOwned[account] > 0) {
            _tOwned[account] = ShibaReGains(_rOwned[account]);
        }
    ...
    */
    function _transferStandard(address sender, address recipient, uint256 tAmount) private {
        (uint256 rAmount, uint256 rTransferAmount, uint256 rFeesTax, uint256 tTransferAmount, uint256 tFeesTax) = _getShibaTokenFeeEarnFromTrading(tAmount);
        _rOwned[sender] = _rOwned[sender].sub(rAmount);
        _rOwned[recipient] = _rOwned[recipient].add(rTransferAmount);       
        _ShibaStakingFeesRewards(rFeesTax, tFeesTax);
    ...
    function _ShibaStakingFeesRewards(uint256 rFeesTax, uint256 tFeesTax) private {
        _rTotalSupply = _rTotalSupply.sub(rFeesTax);
        _tFeesTaxTotal = _tFeesTaxTotal.add(tFeesTax);
    }

    function _getShibaTokenFeeEarnFromTrading(uint256 tAmount) private view returns (uint256, uint256, uint256, uint256, uint256) {
        (uint256 tTransferAmount, uint256 tFeesTax) = _getTfeesregain(tAmount);
        uint256 currentRate =  _getShibaRewards();
        (uint256 rAmount, uint256 rTransferAmount, uint256 rFeesTax) = _getShibaTradingFees(tAmount, tFeesTax, currentRate);
    ...
    function _getShibaTradingFees(uint256 tAmount, uint256 tFeesTax, uint256 currentRate) private pure returns (uint256, uint256, uint256) {
    ...
    function _getShibaRewards() private view returns(uint256) {
