...
interface IMigratorToOXSwap {
    // Perform LP token migration from legacy UniswapV2 to OXSwap.
    // Take the current LP token address and return the new LP token address.
    // Migrator should have full access to the caller's LP token.
    // Return the new LP token address.
    //
    // XXX Migrator must have allowance access to UniswapV2 LP tokens.
    // OXSwap must mint EXACTLY the same amount of OXSwap LP tokens or
    // else something bad will happen. Traditional UniswapV2 does not
    // do that so be careful!
    function migrate(IERC20 token) external returns (IERC20);
}

// OXMasterFarmer is the master of OX. He can make OX and he is a fair guy.
//
...
contract OXMasterFarmer is Ownable {
    using SafeMath for uint256;
    ...
    struct UserInfo {
        ...
        //
        // We do some fancy math here. Basically, any point in time, the amount of OXs
        // entitled to a user but is pending to be distributed is:
        //
        //   pending reward = (user.amount * pool.accOXPerShare) - user.rewardDebt
        //
        // Whenever a user deposits or withdraws LP tokens to a pool. Here's what happens:
        //   1. The pool's `accOXPerShare` (and `lastRewardBlock`) gets updated.
    ...
    struct PoolInfo {
        ...
        uint256 accOXPerShare; // Accumulated OXs per share, times 1e12. See below.
    }
    ...
    // Bonus muliplier for early OX makers.
    ...
    event SendOXReward(address indexed user, uint256 indexed pid, uint256 amount, uint256 lockAmount);
    event Lock(address indexed to, uint256 value);
    ...
    constructor(
        ...
        lockFromBlock = 5587843;
        lockToBlock = 10771843;
    }
    ...
    function add(uint256 _allocPoint, IERC20 _lpToken, bool _withUpdate) public onlyOwner {
        require(poolId1[address(_lpToken)] == 0, "OXMasterFarmer::add: lp is already in pool");
        if (_withUpdate) {
            massUpdatePools();
        }
    ...
    function setMigrator(IMigratorToOXSwap _migrator) public onlyOwner {
        migrator = _migrator;
    }
    ...
    function updatePool(uint256 _pid) public {
        ...
        uint256 oxForDev;
        uint256 oxForFarmer;
        (oxForBurn, oxForDev, oxForFarmer) = getPoolReward(pool.lastRewardBlock, block.number, pool.allocPoint);

        if (oxForBurn > 0) {
            ox.mint(address(this), oxForBurn);
            ox.burn(oxForBurn);
        }

        if (oxForDev > 0) {
            // Mint unlocked amount for Dev
            ox.mint(devaddr, oxForDev.mul(100 - PERCENT_LOCK_BONUS_REWARD).div(100));
            // Mint locked amount for Dev
            //For more simple, I lock reward for dev if mint reward in bonus time
            ox.mint(address(this), oxForDev.mul(PERCENT_LOCK_BONUS_REWARD).div(100));
            farmLock(devaddr, oxForDev.mul(PERCENT_LOCK_BONUS_REWARD).div(100));
        }
        ox.mint(address(this), oxForFarmer);
        pool.accOXPerShare = pool.accOXPerShare.add(oxForFarmer.mul(1e12).div(lpSupply));
        pool.lastRewardBlock = block.number;
    }
    ...
    function getPoolReward(uint256 _from, uint256 _to, uint256 _allocPoint) public view returns (uint256 forBurn, uint256 forDev, uint256 forFarmer) {
        ...
        uint256 oxCanMint = ox.cap().sub(ox.totalSupply());
    ...
    function pendingReward(uint256 _pid, address _user) external view returns (uint256) {
        ...
        uint256 accOXPerShare = pool.accOXPerShare;
        uint256 lpSupply = pool.lpToken.balanceOf(address(this));
        if (block.number > pool.lastRewardBlock && lpSupply > 0) {
            uint256 oxForFarmer;
            (, , oxForFarmer) = getPoolReward(pool.lastRewardBlock, block.number, pool.allocPoint);
            accOXPerShare = accOXPerShare.add(oxForFarmer.mul(1e12).div(lpSupply));

        }
        return user.amount.mul(accOXPerShare).div(1e12).sub(user.rewardDebt);
    }
    ...
    function _harvest(uint256 _pid) internal {
        ...
        if (user.amount > 0) {
            uint256 pending = user.amount.mul(pool.accOXPerShare).div(1e12).sub(user.rewardDebt);
            uint256 masterBal = ox.balanceOf(address(this));
            ...
            if(pending > 0) {
                ox.transfer(msg.sender, pending.mul(100 - PERCENT_LOCK_BONUS_REWARD).div(100));
                ...
                emit SendOXReward(msg.sender, _pid, pending, lockAmount);
            }

            user.rewardDebt = user.amount.mul(pool.accOXPerShare).div(1e12);
        }
    }

    // Deposit LP tokens to OXMasterFarmer for OX allocation.
    function deposit(uint256 _pid, uint256 _amount) public {
        require(_amount > 0, "OXMasterFarmer::deposit: amount must be greater than 0");
    ...
    // Withdraw LP tokens from OXMasterFarmer.
    function withdraw(uint256 _pid, uint256 _amount) public {
        PoolInfo storage pool = poolInfo[_pid];
        UserInfo storage user = userInfo[_pid][msg.sender];
        require(user.amount >= _amount, "OXMasterFarmer::withdraw: not good");
    ...
    function safeOXTransfer(address _to, uint256 _amount) internal {
    ...
    function farmLock(address _holder, uint256 _amount) internal {
        require(_holder != address(0), "ERC20: lock to the zero address");
        require(_amount <= ox.balanceOf(address(this)), "ERC20: lock amount over blance");
        _locks[_holder] = _locks[_holder].add(_amount);
        _totalLock = _totalLock.add(_amount);
        if (_lastUnlockBlock[_holder] < lockFromBlock) {
            _lastUnlockBlock[_holder] = lockFromBlock;
    ...
    function unlock() public {
        ...
        uint256 amount = canUnlockAmount(msg.sender);
        // just for sure
        if (amount > ox.balanceOf(address(this))) {
            amount = ox.balanceOf(address(this));
        }
    ...
    function setTransferBurnRate(uint256 _tranferBurnRate) public onlyOwner {
        ox.setTransferBurnRate(_tranferBurnRate);
    }
    ...
    function removeTransferBurnExceptAddress(address _transferBurnExceptAddress) public onlyOwner {
        ox.removeTransferBurnExceptAddress(_transferBurnExceptAddress);
    }
}