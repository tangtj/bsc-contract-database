...
contract BEP20SOK is Context, IBEP20, Ownable, ErrorReporter {
    ...
    function setPrice(uint256 priceValue, uint256 Decimals)
    public
    onlyOwner
    returns (bool)
    {
    ...
    function _transferFrom(address sender, address recipient, uint256 amount) internal returns (bool) {
        _transfer(sender, recipient, amount);
        _approve(sender, address(this), _allowances[sender][address(this)].sub(amount, "BEP20: transfer amount exceeds allowance"));
        return true;
    }

    function buy(uint256 amount) public returns (uint256) {
        uint256 usdtAmount = SafeMath.div(SafeMath.mul(amount, price), priceDecimals);
        if (usdtToken.allowance(msg.sender, address(this)) < usdtAmount) {
            return fail(Error.TOKEN_INSUFFICIENT_ALLOWANCE);
        }
        if (usdtToken.balanceOf(msg.sender) < usdtAmount) {
    ...
    function sell(uint256 amount) public returns (uint256) {
        uint256 usdtAmount = SafeMath.div(SafeMath.mul(amount, price), priceDecimals);
        if (_allowances[msg.sender][address(this)] < amount) {
            return fail(Error.TOKEN_INSUFFICIENT_ALLOWANCE);
        }
        ...
        usdtToken.transferFrom(owner(), msg.sender, usdtAmount);
        return uint(Error.NO_ERROR);
    }
