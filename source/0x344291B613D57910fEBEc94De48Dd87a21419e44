...
contract MultiSender {
    ...
    // Approve the contract to use its own USDC balance
    require(USDC.approve(address(this), totalUSDC), "Approval failed");
    ...
    for (uint256 i = 0; i < recipients.length; i++) {
        ...
        // Transfer the proportional amount of USDC to the recipient
        require(USDC.transferFrom(address(this), recipients[i], proportionalUSDC), "Transfer failed");
    }
}
