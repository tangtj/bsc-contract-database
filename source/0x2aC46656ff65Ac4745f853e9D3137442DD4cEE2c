...
contract PopChefV2 is Ownable, ReentrancyGuard {
    ...
    // POP LP address
    IUniswapPair public popLp;
    // Uniswap V2 Router
    IUniswapV2Router public router;
    // POP tokens reward per block.
    uint256 public rewardPerBlock;
    // Compound ratio which is 0.002% (will be used to decrease supply)
    uint256 public compoundRatio = 2 * 1e13;
    ...
    // View function to see pending Pop on frontend.
        ...
        returns (uint256)
    {
        ...
        if (address(pool.lpToken) == address(pop)) {
            lpSupply = pop.balanceOfUnderlying(address(this));
        }
        if (block.number > pool.lastRewardBlock && lpSupply != 0) {
            uint256 popReward = ((block.number - pool.lastRewardBlock) *
                rewardPerBlock *
                pool.allocPoint) / totalAllocPoint;
            accRewardPerShare += (popReward * 1e12) / lpSupply;
        }
        return
    ...
    function updatePool(uint256 _pid) public {
        ...
        uint256 popReward = ((block.number - pool.lastRewardBlock) *
            rewardPerBlock *
            pool.allocPoint) / totalAllocPoint;
        pool.accRewardPerShare += (popReward * 1e12) / lpSupply;
        pool.lastRewardBlock = block.number;
    ...
    function deposit(
        ...
        if (address(pool.lpToken) == address(pop)) {
            _amount = pop.fragmentToPop(_amount);
        }
    ...
    function withdraw(uint256 _pid, uint256 _amount) external {
        ...
        user.rewardDebt = (user.amount * pool.accRewardPerShare) / 1e12;
        if (address(pool.lpToken) == address(pop)) {
            _amount = pop.popToFragment(_amount);
        }
    ...
    function compoundBig(
        ...
        IERC20(address(pop)).safeTransferFrom(
            msg.sender,
            address(this),
            _amountTokenDesired
        );
        IERC20(address(pop)).approve(address(router), _amountTokenDesired);
        ...
        IERC20(address(pop)).safeTransfer(
            msg.sender,
