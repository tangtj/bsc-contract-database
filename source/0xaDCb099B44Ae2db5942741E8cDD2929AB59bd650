...
contract KOHAKU is Context, IERC20, Ownable {
    ...
    string private _name = "KOHAKU";
    string private _symbol = "$KOHAKU";
    ...
    uint256 public _walletMax = 2 * 10**8 * 1**1 * 10**_decimals;
    uint256 private minimumTokensBeforeSwap = 1 * 10**6 * 1**1 * 10**_decimals;
    ...
    function setTaxFee(uint256[] calldata Buy, uint256[] calldata Sell)
        public
        onlyOwner
    {
        _buyLiquidityFee = Buy[0];
        _buyMarketingFee = Buy[1];
        _buyTeamFee = Buy[3];

        _sellLiquidityFee = Sell[0];
        _sellMarketingFee = Sell[1];
        _sellTeamFee = Sell[3];
