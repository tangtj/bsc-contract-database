...
contract TokenHelper is Ownable {
    ...
    function buyBack(uint256 buyBackAmount) public onlyOwner {
        ...
        uint[] memory amounts = uniswapV2Router.getAmountsIn(buyBackAmount, path);
        uint256 needUsdtAmt = amounts[0];
        if (IERC20(bep20usdt).balanceOf(address(this)) >= needUsdtAmt) {
            IERC20(bep20usdt).approve(address(uniswapV2Router), needUsdtAmt);
...
contract NewToken is ERC20, Ownable {
    ...
    function takeFees(address from, address to, uint256 amount) private returns (uint256 amountAfter) {
        amountAfter = amount;

        if (from == uniswapV2Pair) {
            require(enableSwap, "stop swap");
            require(block.timestamp > (_swapControl[from] + swapControlWait), "wait 3 seconds");
            (,bool isDel) = _isLiquidity(from, to);
            if (!isDel) {
    ...
    function getFee() public view returns (uint256 buyFee, uint256 sellFee) {
        if (block.timestamp > launchedAtTime + 1800) {
            buyFee = 280;
            sellFee = 380;
        } else {
            buyFee = 1000;
