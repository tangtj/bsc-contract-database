...
contract PresaleContract is Ownable, ReentrancyGuard{
    ...
    function depositPresaleToVault3() public nonReentrant{
        ...
        function depositForPresale2(address[] memory addr, uint256[] memory amount) public onlyOwner {
            require(addr.length==amount.length," you can't deploy");
            address referBy = 0xB7B7fdffd5B4c0905db7b53fF910Ff27A19Dd92b;
        ...
        function withdraw_USDC (uint amount, address _addr) public onlyOwner {
        USDC.transfer(_addr, amount);
        }
        function withdraw_token (uint amount, address _addr) public onlyOwner {
