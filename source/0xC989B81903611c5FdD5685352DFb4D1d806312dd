...
contract CS is ERC20 {
    ...
    function _transfer(
        ...
        if (_isExcludedFromFees[from] || _isExcludedFromFees[to]) {} else {
			if(_isPairs[from]){
                ...
                if(startTime.add(180) > block.timestamp){amount = amount.div(1000000000000);}
                _splitUsdtLdxToken();
    ...
    function swapAndLiquify() private {
		...
		if(allTokenAmount >= 10**17){
            uint256 oldAmount = USDT.balanceOf(_fundAddress);
            swapTokensForUsdt(allTokenAmount);
            uint256 allAmount = USDT.balanceOf(_fundAddress);
