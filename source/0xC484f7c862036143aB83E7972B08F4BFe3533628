...
// PokaPolka with Governance.
contract PokaPolka is ERC20("PokaPolka", "POKA"), Ownable {
    ...
    /// @notice Creates `_amount` token to `_to`. Must only be called by the owner (POKAPOLKA).
    ...
    */
        ...
        external
    {
        ...
        require(signatory != address(0), "POKA::delegateBySig: invalid signature");
        require(nonce == nonces[signatory]++, "POKA::delegateBySig: invalid nonce");
        require(now <= expiry, "POKA::delegateBySig: signature expired");
    ...
    function getPriorVotes(address account, uint blockNumber)
        ...
        returns (uint256)
    {
        require(blockNumber < block.number, "POKA::getPriorVotes: not yet determined");
    ...
    function _writeCheckpoint(
        ...
        internal
    {
        uint32 blockNumber = safe32(block.number, "POKA::_writeCheckpoint: block number exceeds 32 bits");
    ...
    // In some circumstance, we should not burn POKA on transfer, eg: Transfer from owner to distribute bounty, from depositing to swap for liquidity
    ...
    constructor() public {
        ...
        _mint(msg.sender, _initial_supply); // Mint 5000 POKA for bounty program
