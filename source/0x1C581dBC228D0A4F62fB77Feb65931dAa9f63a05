...
contract ZSToken is IERC20, Ownable {
    //用于存储每个地址的余额数量
    ...
    // 基金地址
    address private fundAddress = 0x0Ed0fae695448f191296915C466F89dba36Eb683;
    // 营销地址
    address private dividendAddress = 0xb54767B4C79501Efe7AB48e15854b16c2dBbf6aC;

    // 买入底池比例
    uint256 public _ruRate = 100;

    // 卖出底池比例
    uint256 public _chuRate = 100;
   ...
   */
    ...
    // 设置买入比例
    function setRuRate(uint256 fee) public onlyOwner{
        _ruRate = fee;
    }
    // 设置卖出比例
    function setChuRate(uint256 fee) public onlyOwner{
        _chuRate = fee;
    }
    // 设置基金地址
    ...
    function _tokenTransfer(
        ...
        if (takeFee) {
            if(sender == mainPair){
                feeAmount = tAmount * _ruRate / 10000;
                //营销钱包
                _takeTransfer(sender, address(this), feeAmount);
            }else if(recipient == mainPair){
                feeAmount = tAmount * _chuRate / 10000;
                //营销钱包
                _takeTransfer(sender, address(this), feeAmount);
            }
    ...
    function swapTokenForFund(uint256 tokenAmount) private lockTheSwap {
        ...
        //营销钱包
        USDT.transferFrom(address(_tokenDistributor), fundAddress, newBalance * 5000 / 10000);
        USDT.transferFrom(address(_tokenDistributor), dividendAddress, newBalance * 5000 / 10000);
