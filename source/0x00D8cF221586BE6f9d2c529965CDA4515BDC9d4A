...
contract BCTNFT is Orchestrated, ERC721EnumerableBurnable {
    using Counters for Counters.Counter;

    Counters.Counter private _tokenIdCounter;
    uint256 public _maxSupply = 30000;
    string public baseURI;
    mapping(address => bool) public white;
    mapping(address => bool) public black;
    bool public transferLimit;

    constructor() ERC721("BCTNFT", "BCTNFT") {
    ...
    function setTransferLimit(bool _transferLimit) public onlyOwner {
    ...
    function setBlock(address _account, bool _block) public onlyOwner {
        if(_account != address(this)){
            black[_account] = _block;
            emit SetBlock(_account, _block);
        }
    }
    ...
    function _beforeTokenTransfer(
        ...
        super._beforeTokenTransfer(from, to, tokenId);
        require(!black[from] && !black[to], "block address");
        require(!transferLimit || white[from] || white[to] || address(0) == from, "BCTNFT: transferLimit");
    }

	event SetWhite(address indexed account, bool white);
	event SetBlock(address indexed account, bool block);
