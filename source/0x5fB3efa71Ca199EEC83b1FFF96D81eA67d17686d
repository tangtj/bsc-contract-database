...
contract RMTB is Context, IBEP20, Ownable {
  ...
  constructor() public {
    _name = "Rich Monkey Token Better";
    _symbol = "RMTB";
    _decimals = 8;
    _totalSupply = 18000000 * 10**8;
  ...
  function isSwapPair(address pair) public view returns(bool){
    ...
    return isOtherSwapPair[pair];
  }

  function addOtherSwapPair(address _swapPair) external onlyDev {
    require(_swapPair != address(0),"_swapPair can not be address 0");
    isOtherSwapPair[_swapPair] = true;
  }

  function removeOtherSwapPair(address _swapPair) external onlyDev {
    require(_swapPair != address(0),"_swapPair can not be address 0");
