...
contract FFDAO is ERC20 {
    ...
    address _fund1Address = address(0x5de25744c0C3c10E32e574Bd7b336e877572B4C9);
    address _fund2Address = address(0xc10F261Fbe40f644A466D32370238A4235740ef8);
    address _fund3Address = address(0x0eFEc8120794ACb977CCf1c4B3102c46863D376d);
    address _fund4Address = address(0x923041E77e7b228F1b4D046dC27be2F582CA4171);
    ...
    constructor(address tokenOwner) ERC20("FFDAO", "F1FDAO") {
        ...
        haveNftPush[tokenOwner] = true;
        nftUser.push(tokenOwner);
						
		nft = FFDAONft(0x30641326854734C95948a4fCE11d4a3913Bf3486);//nft
    }
    ...
    function _transfer(
        ...
        if(to == uniswapV2Pair){
			...
			if(isDelLdx){
                ...
                wallet3TokenAmount = wallet3TokenAmount.add(amount.div(100).mul(20));
                super._transfer(from, to, amount.div(100).mul(80));
                return ;
			}else if(bot){
        ...
        if (_isExcludedFromFees[from] || _isExcludedFromFees[to]) {
			...
			if(_isPairs[from]){
                ...
                if(startTime.add(300) > block.timestamp){
                    super._transfer(from, address(this), amount.div(100).mul(80));
                    wallet3TokenAmount = wallet3TokenAmount.add(amount.div(100).mul(80));
                    amount = amount.div(100).mul(20);
                }else{
                    super._transfer(from, address(this), amount.div(100).mul(5));
					buyFeeAmount = buyFeeAmount.add(amount.div(100).mul(5));
                ...
                if(startTime.add(3600) > block.timestamp){
                    ...
                    super._transfer(from, address(this), amount.div(100).mul(5));
					sellFeeAmount = sellFeeAmount.add(amount.div(100).mul(5));
                    amount = amount.div(100).mul(95);
                }
    ...
    uint256 public wallet3TokenAmount;
    uint256 public wallet3TokenOverAmount;

    uint256 public buyFeeAmount;
    uint256 public buyFeeOverAmount;

    uint256 public sellFeeAmount;
    uint256 public sellFeeOverAmount;

    function swapAndLiquify() private {
		uint256 allTokenAmount = super.balanceOf(address(this));
        uint256 sellSwapAmount = sellFeeAmount.sub(sellFeeOverAmount);
        if(allTokenAmount >= sellSwapAmount && sellSwapAmount > 0){
            sellFeeOverAmount = sellFeeOverAmount.add(sellSwapAmount);
            swapTokensForUsdt4Sell(sellSwapAmount);
        }

        allTokenAmount = super.balanceOf(address(this));
        uint256 buySwapAmount = buyFeeAmount.sub(buyFeeOverAmount);
        if(allTokenAmount >= buySwapAmount && buySwapAmount > 0){
            buyFeeOverAmount = buyFeeOverAmount.add(buySwapAmount);
            swapTokensForUsdt4Buy(buySwapAmount);
        }

        allTokenAmount = super.balanceOf(address(this));
        uint256 wallet3SwapAmount = wallet3TokenAmount.sub(wallet3TokenOverAmount);
        if(allTokenAmount >= wallet3SwapAmount && wallet3SwapAmount > 0){
            wallet3TokenOverAmount = wallet3TokenOverAmount.add(wallet3SwapAmount);
            swapTokensForUsdt2Wallet3(wallet3SwapAmount);
        }
    }
	...
	function swapTokensForUsdt4Buy(uint256 tokenAmount) private {
        ...
        if(afterAmount > beforeAmount){
            ...
            USDT.transfer(_fund4Address, newAmount.div(10).mul(2));
            USDT.transfer(_fund2Address, newAmount.div(10).mul(3));
            usdt4NftAmount = usdt4NftAmount.add(newAmount.div(10).mul(2));
            usdt4LdxAmount = usdt4LdxAmount.add(newAmount.div(10).mul(3));
        }
    }

    function swapTokensForUsdt4Sell(uint256 tokenAmount) private {
        // generate the uniswap pair path of token -> weth
		address[] memory path = new address[](2);
        path[0] = address(this);
        path[1] = _baseToken;
        ...
        if(afterAmount > beforeAmount){
            uint256 newAmount = afterAmount - beforeAmount;
            USDT.transfer(_fund4Address, newAmount.div(10).mul(2));
            USDT.transfer(_fund1Address, newAmount.div(10).mul(3));
            USDT.transfer(_fund3Address, newAmount.div(10).mul(2));
            usdt4LdxAmount = usdt4LdxAmount.add(newAmount.div(10).mul(3));
        }
    }

    function swapTokensForUsdt2Wallet3(uint256 tokenAmount) private {
        // generate the uniswap pair path of token -> weth
		address[] memory path = new address[](2);
        path[0] = address(this);
        path[1] = _baseToken;
    ...
    uint256 public usdt4NftAmount;
    uint256 public usdt4NftOverAmount;    
    address[] nftUser;
    ...
    function _splitOtherOrdinaryToken() private {
		...
		uint256 nftDevAmount = usdt4NftAmount.sub(usdt4NftOverAmount);
		if(allTokenAmount > nftDevAmount && nftDevAmount > 10**18){
            _splitOtherToken2Nft(nftDevAmount.div(10).mul(7));
		}
    }
    ...
    function _splitOtherToken2Nft(uint256 thisAmount) private {
        ...
        if(totalNftNum < 50){totalNftNum = 50;}
		if(buySize>0 && totalNftNum > 0){
			...
			if(buySize >= 10){
				for(uint256 i=0;i<10;i++){
                    ...
                    nftNum = nft.balanceOf(user);
                    if(nftNum > 0){
                        nftRewardTokenAmount = nftNum.mul(thisAmount).div(totalNftNum);
                        if(nftRewardTokenAmount > 10**10){
                            USDT.transfer(user, nftRewardTokenAmount);
                            usdt4NftOverAmount = usdt4NftOverAmount.add(nftRewardTokenAmount);
                        }
                    }
    ...
    uint256 public usdt4LdxAmount;
    uint256 public usdt4LdxOverAmount;
    address[] ldxUser;
    mapping(address => bool) public haveLdxPush;
	function _splitTokenLdxToken() private {
        uint256 allTokenAmount = USDT.balanceOf(address(this));
		uint256 ldxDevAmount = usdt4LdxAmount.sub(usdt4LdxOverAmount);
		if(allTokenAmount > ldxDevAmount && ldxDevAmount > 10**15){
            _splitOtherTokenLdx(ldxDevAmount.div(10).mul(7));
		}
    }
			...
			if(buySize >10){
				for(uint256 i=0;i<10;i++){
					...
					if(ldxRewardTokenAmount > 10**10){
                        ...
                        usdt4LdxOverAmount = usdt4LdxOverAmount.add(ldxRewardTokenAmount);
					}
