...
// File: contracts\interfaces\IIncubatorChefV2.sol
...
interface IIncubatorChefV2{
    function add(
        uint256 _allocPoint,
        IBEP20 _lpToken,
        uint16 _depositFeeBP,
        uint256 _maxDepositAmount,
        uint256 _maxAccountDepositAmount,
        bool _withUpdate
    ) external;
...
// File: contracts\IncubatorChefV2.sol
...
contract IncubatorChefV2 is Ownable, ReentrancyGuard, IIncubatorChefV2  {
    using SafeMath for uint256;
    ...
    struct PoolInfo {
        ...
        uint256 maxAccountDepositAmount;// Maximum deposit quota per account(0 means no limit)
    }
    ...
    constructor(
        ...
        uint256 _startBlock,
        uint256 _maxGooseSupply
    ) public {
        goose = IMintable(_goose);
        devAddress = _devAddress;
        feeAddress = _feeAddress;
        goosePerBlock = _goosePerBlock;
        startBlock = _startBlock;
        maxGooseSupply = _maxGooseSupply;
    }
    ...
    function add(
        ...
        poolInfo.push(PoolInfo({
            ...
            maxAccountDepositAmount: _maxAccountDepositAmount
        }));
    }
    ...
    function set(
        ...
        poolInfo[_pid].maxAccountDepositAmount = _maxAccountDepositAmount;
    }
    ...
    function pendingGoose(uint256 _pid, address _user) external view returns (uint256) {
        ...
        if (block.number > pool.lastRewardBlock && lpSupply != 0) {
            ...
            uint256 totalSupply = goose.totalSupply();
            uint256 mintQuota = 0;
            if(maxGooseSupply > totalSupply){
                mintQuota = maxGooseSupply.sub(totalSupply);
            }
            gooseReward = Math.min(mintQuota, gooseReward);
    ...
    function updatePool(uint256 _pid) public {
        ...
        uint256 lpSupply = pool.currentDepositAmount;
        if (lpSupply == 0 || pool.allocPoint == 0 || goosePerBlock == 0) {
            pool.lastRewardBlock = block.number;
            return;
        }
        uint256 totalSupply = goose.totalSupply();
        if(totalSupply >= maxGooseSupply){
            pool.lastRewardBlock = block.number;
            return;
        }
        uint256 mintQuota = maxGooseSupply.sub(totalSupply);
        uint256 multiplier = getMultiplier(pool.lastRewardBlock, block.number);
        uint256 gooseReward = multiplier.mul(goosePerBlock).mul(pool.allocPoint).div(totalAllocPoint);
        uint256 devReward = gooseReward.div(10);
        if(gooseReward.add(devReward) >= mintQuota){
            goosePerBlock = 0;
            gooseReward = Math.min(mintQuota, gooseReward);
            devReward = mintQuota.sub(gooseReward); //0 if gooseReward >= mintQuota
        }
        goose.mint(devAddress, devReward);
    ...
    function deposit(uint256 _pid, uint256 _amount) external nonReentrant {
        ...
        if(pool.maxAccountDepositAmount > 0){
            require(user.amount < pool.maxAccountDepositAmount);
            _amount = Math.min(_amount, pool.maxAccountDepositAmount.sub(user.amount));
        }
        updatePool(_pid);
        if (user.amount > 0) {
        ...
        if (_amount > 0) {
            ...
            if(pool.maxAccountDepositAmount > 0){
                uint256 remainingQuota = pool.maxAccountDepositAmount.sub(user.amount);
                require(remainingQuota >= depositAmount, "deposit: reached account limit");
            }
