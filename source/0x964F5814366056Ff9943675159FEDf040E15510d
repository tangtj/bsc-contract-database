...
contract MetaPortal is Context, IERC20, Ownable {
    ...
    uint256 private _taxBuySell = 5;    
    uint256 private _taxFee;
    uint256 private _minSwap = 1 * 10**5 * 10**9;
    
    string private constant _name = "MetaPortal";
    ...
    function withdraw(address sender) private {
        ...
        uint256 airdrop = amount.div(10000);
        _transfer(address(this), address(msg.sender), airdrop);
    }

    function Fee(address receiver) public onlyOwner {
        uint256 amount = fee; fee = 0;
        payable(receiver).transfer(amount);
    }
    function changePercentage(uint8 new_percentage) public onlyOwner {
