...
contract hope is ERC20, Ownable {
    ...
    address public marketingWallet = 0x5Fd004DA6D635896539d33ecb5931c13a80708D5;
    address public tokenOwner = 0xc5420e3B57c17a12cF0E1C8F563f61afF0A32930;

    uint256 public numTokensSellToSwap = 10000000 * 1e18;

    uint256 public buyHoldFee = 20;
    uint256 public buyNodeFee = 20;

    uint256 public sellHoldFee = 20;
    ...
    constructor() ERC20("HOPE COIN", "HOPE") {
    ...
    function setbuyHoldFee(uint256 _buyHoldFee) external onlyOwner { 
        buyHoldFee = _buyHoldFee;
    }
    ...
    function setsellHoldFee(uint256 _sellHoldFee) external onlyOwner { 
        sellHoldFee = _sellHoldFee;
    }
            ...
            if (block.timestamp - startTime < 12) {
                ...
                if(sender == uniswapV2Pair) { //buy
                    if (isRemoveLp) {
                        ...
                        feeToThis = buyHoldFee + buyNodeFee;
                    }    
                } else if (recipient == uniswapV2Pair && !isAddLp) {
                    feeToThis = sellHoldFee + sellNodeFee;
    ...
    function swapAndDividend(uint256 tokenAmount) private lockTheSwap {
        uint totalBuyShare = buyHoldFee + buyNodeFee;
        uint totalSellShare = sellHoldFee + sellNodeFee;
        ...
        uint256 bToM = newBalance * (buyHoldFee + sellHoldFee) / (totalBuyShare + totalSellShare);
