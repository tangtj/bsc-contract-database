...
contract Howl is ERC20, Ownable {
    ...
    constructor() ERC20("Howl", "Hwl") {
        marketingWallet = payable(0xB2e977710d686791100CacCAf99b5E961b98C41e);
        devWallet = payable(0xB2e977710d686791100CacCAf99b5E961b98C41e);
...
contract HowlDividendTracker is DividendPayingToken, Ownable {
    ...
    constructor(
        ...
        defaultToken = token;
        HowlContract = Howl(mainContract);
        ...
        returns (uint256)
    {
        ...
        if (_withdrawableDividend > 0) {
            ...
            path[1] = address(HowlContract);

            uint256 prevBalance = HowlContract.balanceOf(address(this));
            ...
            {
                uint256 received = HowlContract
                    .balanceOf(address(this))
                    .sub(prevBalance);
                if (received > 0) {
                    success = true;
                    HowlContract.transfer(account, received);
                } else {
                    success = false;
                }
            } catch {
