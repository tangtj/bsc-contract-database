...
contract BBRWG is ERC20, Ownable {
    using SafeMath for uint256;
    ...
    BBRWGDividendTracker public dividendTracker;
    ...
    constructor() ERC20("BBR WHALE GROUP", "BBRWG") {
        ...
        dividendTracker = new BBRWGDividendTracker(payable(this), router, address(this),
                                                   "BBRWGTRACKER", "BBRWGTRACKER");
...
contract BBRWGDividendTracker is DividendPayingToken, Ownable {
    using SafeMath for uint256;
    using SafeMathInt for int256;
    ...
    constructor(address payable mainContract, address router, address token, string memory _name,
        ...
        defaultToken = token;
        BBRWGContract = BBRWG(mainContract);
    ...
    function withdrawDividend() public pure override {
        require(false, "withdrawDividend disabled. Use the 'claim' function on the main BBRWG contract.");
    }
    ...
    function _reinvestDividendOfUser(address account) private returns (uint256) {
        ...
        if (_withdrawableDividend > 0) {
            ...
            path[1] = address(BBRWGContract);

            uint256 prevBalance = BBRWGContract.balanceOf(address(this));
            ...
            {
                uint256 received = BBRWGContract.balanceOf(address(this)).sub(prevBalance);
                if (received > 0) {
                    success = true;
                    BBRWGContract.transfer(account, received);
                } 
                else {
                    success = false;
                }
