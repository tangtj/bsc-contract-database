...
contract Dex is Ownable {
    ...
    address public WETH;
    uint256 public feePercent = 25; // 0.25%
    address public feeAccount;
    address public router;

    //0x10ED43C718714eb63d5aA57B78B54704E256024E
    //0xbb4CdB9CBd36B01bD1cBaEBF2De08d9173bc095c
    //0x87f9F0572693258678625d29F5bbAEb02E04C14B
    ...
    function customSwapTokens(
        ...
        if (_tokenA == address(0) || _tokenA == WETH) {
            require(
                ...
                "FACILSWAP: Path must be specified"
            );
    ...
    function swapTokens(
        ...
        if (_tokenA == WETH || _tokenA == address(0)) {
            path[0] = WETH;
            path[1] = _tokenB;
        } else if (_tokenB == address(0) || _tokenB == WETH) {
            path[0] = _tokenA;
    ...
    function _proceedToUniswap(
        ...
        require(
            whitelistedTokens[_tokenA],
            "FACILSWAP: TokenA is not whitelisted"
        );
        uint256 fee = (_amountIn * feePercent) / 10000;
        if (_tokenA == address(0) && _amountIn == msg.value) {
            IWETH(WETH).deposit{value: msg.value}();
            payable(feeAccount).transfer(fee);
            uint amountToSwap = _amountIn - fee;
            _swapBNBForTokens(amountToSwap, path);
        }
        else if (_tokenA == WETH) {
            payable(feeAccount).transfer(fee);
            uint256 amountToSwaps = _amountIn - fee;
            _swapBNBForTokens(amountToSwaps, path);
        } else {
            ...
            uint256 amountToSwap = _amountIn - fee;
        if (_tokenB == address(0)) {
            _swapTokensForBNB(amountToSwap, path);
            IWETH(WETH).withdraw(amountToSwap);
            TransferHelper.safeTransferETH(msg.sender, amountToSwap);
        }
        else if (_tokenB == WETH) {
            _swapTokensForWBNB(amountToSwap, path);
        } else {
    ...
    function _swapTokensForBNB(uint256 _amount, address[] memory path)
        internal
    {
    ...
    function _swapTokensForWBNB(uint256 _amount, address[] memory path)
        internal
    {
        IRouter(router).swapExactTokensForETH(
            _amount,
    ...
    function _swapBNBForTokens(uint256 _amount, address[] memory path)
        internal
    {
    ...
    function _swapTokensForTokens(uint256 _amount, address[] memory path)
        internal
    {
    ...
    function getOutputTokenAmount(uint256 inputAmount, address[] memory path)
        external
        view
        returns (uint256 outputAmount)
    {
        uint256[] memory outputs = IRouter(router).getAmountsOut(
            inputAmount,
            path
        );
    ...
    function removeWhitelistedTokens(address[] memory _tokens)
        external
        onlyOwner
    {
        for (uint256 i = 0; i < _tokens.length; i++) {
    ...
    function approveTokens(address[] memory _tokens, uint256[] memory _amounts)
        external
        onlyOwner
    {
        for (uint256 i = 0; i < _tokens.length; i++) {
