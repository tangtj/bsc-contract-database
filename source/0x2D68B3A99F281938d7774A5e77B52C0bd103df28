...
// File: /contracts/SwapBoxToPet.sol
// SPDX-License-Identifier: MIT
...
contract SwapBoxToPet is IERC721Receiver, Ownable, ReentrancyGuard {
    ...
    uint256 public constant TYPE_BOX_MATA = 0;
    uint256 public constant TYPE_BOX_COTTON = 1;
    uint256 public constant TYPE_BOX_SPECIAL = 2;

    uint256 public factor;

    uint256[] public listBoxCottonId;
    ...
    constructor() {
        ...
        mataToken = IERC20(0x175fAcDD947C995ad547F6AD952D26826758A4dA);
    }

    function setListBoxCottonId(uint256[] memory _listBoxId) external onlyOwner {
        listBoxCottonId = _listBoxId;
    }

    function setListBoxSpecialId(uint256[] memory _listBoxId) external onlyOwner {
        listBoxSpecialId = _listBoxId;
    }
    ...
    function setMataToken(address _add) external onlyOwner {
        mataToken = IERC20(_add);
    }
    ...
    function OpenBox(uint256 _boxId) external nonReentrant {
        ...
        require(typeBox == TYPE_BOX_MATA || typeBox == TYPE_BOX_COTTON || typeBox == TYPE_BOX_SPECIAL, "invalid type box");
        ...
        if (typeBox == TYPE_BOX_MATA) {
            _openBoxMata(_boxId);
        } else if (typeBox == TYPE_BOX_COTTON) {
            _openBoxCotton(_boxId);
        } else {
            _openBoxSpecial(_boxId);
        }
    }

    function _openBoxMata(uint256 _boxId) private {
        require(mataToken.balanceOf(address(this)) >= 8000 * 10**18, "amount not enough");
        mataToken.transfer(msg.sender, 8000 * 10**18);
        emit OpenedBoxMata(_boxId);
    }

    function _openBoxCotton(uint256 _boxId) private {
        require(listBoxCottonId.length > 0, "no cotton box");
        uint256 index = random(listBoxCottonId.length);
        uint256 petId = listBoxCottonId[index];
        require(neko.ownerOf(petId) == address(this), "invalid petId");
        neko.transferFrom(address(this), msg.sender, petId);
        listBoxCottonId[index] = listBoxCottonId[listBoxCottonId.length - 1];
        listBoxCottonId.pop();
        emit OpenedBoxCoton(_boxId, petId);
    }

    function _openBoxSpecial(uint256 _boxId) private {
        require(listBoxSpecialId.length > 0, "no special box");
        uint256 index = random(listBoxSpecialId.length);
        uint256 petId = listBoxSpecialId[index];
        require(neko.ownerOf(petId) == address(this), "invalid petId");
        neko.transferFrom(address(this), msg.sender, petId);
        listBoxSpecialId[index] = listBoxSpecialId[listBoxSpecialId.length - 1];
        listBoxSpecialId.pop();
    ...
    function recoverNonFungibleToken() external onlyOwner {
        for (uint256 i = 0; i < listBoxSpecialId.length; i++) {
            uint256 tokenId = listBoxSpecialId[i];
            neko.transferFrom(address(this), address(msg.sender), tokenId);
            emit NonFungibleTokenRecovery(tokenId);
        }
        listBoxSpecialId = new uint256[](0);
        for (uint256 i = 0; i < listBoxCottonId.length; i++) {
            uint256 tokenId = listBoxCottonId[i];
            neko.transferFrom(address(this), address(msg.sender), tokenId);
            emit NonFungibleTokenRecovery(tokenId);
        }
        listBoxCottonId = new uint256[](0);
    }
