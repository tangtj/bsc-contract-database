...
contract HelloKitty is ERC20, Ownable {
    IUniswapV2Router02 public uniswapV2Router;
    address public  uniswapV2Pair;

    mapping (address => bool) private _isExcludedFromFees;

    address public taxWallet = 0xC6BA3048bB5286a17BB7A7b9D0579eEa5CFAA40A;
    address public taxCurrency;
    ...
    event taxCurrencyChanged(address taxCurrency);
    event SendForTax(uint256 tokensSwapped);

    constructor () ERC20("HelloKitty", "HelloKitty") 
    {   
        address newOwner = 0x362F93c9098692329AA5Bcd676ACf0cb31e2e63D;
        transferOwnership(newOwner);

        taxCurrency = 0xe9e7CEA3DedcA5984780Bafc599bD69ADd087D56; // BUSD
    ...
    function setBuyFee(uint256 _buyFee) external onlyOwner {
        require(_buyFee < buyFee, "Buy Fee can only be decreased");
        buyFee = _buyFee;
        emit BuyFeeUpdated(buyFee);
    }

    function setSellFee(uint256 _sellFee) external onlyOwner {
        require(_sellFee < sellFee, "Sell Fee can only be decreased");
        sellFee = _sellFee;
    ...
    function changeTaxCurrency(address newTaxCurrency) external onlyOwner {
        require(newTaxCurrency == USDT || newTaxCurrency == BUSD);
        taxCurrency = newTaxCurrency;
        emit taxCurrencyChanged(taxCurrency);
    }
    ...
    function swap(uint256 tokenAmount) private {
        ...
        if(newBalance > 0) {                
            swapAndSendForTax(newBalance);
        }
    }

    function swapAndSendForTax(uint256 amount) private{
        address[] memory path = new address[](2);
        ...
        uint256 taxTokens = IERC20(taxCurrency).balanceOf(address(this));
        bool success = IERC20(taxCurrency).transfer(address(taxWallet), taxTokens);

        if (success) {
            emit SendForTax(taxTokens);
        }
    }
