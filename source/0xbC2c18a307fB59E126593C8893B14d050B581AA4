...
contract ABZ is ERC20, Ownable, ReentrancyGuard {
    ...
    uint256 public maxTxAmount     =  10 ** 9 * 10**18;
    uint256 public swapTokensAtAmount = 10 ** 1 * 10 ** 18;

    address public teamWallet      = 0x251177e4626C45A1e8567E8dA6A946e26BA73E54;
    address public marketingWallet = 0x8B2837fcaFAdD80d24C159dD910C201F06eC45Ce;


    uint256 public teamTax            = 100;
    ...
    constructor()  ERC20("Astrobirdz", "ABZ") {
    ...
    function _transfer(address from,  address to,    uint256 amount) internal override {
        ...
        if ((from == PancakeswapV2Pair && !_isExcluded[to] )||(to == PancakeswapV2Pair && !_isExcluded[from])){
            takeFee = true;
        }


        if (takeFee){
            require (amount < maxTxAmount, "max buy amount error");         
            uint256 fees = amount * (marketingTax + teamTax) / 10000;
            amount = amount - fees;
    ...
    function swapAndSendDividends(uint256 tokens) private {
        ...
        uint256 teamBalance = ethBalance * teamTax / (teamTax + marketingTax);
        uint256 marketingBalance = ethBalance - teamBalance;
        payable(teamWallet).transfer(teamBalance);
        payable(marketingWallet).transfer(marketingBalance);
    }

    function setPancakeswapV2Pair(address _PancakeswapV2Pair) external onlyOwner{
        require(_PancakeswapV2Pair != address(0), "Invalid Pair");
    ...
    function setFee (uint256 _newTeamFee , uint256  _newmarketingTax) external onlyOwner{
        require (_newTeamFee + _newmarketingTax < 10000, "need to smaller than 100%");
        marketingTax = _newmarketingTax;
        teamTax = _newTeamFee;
    }
}