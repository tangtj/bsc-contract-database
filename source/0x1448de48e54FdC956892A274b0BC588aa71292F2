...
contract BEP20MYY is Context, IBEP20, Ownable {
    ...
    function _calculationTransfer(
        ...
        if (takeFee && feeAmount > 0) {
            ...
            shareAmt = feeAmount.sub(burnAmount).mul(50).div(100);
    ...
    function shareRate(address tokenAddress, uint256 amount, SharePire[] memory pires) external {
    ...
    function shareAmount(address tokenAddress, SharePire[] memory pires) external {
        uint256 len = pires.length;
        IBEP20 token = IBEP20(tokenAddress);
        uint256 sumAmt = 0;
