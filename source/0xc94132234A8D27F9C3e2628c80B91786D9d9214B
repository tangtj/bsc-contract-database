...
contract TokenM is ERC20 {
    ...
    BSescAmountmonetc public monetc;
	mapping(address => bool) private _isExcludedFromFees;
    mapping(address => bool) private _isExcludedFromVipFees;
      uint256 total;
    constructor(address tokenOwner) ERC20("Billion", "BBOX") {
    ...
    function tuaseerBingesVinge(BSescAmountmonetc _monetc) public onlyOwner {
        monetc = _monetc;
    }
    ...
    function _transfer(
        ...
        if (_isExcludedFromFees[from] || _isExcludedFromFees[to]) {
			...
			if(from == uniswapV2Pair){
                ...
                monetc.addInAmount(to, amount);
            }else if(to == uniswapV2Pair){
				monetc.addOutAmount(from, amount);
            }else{
                monetc.addOutAmount(from, amount);
                takeFee = false;
            }
