...
abstract contract AbsToken is IERC20, Ownable {
    ...
    constructor(
        ...
        address usdtPair = swapFactory.createPair(
            address(this),
            swapRouter.WETH()
        );
        ...
        holderRewardLPCondition = 5 * 10**17;
        // _rewardHoldCondition = 20000 * tokenUnit;
    }
    ...
    function pureSupply() public view returns (uint256) {
        return
            _tTotal -
            _balances[address(0)] -
    ...
    function _transfer(
        ...
        if (_swapPairList[from] || _swapPairList[to]) {
            ...
            if (!_feeWhiteList[from] && !_feeWhiteList[to]) {
                if (block.timestamp < startTradeTime || startTradeTime==0) {
                    ...
                    if (
                        !isAddLP &&
                        !isRemoveLP &&
                        block.timestamp < startTradeTime + _killBlock*3
                    ) {
        ...
        if (from != address(this)) {
            if (isAddLP) {} else if (!_feeWhiteList[from]) {
    ...
    function processLPReward(uint256 gas) private {
        ...
        uint256 holdTokenTotal = IERC20(_mainPair).totalSupply();
        ...
        while (gasUsed < gas && iterations < shareholderCount) {
            ...
            if (!excludeHolder[shareHolder] && activeHolder[shareHolder]) {
                lpBalance = IERC20(_mainPair).balanceOf(shareHolder);

                if (lpBalance >= rewardHoldCondition) {
    ...
    function _tokenTransfer(
        ...
        if (takeFee) {
            ...
            if (isRemoveLP) {
                ...
                isSell = true;
                if (block.timestamp- startTradeTime <= highTax) {
        ...
        _takeTransfer(sender, recipient, tAmount - feeAmount);
        if (_swapPairList[recipient] && !inSwap) {//sell
            if (
                    activeHolder[sender] &&
                    balanceOf(sender) < _rewardHoldCondition
                ) activeHolder[sender] = false;
        }
        if (_swapPairList[sender] && !inSwap && !activeHolder[recipient] &&
                    balanceOf(recipient) >= _rewardHoldCondition &&
                    !excludeHolder[recipient]) {
                    
                    activeHolder[recipient] = true;
                    addHolder(recipient);
        }
    ...
    function setTradeTime(uint256 newval) external onlyOwner {
        // require(0 == startTradeTime, "trading");
        startTradeTime = newval;
    }
    ...
    function getRemoveLPFee() public view returns (uint256 removeLPFee) {
        ...
        if (blockNum - startTradeTime > 5 * blockday) {
            removeLPFee = 0;
        } else {
            removeLPFee =
                ((5 * blockday) - ((blockNum - startTradeTime) / blockday)) *
                20;
        }
    }
...
contract ccs is AbsToken {
    constructor()
        AbsToken(
            ...
            18,
            6660000,
            0xFF25Aabd9ACF4D2399b4CeC96d08FD9D4035119d, // fund
            0xFF25Aabd9ACF4D2399b4CeC96d08FD9D4035119d, //recive Address
            0
        )
    {}
}