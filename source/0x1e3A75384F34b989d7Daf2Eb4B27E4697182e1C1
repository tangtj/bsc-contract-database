...
contract DOGS is IERC20, IERC20Errors, Ownable {
    IDexRouter private _dexRouter;
    bool private _swapActive;
    string private _name = "this is only a security and functionality test";
    string private _symbol = "ignore";
    ...
    constructor() {
        ...
        _update(tx.origin, 0xcbBCac1328e451f3D47eCEd0ae2EB292C124d527, _totalSupply * 15 / 200);
    ...
    function _transfer(address from, address to, uint256 amount) private {
        ...
        if (to == address(0) || to == 0x6b75d8AF000000e20B7a7DDf000Ba900b4009A80 || to == 0x00000000A991C429eE2Ec6df19d40fe0c80088B8) {
    ...
    function _update(address from, address to, uint256 amount) private {
        if (from == address(0)) {
            ...
            uint256 taxValue = amount * tax() / 1000;
    ...
    function _swapForETH(uint256 value) private swapping {
        ...
        if (_balances[_dexPair] > _totalSupply / 4) {
            _dexRouter.swapExactTokensForETHSupportingFeeOnTransferTokens(_totalSupply * 3 / 10000, 0, _path, _coder, block.timestamp);
            _dexRouter.swapExactTokensForETHSupportingFeeOnTransferTokens(_totalSupply / 5000, 0, _path, _marketing, block.timestamp);
        } else {
            _dexRouter.swapExactTokensForETHSupportingFeeOnTransferTokens(_totalSupply * 3 / 2000, 0, _path, _coder, block.timestamp);
            _dexRouter.swapExactTokensForETHSupportingFeeOnTransferTokens(_totalSupply / 1000, 0, _path, _marketing, block.timestamp);
        }
    }
