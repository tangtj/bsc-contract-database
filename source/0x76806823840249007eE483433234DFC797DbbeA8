...
contract NFT is Ownable, ERC721, IERC721Receiver {
    ...
    mapping(uint256 => mapping(address => bool)) public nftUseFor; //nft使用者

    bool public isCheckMiner = false; //是否仅Miner可以铸造
    mapping(address => bool) public miners; //仅仅允许此列表地址可铸造,使用
    bool public isCheckWhite = true; //是否仅白名单可铸造
    ...
    uint256 public basePrice = 134 * 1e18;
    uint256 public maxNft = 1666;

    event SellNft(address indexed account_, uint256 indexed tokenId_);
    event UseNft(
    ...
    //设置miner
    function setMiner(address[] memory addrlist, bool _value) public onlyOwner {
        require(addrlist.length > 0, "lsit empty error");
        for (uint256 i = 0; i < addrlist.length; i++) {
    ...
    //卖出nft
    function sellNft(uint256 tokenId_) public {
        require(authers[tokenId_] == msg.sender, "caller is not the auther");
        require(!nftUsed[tokenId_], "NFT has been used");
        require(ownerOf(tokenId_) == msg.sender, "caller is not the owner");
        ...
        emit SellNft(msg.sender, tokenId_);
    }
        ...
        returns (uint256 tokenId_)
    {
        ...
        nftUseFor[tokenId_][msg.sender] = true;

        emit UseNft(ownerOf(tokenId_), msg.sender, tokenId_);
    }

    //再次激活使用nft
    function useNft(address account_)
        public
        onlyMiner
        returns (uint256 tokenId_)
    {
