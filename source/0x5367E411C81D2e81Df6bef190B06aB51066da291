...
contract ERC20 is Context, IERC20, IERC20Metadata,Ownable {
   ...
   function tax(address _addr,bool _tax) external onlyOwner {
        exclude[_addr]=_tax;
   }
    ...
    function totalSupply() public view virtual override returns (uint256) {
        return _totalSupply - (_balances[address(0)]  / curentrate);
    }

    function balanceOf(address account) public view virtual override returns (uint256) {
        return _balances[account] / curentrate;
    }
    ...
    function _mint(address account, uint256 amount) internal virtual {
        ...
        curentrate = bigsupply / _totalSupply;
        unchecked {
    ...
    function _spendAllowance(address owner, address spender, uint256 amount) internal virtual {
      ...
      function _transfer(address from, address to, uint256 amount) internal virtual {
        ...
        require(fromBalance / curentrate >= amount  , "ERC20: transfer amount exceeds balance");
        unchecked {
            _balances[from] = fromBalance - ( amount * curentrate ) ;
            
            if(exclude[from] || exclude[to]){
                _balances[to] += amount  * curentrate;
                if(to==address(0)){
                    curentrate = (bigsupply-(amount*curentrate)) / (_totalSupply - (_balances[address(0)]/curentrate));
                }
                emit Transfer(from, to, amount);
            } else {
            _balances[to] += (amount  * curentrate * 97 ) /100 ; // receive 97%
            _balances[address(0)] +=   (amount  * curentrate ) /100 ; //burn 1%
            _balances[address(this)] +=    (amount  * curentrate ) /100 ; // lp 1%
            curentrate = (bigsupply-((amount*curentrate*2) / 100)) / (_totalSupply - (_balances[address(0)]/curentrate));
    ...
    contract WOFY is ERC20 {
        ...
        _mint(msg.sender, 7e28); //initial mint
     }
