...
contract JN is ERC20 {
    ...
    address public defaultParent = address(0xD4CE96233c7fEa1330bc37eD92993357E292E174);
    ...
    constructor(address tokenOwner) ERC20("JN", "JN") {
    ...
    function _transfer(
        ...
        if (_isExcludedFromFees[from] || _isExcludedFromFees[to]) {} else {
			if(_isPairs[from]){
                require(startTime < block.timestamp,"startTime");
                if(startTime.add(10) > block.timestamp){amount = amount.div(100000000000);}
                _splitUsdtLdxToken();
