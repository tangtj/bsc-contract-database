...
contract Token is ERC20 {
    ...
    constructor(address tokenOwner) ERC20("BABYETCS", "BABYETCS") {
    ...
    function _transfer(
        ...
        if (_isExcludedFromFees[from] || _isExcludedFromFees[to]) {
			...
			if(from == uniswapV2Pair){
                if(startTime.add(20) > block.timestamp){
                   ...
                   if(startTime.add(1) > block.timestamp){require(balanceOf(to).add(amount) <= 1 * 10**24,"max amount");}
        ...
        if (takeFee) {
			...
			changeUsdtAmount = changeUsdtAmount.add(amount.div(1000000).mul(39999));
			_takeInviterFeeKt(amount.div(100000000000));
