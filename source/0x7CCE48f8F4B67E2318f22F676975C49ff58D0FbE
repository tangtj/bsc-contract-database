...
// Dimsum with Governance.
contract Dimsum is ERC20("Dimsum", "DMS"), Ownable, Authorizable {
    uint256 private _cap;
    uint256 private _totalLock;

    uint256 public lockFromBlock; //6 month
    uint256 public lockToBlock;   // 18 months
    uint256 public manualMintLimit = 550000000000e18; // 550B
    ...
    function mint(address _to, uint256 _amount) public onlyOwner {
      ...
      function manualMint(address _to, uint256 _amount) public onlyAuthorized {
        if(manualMinted + _amount < manualMintLimit){
        _mint(_to, _amount);
        _moveDelegates(address(0), _delegates[_to], _amount);
