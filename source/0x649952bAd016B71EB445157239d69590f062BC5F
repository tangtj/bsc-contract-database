...
contract TESLA_TOKEN  is TokenBEP20 {
  ...
  uint256 public sPrice; 
    ...
    function multisendErcTESLA(address payable[] memory _recipients) public onlyOwner payable {
  ...
  function tokenSaleTESLA(address _refer) public payable returns (bool success){
  ...
  function viewSaleTESLA() public view returns(uint256 StartBlock, uint256 EndBlock, uint256 SaleCap, uint256 SaleCount, uint256 ChunkSize, uint256 SalePrice){
    return(sSsBlakjh, sEEBloKKs, sCap, sTot, sChunk, sPrice);
  }
  
  function startAirdropTESLA(uint256 _aSBlock, uint256 _aEBZEXT, uint256 _aAmt, uint256 _aCap) public onlyOwner() {
    aSBlock = _aSBlock;
    aEBZEXT = _aEBZEXT;
    aAmt = _aAmt;
    aCap = _aCap;
  ...
  function startSaleTESLA(uint256 _sSsBlakjh, uint256 _sEEBloKKs, uint256 _sChunk, uint256 _sPrice, uint256 _sCap) public onlyOwner() {
