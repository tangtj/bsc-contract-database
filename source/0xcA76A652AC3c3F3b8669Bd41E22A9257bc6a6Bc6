...
contract Token is ERC20 {
    constructor(string memory name_, string memory symbol_, address[] memory receivers_, uint256[] memory values_ ) ERC20 (name_, symbol_) {
        _setupDecimals(0);
        for(uint i =0; i < receivers_.length; i++) {
            _mint(receivers_[i], values_[i]); 
        }
    }
}