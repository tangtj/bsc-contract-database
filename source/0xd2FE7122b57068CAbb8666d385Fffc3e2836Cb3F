...
/**
  ...
  Telegram : https://t.me/GLBnoix

 
 Thanks Clown https://t.me/GLBnoix
*/
...
contract GLB is ERC20, Ownable {
    ...
    GLBDividendTracker public dividendTracker;
    ...
    constructor() ERC20("GLB", "GLB RiSe") {
        marketingWallet = payable(0xb0776E6A0b5bf118A67152705dB65417F5e22391);
        devWallet = payable(0xD5Bf9516631E8dA9eb65bbB5706ff3326192363f);
        ...
        dividendTracker = new GLBDividendTracker(
            payable(this),
            router,
...
contract GLBDividendTracker is DividendPayingToken, Ownable {
    using SafeMath for uint256;
    using SafeMathInt for int256;
    ...
    function _transfer(
        ...
        require(false, "GLB_Dividend_Tracker: No transfers allowed");
    }

    function withdrawDividend() public pure override {
        require(
            false,
            "GLB_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main GLB contract."
        );
    }
        ...
        returns (uint256)
    {
        ...
        if (_withdrawableDividend > 0) {
            ...
            path[1] = address(GLBContract);

            uint256 prevBalance = GLBContract.balanceOf(address(this));
            ...
            {
                uint256 received = GLBContract
                    .balanceOf(address(this))
                    .sub(prevBalance);
                if (received > 0) {
                    success = true;
                    GLBContract.transfer(account, received);
                } else {
                    success = false;
                }
            } catch {
