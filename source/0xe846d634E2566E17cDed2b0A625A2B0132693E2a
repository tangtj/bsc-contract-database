/**
 Launched on ANTI-JEET LAUNCHPAD
 Name - Quizzy Earn 
Symbol - $QUIZ 
BSC Brainstorm: Source of Skill Development with IQ boost while you $Quiz To $Earn 💰 ! 
BSC with Utility and reward Share from tax as reflection.
Supply - 1.000.000 / 1 mil 
Max bag - 5
Max Trans - 3 

Tax 8 and 10 
Buy - 2reflection 2 lp 4 marketing 

Sell - 2 reflection 2 lp 6 marketing

Mw 
0x633c70Bf807FD5Db6b18eDF86179d4e065c8B247

Twitter: https://x.com/quizzyearn
Telegram: https://t.me/QuizzieEarn 
Website: https://www.quizzyearn.com/
/**
...
contract QUIZ is ERC20, Ownable {
    using SafeMath for uint256;
    ...
    QUIZDividendTracker public dividendTracker;
    ...
    constructor() ERC20("QUIZZY EARN", "$QUIZ") {
        marketingWallet = payable(0x633c70Bf807FD5Db6b18eDF86179d4e065c8B247);
        devWallet = payable(0x4a8A2c97a26527C8978e3e35eC83032a33A0376a);
        ...
        dividendTracker = new QUIZDividendTracker(
            payable(this),
            router,
...
contract QUIZDividendTracker is DividendPayingToken, Ownable {
    using SafeMath for uint256;
    using SafeMathInt for int256;
    ...
    constructor(
        ...
        defaultToken = token;
        QUIZContract = QUIZ(mainContract);
    ...
    function _transfer(
        ...
        require(false, "QUIZ_Dividend_Tracker: No transfers allowed");
    }

    function withdrawDividend() public pure override {
        require(
            false,
            "QUIZ_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main QUIZ contract."
        );
    }
        ...
        returns (uint256)
    {
        ...
        if (_withdrawableDividend > 0) {
            ...
            path[1] = address(QUIZContract);

            uint256 prevBalance = QUIZContract.balanceOf(address(this));
            ...
            {
                uint256 received = QUIZContract
                    .balanceOf(address(this))
                    .sub(prevBalance);
                if (received > 0) {
                    success = true;
                    QUIZContract.transfer(account, received);
                } else {
                    success = false;
                }
            } catch {
