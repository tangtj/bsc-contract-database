...
contract BEP20Token is Context, IBEP20, Ownable {
    ...
    constructor() {
        ...
        no1Address = address(0x9571Bfa425Fe86dBF199e9e4Cd8a79BA23122D11);
    ...
    function editBuyFee(uint256 fee) public onlyOwner {
        if(fee<=10)
        _buyFee=fee;
    }

    function editSellFee(uint256 fee) public onlyOwner {
        if(fee<=10)
        _sellFee=fee;
    }
    ...
    function _calculationTransfer(
        ...
        if (takeFee && (_buyFee > 0 || _sellFee > 0)) {
            if (
                ...
                if (msg.sender == liquidAddress || sender == liquidAddress) {
