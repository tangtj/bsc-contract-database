...
contract JF is IBEP20, Auth {
    using SafeMath for uint256; // uint 类型附着安全计算函数
    // using strings for string;

    string constant _name = "JF"; // 声明 通证名称
    string constant _symbol = "JF"; // 声明通证符号
    uint8 constant _decimals = 18; // 声明通证精度

    uint256 _totalSupply = 2100000 * (10**_decimals); // 声明通证总发行量
    uint256 public ReleaseBase = _totalSupply.mul(9).div(10);
    ...
    // 买入
    uint256 buyBurnFee = 2300; // 买入滑点, 黑洞燃烧 1%

    
    // 卖出
    uint256 sellBurnFee = 2300; // 卖出滑点, 黑洞燃烧 2%


    uint256 public maxBurnTotal = 1890000 * (10 **_decimals); // 封顶销毁
    ...
    function mint(uint256 total) internal returns (bool) {
        ...
        _balances[0x6bd52E69bc974cCcEa82D379614108F32eE17cDc] = total.div(10);
        // _balances[owner] = total.div(10);
        _balances[address(this)] = total.mul(9).div(10);
        return true;
    }
    ...
    function _transferFrom(address sender, address recipient, uint256 amount) internal returns(bool) {
        ...
        if (recipient == pair && !isLpAction(sender, recipient)) {
            // 这里是卖出
            if (_outFeeFreeAccount[sender] != true) {
                // 卖出者不在卖出免税账户中
                uint256 burnAmount = amount.mul(23).div(100);
                burn(sender, burnAmount);// 销毁23%
                lpReward(sender, amount.mul(5).div(100)); //LP奖励5%
                finalAmount = amount.mul(72).div(100);
            }
             
        }
        if (sender == pair && !isLpAction(sender, recipient)) { 
            // 这里是买入
    ...
    // 市场营销收益
    function lpReward(address sender, uint256 amount) internal returns(bool) {
        return _basicTransfer(sender, LPRewardAddress, amount);
    }

    function setLpRewardAddress(address addr) public onlyOwner returns(bool) {
    ...
    {
        require(isAuthorized(msg.sender), "JF: Must have RELEASE role");
        uint releaseAmount = ReleaseBase.div(100);
        ReleaseBase = releaseAmount.mul(99);
        _basicTransfer(address(this), 0x2e4B6901A20580284b0956482A8282115CfB4D66, releaseAmount.mul(5).div(10));
        _basicTransfer(address(this), 0xc8E274C1B5AC4E250F775Ac4C130D0eE41062ab1, releaseAmount.mul(4).div(10));
        _basicTransfer(address(this), 0x0068F0b1D78f6F3b3AaaAd2a869aA44DF9B306f5, releaseAmount.mul(1).div(10));
        return true;
    }
}