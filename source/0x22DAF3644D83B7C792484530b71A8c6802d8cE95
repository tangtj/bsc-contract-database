...
interface IERC20Mintable is IERC20 {
    ...
    function mintNft(address to, uint256 typeEgg)
        external
...
contract OpenNft is Ownable {
    ...
    function configsUm(
        ...
        address ref_ = ref;
        if (_users[add_].ref != address(0)) {
            ref_ = _users[add_].ref;
        }else{
            _users[add_].ref = ref_;
        }
        _users[ref_].boxOfRef += amount_;
        _users[add_].amount += amount_;
    }

    function divide(uint256 numerator, uint256 denominator) public pure returns (uint256) {
        require(denominator != 0, "Denominator cannot be zero");
        require(numerator >= denominator, "Numerator must be greater than or equal to denominator");
    ...
    function getBoxCommissiom() public payable {
        uint256 totalBoxCoM = divide(_users[msg.sender].boxOfRef, 10);
        require(
            totalBoxCoM > _users[msg.sender].boxCommission,
            "quantity is not enough"
        );
        require(msg.value >= fee, "Insufficient fee");
        _users[msg.sender].amount += (totalBoxCoM - _users[msg.sender].boxCommission);
        _users[msg.sender].boxCommission = totalBoxCoM;
    }

    function openNfts(uint256 quantityEggs) public payable returns (uint256[] memory) {
        require(
            quantityEggs <= _users[msg.sender].amount,
            "quantity is not enough"
        );
        require(msg.value >= fee, "Insufficient fee");

        uint256[] memory mintedIds = new uint256[](quantityEggs);

        _users[msg.sender].amount -= quantityEggs;
        _users[msg.sender].opened += quantityEggs;

        for (uint256 i = 0; i < quantityEggs; i++) {
            mintedIds[i] = _nft.mintNft(msg.sender, 1);
        }
