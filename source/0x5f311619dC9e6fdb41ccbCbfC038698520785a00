...
contract StakingFactory is CfoTakeableV2,ReentrancyGuard,IStakingFactory {
    ...
    modifier getAuth() {
        require(isRewardOperator[msg.sender] == true,"no op rights");
        _;
    }
    ...
    function setRewardSigner(address account,bool status) external getAuth {
        require(account!=address(0),"account can not be address 0");
    ...
    function setSwapRouter(address _swapRouter) external getAuth {
        swapRouter = _swapRouter;
    }
}