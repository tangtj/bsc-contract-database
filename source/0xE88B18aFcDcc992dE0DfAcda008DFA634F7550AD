...
contract LPTokenWrapper {
    ...
    IERC20 public x = IERC20(0x7F1184f1a647D68235FDA468C2951e2AA4CFaf46);
...
contract CarbonCCLPFarm is LPTokenWrapper, Ownable{
    using SafeMath for uint256;
    using Address for address;

    IERC20 public CarbonCC = IERC20(0xD03811a88bf40db9C246E7BD20f0E6c50eE95B38);
    IERC20 public CarbonCCLP = IERC20(0x7F1184f1a647D68235FDA468C2951e2AA4CFaf46);
    address public lpFeeAddress = 0xFBC44E6a83b057848dE43118551dF47036326D36;
    address public sponsorRewardAddress = 0xD71908cf6080295752b31b70f6d7b48AE2187B8E;
    address public treasuryAddress = 0xFBC44E6a83b057848dE43118551dF47036326D36;
    address public rewardProgramAddress = 0xD71908cf6080295752b31b70f6d7b48AE2187B8E;
    ...
    event lpFeeAdd(address indexed _address, uint indexed _amount);
    event sponsorReward(address indexed _address, uint indexed _amount);
    ...
    function Deposit(uint256 amount) override public updateReward(msg.sender) checkStart { 
        require(amount > 0, "Cannot stake 0");
        require(CarbonCCLP.balanceOf(msg.sender) >= amount, "Insufficient balance");
        
        CarbonCCLP.transferFrom(msg.sender, address(this), amount);

        uint256 feeAmount = amount * 2 / 100;
        uint256 taxAmount = amount * 8 / 100;
        amount -= feeAmount; 
        CarbonCCLP.transfer(lpFeeAddress, feeAmount); 

        uint256 remainingAmount = amount - feeAmount - taxAmount; 
        CarbonCCLP.transfer(sponsorRewardAddress, taxAmount);
        
        emit lpFeeAdd(msg.sender, feeAmount);
        emit sponsorReward(msg.sender, taxAmount);

        super.Deposit(remainingAmount);
        emit InsertUser(msg.sender, 0, remainingAmount);
    }
    ...
    function Claim() public updateReward(msg.sender) checkStart {
        ...
        if (reward > 0) {
            ...
            // Take 30% tax
            uint256 taxAmount = reward * 30 / 100; 
            ...
            uint256 remainingRewardProgramAmount = splitTaxAmount - rewardHoldingWalletAmount;

            CarbonCC.transfer(rewardHoldingWalletAddress, rewardHoldingWalletAmount);
            emit rewardHoldingWalletAdd(msg.sender, rewardHoldingWalletAmount);

            CarbonCC.transfer(rewardProgramAddress, remainingRewardProgramAmount);
    ...
    function distributePrevCycleReward(address[] memory addresses) public checkStart onlyOwner {
    ...
    function setAddress(address _CCC, address _CarbonCClp, address _lpFeeAddress, address _sponsorRewardAddress, address _treasuryAddress, address _rewardProgramAddress, address _rewardHoldingWalletAddress) external onlyOwner{
        CarbonCC = IERC20(_CCC);
        CarbonCCLP = IERC20(_CarbonCClp);
        lpFeeAddress = _lpFeeAddress;
        sponsorRewardAddress = _sponsorRewardAddress;
    ...
    function setReward(uint256 initamount, uint256 duration, bool force) public updateReward(address(0)) onlyOwner returns (uint){
        if(!force){
    ...
    function emergencywithdrawal(IERC20 token, uint256 amount) external onlyOwner{
