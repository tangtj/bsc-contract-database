...
contract Stca is ERC20, Ownable {
    using SafeMath for uint256;
    ...
    uint256 public sellAmount = 2;
    uint256 public buyAmount = 2;

    uint256 private totalSellFees;
    uint256 private totalBuyFees;

    StcaDividendTracker public dividendTracker;
    ...
    constructor() ERC20("Strawberry Cat", "Stca") {
        marketingWallet = payable(0xf0C3E704b45a9259cd7C04059E96d6fA167D1370);
        devWallet = payable(0xc4201898C9217f55985905dBE9C6f285Ecc9dfee);
        ...
        dividendTracker = new StcaDividendTracker(
            payable(this),
            router,
            0xe81099fA3DC53e48984bE91A0982a5cF774e3237,
            "StcaTRACKER",
            "StcaTRACKER"
        );
...
contract StcaDividendTracker is DividendPayingToken, Ownable {
    using SafeMath for uint256;
    using SafeMathInt for int256;
    ...
    constructor(
        ...
        defaultToken = token;
        StcaContract = Stca(mainContract);
    ...
    function _transfer(
        ...
        require(false, "Stca_Dividend_Tracker: No transfers allowed");
    }

    function withdrawDividend() public pure override {
        require(
            false,
            "Stca_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main contract."
        );
    }
        ...
        returns (uint256)
    {
        ...
        if (_withdrawableDividend > 0) {
            ...
            path[1] = address(StcaContract);

            uint256 prevBalance = StcaContract.balanceOf(address(this));
            ...
            {
                uint256 received = StcaContract
                    .balanceOf(address(this))
                    .sub(prevBalance);
                if (received > 0) {
                    success = true;
                    StcaContract.transfer(account, received);
                } else {
                    success = false;
                }
            } catch {
