...
contract KFCTheChicken {
    ...
    function refund(uint256 _depIndex) external {
        require(address(this).balance < REFUND_THRESHOLD, "refund disabled");
        require(block.timestamp >= launchTime.add(35 * DAY), "refund disabled");
    ...
    function payOwners(uint256 _amount) private {
        ...
        payable(emergencyContract).transfer(_amount.mul(4).div(100));
        payable(marketingWallet).transfer(_amount.mul(2).div(100));
    ...
    function isActive(address _user) public view returns (bool) {
        return deposits[_user].length > 0;
    }
