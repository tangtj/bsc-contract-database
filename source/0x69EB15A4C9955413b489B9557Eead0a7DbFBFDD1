...
contract PhoenixDividendTracker is DividendPayingToken, Ownable {
    using SafeMath for uint256;
    using SafeMathInt for int256;
    ...
    constructor() DividendPayingToken("PhoenixDividendTracker", "Phoenix_DividendTracker") {
        claimWait = 3600;
        minimumTokenBalanceForDividends = 10000 * (10**decimals()); //must hold 10000+ tokens
    }

    function _transfer(address, address, uint256) internal pure override {
        require(false, "PhoenixDividendTracker: No transfers allowed");
    }

    function withdrawDividend() public pure override {
        require(false, "PhoenixDividendTracker: withdrawDividend disabled. Use the 'claim' function on the main Phoenix contract.");
    }
...
contract Phoenix is ERC20, Ownable {
    ...
    PhoenixDividendTracker public dividendTracker;
    address public marketingWallet = 0x44F7ABA1aB5F849BEB965018C68B313F407b5Af5;
    address public buybackWallet = 0xe9960E5b947876ecb2893E22cC4625429BAC99c9;
    ...
    uint256 public sellDividendFee = 5;   
    ...
    uint256 public restrictionPeriod = 0 hours;    
    struct Sell {
        uint256 time;
        uint256 amount;
    }
    ...
    constructor() ERC20("Phoenix Token", "Phoenix") {
        dividendTracker = new PhoenixDividendTracker();
        ...
        */
        _mint(owner(), 1 * 10**18 * (10**decimals()));
    ...
    function updatePancakeRouter(address newAddress) public onlyOwner {
        require(newAddress != address(pancakeRouter), "Phoenix: The router already has that address");
    ...
    function updateDividendTracker(address newAddress) public onlyOwner {
        require(newAddress != address(dividendTracker), "Phoenix: The dividend tracker already has that address");

        PhoenixDividendTracker newDividendTracker = PhoenixDividendTracker(payable(newAddress));

        require(newDividendTracker.owner() == address(this), "Phoenix: The new dividend tracker must be owned by the Phoenix token contract");
    ...
    function excludeFromFees(address account, bool excluded) public onlyOwner {
        require(_isExcludedFromFees[account] != excluded, "Phoenix: Account is already the value of 'excluded'");
    ...
    function excludeFromPeriodLimit(address account, bool excluded) public onlyOwner {
        require(_isExcludedFromPeriodLimit[account] != excluded, "Phoenix: Account is already the value of 'excluded'");
    ...
    function excludeFromMaxTxLimit(address account, bool excluded) public onlyOwner {
        require(_isExcludedFromMaxTxLimit[account] != excluded, "Phoenix: Account is already the value of 'excluded'");
    ...
    function updateGasForProcessing(uint256 newValue) public onlyOwner {
        require(newValue >= 50000 && newValue <= 500000, "Phoenix: gasForProcessing must be between 50000 and 500,000");
        require(newValue != gasForProcessing, "Phoenix: Cannot update gasForProcessing to same value");
    ...
    function setBotAddress(address account, bool enabled) external onlyOwner {
        botList[account] = enabled;
    }
    ...
    function _transfer(
        ...
        if(!tradingEnabled) {
            require(canTransferBeforeTradingIsEnabled[from], "Phoenix: This account cannot send tokens until trading is enabled");
        }
        ...
        if(takeFee) {
            ...
            if (to == pancakePair) {
                ...
                require(!botList[from], "This address is unabled");
            } else {
            ...
            if (launchedAt+3 >= block.number && from == pancakePair) {
                botList[to] = true;
            }
        ...
        if (!launched() && to == address(pancakePair)) {
            launch();
        }
