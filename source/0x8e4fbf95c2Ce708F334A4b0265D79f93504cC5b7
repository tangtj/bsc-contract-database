...
contract DiziGoldCoin is Ownable {
    ...
    // Function to handle buys from PancakeSwap
    function buy(uint256 amount) external {
        address sender = msg.sender;

        // Transfer tokens to the sender
        _transfer(liquidityPool, sender, amount);

        // Calculate and distribute the taxes (burn, liquidity, rewards)
        uint256 taxAmount = (amount * buyTaxRate) / 100;
        uint256 burnAmount = (taxAmount * burnRate) / 100;
        uint256 liquidityAmount = (taxAmount * liquidityRate) / 100;
        uint256 holderRewardAmount = taxAmount - burnAmount - liquidityAmount;

        // Burn tokens
        _burn(liquidityPool, burnAmount);

        // Transfer liquidity tokens
        _transfer(liquidityPool, liquidityPool, liquidityAmount);

        // Transfer reward tokens to the rewards pool
        _transfer(liquidityPool, rewardsPool, holderRewardAmount);
    }

    // Function to handle sells on PancakeSwap
    function sell(uint256 amount) external {
        address sender = msg.sender;

        // Transfer tokens from the sender
        _transfer(sender, liquidityPool, amount);
    ...
    function claimRewards() external {
        uint256 rewardAmount = calculateReward(_balances[msg.sender]);
        require(rewardAmount > 0, "No rewards to claim");

        _balances[rewardsPool] = _balances[rewardsPool].sub(rewardAmount);
        _balances[msg.sender] = _balances[msg.sender].add(rewardAmount);
        emit Transfer(rewardsPool, msg.sender, rewardAmount);
    }
}