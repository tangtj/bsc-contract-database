...
contract CuexCapital is Context, IERC20, Ownable {
    ...
    string private _name = "CUEX Capital";
    string private _symbol = "CCAP";
    uint8 private _decimals = 18;

    address payable public _TreasuryWallet = 0x809341c1c1cbA2D6ACCF8188BCA463A119443D44;
    ...
    //10% tax
    uint256 public _refSplit = 0; 
    uint256 public _autoLiquiditySplit = 5000; 
    uint256 public _treasurySplit = 5000;  

    uint256 public _liquidityAT;
    uint256 public _treasuryAT;
                                    
    uint256 public _minTokensForLiquidity = 30000000000000000000000000;                                  
    ...
    function _reflectFee(uint256 rFee, uint256 tFee) private {
        ...
        uint256 newTFee = tFee.div(100).mul(_refSplit);
        uint256 newRFee = rFee.div(100).mul(_refSplit);
    ...
    function _setRefSplit(uint256 refSplit) external onlyOwner() {

        _refSplit = refSplit;
    }

    function _setAutoLiquiditySplit(uint256 autoLiquiditySplit) external onlyOwner() {

        _autoLiquiditySplit = autoLiquiditySplit;
    }

    function _setTreasurySplit(uint256 treasurySplit) external onlyOwner() {
    ...
    function setTreasuryallet(address payable treasuryWallet) public onlyOwner() {

        _TreasuryWallet = treasuryWallet;
    }

    function _execLiquidity() public lockLiquidities {
        
        uint256 amount = balanceOf(address(this));
        
        if(amount >= _minTokensForLiquidity && _lockLiquiditiesEnabled == true && amount > 0) {

            uint256 liquidityVal = _findPercent(amount, _autoLiquiditySplit);
            uint256 treasuryVal = amount.sub(liquidityVal);
    
            _execLiquidity(liquidityVal);
            _execTreasury(treasuryVal);
        }
    }

    function _execLiquidity(uint256 amount) private {
        ...
        swapTokensForEth(bnbHalf, address(this)); 

        uint256 bnbNewBalance = address(this).balance.sub(bnbBalance); 

        addLiquidity(tokenHalf, bnbNewBalance); 
    }

    function _execTreasury(uint256 amount) private {
        ...
        _TreasuryWallet.transfer(bnbNewBalance); 

        _treasuryAT += bnbNewBalance;
    }
    ...
    function treasuryFallBack() public onlyOwner() {

         if(balanceOf(address(this)) > 0) {
            this.transfer(_TreasuryWallet, balanceOf(address(this)));
         }
