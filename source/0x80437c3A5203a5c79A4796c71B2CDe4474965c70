...
contract bobx is Context, IERC20, Ownable {
    ...
    string private _name = "box X";
    string private _symbol = "$box X";
    uint8 private _decimals = 9;

    address payable public MarketingWallet = payable(0xabfb6B1551eAEcA7B98c66cc230708f6D317B62b);
    address payable public DevWallet = payable(0xabfb6B1551eAEcA7B98c66cc230708f6D317B62b);
   ...
   function swapAndLiquify(uint256 tAmount) private lockTheSwap {
    ...
    if (_totalDistributionShares > 0) {
        tokensForLP = tAmount.mul(_liquidityShare).div(_totalDistributionShares).div(2);
        tokensForSwap = tAmount.sub(tokensForLP);
    }
    ...
    if (_liquidityShare > 0) {
        totalETHFee = _totalDistributionShares.sub(_liquidityShare.div(2));
    }
    ...
    if (totalETHFee > 0) {
        amountETHLiquidity = amountReceived.mul(_liquidityShare).div(totalETHFee).div(2);
        amountETHDev = amountReceived.mul(_devShare).div(totalETHFee);
        amountETHMarketing = amountReceived.sub(amountETHLiquidity).sub(amountETHDev);
    }
