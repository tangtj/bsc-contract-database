...
contract GenMarket is ERC1155Receiver {
    ...
    function buy(uint256 _id, uint256 _amount) external payable {
        require(!enableUsdt, "GenMarket: Access Denied");
    ...
    // to transfer unsold tickets to creator wallet when pool ends
...
contract GenMarketFactory is IGenMarketFactory {
    ...
    function createGenMarket(
        address _genTicket,
        address _usdt,
        // Prices could be in Native(18 dec) or USDT(6 dec or check usdt's decimals) 
        uint256[] memory _prices,
        // Number of each ticket type being sold
        uint256[] memory _numTickets,
        uint256[] memory _purchaseLimits
        ...
        GenMarket gm = new GenMarket(_genTicket, _usdt, _prices, _numTickets, _purchaseLimits, this, msg.sender);
        // Populate mapping
