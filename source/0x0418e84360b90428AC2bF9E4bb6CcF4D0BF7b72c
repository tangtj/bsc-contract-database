...
contract MasterChefNFTPool is Ownable {
    ...
    function setInvReward(uint8 _invReward) public onlyOwner {
        invRewardNum = _invReward;
    }
    ...
    function add(uint256 _allocPoint, IERC20 _token, bool _withUpdate) public onlyOwner {
        require(address(_token) != address(0), "_lpToken is the zero address");
        if (_withUpdate) {
            massUpdatePools();
        }
    ...
    function deposit(uint256 _amount,uint256 _pid) public notPause {
    ...
    function depositDex(uint256 _pid, uint256 _amount, address _user) private {
        ...
        if (_amount > 0) {
            ...
            _balances[_pid][_user] += _amount;
            pool.totalAmount = pool.totalAmount.add(_amount);
            user.amount = user.amount.add(_amount);
            poolTotalAmount[_pid] = poolTotalAmount[_pid].add(_amount);
        }
        user.rewardDebt = user.amount.mul(pool.accDexPerShare).div(1e12);
        emit Deposit(_user, _pid, _amount);
    }

    function tokensOf(address owner,uint256 _pid)
    ...
    function withdraw(uint _pid,uint256 _amount) public notPause {
    ...
    function withdrawDex(uint256 _pid, uint256 _amount, address _user) private {
        ...
        if (_amount > 0) {
            ...
            poolTotalAmount[_pid] = poolTotalAmount[_pid].sub(_amount);
        }
    ...
    function safeDexTransfer(address _to, uint256 _amount) internal returns(uint) {
        ...
        if (_amount > dexBal) {
            ...
            amount = _amount;
            uint256 myInvReward = invRewardAmount[_to];
            if(myInvReward > 0 && dexBal - amount > myInvReward){
                IERC20(dex).transfer(_to, myInvReward);
                invRewardAmount[_to] = 0;
            }
        }
