...
contract DTToken is ERC20, ERC20Pausable, CoinFactory, Blacklist {
    ...
    function transfer(address to, uint256 value) public whenNotPaused returns (bool) {
        ...
        require(!isBlacklist(msg.sender), "DTToken: caller in blacklist can't transfer");

        require(!isBlacklist(to), "DTToken: not allow to transfer to recipient address in blacklist");
    ...
    function transferFrom(address from, address to, uint256 value) public whenNotPaused returns (bool) {
        ...
        require(!isBlacklist(msg.sender), "DTToken: caller in blacklist can't transferFrom");

        require(!isBlacklist(from), "DTToken: from in blacklist can't transfer");
