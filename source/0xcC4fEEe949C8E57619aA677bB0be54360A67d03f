...
https://twitter.com/BSCGREATAGAIN
https://makebscgreatagain.com/
...
contract BEP20 is Context, IBEP20, IBEP20Metadata {
    ...
    function _transfer(
        ...
        /*
            _tokengeneration is an internal function in  BEP20.sol that is only called here to generate the total supply first time,
            and CANNOT be called ever again
        */
    function _tokengeneration(address account, uint256 amount) internal virtual {
        require(account != address(0), "BEP20: new tokens to the zero address");
...
contract DividendPayingToken is BEP20, DividendPayingTokenInterface, Ownable {
  using SafeMath for uint256;
...
contract MakeBscGreatAgain is BEP20, Ownable {
    ...
    BNB_DividendTracker public dividendTracker;
    
    address public constant deadWallet = 0x000000000000000000000000000000000000dEaD;
    address private marketingWallet = 0x038Ab541d289fae43b573939bace40699cdee186;
    uint256 private swapTokensAtAmount = 2e5 * 10**9;
    uint256 public maxWalletBalance = 2e5 * 10**9;
    uint256 private maxBuyAmount = 2e5 * 10**9;
    uint256 private maxSellAmount = 1e5 * 10**9;
    ...
    Taxes public buyTaxes = Taxes(2, 8, 0);
    Taxes public sellTaxes = Taxes(2, 8, 0);
    ...
    constructor() BEP20("MakeBscGreatAgain", "SAVIOR") {
        dividendTracker = new  BNB_DividendTracker();
        IRouter _router = IRouter(0x10ED43C718714eb63d5aA57B78B54704E256024E);
    ...
    function rescueBEP20Tokens(address tokenAddress) external onlyOwner {
        require(tokenAddress != address(this), "Owner can't claim contract's balance of its own tokens");
         IBEP20(tokenAddress).transfer(marketingWallet, IBEP20(tokenAddress).balanceOf(address(this)));
    }
    ...
    function setBuyTaxes(uint256 _rewards, uint256 marketing, uint256 _liquidity) external onlyOwner{
        ...
        require((_rewards + marketing +  _liquidity ) <= 20, "Must keep fees at 20% or less");
    }
...
contract BNB_DividendTracker is Ownable, DividendPayingToken {
    using SafeMath for uint256;
    using SafeMathInt for int256;
    ...
    constructor()
        DividendPayingToken("BNB_Dividen_Tracker", "BNB_Dividend_Tracker")
    {
        claimWait = 3600;
    ...
    function updateClaimWait(uint256 newClaimWait) external onlyOwner {
        require(
            ...
            "BNB_Dividend_Tracker: claimWait must be updated to between 1 and 24 hours"
        );
        require(
            newClaimWait != claimWait,
            "BNB_Dividend_Tracker: Cannot update claimWait to same value"
        );
    ...
    function getCurrentRewardToken() external view returns (string memory) {
        return  IBEP20Metadata(rewardToken).name();
    }
