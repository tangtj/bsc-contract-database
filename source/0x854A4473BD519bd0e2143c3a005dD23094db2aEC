...
// File: token/onft/IONFT721Core.sol
...
// File: token/onft/ONFT721Core.sol
...
contract OmniWonders is ONFT721 {
    uint public nextMintId;
    uint public maxMintId;
    uint public mintPrice = 0.00001 ether; // Your price
    uint256 public totalSupply;

    string public Base_URI;

    bool public Public_Sale_State = true;

    uint public gasLzReceive = 350000;

    mapping (address => uint) public addressToMintCount;

    constructor(address _layerZerolzEndpoint, string memory _baseTokenURI, uint _startMintId, uint _endMintId) ONFT721("Omni Wonders", "OWOW", _layerZerolzEndpoint) {
        nextMintId = _startMintId;
        maxMintId = _endMintId;
        setBaseURI(_baseTokenURI);
    }

    function mint(uint256 numberOfTokens) payable public onlyAccounts  {
        require(Public_Sale_State, "Sale hasn't started");
        require(msg.value >= mintPrice * numberOfTokens, "Insufficient balance to mint tokens");
    ...
    function setFee(uint _newEthAmount) public onlyOwner {
        mintPrice = _newEthAmount;
    }
    ...
    function _withdraw(address payable _address, uint256 _amount) private {
    ...
    function traverseChains(uint16 _chainId, uint tokenId) public payable {
        ...
        require(trustedRemoteLookup[_chainId].length > 0, "You cannot make your ONFT travel to this chain right now.");
        ...
        require(msg.value >= messageFee, "Insufficient value to cover messageFee");
    ...
    function setGasLzReceive(uint newVal) external onlyOwner {
        gasLzReceive = newVal;
    }
}