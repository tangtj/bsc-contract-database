...
// File: https://github.com/Uniswap/solidity-lib/blob/master/contracts/libraries/BitMath.sol
...
// File: https://github.com/Uniswap/solidity-lib/blob/master/contracts/libraries/Babylonian.sol
...
// File: https://github.com/Uniswap/solidity-lib/blob/master/contracts/libraries/FullMath.sol
...
// File: https://github.com/Uniswap/solidity-lib/blob/master/contracts/libraries/FixedPoint.sol
...
// File: PcsSnapshotTwapOracle.sol
...
contract PcsSnapshotTwapOracle is Whitelist{
    ...
    constructor(address factory_, uint32 periodSize_) Ownable() public {
        require(periodSize_ > 1 minutes, 'periodSize must be greater than 60 seconds');
        factory = factory_;
    ...
    function consultAmountsOut(uint amountIn, address[] memory path) public view returns (uint[] memory amounts) {
        require(path.length >= 2, 'PancakeLibrary: INVALID_PATH');
    ...
    function consult(address tokenIn, uint amountIn, address tokenOut) public view returns (uint amountOut) {
        ...
        require(observation.timestamp > 0 , 'PcsPeriodicOracle: PAIR_UNINITIALIZED'); 
    ...
    function update(address tokenA, address tokenB) private returns (bool success, address pair, uint32 timestamp) {
        ...
        if (observation.timestamp == 0){
            ...
            observation.price0Cumulative =  IPancakePair(pair).price0CumulativeLast();
            observation.price1Cumulative =  IPancakePair(pair).price1CumulativeLast();
        }

        (uint price0Cumulative, uint price1Cumulative, uint32 blockTimestamp) = PancakeOracleLibrary.currentCumulativePrices(pair);
...
library PancakeLibrary {
    ...
    function pairFor(address factory, address tokenA, address tokenB) internal view returns (address pair) {
        IPancakeFactory _factory = IPancakeFactory(factory);
