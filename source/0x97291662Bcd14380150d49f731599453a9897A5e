...
contract AlphanetStaking is ReentrancyGuard, Pausable {
    ...
    string [] levels = ["T0","T1","T2","T3","T4","T5","T6","T7","T8","T9","T10"];
    ...
    function initLevel() internal {
        ...
        _ratesLevel[levels[4]] = RateLevel({min:100000,max:150000,weight:200});
        _ratesLevel[levels[5]] = RateLevel({min:150000,max:200000,weight:225});
        _ratesLevel[levels[6]] = RateLevel({min:200000,max:250000,weight:250});
        _ratesLevel[levels[7]] = RateLevel({min:250000,max:300000,weight:275});
        _ratesLevel[levels[8]] = RateLevel({min:300000,max:350000,weight:300});
        _ratesLevel[levels[9]] = RateLevel({min:350000,max:400000,weight:325});
        _ratesLevel[levels[10]] = RateLevel({min:400000,max:999999999,weight:350});
    }
    ...
    function stake(uint256 amount,uint256 userId) external nonReentrant notPaused returns (bool){
        ...
        _idBalances[userId] =_idBalances[userId].add(amount);
