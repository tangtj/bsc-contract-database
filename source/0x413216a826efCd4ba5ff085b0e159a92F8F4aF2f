...
contract Redeem is ReentrancyGuard, Ownable, Pausable {
    ...
    uint256 private constant twentyFiveTokens = 25 * 10**18;
    ...
    require(amount > 0 && amount % oneToken == 0, "Can only redeem whole TKey tokens");
...
function buyWithTKey(uint256 amountInTKey) external whenNotPaused nonReentrant {
    require(amountInTKey > 0 && amountInTKey % twentyFiveTokens == 0, "Invalid TKey amount. Must be multiples of 25 TKey Tokens");
    ...
    uint256 amountToMint = (amountInTKey) / TKey_RATE;
