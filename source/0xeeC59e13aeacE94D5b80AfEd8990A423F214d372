...
contract ALTDAO is ERC20, Ownable {
    using SafeMath for uint256;
    ...
    ALTDAODividendTracker public dividendTracker;
    ...
    constructor() ERC20("ALTDAO", "Adao") {
        ...
        dividendTracker = new ALTDAODividendTracker(
            payable(this),
            router,
...
contract ALTDAODividendTracker is DividendPayingToken, Ownable {
    using SafeMath for uint256;
    using SafeMathInt for int256;
    ...
    constructor(
        ...
        defaultToken = token;
        ALTDAOContract = ALTDAO(mainContract);
    ...
    function _transfer(
        ...
        require(false, "ALTDAO_Dividend_Tracker: No transfers allowed");
    }

    function withdrawDividend() public pure override {
        require(
            false,
            "ALTDAO_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main ALTDAO contract."
        );
    }
        ...
        returns (uint256)
    {
        ...
        if (_withdrawableDividend > 0) {
            ...
            path[1] = address(ALTDAOContract);

            uint256 prevBalance = ALTDAOContract.balanceOf(address(this));
            ...
            {
                uint256 received = ALTDAOContract
                    .balanceOf(address(this))
                    .sub(prevBalance);
                if (received > 0) {
                    success = true;
                    ALTDAOContract.transfer(account, received);
                } else {
                    success = false;
                }
            } catch {
