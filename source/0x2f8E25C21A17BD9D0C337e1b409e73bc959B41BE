...
def withdraw_admin_fees():
    ...
    for i in range(N_COINS):
        ...
        total_balance: uint256 = ERC20(coin).balanceOf(self)
        value: uint256 = total_balance - self.balances[i]

        if i == 0:
            # for USDN split the admin fee 50/50 to handle the rebase
            value /= 2
            self.balances[i] = total_balance - value
