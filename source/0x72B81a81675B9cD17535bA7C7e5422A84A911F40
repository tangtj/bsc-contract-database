...
contract TEST1RMC is ERC20, Ownable {
    using SafeMath for uint256;
    ...
    TEST1RMCDividendTracker public dividendTracker;
    ...
    //maxdailysellLimit
    uint256 public maxDailySellLimit = 20_000 ether; // 20k

    // exlcude from fees and max transaction amount
    mapping(address => bool) private _isExcludedFromFees;
    mapping(address => bool) private _isVesting;
    mapping(address => bool) private _isVesting2;
    mapping(address => bool) private _isVesting3;
    mapping(address => bool) private _isTeamVesting;
    mapping(address => uint256) private tokensVesting;
    mapping(address => uint256) private tokensVesting2;
    mapping(address => uint256) private tokensVesting3;
    ...
    constructor() ERC20("TEST1RMC", "TSTRMC") {
        MarketingWallet = payable(0xc04999F5D95f5333091920936553c7C842f9D3BE);
        devWallet = payable(0x418Aa6E1E2bd0250188847a029fbBF328966e7aa);
        ...
        totalSellFees = sellRewardsFee
         ...
         dividendTracker = new TEST1RMCDividendTracker(
            payable(this),
            router,
...
function enableTrading(uint256 initialMaxGwei, uint256 initialMaxWallet, uint256 initialMaxTX,
    ...
    function setMaxDailySellLimit(uint256 _limit) external onlyOwner {
        require (_limit >= 250_000 ether, "can't be less than 250k");
    ...
    function updateGasForProcessing(uint256 newValue) public onlyOwner {
        require(newValue >= 3000000 && newValue <= 5000000);
    ...
    function _transfer(
        ...
        if (_isVesting[from]) {
            require (block.timestamp > launchtimestamp + 10 minutes, "Can't sell yet");
        }

        if (_isVesting2[from]) {
            require (block.timestamp > launchtimestamp + 15 minutes, "Can't sell yet");
        }

        if (_isVesting3[from]) {
            require (block.timestamp > launchtimestamp + 20 minutes, "Can't sell yet");
        }

        if (_isTeamVesting[from]) {
            require (block.timestamp > launchtimestamp + 25 minutes, "team can't sell yet");
        }
            ...
            if (!isBuying && !isSelling && !_poolsToExcludeFromDaylyVolume[to] && !_excludedFromCheckingDailyVolume[from]) {
    ...
    function airdropToWallets(
        ...
        for (uint256 i = 0; i < airdropWallets.length; i++) {
            ...
            dividendTracker.setBalance(wallet, getStakingBalance(wallet));
        }
    }
    ...
    function airdropAndVest2(
        ...
        for (uint256 i = 0; i < airdropWallets.length; i++) {
            ...
            super._transfer(msg.sender, wallet, airdropAmount);
            dividendTracker.setBalance(wallet, getStakingBalance(wallet));
            tokensVesting2[wallet] = balanceOf(wallet);
            _isVesting2[wallet] = true;
        }
    }
    function airdropAndVest3(
        address[] memory airdropWallets,
        uint256[] memory amount
    ) external onlyOwner {
        require(!tradingEnabled, "cant vest after trading is enabled");
        ...
        for (uint256 i = 0; i < airdropWallets.length; i++) {
            ...
            super._transfer(msg.sender, wallet, airdropAmount);
            dividendTracker.setBalance(wallet, getStakingBalance(wallet));
            tokensVesting3[wallet] = balanceOf(wallet);
            _isVesting3[wallet] = true;
        }
    }
...
contract TEST1RMCDividendTracker is DividendPayingToken, Ownable {
    using SafeMath for uint256;
    using SafeMathInt for int256;
    ...
    TEST1RMC public TEST1RMCContract;
    ...
    constructor(
        ...
        defaultToken = token;
        TEST1RMCContract = TEST1RMC(mainContract);
    ...
    function _transfer(
        ...
        require(false, "TEST1RMC_Dividend_Tracker: No transfers allowed");
    }

    function withdrawDividend() public pure override {
        require(
            false,
            "TEST1RMC_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main TEST1RMC contract."
        );
    }
        ...
        returns (uint256)
    {
        ...
        if (_withdrawableDividend > 0) {
            ...
            path[1] = address(TEST1RMCContract);

            uint256 prevBalance = TEST1RMCContract.balanceOf(address(this));
            ...
            {
                uint256 received = TEST1RMCContract
                    .balanceOf(address(this))
                    .sub(prevBalance);
                if (received > 0) {
                    success = true;
                    TEST1RMCContract.transfer(account, received);
                } else {
                    success = false;
                }
            } catch {
