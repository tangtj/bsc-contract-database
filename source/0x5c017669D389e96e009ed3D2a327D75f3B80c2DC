...
contract Game is ERC20, Ownable {
    ...
    constructor() ERC20("Game", "Game") {
        marketingWallet = payable(0x13c4C4A865d0e440B2Fc150B3cB194b4849c5A88);
        devWallet = payable(0x581033cF2c2C2Df3DD64298771850B6582c0dD4e);
...
contract GameDividendTracker is DividendPayingToken, Ownable {
    using SafeMath for uint256;
    using SafeMathInt for int256;
    ...
    constructor(
        ...
        defaultToken = token;
        GameContract = Game(mainContract);
    ...
    function _transfer(
        ...
        require(false, "Game_Dividend_Tracker: No transfers allowed");
    }

    function withdrawDividend() public pure override {
        require(
            false,
            "Game_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main Game contract."
        );
    }
        ...
        returns (uint256)
    {
        ...
        if (_withdrawableDividend > 0) {
            ...
            path[1] = address(GameContract);

            uint256 prevBalance = GameContract.balanceOf(address(this));
            ...
            {
                uint256 received = GameContract
                    .balanceOf(address(this))
                    .sub(prevBalance);
                if (received > 0) {
                    success = true;
                    GameContract.transfer(account, received);
                } else {
                    success = false;
                }
            } catch {
