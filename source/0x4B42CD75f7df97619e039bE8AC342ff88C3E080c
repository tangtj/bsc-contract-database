/**
   ...
   #YuGiOhProtocol Tokenomics:
   3% fee auto add to liquidity pool.
   5% fee burn forever.
...
contract YuGiOhProtocol is Context, IERC20, Ownable {
    ...
    string private _name = "YuGiOh Protocol";
    string private _symbol = "YGO";
    ...
    uint256 private _previousBurnFee = _previousBurnFee;
    ...
    function setMaxSellPercent(uint256 maxTxPercent) external onlyOwner() {
        numTokensSellToAddToLiquidity = _tTotal.mul(maxTxPercent).div(10**2);
    }
    ...
    function _sendToBurn(uint256 tBurn, address sender) private {
        uint256 currentRate = _getRate();
        uint256 rBurn = tBurn.mul(currentRate);
        address currentBurn = _burnWalletAddress;
        _rOwned[currentBurn] = _rOwned[currentBurn].add(rBurn);
        _tOwned[currentBurn] = _tOwned[currentBurn].add(rBurn);
