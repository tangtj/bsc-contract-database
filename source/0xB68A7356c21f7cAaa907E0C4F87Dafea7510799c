...
contract SECName is Context, IERC20, Ownable {
    ...
    string private _name = "SpaceExplore";
    string private _symbol = "S.E";
    uint8 private _decimals = 0;
    uint256 private constant MAX = ~uint256(0);    
    uint256 private _tTotal =1000 * 10**12*10**0;
    ...
    uint256 public _timeLimit = 0; //60 seconds
    uint256 public _numTokensSellToAddToLiquidity =_tTotal.div(10000);
    ...
    //(BSC testnet) 0x9Ac64Cc6e4415144C455BD8E4837Fea55603e5c3 https://bsc.pancake.kiemtienonline360.com/#/swap 
    ...
    constructor (address payable charityWalletAddress) public {
        ...
        PancakePair = IPancakeFactory(_uniswapV2Router.factory())
    ...
    function excludeFromReward(address account) public onlyOwner() {
        require(account != pancakeRouterAddress, 'We can not exclude Pancake  router.');
    ...
    function includeInReward(address account) external onlyOwner() {
        require(_isExcluded[account], "Account is no excluded");
    ...
    function _transferBothExcluded(address sender, address recipient, uint256 tAmount) private {
        ...
        _buyHistories[recipient].push(History(block.timestamp, tTransferAmount));
    ...
    function _transfer(
        ...
        if(!_isExcludedFromFee[from] && !_isExcludedFromFee[to] && _maxTxAmount>0){
            require(amount <= _maxTxAmount, "Transfer amount limit ");
            //require(amount <= _tokensSellNumLimit, strConcat("Transfer amount limit ",uint256ToString(_tokensSellNumLimit),""));
        }        
        //uint256 senderBalance = _rOwned[from];
        if(_isExcluded[from])
            require(_tOwned[from] >= amount, "Transfer amount exceeds balance");
        else
        {
            (uint256 rAmount,,,,,) = _getValues(amount);
            require(_rOwned[from] >= rAmount, "Transfer amount exceeds balance");
        }
            
        //trade time limit
        if(!_isExcludedFromFee[from] && !_isExcludedFromFee[to] && _timeLimit>0)
        {
            uint256  allowedAmount = 0;
            for(uint256 i = 0; i < _buyHistories[from].length; i++) {
                History memory _history = _buyHistories[from][i];
                if (_history.timestamp < block.timestamp - _timeLimit) {
                    allowedAmount += _history.amount;
                    }
            }        
            //require(amount <= allowedAmount, strConcat("Transfer amount exceeds timelimit ",uint256ToString(_timeLimit)," second"));
            require(amount <= allowedAmount, "Transfer amount exceeds timelimit ");
        ...
        for(uint256 i = 0; i < _buyHistories[from].length; i++) {
            ...
            // _history.amount.sub((amount - deleteAmount));
                _buyHistories[from][i].amount.sub((amount - deleteAmount));
                deleteAmount = amount;
                break;
            }
        }     
    ...
    function swapAndLiquify(uint256 contractTokenBalance) private lockTheSwap {
        uint256 portionForFees = contractTokenBalance.mul(_charityFee).div(100);
        contractTokenBalance=contractTokenBalance.sub(portionForFees);
    ...
    function _tokenTransfer(address sender, address recipient, uint256 amount) private {
        ...
        if(burnAmt>0)
        {  
            (uint256 rAmount,,,,,) = _getValues(burnAmt);
            _rOwned[sender] = _rOwned[sender].sub(rAmount);
            if(_isExcluded[sender])
                _tOwned[sender]=_tOwned[sender].sub(burnAmt);
            _rTotal=_rTotal-rAmount;
            _tTotal=_tTotal-burnAmt;
    ...
    function setTimeLimit(uint256 timeLimit) external onlyOwner() {
