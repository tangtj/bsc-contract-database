...
// File: contracts/Stakes/AdminMint.sol
...
contract ADMINMINT is Ownable {
    ...
    function mint(uint8 _type) public onlyOwner returns(uint) {
        require(_type >= 1 && _type <= maxTier, "Invalid NFT type");
        return nftCon.safeMint(msg.sender, _type);
    }
