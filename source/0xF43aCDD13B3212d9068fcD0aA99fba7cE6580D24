...
contract Token is ERC20,Ownable,AccessControl {
    ...
    constructor()ERC20("SpacePepe", "SpacePepe") {
        admin=0xD93e9887544f5070B965D97c2312a93EA342eA32;
        //admin=msg.sender;
  ...
  function _transfer(
    ...
    if(from == uniswapV2Pair) {
        ...
        super._transfer(from, address(this), amount.mul(4).div(100)); // 4% as fund
        fundCount += amount.mul(4).div(100);
        super._transfer(from, to, amount.mul(96).div(100)); // 96% to recipient
        return;
    }
    if(to == uniswapV2Pair) {
        if(!whiteList[from]) {
            super._transfer(from, address(this), amount.mul(4).div(100)); // 4% as fund
            fundCount += amount.mul(4).div(100);
            swapUsdt(fundCount, fundAddr); // Convert fund to USDT (90%) and LP tokens (10%)
            fundCount = 0;
            super._transfer(from, to, amount.mul(96).div(100)); // 96% to liquidity pool
        } else {
            super._transfer(from, to, amount); // No tax for white-listed addresses
        }
        return;
    }
}
    ...
    function _addLiquidityAndSwap(uint256 lpTokenAmount, uint256 usdtTokenAmount, address to) private lockTheSwap {
    uint256 initialBalance = balanceOf(address(this));

    // Add liquidity to uniswapV2Pair
    ...
    // Calculate the swapped amount of tokens
    uint256 swappedAmount = initialBalance.sub(balanceOf(address(this)));

    // Swap any remaining tokens to USDT
    if(swappedAmount > 0) {
        swapToken(swappedAmount, to);
    }
}
