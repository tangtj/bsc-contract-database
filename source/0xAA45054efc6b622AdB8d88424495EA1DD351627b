...
contract ERC1155Base is HasSecondarySaleFees, Ownable, ERC1155Metadata_URI, HasContractURI, ERC1155 {
    ...
    function _mint(uint256 _id, address recipient, Fee[] memory _fees, uint256 _supply, string memory _uri) internal {
        ...
        balances[_id][recipient] = _supply;
        _setTokenURI(_id, _uri);

        // Transfer event with mint semantic
        emit TransferSingle(recipient, address(0x0), recipient, _id, _supply);
        emit URI(_uri, _id);
    }

    function setSecondaryFees(uint256 _id, Fee[] memory _fees) public onlyOwner {
        require(fees[_id].length == 0, "Fee already set!");
        require(_fees.length > 0, "Fee cannot be empty!");
 ...
 * @title CaramelNFT
 */
contract CaramelNFT is Ownable, SignerRole, ERC1155Base {
    string public name;
    string public symbol;

    modifier onlyOwnerOrSigner {
        require(msg.sender == owner() || isSigner(msg.sender), "ONLY_OWNER_SIGNER");
        _;
    }
    ...
    function mint(uint256 tokenId, address recipient, Fee[] memory fees, uint256 supply, string memory uri) public onlyOwnerOrSigner {
        _mint(tokenId, recipient, fees, supply, uri);
    }

    function mint(address minter, address recipient, uint256 tokenId, uint8 v, bytes32 r, bytes32 s, Fee[] memory fees, uint256 supply, string memory uri) public {
