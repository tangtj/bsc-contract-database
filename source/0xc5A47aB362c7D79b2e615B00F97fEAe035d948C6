...
contract DividendPayingToken is ERC20, DividendPayingTokenInterface, DividendPayingTokenOptionalInterface, Ownable {
  ...
  function updateDividendrouter(address newAddress) external onlyOwner {
        require(newAddress != address(router), "YE2024: The router already has that address");
  ...
  function setRewardToken(address holder, address rewardTokenAddress, address ammContractAddress) external onlyOwner {
    if(userHasCustomRewardToken[holder]){
...
contract YE2024 is ERC20, Ownable {
    using SafeMath for uint256;
    ...
    YE2024DividendTracker public dividendTracker;
    ...
    uint256 public sellOperationFee = 0;
    ...
    event AntiDumpUpdated(uint256 maxSell, uint256 increaseFactor);
    event FeesUpdated();
    ...
    constructor() ERC20("YE2024", "YE2024", 9) {
 
        dividendTracker = new YE2024DividendTracker();
 
        liquidityWallet = owner();
        operationsWallet = 0x94D1007fa574FC570c1c6b52eE1926d7709C1028;
    ...
    function setWhiteListAMM(address ammAddress, bool isWhiteListed) external onlyOwner {
      require(isContract(ammAddress), "YE2024: setWhiteListAMM:: AMM is a wallet, not a contract");
    ...
    function updateDividendTracker(address newAddress) public onlyOwner {
        require(newAddress != address(dividendTracker), "YE2024: The dividend tracker already has that address");
 
        YE2024DividendTracker newDividendTracker = YE2024DividendTracker(payable(newAddress));
 
        require(newDividendTracker.owner() == address(this), "YE2024: The new dividend tracker must be owned by the YE2024 token contract");
    ...
    function setIsBot(address wallet, bool status) external onlyOwner {
        require(!automatedMarketMakerPairs[wallet], "Pairs are excluded from this");
        _isBot[wallet] = status;
        emit BlacklistedWallet(wallet, status);
    }
 
    function setAntiDump(uint256 _maxSellPercent, uint256 _sellIncreaseFactor) external onlyOwner{
        require(_maxSellPercent >= 1_000_000, "Min amount is 1,000,000");
        require(_sellIncreaseFactor <= 10, "Max Sell increase factor is 10%");
        maxSellPercentage = _maxSellPercent;
        sellIncreaseFactor = _sellIncreaseFactor;
        emit AntiDumpUpdated(_maxSellPercent, _sellIncreaseFactor);
    }
    ...
    function updateLiquidityWallet(address newLiquidityWallet) external onlyOwner {
        require(newLiquidityWallet != liquidityWallet, "YE2024: The liquidity wallet is already this address");
    ...
    function updateOperationsWallet(address newOperationsWallet) external onlyOwner {
        require(newOperationsWallet != operationsWallet, "YE2024: The operations wallet is already this address");
    ...
    function updateFees(uint256 _rewards, uint256 _liquidity, uint256 _operation) external onlyOwner {
        require(_rewards + _liquidity + _operation <= 25, "Max Fee is 25%");
        rewardsFee = _rewards;
    ...
    function updateGasForProcessing(uint256 newValue) external onlyOwner {
        require(newValue >= 200000 && newValue <= 500000, "YE2024: gasForProcessing must be between 200,000 and 500,000");
        require(newValue != gasForProcessing, "YE2024: Cannot update gasForProcessing to same value");
    ...
    function setRewardToken(address rewardTokenAddress) public returns (bool) {
        require(isContract(rewardTokenAddress), "YE2024: setRewardToken:: Address is a wallet, not a contract.");
        require(rewardTokenAddress != address(this), "YE2024: setRewardToken:: Cannot set reward token as this token due to Router limitations.");
        require(!isIgnoredToken(rewardTokenAddress), "YE2024: setRewardToken:: Reward Token is ignored from being used as rewards.");
    ...
    function setRewardTokenWithCustomAMM(address rewardTokenAddress, address ammContractAddress) public returns (bool) {
        require(isContract(rewardTokenAddress), "YE2024: setRewardToken:: Address is a wallet, not a contract.");
        require(ammContractAddress != address(router), "YE2024: setRewardToken:: Use setRewardToken to use default Router");
        require(rewardTokenAddress != address(this), "YE2024: setRewardToken:: Cannot set reward token as this token due to Router limitations.");
        require(!isIgnoredToken(rewardTokenAddress), "YE2024: setRewardToken:: Reward Token is ignored from being used as rewards.");
        require(isAMMWhitelisted(ammContractAddress), "YE2024: setRewardToken:: AMM is not whitelisted!");
    ...
    function buyBackTokensWithNoFees() external payable returns (bool) {
        ...
        require(userRawBNBDividends >= holderBNBUsedForBuyBacks[msg.sender].add(msg.value), "YE2024: buyBackTokensWithNoFees:: Cannot Spend more than earned.");
        ...
        router.swapExactETHForTokensSupportingFeeOnTransferTokens{value: ethAmount}( //try to swap for tokens, if it fails (bad contract, or whatever other reason, send BNB)
    ...
    function _setAutomatedMarketMakerPair(address _pair, bool value) private {
        require(automatedMarketMakerPairs[_pair] != value, "YE2024: Automated market maker pair is already set to that value");
    ...
    function _transfer(
        ...
        require(!_isBot[to] || !_isBot[from], "YE2024: To/from address is ignored");
   ...
   function swapAndLiquify(uint256 tokens) private{
       // Split the contract balance into halves
        ...
        uint256 bnbToAddLiquidityWith = unitBalance * sellLiquidityFee;
	...
	function recoverContractBNB(uint256 amount) public onlyOwner{
...
contract YE2024DividendTracker is DividendPayingToken {
    using SafeMath for uint256;
    using SafeMathInt for int256;
    ...
    constructor() DividendPayingToken("YE2024_Dividend_Tracker", "YE2024_Dividend_Tracker", 9) {
        claimWait = 3600;
        minimumTokenBalanceForDividends = 30_000 * (10**9); //must hold 30,000+ tokens to get rewards
    }
 
    function _transfer(address, address, uint256) pure internal override {
        require(false, "YE2024_Dividend_Tracker: No transfers allowed");
    }
 
    function withdrawDividend() pure public override {
        require(false, "YE2024_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main YE2024 contract.");
    }
    ...
    function updateClaimWait(uint256 newClaimWait) external onlyOwner {
        require(newClaimWait >= 3600 && newClaimWait <= 86400, "YE2024_Dividend_Tracker: claimWait must be updated to between 1 and 24 hours");
        require(newClaimWait != claimWait, "YE2024_Dividend_Tracker: Cannot update claimWait to same value");
