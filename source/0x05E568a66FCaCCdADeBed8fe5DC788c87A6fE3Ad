...
contract Withdraw is ERC20, SafeOwnable {
    using SafeMath for uint256;
    ...
    uint256 private DestroyRate=10;
    uint256 private WithdrawMax=1000*10**18;

    event withdrawEvent(uint256 createTime,uint256 amount,address wAddress);

    IRouter public router;
    constructor() ERC20("Withdraw", "Withdraw") {
       router=IRouter(routerAddress);
       setManager(msg.sender,true);
    }

    function withdraw(uint256 _amount, address _address) external onlyManager{
        uint256 _createTime=block.timestamp;
        require(_amount<=WithdrawMax,"Error: more than max");
        ...
        TransferHelper.safeTransfer(usdtAddress,_address,_amount2);

        emit withdrawEvent(_createTime,_amount,_address);
    }
    ...
    function setWithdrawMax(uint256 _max) external onlyOwner{
        WithdrawMax = _max;
    }
