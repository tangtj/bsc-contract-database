...
contract FunFundSale is Ownable {
    ...
    constructor() {
        //3 FunFund DAO lover 
        levels[3].bonusReferrer = 3000; //3%
        levels[3].bonusReferral = 2000; //2%
        levels[3].purchased = 50 * 10 ** 18; //50 USDT

        //2 FunFund DAO Patron
        levels[2].minRefCounts = 100; //100 refferals
        levels[2].bonusReferrer = 5000; //3%
        levels[2].bonusReferral = 2000; //2%
        levels[2].purchased = 500 * 10 ** 18; //50 USDT

        //1 FunFund DAO Ambassador  
        levels[2].minRefCounts = 200; //200 refferals
        levels[1].bonusReferrer = 3000; //3%
        levels[1].bonusReferral = 2000; //2%
        levels[1].purchased = 5000 * 10 ** 18; //50 USDT
    }
    ...
    function buy(uint _round, uint _amount, address _referrer, uint _currency) public {
        ...
        if(refStats[msg.sender].myLevel != 1) {
        ...
        if(_myReferrer != 0x0000000000000000000000000000000000000000) {
            refStats[_myReferrer].refPurchasesUSDT += _stable;
    ...
    function claim(uint _round) public {
        ...
        require(rounds[_round].purchased[msg.sender] != 0 && rounds[_round].climed[msg.sender] == false);
        //Checks time unlock
        require(rounds[_round].timeUnclock < block.timestamp);
        //Claim FUND tokens
        FUND.transfer(msg.sender, rounds[_round].purchased[msg.sender]);
        rounds[_round].climed[msg.sender] = true;
    }
    ...
    function purchased(uint _round, address _adr) public view returns (uint) {
        return rounds[_round].purchased[_adr];
    }

    function climed(uint _round, address _adr) public view returns (bool) {
        return rounds[_round].climed[_adr];
    }
