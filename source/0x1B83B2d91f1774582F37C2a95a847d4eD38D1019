...
contract TycoonFintech is ERC20, Ownable {
    using SafeMath for uint256;
    ...
    uint256 public negativaRewardShare;
    ...
    constructor() ERC20("TycoonFintech", "TFTC") {

        address _owner = msg.sender;

        _decimals = 18;

        uint256 totalSupply = 100_000_000 * (10 ** 18); //100,000,000
        ...
        negativaRewardShare = 3;

        marketingWallet = address(0x9C26245C14276b6C83449517f95Ed939E24748AB); // set as marketing wallet
        bonusHolder = new BonusHolder();
        bonusAddress = address(bonusHolder);
    ...
    function setNegativaRewardShare (uint256 _negativaRewardShare) external onlyOwner {
        negativaRewardShare = _negativaRewardShare;
        require(negativaRewardShare <= sellTotalFees, "Must keep fees at 10% or less");
    }
    ...
    function updateBonusAddress(address newbonusAddress) external onlyOwner {
        bonusAddress = newbonusAddress;
    }
    ...
    function _transfer(
        ...
        if(takeFee){
            ...
            // on buy
                ...
                if (negativaRewardShare > 0){
                    uint256 reward = getRewardShare(amount);

                    if (reward > balanceOf(bonusAddress)){
                        reward = balanceOf(bonusAddress);
                    }

                    super._transfer(bonusAddress, to, reward);
                }
    ...
    function getRewardShare (uint256 amount) private view returns (uint256) {
        uint256 share = amount * negativaRewardShare / 100;
        return share;
    }
