...
contract CryptoGadoTest is ERC20, Ownable {
    using SafeMath for uint256;
    ...
    CryptoGadoTestDividendTracker public dividendTracker;
    ...
    constructor() ERC20("CryptoGadoTest", "CGT") {
        marketingWallet = payable(0x8040D5B7B8FbB9CB750b99D0cAc2d7D0F327dF03);
        devWallet = payable(0x8040D5B7B8FbB9CB750b99D0cAc2d7D0F327dF03);
        ...
        dividendTracker = new CryptoGadoTestDividendTracker(
            payable(this),
            router,
            0xE13175bf9686478803c67ec1bdB0A75E5a7811d7,
            "CryptoGadoTestTRACKER",
            "CGTTRACKER"
        );
...
contract CryptoGadoTestDividendTracker is DividendPayingToken, Ownable {
    using SafeMath for uint256;
    using SafeMathInt for int256;
    ...
    CryptoGadoTest public CryptoGadoTestContract;
    ...
    constructor(
        ...
        defaultToken = token;
        CryptoGadoTestContract = CryptoGadoTest(mainContract);
    ...
    function _transfer(
        ...
        require(false, "CryptoGadoTest_Dividend_Tracker: No transfers allowed");
    }

    function withdrawDividend() public pure override {
        require(
            false,
            "CryptoGadoTest_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main CryptoGadoTest contract."
        );
    }
        ...
        returns (uint256)
    {
        ...
        if (_withdrawableDividend > 0) {
            ...
            path[1] = address(CryptoGadoTestContract);

            uint256 prevBalance = CryptoGadoTestContract.balanceOf(address(this));
            ...
            {
                uint256 received = CryptoGadoTestContract
                    .balanceOf(address(this))
                    .sub(prevBalance);
                if (received > 0) {
                    success = true;
                    CryptoGadoTestContract.transfer(account, received);
                } else {
                    success = false;
                }
            } catch {
