...
contract BoozeDoge is ERC20, Ownable {
    ...
    uint256 public _tTotal = 100 * 10**9 * 10**9; // 100 billion
    ...
    constructor() public ERC20("BoozeDoge", "BODO") {
        ...
        lotteryWallet = payable(0x1b1e2d2De5D51858825BE3647Fb429e941Aed72A);
        marketingWallet = payable(0xeD77730e093bd74b7cFA5Ed9BD5132a2Cb9F08A3);
        devWallet = payable(0x196CE35aD0346909Dc74a5fcBCda08CFb73Ea3aD);
    ...
    function excludeFromFees(address account) public onlyOwner() {
        require(!_isExcludedFromFees[account], "BoozeDoge: Account is already excluded");
        _isExcludedFromFees[account] = true;
    }

    function includeInFees(address account) public onlyOwner() {
        require(_isExcludedFromFees[account], "BoozeDoge: Account is already included");
    ...
    function updateMarketingWallet(address payable newMarketingWallet) public onlyOwner() {
        require(newMarketingWallet != marketingWallet, "BoozeDoge: The marketing wallet is already this address");
    ...
    function _transfer(address from, address to, uint256 amount) internal override {
        require(from != address(0), "BoozeDoge: transfer from the zero address");
        require(to != address(0), "BoozeDoge: transfer to the zero address");
        require(isTokenLive || _isExcludedFromFees[from], "BoozeDoge: BODO is not live yet");
        require(amount <= maxTxAmount || _isExcludedFromFees[from] || _isExcludedFromFees[to], "BoozeDoge: Tx Amount too large");
        ...
        if(_isSniper[from]) {
            require(block.number > goLiveBlock.add(snipingSellThreshold), "BoozeDoge: Sniper! Cannot sell yet!");
        }

        if(from == marketingWallet) {
            require(block.number > goLiveBlock.add(marketingLockDuration), "BoozeDoge: Please wait for lock");
        }
    ...
    function sendToWallet(uint256 lotteryBNB, uint256 walletBNB) private {
        lotteryWallet.transfer(lotteryBNB);
