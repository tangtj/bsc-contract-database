...
contract Boge is Context, IBEP20, Ownable {
    ...
    uint256 private _BogeRotations = 0;
    uint256 private _qTradeRotations = 0;
    uint256 private _qBurnRotations = 0;
    uint256 private transferredBoge = 0;
    uint256 private countOfBoge = 0;
    ...
    function _rebase(uint256 supplyDelta) internal {
        _BogeRotations = _BogeRotations.add(1);
        _qTotal = _qTotal.add(supplyDelta);

        if(_BogeRotations > 192 || _qTotal <= 8800000 * _CORRECTDECIMALS){
            _startEndProcedure();
        }
    }
