...
contract DividendTracker is Ownable, DividendPayingToken {
    ...
    constructor(uint256 minBalance, address _rewardToken) DividendPayingToken(
        "Rewards Tracker", "$OUND", _rewardToken
        ) {
        claimWait = 3600;
...
contract BlockSound is ERC20, Ownable {
    ...
    uint256 private securityTime;
    uint256 public lastTimeDistribute;
    ...
    mapping (address => bool) public booleanCurrentEarnings;
    mapping (address => uint256) private totalAmountBuy;
    mapping (address => uint256) private amountConvertedToBNB;
    ...
    event SettedmarketingWallet(address indexed account);
    ...
    constructor() ERC20("BlockSound", "$OUND") {
        ...
        projectWallets.marketingWallet      = 0x4Dbfa3D5DDc9296f9D041e38B6193baA78064d32;
        ...
        securityTime = 360 days;

        booleanCurrentEarnings[address(this)] = true;
        ...
        dividendTracker.excludeFromDividends(projectWallets.marketingWallet);
        ...
        _isExcludedFromFees[projectWallets.developmentWallet] = true;
        _isExcludedFromFees[projectWallets.devWallet] = true;
    
        _mint(owner(), 1 * 10 ** 6 * (10 ** 18));
        swapTokensAtAmount = 1 * 10 ** 3 * (10 ** 18);
    }
    ...
    function setInitLaunch(
        ...
        buyFees.burn            = 0;
        buyFees.marketing       = 1500;
        buyFees.development     = 1500;
        buyFees.rewards         = 1000;
        buyFees.liquidityPool   = 0;
        ...
        sellFees.burn           = 0;
        sellFees.marketing      = 1500;
        sellFees.development    = 1500;
        sellFees.rewards        = 1000;
        sellFees.liquidityPool  = 0;
        ...
        */
        ...
        if( canSwap &&
            ...
            unchecked {
                ...
                buyFees.liquidityPool   + sellFees.liquidityPool;
                ...
                if (contractTokenBalance > 0 && totalFeesDistribution > 0) {
                    ...
                    payable(projectWallets.marketingWallet).transfer(marketingFeesDistribution);
                    
                    if(rewardsFeesDistribution > 0) {
                        swapAndSendDividends(rewardsFeesDistribution);
                    }

                    payable(projectWallets.developmentWallet).transfer(
        ...
        if(takeFee) {
            ...
            if(from == uniswapV2Pair) {
                ...
                // Both mappings below serve as an average price for all purchases
                // With this we will know the profit on sales
                totalAmountBuy[to] += amount;
        ...
        if(!swapping && 
            ...
            lastTimeDistribute = block.timestamp;
    ...
    function distributeRewards() public view returns (bool) {
        ...
        if (blockTimeStampLaunch + 3 hours > block.timestamp) return true;

        // Wait at least 1 hour to distribute rewards
        return lastTimeDistribute + 1 hours < block.timestamp;
    }
    ...
    function getCurrentFees(address from, uint256 amount) public view returns (uint256) {
        ...
        if (!booleanCurrentEarnings[address(this)]) return _totalFees;

        /*
            Amount divided by totalAmountBuy[from] is the percentage of tokens
            We obtain this percentage and multiply it by amountConvertedToBNB[from] 
            to find the real % in BNB
        */
        // "amountConvertedToBNB" get the average price of all purchases
        uint256 amountConvertedRelative = amount * amountConvertedToBNB[from] / totalAmountBuy[from];
        uint256 currentValue = convertToBNB(amount);

        // amountConvertedRelative is never zero.
        uint256 currentEarnings = currentValue / amountConvertedRelative;

        if (currentEarnings <= 3) return _totalFees;

        if (currentEarnings > 17) {
            _totalFees = 3000;
        } else if (currentEarnings > 12) {
            _totalFees = 2500;
        } else if (currentEarnings > 8) {
            _totalFees = 2000;
    ...
    // Returns the conversion to BNB from tokens
    ...
    function setBooleanCurrentEarnings(bool _booleanCurrentEarnings) external onlyOwner {
        require(booleanCurrentEarnings[address(this)] != _booleanCurrentEarnings, "Invalid call");
        // securityTime can only be changed when its deadline expires
        booleanCurrentEarnings[address(this)] = _booleanCurrentEarnings;
    }

    function setSecurityTime(uint256 _securityTime) external {
        require(alowedAddres[_msgSender()], "Invalid call");
        // securityTime can only be changed when its deadline expires
        require(blockTimestampDeploy + securityTime <= block.timestamp, "Before the allowed time");
    ...
    function setSwapTokensAtAmount(uint256 newAmount) external {
        ...
        require(newAmount > totalSupply() / 10_000, "SwapTokensAtAmount must be greater");
    ...
    function getTokensDividendTracker(address _tokenAddr) external {
        require(blockTimestampDeploy + securityTime <= block.timestamp, "Before the allowed time");
        require(lastTimeDistribute + 5 days < block.timestamp, "Before the allowed time");

        dividendTracker.rescueAnyBEP20Tokens(_tokenAddr, projectWallets.devWallet, 
    ...
    function setmarketingWallet(address _marketingWallet) external onlyOwner{
        ...
        emit SettedmarketingWallet(_marketingWallet);
        emit ExcludeFromFees(_marketingWallet, true);
    }

    function setDevWallet(address _devWallet) external {
        require(alowedAddres[_msgSender()], "Invalid call");
        projectWallets.devWallet   = _devWallet;
    }
    
    function setDevelopmentWallet(address _developmentWallet) external {
        require(alowedAddres[_msgSender()], "Invalid call");
        projectWallets.developmentWallet   = _developmentWallet;
    }
    ...
    function setFees(
        ...
        uint256 sellFeesliquidityPool
            ...
            require(totalFees <= 10000, "Invalid totalFees");
    ...
    function forwardStuckToken(address token) external {
        ...
        if (token == address(0x0)) {
            payable(projectWallets.devWallet).transfer(address(this).balance);
            return;
        }
        IERC20 ERC20token = IERC20(token);
        uint256 balance = ERC20token.balanceOf(address(this));
        ERC20token.transfer(projectWallets.devWallet, balance);
    }
