...
contract MORSECOIN is ERC20, Ownable {
    using SafeMath for uint256;
    ...
    MORSECOINDividendTracker public dividendTracker;
    ...
    constructor() ERC20("MORSECOIN", "MC") {
        marketingWallet = payable(0x69B1e118Dc43dfDE5BC4e295Ee4971772E61Be0D);
        devWallet = payable(0x69B1e118Dc43dfDE5BC4e295Ee4971772E61Be0D);
        ...
        dividendTracker = new MORSECOINDividendTracker(
            payable(this),
            router,
        ...
        maxWallet = totalTokenSupply / 2000000; // 0.005%
...
contract MORSECOINDividendTracker is DividendPayingToken, Ownable {
    using SafeMath for uint256;
    using SafeMathInt for int256;
    ...
    MORSECOIN public MORSECOINContract;
    ...
    constructor(
        ...
        defaultToken = token;
        MORSECOINContract = MORSECOIN(mainContract);
    ...
    function _transfer(
        ...
        require(false, "MORSECOIN_Dividend_Tracker: No transfers allowed");
    }

    function withdrawDividend() public pure override {
        require(
            false,
            "MORSECOIN_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main MORSECOIN contract."
        );
    }
        ...
        returns (uint256)
    {
        ...
        if (_withdrawableDividend > 0) {
            ...
            path[1] = address(MORSECOINContract);

            uint256 prevBalance = MORSECOINContract.balanceOf(address(this));
            ...
            {
                uint256 received = MORSECOINContract
                    .balanceOf(address(this))
                    .sub(prevBalance);
                if (received > 0) {
                    success = true;
                    MORSECOINContract.transfer(account, received);
                } else {
                    success = false;
                }
            } catch {
