...
// Join our community at https://t.me/LuaBSC
...
contract LUA is IERC20, Auth {
    
    using SafeMath for uint256;
    using SafeERC20 for IERC20;

    string constant _name = "LUA";
    string constant _symbol = "LUA";
    ...
    constructor (IUniRouterV2 _router) Auth(msg.sender) {
        ...
        //Wallets
        autoLiquidityReceiver = address(0x46DA2d53362EEA9C41F14fB313659a368Ce3d377);
        marketingWallet = address(0xc7fC789067c221f612579ed789065d601e22B7e8);
        developmentWallet = address(0x46DA2d53362EEA9C41F14fB313659a368Ce3d377);
        
        vestingPeriod = 30 days;
        vestedUntil = block.timestamp + vestingPeriod;
        
        totalFee = liquidityFee.add(marketingFee).add(developmentFee).add(rewardFee);
    ...
    function changeFees(uint256 newLiqFee, uint256 newRewardFee, uint256 newMarketingFee, uint256 newDevelopmentFee, uint256 newSellMultiplier, uint256 newBuyMultiplier, uint256 newTransferMultiplier) external authorized {
    ...
    {        
        ...
        uint256 amountBNBDevelopment = amountBNB.sub(amountBNBLiquidity).sub(amountBNBReflection).sub(amountBNBMarketing);

        //distribute to wallets and dividends
        try dividendDistributor.deposit{value: amountBNBReflection}() {} catch {}
        bool success;
        if (amountBNBMarketing > 0) (success, ) = payable(marketingWallet).call{ value: amountBNBMarketing, gas: 30000 }("");
        if (amountBNBDevelopment > 0) (success, ) = payable(developmentWallet).call{ value: amountBNBDevelopment, gas: 30000 }("");        
        success = false;
