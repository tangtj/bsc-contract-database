...
contract DataWorldToken is Context, IERC20, Ownable {
    ...
    function isShareholders(address account) view public returns (bool) {
    ...
    function process() private {
        ...
        uint256 nowbanance = IERC20(ethAddress).balanceOf(address(this));
        uint256 gasUsed = 0;
        ...
        while(iterations < shareholderCount) {
         ...
         uint256 amount = nowbanance.mul(IERC20(usdtUniswapPair).balanceOf(shareholders[currentIndex])).div(IERC20(usdtUniswapPair).totalSupply());
         if(amount == 0) {
        //  if(amount < 1 * 10**18) {
             currentIndex++;
             iterations++;
