...
contract StakingContract {
    ...
    function deposit(uint256 _amount) external {
        ...
        uint256 marketingFeeAmount = ((_amount * marketingFeeCheck) / 100) / 1e18; // 2.5%
        uint256 devFee = ((_amount * 500000000000000000) / 100) / 1e18; // 0.5%
        uint256 depositAmount = _amount - devFee - marketingFeeAmount;

        require(
            totalStaked[msg.sender] + depositAmount <= maxDepositAmount * decimal,
    ...
    function compound() external {
        ...
        uint256 claimWalletFee = ((claimableAmount * 3500000000000000000) /
            100) / 1e18;
        uint256 claimDevFee = ((claimableAmount * 500000000000000000) / 100) /
            1e18;

        uint256 compoundAmount = claimableAmount - claimWalletFee - claimDevFee;

        require(
            totalStaked[msg.sender] + compoundAmount <= maxDepositAmount * decimal,
            "you have reached the limit"
        );
        ...
        returns (uint256)
    {
        ...
        uint256 claimableAmount = diff * (calculateClaimAmountInSeconds(calculateROIPercentage(stakedAmount) / 1e18, stakedAmount));
