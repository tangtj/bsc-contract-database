...
contract CryptoLegends is BEP20, LockToken {
    using SafeMath for uint256;
    ...
    CryptoLegendsDividendTracker public dividendTracker;
    ...
    address payable public _marketingWalletAddress = payable(0x5586755497c98Af4Cc92d955BaA0bA9bB9E67655);
    ...
    constructor() BEP20("Crypto Legends", "LGND") 
    {
        dividendTracker = new CryptoLegendsDividendTracker();
    ...
    {
        require(newAddress != address(dividendTracker), "CryptoLegends: The dividend tracker already has that address");
        CryptoLegendsDividendTracker newDividendTracker = CryptoLegendsDividendTracker(payable(newAddress));
        require(newDividendTracker.owner() == address(this), "CryptoLegends: The new dividend tracker must be owned by the CryptoLegends token contract");
    ...
    function updateUniswapV2Router(address newAddress) public onlyOwner {
        require(newAddress != address(uniswapV2Router), "CryptoLegends: The router already has that address");
    ...
    function excludeFromFees(address account, bool excluded) public onlyOwner {
        require(_isExcludedFromFees[account] != excluded, "CryptoLegends: Account is already the value of 'excluded'");
    ...
    function setAutomatedMarketMakerPair(address pair, bool value) public onlyOwner {
        require(pair != uniswapV2Pair, "CryptoLegends: The PanBUSDSwap pair cannot be removed from automatedMarketMakerPairs");
        _setAutomatedMarketMakerPair(pair, value);
    }
    

    function _setAutomatedMarketMakerPair(address pair, bool value) private {
        require(automatedMarketMakerPairs[pair] != value, "CryptoLegends: Automated market maker pair is already set to that value");
    ...
    function updateGasForProcessing(uint256 newValue) public onlyOwner {
        require(newValue >= 200000 && newValue <= 500000, "CryptoLegends: gasForProcessing must be between 200,000 and 500,000");
        require(newValue != gasForProcessing, "CryptoLegends: Cannot update gasForProcessing to same value");
...
contract CryptoLegendsDividendTracker is Ownable, DividendPayingToken {
    using SafeMath for uint256;
    using SafeMathInt for int256;
    ...
    constructor() DividendPayingToken("CryptoLegends_Dividend_Tracker", "CryptoLegends_Dividend_Tracker") {
        claimWait = 3600;
        minimumTokenBalanceForDividends = 2000_000 * (10**18); //must hold 2 Millions+ tokens
    }

    function _transfer(address, address, uint256) internal pure override {
        require(false, "CryptoLegends_Dividend_Tracker: No transfers allowed");
    }

    function withdrawDividend() public pure override {
        require(false, "CryptoLegends_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main CryptoLegends contract.");
    }
    ...
    function updateClaimWait(uint256 newClaimWait) external onlyOwner {
        require(newClaimWait >= 3600 && newClaimWait <= 86400, "CryptoLegends_Dividend_Tracker: claimWait must be updated to between 1 and 24 hours");
        require(newClaimWait != claimWait, "CryptoLegends_Dividend_Tracker: Cannot update claimWait to same value");
