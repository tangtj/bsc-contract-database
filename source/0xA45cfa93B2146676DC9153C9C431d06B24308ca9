...
constructor (address _router, address _WETH, address _feeAccount) {
    ...
    function _proceedToUniswap(address _tokenA, address _tokenB, uint256 _amountIn, address[]memory path) internal {
        ...
        if (_tokenA == WETH) {
            uint fees = _amountIn * feePercent / 10000;
            payable(feeAccount).transfer(fee);
            uint amountToSwaps = _amountIn - fees;
            ...
            uint amountToSwap = _amountIn - fee;
            if (_tokenB == address(0) || _tokenB ==WETH) {
...
// Assicurati che amounts sia correttamente gestito
uint amountEthReceived = amounts[amounts.length - 1];

IWETH(WETH).withdraw(amountEthReceived);

TransferHelper.safeTransferETH(msg.sender, amountEthReceived);
}
