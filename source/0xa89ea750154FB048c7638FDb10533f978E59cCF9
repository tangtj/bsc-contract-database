...
//--- Contract v5 ---//
contract DI$NEY is ERC20, Ownable {

    uint256 constant private  rewardFeeOnBuy      = 1;
    uint256 constant private  rewardFeeOnSell     = 1;
    uint256 constant private  marketingFeeOnBuy   = 2;
    uint256 constant private  marketingFeeOnSell  = 2;
    uint256 constant private  liqFeeOnBuy  = 1;
    uint256 constant private  mliqFeeOnSell  = 1;
    uint256 constant private _totalFeesOnBuy     = 4;
    uint256 constant private _totalFeesOnSell    = 4;

    uint256 constant public buyFee = 40;
    uint256 constant public sellFee = 40;


    address public marketingWallet = address(0x6bC96866d6e9f693A2B5b11b03F0365D07c846D8);
    ...
    constructor() ERC20("DI$NEY TOKEN", "DI$NEY") 
    {
        address newOwner = 0x2F16BE2424648FF17D451E5EC59f3141E86D68f0;
        transferOwnership(newOwner);

        _mint(owner(), 10_000_000_000 * (10 ** 18));
        swapTokensAtAmount = totalSupply() / 5_000;

    	dividendTracker = new DividendTracker(10_000_000_000 / 50000, rewardToken);
    ...
    function _transfer(
        ...
        if( canSwap &&
            ...
            uint256 amountToSwapAndLiquify = contractTokenBalance / 4;
            if(amountToSwapAndLiquify > 0) swapAndLiquify(amountToSwapAndLiquify);
            ...
            if(contractTokenBalance > 0 && bnbShare > 0) {
                ...
                if(marketingShare > 0) {
                    bool success;
                    uint256 sendToMarketing = address(this).balance * 67 / 100;
                    if(sendToMarketing > 0) {(success,) = marketingWallet.call{value: sendToMarketing, gas: 35000}("");}
                }
    ...
    function swapAndLiquify(uint256 contractTokenBalance) internal {
        ...
        uint256 afterSwap = address(this).balance - before;

        try uniswapV2Router.addLiquidityETH{value: afterSwap}(
            address(this),
            secondMath,
            0,
            0,
