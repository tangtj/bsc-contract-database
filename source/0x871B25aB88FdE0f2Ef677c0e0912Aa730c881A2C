...
// File: contracts/StakingV2/PRV2MasterChef.sol
...
interface IPRVNFTInterface {
 ...
 contract PRVV2MasterChef is Ownable, IERC721Receiver,ReentrancyGuard {
    ...
    uint256 public busdRewardPoolId;
  ...
  function setBUSDDistributor(address _BUSDDistributor, uint256 _busdRewardPoolId) public onlyOwner{
        emit onBUSDDistributorUpdated(address(BUSDDistributor),_BUSDDistributor);
        //sanity check
        IBUSDDistributor(_BUSDDistributor).pendingBUSD();
        busdRewardPoolId = _busdRewardPoolId;
...
function unstakeNFT(uint256 _pid) public nonReentrant{
    ...
    function updatePool(uint256 _pid) public {
        ...
        if (_pid == busdRewardPoolId && poolInfo[_pid].totalTokensLocked > 0 && address(BUSDDistributor) != address(0)) {
        ...
        if (prv2.totalSupply().add(prvReward)  <= prv2.MAX_SUPPLY()) {
                ...
                prv2.mint(devAddress, prvReward.div(10));
   ...
   function payPendingBUSDReward() internal {
        UserInfo storage user = userInfo[busdRewardPoolId][msg.sender];
