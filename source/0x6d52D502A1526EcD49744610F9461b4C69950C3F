...
contract vSpcToken is ERC20, CheckOper, ReentrancyGuard {
    using SafeMath for uint;
    event MintvSpc(address indexed user, uint256 mint);
    event RedeemVSpc(address indexed user, uint256 burn);
    event NewFee(address indexed user, uint newFee, uint oldFee);
    event WithdrawFee(address indexed user, uint fee);


    uint public constant ACC_PRECISION = 1e18;
    uint256 public fee = 7 * 10 ** 16;// 7%
    IERC20 public spc;
    uint public withdrawFee;

    constructor(address oper, address spc_) ERC20("SPC VIP Token", "vSPC") CheckOper(oper){
        require(spc_ != address(0), "spc is 0");
        spc = IERC20(spc_);
    }


    function claimWithdrawFee() external onlyOperator {
        TransferHelper.safeTransfer(address(spc), msg.sender, withdrawFee);
        withdrawFee = 0;
        emit WithdrawFee(msg.sender, withdrawFee);
    }


    function setFee(uint fee_) external onlyOperator {
        uint oldFee = fee;
        fee = fee_;
        emit NewFee(msg.sender, fee_, oldFee);
    }

    function mint(uint256 _amount) public nonReentrant {
        require(_amount > 0, "amount>0");
        uint balanceBefore = spc.balanceOf(address(this));
        TransferHelper.safeTransferFrom(address(spc), msg.sender, address(this), _amount);
        uint balanceAfter = spc.balanceOf(address(this));
        require(balanceAfter >= balanceBefore, "TOKEN_TRANSFER_IN_OVERFLOW");
        _amount = balanceAfter.sub(balanceBefore);
        _mint(msg.sender, _amount);
        emit MintvSpc(msg.sender, _amount);
    }

    function redeem(uint256 _amount) public nonReentrant {
        require(_amount > 0, "amount>0");
        uint balanceBefore = IERC20(address(this)).balanceOf(address(this));
        TransferHelper.safeTransferFrom(address(this), msg.sender, address(this), _amount);
        uint balanceAfter = IERC20(address(this)).balanceOf(address(this));
        require(balanceAfter >= balanceBefore, "TOKEN_TRANSFER_IN_OVERFLOW");
        _amount = balanceAfter.sub(balanceBefore);
        uint _bal = spc.balanceOf(address(this));
        require(_bal >= _amount, "balance is not enough");
        uint feeAmount = _amount.mul(fee).div(ACC_PRECISION);
        withdrawFee = withdrawFee.add(feeAmount);
        TransferHelper.safeTransfer(address(spc), msg.sender, _amount.sub(feeAmount));
        _burn(address(this), _amount);
        emit RedeemVSpc(msg.sender, _amount);
    }
