...
contract MfrmToken is ERC20("MemeFarm", "MFRM"), Ownable {
...
contract MasterChef is Ownable {
    ...
    // block number when distribution stops
    uint256 public distributionStopBlock;
    ...
    constructor(
        ...
        Mfrm = _Mfrm;
        MfrmPerBlock = 2272727272727272727;
        startBlock = _startBlock;
        daoStartBlock = _daoStartBlock;
        distributionStopBlock = _startBlock + 3080000;
    }
    ...
    function getMultiplier(uint256 _from, uint256 _to) public view returns (uint256) {
        if (_from >= distributionStopBlock) {
            return 0;
        } else if (_to >= distributionStopBlock) {
            return distributionStopBlock.sub(_from);
        } else {
    ...
    function updateMfrmPerBlock(uint256 newAmountPerBlock) external onlyOwner {
        MfrmPerBlock = newAmountPerBlock;
    }

    function updateDistributionStopBlock(uint256 _distributionStopBlock) external onlyOwner {
        require(_distributionStopBlock > block.number, "_distributionStopBlock must be in future");
        distributionStopBlock = _distributionStopBlock;
    }
}