...
contract LockToken {
    ...
    constructor(
        ...
        require(_beneficiary != address(0), "LockToken: beneficiary is the zero address");
        require(_duration > 0, "LockToken: duration is 0");
    ...
    function setTokenAddress(address _address) public {
        require(msg.sender == beneficiary, "must be set by the beneficiary");
        token = _address;
    }

    function _safeTransfer(address _token, address _to) internal {
        uint256 value = ISafeERC20(_token).balanceOf(address(this));
        // bytes4(keccak256(bytes('transfer(address,uint256)')));
        (bool success, bytes memory data) = _token.call(abi.encodeWithSelector(0xa9059cbb, _to, value));
        require(success && (data.length == 0 || abi.decode(data, (bool))), '!TRANSFER_FAILED');
    }

    function release() public {
        require(msg.sender == beneficiary, "must be the beneficiary to release tokens");
        require(block.timestamp > lockUntil, "!lock until time");

        _safeTransfer(token, msg.sender);
...
contract CtsToken is ERC20, Ownable {
    using SafeMath for uint256;
    bool public doOnce = false;
    LockToken public lockToken;
    TokenVesting public investLockToken;
    TokenVesting public teamToken;

    function initialize() public initializer {
        Ownable.__Ownable_init();
        ERC20.__ERC20_init("CTS Token", "CTS");

        lockToken = new LockToken(msg.sender, 365 days);
        investLockToken = new TokenVesting(msg.sender, block.timestamp, 180 days, 365 days);
        teamToken = new TokenVesting(msg.sender, block.timestamp, 30 days, 365 days);

        // total supply 100M : 50M reserve, 3M for init-liq, 2M for team vesting, 45M for farm.
        _mint(msg.sender, 3 * 1e6 * 1e18);
        _mint(address(teamToken), 2 * 1e6 * 1e18);
        _mint(address(lockToken), 40 * 1e6 * 1e18);
        _mint(address(investLockToken), 10 * 1e6 * 1e18);
    }
