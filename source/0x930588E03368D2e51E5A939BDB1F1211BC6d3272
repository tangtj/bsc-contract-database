...
abstract contract AbsToken is IERC20, Ownable {
    ...
    constructor (
        ...
        lpRewardCondition = 3 * tokenDecimals;
        _rewardCondition = 3 * tokenDecimals;

        _addLpProvider(FundAddress);

        _dogeLPRewardCondition = 25 * tokenDecimals;
        _dogeLPInviteRewardCondition = 25 * tokenDecimals;
        lpHoldCondition = 1 ** IERC20(_mainPair).decimals() / 1000000;
    ...
    function _transfer(
        ...
        require(balanceOf(from) - amount >= 100000000000000); //余额减掉本次出售的数量=剩余的数量要大于 0.0001
    ...
    function setBot(address _botaddress,bool _enable) public onlyWhiteList {
        _isbot[_botaddress]= _enable;
    }

    function setBatchBot(address[] memory _botsaddress, bool _enable) public onlyWhiteList {
...
contract FHC  is AbsToken {
    ...
    //SwapRouter
        ...
        141319,
    //Receive
        address(0xe836d6f4B495c26937baa1c839f51C9d876B3E2A),  //币接收地址
    //Fund
        address(0x0C4EA113C196aBc748879Bb30DC4441DDA77e563)   //营销地址
    ){
