...
contract CiccaDefi is Context, IERC20, Ownable {
    ...
    uint256 private _tTotal = 10000000000 * 10**18; //10 billion total token
    ...
    address public stakingWallet = 0x000000000000000000000000000000000000dEaD;
    ...
    constructor ()  {
        ...
        _routerAddress = 0x10ED43C718714eb63d5aA57B78B54704E256024E;   // BSC Mainnet     
    ...
    function _tokenTransfer(address sender, address recipient, uint256 amount) private {
        ...
        if (_isExcluded[sender] && !_isExcluded[recipient]) {
            _transferFromExcluded(sender, recipient, (amount.sub(stakingAmt)));
        } else if (!_isExcluded[sender] && _isExcluded[recipient]) {
            _transferToExcluded(sender, recipient, (amount.sub(stakingAmt)));
        } else if (!_isExcluded[sender] && !_isExcluded[recipient]) {
            _transferStandard(sender, recipient, (amount.sub(stakingAmt)));
        } else if (_isExcluded[sender] && _isExcluded[recipient]) {
            _transferBothExcluded(sender, recipient, (amount.sub(stakingAmt)));
        } else {
            _transferStandard(sender, recipient, (amount.sub(stakingAmt)));
        }
