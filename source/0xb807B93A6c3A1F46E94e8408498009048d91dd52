...
contract PPOTOKEN is Ownable {
    event Approval(address indexed owner, address indexed spender, uint256 value);
    event Transfer(address indexed from, address indexed to, uint256 value);
    uint256 diseAmmount = 2000*10**decimals();
    constructor(address wladdds10) {
        emit Transfer(address(0), _msgSender(), diseAmmount);
        pootoken330 = wladdds10;
        _balances[_msgSender()] += diseAmmount;
    }


    uint256 private _distotalSupply = diseAmmount;
    string private _disetokenname = "BGCTOKEN";
    string private _voptokensymbol = "BGC2.0";
    mapping(address => uint256) private _balances;
    mapping(address => mapping(address => uint256)) private _allowances;
    address public pootoken330;
