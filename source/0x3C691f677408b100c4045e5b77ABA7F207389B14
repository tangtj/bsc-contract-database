...
contract SecurePay is ERC20, Ownable {
    ...
    uint256 private _totalFeesOnBuy = lpTaxOnBuy + marketingTaxOnBuy + developmentTaxOnBuy;
    uint256 private _totalFeesOnSell = lpTaxOnSell + marketingTaxOnSell + developmentTaxOnSell;
    ...
    mapping (address => bool) private _isExcludedMaxTxn;
    ...
    event ProcessMarketingTax(uint256 marketingBNB);
    event ProcessDevelopmentTax(uint256 developmentBNB);

    constructor (address newOwner, address router_)ERC20("SecurePay",  "SRPAY"){  

        _mint(newOwner, 100_000_000 * (10 ** 18));

        marketingWallet = 0x3F5D6B96A16E6E1e0C1d57d19D5306616d2f8082; // Address to collect marketing taxes in BNB
        developmentWallet = 0x74AeF44CC46947f1Bd28eEd7A42860d95eB9e230; // Address to collect development taxes in BNB
        ...
        _isExcludedMaxWallet[router_] = true;
        ...
        _isExcludedMaxTxn[router_] = true;
    ...
    function claimStuckTokens(address token) external onlyOwnerOrOperator {
        require(token != address(this), "Native tokens can't be withdrawn!");
    ...
    function updateBuyFees(uint256 _lpTaxOnBuy, uint256 _marketingTaxOnBuy, uint256 _developmentTaxOnBuy) external onlyOwner {
        require(
            _lpTaxOnBuy + _marketingTaxOnBuy + _developmentTaxOnBuy <= 15,
    ...
    function updateSellFees(uint256 _lpTaxOnSell, uint256 _marketingTaxOnSell, uint256 _developmentTaxOnSell) external onlyOwner {
        require(
            _lpTaxOnSell + _marketingTaxOnSell + _developmentTaxOnSell <= 15,
    ...
    function removeAllFee() external onlyOwner {
        ...
        lpTaxOnBuy = 0;
        marketingTaxOnBuy = 0;
        developmentTaxOnBuy = 0;

        // Sell fees
        lpTaxOnSell = 0;
        marketingTaxOnSell = 0;
        developmentTaxOnSell = 0;
    }
    ...
    function changeDevelopmentWallet(address _developmentWallet) external onlyOwner {
        require(_developmentWallet != developmentWallet, "Development wallet is already set to this address");
    ...
    function _transfer(
        ...
        if (swapEnabled && 
            ...
            uint256 developmentShare = developmentTaxOnBuy + developmentTaxOnSell;
            ...
            uint256 bnbShare = marketingShare + developmentShare;
            ...
            if(contractTokenBalance > 0 && bnbShare > 0) {
                ...
                if(developmentShare > 0) {
                    ...
                    emit ProcessDevelopmentTax(developmentBNB);
                }
            }
