...
contract TokenStaking is Ownable, ReentrancyGuard, Initializable {
  ...
  function unstake(uint256 _amount) external nonReentrant {
    ...
    if (!stakePeriodOver) {
        fee = (_amount * _earlyUnstakeFeePercentage) / PERCENTAGE_DENOMINATOR;
        _amount -= fee;  // Decrease the unstaking amount by the early unstake fee
    }
    ...
    _totalStakedTokens -= _amount;
    IERC20(_tokenAddress).transfer(user, _amount - fee); // apply fee deduction here
    emit UnStake(user, _amount - fee); // show fee deducted amount

    // If fee is charged, log it
    if (fee > 0) {
        emit EarlyUnStakeFee(user, fee);
    }
