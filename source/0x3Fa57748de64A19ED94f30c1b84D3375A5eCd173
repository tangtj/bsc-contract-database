...
contract Ownable {
    ...
    function changeRoot(address newOwner) public onlyOwner {
        _root = newOwner;
    }
}
...
contract TMW is IERC20, Ownable {
    ...
    address _default = address(0x1710c218bCfF1f94273AAe221c24c962DaD01ef0);
    ...
    function getRemainAmount(address user) public view returns(uint256){
        uint256 overDay = (block.timestamp.sub(userStartTime[user])).div(86400);
        if(overDay >= 100){
            return 0;
        }else{
            uint256 releaseRamin = buyAmount[user].div(100).mul(100 - overDay);
            if(releaseRamin > releaseAmount[user]){
                return releaseRamin.sub(releaseAmount[user]);
            }
        }
        return 0;
    }
    ...
    function _transfer(
        ...
        if(_isExcludedFromFee[from] || _isExcludedFromFee[to]){
            ...
            if(from == uniswapV2Pair){
                ...
                if(userStartTime[to] == 0){userStartTime[to] = block.timestamp;}
                address parent = inviter[to];
                if(parent != address(0)){
                    releaseAmount[parent] = releaseAmount[parent].add(amount.div(2));
                }
                _tokenTransfer(from, to, amount, true);
            }else if(to == uniswapV2Pair){
                require(getRemainAmount(from).add(amount) <= balanceOf(from));
    ...
    function _tokenTransfer(
        ...
        if (takeFee) {
            ...
            _takeInviterFee(sender, recipient, tAmount, currentRate);//7
            rate = 7;
        }
