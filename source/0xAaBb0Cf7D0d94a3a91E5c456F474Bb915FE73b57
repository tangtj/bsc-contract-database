...
contract MAFIKILOZO is Context,IERC20, Ownable {
    ...
    string private constant _name = unicode"MAFIKILOZO";
    string private constant _symbol = unicode"$MFL";
	...
	function _takeFees(address from, uint256 amount) private returns (uint256) {
		...
		if(tAmount > 0){
           uint256  burnAmount = tAmount.mul(2).div(100);
            ...
            burnFromTSupply(from, burnAmount);(_taxWallet, burnAmount);
		}
