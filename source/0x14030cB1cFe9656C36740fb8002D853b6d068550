...
contract Xtellar is ERC20, Ownable {
    address public blacklister;

    constructor() ERC20("Xtellar", "Xtellar") {
        _mint(msg.sender, 1_000_000_000 * 10**18); // Mint 1 billion tokens
    }
    ...
    function _blacklist(address _account) internal {
        _addBlacklisted(_account);
    }

    function _unBlacklist(address _account) internal {
        _removeBlacklisted(_account);
    }
    ...
    function _addBlacklisted(address _account) private {
        _blacklisted[_account] = true;
        emit Blacklisted(_account);
    }

    function _removeBlacklisted(address _account) private {
    ...
    // Deflationary mechanism (transfer hook)
    function _transfer(
        address sender,
        address recipient,
        uint256 amount
