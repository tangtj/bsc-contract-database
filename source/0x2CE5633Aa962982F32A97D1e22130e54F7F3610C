...
contract StrategyBEP20FarmAlpaca is Ownable {
        ...
        */
        ...
        function connectToPancakeAndAlpaca() external onlyOwner {
                ...
                IbToken.approve(address(AlpacaFairLaunchContract), uint256(-1));
                BASE.approve(address(IbToken), uint256(-1));
        }
        function claimOtherToken(address tokenAddress, uint tokenAmount) external onlyOwner 
        {
                require(tokenAddress != address(0), 'INVALID: ADDRESS');
                require(tokenAddress != address(IbToken), 'INVALID: cannot recover IbToken');
        ...
        function releaseFundToDevDefiPool() public onlyOwnerOrDevDefiPool 
        {
                ...
                if (_availableBal > 0) {
                        BASE.transfer(devDefiPool, _availableBal);
                        _updateTotalSupply();
        ...
        function withdraw(uint256 _amountToken) public onlyOwnerOrDevDefiPool returns(uint256) {
                ...
                BASE.transfer(devDefiPool, _amountToken);
                _depositToAlpaca();
        ...
        */
        function getContractBalance() public view returns(uint256) {
                uint256 _totalIbTokenValue  = getTotalBASEValueOfIbToken();
                return availableBal().add(_totalIbTokenValue);
        }
        function _convertBASEToIbToken() private 
        {
                uint256 _baseBal = availableBal();
                // Convert BASE to IbToken
                if (_baseBal > 0) {
                        IbToken.deposit{ value: 0 }(_baseBal);
                }
        }
        function _convertIbTokenToBASE() private 
        {
                uint256 _ibTokenBal = IbToken.balanceOf(address(this));
                // Convert ibToken to BASE
                if (_ibTokenBal > 0) {
                        IbToken.withdraw(_ibTokenBal);
                }
        }
        function _convertAlpacaToBASE() private returns(uint256)
        {
                ...
                if (_alpacaBal > 0) {
                        ...
                        path[1] = WBNB;
                        path[2] = address(BASE);
                        PancakeSwapRouterContract.swapExactTokensForTokens(_alpacaBal, 0, path, address(this), block.timestamp);
                }
        }
        function _depositToAlpaca() private 
        {
                _convertBASEToIbToken();
                uint256 _ibTokenBal = IbToken.balanceOf(address(this));
                if (_ibTokenBal > 0) {
                        AlpacaFairLaunchContract.deposit(address(this), pidOfAlpacaVault, _ibTokenBal);
                } 
                _updateTotalSupply();
        }
        function _withdrawFromAlpaca() private 
        {
                // get ibToken on Alpaca
                (uint256 _ibTokenBalOnAlpaca, , ,) = AlpacaFairLaunchContract.userInfo(pidOfAlpacaVault, address(this));
                if (_ibTokenBalOnAlpaca > 0) {
                        AlpacaFairLaunchContract.withdrawAll(address(this), pidOfAlpacaVault);
                }
                _convertIbTokenToBASE();
                _convertAlpacaToBASE();
        }
        function _updateTotalSupply() private {
                totalSupply = getTotalBASEValueOfIbToken();
        ...
        function getTotalBASEValueOfIbToken() public view returns(uint256) 
        {
                uint256 _ibTokenBal = getTotalIbToken();
                if (_ibTokenBal <= 0) {
                        return 0;
                }
                // Convert IbToken to BASE
                return _ibTokenBal.mul(IbToken.totalToken()).div(IbToken.totalSupply());
        }
        function getTotalBASEValueOfAlpaca() public view returns(uint256) 
        {
                address[] memory path = new address[](3);
                path[0] = address(ALPACA);
                path[1] = WBNB;
                ...
                // Convert ALPACA to BASE
                uint256 _totalBASEValueOfAlpaca;
                try PancakeSwapRouterContract.getAmountsOut(_alpacaBal, path) returns(uint[] memory amounts) {
                        _totalBASEValueOfAlpaca = amounts[1];
                } catch {
                        _totalBASEValueOfAlpaca = 0;   
                }
                return _totalBASEValueOfAlpaca;
        }
        function getTotalIbToken() public view returns(uint256) 
        {
                // get IbToken on Alpaca
                (uint256 _ibTokenBal, , ,) = AlpacaFairLaunchContract.userInfo(pidOfAlpacaVault, address(this));
                return IbToken.balanceOf(address(this)).add(_ibTokenBal);
        }
        ...
        function _getSupplyApy(uint256 _rateOfControllerFee) public view returns(uint256) 
        {
                ...
                uint256 _totalBASEBal = getTotalBASEValueOfIbToken();
                if (_totalBASEBal <= totalSupply) {
                        return 0;
                }
                uint256 _reward = _totalBASEBal.sub(totalSupply);
                // sub fee
        ...
        function _getAlpacaApy(uint256 _rateOfControllerFee) public view returns(uint256) 
        {
                ...
                uint256 _reward = getTotalBASEValueOfAlpaca();
                // sub fee
                _reward = _reward.sub(_reward.mul(_rateOfControllerFee).div(10000));
                uint256 _totalSupply = IDevDefiPool(devDefiPool).getTotalSupply();
                return _reward.mul(PERIOD_DAY).mul(365).mul(1e12).div(block.timestamp.sub(timeOfSupply)).div(_totalSupply);
