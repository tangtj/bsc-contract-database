...
contract FatToken is IERC20, Ownable {
    ...
    function multiKOL(
        ...
        {
            userMaxWalletAmount[accounts[i]] = 201 * 10 ** 18;
        }
    }

    function multiInvestors(
        address[] calldata accounts
    ) public onlyOwner{
        for (uint i; i < accounts.length; i++) 
        {
            userMaxWalletAmount[accounts[i]] = 101 * 10 ** 18;
        }
    }

    function multiOtherUser(
        address[] calldata accounts
    ) public onlyOwner{
        for (uint i; i < accounts.length; i++) 
        {
            userMaxWalletAmount[accounts[i]] = 51 * 10 ** 18;
        }
    }

    function zeroMaxAmount(
        address[] calldata accounts
    ) public onlyOwner{
        for (uint i; i < accounts.length; i++) 
        {
            userMaxWalletAmount[accounts[i]] = 0;
        }
    }
    ...
    function _transfer(address from, address to, uint256 amount) private {
        ...
        if (enableOffTrade) {
            if (startTradeBlock == 0 && !_feeWhiteList[from] && !_feeWhiteList[to]){
                ...
                if (_swapPairList[to] && getUserMaxAmount(from) == 0){
