...
contract Business {
    ...
    function Deposit(uint256 _amount) external {
        ...
        //Convert XMM to USD
        require(TokenPer_USD > 0, "TokenPer_USD not set"); // Make sure the conversion rate is set up
    ...
    function Reinvest() external {
        ...
        IERC20 token = IERC20(tokenAddress);
        require(
            token.transfer(msg.sender, userDepositAmount),
            "Withdrawal failed"
        );
    ...
    function Harvest(address _tokenAddress, uint256 _amount) external {
        ...
        require(
            block.timestamp >= userDeposits[msg.sender].depositTime + 60,
            "Cannot harvest within 1 second"
        );
        ...
        require(
            ...
            "Withdrawal not allowed for this token."
        );
    ...
    function getUserUSD(address _user) external view returns (uint256) {
        return userUSD[_user];
    }

    function EarnedUSD(address user) public view returns (uint256) {
        uint256 timePassed = block.timestamp - depositTime_pev[user];
    ...
    function getUserTotalDeposits_pev(address _user)
        public
        view
        returns (uint256)
    {
