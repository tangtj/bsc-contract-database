...
/*
   ...
   #Official Family Doge Coin More than meme tokens Links:
   Website: www.Familydogecoin.com
   Telegram Community Group: www.t.me/FamilyDogeCoin
   Twitter: www.twitter.com/FamilyDogeBSC

   #FamilyDoge Transaction Fees:
   2% transaction fee = sent into the locked liquidity pool.
   12% transaction fee = swapped to BNB and redistributed between all holders.
   
   #BNB Reward:
   Auto-Claim BNB reward is redistributed automatically every 30 minutes.
*/
...
contract FamilyDoge is ERC20, Ownable {
    using SafeMath for uint256;
    ...
    FamilyDogeDividendTracker public dividendTracker;
    ...
    constructor() public ERC20("Family Doge", "FamilyDoge") {
    	...
    	dividendTracker = new FamilyDogeDividendTracker();
    ...
    function updateDividendTracker(address newAddress) public onlyOwner {
        require(newAddress != address(dividendTracker), "FamilyDoge: The dividend tracker already has that address");

        FamilyDogeDividendTracker newDividendTracker = FamilyDogeDividendTracker(payable(newAddress));

        require(newDividendTracker.owner() == address(this), "FamilyDoge: The new dividend tracker must be owned by the FamilyDoge token contract");
    ...
    function updateUniswapV2Router(address newAddress) public onlyOwner {
        require(newAddress != address(uniswapV2Router), "FamilyDoge: The router already has that address");
        emit UpdateUniswapV2Router(newAddress, address(uniswapV2Router));
        uniswapV2Router = IUniswapV2Router02(newAddress);
    }

    function excludeFromFees(address account, bool excluded) public onlyOwner {
        require(_isExcludedFromFees[account] != excluded, "FamilyDoge: Account is already the value of 'excluded'");
    ...
    function setAutomatedMarketMakerPair(address pair, bool value) public onlyOwner {
        require(pair != uniswapV2Pair, "FamilyDoge: The PancakeSwap pair cannot be removed from automatedMarketMakerPairs");

        _setAutomatedMarketMakerPair(pair, value);
    }

    function _setAutomatedMarketMakerPair(address pair, bool value) private {
        require(automatedMarketMakerPairs[pair] != value, "FamilyDoge: Automated market maker pair is already set to that value");
    ...
    function updateLiquidityWallet(address newLiquidityWallet) public onlyOwner {
        require(newLiquidityWallet != liquidityWallet, "FamilyDoge: The liquidity wallet is already this address");
    ...
    function updateGasForProcessing(uint256 newValue) public onlyOwner {
        require(newValue >= 200000 && newValue <= 500000, "FamilyDoge: gasForProcessing must be between 200,000 and 500,000");
        require(newValue != gasForProcessing, "FamilyDoge: Cannot update gasForProcessing to same value");
    ...
    function _transfer(
        ...
        if(!tradingIsEnabled) {
            require(canTransferBeforeTradingIsEnabled[from], "FamilyDoge: This account cannot send tokens until trading is enabled");
        }
        ...
        if(isFixedSaleBuy) {
            require(block.timestamp >= fixedSaleStartTimestamp, "FamilyDoge: The fixed-sale has not started yet.");
            ...
            if(!openToEveryone) {
                require(fixedSaleEarlyParticipants[to], "FamilyDoge: The fixed-sale is only available to certain participants at the start");
            }
...
contract FamilyDogeDividendTracker is DividendPayingToken, Ownable {
    using SafeMath for uint256;
    using SafeMathInt for int256;
    ...
    constructor() public DividendPayingToken("FamilyDoge_Dividend_Tracker", "FamilyDoge_Dividend_Tracker") {
    	claimWait = 1800;
        minimumTokenBalanceForDividends = 100000000 * (10**18); //must hold 10000+ tokens
    }

    function _transfer(address, address, uint256) internal override {
        require(false, "FamilyDoge_Dividend_Tracker: No transfers allowed");
    }

    function withdrawDividend() public override {
        require(false, "FamilyDoge_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main FamilyDoge contract.");
    }
    ...
    function updateClaimWait(uint256 newClaimWait) external onlyOwner {
        require(newClaimWait >= 1200 && newClaimWait <= 86400, "FamilyDoge_Dividend_Tracker: claimWait must be updated to between 1 and 24 hours");
        require(newClaimWait != claimWait, "FamilyDoge_Dividend_Tracker: Cannot update claimWait to same value");
