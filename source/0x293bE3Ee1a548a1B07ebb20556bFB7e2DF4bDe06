...
contract Sale_Contract {
    ...
    uint256 referralBonus = weiAmount.mul(f1BonusRate).div(1000); // Calculate f1 referral bonus in BNB
    ...
    function _deliverTokens(address _beneficiary, uint256 _tokenAmount) internal {
      ...
      function _processPurchase(address _buyer, address _referrer, uint256 _tokenAmount, uint256 _referralBonus) internal {
        ...
        uint256 f1Bonus = _referralBonus.mul(f1BonusRate).div(100);
        uint256 f2Bonus = _referralBonus.mul(f2BonusRate).div(100);
