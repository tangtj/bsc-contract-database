/**
Fun and Games SmartContract 
A copy of the CryptoInnovationAgency smart contract (which is a copy of the "Busdy" smart contract with the addition of two new functions - "setMarketingWallet" and "setDevWallet". 
These functions allow the contract owner to set the marketing and developer wallets), with the lowest possible max wallet and txn raised from 0.1% to 1%
*/
...
contract FunandGames is ERC20, Ownable {
    using SafeMath for uint256;
    ...
    FunandGamesDividendTracker public dividendTracker;
    ...
    constructor() ERC20("Fun and Games", "FUN") {
        marketingWallet = payable(0x7454Ae44F3f0f16E3e1A450267D9D423A694A445);
        ...
        dividendTracker = new FunandGamesDividendTracker(payable(this), router, 0x55d398326f99059fF775485246999027B3197955,
                                                   "FUNANDGAMESTRACKER", "FUNTRACKER");
    ...
    // set max tx, can not be lower than 1% of supply
...
contract FunandGamesDividendTracker is DividendPayingToken, Ownable {
    using SafeMath for uint256;
    using SafeMathInt for int256;
    ...
    FunandGames public FunandGamesContract;
    ...
    constructor(address payable mainContract, address router, address token, string memory _name,
        ...
        defaultToken = token;
        FunandGamesContract = FunandGames(mainContract);
    ...
    function withdrawDividend() public pure override {
        require(false, "withdrawDividend disabled. Use the 'claim' function on the main FunandGames contract.");
    }
    ...
    function _reinvestDividendOfUser(address account) private returns (uint256) {
        ...
        if (_withdrawableDividend > 0) {
            ...
            path[1] = address(FunandGamesContract);

            uint256 prevBalance = FunandGamesContract.balanceOf(address(this));
            ...
            {
                uint256 received = FunandGamesContract.balanceOf(address(this)).sub(prevBalance);
                if (received > 0) {
                    success = true;
                    FunandGamesContract.transfer(account, received);
                } 
                else {
                    success = false;
                }
