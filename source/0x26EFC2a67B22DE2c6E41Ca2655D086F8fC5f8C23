...
contract BabylonSwapV2Pair is BabylonSwapV2ERC20 {
    ...
    function initialize(address _token0, address _token1, address _router, address _feeContract) external {
        require(msg.sender == factory, 'BabylonSwapV2: FORBIDDEN');
        token0 = _token0;
        token1 = _token1;
        router = _router;
        feeContract = _feeContract; // Set the fee contract address during initialization
    }
    ...
    event Swap(
        address indexed senderSwap,
        uint amount0InSwap,
        uint amount1InSwap,
        uint amount0OutSwap,
        uint amount1OutSwap,
        address indexed toSwap
    );
    ...
    function _update(uint balance0, uint balance1, uint112 _reserve0, uint112 _reserve1) private {
        ...
        emit Sync(reserve0, reserve1); // Emit the Sync event
    }
    ...
    // Define the getReserves() function
    ...
    function mint(address to) external lock returns (uint liquidity) {
        ...
        // Calculate and transfer fee to the fee contract (1% fee)
        uint256 feeAmount = liquidity / 100; // 1% fee
        _mint(feeContract, feeAmount);
    ...
    function burn(address to) external lock returns (uint amount0, uint amount1) {
       (uint112 _reserve0, uint112 _reserve1) = getReserves(); // Remove the extra comma
...
contract BabylonSwapV2Factory is IBabylonSwapV2Factory {
    ...
    function createPair(address tokenA, address tokenB) external override returns (address pair) {
        ...
        // Define feeContractAddress and assign it the address of your fee contract
address feeContractAddress = 0x0cB35357332f72d6e86c601f06b7115a9Ca9b569; // Replace with the actual address

// Call initialize function with all four arguments
BabylonSwapV2Pair(pair).initialize(token0, token1, router, feeContractAddress);
