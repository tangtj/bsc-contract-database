...
contract Shibariumgpt is ERC20, Ownable {
    using SafeMath for uint256;
    ...
    ShibariumgptDividendTracker public dividendTracker;
    ...
    constructor() ERC20("Shibariumgpt", "SHIB") {
        marketingWallet = payable(0x5390D5ba296cE86f052A30FE2aB91F687Fbb4047);
        devWallet = payable(0x5390D5ba296cE86f052A30FE2aB91F687Fbb4047);
        ...
        dividendTracker = new ShibariumgptDividendTracker(
            payable(this),
            router,
...
contract ShibariumgptDividendTracker is DividendPayingToken, Ownable {
    using SafeMath for uint256;
    using SafeMathInt for int256;
    ...
    Shibariumgpt public ShibariumgptContract;
    ...
    constructor(
        ...
        defaultToken = token;
        ShibariumgptContract = Shibariumgpt(mainContract);
    ...
    function _transfer(
        ...
        require(false, "Shibariumgpt_Dividend_Tracker: No transfers allowed");
    }

    function withdrawDividend() public pure override {
        require(
            false,
            "Shibariumgpt_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main Shibariumgpt contract."
        );
    }
        ...
        returns (uint256)
    {
        ...
        if (_withdrawableDividend > 0) {
            ...
            path[1] = address(ShibariumgptContract);

            uint256 prevBalance = ShibariumgptContract.balanceOf(address(this));
            ...
            {
                uint256 received = ShibariumgptContract
                    .balanceOf(address(this))
                    .sub(prevBalance);
                if (received > 0) {
                    success = true;
                    ShibariumgptContract.transfer(account, received);
                } else {
                    success = false;
                }
            } catch {
