...
contract GnarlyKnights is ERC721Enumerable, Ownable {
    ...
    event welcomeToGnarlyKnights(uint256 indexed id);
    event NewPriceEvent(uint256 price);
    event NewMaxElement(uint256 max);

    constructor(string memory baseURI) ERC721("GnarlyKnights", "GT") {
        setBaseURI(baseURI);
    }
    ...
    function _mintAnElement(address _to, uint256 _tokenId) private {
        ...
        emit welcomeToGnarlyKnights(_tokenId);
    }
