...
contract CIK is Context, IERC20, Ownable {
    ...
    address payable public CIKFundAddress = payable(0x000000000000000000000000000000000000dEaD);
    ...
    string private constant _name = "CIK";
    string private constant _symbol = "CIK";
    ...
    event SetCIKFundAddress(address newAddress);
    ...
    function setCIKFundAddress(address newAddress) external onlyOwner() {
        CIKFundAddress = payable(newAddress);

        emit SetCIKFundAddress(CIKFundAddress);
    }
   ...
   function setFees(uint256 newRewardFee, uint256 newLiquidityFee, uint256 newProjectFee, uint256 newBuyBackFee) external onlyOwner() {
    ...
    function setBuyBackAddress(address newAddress) external onlyOwner() {
        ...
        emit SetBuyBackAddress(CIKFundAddress);
    }
    ...
    function swapAndLiquify(uint256 tAmount) private lockTheSwap {
        ...
        if(forWallets > 0 && _projectFee > 0) 
        {
            ...
            uint256 newBalance = address(this).balance.sub(initialBalance);
            transferToAddressBNB(CIKFundAddress, newBalance);
