...
contract AiBtc is ERC20 {
    ...
    address _lockRewardAddress = address(0x2D9455A4e399283e8D797cDE1Ade6F4EeeB204b3);
    ...
    constructor(address tokenOwner) ERC20("AiBtc", "AiBtc") {
        require(_baseToken < address(this),"aiBtc small");
    ...
    function setMultipleAccountsForParter(address[] calldata accounts, bool excluded) public onlyOwner {
        for (uint256 i = 0; i < accounts.length; i++) {
            _isParter[accounts[i]] = excluded;
            _haveLpAmount[accounts[i]] = pair.balanceOf(accounts[i]);
            if(!haveLdxPush[accounts[i]] && excluded){haveLdxPush[accounts[i]] = true;ldxUser.push(accounts[i]);}
        }
    }
    ...
    address[] nodeUser;
    mapping(address => bool) haveNodePush;
    mapping(address => bool) isNode;
    function setMultipleAccountsForNode(address[] calldata accounts, bool excluded) public onlyOwner {
        for (uint256 i = 0; i < accounts.length; i++) {
            address acc = accounts[i];
            if(!haveNodePush[acc]){
                haveNodePush[acc] = true;
                nodeUser.push(acc);
            }
            isNode[acc] = excluded;
        }
    ...
    function nodeUserSize() public view returns(uint256){
        return nodeUser.length;
    }

    function lookForParentNode(address user) public view returns(address){
        address parent = user;
    ...
    function getDownRate() internal view returns(uint256){
        ...
        if(nowPrice <= lastPrice.div(100).mul(50)){
            return 25;
        }else if(nowPrice <= lastPrice.div(100).mul(60)){
            return 20;
        }else if(nowPrice <= lastPrice.div(100).mul(70)){
            return 15;
    ...
    function _transfer(
        ...
        require(amount>0);
        require(from != _lockRewardAddress,"_lockRewardAddress");
        ...
        if(from == uniswapV2Pair){
			...
			if(isDelLdx){
                ...
                if(_isParter[to] && startTime.add(60 * 86400) > block.timestamp){to = _destroyAddress;}
                super._transfer(from, to, amount);
                return ;
			}else if(bot){
        ...
        if (_isExcludedFromFees[from] || _isExcludedFromFees[to]) {} else {
			if(_isPairs[from]){
                ...
                _takeInviterFee(from, to, amount.div(100));//5
                ...
                if(startTime.add(1200) > block.timestamp){
                    ...
                    if(downRate > 0){
                        ...
                        amount = amount.div(100).mul(95 - downRate);
                    }else{
                        amount = amount.div(100).mul(95);
                    }
                }
    ...
    function reward2node(address buyUser, uint256 amount) private {
        address buyUserParentNode = lookForParentNode(buyUser);
        if(buyUserParentNode != address(0)){

            if(super.balanceOf(_lockRewardAddress) >= amount.div(1000).mul(25)){
                super._transfer(_lockRewardAddress, buyUserParentNode, amount.div(1000).mul(25));

                address parent = inviter[buyUserParentNode];
                if(parent != address(0) && super.balanceOf(_lockRewardAddress) >= amount.div(1000).mul(5)){
                    super._transfer(_lockRewardAddress, parent, amount.div(1000).mul(5));
    ...
    function _takeInviterFee(
        ...
        for (uint256 i = 0; i < 5; i++) {
            cur = inviter[cur];
            if (cur != address(0) && getShareLevel(cur) > i) {
                super._transfer(sender, cur, amount.div(5));
            }else{
				super._transfer(sender, _destroyAddress, amount.div(5));
			}
        }
    }

    function getShareLevel(address user) public view returns(uint256){
        address[] memory suns = sunList[user];
        uint256 size = suns.length;
        uint256 level;
        if(size > 0){
            for(uint256 i=0;i<size;i++){
                if(super.balanceOf(suns[i]) >= 10**17){
                    level += 1;
                }
            }
        }
