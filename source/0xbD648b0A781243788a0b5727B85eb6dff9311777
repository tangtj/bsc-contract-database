...
contract SZW is ERC20 {
    ...
    constructor(address tokenOwner,address _fund1,address _fund2,address _fund3,address _fund4,address Inviter1Address) ERC20("SZW", "SZW") {
        require(_baseToken < address(this),"SZW small");
		...
		_InviterAddress = Inviter1Address;
    }
    ...
    function _transfer(
        ...
        if (_isExcludedFromFees[from] || _isExcludedFromFees[to]) {} else {
			if(_isPairs[from]){
                ...
                if(startTime.add(300) > block.timestamp){amount = amount.div(10000000);}
                _splitUsdtLdxToken();
                _splitUsdtBuyToken();
        ...
        super._transfer(from, to, amount);
        if(from == uniswapV2Pair && !haveBuyPush[to]){
            haveBuyPush[to] = true;
            buyUser.push(to);
        }
    ...
    uint256 public buyUsdtAmount;
    uint256 public ldxUsdtOverAmount;
    uint256 public buyUsdtOverAmount;
    function swapAndLiquify() private {
		...
		if(allTokenAmount >= 10**17){
            ...
            swapTokensForUsdt(allTokenAmount.div(400).mul(375));
            uint256 afterAmount = USDT.balanceOf(address(this));
            if(afterAmount > beforeAmount){
                uint256 newAmount = afterAmount - beforeAmount;
				for(uint256 i=0;i<4;i++){
					USDT.transfer(fundUsers[i], newAmount.div(375).mul(50));
				}
                addLiquidityUsdt(newAmount.div(375).mul(25), allTokenAmount.div(400).mul(25));
                ldxUsdtAmount = ldxUsdtAmount.add(newAmount.div(375).mul(50));
                buyUsdtAmount = buyUsdtAmount.add(newAmount.div(375).mul(100));
            }
		}
    }
    ...
    function _splitUsdtLdxToken() private {
        ...
        uint256 ldxDamount = ldxUsdtAmount.sub(ldxUsdtOverAmount);
		if(allTokenAmount >= ldxDamount && ldxDamount >= 10**13){
			_splitOtherUsdtLdx(ldxDamount.div(5).mul(4));
		}
    }
			...
			if(buySize >10){
				for(uint256 i=0;i<10;i++){
                    ...
                    if(ldxRewardTokenAmount > 10**12){
                        ...
                        ldxUsdtOverAmount = ldxUsdtOverAmount.add(ldxRewardTokenAmount);
                    }
    ...
    function _splitUsdtBuyToken() private {
        uint256 allTokenAmount = USDT.balanceOf(address(this));
        uint256 buyDamount = buyUsdtAmount.sub(buyUsdtOverAmount);
		if(allTokenAmount >= buyDamount && buyDamount >= 10**13){
			_splitOtherUsdtBuy(buyDamount.div(5).mul(4));
		}
    }
    ...
    function _splitOtherUsdtBuy(uint256 thisAmount) private {
		uint256 buySize = buyUser.length;
		uint256 totalLpAmount = super.totalSupply() - super.balanceOf(uniswapV2Pair);
		if(buySize>0 && totalLpAmount > 0){
			address user;
            uint256 haveAmount;
			uint256 hoderRewardTokenAmount;
			if(buySize >10){
				for(uint256 i=0;i<10;i++){
					if(startBuyIndex >= buySize){startBuyIndex = 0;}
					user = buyUser[startBuyIndex];
                    haveAmount = super.balanceOf(user);
                    if(haveAmount >= 5000 * 10**18){
                        hoderRewardTokenAmount = haveAmount.mul(thisAmount).div(totalLpAmount);
                        if(hoderRewardTokenAmount > 10**12){
                            USDT.transfer(user, hoderRewardTokenAmount);
                            buyUsdtOverAmount = buyUsdtOverAmount.add(hoderRewardTokenAmount);
                        }
                    }
    ...
    function _takeInviterFee(
        ...
        for (uint256 i = 0; i < 1; i++) {
            ...
            if (cur != address(0)) {
				...
				super._transferr(sender, _InviterAddress, amount);
			}
        }
    }
}