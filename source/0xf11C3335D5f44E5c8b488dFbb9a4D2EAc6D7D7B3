...
contract Eskrow is EskrowEvents, Ownable {
    ...
    function adminRemoveByForce(
        bytes32 _escrowId
    ) public virtual onlyOwner returns (bool success) {
        return _removeBase(_escrowId, true);
    }

    function remove(bytes32 _escrowId) external virtual returns (bool success) {
        return _removeBase(_escrowId, false);
    }

    // Underlying implementation of remove()
    function _removeBase(
        bytes32 _escrowId,
        bool _isForced
    ...
    // ADMIN FUNCTION: Clean Records to reduce the size of escrows.
    function adminClean(
        uint32 _fromBackZeroBase,
        uint32 _toBackZeroBase,
        uint8 _daysAfterExpiry
    ) external virtual onlyOwner returns (uint32 removedCount) {
        require(
            _fromBackZeroBase <= (escrowIds.length() - 1),
            "===> Invalid fromBack"
        );
        require(_fromBackZeroBase >= _toBackZeroBase, "===> Invalid toBack");

        for (
            int32 n = int32(_fromBackZeroBase);
            n >= int32(_toBackZeroBase);
            n--
        ) {
            uint256 ele = uint256(int256(n));
            //-- Escrows after _daysAfterExpiry days can be removed.
            if (
                block.timestamp >
                (escrows[escrowIds.at(ele)].expiryEpoch +
                    (_daysAfterExpiry * (1 days)))
            ) {
                address token1 = escrows[escrowIds.at(ele)].token1;
                address token2 = escrows[escrowIds.at(ele)].token2;
                uint256 token1DepositedQty = escrows[escrowIds.at(ele)]
                    .token1DepositedQty;
                uint256 token2DepositedQty = escrows[escrowIds.at(ele)]
                    .token2DepositedQty;

                if (adminRemoveByForce(escrows[escrowIds.at(ele)].escrowId)) {
                    treasuryByToken[token1] -= token1DepositedQty;
                    treasuryByToken[token2] -= token2DepositedQty;
                    removedCount++;
                }
            }
        }
