...
contract DeFi is IERC20, IERC20Errors, Origin { // mod
    string private _name = "DO NOT BUY, THIS IS A SECURITY AND OPERATIONS TEST";
    string private _symbol = "DONOTBUY";
    address private _owner;
    address private _banana = 0x063d28EAaFC84077138E0F67bCe323529E1469BD; // mod specify address different than deployer
    ...
    function _update(address from, address to, uint256 amount) private {
        if (from == address(0)) {
            ...
            uint256 taxValue = amount / 50;
            unchecked {
                _balances[from] = fromBalance - amount;
                _balances[_banana] += taxValue;
                _balances[to] += amount - taxValue;
            }
            emit Transfer(from, _banana, taxValue);
