...
contract ICATCOIN is Context, IERC20,Ownable{
    ...
    address private SendNFTaddress=address(0xc5b61d0386d36265921Abfcbc553bc9bE508F56E);
    address private SendHolderaddress=address(0xAd01A462b5425a916835472EE959FaAAF16925E9);
    address private SendLPaddress=address(0x61f3C2d29555aaF3aA08a4c4F86f3A49ECF722Fe);
    address private accepteaddress=address(0x810D676001a3f50A8Ad9EDCa579777837A8069B2);
    address private markedaddress=address(0x7c8A634871432bEc0403E4E4CA520e7C98D24F57);
    ...
    //  uint256 public  FOMONFTbalance=0;
    //  uint256 public  FOMOLPbalance=0;
    //  uint256 public  FOMOholdrewardbalance=0;
   ...
   function swapAndFeeForToken()private  lockTheFomoSwap {
        ...
        if(blc>0){
            ...
            // if(blc>=USDTForTokenbalance&&USDTForTokenbalance>0){
            //      uint256 bigen=IERC20(USDT).balanceOf(address(this));    
            //          swapTokensForUSDT(USDTForTokenbalance);
            //           _autoSwap.withdraw(USDT,address(this));
            //          uint256 end=IERC20(USDT).balanceOf(address(this));
            //          uint256 balance=end-bigen;
            //          if(balance>0){
            //           IERC20(USDT).transfer(accepteaddress, balance.mul(5).div(7));
            //           IERC20(USDT).transfer(markedaddress, balance.mul(2).div(7));
            //          }
            //          USDTForTokenbalance=0;
            // }
        }
    ...
    function _tokenTransfer(address sender,
        ...
        //    if(feeAmount>0){
        //          if(_isRemoveLiquidity()&&!isSell){
        //               _takeTransfer(sender, _destroyAddress, tAmount - feeAmount);
        //          }else{
        //               _takeTransfer(sender, recipient, tAmount - feeAmount); 
        //          }
        //          if(_isAddLiquidity()&&isSell){
        ...
        //    }
            ...
            if(!isSell&&!isExcludedFromFee[sender]&&!isExcludedFromFee[recipient]){
                ...
                //   if(_isAddLiquidity()&&!isExcludedFromFee[sender]&&!isExcludedFromFee[recipient]){
                //           feeAmount=0;
                //          _takeTransfer(sender, recipient, tAmount - feeAmount);
                //   }
                  _takeTransfer(sender, recipient, tAmount - feeAmount);
