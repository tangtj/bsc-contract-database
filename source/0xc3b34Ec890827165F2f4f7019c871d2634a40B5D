...
contract EGODFoundation is ERC20, Ownable {
  constructor() public ERC20("t.me/EGODv2", "EGODv2") { // Symbol and Name
    // Mint EGODv2 (18 Decimals)
  ...
  // Exception to transfer fees, for example for Pancakeswap contracts.
  ...
  function setTransferFee(uint256 fee) public onlyOwner {
    require(fee <= 4000, "Fee cannot be greater than 40%");
    if (fee == 0) {
      activeFee = false;
    } else {
      activeFee = true;
