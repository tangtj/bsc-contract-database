...
contract DA_COIN_OPEN is IERC20,Ownable{
	using SafeMath for uint256;
    using Address for address;
    string private constant _symbol = "DA";
    string private constant _name = "DA COIN";
    uint8 private constant _decimals = 18;
    uint256 private constant _allBalances = 65000 *10**18;
    uint256 public constant _liqSellRatio = 5;
    uint256 public constant _liqBuyRatio = 3;
	mapping(address => uint256) private _balances;
    mapping(address => mapping(address => uint256)) private _allowances;
    address public _tokenOwner = 0x4C8C202C7B2B9a9a69e75bA0F9E8349f2f23087a;
    bool public _isMainOpen = true;
    bool public _isOpenSell;
    ...
    address public _buybackLP = 0xE90b63F2CA969E0050524D84A5C5C40Fa4f6975E;
    receive() external payable {}
	IUniswapV2Pair public _uniswapV2UsdtPair;
    mapping(address => bool) public _isSetUniswapV2UsdtPair;
	constructor() {
        address router;
        IERC20 _usdt;
        ...
        _setLpWihleList(0xd7Ca6D2732eA4a23ea469D8D40a9fD597E9D623A,true);
        _setLpWihleList(0x7b28CC44af350F06469D790C5d672d78c746BfD6,true);
        _setLpWihleList(_tokenOwner,true);
        _balances[_tokenOwner] = _allBalances;
        emit Transfer(address(0), _tokenOwner, _allBalances);
    }
    ...
    function _takeLiquidity(address fromAddress,uint256 amount,uint256 tLiquidityFee) private {
        if(tLiquidityFee == 3){
            // buy 3% 
            uint256 _amount = calcRatio(amount,tLiquidityFee);
            _balances[_buybackLP] = _balances[_buybackLP].add(_amount);
            emit Transfer(fromAddress, _buybackLP, _amount);
        }else{
            // sell 5%
            uint256 _amount = calcRatio(amount,tLiquidityFee);
            _balances[Model.deadAddress] = _balances[Model.deadAddress].add(_amount);
            emit Transfer(fromAddress, Model.deadAddress, _amount);
        }
    }
    function _tokenTransfer(
        ...
        if (tLiquidity > 0) _takeLiquidity(sender,amount,tLiquidityFee);
    ...
    function setBuybackLP(address buybackLP) external onlyOwner {
        _buybackLP = buybackLP;
    }
}