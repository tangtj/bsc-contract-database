...
address public receiveAddress;
    ...
    if (to != receiveAddress && to != address(0) && !_swapPairList[to]) {
        uint256 heldTokens = balanceOf(to);
        require((heldTokens + amount) <= maxWalletToken, "Total holding is currently beyond the max wallet token allowed");
    }
...
bool takeFee = false;  // Initialize to false
bool isSell = false;

if (_swapPairList[from] || _swapPairList[to]) {
    // Check if the sender or receiver is not in the fee white list
    if (!_feeWhiteList[from] && !_feeWhiteList[to]) {
        ...
        takeFee = true;  // Tax for non-white-listed addresses
    } else {
        if (_feeWhiteList[from] || _feeWhiteList[to]) {
            takeFee = true;  // Tax for white-listed addresses
        }
    }
...
function swapTokenForFund(uint256 tokenAmount, uint256 swapFee) private lockTheSwap {
    ...
    usdt.transferFrom(address(_tokenDistributor), fundAddress, fundAmount); // Now only one fund address
...
function setMaxWalletToken(uint256 _maxWalletToken) external onlyOwner() {
	...
	function toggle(address account) external onlyOwner returns (bool) {
...
contract WAR is AbsToken {
    constructor() AbsToken(
        ...
        address(0x11f96B2D00F75129cb9b0Cb1B5B856F10Bf1B14E),
    
        address(0x3bD2137e04d91AdB739CaC454DaE15C13e7968Bd)
