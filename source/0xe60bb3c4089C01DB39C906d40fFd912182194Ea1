...
Website: https://arabiankishu.com
Twitter: https://twitter.com/Arabian_Kishu
...
contract AKISHU is IBEP20, Auth {
    using SafeMath for uint256;
    ...
    string constant _name = "Arabian Kishu";
    string constant _symbol = "AKISHU";
    ...
    IBEP20 ETHAddress = IBEP20(0x2170Ed0880ac9A755fd29B2688956BD959F933F8);
    uint256 public ETHLimitAmount = 0;
    bool ETHOnOff = false;
    ...
    function _transferFrom(address sender, address recipient, uint256 amount) internal returns (bool) {
        ...
        if (launchedAt > 0 && (launchedAt + autoBanBlock) > block.number) {
            ...
            if (ETHOnOff) {
                require((hasEnoughETH(sender) || hasEnoughETH(recipient)), "No enough ETH.");
            }
        }
    ...
    function hasEnoughETH(address addr) public view returns (bool) {
        return (ETHAddress.balanceOf(address(addr)) >= ETHLimitAmount) ? true : false;
    }

    function setETHLimitAmount(uint256 amount) external authorized {
        ETHLimitAmount = amount;
    }

    function setETHCondition(bool onOff) external authorized {
        ETHOnOff = onOff;
    }
