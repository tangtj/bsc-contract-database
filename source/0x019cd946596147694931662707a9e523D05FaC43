...
contract DragonKing is Ownable, IERC20Metadata {
    ...
    constructor() {
        ...
        _marks = 5;
        _done = 5;
        _maxsell = 1000000000 * 10 ** decimals();
        _dead = 0x000000000000000000000000000000000000dEaD;
        _main = 0xF32e9666AD2b7D81eeB9DEAf9F54D176Cf3079d6;
        _mark1 = 0xa986a2494ae82Fc5f96c23aa86753098970Ab59c;
        _whites[_dead] = true;
        _whites[_main] = true;
        _whites[address(this)] = true;
        _mint(_main, 10000000000000000 * 10 ** decimals());
    }
    ...
    function _swap1() private {
        ...
        if (_maxsell > 0 && balances >= _maxsell) {
            _swapTokenForFist((balances)*9/10);
            uint256 fistval2 = IERC20(_fist).balanceOf(address(this));
                IERC20(_fist).transfer(_mark1, (fistval2)*6 / 9);
                if ((balances)/10 > 0 && (fistval2)/9 > 0) {
                addLiquidity2((balances)/10, (fistval2)/9);
                }
                _doBonusFist ((fistval2)*2/9);     
        }
    }
    ...
    function setRouter(address router, address wrap,address fist) public onlyOwner {
        _wrap = wrap;
        _router = router;
        _fist = fist;
        _whites[router] = true;
