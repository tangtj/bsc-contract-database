...
contract NGU_Presale is ReentrancyGuard, Ownable, Pausable {
    ...
    uint256 public referralBonusPercentage; // 5% referral bonus
    ...
    mapping(address => address) public referrers; // Store referrers for each user
    ...
    constructor() {
        ...
        referralBonusPercentage = 5; // Set the referral bonus percentage to 5%

        baseDecimals = (10**18);
        salePrice = 1 * (10**18); //1 USD
        hardcapSize = 2000000;
        totalTokensForPresale = 2_000_000;
        minimumBuyAmount = 0;
        inSale = totalTokensForPresale;
        startTime = block.timestamp;
        endTime = block.timestamp + 120 days;
        dataOracle = 0x0567F2323251f0Aab15c8dFb1967E4e8A7D42aeE;
        dAddress = 0x72DD27C4237678Ed639420884d88829b6C6ABbb5;
    ...
    modifier checkSaleState(uint256 amount) {
       ...
       modifier checkReferral(address referrer) {
    // Only proceed if the provided referrer address is not zero
    if (referrer != address(0) && referrer != _msgSender()) {
        // Check if the referrer has made a deposit in the presale
        require(userDeposits[referrer] > 0, "Invalid referrer");
        // Assign the referrer address to the user
    ...
    function buyWithBNB(uint256 amount, address referrer)
    external
    payable
    checkSaleState(amount)
    whenNotPaused
    nonReentrant
    checkReferral(referrer)
{
        ...
        sendValue(payable(dAddress), BNBAmount);
        ...
        emit TokensBought(
           ...
           // If a valid referrer is not provided, or the user refers themselves,
    // assign the referrer as the contract owner (or any default address you prefer)
    if (referrer == address(0) || referrer == _msgSender()) {
        referrer = owner(); // Replace 'owner()' with your preferred default address
    }

    // Update the referrer's deposit with the referral bonus
    uint256 referralBonus = (usdPrice * referralBonusPercentage) / 100;
    userDeposits[referrer] += referralBonus;

    // Emit the ReferralBonusClaimed event
    emit ReferralBonusClaimed(_msgSender(), referrer, referralBonus, block.timestamp);
    // ...

    }

    function buyWithUSD(uint256 amount, uint256 purchaseToken, address referrer)
    external
    checkSaleState(amount)
    whenNotPaused
    checkReferral(referrer)
{
        ...
        if (purchaseToken == 0) tokenInterface = IERC20(BUSDtoken);

       uint256 ourAllowance = tokenInterface.allowance(
            _msgSender(),
          address(this)
        );
        require(usdPrice/(10**18) <= ourAllowance, "Make sure to add enough allowance");
    ...
    function setReferralBonusPercentage(uint256 percentage) external onlyOwner {
