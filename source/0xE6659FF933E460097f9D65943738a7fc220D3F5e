...
contract TRRRR is Ownable {
    ...
    uint256 public BOOMMEntryFee = 10;
    uint256 public BNBEntryFee = 0;

    IBEP20 public BNBToken = IBEP20(0xbb4CdB9CBd36B01bD1cBaEBF2De08d9173bc095c);
    ...
    function enterWithBOOMM(address referrer) external payable {
        ...
        require(msg.value >= BOOMMEntryFee, "Not enough funds");

        // Make sure is right before start the game ;)        
        if(msg.value >= BOOMMEntryFee){
            startKaboomm(false, referrer);
        }
        
    }


    function enterWithBNB(address referrer) external payable {
        ...
        if(msg.value >= BNBEntryFee){
            startKaboomm(true, referrer);
        }
    }


    function startKaboomm(bool _useBOOMM, address referrer) private {
        address _player = _msgSender();
