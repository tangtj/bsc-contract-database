...
/**
 * @dev The official Diggerinu smart contract
 */
contract Diggerinu is Context, IERC20, Ownable {
    using SafeMath for uint256;
    using Address for address;
    
    // General Info
    string private _name     = "Digger inu";   //elon musk mine - miner token - find me :D
    string private _symbol   = "Diggerinu";
    uint8  private _decimals = 18;
    
    // Liquidity Settings
    IUniswapV2Router02 public _pancakeswapV2Router; // The address of the PancakeSwap V2 Router
    address public _pancakeswapV2LiquidityPair;     // The address of the PancakeSwap V2 liquidity pairing for Diggerinu/WBNB
    ...
    // Addresses
    address payable public _marketingAddress = payable(0x91D98Acb61fEEE45F7Bf054DAf123ED794c05AA2); // Marketing address used to pay for marketing
    ...
    uint256 private _tTotal = 1971628000000 * 10**9 * 10**9;
    uint256 private _rTotal = (MAX - (MAX % _tTotal));
    uint256 private _totalReflections; // Total reflections
    
    // Token Tax Settings
    uint256 public  _taxFee           = 0;  // 1% tax 
    uint256 public  _sellTaxFee       = 0; // 2% tax 
    uint256 public  _whaleSellTaxFee  = 0; // 3% tax 
    uint256 private _previousTaxFee;
    
    // Token Limits
    uint256 public _maxTxAmount        = 1971628000000 * 10**9 * 10**9;  // 1.971.628.000.000
    uint256 public _tokenSwapThreshold = 100 * 10**5 * 10**9; // 10.000.000
    
    // Timer Constants 
    uint private constant DAY = 0; // How many seconds in a day
    
    // Anti-Whale Settings 
    uint256 public _whaleSellThreshold = 10 * 10**7 * 10**9; // 100.000.000
    uint    public _whaleSellTimer     = DAY;                 // 72 hours
    ...
    uint256 public _buybackBNBThreshold  = 1 * 10**6;        // 1 BNB will be required to be inside the contract before a buyback will be performed
    uint256 public _buybackUpperLimit    = 1 * 10**6 * 10**9; // 1 million $Diggerinu will be used to buy at maximum in any one trade
    ...
    uint256 public  _lotteryThreshold    = 10 * 10**3 * 10**11; // 10.000 tokens required to be in the pool before lottery can be triggered
    uint256 public  _lotteryMinimumSpend = 10 * 10**3 * 10**11; // 10.000 tokens required to buy before before lottery can be triggered
    ...
    /**
     * @notice Withdraws non-Diggerinu tokens that are stuck as to not interfere with the liquidity
     */
    ...
    function _transfer(address from, address to, uint256 amount) private {
        ...
        // Gets the contracts Diggerinu balance for buybacks, charity, liquidity and marketing
    ...
    /**
     * @notice Burns Diggerinu tokens straight to the burn address
     */
    ...
    function swapAndLiquify(uint256 tokenAmount) private lockSwapping {
        ...
        // Swap 7/8ths of Diggerinu tokens for BNB
        ...
        // We now have 6/7ths left of BNB (converted from Diggerinu)
    ...
    function swapBNBForTokens(uint256 amount) private {
        ...
        _pancakeswapV2Router.swapExactETHForTokensSupportingFeeOnTransferTokens{value: amount}(
            0, // Accept any amount of pepDiggerinu
            path,
