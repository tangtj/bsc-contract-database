...
contract Dogeking is ERC20 {
    ...
    address _reciverAddress = address(0xf87ed038bAB331d949a29e7c4c80DE73aa3f2305);
    ...
    constructor(address tokenOwner) ERC20("Dogeking", "Dogeking") {
        require(_baseToken < address(this),"Dogeking small");
        ...
        nft = DogekingNft(0xDEFE16ecBdaa04D5a7C5Fb45ddD3dF3e0855b22F);

        haveBuyPush[tokenOwner] = true;
    ...
    function swapAndLiquify() private {
        ...
        if(allTokenAmount >= 10**18){
            ...
            swapTokensForUsdt(allTokenAmount.div(480).mul(430));
            uint256 afterAmount = USDT.balanceOf(address(this));
            if(afterAmount > beforAmount){
                uint256 newAmount = afterAmount - beforAmount;
                usdt4HodlerAmount = usdt4HodlerAmount.add(newAmount.div(430).mul(320));
                usdt4NftAmount = usdt4NftAmount.add(newAmount.div(430).mul(50));
                addLiquidityUsdt(newAmount.div(430).mul(50), allTokenAmount.div(480).mul(50));
            }
        }
    }
    ...
    uint256 public usdt4HodlerAmount;
    uint256 public usdt4HodlerOverAmount;
    address[] buyUser;
    mapping(address => bool) haveBuyPush;
	function _splitTokenHodlerToken() private {
        uint256 allTokenAmount = USDT.balanceOf(address(this));
		uint256 hodlerDevAmount = usdt4HodlerAmount.sub(usdt4HodlerOverAmount);
		if(allTokenAmount > hodlerDevAmount && hodlerDevAmount > 10**15){
            _splitOtherTokenHodler(hodlerDevAmount.div(10).mul(7));
		}
    }
    ...
    function _splitOtherTokenHodler(uint256 thisAmount) private {
		uint256 buySize = buyUser.length;
		uint256 totalLpAmount = super.totalSupply();
			...
			if(buySize >10){
				for(uint256 i=0;i<10;i++){
					...
					if(haveAmount > 10**19){
                        ...
                        usdt4HodlerOverAmount = usdt4HodlerOverAmount.add(ldxRewardTokenAmount);
					}
