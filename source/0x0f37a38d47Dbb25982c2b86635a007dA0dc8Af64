// File: contracts/Launhpad/TokenIdentifiers.sol
...
// File: contracts/Launhpad/MintingFee.sol
...
contract MintingFee is Ownable {
    ...
    address public devFeeAccount;

    function setFeeAccount(address _account) external onlyOwner {
        feeAccount = _account;
    }

    function setDevFeeAccount(address _account) external onlyOwner {
        devFeeAccount = _account;
    }

    function setFixedFee(uint8 _num) external onlyOwner {
        mintingFee = _num;
    }

    function setRoyaltyFeeRate(uint256 num) external onlyOwner {
        royaltyFeeRate = num;
    }

    function _chargeMintingFee() internal {
        (bool sent, ) = payable(feeAccount).call{value: mintingFee}("");
...
// File: contracts/Launhpad/LazyNFT.sol
...
contract LazyNFT is ERC721URIStorage, Ownable {
    using SafeMath for uint256;
    using TokenIdentifiers for uint256;

    mapping(uint256 => address) private _creators;
    mapping(uint256 => uint8) private _creatorEarnings;
    ...
    function mint(
        ...
        require(_minters[msg.sender], "Unauthorized");
        require(_owner == _tokenId.tokenCreator(), "Unauthorized for tokenID");
    ...
    function setPermanentURI(uint256 tokenId, string memory uri) external {
        require(bytes(uri).length > 0, "Invalid URI");
        require(!isPermanentURI(tokenId), "URI is already permanent");
        require(
            _exists(tokenId),
    ...
    function setCreator(uint256 tokenId, address _creator) external {
        require(msg.sender == creator(tokenId), "Unauthorized");
        _creators[tokenId] = _creator;
    }

    function creatorEarning(uint256 tokenId) external view returns (uint8) {
        return _creatorEarnings[tokenId];
    }

    function setCreatorEarning(uint256 tokenId, uint8 _value) external {
        require(_value <= 10, "Can only set up to 10%");
        require(msg.sender == creator(tokenId), "Unauthorized");

        _creatorEarnings[tokenId] = _value;
    }

    function setMinter(address _minter, bool isMinter) external onlyOwner {
        require(_minter != address(0), "Zero address is not allowed");
        _minters[_minter] = isMinter;
    }
}