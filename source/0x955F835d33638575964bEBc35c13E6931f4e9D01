...
contract Business {
    ...
    mapping(address => uint256) public earningsPerSecond;
    ...
    uint256 timePassed = block.timestamp - lastActionTimestamp[msg.sender];
    uint256 earnedAmount = (_user.amount * 115741 * timePassed) / 10000000000;
    _user.amount += earnedAmount;

    lastActionTimestamp[msg.sender] = block.timestamp;
    }

    function getEarningsPerSecond(address _user) external view returns (uint256) {
    return earningsPerSecond[_user];
    }
    ...
    uint256 fees = totalBonus * SCHOLARSHIP_FEE / PERCENTS_DIVIDER;
    uint256 actualAmountToSend = totalBonus - fees + earnedAmount;
    ...
    token.transfer(scholarship, fees);
