...
contract Sale_Contract {
    ...
    uint256 currentReferralCount = referralCount[msg.sender];
    uint256 referralGroup = (currentReferralCount / 3); // Calculate the current referral group

    // Calculate the bonus rate based on the referral group
    uint256 bonusRate = 10 + referralGroup * 5;

    // Cap the bonus rate at the maximum of 80%
    if (bonusRate > maxReferralBonusRate) {
        bonusRate = maxReferralBonusRate;
    }

    uint256 referralBonus = _weiAmount.mul(bonusRate).div(100);
