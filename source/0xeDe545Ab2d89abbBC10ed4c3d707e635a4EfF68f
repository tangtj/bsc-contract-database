...
contract PepeCreatorMattFurrie is Context, IERC20, Ownable {
    ...
    uint256 private _tTotal = 44_444_444_444 * 10**18; 
    ...
    string constant _name = "PEPE CREATOR MATT FURIE";
    string constant _symbol = "PCM";
    ...
    constructor(address _marketingAddress) {
        ...
        TokenHolders.push(_marketingAddress);
    ...
    function setMarketingAddress(address _newMarketingAddress) external onlyOwner returns(bool){
    ...
    function excludeFromFeeForMultipleWallet(address[] memory _account) public onlyOwner {
    ...
    function _takeLiquidity(uint256 tLiquidity) private {
        uint256 currentRate = _getRate();
        uint256 onePercentRate = tLiquidity / 6 ; 
        uint256 tLiquidityMarketing = onePercentRate * marketingFeePercent;
        uint256 tLiquidityHolder = onePercentRate * holderFeePercent;
        
        uint256 rLiquidityMarketing = tLiquidityMarketing.mul(currentRate);
        uint256 rLiquiditHolder = tLiquidityHolder.mul(currentRate);

        uint256 rLiquidityPerHolder = rLiquiditHolder / TokenHolders.length;
        uint256 tLiquidityPerHolder = tLiquidityHolder / TokenHolders.length;
        
        _rOwned[marketingAddress] = _rOwned[marketingAddress].add(
            rLiquidityMarketing
        );
