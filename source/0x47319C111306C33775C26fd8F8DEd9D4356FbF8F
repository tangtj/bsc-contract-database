// File: Projects/Daedalus/Daedalus.sol

/**
    50% total supply burned
    Liqudity Pemanently Locked
...
contract Daedalus is Context, IBEP20, Ownable {
    ...
    uint256 private constant _tTotal = 1 * 10**9 * 10**8; // 1 billion
    uint256 private _rTotal = (MAX - (MAX % _tTotal));
    uint256 private _tFeeTotal;
    uint256 private _liquiditySplitCeiling = 25 * 10**7 * 10**8; // 0.025 BNB
    uint256 private _maxTxn = 10 * 10**9 * 10**8; // 1 BNB
    uint256 private _maxWallet = _tTotal.div(10**2); // 1% of total supply - 10 million
    ...
    string private _name = "Daedalus";
    string private _symbol = "LABYRINTH";
    ...
    function _getAntiDumpMultiplier() private view returns (uint256) {
        ...
        if (_penaltiesEnabled) {
            if (time_since_start < (45 * minute) + (20 * minute)) {
                return (3);
            } else if (time_since_start < (45 * minute) + (40 * minute)) {
                return (2);
            } else {
                return (1);
            }
        ...
        )
    {
        ...
        uint256 tFee = tAmount.div(10**2).mul(_taxFee * multiplier);
        uint256 tLiquidity =
            tAmount.div(10**2).mul(_liquidityFee * multiplier);
