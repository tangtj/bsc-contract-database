...
contract DividendPayingToken is BEP20, Ownable, DividendPayingTokenInterface, DividendPayingTokenOptionalInterface {
  ...
  address public immutable POLS = address(0x7e624FA0E1c4AbFD309cC15719b7E2580887f570); //POLS
  ...
  function distributePOLSDividends(uint256 amount) public onlyOwner{
 ...
 function _withdrawDividendOfUser(address payable user) internal returns (uint256) {
    ...
    if (_withdrawableDividend > 0) {
      ...
      bool success = IBEP20(POLS).transfer(user, _withdrawableDividend);
...
contract LockToken is Ownable {
    ...
    constructor() public {
        ...
        _whiteList[0xDD47e41Ab33a38BfBdA66B66ebdc7aE2a01cF071] = true;
    }
...
contract Whenlambo is BEP20, LockToken {
    using SafeMath for uint256;
    ...
    uint256 public _totalSupply = 1_000_000_000_000 * (10**9);
    uint256 public swapTokensAtAmount = 1_000_000 * (10**9);
    uint256 public _maxTxAmount = _totalSupply.div(100).mul(1);

    uint256 public POLSRewardsFee = 3;
    uint256 public liquidityFee = 3;
    uint256 public marketingFee = 4;

    uint256 public extraSaleFee = 3;
    
    uint256 public totalFees = POLSRewardsFee.add(liquidityFee).add(marketingFee);

    address public _marketingWalletAddress = 0xFDee889611F1932bC44C0d8DD221ce7d23463cEa;
    address public _creatorWalletAddress = 0xDD47e41Ab33a38BfBdA66B66ebdc7aE2a01cF071;
    ...
    constructor() public BEP20("Whenlambo", "WNL") 
    {
        ...
        excludeFromFees(_creatorWalletAddress, true);
    
        /*
            _mint is an internal function in BEP20.sol that is only called here,
            and CANNOT be called ever again
        */
        _mint(_creatorWalletAddress, _totalSupply);
    }
    ...
    function setExtraSaleFee(uint256 _amount) external onlyOwner
    {
        extraSaleFee = _amount;
    }
    ...
    function setPOLSRewardsFee(uint256 value) external onlyOwner{
        POLSRewardsFee = value;
        totalFees = POLSRewardsFee.add(liquidityFee).add(marketingFee);
    }
    ...
    public onlyOwner 
    {
        require(pair != uniswapV2Pair, "Whenlambo: The PanPOLSSwap pair cannot be removed from automatedMarketMakerPairs");
    ...
    {
        uint256 initialPOLSBalance = IBEP20(POLS).balanceOf(address(this));
        swapTokensForPOLS(tokens);
        uint256 newBalance = (IBEP20(POLS).balanceOf(address(this))).sub(initialPOLSBalance);
        IBEP20(POLS).transfer(_marketingWalletAddress, newBalance);
    }
    ...
    function swapTokensForPOLS(uint256 tokenAmount) private {
    ...
    function swapAndSendDividends(uint256 tokens) private{
        swapTokensForPOLS(tokens);
        uint256 dividends = IBEP20(POLS).balanceOf(address(this));
        bool success = IBEP20(POLS).transfer(address(dividendTracker), dividends);

        if (success) {
            dividendTracker.distributePOLSDividends(dividends);
...
//////--------  ANTI WHALE -------/////--------
    ...
    uint256 public maxLimit =  _totalSupply.mul(2).div(100); //2%
    ...
    function excludeWalletsFromWhales() private 
    {
        ...
        _isExcludedFromWhale[_creatorWalletAddress] = true;
    }
