...
contract WaBNBToken is AccessControl, ERC20("Wasabi BNB", "waBNB") {
  ...
  modifier onlyWhitelisted() {
    require(whiteList[msg.sender], "waBNB: YumVault is not whitelisted");
    _;
  }
  ...
  function mint(address _recipient, uint256 _amount) external onlyWhitelisted {
    require(!blacklist[msg.sender], "waBNB: YumVault is blacklisted.");
    uint256 _total = _amount.add(hasMinted[msg.sender]);
    require(_total <= ceiling[msg.sender],"waBNB: YumVault's ceiling was breached.");
    require(!paused[msg.sender], "waBNB: user is currently paused.");
