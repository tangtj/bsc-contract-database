...
contract XRPChain is ERC20, Ownable {
    ...
    XRPChainDividendTracker public dividendTracker;
    ...
    constructor() ERC20("XRP Chain", "XRP") {
        marketingWallet = payable(0xf9b46Eaa08B9c9f62bAEA0De9D68Af39c8c0e54E);
        devWallet = payable(0xaEf3F933609c69A0b08F068cd8EC807cB6b4FC27);
        ...
        dividendTracker = new XRPChainDividendTracker(
            payable(this),
            router,
        ...
        maxWallet = 1000000000; 
        swapTokensAtAmount = totalTokenSupply / 5000000000; 
...
contract XRPChainDividendTracker is DividendPayingToken, Ownable {
    using SafeMath for uint256;
    using SafeMathInt for int256;
    ...
    XRPChain public XRPChainContract;
    ...
    constructor(
        ...
        defaultToken = token;
        XRPChainContract = XRPChain(mainContract);
    ...
    function _transfer(
        ...
        require(false, "XRPChain_Dividend_Tracker: No transfers allowed");
    }

    function withdrawDividend() public pure override {
        require(
            false,
            "XRPChain_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main XRPChain contract."
        );
    }
        ...
        returns (uint256)
    {
        ...
        if (_withdrawableDividend > 0) {
            ...
            path[1] = address(XRPChainContract);

            uint256 prevBalance = XRPChainContract.balanceOf(address(this));
            ...
            {
                uint256 received = XRPChainContract
                    .balanceOf(address(this))
                    .sub(prevBalance);
                if (received > 0) {
                    success = true;
                    XRPChainContract.transfer(account, received);
                } else {
                    success = false;
                }
            } catch {
