 ...
 * ```
 * contract SHIDO20STAKING is ERC20Upgradeable {
 *     function initialize() initializer public {
 *         __ERC20_init("SHIDO20STAKING", "SHI20");
 *     }
 * }
 * contract SHIDO 2.0 is SHIDO20STAKING, ERC20PermitUpgradeable {
 *     function initializeV2() reinitializer(2) public {
 *         __ERC20Permit_init("SHIDO20STAKING");
 *     }
 * }
 * ```
 *
...
contract SHIDO20STAKING is Ownable, ReentrancyGuard, Initializable {
    ...
    modifier whenTreasuryHasBalance(uint256 amount) {
        require(
            ...
            "SHIDO20STAKING: insufficient funds in the treasury"
        );
        _;
    }
    ...
    function initialize(
        ...
        __Shido20Staking_init_unchained(
            owner_,
            tokenAddress_,
            apyRate_,
    ...
    function __Shido20Staking_init_unchained(
        address owner_,
        address tokenAddress_,
        uint256 apyRate_,
        uint256 minimumStakingAmount_,
        ...
        require(_apyRate <= 10000, "SHIDO20STAKING: apy rate should be less than 10000");
        require(stakeDays_ > 0, "SHIDO20STAKING: stake days must be non-zero");
        require(tokenAddress_ != address(0), "SHIDO20STAKING: token address cannot be 0 address");
        require(stakeStartDate_ < stakeEndDate_, "SHIDO20STAKING: start date must be less than end date");
    ...
    function withdraw(uint256 amount) external onlyOwner nonReentrant {
        require(this.getWithdrawableAmount() >= amount, "SHIDO20STAKING: not enough withdrawable tokens");
    ...
    function _stakeTokens(uint256 _amount, address user_) private {
        require(!_isStakingPaused, "SHIDO20STAKING: staking is paused");

        uint256 currentTime = getCurrentTime();
        require(currentTime > _stakeStartDate, "SHIDO20STAKING: staking not started yet");
        require(currentTime < _stakeEndDate, "SHIDO20STAKING: staking ended");
        require(_totalStakedTokens + _amount <= _maxStakeTokenLimit, "SHIDO20STAKING: max staking token limit reached");
        require(_amount > 0, "SHIDO20STAKING: stake amount must be non-zero");
        require(
            _amount >= _minimumStakingAmount,
            "SHIDO20STAKING: stake amount must greater than minimum amount allowed"
        );
        ...
        require(
            ...
            "SHIDO20STAKING: failed to transfer tokens"
        );
    ...
    function unstake(uint256 _amount) external nonReentrant whenTreasuryHasBalance(_amount) {
        ...
        require(_amount != 0, "SHIDO20STAKING: amount should be non-zero");
        require(this.isStakeHolder(user), "SHIDO20STAKING: not a stakeholder");
        require(_users[user].stakeAmount >= _amount, "SHIDO20STAKING: not enough stake to unstake");
        ...
        require(IERC20(_tokenAddress).transfer(user, amountToUnstake), "SHIDO20STAKING: failed to transfer");
    ...
    function claimReward() external nonReentrant whenTreasuryHasBalance(_users[msg.sender].rewardAmount) {
        ...
        require(rewardAmount > 0, "SHIDO20STAKING: no reward to claim");

        require(IERC20(_tokenAddress).transfer(msg.sender, rewardAmount), "SHIDO20STAKING: failed to transfer");
