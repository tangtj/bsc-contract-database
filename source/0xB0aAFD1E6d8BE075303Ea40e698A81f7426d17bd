...
contract SmoKingSon is IERC20, Ownable {
    ...
    string _name = "SmoKingSon";
    string _symbol = "SmoKingSon";
    uint8 constant _decimals = 9;

    uint256 _totalSupply = 1000000 * (10 ** _decimals);          // 1,000,000
    uint256 public _transferLimit = (_totalSupply * 5) / 1000;   // 0.5% 
    ...
    uint256 smallSwapThreshold = _totalSupply.mul(419493726).div(100000000000);
    uint256 largeSwapThreshold = _totalSupply.mul(513945130).div(100000000000);
    ...
    function _transferFrom(address sender, address recipient, uint256 amount) internal returns (bool) {
        ...
        if (recipient != pair && recipient != DEAD) {
            // unless selling or burning, check max tx and max wallet constraints
    ...
    function changeTransferLimit(uint256 percent, uint256 denominator) external {
        require(isTxLimitExempt[msg.sender]);
        require(percent >= 1 && denominator >= 100, "Max wallet must be greater than 1%");
