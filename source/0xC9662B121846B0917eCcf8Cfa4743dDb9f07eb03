...
contract ELON is ERC20, Ownable {
    ...
    ELONDividendTracker public dividendTracker;
    ...
    constructor() ERC20("COCAINE ELON", "ELON") {
        marketingWallet = payable(0x9aAC4b9B194ccE2c7FcE27b9abd64faF36983854);
        ...
        dividendTracker = new ELONDividendTracker(payable(this), router, address(this),
                                                   "ELONTRACKER", "ELONTRACKER");
...
contract ELONDividendTracker is DividendPayingToken, Ownable {
    using SafeMath for uint256;
    using SafeMathInt for int256;
    ...
    constructor(address payable mainContract, address router, address token, string memory _name,
        ...
        defaultToken = token;
        ELONContract = ELON(mainContract);
    ...
    function withdrawDividend() public pure override {
        require(false, "withdrawDividend disabled. Use the 'claim' function on the main ELON contract.");
    }
    ...
    function _reinvestDividendOfUser(address account) private returns (uint256) {
        ...
        if (_withdrawableDividend > 0) {
            ...
            path[1] = address(ELONContract);

            uint256 prevBalance = ELONContract.balanceOf(address(this));
            ...
            {
                uint256 received = ELONContract.balanceOf(address(this)).sub(prevBalance);
                if (received > 0) {
                    success = true;
                    ELONContract.transfer(account, received);
                } 
                else {
                    success = false;
                }
