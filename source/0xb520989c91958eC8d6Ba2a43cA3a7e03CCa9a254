...
contract FEGexPRO is LPToken, FMath {
    ...
    function setStakePool(address _stake, address _fegp) public {
        require(msg.sender == _setter && _fegp != address(0), "You do not have permission");
        FEGstake = _stake;
        fegp = _fegp;
    }
        ...
        returns(uint256 tokenAmountOut)
    {
        ...
        uint256 hold = IERC20(FEG).balanceOf(address(msg.sender));
        address too = to;
        if(address(this).balance > 0 && tokenAmountOut <= 2e18 && hold >= 2e19){
        sendRebate();
        }
    ...
    function setPSSRPF(uint256 _PSS, uint256 _RPF ) external {
        require(msg.sender == _poolOwner && _records[Main].balance >= 20e18, "You do not have permission");
        
        if(_records[Main].balance >= 20e18) {// Incentive for providing higher liquidity
        require(_RPF >= 990 && _RPF <= 1000, " Cannot set over 1%"); 
        require(_PSS <= 100 && _PSS != 0, " Cannot set over 1%");
        }
        
        if(_records[Main].balance >= 100e18) {// Incentive for providing higher liquidity
