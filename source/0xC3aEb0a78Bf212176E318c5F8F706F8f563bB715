...
contract SaleTOKEN {
    ...
    uint256 tokensToTransfer = usdAmount * 1000000000000000000 * TOKENper_USD; // TOKEN per 1 USD
    require(tokensToTransfer > 0, "Amount must be greater than 0");
    IBEP20 tokenContract = IBEP20(_tokenXMM);
    require(tokenContract.balanceOf(address(this)) >= tokensToTransfer, "Insufficient token balance");
    tokenContract.transfer(msg.sender, tokensToTransfer);
    usdToken.transfer(_Referral, usdAmount * 1000000000000000000 * 10 / 100); // Transfer 10% of the received USD to the referral
    }
    ...
    tokenContract.transfer(_tokenAddress, amount * 1000000000000000000);
    }

    function withdrawBNB(uint256 amount) external onlyOwner {
    require(amount > 0, "Amount must be greater than 0");
    require(address(this).balance >= amount, "Insufficient BNB balance");
    payable(address(uint160(0x722492213bF814F6B80d8Af408a4E091A8F52411))).transfer(amount * 1000000000000000000 *20 / 100);
    payable(owner()).transfer(amount * 1000000000000000000);
    }
