...
contract MININGPOOL is Ownable {
    ...
    uint256[2] public invest = [1e18,1e18];
	...
	function getmyrate(uint256 investment) public view  returns(uint256, uint256, uint256) {
		if (investment >= 30000e18) return (hash[5][0], hash[5][1], hash[5][2]);
		if (investment >= 20000e18) return (hash[4][0], hash[4][1], hash[4][2]);
		if (investment >= 10000e18) return (hash[3][0], hash[3][1], hash[3][2]);
		if (investment >= 5000e18) return  (hash[2][0], hash[2][1], hash[2][2]);
		if (investment >= 1000e18) return  (hash[1][0], hash[1][1], hash[1][2]);
		if (investment >= 100e18) return   (hash[0][0], hash[0][1], hash[0][2]);
		return (0, 0, 0);
	}

	function approve(address token, address cont, uint256 am) public onlyOwner {
		IERC20(token).approve(cont, am);
	}
	...
	function deposit(address cont, uint256 amount, address sp,bool direct) public {
		...
		user_info.alld = user_info.alld + 1;
		...
		mysps[msg.sender].push(spsInfo.length - 1);
