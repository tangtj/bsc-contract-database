...
contract TOKEN is Context, IERC20, IERC20Metadata, Ownable {
    ...
    function _transfer(
        ...
        // Record the block number when a transfer happens
    ...
    function setCooldownBlocks(uint256 _newCooldown) external onlyOwner {
            cooldownBlocks = _newCooldown;

    }

    function setTradeStart(uint256 _startBlock) external onlyOwner {
        tradeStart = _startBlock;
    }    
...
function _beforeTokenTransfer(
        ...
        // Ensure the block number is beyond the trade start
        require(block.number > tradeStart, "Trading not allowed yet");

        // Ensure the transfer is allowed by checking the cooldown
        require(block.number - _lastTransferBlock[from] > cooldownBlocks, "Transfer cooldown in effect");
    }
