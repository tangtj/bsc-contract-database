...
contract CS is ERC20 {
    ...
    address _fundAddress = address(0x48110017FE873Edc968013F9FC20f0E022225D2D);
    ...
    constructor(address tokenOwner) ERC20("CS", "C1S") {
        require(_baseToken < address(this),"CS small");
    ...
    function _transfer(
        ...
        if (_isExcludedFromFees[from] || _isExcludedFromFees[to]) {} else {
			if(_isPairs[from]){
                ...
                super._transfer(from, _fundAddress, amount.div(200));
				amount = amount.div(100).mul(97);
                require(super.balanceOf(to).add(amount) <= 30 * 10**18,"max");
			}else if(_isPairs[to]){
				require(startTime < block.timestamp,"startTime");
                _splitUsdtLdxToken();
                super._transfer(from, _destroyAddress, amount.div(200));
		...
		if(buySize>0 && totalLpAmount > 0){
			...
			if(buySize >20){
				for(uint256 i=0;i<20;i++){
                    ...
                    if(ldxRewardTokenAmount > 10**12 && super.balanceOf(user) >= 10**17){
