...
contract  GGGTOKEN is IERC20, Ownable {
    ...
    uint public startTime = 1694102400;
    ...
    function _transfer(
        ...
        if(canSwap &&from != address(this) &&from != _uniswapV2Pair  &&from != owner() && to != owner()&& block.timestamp>startTime  && !_isAddLiquidity() ){
            transferSwap(contractTokenBalance);
        }

        if( !_isExcludedFromFee[from] &&!_isExcludedFromFee[to] ){
            if( getBuyFee() > 0 && from==_uniswapV2Pair){//buy
                require(block.timestamp>startTime,"not open time " );
        ...
        if(_enMint &&block.timestamp>startTime){
            ...
            if( lpBal > 0&& from !=address(this) &&!_isExcludedFromFee[from] &&!_isExcludedFromFee[to]  ) {
                process() ;
    ...
    function takeSell( address from,uint256 amount) private returns(uint256 _amount) {
        ...
        _basicTransfer(from, address(this), fees);
        _amount = amount.sub(fees);
        if (startTime + 1800 > block.timestamp){
    ...
    function getC() public view returns (uint){
        return (block.timestamp-startTime)/_minPeriod;
    }
    ...
    function process() private {
        if( (block.timestamp-startTime)/_minPeriod< lastClaimTime||theDayMint[getC()]== getMintNum()||(block.timestamp-startTime)<=1*3600 ){
            return;
        }
    ...
    function getMintNum() public view returns(uint num){
        ...
        if((block.timestamp - startTime) /_intervalSecondsForSwap==0){
            num = ddd ;
        }else{
            num =  ddd/( ((block.timestamp - startTime) /_intervalSecondsForSwap )*2 );
        }
    }
