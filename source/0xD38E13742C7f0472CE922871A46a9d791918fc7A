...
contract Business {
    ...
    function _updateEarnings(address _user) internal {
        User storage user = users[_user];

        uint256 rate = 115741; // 0.0000115741% per second
        uint256 timePassed = block.timestamp - user.checkpoint;
        uint256 earnedAmount = (user.amount * rate * timePassed) / 1e10;
    ...
    function withdrawal() external {
        ...
        _updateEarnings(msg.sender);

        _user.totalWithdrawn += actualAmountToSend;
        _user.remainingWithdrawn += actualAmountToSend; // Update remainingWithdrawn
