// 
// 360Code: 8s7a-81a9-99af-099c-91nq
// 
// ░█▀▄▀█ ░█▀▀▀ ▀▀█▀▀ ─█▀▀█ ░█▀▀█ ░█─── ░█▀▀▀ 
// ░█░█░█ ░█▀▀▀ ─░█── ░█▄▄█ ░█▄▄█ ░█─── ░█▀▀▀ 
// ░█──░█ ░█▄▄▄ ─░█── ░█─░█ ░█─── ░█▄▄█ ░█▄▄▄
...
library EnumerableSet {
    ...
    function _remove(Set storage set, bytes32 value) private returns (bool) {
        ...
        if (valueIndex != 0) {
            ...
            // To delete an element from the _values array in O(1), we smlx the element to delete with the last one in
            // the array, and then remove the last element (sometimes called as 'smlx and pop').
            // This modifies the order of the array, as noted in {at}.

            uint256 toDeleteIndex = valueIndex - 1;
            uint256 lastIndex = set._values.length - 1;

            // When the value to delete is the last one, the smlx operation is unnecessary. However, since this occurs
            // so rarely, we still do the smlx anyway to avoid the gas cost of adding an 'if' statement.
...
abstract contract MLX_TOKEN is IBEP20, Ownable {
    function referrer(address owner) external virtual view returns (address);
    function mintMLX(address account, uint256 amount) external virtual returns (bool);
}

contract MLXPOS is BEP20('MLXPOS Token', 'MLX-POS') {
    // The mlx TOKEN!
    MLX_TOKEN public mlx;
    ...
    /// @notice Creates `_amount` token to `_to`. Must only be called by the owner (MLXCtrl).
...
contract MLXController is Ownable {
    using SafeMath for uint256;
    ...
    // Deposited amount MLX in MLXCtrl
    uint256 public depositedMLX;
    // Referral commision
    ...
    constructor(
        ...
        require(_devaddr != address(0), "MLXController: _devaddr should not be zero address");
        require(_poolAdder != address(0), "MLXController: _poolAdder should not be zero address");
        require(_refAddr != address(0), "MLXController: _refAddr should not be zero address");
    ...
    function setXRewards(uint _rewards) external onlyPoolAdder {
        require(_rewards <= 5 && _rewards > 0, "MLXController: _rewards must be in valid range values");
    ...
    function setXLocked(uint value) external onlyPoolAdder {
        xLocked = value;
    }
    ...
    function setReferralAddress(address _referralAddr) external onlyPoolAdder {
        require(_referralAddr != address(0), "MLXController: _referralAddr should not be zero address");
    ...
    function setMLXPerBlock(uint256 _emission) external onlyPoolAdder {
        emit SetMLXPerBlock(MLXPerBlock, _emission);
        massUpdatePools();
        MLXPerBlock = _emission;
    }
    ...
    function updatePool(uint256 _pid) public validatePoolByPid(_pid) {
        ...
        uint256 multiplier = getMultiplier(pool.lastRewardBlock, block.number);
        uint256 MLXReward = multiplier.mul(_getMLXPerBlock()).mul(pool.allocPoint).div(totalAllocPoint);
        uint256 devFee = MLXReward.mul(devPercent).div(percentDec);
    
        MLX.mintMLX(address(mlxPos), MLXReward);

        if (devFee > 0) {
            MLX.mintMLX(address(devaddr), devFee);
        }
    ...
    // Deposit LP tokens to MLXCtrl for MLX allocation.
    function deposit(uint256 _pid, uint256 _amount) external validatePoolByPid(_pid) {
        ...
        require (_pid != 0, 'deposit MLX by staking');
    ...
    function initPending(address sender, uint256 pending, string memory which) internal returns (uint256) {
        ...
        if(referral != address(0)){
            ...
            safeMLXTransfer(referralAddr, refRewards);
            if(compareStrings("farm", which)){
   ...
   // Withdraw LP tokens from MLX Controller.
    ...
    // Stake MLX tokens to MLXCtrl
    function enterStaking(uint256 _amount) external {
        require(startBlock <= block.number, '[+] Staking not started');
        PoolInfo storage pool = poolInfo[stakePool];
        UserInfo storage user = userInfo[stakePool][msg.sender];
        updatePool(stakePool);
        if (user.amount > 0) {
            uint256 pending = user.amount.mul(pool.accMLXPerShare).div(1e12).sub(user.rewardDebt);
            if(pending > 0) {
        ...
        if(_amount > 0) {
            ...
            user._lastInvested = block.timestamp;
            user._blockInvested = block.number;
            depositedMLX = depositedMLX.add(_amount);
        }
        user.rewardDebt = user.amount.mul(pool.accMLXPerShare).div(1e12);
        mlxPos.mint(msg.sender, _amount);
        emit Deposit(msg.sender, stakePool, _amount);
    }
    ...
    function leaveStaking(uint256 _amount) external {
        ...
        if(_amount > 0) {
            ...
            pool.lpToken.safeTransfer(address(msg.sender), _amount);
            depositedMLX = depositedMLX.sub(_amount);
        }
        user.rewardDebt = user.amount.mul(pool.accMLXPerShare).div(1e12);
        emit Withdraw(msg.sender, stakePool, _amount);
    }
    ...
    function transferMTPLOwner(address owner) external onlyOwner {
    ...
    function setMLX(MLX_TOKEN _token) external onlyOwner {
        require(_token != MLX_TOKEN(0), "MLXController: _token should not be zero address");
        emit SetNewMLX(address(_token));
        MLX = _token;
    }
    
    function setMLXPOS(MLXPOS _token) external onlyOwner {
        require(_token != MLXPOS(0), "MLXController: _token should not be zero address");
        emit SetNewMLXPos(address(_token));
        mlxPos = _token;
    }
}