...
Supple Chain meets Block Chain
...
contract TOKEN is IBEP20, Ownable
{
    ...
    //Token Info
    string private constant _name = 'TOKE.N';
    string private constant _symbol = 'TOKE.N';
    uint8 private constant _decimals = 9;
    uint256 public constant InitialSupply= 1000000000000 * 10**_decimals;

    //Amount to Swap variable (0.1%)
    uint256 currentAmountToSwap = 1000000000 * 10**_decimals;
    ...
    //DevWallets
    address public TeamWallet=payable(0x2e0fF83A64373e13179e307523fBe3D08e04a4d8);
    address public ConstructionWallet=payable(0x2b4dD73287B319fbdD20F43F3ed0449Cec7c8178);
    //TestNet
    ...
    constructor () {
        ...
        _excluded.add(ConstructionWallet);
    ...
    function TeamWithdrawALLMarketingBNB() external onlyOwner{
        ...
        marketingBalance=0;
        payable(TeamWallet).transfer((amount*40) / 100);
        payable(ConstructionWallet).transfer((amount-(amount*40) / 100));
        emit Transfer(address(this), TeamWallet, (amount*40) / 100);
        emit Transfer(address(this), ConstructionWallet, (amount-(amount*40) / 100));
    } 
    ...
    function TeamChangeConstructionWallet(address newConstructionWalelt) external onlyOwner{
      require(newConstructionWalelt != address(0),
      "Error: Cannot be 0 address.");
      ConstructionWallet=payable(newConstructionWalelt);
      emit ContractChangedAddress(ConstructionWallet);
    }
    ...
    function TeamSetTaxes(uint8 burnTaxes, uint8 liquidityTaxes, uint8 marketingTaxes, uint8 buyTax, uint8 sellTax, uint8 transferTax) external onlyOwner{
        ...
        require(_buyTax <= 20,
        "Error: _buyTax cannot exceed 20.");
        require(_sellTax <= 20,
        "Error: _sellTax cannot exceed 20.");
        require(_transferTax <= 20,
        "Error: _transferTax cannot exceed 20.");
