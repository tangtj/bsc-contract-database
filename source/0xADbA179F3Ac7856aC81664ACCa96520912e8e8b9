...
contract CStorage {
    ...
    address public bombPriceOracle;
...
// File contracts/interfaces/IBombPriceOracle.sol

pragma solidity >=0.5.0;

interface IBombPriceOracle {
    event PriceUpdate(
...
contract CSetter is PoolToken, CStorage {
    ...
    function _initialize(
        ...
        bombPriceOracle = IFactory(factory).bombPriceOracle();
    }
...
contract Collateral is ICollateral, PoolToken, CStorage, CSetter {
    ...
    function getPrices() public returns (uint256 price0, uint256 price1) {
        (uint224 twapPrice112x112, ) = IBombPriceOracle(bombPriceOracle)
         ...
         */
        require(price0 > 100, "Bomb: PRICE_CALCULATION_ERROR");
        require(price1 > 100, "Bomb: PRICE_CALCULATION_ERROR");
    }
    ...
    function _transfer(
        ...
        require(tokensUnlocked(from, value), "Bomb: INSUFFICIENT_LIQUIDITY");
    ...
    function seize(
        ...
        require(shortfall > 0, "Bomb: INSUFFICIENT_SHORTFALL");
    ...
    function flashRedeem(
        ...
        _safeTransfer(redeemer, redeemAmount);
        if (data.length > 0)
            IBombCallee(redeemer).bombRedeem(msg.sender, redeemAmount, data);
        ...
        require(
            ...
            "Bomb: INSUFFICIENT_REDEEM_TOKENS"
        );
