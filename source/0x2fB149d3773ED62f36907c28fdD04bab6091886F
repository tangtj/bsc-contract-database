...
contract X is IBEP20, Ownable {
    ...
    mapping (address => bool) public isTimelockExempt; // No Wait Time X
    mapping (address => uint) public cooldownTimerBuy; // Buy time X
    mapping (address => uint) public cooldownTimerSell; // Sell Time X
    /*=== Unitary ===*/
    ...
    uint256 private _tSupply = 1000 * _decimalFactor; // Supply Token X
    uint256 public buyFee = 10; // Buy tax
    uint256 private previousBuyFee; // Stores Purchase Fee X
    uint256 public sellFee = 10; // Sell Tax X
    uint256 private previousSellFee; // Stores Sales Tax
    uint256 private limitBurn = 40 * _decimalFactor; // Burn 4% BURN BASIC LOL
    uint256 public liquidityPercent = 2; // Liq return 2% BASIC
    uint256 public maxWalletBalance = 200 * _decimalFactor; // 2% of Supply BASIC
    uint256 public maxBuyAmount = 200 * _decimalFactor; // 2% of Supply X
    uint256 public maxSellAmount = 200 * _decimalFactor; // 2% of Supply X
    uint256 public numberOfTokensToSwapToLiquidity = 10 * _decimalFactor; // 1% do Supply
    /*=== Boolean ===*/
