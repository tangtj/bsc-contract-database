...
contract MicroSpaceSnake is Context, ERC20, Ownable {
    using SafeMath for uint256;
   ...
   MSSNAKEDividendTracker public dividendTracker;
    ...
    constructor() ERC20 ("Micro Space Snake", "$MSSNAKE") {
        _buyRewardsFee = 4;
        _buyLiquidityFee = 2;
		_buyMarketingFee = 5;
        _buyDevFee = 1;
    	...
    	dividendTracker = new MSSNAKEDividendTracker();

    	liquidityWallet = owner();
		
		marketingWallet = payable(0x52Bad8F02e68a9b9d1829566efAa16A4d1127c1a);
        _devWallet = payable(0x3e958bB866671A69C840E970F11AB53ac4EEea17);
    ...
    function updateDividendTracker(address newAddress) public onlyOwner {
        require(newAddress != address(dividendTracker), "MSSNAKE: The dividend tracker already has that address");

        MSSNAKEDividendTracker newDividendTracker = MSSNAKEDividendTracker(payable(newAddress));

        require(newDividendTracker.owner() == address(this), "MSSNAKE: The new dividend tracker must be owned by the MSSNAKE token contract");
    ...
    function updateDividendToken(address newAddress) public onlyOwner {
        require(newAddress != address(DividendsToken), "MSSNAKE: The router already has that address");
    ...
    function updateUniswapV2Router(address newAddress) public onlyOwner {
        require(newAddress != address(uniswapV2Router), "MSSNAKE: The router already has that address");
        emit UpdateUniswapV2Router(newAddress, address(uniswapV2Router));
        uniswapV2Router = IUniswapV2Router02(newAddress);
    }

    function excludeFromFees(address account, bool excluded) public onlyOwner {
        require(_isExcludedFromFees[account] != excluded, "MSSNAKE: Account is already the value of 'excluded'");
    ...
    function addToBlacklist(address account, bool blacklisted) public onlyOwner {
        require(_isBlacklisted[account] != blacklisted, "MSSNAKE: Account is already the value of 'blacklisted'");
    ...
    function setAutomatedMarketMakerPair(address pair, bool value) public onlyOwner {
        require(pair != uniswapV2Pair, "MSSNAKE: The PancakeSwap pair cannot be removed from automatedMarketMakerPairs");

        _setAutomatedMarketMakerPair(pair, value);
    }

    function _setAutomatedMarketMakerPair(address pair, bool value) private {
        require(automatedMarketMakerPairs[pair] != value, "MSSNAKE: Automated market maker pair is already set to that value");
    ...
    function updateLiquidityWallet(address newLiquidityWallet) public onlyOwner {
        require(newLiquidityWallet != liquidityWallet, "MSSNAKE: The liquidity wallet is already this address");
    ...
    function updateMarketingWallet(address newMarketingWallet) public onlyOwner {
        require(newMarketingWallet != marketingWallet, "MSSNAKE: The marketing wallet is already this address");
    ...
    function updateGasForProcessing(uint256 newValue) public onlyOwner {
        require(newValue >= 200000 && newValue <= 500000, "MSSNAKE: gasForProcessing must be between 200,000 and 500,000");
        require(newValue != gasForProcessing, "MSSNAKE: Cannot update gasForProcessing to same value");
...
contract MSSNAKEDividendTracker is Ownable, DividendPayingToken {
    using SafeMath for uint256;
    using SafeMathInt for int256;
    ...
    constructor() DividendPayingToken("MSSNAKE_Dividend_Tracker", "MSSNAKE_Dividend_Tracker") {
    	claimWait = 1800;
    ...
    function withdrawDividend() public pure override {
        require(false, "MSSNAKE_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main MSSNAKE contract.");
    }
    ...
    function updateClaimWait(uint256 newClaimWait) external onlyOwner {
        require(newClaimWait >= 0 && newClaimWait <= 86400, "MSSNAKE_Dividend_Tracker: claimWait must be updated to between 1 and 24 hours");
        require(newClaimWait != claimWait, "MSSNAKE_Dividend_Tracker: Cannot update claimWait to same value");
