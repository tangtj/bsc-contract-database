...
contract PreSaleManager is Ownable {
    ...
    function addTokenToCurrentPrice(uint256 aumont)public onlyOwner { 
        require(_tokensToSale+aumont-_tokensSold<=movecoin.balanceOf(address(this)),"the contract has less tokens than the number of tokens for sale.");
        _tokensToSale+=aumont;
    }
    function startPresale(uint256 tokenPerDollar, uint256 tokensToSaleQuantity, uint256 indicationPercent)public onlyOwner {        
        presaleEnabled = true;  
        _tokensPerDollar = tokenPerDollar; 
        _tokensSold=0;   
        _tokensToSale= tokensToSaleQuantity;    
        _indicationPercent = indicationPercent;
    }
