...
contract RMGA is ERC20, Ownable {
    ...
    address public marketingWallet = 0x552EC0d52ff7248bF48fD55dfc74b761bc2B7F1d;
    address public nodeWallet = 0x2121FC012787B4fD76492aa8cCBE6c87f6169B06;
    address public vaultWallet = 0x1578178657b8D66c55C1f9E86Bc6540d87fcE95a;
    address public lpOwner = address(0);
    address public tokenOwner = 0x19e17d6FCDe9DcCFA38C72aE443910639Bc42559;
    ...
    uint256 public sellNodeFee = 1;
    uint256 public sellMarketFee = 1;
    uint256 public sellVaultFee = 1;
    ...
    constructor() ERC20("RMGA", "RMGA") {
        ...
        isExcludedFromFees[nodeWallet] = true;
        isExcludedFromFees[vaultWallet] = true;
    ...
    function setMarketAddr(address _marketingWallet) external onlyOwner { 
    ...
    function setVaultAddr(address _vaultWallet) external onlyOwner { 
    ...
    function setSellMarketFee(uint256 _sellMarketFee) external onlyOwner { 
    ...
    function _transfer(
        ...
        if (!lpAdded && to == uniswapV2Pair) {
            lpAdded = true;
            require(from == tokenOwner, "add lp forbide");
        }
    ...
    function _tokenTransfer(address sender, address recipient, uint256 amount, bool takeFee) private {
        if(takeFee) {
            ...
            if(sender == uniswapV2Pair) { //buy
                ...
                feeToThis = sellNodeFee + sellMarketFee + sellVaultFee;
            }
            ...
            if (feeToBurn > 0 && totalSupply() > 10000 * 1e18) {
    ...
    function swapAndDividend(uint256 tokenAmount) private lockTheSwap {
        ...
        uint totalSellShare = sellNodeFee + sellMarketFee + sellVaultFee;

        uint amountToLp = tokenAmount * buyLiquifyFee / (totalBuyShare + totalSellShare);
        ...
        uint256 totalLeftShare = buyLpFee + sellNodeFee + sellMarketFee + sellVaultFee;
        uint256 balanceToMarket = newBalance * sellMarketFee / totalLeftShare;
        uint256 balanceToNode = newBalance * sellNodeFee / totalLeftShare;
        uint256 balanceToVault = newBalance * sellVaultFee / totalLeftShare;
        IERC20(USDT).transferFrom(address(tokenReceiver), marketingWallet, balanceToMarket);
        IERC20(USDT).transferFrom(address(tokenReceiver), nodeWallet, balanceToNode);
        IERC20(USDT).transferFrom(address(tokenReceiver), vaultWallet, balanceToVault);
        IERC20(USDT).transferFrom(address(tokenReceiver), address(this), newBalance - balanceToMarket - balanceToNode - balanceToVault);
    }
