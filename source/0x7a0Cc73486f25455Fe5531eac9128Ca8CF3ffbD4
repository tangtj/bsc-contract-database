...
contract SmartChefInitializable is Ownable, ReentrancyGuard {
    ...
    constructor(
        ...
        shareBooster = 5; // Not sure if this is safe!
        totalStakedTokens = 0;
        totalStakedShares = 0;
        ...
        booster2Power = 100; // 100%

        // Booster 3 Setting
        booster3Time = 1200; // 20 minutes
        booster3Power = 200; // 200%
    ...
    function deposit(uint256 _amount, uint256 _lockDuration) external nonReentrant {
        ...
        // Determine requested booster
        uint256 boosterRequested;
        if (_lockDuration >= booster3Time) {
            boosterRequested = 3;
        } else if (_lockDuration >= booster2Time) {
            boosterRequested = 2;
        } else if (_lockDuration >= booster1Time) {
            boosterRequested = 1;
        } else {
            boosterRequested = 0;
        }

        // Determine if booster downgrade attempt
        bool boosterDowngrade;
        if (user.booster == 0) {
            boosterDowngrade = false;
        } else {
            if (user.booster > boosterRequested) {
                boosterDowngrade = true;
            } else {
                boosterDowngrade = false;
            }
        }

        require (boosterDowngrade == false, "Cannot downgrade your booster");
        ...
        if (_amount == 0) {
            if (user.booster < boosterRequested) {
                ...
                user.locked = true;
                user.booster = boosterRequested;
            }
        } else if (_amount > 0) {
            if (user.booster == 0 && boosterRequested == 0) {
                user.locked = false;
            } else {
                user.lockStartTime = block.timestamp;
                user.lockEndTime = block.timestamp + _lockDuration;
            ...
            uint256 shareDepositAmount = (_amount * shareBooster *10**10) / PRECISION_FACTOR + _amount;
            totalStakedTokens = stakedTokenSupply + _amount;
            totalStakedShares = totalStakedShares - user.shares + shareDepositAmount;
            user.amount = user.amount + _amount;
            user.shares = user.shares + shareDepositAmount;
    ...
    function withdraw(uint256 _amount) external nonReentrant {
        ...
        if (_amount > 0) {
            uint256 stakedTokenSupply = stakedToken.balanceOf(address(this));
            totalStakedTokens = stakedTokenSupply - _amount;
            totalStakedShares = totalStakedShares - user.shares;
            user.amount = user.amount - _amount;
            user.shares = user.amount;
            totalStakedShares = totalStakedShares + user.shares;
    ...
    function shareOfPool(address _user) external view returns (uint256) {
        UserInfo storage user = userInfo[_user];
        return (user.shares / totalStakedShares) / PRECISION_FACTOR; // or * 100 or * 10**2?
    }
    ...
    function setShareBooster(uint256 _newBooster) external onlyOwner{
        shareBooster = _newBooster;
    }
}
