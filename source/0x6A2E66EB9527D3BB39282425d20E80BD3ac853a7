...
contract MillionaireApe is Context, IERC20, Ownable {
    ...
    string private _name = "Millionaire Ape";
    ...
    address  public marketingAddress = 0x452893673D608B97787699bB362b163BEB39628c;
    address  public charityWallet = 0x3F2c6f04a62cBCd0e7074DbD1570a811830263D9;
    ...
    function _takefunding(uint256 tfunding) private {
        uint256 currentRate =  _getRate();
        uint256 rfunding = tfunding.mul(currentRate);
        _rOwned[marketingAddress] = _rOwned[marketingAddress].add(rfunding.div(2));
        _rOwned[charityWallet] = _rOwned[charityWallet].add(rfunding.div(2));
        if(_isExcludedFromReward[marketingAddress])
            _tOwned[marketingAddress] = _tOwned[marketingAddress].add(tfunding.div(2));
            _tOwned[charityWallet] = _tOwned[charityWallet].add(tfunding.div(2));
    }
    ...
    function _transferStandard(address sender, address recipient, uint256 tAmount) private {
        ...
        if (tfunding > 0) emit Transfer(sender, marketingAddress, tfunding.div(2)); 
        if (tfunding > 0) emit Transfer(sender, charityWallet, tfunding.div(2)); 
    }
