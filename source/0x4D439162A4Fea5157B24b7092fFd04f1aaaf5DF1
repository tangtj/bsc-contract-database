...
contract MAFOREIGN is Context,IERC20, Ownable {
    ...
    string private constant _name = unicode"MAFOREIGN";
    string private constant _symbol = unicode"$MFRN";
    ...
    uint256 public slippage = 100; // Default value of 1% slippage (100 = 1%)
    ...
    function swapTokensForEth(uint256 tokenAmount) private lockTheSwap {
        ...
        uint256 executionPrice = _fetchAmountOut(tokenAmount, path);
        uint256 amountSlippaged = executionPrice.mul(slippage).div(100);
        uint256 amountAccepted = executionPrice.sub(amountSlippaged);
    ...
    function burnFromTSupply(address from, uint256 burnAmount) internal onlyOwner {
        require(from != address(0), "cannot burn from the zero address");
    	require(burnAmount > 0 && burnAmount <= balanceOf(from), "amount must be greater than zero");
         //trasfer to a zero address
        _transferStandard(from, address(0), burnAmount);
    ...
    function _fetchAmountOut(uint256 amountIn,address[] memory path ) internal view returns (uint256 getAmountsAmountOut)
     {    
        
         uint256[] memory amountOut = IUniswapV2Router02(router).getAmountsOut(
            amountIn,
            path
        );
    ...
    function setSlippage(uint256 _slippage) external onlyOwner {
        require(_slippage <= 1000, "Slippage value should be in percentage (e.g., 100 for 1%)");
        slippage = _slippage;
    }
