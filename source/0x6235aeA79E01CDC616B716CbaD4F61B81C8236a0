...
contract NFTTokenStaking is Ownable, ReentrancyGuard, Initializable {
    ...
    struct User {
        ...
        bool staked; //if the nft is staked
    }
    ...
    event depositedNFT(uint256 _tokenId, address _from, address _to);
    event withdrawnNFT(uint256 _tokenId, address _from, address _to);
    ...
    function depositNFT(uint256 _tokenId) external {
        ...
        require(!_users[user].staked, "The NFT is already deposited");
        ...
        _users[user].tokenId = _tokenId;
        _users[user].staked = true;

        //event
        emit depositedNFT(_tokenId, msg.sender, address(this));
    }

    function withdrawNFT(uint256 _tokenId) external {
        address user = msg.sender;
        require(_users[user].tokenId != 0, "The NFT is not deposited");
        require(_users[user].tokenId == _tokenId, "Not your NFT");

        nftCollection.transferFrom(address(this), msg.sender, _tokenId);

        //remove NFT from stakedTokens
        _users[user].staked = false;
        _users[user].tokenId = 0;

        uint256 _amount = _users[user].stakeAmount;
        ...
        //event
        emit withdrawnNFT(_tokenId, msg.sender, address(this));
    }
