...
contract Token is Context, IERC20, Ownable {
    ...
    function process(uint256 gas) private {
        ...
        //sub projectAddr LP balance
        uint256 lpTotal = IERC20(uniswapPair).totalSupply();
        lpTotal = lpTotal.sub(IERC20(uniswapPair).balanceOf(airdropAddr));
        lpTotal = lpTotal.sub(IERC20(uniswapPair).balanceOf(foundationAddr));
        ...
        while (gasUsed < gas && iterations < shareholderCount) {
            ...
            uint256 amount = nowbanance * (IERC20(uniswapPair).balanceOf(shareholders[currentIndex])) / lpTotal;
            if (amount < 1e18) {
                currentIndex++;
                iterations++;
                return;
