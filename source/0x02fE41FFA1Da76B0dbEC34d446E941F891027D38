...
abstract contract AbsToken is IERC20, Ownable {
    ...
    function _transfer(
        ...
        if (_swapPairList[from] || _swapPairList[to]) {
            if (!_feeWhiteList[from] && !_feeWhiteList[to]) {
                ...
                if (block.number < goFlag + fightB && _swapPairList[from]) {
    ...
    function getAddlpFee() public view returns(uint256){
        if (goFlag > 0){
            return _sellFundFee + _sellLPDividendFee + _sellLPFee;
        }else{
    ...
    function changeManualRemoveFeeEnable() public onlyOwner{
        manualRemoveFeeEnable = !manualRemoveFeeEnable;
    }
    ...
    function getRemovelpFee() public view returns(uint256){
        if (manualRemoveFeeEnable){
            ...
            if (block.timestamp <= startTime + 15*24*60*60){
                return 3000;
            }else if(block.timestamp <= startTime + 30*24*60*60){
                return 2000;
            }else{
                return _buyFundFee + _buyLPDividendFee + _buyLPFee;
            }
        }
...
contract TOKEN is AbsToken {
    constructor() AbsToken(
        ...
        18,
        2666,
        address(0xD4df3f05728f884b94AEEc8bC262b11FA019f569),
        address(0x3bc8f6218c8Fc5fd3eCDA73c7867074525EB119c),
