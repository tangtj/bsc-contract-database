...
contract Presale is Ownable {
    ...
    uint256 tokenPrice = 240000; // The price of a single token in BNB (in wei)

   // Check if bnbAmount is in the valid range (from 0.01 to 10 BNB)
    require(bnbAmount >= 0.01 ether && bnbAmount <= 10 ether, "Amount not within limits");

    // Calculate token amount to be transferred based on the exchange rate
    uint256 tokenAmount = (bnbAmount * 10**18) / tokenPrice;
    ...
    if (msg.sender != _refer && _refer != address(0)) {
        ...
        uint256 referEth = (bnbAmount * _referEth) / 10000;
        payable(_refer).transfer(referEth);
    }

    // Calculate and transfer developer, team, advertising, and marketing rewards in ETH
    uint256 developerReward = (bnbAmount * _developerReward) / 10000;
    uint256 teamReward = (bnbAmount * _teamReward) / 10000;
    uint256 advertisingReward = (bnbAmount * _advertisingReward) / 10000;
    uint256 marketingReward = (bnbAmount * _marketingReward) / 10000;
