...
contract Metlaxes is ReentrancyGuard, Ownable {
    ...
    function activateAccount(uint256 packageAmount) public {
    ...
    function upgradeAccount(uint256 packageAmount) public {
        require(msg.sender != address(0), "Invaild wallet address!");
        require(token.allowance(msg.sender, address(this)) >= packageAmount, "The contract does not have enough of an allowance to escrow!");
        token.transferFrom(msg.sender, receivingAddress, packageAmount);        
        emit UpgradeAccount(msg.sender, packageAmount, block.timestamp);
    }
