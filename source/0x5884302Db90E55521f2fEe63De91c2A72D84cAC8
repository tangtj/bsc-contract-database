...
contract RoflcoinAirdrop is Ownable {
    ...
    uint256 public referralBonusPercentage = 30;
    ...
    function claimAirdropWithReferral(string memory referralCode) external payable isEligibleForAirdrop {
        ...
        ERC20(roflcoinTokenAddress).transfer(msg.sender, airdropAmount);

        // Update referral bonus
        if (bytes(referralCode).length > 0) {
            address referrer = address(bytes20(keccak256(abi.encodePacked("https://roflcoin.com/", referralCode))));
    ...
    function distributeTokens(address recipient, uint256 amount) external onlyOwner {
        ...
        require(ERC20(roflcoinTokenAddress).balanceOf(address(this)) >= amount, "Insufficient contract balance");

        // Transfer the specified amount of tokens to the recipient
        ERC20(roflcoinTokenAddress).transfer(recipient, amount);
    ...
    function withdrawROFL() external onlyOwner {
        uint256 remainingROFL = ERC20(roflcoinTokenAddress).balanceOf(address(this));
        require(remainingROFL > 0, "Contract has no remaining ROFL balance");
        ERC20(roflcoinTokenAddress).transfer(owner(), remainingROFL);
    }
}