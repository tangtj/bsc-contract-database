...
contract MUSKVSZUCK is ERC20, Ownable {
    ...
    constructor(
        ...
        address owner,
        address mintToAddr
    ) ERC20('MUSK VS ZUCKL', 'MVZ') {
        require(
            feeAccount != address(0),
            'MUSKVSZUCK: fee account from the zero address'
        );
        require(feePercent <= PRECISION, 'MUSKVSZUCK: incorrect fee percent');
        require(owner != address(0), 'MUSKVSZUCK: incorrect owner address');
        require(mintToAddr != address(0), 'MUSKVSZUCK: incorrect mintToAddr address');
        if (_msgSender() != owner) {
    ...
    function _transfer(
        ...
        require(sender != address(0), 'MUSKVSZUCK: transfer from the zero address');
        require(recipient != address(0), 'MUSKVSZUCK: transfer to the zero address');
