...
contract PledgeEafAndOsk{
    ...
    address public _osk = address(0x3bbadcE24FC5258D70F9fF6ee1C22DD45864428b);
    ...
    IPair public _pairOsk = IPair(0x470A47e71DA6fe907d4C48D499c3465d1151D13A);
    ...
    function pledge(uint256 orderId, uint256 amount, bytes calldata signature) external onlyValidRecharge(orderId, amount, signature){
        require(amount > 0, "Recharge quantity must be greater than zero");
        uint256 oAmount = calcOsk(amount);
        IBEP20(_osk).transferFrom(msg.sender, _to, oAmount);
    ...
    function calcOsk(uint256 uAmount) public view returns(uint256){
        (uint256 r0, uint256 r1, ) = _pairOsk.getReserves();
        (uint256 ru, uint256 ro) = _usdt == _pairOsk.token0() ? (r0, r1) : (r1, r0);
