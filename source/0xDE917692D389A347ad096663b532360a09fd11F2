...
contract SafuToken is Context, IERC20 {
    ...
    uint private ReflectionFeeonBuy=0;
    uint private ReflectionFeeonSell=0;

    uint private liquidityFeeonBuy=0;
    uint private liquidityFeeonSell=0;

    uint public marketingFeeonBuy=20;
    uint public marketingFeeonSell=20;

    uint private burnFeeOnBuy=0;
    uint private burnFeeOnSell=0;
    ...
    uint256 public maxTransactionAmountBuy=40000000 * (10 ** decimals());
    uint256 public maxTransactionAmountSell=40000000 * (10 ** decimals());

    
    uint256 public maxWalletAmount=40000000 * (10 ** decimals());
    ...
    require(msg.value >= 0 ether, "Insufficient value for fee receiver");
    
    
    serviceFee = 0.00000000000000000 ether;
    ...
    function lockOnUNCX(address _uncxAddress, uint256 _lockToTime) public payable returns(bool) {
        require(address(msg.sender) == deployer, "Only deployer can call this function");
		createLPPairIfRequired();
        ...
        bytes memory data;
        if(block.chainid == 5){
            data = abi.encodeWithSelector(bytes4(keccak256("lockLPToken(address,uint256,uint256,address,bool,address,uint16)")), uniswapV2Pair, pair.balanceOf(address(this)), block.timestamp + _lockToTime, address(0), true, deployer, uint16(1));
        }
        else {
            data = abi.encodeWithSelector(bytes4(keccak256("lockLPToken(address,uint256,uint256,address,bool,address)")), uniswapV2Pair, pair.balanceOf(address(this)), block.timestamp + _lockToTime, address(0), true, deployer);
        }
    ...
    function lockOnTeam(address _teamAddress, uint256 _lockToTime) public payable returns(bool) {
        require(address(msg.sender) == deployer, "Only deployer can call this function");
		createLPPairIfRequired();
        ...
        locker.lockToken{value: fee}(uniswapV2Pair, deployer, pair.balanceOf(address(this)), block.timestamp + _lockToTime, false, address(0));
        swapEnabled = true;
        return true;
    }
