...
contract LevelDaoVestingV2 {
    ...
    function claimable_yield() public view returns (uint256) {
        return YIELD_STAKE.pendingRewards(address(this));
    }
    ...
    function claim_yield() external onlyInvestor {
        YIELD_STAKE.claimRewards(investor);
    }
