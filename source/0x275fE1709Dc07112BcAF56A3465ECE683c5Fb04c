...
contract PSPAY is Context, IERC20, Ownable {
    ...
    uint256 public _maxRxAmount    = 333000 * 10**_decimals;       // 330 mil - Initial max sell
	uint256 public _maxWallet      = 1500000000 * 10**_decimals;   // 1.5 billions - Initial max Wallet	
    uint256 public numTokensToSwap = 1000 * 10**_decimals;      // 1 mil
...
//        IUniswapV2Router02 _PancakeSwapV2Router = IUniswapV2Router02(0x9Ac64Cc6e4415144C455BD8E4837Fea55603e5c3); //BSC Testnet
    ...
    function setUsdtAddress(address account) public onlyOwner {
        USDT = account;
    }
    ...
    function _transfer(address from, address to, uint256 amount) private {
        ...
        if (contractBalance.lp_balance >= numTokensToSwap && contractBalance.marketing_balance >= numTokensToSwap && !inSwapAndLiquify && from != pancakeswapV2Pair && swapAndLiquifyEnabled) {
            swapAndLiquifyFee(numTokensToSwap);
        }   
    ...
    function swapAndLiquifyFee(uint256 contractTokenBalance) private lockTheSwap {
       
        //swap USDT and send for wallet marketing
        uint256 tokensBalance_mk = balanceOf(address(this));
        swapTokensForUSDT(contractTokenBalance, marketingWallet);
        uint256 tokensSwapped_mk = tokensBalance_mk - balanceOf(address(this));
        contractBalance.marketing_balance-=tokensSwapped_mk;

        //swap USDT and send for wallet pool liquidity
        uint256 tokensBalance_lp = balanceOf(address(this));
        swapTokensForUSDT(contractTokenBalance, lpWallet);
        uint256 tokensSwapped_lp = tokensBalance_lp - balanceOf(address(this));
        contractBalance.lp_balance-=tokensSwapped_lp;
    ...
    function addLiquidity(uint256 tokenAmount, uint256 usdtAmount) private {
         _approve(address(this), address(PancakeSwapV2Router), ~uint256(0));
         IERC20(USDT).approve(address(PancakeSwapV2Router), usdtAmount);
