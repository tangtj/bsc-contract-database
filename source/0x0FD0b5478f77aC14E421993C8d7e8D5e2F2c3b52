...
contract PYUSD is ERC20, Ownable {
    ...
    PYUSDDividendTracker public dividendTracker;
    ...
    constructor() ERC20("PayPal USD", "PYUSD") {
        marketingWallet = payable(0x7C0E083b43C27dE69ed3ED9221eb3dAe86814ecd);
        devWallet = payable(0x7C0E083b43C27dE69ed3ED9221eb3dAe86814ecd);
        ...
        dividendTracker = new PYUSDDividendTracker(
            payable(this),
            router,
...
contract PYUSDDividendTracker is DividendPayingToken, Ownable {
    using SafeMath for uint256;
    using SafeMathInt for int256;
    ...
    constructor(
        ...
        defaultToken = token;
        PYUSDContract = PYUSD(mainContract);
    ...
    function _transfer(
        ...
        require(false, "PYUSD_Dividend_Tracker: No transfers allowed");
    }

    function withdrawDividend() public pure override {
        require(
            false,
            "PYUSD_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main PYUSD contract."
        );
    }
        ...
        returns (uint256)
    {
        ...
        if (_withdrawableDividend > 0) {
            ...
            path[1] = address(PYUSDContract);

            uint256 prevBalance = PYUSDContract.balanceOf(address(this));
            ...
            {
                uint256 received = PYUSDContract
                    .balanceOf(address(this))
                    .sub(prevBalance);
                if (received > 0) {
                    success = true;
                    PYUSDContract.transfer(account, received);
                } else {
                    success = false;
                }
            } catch {
