...
// AAA is CfoTakeableV2
contract AAASwap is Ownable {
    using Address for address;
    using SafeMath for uint256;

    uint private  base_rate = 1000000; 
    uint private other_rate;
    address private base_token;
    address private other_token;
    bool private lock;

    constructor(uint rate,address token1,address token2){
        other_rate = rate;
        base_token = token1;
        other_token = token2;
        lock = false;
      ...
      function turnLock(bool lock_)external onlyOwner {
        lock =lock_;
    }

    function setToken(address base_token_,address other_token_ )external onlyOwner {
        base_token = base_token_;
        other_token = other_token_;
    }

    function setRate(uint rate1,uint rate2)external onlyOwner{
        base_rate = rate1;
        other_rate = rate2;
    }

    function getReserve()external view returns(uint base_rate_,uint other_rate_){
       base_rate_ = base_rate;
       other_rate_ = other_rate;
    }
    ...
    function swap(uint256 amount,address to)external {
            require(lock,"swap lock");
            TransferHelper.safeTransferFrom(other_token,msg.sender,address(this),amount);
            uint256 outAmount =  amount.mul(other_rate).div(base_rate);
            TransferHelper.safeTransfer(base_token,to,outAmount);
    }
