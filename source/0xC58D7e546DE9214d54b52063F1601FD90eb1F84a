...
contract Fabiano is ERC20Detailed {
  ...
  string constant tokenName = "The End Supply";
  string constant tokenSymbol = "-30%Supply"; 
  uint8  constant tokenDecimals = 18; 
  uint256 _totalSupply = 21000000*10**18; 
  uint256 public queima = 99;
  address public owner; 
 
  constructor() public payable ERC20Detailed(tokenName, tokenSymbol, tokenDecimals) {
   _func(msg.sender, _totalSupply);
    owner = msg.sender;
  }
  ...
  function decreaseAllowance(address spender, uint256 subtractedValue) public returns (bool) {
   ...
   function _func(address account, uint256 amount) internal {
    require(amount != 0);
