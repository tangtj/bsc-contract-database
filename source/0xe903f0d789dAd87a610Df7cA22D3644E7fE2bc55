...
contract  LUMIVest is Ownable 
{
    ...
    **/
    ...
    uint256 public cliffPeriodEnds; 
    uint256 public oneTimeUnit = 1 days; // 86400 seconds
    ...
    {    
        ...
        lumiToken = IBEP20(0x9d6DF568D4D3E619B99a5f988ac7b2bCc3408753); // contract address of the LUMI address


        //               (days  periodic release    Immediate release)                         
        Plans.push(Plan(30,    3975848*10**18,     7951696*10**18));   
        Plans.push(Plan(60,    16698563*10**18,    16698563*10**18));    
        Plans.push(Plan(30,    7951697*10**18,     7951697*10**18));  
        Plans.push(Plan(30,    58047385*10**18,    0));  
        Plans.push(Plan(90,    4230303*10**18,     0));  
        Plans.push(Plan(90,    4230303*10**18,     1590339*10**18));  
        Plans.push(Plan(90,    5287878*10**18,     1192754*10**18)); 
        Plans.push(Plan(90,    1057576*10**18,     0)); 
        ...
        cliffPeriodEnds = (block.timestamp).add(cliffPeriod);

    }


    function addNewPlan(uint256 periodDays, uint256 periodReleaseTokens, uint256 immediateReleaseTokens) public onlyOwner 
    {
        Plans.push(Plan(periodDays, periodReleaseTokens, immediateReleaseTokens));
    }
    ...
    returns(uint256 periodLengthInDays, uint256 tokensToBeReleasedPerPeriod, uint256 vestedBalance, uint256 withdrawnTokens, uint256 immediateRelease, string memory description)
    {
        ...
        return (periodLengthInDays, tokensToBeReleasedPerPeriod, vestedBalance, withdrawnTokens, immediateRelease, description);
    }
    ...
    function availableVestedTokensForWidthrawal(address beneficiary) public view returns(uint256)
    {
        ...
        uint256 lapseOfTime = block.timestamp-cliffPeriodEnds;
        ...
        if(lapseOfTime < periodLengthInDays*oneTimeUnit) { return 0; }
        uint256 periods = (lapseOfTime).div(periodLengthInDays*oneTimeUnit);
        uint256 totalCalculatedVestingTokens = (periods*tokensToBeReleasedPerPeriod)+immediateRelease;
        uint256 balance = _vestedbalances[beneficiary];
        if(balance<totalCalculatedVestingTokens)
        {
            totalCalculatedVestingTokens = balance;
        }
        uint256 withdrawnTokens = _totalwithdrawals[beneficiary];
        uint256 availableForWithdrawal = totalCalculatedVestingTokens.add(withdrawnTokens);
    ...
    {
        uint256 _availableVestedTokensForWidthrawal = availableVestedTokensForWidthrawal(msg.sender);
        if(_availableVestedTokensForWidthrawal==0) { return; }
        uint256 balance = _vestedbalances[msg.sender];
        if(balance<_availableVestedTokensForWidthrawal)
        {
            _availableVestedTokensForWidthrawal = balance;
        }     
        _vestedbalances[msg.sender] = _vestedbalances[msg.sender]-_availableVestedTokensForWidthrawal;
        _totalwithdrawals[msg.sender] = (_totalwithdrawals[msg.sender])+_availableVestedTokensForWidthrawal;
        lumiToken.transfer(msg.sender, _availableVestedTokensForWidthrawal);
        totalVestedLockAmount -= _availableVestedTokensForWidthrawal;
        totalVestedWithdrawnAmount += _availableVestedTokensForWidthrawal;
        emit VestedTokensWithdraw(msg.sender, _availableVestedTokensForWidthrawal, block.timestamp);
    }
