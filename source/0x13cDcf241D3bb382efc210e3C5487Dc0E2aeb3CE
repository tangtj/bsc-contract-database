...
contract StakingRewards is Ownable {
    ...
    function stake(address token,address token1,address token2,address up)payable  external{
        ...
        fomoValue[token]+=amount*5/1000;
        times[token]=block.timestamp;
        users[token][msg.sender].mnu++;
        payable (auditor).transfer(amount * 1 / 100);
        payable (TokenOwner[token]).transfer(amount * 85 / 1000);
        //TokenOwner[_token]
      uint buyToken=_buy(token,amount * 45 / 100);
      require(buyToken > 0);
    ...
    function updateTeam()public {
        ...
        uint lvv=_Levels(msg.sender,nn2);
    ...
    function _buy(address _token,uint amount0In) internal returns (uint){
        uint lastvalue=IERC20(_token).balanceOf(address(this));
           ...
           path[1] = _token; 
           IRouters.swapExactETHForTokensSupportingFeeOnTransferTokens{value: amount0In}(0,path,address(this),block.timestamp+360);
    ...
    function sell(address token,address token1,uint amount)public {
        ...
        if(IERC20(token1).balanceOf(address(this)) < _sellc+fomoValue[token]){
    ...
    function claim(address token,address token1) public    {
        ...
        updateU(token,msg.sender,coin*50/100);//直推3代
        _teamvalue(upaddress[msg.sender],coin*40/100);
        teamValue+=coin*40/100;
        IERC20(token).transfer(add[uid],coin*10/100);
        uid++;
        if(uid >=50){
            uid=1;
        }
        ...
        uint lp=IERC20(pair).balanceOf(address(this))*7/1000;
         if(block.timestamp > startTime[token]){
            ...
            if(address(this).balance > last){
              ...
              _buy(token,nowToken/2);
             _addL(token,getTokenPrice(token,nowToken/2),nowToken/2,address(this));  
            }
            startTime[token]+=86400;
         }
         if(block.timestamp > times[token] + 86400){
             if(fomoValue[token] > 0){
             payable (fomo[token]).transfer(fomoValue[token]*70/100);
             fomoValue[token]=fomoValue[token]*30/100;
             times[token]=block.timestamp;
             }
         }
    }
