...
contract ERC20PledgeToken is Ownable{
    ...
    mapping(address=>PledgeData[]) public pledgeData;
    ...
    function pledge(uint _type, uint amount, uint amount2, uint cycle) external returns(bool){
        ...
        if (_type == 0) {
            ...
            IERC20(_baseToken).transferFrom(sender, receiver, amount);
            IERC20(token).transferFrom(sender, receiver, amount2);
            uint oldLpAmount = IERC20(uniswapV2Pair).balanceOf(address(this));
            addLiquidity(amount, amount2);
