...
contract LimitEntry {
    ...
    mapping (uint256 => LimitSwap)                      public swapOrder;
    uint256                                             public order;        //成交编号对应的订单
    ...
    bool                                                public SellFree = true;         //开启卖出方手续费
    bool                                                public BuyFree = true;          //开启买入方手续费
    ...
    function sellLimit(address asses, uint256 wad, address _pro, uint256 _price,address owner) external  {
        ...
        order +=1;
        uint length = orders[asses][_pro].length;
        orders[asses][_pro].push(order);
        Limit memory merc = limitOrder[order];
            merc.id = order; 
            merc.location = length;
            merc.asses = asses;
    ...
    function modifyAmount(uint256 i,uint256 wad) external  {
         ...
         address _pro = limitOrder[i].pro;
         uint amount = limitOrder[i].amount;
         if(amount == 0 && wad != 0) {
            order +=1;
            limitOrder[order] = limitOrder[i];
            i = order;
            uint length = orders[asses][_pro].length;
            orders[asses][_pro].push(order);
            limitOrder[i].location = length;
            issue[msg.sender] +=1;
            ownerOrder[msg.sender][issue[msg.sender]] = order;
            limitOrder[i].id = order;
         }
         else if(amount !=0 && wad ==0) exitSort(i);
         if(wad > amount){
    ...
    function exitSort(uint256 i) internal {
        Limit storage merc = limitOrder[i];
        address asses = merc.asses;
        address _pro = merc.pro;
        uint256 n = merc.location;
        uint256 length = orders[asses][_pro].length;
        uint256 endid = orders[asses][_pro][length -1];
        orders[asses][_pro][n] = endid;
        orders[asses][_pro].pop();
        limitOrder[endid].location = n;
    } 
     // 接单
    function taker(uint256 i,uint256 wad,address owner) public returns (uint256) {
        ...
        if(limitOrder[i].amount ==0) exitSort(i);
        address asses = limitOrder[i].asses;
        ...
        if(asses>pro) swaps[asses][pro].push(swap);
        else swaps[pro][asses].push(swap);
    ...
    function sort(address asses, address pro,uint256 length,uint256 count) public view returns (Limit[] memory) {
        uint[] memory ord = orders[asses][pro];
        uint leng = ord.length;
        if(leng<length) length = leng;
        Limit[] memory result = new Limit[](length);
        for (uint size = 1; size <= length; ++size) {
            uint i = ord[leng-size];
            Limit memory m = limitOrder[i];
            for (uint j = 0; j < size; ++j) {
                Limit memory n = result[j];
                if (n.pro == address(0)) result[j] = m;
                else if ( m.price < n.price || (m.price == n.price && m.id < n.id)) {
    ...
    function getTransactionlist(address asses,address pro,uint256 count) public view returns (TransactionList[] memory) {
        TransactionList[] memory result = new TransactionList[](count);
        uint[] memory transactions;
        if(asses > pro) transactions = swaps[asses][pro];
        else transactions = swaps[pro][asses];
        uint j;
        uint length = transactions.length;
        for (uint k = 1 ; k <= length ; ++k) {
            uint i = transactions[length - k];
            LimitSwap memory n1 = swapOrder[i];
            Limit memory n2 = limitOrder[n1.limitId];
            if(n2.asses == asses && n2.pro == pro){
              result[j].seller = n2.seller;
              result[j].symbol = TokenLike(asses).symbol();
              result[j].what = 1;
              if(n1.inAmount !=0) result[j].price = n1.outAmount*1E18/n1.inAmount;
              result[j].inAmount = n1.inAmount;
              result[j].outAmount = n1.outAmount;
              result[j].buyer = n1.buyer;
              j++;
            }
            else if(n2.pro == asses && n2.asses == pro){
              result[j].seller = n2.seller;
              result[j].symbol = TokenLike(pro).symbol();
              result[j].what = 2;
              if(n1.outAmount !=0) result[j].price = n1.inAmount*1E18/n1.outAmount;
              result[j].inAmount = n1.outAmount;
              result[j].outAmount = n1.inAmount;
              result[j].buyer = n1.buyer;
              j++;
            }
        } 
