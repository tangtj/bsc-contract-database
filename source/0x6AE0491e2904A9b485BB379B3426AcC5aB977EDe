...
contract Supermoon is Context, IERC20, Ownable {
    ...
    uint256 public maxHoldAmount = 500 * 10 ** _decimals;

    uint256 public maxFistAmount = 20 * 10 ** 18;
    ...
    function setMaxHoldAmount(uint256 _tx)external onlyOwner{
        maxHoldAmount = _tx;
    }
    ...
    function _transfer(
        ...
        if (!_isExcludedFromFee[from] && !_isExcludedFromFee[to] && (ammPairs[from] || (!ammPairs[from] && !ammPairs[to]))) {
          uint256 limitAmount = balanceOf(to).add(amount);
          require(limitAmount <= maxHoldAmount, 'Exceed buying and selling limit');
        }
        ...
        if( 
            ...
            swapAndAward(contractTokenBalance.div(2));
            swapAndMK(contractTokenBalance.div(2));
