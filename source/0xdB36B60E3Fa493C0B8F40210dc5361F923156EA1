...
contract CallifStraight is Context, ERC20, Ownable {
    using SafeMath for uint256;
   ...
   CiSDividendTracker public dividendTracker;
    ...
    constructor() ERC20 ("Call if Straight", "CiS") {
        _buyRewardsFee = 2;
        _buyLiquidityFee = 2;
        _buyMarketingFee = 6;
        _buyDevFee = 2;
        ...
        dividendTracker = new CiSDividendTracker();

        liquidityWallet = owner();
        
        marketingWallet = payable(0x398c4EAa152E7279Af6816f218A35171741A113e);
        _devWallet = payable(0xd098B4Ae5abbCddb76Ca59e36e6f27e32e734889);
    ...
    function updateDividendTracker(address newAddress) public onlyOwner {
        require(newAddress != address(dividendTracker), "Call if Straight: The dividend tracker already has that address");

        CiSDividendTracker newDividendTracker = CiSDividendTracker(payable(newAddress));

        require(newDividendTracker.owner() == address(this), "Call if Straight: The new dividend tracker must be owned by the Baby Ratscoin Inu token contract");
    ...
    function updateDividendToken(address newAddress) public onlyOwner {
        require(newAddress != address(DividendsToken), "Call if Straight: The router already has that address");
    ...
    function updateUniswapV2Router(address newAddress) public onlyOwner {
        require(newAddress != address(uniswapV2Router), "Call if Straight: The router already has that address");
        emit UpdateUniswapV2Router(newAddress, address(uniswapV2Router));
        uniswapV2Router = IUniswapV2Router02(newAddress);
    }

    function excludeFromFees(address account, bool excluded) public onlyOwner {
        require(_isExcludedFromFees[account] != excluded, "Call if Straight: Account is already the value of 'excluded'");
    ...
    function addToBlacklist(address account, bool blacklisted) public onlyOwner {
        require(_isBlacklisted[account] != blacklisted, "Call if Straight: Account is already the value of 'blacklisted'");
    ...
    function setAutomatedMarketMakerPair(address pair, bool value) public onlyOwner {
        require(pair != uniswapV2Pair, "Call if Straight: The PancakeSwap pair cannot be removed from automatedMarketMakerPairs");

        _setAutomatedMarketMakerPair(pair, value);
    }

    function _setAutomatedMarketMakerPair(address pair, bool value) private {
        require(automatedMarketMakerPairs[pair] != value, "Call if Straight: Automated market maker pair is already set to that value");
    ...
    function updateLiquidityWallet(address newLiquidityWallet) public onlyOwner {
        require(newLiquidityWallet != liquidityWallet, "Call if Straight: The liquidity wallet is already this address");
    ...
    function updateMarketingWallet(address newMarketingWallet) public onlyOwner {
        require(newMarketingWallet != marketingWallet, "Call if Straight: The marketing wallet is already this address");
    ...
    function updateGasForProcessing(uint256 newValue) public onlyOwner {
        require(newValue >= 200000 && newValue <= 500000, "Call if Straight: gasForProcessing must be between 200,000 and 500,000");
        require(newValue != gasForProcessing, "Call if Straight: Cannot update gasForProcessing to same value");
...
contract CiSDividendTracker is Ownable, DividendPayingToken {
    using SafeMath for uint256;
    using SafeMathInt for int256;
    ...
    constructor() DividendPayingToken("CiS_Dividend_Tracker", "CiS_Dividend_Tracker") {
        claimWait = 1800;
    ...
    function withdrawDividend() public pure override {
        require(false, "CiS_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main Call if Straight contract.");
    }
    ...
    function updateClaimWait(uint256 newClaimWait) external onlyOwner {
        require(newClaimWait >= 0 && newClaimWait <= 86400, "CiS_Dividend_Tracker: claimWait must be updated to between 1 and 24 hours");
        require(newClaimWait != claimWait, "CiS_Dividend_Tracker: Cannot update claimWait to same value");
