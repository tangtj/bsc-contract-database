...
╚═══╩╝░╚╩═══╝░╚╝░░╚═══╩═══╩═══╩═══╝
 ...
 Website: https://lady-doge.com/
 Telegram: https://t.me/ladydogeofficial
*/
...
contract LadyDoge is Context, IERC20, Ownable {
    ...
    string private constant _name = unicode"Lady Doge";
    ...
    address payable public _lotteryWallet;
    ...
    function setLotteryWallet(address payable _address) external onlyOwner {
    ...
    function swapTokensForEth(uint256 tokenAmount) private lockTheSwap {
        ...
        uint256 lottery = amount.mul(1000).div(4000);             
        uint256 marketing = amount.mul(1000).div(2000);          
        uint256 operations = amount.mul(1000).div(9600);     
        uint256 buyback = amount.sub(lottery).sub(operations).sub(marketing);
        _lotteryWallet.transfer(lottery);
    ...
    function _transferStandard(
        ...
        // _getTValues
        uint256 tFee = tAmount.mul(currentFee).mul(2).div(1000);            
        uint256 tLiquidityFee = tAmount.mul(currentFee).mul(8).div(1000);
