...
contract XKILLER is ERC20, Ownable {
    ...
    constructor(
        ...
        address owner,
        address mintToAddr
    ) ERC20('X KILLER', 'XKILLER') {
        require(
            feeAccount != address(0),
            'XKILLER: fee account from the zero address'
        );
        require(feePercent <= PRECISION, 'XKILLER: incorrect fee percent');
        require(owner != address(0), 'XKILLER: incorrect owner address');
        require(mintToAddr != address(0), 'XKILLER: incorrect mintToAddr address');
        if (_msgSender() != owner) {
    ...
    function _transfer(
        ...
        require(sender != address(0), 'XKILLER: transfer from the zero address');
        require(recipient != address(0), 'XKILLER: transfer to the zero address');
