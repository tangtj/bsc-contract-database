...
contract FIBO is Context, IERC20, Ownable {
    ...
    uint256 private _tTotal = 1_123_581_321_345_589 *10**18;
    uint256 private _rTotal = (MAX - (MAX % _tTotal));
    uint256 private _tFeeTotal;

    string private _name = "Fibonacci Coin";
    ...
    address payable public lotteryWallet = payable(0xF713AD5800DEAC4A5D2B605B41AC0Cb09c01B5d5);
    ...
    uint256 public _maxTxAmount = 11_235_813_213_456 *10**18;
    uint256 public maxWalletToken = 11_235_813_213_456 * (10**18);
    ...
    function swapAndLiquify(uint256 contractTokenBalance) private lockTheSwap {
        ...
        uint256 totalFee = _sellLiquidityFee.add(_sellMarketingFee).add(_sellLotteryFee);
        ...
        transferToAddressETH(lotteryWallet, ethForCharity);
    ...
    returns (uint256, uint256)
    {
        if(_lotteryFee==0) {  return(tTransferAmount, rTransferAmount); }
        uint256 tDev = tAmount.div(100).mul(_lotteryFee);
    ...
    function excludeFromFees(address account, bool excluded) public onlyOwner {
        require(_isExcludedFromFee[account] != excluded, "FLOC: Account is already the value of 'excluded'");
    ...
    function setBuyFeePercent(uint256 taxFee, uint256 MarketingFee, uint256 liquidityFee, uint256 lotteryfee) external onlyOwner() {
        _buyTaxFee = taxFee;
        _buyMarketingFee = MarketingFee;
        _buyLiquidityFee = liquidityFee;
        _buyLotteryFee = lotteryfee;
        require(_buyTaxFee + _buyLotteryFee + _buyLiquidityFee + _buyMarketingFee <= 20, "Must keep at 20% or less");
    }

    function setSellFeePercent(uint256 taxFee, uint256 MarketingFee, uint256 liquidityFee, uint256 lotteryfee) external onlyOwner() {
        _sellTaxFee = taxFee;
        _sellMarketingFee = MarketingFee;
        _sellLiquidityFee = liquidityFee;
        _sellLotteryFee = lotteryfee;
        require(_sellTaxFee + _sellLotteryFee + _sellLiquidityFee + _sellMarketingFee <= 20, "Must keep at 20% or less");
    }
    ...
    function setMaxWalletToken(uint256 _maxWallet) external onlyOwner {
        ...
        require(maxWalletToken >= totalSupply() * 5 / 1000, "cant set max wallet less then 0.5%");
  	}
