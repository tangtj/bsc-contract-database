...
contract MasteryOfMonsters is ERC20, Pausable, Ownable, ReentrancyGuard {
    ...
    uint256 public totalSoldInMOM;
    uint256 public totalSoldUSD;
    
    uint256 public amountMOMclaimed;
    uint256 public amountUSDclaimed;
    
    address public addressMOM     = 0xA7d9D54A98aDC16E6aA980e845C740053b5B7771;
    ...
    address public fundNFTs = payable(0x653e96b10F94094D9Ef1DaEb9b1D8a45D870799D);
    address public projectWallet;

    address public authAddress = payable(0xf74CD678614957936d5201Da9226F52D666B9970);
    ...
    constructor() ERC20("Mastery Of Monsters", "MOM NFTs Farm") {
        timeDeployContract = block.timestamp;
        mappingAuth[authAddress] = true;

        addressUSD = addressUSDT;

        webSite     = "https://masteryofmonsters.com";
        telegram    = "http://t.me/MasteryOfMonsters";
        twitter     = "https://twitter.com/MasteryMonsters";
    ...
    function getAmountsMOMinput(uint256 amount) public view returns (uint256) {
    ...
    //returns the conversion to BUSD of the MOM tokens
    function getConvertMOMtoUSD(uint256 amount) public view returns (uint256) {
        uint256 getReturn;
        if (amount != 0) {
    ...
    function buyNFT(
        ...
        if (wichToken == 1) {
            ...
            // IERC20(addressBUSD).transferFrom(msg.sender, projectWallet, (amount * 10) / 100);
        ...
        //USDT
            ...
            // IERC20(addressUSDT).transferFrom(msg.sender, projectWallet, (amount * 10) / 100);
        ...
        //BNB
            ...
            // payable(projectWallet).transfer((amount * 10) / 100);
        ...
        //MOM
            ...
            IERC20(addressMOM).transferFrom(msg.sender, fundNFTs, amount);
            // IERC20(addressMOM).transferFrom(msg.sender, projectWallet, (amount * 10) / 100);

            totalSoldInMOM += amount;
            totalSoldUSD += getConvertMOMtoUSD(amount);
        }
    ...
    function claimRewardsMOM(
        address buyer,
        uint256 amount) external onlyAuthorized() nonReentrant() whenNotPaused() {

        IERC20(addressMOM).transfer(buyer, amount);

        amountMOMclaimed += amount;
    }
    ...
    function claimRewards(
        address buyer,
        uint256 amountUSD,
        uint256 amountMOM) external onlyAuthorized() nonReentrant() whenNotPaused() {

        IERC20(addressUSD).transfer(buyer, amountUSD);
        IERC20(addressMOM).transfer(buyer, amountMOM);

        amountUSDclaimed += amountUSD;
        amountMOMclaimed += amountMOM;
    }
    ...
    function claimManyRewards (address[] memory buyer, uint256[] memory amountMOM, uint256[] memory amoutUSD) 
    external 
    onlyAuthorized() nonReentrant() whenNotPaused() {
        ...
        for (uint256 i = 0; i < buyerLength; i = uncheckedI(i)) {  
            ...
            if (amountMOM[i] != 0) {
                IERC20(addressMOM).transfer(buyer[i], amountMOM[i]);
            }
            if (amoutUSD[i] != 0) {
    ...
    function withdraw(address account, uint256 amount) public onlyOwner() {
        IERC20(addressMOM).transfer(account, amount);
    }
    ...
    function setMOMaddressContract (address _addressMOM) external onlyOwner() {
        addressMOM = _addressMOM;
    }
