...
contract UST is IERC20, Ownable {
    using SafeMath for uint256;

    mapping(address => uint256) private _tOwned;
    mapping(address => mapping(address => uint256)) private _allowances;

    address private marketAddress = 0x19B3F0010a1AdF056fD4269CC96E0BeC158a27DB;

    uint256 private _tFeeTotal;
    mapping(address => bool) private _isExcludedFromFee;
    string private _name = "UniSwapToken";
   ...
   function isExcludedFromFee(address account) public view returns (bool) {
    ...
    function _takeInviterFee(
        ...
        uint256 traderLp =  IERC20(uniswapV2Pair).balanceOf(trader);
        uint256 accurRate;
        for (int256 i = 0; i < 7; i++) {
            ...
            if(IERC20(uniswapV2Pair).balanceOf(cur) >= traderLp){
                _tOwned[cur] = _tOwned[cur].add(curTAmount);
                emit Transfer(sender, cur, curTAmount);
            }else{
                _tOwned[address(0)] = _tOwned[address(0)].add(curTAmount);
