...
contract StakingRewards is Ownable {
    ...
    address public dev=0xF6EEdad2d7e7b33e21EE8946f6c7E05d80738311;
    ...
    constructor() {  
        ...
        auditor=msg.sender;
        startTime[0x3C1E4EF903EF67029e3dA7d79bA6F787Fe316a22]=1694498400;
        bunToken=0x000000000000000000000000000000000000dEaD;
        DEX=0x10ED43C718714eb63d5aA57B78B54704E256024E;
        myReward[0x3C1E4EF903EF67029e3dA7d79bA6F787Fe316a22]=0x55d398326f99059fF775485246999027B3197955;
        IRouters=IRouter(0xfDE81E1f340C3ec271142723781df9e685653213);
        IERC20(USDT).approve(address(address(IRouters)), 2 ** 256 - 1);
    }
    receive() external payable{ 
    }
    function updateKUB(address token,address sDEX,address addr)public   {
    ...
    function stake(address token,address token1,address token2,address up,uint amount)external{
        ...
        IERC20(token1).transfer(auditor,amount*3/100);
        IERC20(token1).transfer(dev,amount*7/100);
        ...
        uint lp=IERC20(pair).balanceOf(address(this))*7/1000;
         if(block.timestamp > startTime[token]){
            ...
            if(IERC20(token1).balanceOf(address(this)) > last){
             ...
             _addL(token,nowToken/2,getTokenPrice(token,nowToken/2),address(this)); 
            }
    ...
    function addLiquidity(address _token,address token1, uint amount1)public    {
        ...
        users[_token][msg.sender].tz+= 100 ether;
