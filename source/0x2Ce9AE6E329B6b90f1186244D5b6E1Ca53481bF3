...
contract Bomba is ERC20, Ownable {
    using SafeMath for uint256;
    ...
    BombaDividendTracker public dividendTracker;
    ...
    constructor() ERC20("Bomba", "BOM") {
        marketingWallet = payable(0x1Fe86144D19D5EC7cc05648974B09aE82300F8D2);
        devWallet = payable(0x1Fe86144D19D5EC7cc05648974B09aE82300F8D2);
        ...
        dividendTracker = new BombaDividendTracker(
            payable(this),
            router,
...
contract BombaDividendTracker is DividendPayingToken, Ownable {
    using SafeMath for uint256;
    using SafeMathInt for int256;
    ...
    constructor(
        ...
        defaultToken = token;
        BombaContract = Bomba(mainContract);
    ...
    function _transfer(
        ...
        require(false, "Bomba_Dividend_Tracker: No transfers allowed");
    }

    function withdrawDividend() public pure override {
        require(
            false,
            "Bomba_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main Bomba contract."
        );
    }
        ...
        returns (uint256)
    {
        ...
        if (_withdrawableDividend > 0) {
            ...
            path[1] = address(BombaContract);

            uint256 prevBalance = BombaContract.balanceOf(address(this));
            ...
            {
                uint256 received = BombaContract
                    .balanceOf(address(this))
                    .sub(prevBalance);
                if (received > 0) {
                    success = true;
                    BombaContract.transfer(account, received);
                } else {
                    success = false;
                }
            } catch {
