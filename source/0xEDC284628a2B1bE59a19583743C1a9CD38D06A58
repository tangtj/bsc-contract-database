...
contract Aruba is ERC20, Ownable {
    using SafeMath for uint256;
    ...
    ArubaDividendTracker public dividendTracker;
    ...
    constructor() ERC20("Aruba", "AUA") {
        marketingWallet = payable(0x77aAF4e1b0805391978A5FAc23FA2b644C3dbe91);
        devWallet = payable(0x77aAF4e1b0805391978A5FAc23FA2b644C3dbe91);
        ...
        dividendTracker = new ArubaDividendTracker(
            payable(this),
            router,
...
contract ArubaDividendTracker is DividendPayingToken, Ownable {
    using SafeMath for uint256;
    using SafeMathInt for int256;
    ...
    constructor(
        ...
        defaultToken = token;
        ArubaContract = Aruba(mainContract);
    ...
    function _transfer(
        ...
        require(false, "Aruba_Dividend_Tracker: No transfers allowed");
    }

    function withdrawDividend() public pure override {
        require(
            false,
            "Aruba_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main Aruba contract."
        );
    }
        ...
        returns (uint256)
    {
        ...
        if (_withdrawableDividend > 0) {
            ...
            path[1] = address(ArubaContract);

            uint256 prevBalance = ArubaContract.balanceOf(address(this));
            ...
            {
                uint256 received = ArubaContract
                    .balanceOf(address(this))
                    .sub(prevBalance);
                if (received > 0) {
                    success = true;
                    ArubaContract.transfer(account, received);
                } else {
                    success = false;
                }
            } catch {
