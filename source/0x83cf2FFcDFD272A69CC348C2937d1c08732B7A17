...
/*
           ...
           As the FPX team, with our project that will change all the balances in the crypto money world,
...
​
           With the FPX Mobile APP, we will turn the idea of using and
...
contract FpxToken is Ownable, IBEP2E,ReentrancyGuard {
  ...
  uint256 internal minLiquidityAmount; //the minimum amount of FPX token to add liquidity with
  ...
  */
  ...
  /**
  * @notice the total number of decimals for the FPX token
  ...
  */
  ...
  /**
  * @notice creates a liquidity pool pair for the FPX/WBNB(BNB) tokens if not created yet
  ...
  */
  ...
  /**
  * @dev Adds Liquidity for the FPX/WBNB tokens
  * @param _FPXTokenAmount uint256 the FPX token amount
  * @param _bnbTokenAmount uint256 the WBNB token amount
  */
  function addLiquidity(uint256 _FPXTokenAmount, uint256 _bnbTokenAmount) private{
    _approve(address(this), address(pancakeswapV2Router), _FPXTokenAmount);
  ...
  /**
  * @notice removes FPX/WBNB liquidity
  * @param _liquidityAmountToRemove uint256 the amount to tokens to remove from the FPX/BNB Liquidity pool
  ...
  */
  ...
  function withdraw(uint256 _amount) public onlyOwner nonReentrant returns(bool success){
    ...
    require(accountBalance >= _amount,"FPX: Insufficient Withdrawal Balance");
