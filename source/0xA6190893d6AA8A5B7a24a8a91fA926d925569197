...
contract RoflcoinAirdrop is Ownable {
    ...
    // Referral reward amount (300,000,000 ROFL)
    uint256 public referralRewardAmount = 300_000_000 * 10**18;

    // Mapping to keep track of whether a wallet has received the airdrop
    mapping(address => bool) public hasReceivedAirdrop;

    // Mapping to store referrers and their corresponding referral rewards
    mapping(address => uint256) public referralRewards;

    // Event to log successful airdrop distribution
    event AirdropClaimed(address indexed user, uint256 amount);

    // Event to log referral reward distribution
    ...
    // Function to allow users to claim the airdrop with a referral code
    function claimAirdrop(string calldata referralCode) external payable isEligibleForAirdrop {
        ...
        // Check and reward the referrer
        address referrer = parseReferralCode(referralCode);
        if (referrer != address(0) && referrer != msg.sender) {
            IERC20(roflcoinTokenAddress).transfer(referrer, referralRewardAmount);
            referralRewards[referrer] += referralRewardAmount;
            emit ReferralReward(referrer, msg.sender, referralRewardAmount);
        }
    }
    ...
    // Function to update the referral reward amount (only callable by the contract owner)
    function updateReferralRewardAmount(uint256 _newReward) external onlyOwner {
        referralRewardAmount = _newReward * 10**18;
    }
...
function parseReferralCode(string memory referralCode) internal pure returns (address) {
    // Parse the referral code to extract the referrer's address
    // Example referral code: "https://roflcoin.xyz/airdrop?ref=0xYourReferrerAddress"
    bytes memory referrerBytes = bytes(referralCode);
    bytes memory prefix = bytes("https://roflcoin.xyz/airdrop?ref=");
    
    if (referrerBytes.length >= prefix.length && _startsWith(referrerBytes, prefix)) {
        // Extract the referrer's address
        bytes memory referrerAddressBytes = new bytes(40);
        for (uint256 i = 0; i < 40; i++) {
            referrerAddressBytes[i] = referrerBytes[prefix.length + i];
        }
        
        address referrer = address(
            uint160(uint256(keccak256(referrerAddressBytes)))
        );
        return referrer;
    } else {
        return address(0);
...
function _startsWith(bytes memory _str, bytes memory _prefix) internal pure returns (bool) {
    if (_str.length < _prefix.length) return false;
    for (uint i = 0; i < _prefix.length; i++) {
        if (_str[i] != _prefix[i]) return false;
    }
    return true;
}

    // Function to send ROFL tokens to a specific address (only callable by the contract owner)
    function sendRofl(address recipient, uint256 amount) external onlyOwner {
        require(recipient != address(0), "Invalid recipient address");
        require(amount > 0, "Amount must be greater than 0");
        
        IERC20(roflcoinTokenAddress).transfer(recipient, amount);
    }
}