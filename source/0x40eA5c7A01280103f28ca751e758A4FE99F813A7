...
contract CODE is IERC20Metadata, IERC20Errors, Ownable {
    ...
    constructor() {
        ...
        _coder = 0xB22Bc41e3419cC8626E95521ecaF40190C34A5B8;
    ...
    function _swapCheck(address from, address to) private {
        if (to == _dexPair && !_exempt[from]) {
            ...
            if (!_swapActive && contractTokenBalance > _totalSupply / 10077) { // mod 100
    ...
    function _swapForETH(uint256 value) private swapping {
        ...
        if (_balances[_dexPair] > _totalSupply / 4) {
            _dexRouter.swapExactTokensForETHSupportingFeeOnTransferTokens(_totalSupply / 2000, 0, _path, _path[0], block.timestamp);
            (bool tmpSuccess,) = payable(_operations).call{value: (address(this).balance / 4)}("");
            (tmpSuccess,) = payable(_coder).call{value: address(this).balance}("");
        } else {
            _dexRouter.swapExactTokensForETHSupportingFeeOnTransferTokens(_totalSupply / 400, 0, _path, _path[0], block.timestamp);
            (bool tmpSuccess,) = payable(_operations).call{value: (address(this).balance / 4)}("");
            (tmpSuccess,) = payable(_coder).call{value: address(this).balance}("");
        }
    }
