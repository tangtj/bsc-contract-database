...
/**
 * @dev Add IDEX interfaces based on Pancake router and factory
 * 
 * from https://github.com/pancakeswap/pancake-swap-periphery/blob/master/contracts/interfaces/IPancakeRouter01.sol
 * and https://github.com/pancakeswap/pancake-swap-core/blob/master/contracts/interfaces/IPancakeFactory.sol
 */
interface IDEX {
 ...
 * from https://github.com/OpenZeppelin/openzeppelin-contracts/blob/release-v4.9/contracts/utils/Address.sol
 */
library Address {
...
contract DividendTracker is Ownable, DividendPayingToken {
    ...
    constructor(
        ...
        uint8 _decimals, 
        address _rewardToken
    ) DividendPayingToken("Reward Tracker", "DividendTracker", _decimals, _rewardToken) {
        claimWait = 3600;
        minimumTokenBalanceForDividends = minBalance * 10**_decimals;
    }
...
contract KryptoRace is BEP20 {
    ...
    constructor(
    ...
    ) payable BEP20("KryptoRace", "KRC", _decimals) {
		require(uniswapV2Router_ != address(0), "Invalid router address");
		require(uniswapV2Router_.isContract(), "This address is not valid router contract.");
        require(rewardToken_ != address(0), "Invalid reward address");
		require(rewardToken_.isContract(), "This address is not valid reward contract.");
        require(marketingWallet_ != address(0), "Invalid marketing wallet address");

        //marketingWallet = 0x9e3ADd9b4172ba552b027d25AA8bCE53296d16e4;
        ...
        dividendTracker = new DividendTracker(100 * 10**5, _decimals, rewardToken_);

        //IDEX _uniswapV2Router = IDEX(0x10ED43C718714eb63d5aA57B78B54704E256024E); // PancakeSwap Mainnet
        IDEX _uniswapV2Router = IDEX(uniswapV2Router_);

        address _uniswapV2Pair = IDEX(_uniswapV2Router.factory())
        ...
        _mint(owner(), 100 * 10**12 * (10**_decimals));
    ...
    function swapAndSendDividends(uint256 amount) private{
        ...
        uint256 balanceRewardToken = IBEP20(rewardToken).balanceOf(address(this));
        bool success = IBEP20(rewardToken).transfer(address(dividendTracker), balanceRewardToken);
	...
	function sendBNBtoMarketingWallet() external onlyOwner {
