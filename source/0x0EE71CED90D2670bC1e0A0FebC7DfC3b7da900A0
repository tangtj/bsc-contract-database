...
contract ChickenEGGfarm {
    
      // CHICKEN EGG FARM Idle Game //  

    // lower lastPrice changed in half        longevity +  anti-inflation 
    // lower hatchingSpeed of 100 instead of 200      anti-inflation + longevity
    // lower seed of 864000000 instead of 8640000000 to make the early game slower, more fair and stop hyperinflation.   Anti-Inflation - Longevity 
    // lower referral (5%) to fight ref abuse      longevity
    // lower dev fee (2%)    longevity
    
    uint256 public CEO_FEE = 2;
    
    address payable public superPowerfulDragonOwner;
    uint256 public lastPrice = 1000000000000000000;
    ...
    function buySuperPowerFulDragon() public payable {
        ...
        require(isSuperPowerfulDragonEnabled);
        uint currenPrice = SafeMath.add(SafeMath.div(SafeMath.mul(lastPrice, 1),100),lastPrice);
        ...
        ceoEtherBalance += calculatePercentage(msg.value, 1);
        superPowerfulDragonOwner.transfer(msg.value - calculatePercentage(msg.value, 1));
    ...
    function getEggsSinceLastHatch(address adr) public view returns(uint256){
        ...
        uint256 dragonCount = SafeMath.mul(iceDragons[adr], 15);
        dragonCount = SafeMath.add(SafeMath.mul(ultraDragon[adr], 30), dragonCount);
    ...
    function buyIceDrangon() public payable {
        uint dragons = getDragonsToBuy(msg.value, 13);
        iceDragons[msg.sender] += dragons;
    }
    
    function buyUltraDrangon() public payable {
        require(activated);
        uint dragons = getDragonsToBuy(msg.value, 26);
