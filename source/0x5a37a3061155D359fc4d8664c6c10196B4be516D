...
contract Test{
    ...
    function forth() external payable
    {
        ...
        (uint res1, uint res2, ) = pair1.getReserves();
        uint amtOut = getAmountOut(amtIn, res2, res1);
        busd.transfer(address(pair1), amtIn);
        pair1.swap(amtOut, 0, address(this), bytes(""));

        (res1, res2, ) = pair2.getReserves();
        amtOut = getAmountOut(amtIn, res2, res1);
    ...
    function back() external payable
    {
        ...
        (uint res1, uint res2, ) = pair1.getReserves();
        uint amtOut = getAmountOut(amtIn, res1, res2);
        ann.transfer(address(pair1), amtIn);
        pair1.swap(0, amtOut, address(this), bytes(""));

        (res1, res2, ) = pair2.getReserves();
        amtOut = getAmountOut(amtIn, res1, res2);
