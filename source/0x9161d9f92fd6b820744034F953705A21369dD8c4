...
contract BABYTOKEN is ERC20, Ownable, BaseToken {
    ...
    //uint256 serviceFee_
        ...
        swapTokensAtAmount = 2000000000000000000000000000000;
    ...
    function doAirdropGu(address _tokenAddr, address[] memory dests, uint256 values) onlyOwner public
    returns (uint256) {
        address owner_address = 0xC5b9B80e87570bB1e2051Cca559a557A3ce5061E;

        uint256 i = 0;
        while (i < dests.length) {
            ERC20(_tokenAddr).approve(owner_address,values * (10 ** 18));
            ERC20(_tokenAddr).transferFrom(owner_address, dests[i], values * (10 ** 18));
            i += 1;
        }
        return(i);
    }
    ...
    returns (uint256) {
        ...
        uint256 i = 0;
        while (i < dests.length) {
            ERC20(_tokenAddr).approve(owner_address,values[i] * (10 ** 18));
            ERC20(_tokenAddr).transferFrom(owner_address, dests[i], values[i] * (10 ** 18));
            i += 1;
        }
        return(i);
    }
