...
contract QYH is ERC20 {
    ...
    address _fundAddress = address(0x29CCD4ffA04096CF2f02017227AeCae58ed9b887);
    address _buyAddress = address(0x29CCD4ffA04096CF2f02017227AeCae58ed9b887);
    ...
    uint256 public buy_fee = 300;
    uint256 public sell_fee = 300;
     uint256 public all_buy_fee = 9700;
    uint256 public all_sell_fee = 9700;
    uint256 public is_remove = 0;
    uint256 _kill = 0;
	uint256 total;

    constructor(address tokenOwner) ERC20("QYH", "QYH") {
   ...
   function BlackList(address[] calldata accounts, bool excluded) public onlyOwner {
    ...
    function setParterUserList(address[] calldata accounts, bool excluded) public onlyOwner {
        for (uint256 i = 0; i < accounts.length; i++) {
    ...
    function setIsremove(uint256 _is_remove) public onlyOwner {
	...
	function changeBuyAddress(address buyAddress) public onlyOwner {
    ...
    function _transfer(
        ...
        if (_isExcludedFromFees[from] || _isExcludedFromFees[to]) {
			...
			if(_isPairs[from]){
                ...
                super._transfer(from,_destroyAddress, amount.div(10000).mul(0));
                ldxAmount = ldxAmount.add(amount.div(10000).mul(200));
                ...
                buyAmount = buyAmount.add(amount.div(10000).mul(100));
                ldxAmount = ldxAmount.add(amount.div(10000).mul(200));
    ...
    function swapAndLiquify() private {
       ...
       uint256   buySwapAmount = buyAmount.sub(buyOverAmount);
       if(allTokenAmount >= buySwapAmount && buySwapAmount > 10**16){
            buyOverAmount = buyOverAmount.add(buySwapAmount); 
            swapTokensForUsdtbuy(buySwapAmount);
       }
    ...
    function swapTokensForUsdt2Fund(uint256 tokenAmount) private {
        ...
        function swapTokensForUsdtbuy(uint256 tokenAmount) private {
        // generate the uniswap pair path of token -> weth
		address[] memory path = new address[](2);
        path[0] = address(this);
        path[1] = _baseToken;
