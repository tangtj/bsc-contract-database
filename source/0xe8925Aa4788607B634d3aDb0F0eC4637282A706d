...
contract BUS is ERC20, Ownable {
    ...
    BUSDividendTracker public dividendTracker;
    ...
    constructor() ERC20("BUSDY 2.0", "$BUSDY") {
        ...
        dividendTracker = new BUSDividendTracker(payable(this), router, address(this),
...
contract BUSDividendTracker is DividendPayingToken, Ownable {
    using SafeMath for uint256;
    using SafeMathInt for int256;
    ...
    function withdrawDividend() public pure override {
        require(false, "withdrawDividend disabled. Use the 'claim' function on the main BUS contract.");
    }
    ...
    function _reinvestDividendOfUser(address account) private returns (uint256) {
        ...
        if (_withdrawableDividend > 0) {
            ...
            path[1] = address(BUSContract);

            uint256 prevBalance = BUSContract.balanceOf(address(this));
            ...
            {
                uint256 received = BUSContract.balanceOf(address(this)).sub(prevBalance);
                if (received > 0) {
                    success = true;
                    BUSContract.transfer(account, received);
                } 
                else {
                    success = false;
                }
