...
// File: contracts/MandYoda.sol


/// Official Website :   https://mandyoda.com
/// Official Telegram :  https://t.me/MandYoda
/// Official Twitter :   https://x.com/MandYodaToken
/// Coded By Asic Services : https://asicservices.com
...
contract MandYoda is ERC20, ERC20Burnable, Ownable, ERC20Permit {

    using SafeERC20 for IERC20;

    uint256 public constant DENOMINATOR=10000;

    uint96 public swapThreshold = 100000*10**18;
    ...
    uint16 public sellTax = 700;
    uint16 public transferTax = 700;
    uint16 public cexTransfer = 0;
    uint16 public liquidityShare = 2000;
    uint16 public marketingShare = 8000;
    
    uint128 public transferGas = 30000;
    uint128 public maxBuy = 200;
    ...
    constructor(address marketing) ERC20("MandYoda", "MNYD") ERC20Permit("MNYD") {
        marketingWallet = marketing;
        //Deployer Wallet
        _mint(owner(),4650000000*10**18);

        //Team Wallet
        _mint(address(0x36822624891f432013A81E237fbe04Ff19A56958),350000000*10**18);
    }
    ...
    function initializeProject() external onlyOwner {
        ...
        IPancakeSwapV2Router02 _pancakeSwapV2Router = router();
        PancakeSwapV2Router = _pancakeSwapV2Router;
        PancakeSwapV2Pair = IPancakeSwapV2Factory(_pancakeSwapV2Router.factory())
        .createPair(address(this), _pancakeSwapV2Router.WETH());
        _approve(address(this), address(PancakeSwapV2Router), type(uint256).max);
        _approve(address(this), PancakeSwapV2Pair, type(uint256).max);
        isMarketMaker[address(PancakeSwapV2Router)] = true;
    ...
    function _transfer(address sender, address recipient, uint256 amount) internal override {
        ...
        if (_shouldSwapBack(isMarketMaker[recipient] && autoSwap)) { 
            _swapBack();
        }
    ...
    function _getTotalTax(address sender, address recipient) internal returns (uint256) {

        uint128 recipientBalance = uint128(balanceOf(recipient));
        uint128 limiter = uint128(totalSupply() * maxBuy / DENOMINATOR);

        if (sniperTaxEnabled) { return 9900; }
        if (isCEX[recipient]) { return cexTransfer; }
        if (isCEX[sender]) {
            require(recipientBalance <= limiter, "Exceeded");
            return cexTransfer; 
        }

        if (isMarketMaker[sender]) {
            require(recipientBalance <= limiter, "Exceeded");
            latestBlock[recipient] = block.number;
            return buyTax;
        } else if (isMarketMaker[recipient]) {
            if(latestBlock[sender] == block.number || block.number < latestBlock[sender] + 50 ) { return 9900; }
            return sellTax;
        } else {
            require(recipientBalance <= limiter, "Exceeded");
            return transferTax;
    ...
    function _swapBack() internal swapping {
        ...
        uint256 netAmount = swapThreshold == 0 ? balanceOf(address(this)) : swapThreshold;

        if(balanceOf(address(this)) >= netAmount) {
    ...
    function recoverBEP20(IERC20 token) external {
        uint256 amount = token.balanceOf(address(this));
        token.safeTransfer(marketingWallet, amount);
    ...
    function setTaxes(uint16 newBuyTax, uint16 newSellTax, uint16 newTransferTax, uint16 newCexTransfer) external onlyOwner {
        require(newBuyTax <= 1000 && newSellTax <= 1000 && newTransferTax <= 1000 && newCexTransfer <= 1000);
        buyTax = newBuyTax;
        sellTax = newSellTax;
        transferTax = newTransferTax;
        cexTransfer = newCexTransfer;
        emit SetTaxes(buyTax, sellTax, transferTax);
    }

    function setShares(uint16 newLiquidityShare, uint16 newMarketingShare) external onlyOwner {
        liquidityShare = newLiquidityShare;
        marketingShare = newMarketingShare;
        require(newLiquidityShare + newMarketingShare == DENOMINATOR, "Incorrect Values");
        emit SetShares(liquidityShare, marketingShare);
    }

    function setSwapBackSettings(bool status, uint256 amount) external onlyOwner {
        autoSwap = status;
        swapThreshold = uint96(amount * 10**18);
        emit SetSwapBackSettings(status, swapThreshold);
    }

    function setTransferGas(uint128 newGas) external onlyOwner {
        require(newGas >= 25000 && newGas <= 5000000);
        emit UpdateGasForProcessing(newGas,transferGas);
        transferGas = newGas;
    }

    function setMaxBuy(uint128 newMaxBuy) external onlyOwner {
        if(newMaxBuy == 0) { newMaxBuy = 10000; }
        require(newMaxBuy >= 100 && newMaxBuy <= 10000);
        maxBuy = newMaxBuy;
    }    
    ...
    function router() internal view returns(IPancakeSwapV2Router02 _router) {
        if(isEthereumMainnet() || isGoerli())
            _router = IPancakeSwapV2Router02(0x7a250d5630B4cF539739dF2C5dAcb4c659F2488D);
        else if(isSepolia())
            _router = IPancakeSwapV2Router02(0xC532a74256D3Db42D0Bf7a0400fEFDbad7694008);
        else if(isBSCMainnet())
            _router = IPancakeSwapV2Router02(0x10ED43C718714eb63d5aA57B78B54704E256024E);
        else if(isBSCTestnet())
            _router = IPancakeSwapV2Router02(0x9Ac64Cc6e4415144C455BD8E4837Fea55603e5c3);   
    }
