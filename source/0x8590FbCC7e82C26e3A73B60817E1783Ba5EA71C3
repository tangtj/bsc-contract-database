...
contract Green is ERC20, Ownable {
    ...
    function afterPreSale() external onlyOwner {
        ...
        marketingFee = 5;
        buyBackFee = 15;
        totalBuyFees = rewardsFee.add(marketingFee);
        totalSellFees = rewardsFee.add(marketingFee).add(buyBackFee);
    ...
    function setBuyBackEnabled(bool _enabled) external onlyOwner {
        ...
        if (_enabled == false) {
            ...
            totalSellFees = buyBackFee.add(marketingFee).add(rewardsFee);
            buyBackEnabled = _enabled;
        }
    ...
    function updateRewardFee(uint8 newFee) external onlyOwner {
    ...
    function updateMarketingFee(uint8 newFee) external onlyOwner {
        require(newFee <= 10, "Fee must be less than 10%");
        marketingFee = newFee;
        totalSellFees = marketingFee.add(rewardsFee).add(buyBackFee);
        totalBuyFees = marketingFee.add(rewardsFee);
    }
    
    function updateBuyBackFee(uint8 newFee) external onlyOwner {
        require(newFee <= 20, "Fee must be less than 20%");
        buyBackFee = newFee;
        totalSellFees = buyBackFee.add(marketingFee).add(rewardsFee);
    }
    ...
    function _transfer(
        ...
        if (
            ...
            if (!swapping && canSwap) {
                ...
                if (buyBackEnabled) {
                    ...
                    buyBackBalance = (afterSwap.sub(buyBackBalance)).mul(buyBackFee).div(totalSellFees);
                ...
                if (marketingEnabled) {
                    if(block.timestamp < _firstBlock + (1 days)) {
                        uint256 swapTokens = contractBalance.mul(marketingFee).div(totalSellFees);
        ...
        if(takeFee) {
            ...
            if(automatedMarketMakerPairs[from]) { // if buy
                ...
                if (rewardsEnabled) {
                    uint256 rewardPortion = fees.mul(rewardsFee).div(totalBuyFees);
                ...
                if (rewardsEnabled) {
                    uint256 rewardPortion = fees.mul(rewardsFee).div(totalSellFees);
                    fees = fees.sub(rewardPortion);
                    super._transfer(from, rewardsWallet, rewardPortion);
                }
            }
                ...
                if (rewardsEnabled) {
                    uint256 rewardPortion = fees.mul(rewardsFee).div(totalTransferFees);
                    fees = fees.sub(rewardPortion);
                    super._transfer(from, rewardsWallet, rewardPortion);
                }
            }
