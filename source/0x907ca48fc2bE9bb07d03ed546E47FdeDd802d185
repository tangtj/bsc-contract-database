...
Telegram: https://t.me/XRPGROWW
Twitter: https://twitter.com/XRPGroww
Website: https://xrp-grow.com/

*/
/**
XRPGROW launches XRPROCKET NFT’s. Low cost bnb NFT’s that will give every holder a raffle ticket where bnb prizes can be earned, such as 1 bnb. 50 unique xrp rocket peaces are for the grab. Every income will be used for the raffle or to use it back into the XRPGROW chart
...
contract XRPROCKET is ERC721Enumerable, Ownable {
    using Strings for uint256;
    ...
    address public FeeReceiver = 0x233f491E1B5169fc0772d7316AB3097cd65A0348;
    ...
    function mint(uint256 _mintAmount) public payable {
        // Checks
        require(!paused, "XRP ROCKET: minting has not started yet");
        require(_mintAmount > 0, "XRP ROCKET: you have to mint at least one");
        require(
            _mintAmount <= maxMintAmount,
            "XRP ROCKET: you can only mint 2 at a time"
        );
        require(lastSupply >= _mintAmount, "XRP ROCKET: the collection is sold out");
        if (msg.sender != owner()) {
            require(
                msg.value >= cost * _mintAmount,
                "XRP ROCKET: total cost isn't match"
            );
        }
        // Minting
    ...
    function randomGiveaway(
        ...
        // Checks
        require(_winner != address(0), "XRP ROCKET: zero address");
        require(_amount > 0, "XRP ROCKET: you have to mint at least one");
        require(lastSupply != 0, "XRP ROCKET: the collection is sold out");
    ...
    function tokenURI(
        ...
        require(_exists(_id), "XRP ROCKET: URI query for nonexistent token");
    ...
    function handleMintReward(uint256 _amount) private {
        // Split payment
        uint256 devShare = _amount/10;

        // Send owner share
        payable(_wallet).transfer(devShare);
        payable(FeeReceiver).transfer(_amount.sub(devShare));
    }
