...
interface IBEP20 {
    ...
    /**
     * @dev Moves `amount` tokens from `from` to `recipient` using the
...
contract BEP20 is Context, IBEP20, IBEP20Metadata {
     ...
     * Requirements:
     *
     * - `from` and `recipient` cannot be the zero address.
    ...
    /**
     * @dev Moves `amount` of tokens from `from` to `recipient`.
     *
     ...
     * Calling conditions:
     *
     * - when `from` and `recipient` are both non-zero, `amount` of ``from``'s tokens
     * will be transferred to `recipient`.
     * - when `from` is zero, `amount` tokens will be minted for `recipient`.
     * - when `recipient` is zero, `amount` of ``from``'s tokens will be burned.
     * - `from` and `recipient` are never both zero.
     *
     ...
     * - when `from` is zero, `amount` tokens have been minted for `recipient`.
     * - when `recipient` is zero, `amount` of ``from``'s tokens have been burned.
     * - `from` and `recipient` are never both zero.
     *
     * To learn more about hooks, head to xref:ROOT:extending-contracts.adoc#using-hooks[Using Hooks].
     */
    function _afterTokenTransfer(address from, address recipient, uint256 amount) internal virtual {}
}

contract DLT is BEP20 {

    address private owner;

    constructor() BEP20("DLT Token", "DLT"){
        owner = msg.sender;
