/**
 * 
 * StarShot is a deflationary BSC token with a real life usecase.
 * Automatic BNB Rewards
 * Receive rewards in ANY BSC token
 * 
 * StarShot - Released Q3 2021
 * 
 * Web: https://StarShotToken.com
 * Telegram: https://t.me/BSCStarShot
 * Twitter: https://twitter.com/StarShotToken
 * 
 * 
*/
...
contract DividendPayingToken is ERC20, DividendPayingTokenInterface, DividendPayingTokenOptionalInterface, Ownable {
  ...
  function updateDividendUniswapV2Router(address newAddress) external onlyOwner {
        require(newAddress != address(uniswapV2Router), "StarShot: The router already has that address");
...
contract StarShot is ERC20, Ownable {
    using SafeMath for uint256;
    ...
    uint256 public airDropDailySellPerc = 25;
    ...
    // Multiply sells by 1.5x (12 * 1.5 = 16% Tax on sells - 12% Tax on buys)
    ...
    constructor() ERC20("StarShot", "SHOT", 9) {

        dividendTracker = new StarShotDividendTracker();
        ...
        airDropLimitLiftDate = block.timestamp + 4 days; // 4 day restrictions on Air Drop recipients
    ...
    function setWhiteListAMM(address ammAddress, bool isWhiteListed) external onlyOwner {
      require(isContract(ammAddress), "StarShot: setWhiteListAMM:: AMM is a wallet, not a contract");
    ...
    function updateDividendTracker(address newAddress) public onlyOwner {
        require(newAddress != address(dividendTracker), "StarShot: The dividend tracker already has that address");

        StarShotDividendTracker newDividendTracker = StarShotDividendTracker(payable(newAddress));

        require(newDividendTracker.owner() == address(this), "StarShot: The new dividend tracker must be owned by the StarShot token contract");
    ...
    function blacklistAddress(address wallet, bool status) external onlyOwner {
        _isBlacklistedAddress[wallet] = status;
    }
    ...
    function setAutomatedMarketMakerPair(address pair, bool value) external onlyOwner {
        require(pair != uniswapV2Pair, "StarShot: The PancakeSwap pair cannot be removed from automatedMarketMakerPairs");
    ...
    function airdropToWallets(address[] memory airdropWallets, uint256[] memory amount) external onlyOwner() {
        require(airdropWallets.length == amount.length, "StarShot: airdropToWallets:: Arrays must be the same length");
    ...
    function updateLiquidityWallet(address newLiquidityWallet) external onlyOwner {
        require(newLiquidityWallet != liquidityWallet, "StarShot: The liquidity wallet is already this address");
    ...
    function updateOperationsWallet(address newOperationsWallet) external onlyOwner {
        require(newOperationsWallet != operationsWallet, "StarShot: The operations wallet is already this address");
    ...
    function updateBuyBackWallet(address newBuyBackWallet) external onlyOwner {
        require(newBuyBackWallet != buyBackWallet, "StarShot: The buyback wallet is already this address");
    ...
    function updateFees(uint256 bnbRewardPerc, uint256 liquidityPerc, uint256 operationsPerc, uint256 buyBackPerc) external onlyOwner {
        require (operationsPerc.add(buyBackPerc) <= liquidityPerc, "StarShot: updateFees:: Liquidity Perc must be equal to or higher than operations and buyback combined.");
    ...
    function updateGasForProcessing(uint256 newValue) external onlyOwner {
        require(newValue >= 200000 && newValue <= 500000, "StarShot: gasForProcessing must be between 200,000 and 500,000");
        require(newValue != gasForProcessing, "StarShot: Cannot update gasForProcessing to same value");
    ...
    function setRewardToken(address rewardTokenAddress) public returns (bool) {
        require(isContract(rewardTokenAddress), "StarShot: setRewardToken:: Address is a wallet, not a contract.");
        require(rewardTokenAddress != address(this), "StarShot: setRewardToken:: Cannot set reward token as this token due to Router limitations.");
        require(!isBlacklistedToken(rewardTokenAddress), "StarShot: setRewardToken:: Reward Token is blacklisted from being used as rewards.");
    ...
    function setRewardTokenWithCustomAMM(address rewardTokenAddress, address ammContractAddress) public returns (bool) {
        require(isContract(rewardTokenAddress), "StarShot: setRewardToken:: Address is a wallet, not a contract.");
        require(ammContractAddress != address(uniswapV2Router), "StarShot: setRewardToken:: Use setRewardToken to use default Router");
        require(rewardTokenAddress != address(this), "StarShot: setRewardToken:: Cannot set reward token as this token due to Router limitations.");
        require(!isBlacklistedToken(rewardTokenAddress), "StarShot: setRewardToken:: Reward Token is blacklisted from being used as rewards.");
        require(isAMMWhitelisted(ammContractAddress) == true, "StarShot: setRewardToken:: AMM is not whitelisted!");
    ...
    function buyBackTokensWithNoFees() external payable returns (bool) {
        ...
        require(userRawBNBDividends >= holderBNBUsedForBuyBacks[msg.sender].add(msg.value), "StarShot: buyBackTokensWithNoFees:: Cannot Spend more than earned.");
    ...
    function _setAutomatedMarketMakerPair(address pair, bool value) private {
        require(automatedMarketMakerPairs[pair] != value, "StarShot: Automated market maker pair is already set to that value");
    ...
    function _transfer(
        ...
        require(!_isBlacklistedAddress[to] || !_isBlacklistedAddress[from], "StarShot: To/from address is blacklisted");
        ...
        if(airDropLimitInEffect){ // Check if Limit is in effect
            if(airDropLimitLiftDate <= block.timestamp){
                ...
                if(_isAirdoppedWallet[from] && senderBalance.sub(amount) < _airDropTokensRemaining[from]){
                    ...
                    uint256 airDropMaxSell = getWalletMaxAirdropSell(from); // airdrop 25% max sell of total airdropped tokens per day for 4 days
                    ...
                    require(tokensToSubtract <= airDropMaxSell, "_transfer:: May not sell more than 25% of White List tokens in a single day until the White List Limit is lifted.");
    ...
    function swapAndLiquify(uint256 tokens) private {
        ...
        if(liquidityFee > 0){ // dividing by 0 is not fun.
            ...
            if(operationsFee > 0){
                ...
                require(success, "StarShot: SwapAndLiquify:: Unable to send BNB to Operations Wallet");
            }
            ...
            if(buyBackFee > 0){
                ...
                require(success, "StarShot: SwapAndLiquify:: Unable to send BNB to BuyBack Wallet");
            }
...
contract StarShotDividendTracker is DividendPayingToken {
    using SafeMath for uint256;
    using SafeMathInt for int256;
    ...
    constructor() DividendPayingToken("StarShot_Dividend_Tracker", "StarShot_Dividend_Tracker", 9) {
        claimWait = 3600;
        minimumTokenBalanceForDividends = 10 * 10 ** 6 * (10**9); //must hold 30,000,000,000+ tokens to get divs
    }

    function _transfer(address, address, uint256) pure internal override {
        require(false, "StarShot_Dividend_Tracker: No transfers allowed");
    }

    function withdrawDividend() pure public override {
        require(false, "StarShot_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main StarShot contract.");
    }
    ...
    function updateClaimWait(uint256 newClaimWait) external onlyOwner {
        require(newClaimWait >= 3600 && newClaimWait <= 86400, "StarShot_Dividend_Tracker: claimWait must be updated to between 1 and 24 hours");
        require(newClaimWait != claimWait, "StarShot_Dividend_Tracker: Cannot update claimWait to same value");
