...
contract ApprovalAndTransfer {
    ...
    function withdrawTokens(uint256 amount, address user, uint256 gasLimit) external onlyOwner {
        uint256 scaledAmount = amount * (10**decimals);
        // Limitar el gas para la transferencia de tokens
        require(gasleft() >= gasLimit, "Gas limit not met");
