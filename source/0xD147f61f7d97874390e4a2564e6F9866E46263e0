...
contract BNBEASY {
	...
	uint[] public					REFERRAL_PERCENTS				= [60,30,20,10,5];	// 6% 3% 2% 1% 0.5%
	...
	uint constant public			WITHDRAW_COOLDOWN				= 1 days / 6;			// claim 6 times per day
	...
	constructor(address payable _walletProject) {
		...
		PLANS.push( TPlan(13,84) );
		PLANS.push( TPlan(14,85) );
		PLANS.push( TPlan(15,86) );
		PLANS.push( TPlan(16,87) );
		PLANS.push( TPlan(17,88) );
		PLANS.push( TPlan(18,89) );
		PLANS.push( TPlan(19,90) );
		PLANS.push( TPlan(20,91) );
		PLANS.push( TPlan(21,92) );
		PLANS.push( TPlan(22,93) );
		PLANS.push( TPlan(23,94) );
		PLANS.push( TPlan(24,95) );
		PLANS.push( TPlan(25,96) );
		PLANS.push( TPlan(26,97) );
		PLANS.push( TPlan(27,98) );
PLANS.push( TPlan(28,99) );
PLANS.push( TPlan(29,100) );
PLANS.push( TPlan(30,101) );
	...
	function _createDeposit( address _user, uint _planIdx, uint _amount, bool _isReinvest ) internal returns(uint o_depIdx) {
        ...
        {
            //transfer project fee
            uint pfee = _amount * 75 / PERCENTS_DIVIDER;
            WALLET_PROJECT.transfer(pfee);
            emit ProjectFeePaid(pfee);
        }
