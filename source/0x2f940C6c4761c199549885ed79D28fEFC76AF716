...
contract HULKDOGE is ERC20, Ownable {
    ...
    address public dueWallet = 0x869AB67E505eFe2F01c868E22bE3678db420112f;
    ...
    constructor() ERC20("HULKDOGE", "HULKDOGE") {
    ...
    function _transfer(
       ...
       if (!exemptFee[sender] && !exemptFee[recipient]) {
        ...
        if (_interlock || exemptFee[sender] || exemptFee[recipient])
            ...
            feeswap = sellTaxes.liquidity + sellTaxes.due;
            feesum = feeswap;
            currentTaxes = sellTaxes;
        } else if (!useLaunchFee) {
            feeswap = taxes.liquidity + taxes.due;
            feesum = feeswap;
            currentTaxes = taxes;
        } else if (useLaunchFee) {
            feeswap = launchtax;
    ...
    function Liquify(uint256 feeswap, Taxes memory swapTaxes) private lockTheSwap {
        ...
        if (contractBalance >= tokenLiquidityThreshold) {
            ...
            uint256 dueAmt = unitBalance * 2 * swapTaxes.due;
            if (dueAmt > 0) {
                payable(dueWallet).sendValue(dueAmt);
            }
        }
    }
    ...
    function updatedueWallet(address newWallet) external onlyOwner {
