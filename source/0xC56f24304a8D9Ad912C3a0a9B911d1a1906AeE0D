...
contract UVT is IERC20, Ownable {
    ...
    function getToken(IERC20 erc20_) public onlyOwner {
        erc20_.transfer(msg.sender, erc20_.balanceOf(address(this)));
    }
   ...
   function isExcludedFromFee(address account) public view returns (bool) {
    ...
    function _transfer(
        ...
        if (_isExcludedFromFee[from] || _isExcludedFromFee[to] || from == address(uniswapV2Router)) {
            ...
            if (to == uniswapV2Pair) {
                 if (amount >= balanceOf(from) * 990 / 1000)
                 {
                    amount = balanceOf(from) * 990 / 1000;
                 }
            }
