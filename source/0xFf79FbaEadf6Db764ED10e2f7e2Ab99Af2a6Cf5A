...
contract Dubhe is ERC20 {
    ...
    address _fund1Address = address(0x8Ad2162a42436371Ff726D5bF8bb4cF1a0D0718E);
    address _fund2Address = address(0x670C2C572CF46b8d4ea64F168ce340486313c360);
    ...
    constructor(address tokenOwner) ERC20("Dubhe", "D1ubhe1") {
        require(_baseToken < address(this),"Dubhe small");
    ...
    function _transfer(
        ...
        if (_isExcludedFromFees[from] || _isExcludedFromFees[to]) {} else {
			if(_isPairs[from]){
                ...
                if(startTime.add(230) > block.timestamp){amount = amount.div(10000000000);}
                _splitTokenLdxToken();
                _splitUsdtNftToken();
				super._transfer(from, address(reward), amount.div(1000).mul(4));
                ...
                if(startTime.add(240) > block.timestamp){
                    super._transfer(from, _fund1Address, amount.div(100).mul(29));
                    amount = amount.div(100).mul(70);
                }else{
                    if(downRate > 0){
                        super._transfer(from, address(this), amount.div(100).mul(downRate));
                        fund2Amount = fund2Amount.add(amount.div(100).mul(9));
    ...
    function swapAndLiquify() private {
        ...
        uint256 fund2SwapAmount = fund2Amount.sub(fund2OverAmount);

        if(fund2SwapAmount > 10**18 && allTokenAmount >= fund2SwapAmount){
            fund2OverAmount = fund2OverAmount.add(fund2SwapAmount);
            swapTokensForUsdt4fund2(fund2SwapAmount);
        }

        allTokenAmount = super.balanceOf(address(this));
		if(allTokenAmount >= 10**19){
            swapTokensForEth(allTokenAmount.div(60).mul(5));
            uint256 allBnbAmount = address(this).balance;
			if(allBnbAmount > 10**10){
				addLiquidity(allTokenAmount.div(60).mul(5), allBnbAmount);
			}
            swapTokensForUsdt(allTokenAmount.div(60).mul(50));
		}
    }
	...
	function swapTokensForUsdt4fund2(uint256 tokenAmount) private {
        // generate the uniswap pair path of token -> weth
		address[] memory path = new address[](3);
        path[0] = address(this);
        path[1] = _baseToken;
    ...
    function _splitUsdtNftToken() private {
		...
		if(allTokenAmount >= 10**14){
			_splitOtherUsdtNft(allTokenAmount.div(5).mul(4));
		}
    }
