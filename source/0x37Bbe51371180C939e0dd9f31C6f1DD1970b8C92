...
def __init__(_lp_token: address, _minter: address, _admin: address):
    ...
    self.name = concat("Bagel.fi ", symbol, " Gauge Deposit")
    self.symbol = concat(symbol, "-gauge")

    bagel_token: address = Minter(_minter).token()
    controller: address = Minter(_minter).controller()

    self.lp_token = _lp_token
    self.minter = _minter
    self.admin = _admin
    self.bagel_token = bagel_token
    self.controller = controller
    self.voting_escrow = Controller(controller).voting_escrow()

    self.period_timestamp[0] = block.timestamp
    self.inflation_rate = BAGEL20(bagel_token).rate()
    self.future_epoch_time = BAGEL20(bagel_token).future_epoch_time_write()
...
def _update_liquidity_limit(addr: address, l: uint256, L: uint256):
    """
    @notice Calculate limits which depend on the amount of BAGEL token per-user.
            Effectively it calculates working balances to apply amplification
            of BAGEL production by BAGEL
...
def _checkpoint(addr: address):
    ...
    if prev_future_epoch >= _period_time:
        _token: address = self.bagel_token
        self.future_epoch_time = BAGEL20(_token).future_epoch_time_write()
        new_rate = BAGEL20(_token).rate()
...
def reward_contract() -> address:
    """
    @notice Address of the reward contract providing non-BAGEL incentives for this gauge
...
def set_killed(_is_killed: bool):
    ...
    @dev When killed, the gauge always yields a rate of 0 and so cannot mint BAGEL
