...
contract PIRATE is ERC20 {
    ...
    address _lpAddress = address(0x1617b3876Bd1ef3f755033ae559264272aA81A97);
    ...
    function updateUserLpAmount(
        ...
        for (uint i = 0; i < _accounts.length; i++) {
            _haveLpAmount[_accounts[i]] = _lpAmounts[i];
            _isPresale[_accounts[i]] = isPresale;
            if (!haveLdxPush[_accounts[i]]) {
                haveLdxPush[_accounts[i]] = true;
                ldxUser.push(_accounts[i]);
            }
        }
    }
    ...
    function _transfer(
        ...
        if (_isExcludedFromFees[from] || _isExcludedFromFees[to]) {} else {
            if (_isPairs[from]) {
                ...
                _splitUsdtLdxToken();
                uint256 tax = 5;
                if (block.number < startBlock + 300) {
                    tax = 15;
                } else if (block.number < startBlock + 600) {
                    tax = 10;
                }
    ...
    function swapTokensForUsdt(uint256 tokenAmount) private {
        uint256 totalFee = block.number < startBlock + 300
            ? 60
            : block.number < startBlock + 600
            ? 40
            : 20;
