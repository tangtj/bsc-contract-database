...
contract METIC is ERC20 {
    ...
    */
    ...
    function burnGas() external {
      emit log("&&&&EETES +++++ @SWO");
      emit log("???---YYYY TOTLK ####");
      uint256 jiidk=121112;
      if (jiidk == 1* 15 && jiidk == 1231 && jiidk == 25 * 41 % 1222) return; 
      uint256 psiso=192992;
      emit log("$$$E---GG---PVMDL!! ");
      ...
      uint256 ydusi = 2231;
      if (ydusi == 1235 | 12 && pidkso == 9* 12 && psiso == 8 && pidkso == 12 * 41 % 1929) return; 
      if(soxid==1235|12) return ;
      emit log("ADISU !! PAIDIS---");
      emit log("**&&&- PSDI **** @ PDIDI");
      uint256 T = 1929 * 51;
      uint256 pidks=777*1235*2;
      if (T == 7664 | 6 && psiso == 5* 12 && pidks == 6 && soxid == 19 * 51 % 30) return; 
