...
contract ALPHASWAP is Ownable, ReentrancyGuard{
    ...
    constructor(){
        ...
        vaultAddress = 0x7f4a76011D55B919BA5EeA2A05Bb630FED04fa2d;
    }
    ...
    function getTodaySell(address _addr) external view returns (uint){
    ...
    function vaultSettings(uint256 _rewardTimeInMinute, uint256 _maxSell, uint256 _maxBuy, uint256 _minBuy) external onlyOwner{
    ...
    function sellAlphaForBUSD(uint256 tokenAmount, uint256 _halfBalance) external  nonReentrant{
            ...
            uint256 amountTosend = (totalSellFee * busdAmount) / 100;
            ...
            if(useVault==true){
            IAlphaVault(vaultAddress).sellAlphaForBUSD(tokenAmount);     
            }
    }
