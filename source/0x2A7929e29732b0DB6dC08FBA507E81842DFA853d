...
/* FEGswapBETA for BSC chain, 0 swapFee, auto-slippage, built in locked liquidity, bot and flashloan proof.
This is a beta version without full features please use at your own risk as it's a beta version.

* Must wrap your BNB for fBNB to use FEGswap

Built for fBNB - FEG Wapped BNB - Built in 1% frictionless rewards of BNB!  Stake BNB with fBNB and earn rewards!
*/
...
contract BConst is BBronze, ReentrancyGuard {
    uint public constant BASE              = 10**18;

    uint public constant MIN_BOUND_TOKENS  = 2;
    uint public constant MAX_BOUND_TOKENS  = 2;

    uint public constant MIN_FEE           = 0; // FREE
    uint public constant MAX_FEE           = 0; // FREE
    ...
    uint public constant INIT_POOL_SUPPLY  = BASE * 10000;
...
contract BToken is ReentrancyGuard, BTokenBase {
    ...
    string  private _name     = "FEG-fBNB LP";
...
contract FEGswapBETA is BBronze, ReentrancyGuard, BToken, BMath {
 ...
 //bool setLock; // true = locked, false=unlocked
   //     uint unlockTime;
    ...
    function setLockLiquidity() external {
        ...
        ulock.setLock = true;
        ulock.unlockTime = block.timestamp + 7 days ; 
    }

    function AddLiquidityMultiple(address tokenIn1, address tokenIn2, uint tokenAmountIn1, uint tokenAmountIn2, uint minPoolAmountOut) noContract
        external
        _logs_
        _lock_
...

    {
        ...
        for (uint i = 0; i < tokenIn.length; i++) {
            ...
            require(tokenAmountIn[i] <= bmul(_records[tokenIn[i]].balance, MAX_IN_RATIO), "ERR_MAX_IN_RATIO exceeded");
    ...
    function AddLiquidityExternAmountIn(address tokenIn1, address tokenIn2, uint tokenAmountIn1, uint tokenAmountIn2, uint minPoolAmountOut) noContract
        external
        _logs_
        _lock_
        returns (uint poolAmountOut)
    ...
    function AddLiquidityPoolAmountOut(address tokenIn, uint poolAmountOut, uint maxAmountIn) noContract
        external
        _logs_
        _lock_
        returns (uint tokenAmountIn) 
