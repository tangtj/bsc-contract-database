...
import "./Brothers.sol";
import "./base/V3Swap.sol";
import "@openzeppelin/contracts/utils/Address.sol";

contract GWC is ERC20Burnable, Brothers, V3Swap{
    ...
    constructor(
        ...
        address positionManger_
    ) ERC20(name_, symbol_) V3Swap(usdt_, smartRouter_, positionManger_){
        _mint(msg.sender, 10 ** decimals() * amount);
        // maxSupply = 10 ** decimals() * 1000000000;
        scavenger = scavenger_;
    }

    function setScavenger(address scavenger_) external onlyOwner {
        scavenger = scavenger_;
    }
    ...
    function _transfer(
        ...
        if (address(this) == from || address(this) == to || msg.sender == V3Swap.positionManger) {
        ...
        uint256 remain = Brothers._BrothersReward(10, from, to, (fee * 2), super._transfer);
...
// File: /contracts/base/V3Swap.sol
// SPDX-License-Identifier: GPL-3.0-or-later
pragma solidity ^0.8.0;
...
import '@pancakeswap/masterchef-v3/contracts/interfaces/INonfungiblePositionManager.sol';
...
contract V3Swap is Ownable{

    using Address for address;

    uint256 public _backPool;
    address public smartRouter;
    address token0;
    address token1; //usdt
    address public positionManger;
    mapping(uint24 => uint) fee_tokenId;
    uint24[] public fees;

    constructor(address usdt_, address smartRouter_, address positionManger_) {
        token0 = address(this);
        token1 = usdt_;
        smartRouter = smartRouter_;
        positionManger = positionManger_;
        fees = [100, 500, 2500, 10000];
    }

    function setSwapFees(uint24[] calldata fees_) external onlyOwner{
        fees = fees_;
    }

    function setFeeTokenId(uint24 fee, uint tokenId) external onlyOwner {
        fee_tokenId[fee] = tokenId;
    }
    ...
    function _receivedFee() internal view returns(uint24 fee) {
        address deployer = IPeripheryImmutableState(smartRouter).deployer();
        uint liquidity;
        uint maxLiquidity;
        IPancakeV3PoolState pool;
        for(uint i; i < fees.length; i++) {
            pool = IPancakeV3PoolState(SmartRouterHelper.getPool(deployer, token0, token1, fees[i]));
            if (!address(pool).isContract()) continue;
    ...
    function addLiquidity() external{
        if (_backPool < 10 ether) return;
        uint24 fee = _receivedFee();
        if (fee == 0) return;
        uint256 amount = _backPool;
        amount = amount / 2;
        address recipient = address(this);
        IERC20(token0).approve(smartRouter, amount);
        uint amountOut = IV3SwapRouter(smartRouter).exactInputSingle(IV3SwapRouter.ExactInputSingleParams(token0, token1, fee, recipient, amount, 0, 0));
    
        IERC20(token0).approve(positionManger, amount);
        IERC20(token1).approve(positionManger, amountOut);
        if (token0 > token1) (amount, amountOut) = (amountOut, amount);
        INonfungiblePositionManager(positionManger).increaseLiquidity(INonfungiblePositionManagerStruct.IncreaseLiquidityParams({
            tokenId : fee_tokenId[fee],
...
// File: @pancakeswap/v3-periphery/contracts/interfaces/IPeripheryImmutableState.sol
// SPDX-License-Identifier: GPL-2.0-or-later
pragma solidity >=0.5.0;
...
// File: @pancakeswap/v3-core/contracts/libraries/LowGasSafeMath.sol
// SPDX-License-Identifier: GPL-2.0-or-later
...
// File: @pancakeswap/masterchef-v3/contracts/interfaces/INonfungiblePositionManagerStruct.sol
// SPDX-License-Identifier: GPL-2.0-or-later
...
// File: @pancakeswap/masterchef-v3/contracts/interfaces/INonfungiblePositionManager.sol
// SPDX-License-Identifier: GPL-2.0-or-later
pragma solidity ^0.8.10;
