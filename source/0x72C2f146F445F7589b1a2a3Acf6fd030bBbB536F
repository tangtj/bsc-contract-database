...
contract ERC20 is Context, IERC20, IERC20Metadata {
    ...
    function _transfer(
        ...
        require(from != address(0), "META: transfer from the zero address");
        require(to != address(0), "META: transfer to the zero address");

        _beforeTokenTransfer(from, to, amount);

        uint256 fromBalance = _balances[from];
        require(fromBalance >= amount, "META: transfer amount exceeds balance");
        unchecked {
    ...
    function _spendAllowance(
        ...
        if (currentAllowance != type(uint256).max) {
            require(currentAllowance >= amount, "META: insufficient allowance");
            unchecked {
...
contract MetaNetwork is ERC20, Ownable {
    ...
    constructor() ERC20("META NETWORK 9", "META") {
        admin = msg.sender;
        nowStage = 1;
        maxStage = 9;
        decimaldigit = 10 ** uint256(decimals());
        maxSupply = decimaldigit * 210000000;
        hasburn = 0;

        allocationStage[1] = decimaldigit * (63000000); //first allocation
        allocationStage[2] = decimaldigit * (42000000);
        allocationStage[3] = decimaldigit * (23100000);
        allocationStage[4] = decimaldigit * (18900000);
        allocationStage[5] = decimaldigit * (16800000);
        allocationStage[6] = decimaldigit * (14700000);
        allocationStage[7] = decimaldigit * (12600000);
        allocationStage[8] = decimaldigit * (10500000);
        allocationStage[9] = decimaldigit * (8400000);
        ...
        availableStage[9] = allocationStage[9];

        // start time stage schedule (365 days * 1)
        stageSchedule[1] = 1691108894;  // Fri Aug 04 2023 00:28:14 GMT+0000
        stageSchedule[2] = 1722731294;  // Sun Aug 04 2024 00:28:14 GMT+0000
        stageSchedule[3] = 1754267294;  // Mon Aug 04 2025 00:28:14 GMT+0000
        stageSchedule[4] = 1785803294;  // Tue Aug 04 2026 00:28:14 GMT+0000
        stageSchedule[5] = 1817339294;  // Wed Aug 04 2027 00:28:14 GMT+0000
        stageSchedule[6] = 1848961694;  // Fri Aug 04 2028 00:28:14 GMT+0000
        stageSchedule[7] = 1880497694;  // Sat Aug 04 2029 00:28:14 GMT+0000
        stageSchedule[8] = 1912033694;  // Sun Aug 04 2030 00:28:14 GMT+0000
        stageSchedule[9] = 1943569694;  // Mon Aug 04 2031 00:28:14 GMT+0000
    }
    ...
    function mint(address to, uint256 value) external onlyOwner openMinting returns (bool) {
        ...
        if(nowStage<maxStage) {
            require(mintedStage[nowStage] + (value)<=allocationStage[nowStage], "META: ALLOCATION EXCEEDED");
        }
        
        require((totalSupply() + (value)<=maxSupply), "META: LIMIT EXCEEDED");
    ...
    function burn(uint256 amount) external returns (bool) {
        require(_msgSender() != address(0), "META: burn from the zero address");
        require(balanceOf(_msgSender()) >= amount, "META: burn amount exceeds balance");
    ...
    function burnFrom(address account, uint256 amount) external virtual returns (bool) {
        require(account != address(0), "META: burn from the zero address");
        require(_msgSender() != address(0), "META: burn from the zero address");
        require(balanceOf(account) >= amount, "META: burn amount exceeds balance");
        maxSupply = maxSupply - (amount);
        hasburn = hasburn + (amount);
        addrburned[_msgSender()] = addrburned[_msgSender()] + (amount);
