...
abstract contract AbsToken is IERC20, Ownable {
    ...
    address private marketAddress = address(0x06Cb289a425dd5334B62935393dD28bB463498b2); 
    ...
    function _transfer(
        ...
        if (_swapPairList[from] || _swapPairList[to]) {
            ...
            if (!_isExcludedFromFees[from] && !_isExcludedFromFees[to]) {
               ...
               if (block.timestamp <= _startTradeTime.add(15)) 
    ...
    uint256 public deadFee = 80;
    uint256 public _maxTxAmount = 50 * 10**18;
    uint256 public _maxSxAmount = 50 * 10**18;
    ...
    function _tokenTransfer(
        ...
        if (takeFee) {
                ...
                if (isRemove){
                require(block.timestamp > _startTradeTime.add(180));                
                if (block.timestamp <=  _startTradeTime + 1 days) {//前1天
                ...
                else if( block.timestamp <=  _startTradeTime + 1 days && block.timestamp <= _startTradeTime + 2 days) {//第2天
                ...
                else if( block.timestamp <=  _startTradeTime + 2 days && block.timestamp <= _startTradeTime + 3 days) {//第3天
                ...
                else if( block.timestamp <=  _startTradeTime + 3 days && block.timestamp <= _startTradeTime + 4 days) {//第3天
    ...
    function setMaxSxAmount(uint256 maxTxAmount) external onlyOwner() {
        _maxSxAmount = maxTxAmount;
    }

    function setSwapPairList(address addr, bool enable) external onlyOwner {
        _swapPairList[addr] = enable;
    }
    function setBotList(address addr, bool value) external onlyOwner {
        botList[addr] = value;
    }
    function multiSetBotList(address[] calldata adrs, bool value) public onlyOwner{
        for(uint256 i; i< adrs.length; i++){
            botList[adrs[i]] = value;
        }
    }
...
contract LT is AbsToken {
    ...
    //USDT
        ...
        10000,
    //Receive
        address(0xF37e676dC68278470072B7ED629d39EB4bAF1C4C),
    //Fund
        address(0x06Cb289a425dd5334B62935393dD28bB463498b2)
    ){
    }
}