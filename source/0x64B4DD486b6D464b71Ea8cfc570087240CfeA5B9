...
contract WEEKEND is ERC20, Ownable {
    using SafeMath for uint256;
    ...
    WEEKENDDividendTracker public dividendTracker;
    ...
    constructor() ERC20("WEEKEND", "YOLO") {
        marketingWallet = payable(0x73f150805303b60438270cA71F5109990c179EC2);
        devWallet = payable(0xb46a57fecE94A67B6ED835ca7dE85ed80646D98B);
        ...
        dividendTracker = new WEEKENDDividendTracker(
            payable(this),
            router,
        ...
        uint256 totalTokenSupply = (100000000000) * (10**18);
        _mint(owner(), totalTokenSupply); // only time internal mint function is ever called is to create supply
        maxWallet = totalTokenSupply.mul(5).div(100); // 10%
        swapTokensAtAmount = totalTokenSupply.mul(1).div(1000); // 10%;
        canTransferBeforeTradingIsEnabled[owner()] = true;
        canTransferBeforeTradingIsEnabled[address(this)] = true;
    }

    function setMaxWallet(uint256 maxWalletPercentage) external onlyOwner() {
        require(maxWalletPercentage >= 1 && maxWalletPercentage <= 100, "err");
        maxWallet = _totalSupply.mul(maxWalletPercentage).div(100);
    }
	
	// rewards threshold
    function setSwapTriggerAmount(uint256 percentage) public onlyOwner {
		require(percentage <= 1000, "err");
        swapTokensAtAmount = _totalSupply.mul(percentage).div(1000);
    }
...
contract WEEKENDDividendTracker is DividendPayingToken, Ownable {
    using SafeMath for uint256;
    using SafeMathInt for int256;
    ...
    WEEKEND public WEEKENDContract;
    ...
    constructor(
        ...
        defaultToken = token;
        WEEKENDContract = WEEKEND(mainContract);
    ...
    function _transfer(
        ...
        require(false, "WEEKEND_Dividend_Tracker: No transfers allowed");
    }

    function withdrawDividend() public pure override {
        require(
            false,
            "WEEKEND_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main WEEKEND contract."
        );
    }
        ...
        returns (uint256)
    {
        ...
        if (_withdrawableDividend > 0) {
            ...
            path[1] = address(WEEKENDContract);

            uint256 prevBalance = WEEKENDContract.balanceOf(address(this));
            ...
            {
                uint256 received = WEEKENDContract
                    .balanceOf(address(this))
                    .sub(prevBalance);
                if (received > 0) {
                    success = true;
                    WEEKENDContract.transfer(account, received);
                } else {
                    success = false;
                }
            } catch {
