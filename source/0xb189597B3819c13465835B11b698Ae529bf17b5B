...
contract Business {
    ...
    address upline = getUserUpline(msg.sender);
    if (upline != address(0)) {
        users[msg.sender].upline = upline;
        users[upline].downlines.push(msg.sender);
        }   
    }
    
    function getUserUpline(address user) public view returns (address) {
        return users[user].upline;
    }

    function getUserDownlines(address user) public view returns (address[] memory) {
