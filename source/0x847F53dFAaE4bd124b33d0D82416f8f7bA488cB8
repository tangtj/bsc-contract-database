...
// File Downloads/optima.sol

// Original license: SPDX_License_Identifier: UNLICENSED

pragma solidity ^0.8.0;
contract Optima is Context, IERC20, Ownable {
    ...
    uint256 public _maxHoldingPercents = 100;
    ...
    function setLiquidityFeePercent(uint256 liquidityFee) external onlyOwner() {
        require(liquidityFee <= 100, "Liquidity fee cannot be more than 10%");
        _liquidityFee = liquidityFee;
    }

    function setMarketingFeePercent(uint256 marketingFee) external onlyOwner() {
        require(marketingFee <= 100, "Marketing fee cannot be more than 10%");
        _marketingFee = marketingFee;
    }

    function setBurnFeePercent(uint256 burnFee) external onlyOwner() {
        require(burnFee <= 100, "Burn fee cannot be more than 10%");
        _burnFee = burnFee;
    }

    function setSellBurnFeePercent(uint256 sellBurnFee) external onlyOwner() {
        require(sellBurnFee <= 100, "Sell burn fee cannot be more than 10%");
        _sellBurnFee = sellBurnFee;
    }

    function setSellMarketingFeePercent(uint256 sellMarketingFee) external onlyOwner() {
        require(sellMarketingFee <= 100, "Sell marketing fee cannot be more than 10%");
        _sellMarketingFee = sellMarketingFee;
    }

    function setSellLiquidityFeePercent(uint256 sellLiquidityFee) external onlyOwner() {
        require(sellLiquidityFee <= 100, "Sell liquidity fee cannot be more than 10%");
        _sellLiquidityFee = sellLiquidityFee;
    }

    function setSellTaxFeePercent(uint256 sellTaxFee) external onlyOwner() {
        require(sellTaxFee <= 100, "Sell tax fee cannot be more than 10%");
    ...
    function setMaxHoldingPercents(uint256 maxHoldingPercents) external onlyOwner() {
        ...
        require(maxHoldingPercents <= 100, "Max holding percents cannot be more than 3%");
