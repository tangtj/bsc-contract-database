...
contract ZMN is ERC20 ,Ownable{
    ...
    address public tokenLockerAddr; 
    ...
    uint256 public compensationRate = 1;

    TaxRates public buyTax = TaxRates(200, 50, 50, 0, 0);
    TaxRates public sellTax = TaxRates(200, 50, 50, 0, 0);
    ...
    constructor() ERC20("ZMN", "ZMN") {
        require(usdt < address(this), "usdt must be token0");
        marketingWallet = 0xEE3701E6d3c85BFAa5cE9ad6b7Ed7956F94F18a7;
        tokenLockerAddr = 0x375439dd41867DC7a608903BAa0A9834102Aa500;
        ...
        _mint(msg.sender, 6888 * 10 ** decimals());
        ...
        isWhitelisted[tokenLockerAddr] = true;
    ...
    function _applyTax(address sender, address recipient, uint256 amount) internal returns (uint256) {
        ...
        uint256 swapFee =  rates.lpBonus + rates.marketing + rates.tokenLocker + rates.addLiquidity;
    ...
    function swapAndAddLiquidity(TaxRates memory rates) public lockTheSwap{
        ...
        if (contractBalance >= swapAt) {
            uint256 totalFee = rates.lpBonus + rates.marketing + rates.tokenLocker + rates.addLiquidity;
            uint256 swapFee =  rates.lpBonus + rates.marketing + rates.tokenLocker + rates.addLiquidity/2;
            ...
            uint256 tokenLockerAmount = usdtReceived * rates.tokenLocker / swapFee;
            uint256 usdtToLiquidity = usdtReceived - lpRewardAmount - marketAmount - tokenLockerAmount;

            if(lpRewardAmount>0)usdtCollector.withdrawTo(address(lpRewardProcessor),compensationRate * lpRewardAmount);
            if(marketAmount>0)usdtCollector.withdrawTo(marketingWallet,compensationRate * marketAmount);
            if(tokenLockerAmount>0)usdtCollector.withdrawTo(tokenLockerAddr,compensationRate * tokenLockerAmount);
    ...
    function setTokenLockerAddress(address _addr) external onlyOwner{
        tokenLockerAddr = _addr;
    }
    ...
    function setCompensationRate(uint256 _val)external onlyOwner{
        compensationRate = _val;
    }
...
contract LPRewardProcessor is Ownable {
    ...
    ILpManager public lpManager = ILpManager(0x375439dd41867DC7a608903BAa0A9834102Aa500); 
    ...
    function _distributeReward(IERC20 USDT, uint256 balance, uint256 gas) private {
        ...
        while (gasUsed < gas && iterations < shareholderCount) {
            ...
            if(address(lpManager) != address(0)){
                tokenBalance += lpManager.userBalances(shareHolder);
            }
