...
abstract contract ERC721 is Context,ERC165, IERC721, IERC721Metadata,IERC721Receiver,Ownable {
    ...
    function tokenURI(uint256 tokenId) public view virtual override returns (string memory) {
        require(_exists(tokenId), "WCNFT: URI query for nonexistent token");
    ...
    function getWhiteAccountIth(uint256 ith) public view returns (address WhiteAddress){
        require(ith <_WhiteContractArr.length, "WorldCupNFT: no ith White Adress");
        return _WhiteContractArr[ith];
    }
    function addWhiteAccount(address account) external onlyOwner{
        require(!_Is_WhiteContractArr[account], "WorldCupNFT:Account is already White list");
        require(account.isContract(), "WorldCupNFT: not Contract Adress");
    ...
    function removeWhiteAccount(address account) external onlyOwner{
        require(_Is_WhiteContractArr[account], "WorldCupNFT:Account is already out White list");
...
contract WorldCupNFT is ERC721Enumerable,ReentrancyGuard  {
   using Strings for uint256;
   using SafeMath for uint256;
   ...
   uint256 private WC_feedingProportion=90;
   uint256 private FeedWCPrice = 233*10**18;
   uint256 private FeedWorldPrice = 4*10**17;
   IERC20 public WCToken;
   ...
   IStakingRewards public DeFi_NFT_WCToken;
   bool private bFreshDeFiNFT = false;

   uint256[7] public upgradePropertysWCPrice=[10**19,10**19,10**19,10**19,10**19,10**19,10**19];
  ...
  constructor () ERC721("WorldCup Ham NFT", "Ham NFT") {
  }
  ...
  function getBenefitsInvitation_WC(address address1) public view returns(uint256){
     return _inviters[address1].benefitsInvitation_WC;
  }
  ...
  function getPathUsdtToWC() private view returns (address[] memory) {
     address[] memory path = new address[](2);
     path[0] = address(WCToken);
     path[1] = usdtAddress;
     return path;
   }
  ...
  function getUsdtWCForthPrice() public view returns (uint256,uint256) {
      uint256[] memory price1 = pancakeRouter01.getAmountsOut(10**18,getPathUsdtToWC());
      uint256[] memory price2 = pancakeRouter01.getAmountsOut(10**18,getPathUsdtToForth());
      return (price1[1],price2[1]);
  }
  function getUsdtWCPrice() public view returns (uint256) {
      uint256[] memory price1 = pancakeRouter01.getAmountsOut(10**18,getPathUsdtToWC());
      return price1[1];
  }
  function getWCFeedingProportion(uint256 price) public view returns (uint256 ) {
    if(!bChangePriceAuto){
       return WC_feedingProportion;
    }else{
  ...
  function feedWCForthPrice() public view returns (uint256,uint256) {
    if(!bChangePriceAuto){
       return (FeedWCPrice,FeedWorldPrice);
    }else{
      uint256 tUsdt2WC = 100*10**18;
      uint256 tUsdt2Forth = 1*10**18;
      (tUsdt2WC,tUsdt2Forth) = getUsdtWCForthPrice();
      uint256 feedingProportion =  getWCFeedingProportion(tUsdt2WC);
      uint256 tFeedWCPrice = usdt_FeedPrice.mul(feedingProportion).div(100).mul(10**18).div(tUsdt2WC);
      ...
      return (tFeedWCPrice,tFeedFothPrice);
    }
  }
  function feedWCOnlyPrice() public view returns (uint256) {
    if(!bChangePriceAuto){
       return FeedWCPrice.mul(100).div(WC_feedingProportion);
    }else{
      uint256 tFeedWCPrice = usdt_FeedPrice.mul(10**18).div(getUsdtWCPrice());
      return tFeedWCPrice;
    }
  }
  function getParameters(address account) public view returns (uint256[] memory){
      ...
      uint256 tFeedWCPrice;
      uint256 tFeedWorldPrice;
      (tFeedWCPrice,tFeedWorldPrice) = feedWCForthPrice();
      paraList[1]=tFeedWCPrice;
      paraList[2]=tFeedWorldPrice;
      paraList[3]=totalSupply();
      paraList[4]=getBenefitsInvitation_WC(account);
      paraList[5]=getBenefitsInvitation_Forth(account);
      paraList[6]=getInviterNum(account);
      if(!bChangePriceAuto){
        paraList[7]=FeedWCPrice;
      }else{
        paraList[7]=getUsdtWCPrice();
      }
      return paraList;
  }
  ...
  function burnNFT(uint256 tokenId) public returns (uint256) {
      require(_msgSender() == ownerOf(tokenId),"WorldCupNFT: Only the owner of this Token could Burn It!");
      _burn(tokenId);
      return tokenId;
  }
  function mintNFT_AddInviter(address Inviter) public returns (uint256) {
      require(isWhiteContract(_msgSender()), "WorldCupNFT: Contract not in white list!");
      addInviters(Inviter);
      return mintNFT();
  }
  function mintNFTs(uint256 num) public {
      require(num<=100, "WorldCupNFT: num exceed 100!");
      require(isWhiteContract(_msgSender()), "WorldCupNFT: Contract not in white list!");
      for(uint256 i=0; i<num; ++i) {
          _mintNFT();
      }
  ...
  function transNFT(address _to,uint256 tokenId) public returns (uint256) {
      require(_msgSender() == ownerOf(tokenId),"WorldCupNFT: Only the owner of this Token could transfer It!");
      require(isWhiteContract(_msgSender()), "WorldCupNFT: Contract not in white list!");
      _transfer(_msgSender(),_to,tokenId);
      return tokenId;
  }

  function feed_Foth_WCs(uint256[] calldata tokenIds) nonReentrant public{
      require(tokenIds.length<=100, "WorldCupNFT: num exceed 100!");
      require(isWhiteContract(_msgSender()), "WorldCupNFT: Contract not in white list!");
      _feedPay_Foth_WC(tokenIds.length);
      for(uint256 i=0; i<tokenIds.length; ++i) {
          _feed_Foth_WC(tokenIds[i]);
      }
      emit Feeds(_msgSender(), tokenIds);
      return;
  }
  function feed_Foth_WC(uint256 tokenId) nonReentrant public{
      require(isWhiteContract(_msgSender()), "WorldCupNFT: Contract not in white list!");
      _feedPay_Foth_WC(1);
      _feed_Foth_WC(tokenId);
      emit Feed(_msgSender(), tokenId);
      return;
  }
  function _feed_Foth_WC(uint256 tokenId) internal{
      if(_msgSender() != ownerOf(tokenId)){
         if(bFreshDeFiNFT){
           require(_msgSender() == DeFi_NFT_WCToken.ownerTokenId(tokenId), "WorldCupNFT: 1 Only the owner of this Token could feed it");
         }
         else{
           require(_msgSender() == ownerOf(tokenId), "WorldCupNFT: 2 Only the owner of this Token could feed it");
         }
      }
      require(_exists(tokenId), "ERC721: Existent ID");
      require(block.timestamp >= _NftPropertys[tokenId].lastUpdateTime + 12*3600,"WorldCupNFT: onwer can only feed once in 12 hours!");
      require(_NftPropertys[tokenId].value<maxValue, "WorldCupNFT: exceed max Degree");
     ...
     if(bFreshDeFiNFT && _msgSender() != ownerOf(tokenId)){
        DeFi_NFT_WCToken.stakeFresh(_msgSender(),tokenId);
     }
     return ;
  }
  function _feedPay_Foth_WC(uint256 num) internal{
     uint256 tFeedWCPrice;
     uint256 tFeedWorldPrice;
     (tFeedWCPrice,tFeedWorldPrice) = feedWCForthPrice();
     tFeedWCPrice = tFeedWCPrice.mul(num);
     tFeedWorldPrice = tFeedWorldPrice.mul(num);
     address address1 =  _inviters[_msgSender()].inviter;
     if(address1==address(0)){
         WCToken.safeTransferFrom(_msgSender(), address(0), tFeedWCPrice);
         ForthToken.safeTransferFrom(_msgSender(), FundAdress, tFeedWorldPrice);
     }
     else{
       address address2 =  _inviters[address1].inviter;
       if(address2==address(0))
       {
           WCToken.safeTransferFrom(_msgSender(), address1, tFeedWCPrice.mul(10).div(100));
           WCToken.safeTransferFrom(_msgSender(), address(0), tFeedWCPrice.mul(90).div(100));

           ForthToken.safeTransferFrom(_msgSender(), address1, tFeedWorldPrice.mul(10).div(100));
           ForthToken.safeTransferFrom(_msgSender(), FundAdress, tFeedWorldPrice.mul(90).div(100));

           _inviters[address1].benefitsInvitation_WC = _inviters[address1].benefitsInvitation_WC.add(tFeedWCPrice.mul(10).div(100));
           _inviters[address1].benefitsInvitation_Forth = _inviters[address1].benefitsInvitation_Forth.add(tFeedWorldPrice.mul(10).div(100));
       }
       else
       {
         WCToken.safeTransferFrom(_msgSender(), address1, tFeedWCPrice.mul(10).div(100));
         WCToken.safeTransferFrom(_msgSender(), address2, tFeedWCPrice.mul(5).div(100));
         WCToken.safeTransferFrom(_msgSender(),address(0),tFeedWCPrice.mul(85).div(100));

         ForthToken.safeTransferFrom(_msgSender(), address1, tFeedWorldPrice.mul(10).div(100));
         ForthToken.safeTransferFrom(_msgSender(), address2, tFeedWorldPrice.mul(5).div(100));
         ForthToken.safeTransferFrom(_msgSender(), FundAdress,tFeedWorldPrice.mul(85).div(100));

         _inviters[address1].benefitsInvitation_WC = _inviters[address1].benefitsInvitation_WC.add(tFeedWCPrice.mul(10).div(100));
         _inviters[address1].benefitsInvitation_Forth = _inviters[address1].benefitsInvitation_Forth.add(tFeedWorldPrice.mul(10).div(100));

         _inviters[address2].benefitsInvitation_WC = _inviters[address2].benefitsInvitation_WC.add(tFeedWCPrice.mul(5).div(100));
         _inviters[address2].benefitsInvitation_Forth = _inviters[address2].benefitsInvitation_Forth.add(tFeedWorldPrice.mul(5).div(100));
       }
     }
  ...
  function upgrade_Propertys(uint256 tokenId,uint256 ith,uint256 degree) nonReentrant external{
       require(_exists(tokenId), "WorldCupNFT: Existent ID");
       require(ith<7, "WorldCupNFT: Existent ith");
       require(isWhiteContract(_msgSender()), "WorldCupNFT: Contract not in white list!");

       WCToken.safeTransferFrom(_msgSender(), FundAdress, upgradePropertysWCPrice[ith].mul(degree));
  ...
  function addInviters(address Inviter) internal{
       require(_msgSender() != Inviter,"WorldCupNFT: Inviter cannot be self!");
       require(Inviter != address(0), "WorldCupNFT: Inviter cannot be zero address!");
       require(isWhiteContract(_msgSender()), "WorldCupNFT: Contract not in white list!");
  ...
  //---write onlyOwner---//
  function setTokens(address tWCToken,address tWorldToken,address fund_Adress) external onlyOwner {
      WCToken = IERC20(tWCToken);
      ForthToken = IERC20(tWorldToken);
      FundAdress = fund_Adress;
  }
  function setTokensDeFi(address tDeFi_NFT_WCToken,bool tBFreshDeFiNFT) external onlyOwner {
      DeFi_NFT_WCToken = IStakingRewards(tDeFi_NFT_WCToken);
  ...
  function setFeed_Price(uint256 newFeedWCPrice,uint256 newFeedFothPrice) onlyOwner public{
      FeedWCPrice = newFeedWCPrice;
      FeedWorldPrice = newFeedFothPrice;
  }

  function setupgradePropertysWCPrice(uint256[] calldata WCPriceArr) onlyOwner external{
      require(WCPriceArr.length==7, "WorldCupNFT:length of WCPriceArr is not 7");
      for (uint256 i=0; i < WCPriceArr.length; i++){
          upgradePropertysWCPrice[i]=WCPriceArr[i];
      }
  }

  function setWC_feedingProportion(uint256 newWC_feedingProportion) onlyOwner public{
      WC_feedingProportion = newWC_feedingProportion;
  }
