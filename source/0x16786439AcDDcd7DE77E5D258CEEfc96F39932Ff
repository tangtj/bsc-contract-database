...
contract tester is IERC20, tokenStaking, Ownable {
    ...
    uint256 private maxDropAmount = ( _totalSupply * 10000 ) / 10000;
    uint256 minBuyLevelOne = ( _totalSupply * 50 ) / 100000;
    uint256 minBuyLevelTwo = ( _totalSupply * 100 ) / 100000;
    uint256 minBuyLevelThree = ( _totalSupply * 200 ) / 100000;
    address internal constant DEAD = 0x000000000000000000000000000000000000dEaD;
    address internal utility_receiver = 0x3C9876CF1D3271674f377f593F96d4838b481f35;
    ...
    function setBuyMultiplier(address sender, address recipient, uint256 amount) internal {
        if(sender == pair && amount >= minBuyLevelThree && buyMultiplier[recipient] >= 6){buyMultiplier[recipient] = buyMultiplier[recipient].add(1);}
        if(sender == pair && amount >= minBuyLevelTwo && buyMultiplier[recipient] >= 3 && buyMultiplier[recipient] < 6){buyMultiplier[recipient] = buyMultiplier[recipient].add(1);}
        if(sender == pair && amount >= minBuyLevelOne && buyMultiplier[recipient] < 3){buyMultiplier[recipient] = buyMultiplier[recipient].add(1);}
        if(sender == pair && buyMultiplier[recipient] == 0){buyMultiplier[recipient] = uint256(1);}
    ...
    function setBuyMultiplierLevels(uint256 levelOne, uint256 levelTwo, uint256 levelThree) external onlyOwner {
        minBuyLevelOne = _totalSupply.mul(levelOne).div(100000);
        minBuyLevelTwo = _totalSupply.mul(levelTwo).div(100000);
        minBuyLevelThree = _totalSupply.mul(levelThree).div(100000);
    }
    ...
    function setisDividendExempt(address holder, bool exempt) external onlyOwner {
        ...
        if(exempt){setShare(holder, holder, 0);}
        if(buyMultiplier[holder] > 0){setShare(holder, holder, balanceOf(holder).mul(buyMultiplier[holder]));}
        else{setShare(holder, holder, balanceOf(holder));}
    }
    ...
    function processRewards(address sender, address recipient) internal {
        ...
        if(!isDividendExempt[sender]){setShare(recipient, sender, balanceOf(sender));}
        if(!isDividendExempt[recipient]){setShare(recipient, recipient, balanceOf(recipient));}
        if(!isDividendExempt[recipient] && sender == pair){
            setShare(recipient, recipient, balanceOf(recipient).mul(buyMultiplier[recipient]));}
        if(distributionTime.add(distributionInterval) <= block.timestamp && tradingAllowed && 
            ...
            dividendsBeingDistributed = currentDividends;
            currentDividends = uint256(0);
            distributionTime = block.timestamp;
            process(gasAmount);}
        if(dividendsBeingDistributed == 0 && distributingReflections){distributingReflections = false;}
    ...
    function setExcess(uint256 amount) internal {
        ...
        eventStats[reflectionEvent].totalExcess = eventStats[reflectionEvent].totalExcess.add(amount);
    }
    ...
    function setMaxDropAmount(uint256 amount) external onlyOwner {
        maxDropAmount = _totalSupply.mul(amount).div(10000);
    }
    ...
    function setShare(address recipient, address shareholder, uint256 amount) internal {
        ...
        if(recipient == pair && !isFeeExempt[shareholder]){
    ...
    function distributeDividend(address shareholder) internal {
        ...
        if(amount > uint256(0)){
            ...
            setShare(shareholder, shareholder, balanceOf(shareholder));}
    }
    ...
    function process(uint256 gas) public {
        ...
        while(gasUsed < gas && iterations < shareholderCount) {
            ...
            currentIndex++;
            iterations++;}
        if(iterations >= shareholderCount){distributingReflections = false;}
    }
