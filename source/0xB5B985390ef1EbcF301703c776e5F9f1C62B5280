...
contract AutooolContract is Context , owned {
...
//--------------------------------EVENT SECTION --------------------------------------
    ...
    function updateTeamNum(uint pid, uint indx) internal {
    ...
    function autoPoolPosition(address user,uint pid, bool defaultReg) internal {
        ...
        if (!defaultReg){
            ...
            updateTeamNum(pid,indx); // update team number
    ...
    function plannetSponserIncome(uint amount, address _user) internal {
       ...
       address receiver =  userInfos[_user].referrer;
        ...
        if (userInfos[receiver].plannetPurchase[1]==0){
    ...
    function plannetLevelIncome(address user) internal {
       ...
       for(uint i=1;i<=15;i++){
           ...
           if (receiver==address(0) || userInfos[receiver].plannetPurchase[1]==0){
