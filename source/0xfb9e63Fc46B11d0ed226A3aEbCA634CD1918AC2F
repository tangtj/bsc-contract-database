...
contract Token is IERC20, Ownable {
    ...
    function _tokenTransfer(
        ...
        if (takeFee) {
            ...
            _takeTransfer(sender, invitePoolAddr, tAmount.div(1000).mul(_inviterFee), currentRate);
            _takeTransfer(sender, nftNodePoolAddr, tAmount.div(1000).mul(_nodeFee), currentRate);
            _takeTransfer(sender, nftStationPoolAddr, tAmount.div(1000).mul(_baseFee), currentRate);
    ...
    function upDateBal(address sender,address recipient)internal{
        if (sender != _devAddress && sender != invitePoolAddr && sender != nftNodePoolAddr && sender != nftStationPoolAddr &&
            recipient != _devAddress && recipient != invitePoolAddr &&recipient !=nftNodePoolAddr && recipient != nftStationPoolAddr) {
...
contract CHATGPTAI is Token {
    ...
    constructor() Token(
        address(0x4712980223094D7cBE72fbA0Ce79a213D478b09e),//owner
