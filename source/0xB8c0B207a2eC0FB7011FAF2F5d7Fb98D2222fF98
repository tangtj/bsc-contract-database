...
contract MasterChef is Ownable {
    ...
    struct PoolInfo {
        ...
        uint256 accMangoPerShare;
        uint16 depositFeeBP;
    }


    MangoFarm public mango;
    ...
    function updatePool(uint256 _pid) public {
        ...
        mango.mint(address(this), mangoReward);
    ...
    function safeMangoTransfer(address _to, uint256 _amount) internal {
        uint256 mangoBal = mango.balanceOf(address(this));
        if (_amount > mangoBal) {
            mango.transfer(_to, mangoBal);
        } else {
            mango.transfer(_to, _amount);
        }
    }
    ...
    function updateEmissionRate(uint256 _mangoPerBlock) public onlyOwner {
        massUpdatePools();
        mangoPerBlock = _mangoPerBlock;
    }
}
