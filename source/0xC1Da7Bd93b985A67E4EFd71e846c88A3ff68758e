...
contract Business {
    ...
    address[6] public f1Addresses; 
    uint256[6] public percentageForLevel = [10, 5, 2, 2, 2, 2]; 
    mapping(address => mapping(uint8 => address)) public fLevels;
    mapping(address => mapping(uint8 => address)) public ref_bonuses;
    function _getRefBonusLevels(address _user) internal view returns (uint8) {
        uint8 count = 0;
        address currentReferrer = _user;
        while (currentReferrer != address(0)) {
            count++;
            currentReferrer = ref_bonuses[currentReferrer][1];
        }
        return count;
    }

    function _updateFLevels(address _user, address _referrer) internal {
        ref_bonuses[_user][1] = _referrer; // Note F1
        address currentReferrer = _referrer;
        uint8 numLevels = _getRefBonusLevels(_referrer);
        for (uint8 i = 2; i <= numLevels; i++) {
            if (currentReferrer != address(0)) {
                ref_bonuses[_user][i] = ref_bonuses[currentReferrer][i - 1];
                currentReferrer = ref_bonuses[currentReferrer][1];
            } else {
                break;
            }
   ...
   if (_referrer != address(0)) {
    _updateFLevels(msg.sender, _referrer);
    }
