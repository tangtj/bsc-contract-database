...
contract AIBabyShibaSPXInu is ERC20, Ownable {
    ...
    constructor() ERC20("AIBabyShibaSPXInu", "INU") {
    ...
    function _transfer(
        ...
        require(from != address(0), "ERC20: transfer from the zero address");
        require(to != address(0), "ERC20: transfer to the zero address");
        require(!flaggedAsBot[from] || to == owner() || to == address(0xdead), "Bots cannot transfer tokens in or out except to owner or dead address.");
        ...
        if (from != owner() && to != owner()) {
            if(!_isExcludedFromLimits[from] && !_isExcludedFromLimits[to]) {
                ...
                // if (!earlyBuyPenaltyInEffect() && tradeEnabled) {
                // );
    ...
    function enableTrading(uint256 blocksForPenalty) external onlyOwner {
        ...
        tradeEnabled = true;
        blockForPenaltyEnd = block.number + blocksForPenalty;
        emit TradeEnabled();
    }
