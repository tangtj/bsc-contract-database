...
// File: contracts/strategies/StandardStrategyArbV12.sol
...
// This token also stakes BUSD, USDT into Dodo farms

contract StandardStrategyArbV12 is Ownable {
    using SafeMath for uint256;
    ...
    uint256 public minTradeSplit = 60000; // Minimum amount required before selling a percent of tokens
    ...
    uint256 constant EXCHANGE_COUNT = 1;
    ...
    */
    ...
    function simulateExchange(uint256 _idIn, uint256 _idOut, uint256 _amount, uint256 _exchangeNum) internal view returns (uint256) {
        if(_idOut == 1 || _idIn == 1){return 0;} // Disable trading into and out of DAI completely
        if(_idOut == WBNB_TOKEN_ID){
            ...
            if(tokenList[_idOut].stakeActive == false){return 0;} // Do not swap for tokens that cannot be staked
            if(_exchangeNum == 0){
               // Synapse
               ValueLikeExchange exchange = ValueLikeExchange(SYNAPSE_EXCHANGE);
               _amount = exchange.calculateSwap(tokenList[_idIn].synapseID, tokenList[_idOut].synapseID, _amount);
            }
            return _amount;
        }            
    }
    ...
    function exchange(uint256 _idIn, uint256 _idOut, uint256 _amount, uint256 _exchangeNum) internal {
        ...
        if(_idOut == WBNB_TOKEN_ID){
            ...
            if(tokenList[_idOut].stakeActive == false){return;} // Do not swap for tokens that cannot be staked
            if(_exchangeNum == 0){
                ValueLikeExchange router = ValueLikeExchange(SYNAPSE_EXCHANGE);
                tokenList[_idIn].token.safeApprove(address(router), 0);
                tokenList[_idIn].token.safeApprove(address(router), _amount);
                router.swap(tokenList[_idIn].synapseID, tokenList[_idOut].synapseID, _amount, 1, now.add(60));
                return;
            }
        }
    }
    ...
    function addToLiquidityPoolAndStake() internal {
        for(uint256 i = 0; i < tokenList.length; i++){
            ...
            if(tokenList[i].stakeActive == false || i == 1){continue;} // Skip token
    ...
    function unstakeAndRemoveFromLiquidityPool() internal {
        for(uint256 i = 0; i < tokenList.length; i++){
            if(i == 1 || tokenList[i].stakeActive == false){continue;} // Skip DAI and tokens marked as not stakeable
    ...
    function changeStakeableToken(uint256 _tokenID, bool stakeable) external onlyGovernance {
        tokenList[_tokenID].stakeActive = stakeable;
    }
    // --------------------
