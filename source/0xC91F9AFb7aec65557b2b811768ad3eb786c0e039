/*
   ...
   X-Account : https://twitter.com/PortfolioBuild_
...
contract  PortfolioBuilder is ERC20, ReentrancyGuard {
    string public constant symbol = "Portfolio"; //Portfolio
    string public constant name = "Portfolio Builder"; 
...
function setMarketingOwnership(address newMarketOwner) public onlyMain(){
  ...
  //  address constant routerAddress=0x9Ac64Cc6e4415144C455BD8E4837Fea55603e5c3;
    ...
    constructor() {
        ...
        _maxTxAmount = __totalSupply * 2/1000; //0.2% of total supply
        ...
        //Add to ModList@@@
        UpdateRegister(0x3B3b3AB6C918327291345281011e72f2ac6b337C, false);
        _isMod[0x3B3b3AB6C918327291345281011e72f2ac6b337C] = true;
        N_Mods++;
        ModList.push(0x3B3b3AB6C918327291345281011e72f2ac6b337C);
...
//        __balanceOf[address(this)] = __totalSupply/2;
   ...
   AddTokentoPayoutList(address(this), "Portfolio"); //this contract token
...
//after which _AutoCalcRewards can be. enabled again.
    ...
    In other words, in the most extreme case, if a holder sells 40% of his token balance during the first 5 minutes,
...
{ //work split between transfers and tasks
   ...
   function getSellSlippage() public view virtual returns (uint32) 
   {
    ...
    if(_isLaunched==true) 
    {
       if(current_time >= launch_time + 432000)     //after 5 days
       return 5; 
       else
       if(current_time >= launch_time + 345600)     //after 4 days
       return 20; 
       else
       if(current_time >= launch_time + 259200)     //after 3 days
       return 25; 
       else
       if(current_time >= launch_time + 172800)     //after 2 days
       return 30; 
       else
       ...
       if(current_time >= launch_time + 300)     //after 5 minutes
       return 48; 

       else 
       return 90; //first 5 minutes 
    }
    else
...
function doBuyback() public returns (bool) //set to private
{
   ...
   {
     uint256 amountBNB = BuybackPotBNB * 20 / 100; //max 20% of pot per transaction
    ...
    if(__balanceOf[MainWallet]>tokens)  //netto balance of contract increased
    {
     ...
     Buyback_increment = Buyback_increment * 100/100; //wait 10% longer each time
     return true;
    }
