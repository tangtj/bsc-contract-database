...
contract XWOID is ERC20, Ownable {
    ...
    function _internalTransfer(
        ...
        require(!blacklist[from] && !blacklist[to], "Restricted transaction.");

        if (!canAddLiquidityBeforeLaunch[from]) {
            require(isLaunched(), "Trading not open yet");
        }
    ...
    function setBuyFees(uint256 _sellFee) external onlyOwner {
        sellFee = _sellFee;
    }

    function setFeeReceivers(address _devWallet) external onlyOwner {
        devWallet = _devWallet;
    }

    function setExcludedFromFee(address addr, bool isExclude)
        public
        onlyOwner
        returns (bool)
    {
        isExcludedFromFee[addr] = isExclude;
        return true;
    }

    function setOpenSwapTime(uint256 time) public onlyOwner {
        openSwapTime = time;
    }


    function setCanAddLiquidityBeforeLaunch(address _address, bool enable)
        public
        onlyOwner
    {
    ...
    function setBlack(address addr, bool enable) external onlyOwner {
        blacklist[addr] = enable;
    }

    function setBlackList(address[] memory addressList, bool enable)
        external
        onlyOwner
    {
        for(uint i =0; i<addressList.length;i++){
               blacklist[addressList[i]] = enable;
        }
    ...
    function getPair(uint256 index) public view returns (address) {
        require(index <= _pairs.length() - 1, "Error: index out of bounds");
        return _pairs.at(index);
    }

    function isLaunched() public view returns (bool) {
        return block.timestamp >= openSwapTime && openSwapTime != 0;
    }
    ...
    function clearStuckEthBalance() external onlyOwner {
        ...
        require(success, "BNB: BNB_TRANSFER_FAILED");
    }
