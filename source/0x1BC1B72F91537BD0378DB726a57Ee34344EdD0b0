...
contract PIPINU is ERC20, Ownable {
    ...
    event UpdateFees(uint256 buyBackFeeOnSell, uint256 marketingFeeOnSell);
    event SwapAndSend(uint256 tokensSwapped, uint256 bnbSend);
    event SwapTokensAtAmountUpdated(uint256 swapTokensAtAmount);

    constructor () ERC20("PIPINU", "PIPINU") 
    {   
        address router;
        if (block.chainid == 56) {
        ...
        totalFeeOnSell = marketingFeeOnSell + buyBackFeeOnSell;

        marketingWallet = 0xA15DE9acc504516AE36A985d052c4dC8d20c1ae6;
    ...
    function updateFees(uint256 _marketingFeeOnSell, uint256 _buyBackFeeOnSell) external onlyOwner {
        marketingFeeOnSell = _marketingFeeOnSell;
        buyBackFeeOnSell = _buyBackFeeOnSell;
        
        totalFeeOnSell = marketingFeeOnSell + buyBackFeeOnSell;
        require(_marketingFeeOnSell >= 5, "Marketing Fee need to be more than 1.");
        require(_buyBackFeeOnSell >= 1, "Buyback Fee need to be more than 1.");
        require(totalFeeOnSell <= 20, "Total Fees cannot exceed the maximum");

        emit UpdateFees(buyBackFeeOnSell, marketingFeeOnSell);
    }
    ...
    function swapAndSend(uint256 tokenAmount) private {
        ...
        uint256 marketing = newBalance * marketingFeeOnSell / totalFeeOnSell;
