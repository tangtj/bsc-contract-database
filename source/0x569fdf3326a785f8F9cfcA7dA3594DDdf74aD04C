...
contract SleepCare is Context, IERC20, Ownable {
    ...
    uint256 public _maxTxAmount = 50*1e4  * 10**18;
    ...
    constructor () public {
        ...
        _isExcludedFromFee[advertisementWallet] = true;
    ...
    function setBurnFee(uint256 fee) external onlyOwner() {
        _burnFee = fee;
        require(fee + _sellTaxFee + _sellAdvestisementFee <= 25, "Fees is high for sell");
        require(fee + _buyTaxFee + _buyAdvestisementFee <= 25, "Fees is high for buy");
    }

    function setTaxFee(uint256 buyTaxFee, uint256 sellTaxFee) external onlyOwner() {
        _sellTaxFee = sellTaxFee;
        _buyTaxFee = buyTaxFee;
        require(_burnFee + _sellTaxFee + _sellAdvestisementFee <= 25, "high fee");
        require(_burnFee + _buyTaxFee + _buyAdvestisementFee <= 25, "high fee");
    }

    function setAdvestisementFee(uint256 buyAdvestisementFee, uint256 sellAdvestisementFee) external onlyOwner() {
    ...
    function setMaxTxAmount(uint256 newAmount) external onlyOwner {
        require(newAmount >= 50000, "Cannot set new amount lower than 50 000");
