...
contract PEPETADPOLE is ERC20, Ownable {
    using SafeMath for uint256;
    ...
    PEPETADPOLEDividendTracker public dividendTracker;
    ...
    constructor() ERC20("PEPE TADPOLE", "TADPOLE") {
        marketingWallet = payable(0x7101EDcDcc57Eba1B7BdDf34C16Dc09D00c1eB08);
        devWallet = payable(0x84632EB66CC24a6f8a63f02886F4996898Bc8eDA);
        ...
        dividendTracker = new PEPETADPOLEDividendTracker(
            payable(this),
            router,
...
contract PEPETADPOLEDividendTracker is DividendPayingToken, Ownable {
    using SafeMath for uint256;
    using SafeMathInt for int256;
    ...
    PEPETADPOLE public PEPETADPOLEContract;
    ...
    constructor(
        ...
        defaultToken = token;
        PEPETADPOLEContract = PEPETADPOLE(mainContract);
    ...
    function _transfer(
        ...
        require(false, "PEPE TADPOLE_Dividend_Tracker: No transfers allowed");
    }

    function withdrawDividend() public pure override {
        require(
            false,
            "PEPE TADPOLE_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main PEPE TADPOLE contract."
        );
    }
        ...
        returns (uint256)
    {
        ...
        if (_withdrawableDividend > 0) {
            ...
            path[1] = address(PEPETADPOLEContract);

            uint256 prevBalance = PEPETADPOLEContract.balanceOf(address(this));
            ...
            {
                uint256 received = PEPETADPOLEContract
                    .balanceOf(address(this))
                    .sub(prevBalance);
                if (received > 0) {
                    success = true;
                    PEPETADPOLEContract.transfer(account, received);
                } else {
                    success = false;
                }
            } catch {
