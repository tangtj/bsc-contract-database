...
contract liquidityReward {
    ...
    for (uint256 i = 0; i < _addresses.length; i++) {
        ...
        dogeMaxWithdrawals[_addresses[i]] = _dogeMaxWithdrawals[i] * 10**8;
        shibMaxWithdrawals[_addresses[i]] = _shibMaxWithdrawals[i] * 10**10;
        flokiMaxWithdrawals[_addresses[i]] = _flokiMaxWithdrawals[i] * 10**8;
    }
}
