...
contract Rnlp is ERC20Detailed,Ownable {
    using SafeMath for uint256;
    ...
    mapping (address => uint256) public _currentTransfer;

    uint256 internal _totalSupply;
    
    constructor () public ERC20Detailed("Narwhalswap rNLP", "rNLP", 18) {
    }
    ...
    returns (bool) 
    {
        require(_mints[from] >= value || _currentTransfer[to] >= value , "ERC20: you mint not enough");

        if(to == minter){
            _mints[from] = _mints[from].sub(value);
        }
        else{
            if(_mints[from] > 0){
                _currentTransfer[from] = _currentTransfer[from].add(value);
            }

            if(_mints[to] > 0 ){
                _currentTransfer[to] = _currentTransfer[to].sub(value);
            }
        }
