...
contract FFI is ERC20,Ownable {
    ...
    constructor() ERC20("Free Finance Invitation", "FFI") Ownable() {
        users[msg.sender] = User({parent:address(0)});
    ...
    event logRelation(address from,address to,uint time);

    function checkRelation(address to) private view returns(bool){
        if (users[to].parent != address(0)){
            return true;
        }
        return false;
    }
    ...
    function _afterTokenTransfer(address from,address to,uint256 amount) internal override {
        ...
        if (amount >= 100000000000000000){
            if (!checkRelation(to)){
                users[to] = User({parent:from});
                emit logRelation(from, to,block.timestamp);
            }
        }
    }
    ...
    function getParents(address user) public view returns(address[] memory) {
        ...
        User storage cur = users[user];
        uint maxLength = 0;
        for(uint i=0; cur.parent != address(0) && i < 100 ;cur =users[cur.parent] ){
            temp[i] = cur.parent;
            i++;
            maxLength = i;
        }
    ...
    function faucet() public {
        address addr = msg.sender;
        require(users[addr].parent != address(0),"noParent");
        _mint(addr, tokenNum(500));
    }
