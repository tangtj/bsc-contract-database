...
contract Savior is ERC20, Ownable {
    ...
    constructor() ERC20("Savior", "SVR") {
        address newOwner = address(0xbF2Dd314c8beE43dcF778870D48693e8d869A22A);
        _decimals= 18;
    
        uint256 totalSupply = 1e15 * (10 ** _decimals);

        rewardsSellFee = 2;
        marketingSellFee = 1;
        burnSellFee = 1;
        treasurySellFee = 1;
        totalSellFees = rewardsSellFee + marketingSellFee + burnSellFee + treasurySellFee;
        
        rewardsBuyFee = 2;
        marketingBuyFee = 1;
        burnBuyFee = 1;
        treasuryBuyFee = 1;
        totalBuyFees = rewardsBuyFee + marketingBuyFee + burnBuyFee + treasuryBuyFee;
        
    	dividendTracker = new DividendTracker();
    	
    	marketingWallet = address(0x909cA352bD1EC7C3a3fEA1C6D249d13a4F8343c4); // set as marketing wallet
        treasuryWallet = address(0xE0D1F22B95e3fAF845505258721bb2D66990e157); // set as marketing wallet
    ...
    function updateBuyFees(uint256 _marketingFee, uint256 _rewardsFee, uint256 _burnFee, uint256 _treasuryFee) external onlyOwner {
        marketingBuyFee = _marketingFee;
        rewardsBuyFee = _rewardsFee;
        burnBuyFee = _burnFee;
        treasuryBuyFee = _treasuryFee;
        totalBuyFees = marketingBuyFee + rewardsBuyFee + burnBuyFee + treasuryBuyFee;
        require(totalBuyFees <= 10, "Must keep fees at 10% or less");
    }
    
    function updateSellFees(uint256 _marketingFee, uint256 _rewardsFee, uint256 _burnFee, uint256 _treasuryFee) external onlyOwner {
        marketingSellFee = _marketingFee;
        rewardsSellFee = _rewardsFee;
        burnSellFee = _burnFee;
        treasurySellFee = _treasuryFee;
        totalSellFees = marketingSellFee + rewardsSellFee + burnSellFee + treasurySellFee;
        require(totalBuyFees <= 10, "Must keep fees at 10% or less");
    }
    ...
    function swapBack() private {
        ...
        uint256 totalTokensToSwap = tokensForBurn + tokensForMarketing + tokensForRewards + tokensForTreasury;
        ...
        if (balanceOf(address(this)) > 0){
            ...
            super._transfer(address(this), 0x000000000000000000000000000000000000dEaD, _amount);
        }
