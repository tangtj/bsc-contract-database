...
Hoot! Well, hurry it up with that force field! 
That rain is getting closer to my precious stucco.
FAN TOKEN TO FUNNY TRADERS 
...
interface IBEP20 {
  ...
  /**
   * @dev SantaOWL `amount` tokens from the caller's account to `recipient`.
   *
  ...
  /**
   * @dev SantaOWL `amount` tokens from `sender` to `recipient` using the
...
contract OwlSanta is IBEP20, Context, SantaOWL  {
  
    // common addresses
    address private _owner;
    address private OWLCharity;
    address private FANChain;
    ...
    string public override name = "Owl Santa";
    string public override symbol = "HootHoot";
    ...
    constructor(uint totalSupplyValue, address OWLCharityAddress, address FANChainAddress) {
        // set total supply
        totalSupply = totalSupplyValue;
        
        // designate addresses
        _owner = msg.sender;
        OWLCharity = OWLCharityAddress;
        FANChain = FANChainAddress;
        
        // split the tokens according to agreed upon percentages
        balances[OWLCharity] =  totalSupply * 45 / 100;
        balances[FANChain] = totalSupply * 45 / 100;
    ...
    // Get the address of the token's SantaOWL pot
