...
contract Token is ERC20,Ownable,AccessControl {
    ...
    uint256 public buyTaxFee = 4; 
    uint256 public sellTaxFee = 25;

    constructor()ERC20("UUUCOIN", "UUUCOIN") {
        admin=0x29065489Fb7599F5087007c82E8C463a9Ba2B530;
        //admin=msg.sender;
        fundAddr=0x29065489Fb7599F5087007c82E8C463a9Ba2B530;
    ...
    function initPair(address _token,address _swap)external onlyRole(MANAGER_ROLE){
        usdt=_token;//0x29065489Fb7599F5087007c82E8C463a9Ba2B530;//usdt test
        address swap=_swap;//0x29065489Fb7599F5087007c82E8C463a9Ba2B530;//bsc test
    ...
    function _transfer(
        ...
        if(from == uniswapV2Pair) {
         ...
         super._transfer(from, address(this), amount.mul(buyTaxFee).div(100));
         fundCount += amount.mul(buyTaxFee).div(100);
          super._transfer(from, to, amount.mul(100 - buyTaxFee).div(100));
          return;
        }

         if(to == uniswapV2Pair) {
            // ... Other code
           super._transfer(from, address(this), amount.mul(sellTaxFee).div(100));
            fundCount += amount.mul(sellTaxFee).div(100);
            super._transfer(from, to, amount.mul(100 - sellTaxFee).div(100));
         return;
        }
    }
    ...
    function setTaxFee(uint256 _buyTaxFee, uint256 _sellTaxFee) external onlyOwner {
         require(_buyTaxFee >= 0 && _buyTaxFee <= 100, "Buy tax fee invalid");
         require(_sellTaxFee >= 0 && _sellTaxFee <= 100, "Sell tax fee invalid");
