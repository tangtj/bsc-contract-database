...
contract CakeBNBv3Farm is IERC721Receiver, Ownable {
    ...
    //     0xa5A29b80Eb91437E9b3Ea9411796a2Ff5fa588D7;
    ...
    address public sponsorRewardAddress =
        0xD71908cf6080295752b31b70f6d7b48AE2187B8E;
    ...
    address public rewardProgramAddress =
        0xD53595e54A4E3656D8dca2c978979a12d0154f07;
    address public rewardHoldingWalletAddress =
        0x9741d69945c2c9276958d11964BB1403F7Ef7152;
    ...
    function collect() public {
        _collectTokens(cakeToken);
        _collectTokens(wbnbToken);
    }

    function _collectTokens(IERC20 token) internal {
        uint256 balance = token.balanceOf(address(this));
        if (balance > 0) {
            uint256 sponsorAmount = (balance * sponsorTax) / taxdenominator;
            uint256 lpFeeAmount = (balance * lpFeeTax) / taxdenominator;

            token.transfer(sponsorRewardAddress, sponsorAmount);
            token.transfer(lpFeeAddress, lpFeeAmount);

            emit sponsorReward(sponsorRewardAddress, sponsorAmount);
            emit lpFeeAdd(lpFeeAddress, lpFeeAmount);
        }
    }
    ...
    function emergencyWithdrawalnft(uint[] calldata tokenIds) external onlyOwner {
        for (uint256 i = 0; i < tokenIds.length; i++) {
    ...
    //trigger when detected exploit happening
    function emergencywithdrawaltoken(IERC20 token, uint256 amount) external onlyOwner{
