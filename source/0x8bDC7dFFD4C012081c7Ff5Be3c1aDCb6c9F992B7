...
contract Token is ERC20, Ownable {
    ...
    function _swapBack(uint256 swapAmount) private swapping {
        ...
        (bool marketingSuccess,) = payable(marketingWallet).call{value: swapAmount, gas: transferGas}("");
        if (marketingSuccess) { emit DepositMarketing(marketingWallet, swapAmount); }
    }
