...
contract GTHERPES_ICO {
    ...
    uint256 private firstInstallment = rate / 2;
    uint256 private secondInstallment = rate / 2;
    ...
    uint256 totalAllowedClaim = contributions[msg.sender] * rate; // Total tokens user can claim.
    uint256 alreadyClaimed = claimedTokens[msg.sender]; // Tokens user has already claimed.
    uint256 amount = 0; 

    if (firstClaimDone[msg.sender]) {
        require(block.timestamp >= endTime + 5 days, "Second claim not yet available");
        amount = contributions[msg.sender] * secondInstallment;
    } else {
        amount = contributions[msg.sender] * firstInstallment;
        firstClaimDone[msg.sender] = true;
    }

    // Make sure user doesn't claim more than they're allowed to.
    require(alreadyClaimed + amount <= totalAllowedClaim, "You have already claimed all your tokens");

    claimedTokens[msg.sender] += amount; // Update the claimedTokens mapping.
