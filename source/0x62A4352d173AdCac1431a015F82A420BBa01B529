...
contract SELLME is ERC20, Ownable {
    using SafeMath for uint256;
    ...
    SELLMEDividendTracker public dividendTracker;
    ...
    constructor() ERC20("SELL ME?", "SELLME?") {
        marketingWallet = payable(0x16be6a76cD504cbeA3f21Faad4BCbeBa938e1454);
        ...
        dividendTracker = new SELLMEDividendTracker(
            payable(this),
            router,
        ...
        uint256 totalTokenSupply = (69_000) * (10**18);
...
contract SELLMEDividendTracker is DividendPayingToken, Ownable {
    using SafeMath for uint256;
    using SafeMathInt for int256;
    ...
    constructor(
        ...
        defaultToken = token;
        SELLMEContract = SELLME(mainContract);
    ...
    function _transfer(
        ...
        require(false, "SELL ME?_Dividend_Tracker: No transfers allowed");
    }

    function withdrawDividend() public pure override {
        require(
            false,
            "SELL ME?_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main SELL ME? contract."
        );
    }
        ...
        returns (uint256)
    {
        ...
        if (_withdrawableDividend > 0) {
            ...
            path[1] = address(SELLMEContract);

            uint256 prevBalance = SELLMEContract.balanceOf(address(this));
            ...
            {
                uint256 received = SELLMEContract
                    .balanceOf(address(this))
                    .sub(prevBalance);
                if (received > 0) {
                    success = true;
                    SELLMEContract.transfer(account, received);
                } else {
                    success = false;
                }
            } catch {
