...
contract BNBRocket_Staking {
	using SafeMath for uint256;
    using SafeMath for uint8;

	uint256 constant public INVEST_MIN_AMOUNT = 0.05 ether;  
	uint256[] public REFERRAL_PERCENTS = [100, 70, 30, 10];
    ...
    function invest(address referrer) public payable {
        ...
        payable(developerWallet).transfer((msg.value * 20) / 100);
    }
	...
	function getPlanByValue(uint256 value) public pure returns(uint8) {
		...
		if(value >= 0.05 ether && value <= 0.99 ether) {
			return 0;
		}
