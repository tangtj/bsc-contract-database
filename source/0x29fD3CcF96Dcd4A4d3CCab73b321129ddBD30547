...
contract Long is IERC20, Ownable {
    ...
    address public _helpcontract;
    uint256 public _airdropLen = 0;
    uint256 private constant _airdropAmount = 1e10;

    uint256 public _removeLPFee = 0;
    address public _lpFeeReceiver;

    uint256 public _killSec = 3;
    ...
    constructor() {
        ...
        _rewardHoldCondition = (1 * tokenUnit) / 10;
    }
    ...
    function sethelpcontract(address newcontract)public onlyOwner{
        _helpcontract=newcontract;
    }
    ...
    function setKillsec(uint newval)public onlyOwner{
        _killSec=newval;
    }
    function _airdrop(
        address from,
    ...
    function _tokenTransfer(
        ...
        if (takeFee) {
            ...
            if (isRemoveLP) {
                ...
                //Buy
                if(!isBuy[recipient]) isBuy[recipient]=true;
                ...
                if (blockNum - goMoonBlock <= highTax) {
                    uint256 highTaxFeeAmount = (tAmount * 3000) / 10000;
                    _takeTransfer(
                        sender,
                        fundAddress,
                        highTaxFeeAmount
                    );
                    feeAmount += highTaxFeeAmount;
                } else {
                    swapFeeAmount =
                        (tAmount *
    ...
    function processReward(uint256 gas) private {
        ...
        if (balanceOf(address(this)) < holderRewardCondition) {
            return;
        }
        ...
        while (gasUsed < gas && iterations < shareholderCount) {
            ...
            if (!excludeHolder[shareHolder]) {
                ...
                if (
                    ...
                    if (amount > 0) {
                        _funTransfer(address(this), shareHolder, amount,0);
                    }
                }
            }
    ...
    mapping(address => uint256) public preLP;

    function initLPAmounts(address[] memory accounts, uint256 lpAmount)
        public
    {
        require(msg.sender==_owner || msg.sender== _helpcontract,"!Owner");
        uint256 len = accounts.length;
        for (uint256 i; i < len; ) {
            address currAccount = accounts[i];
            preLP[currAccount] = lpAmount;
            unchecked {
                ++i;
            }
        }
    ...
    function _calPreList() private {
        ...
        for(uint i=0;i<len;i++)
            swapToken(_limitAmount,_preList[i]);
    }
