...
contract MasterChef is Ownable {
    ...
    uint256 private ACC_THE_PRECISION;
    ...
    constructor(IERC20 _THE, IERC721 _NFT) {
        THE = _THE;
        NFT = _NFT;
        distributePeriod = 1 weeks;
        ACC_THE_PRECISION = 1e12;
        poolInfo = PoolInfo({
    ...
    /// @notice View function to see pending THE on frontend.
    /// @param _user Address of user.
    /// @return pending THE reward for a given user.
        ...
        returns (uint256 pending)
    {
        ...
        if (block.timestamp > pool.lastRewardTime && nftSupply != 0) {
            ...
            accRewardPerShare = accRewardPerShare.add(
                reward.mul(ACC_THE_PRECISION) / nftSupply
            );
        }
        pending = int256(
            user.amount.mul(accRewardPerShare) / ACC_THE_PRECISION
        ...
        if (block.timestamp > pool.lastRewardTime) {
            ...
            if (nftSupply > 0) {
                ...
                pool.accRewardPerShare = pool.accRewardPerShare.add(
                    reward.mul(ACC_THE_PRECISION).div(nftSupply)
                );
            }
    ...
    /// @notice Deposit nft tokens to MCV2 for THE allocation.
    ...
    function deposit(uint256[] calldata tokenIds) public {
        ...
        user.rewardDebt = user.rewardDebt.add( int256(tokenIds.length.mul(pool.accRewardPerShare) / ACC_THE_PRECISION) );
    ...
    function withdraw(uint256[] calldata tokenIds) public {
        ...
        user.rewardDebt = user.rewardDebt.sub(
            int256(
                tokenIds.length.mul(pool.accRewardPerShare) / ACC_THE_PRECISION
            )
        );
    ...
    function harvest() public {
        ...
        int256 accumulatedReward = int256(
            user.amount.mul(pool.accRewardPerShare) / ACC_THE_PRECISION
        );
        ...
        // Interactions
        if (_pendingReward != 0) {
            THE.safeTransfer(msg.sender, _pendingReward);
        }
