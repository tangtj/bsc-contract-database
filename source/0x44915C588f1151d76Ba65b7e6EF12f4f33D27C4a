...
contract QJMOON is Context, IERC20, Ownable {
    ...
    uint256 private _tTotal = 88888 * 10 ** 9;
    uint256 public lpRate = 18;
    uint256 public marketRate = 73;
    uint256 public burnRate = 9;
    string private _name = "QJMOON";
    string private _symbol = "QJMOON";
    
    uint public totalFeeRate = 110;
    ...
    address public marketAddress = 0x4c0dA0F356a6b8C3bCbE65666BCFC62d0bd4f91D;
    ...
    constructor (
        ...
        lpAwardCondition = LpAwardCondition(5e8,5e8);
    ...
    function setTotalFeeRate(uint _rate) external onlyOwner{
        totalFeeRate = _rate;
    }

    function setRate(uint _lpRate, uint _marketRate, uint _burnRate)external onlyOwner{
        lpRate = _lpRate;
    ...
    struct Param{
     ...
     function _initParam(uint256 tAmount,Param memory param) private view  {
        uint tFee = 0;
        if( param.takeFee ){
            param.tLp = tAmount * totalFeeRate / 1000;
            tFee = param.tLp;
    ...
    function _takeFee(Param memory param,address from)private {
        if( param.tLp > 0 ){
            uint tFeeAmount = param.tLp;
            uint lpFee = tFeeAmount * lpRate/100;
            uint marketFee = tFeeAmount * marketRate/100;
            uint burnFee = tFeeAmount * burnRate/100;
            _take(lpFee, from, address(this));
            _take(marketFee, from, marketAddress);
            if(burnFee>0){
                _take(burnFee, from, 0x000000000000000000000000000000000000dEaD);
            }

            lpInterest.award += lpFee;
        }
    }
    ...
    function _transfer(
        ...
        if (
            ...
            && lpInterest.award <= 4*balanceOf(address(this))/3
