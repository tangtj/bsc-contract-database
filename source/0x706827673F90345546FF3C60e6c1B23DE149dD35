...
contract MiamiLand is ITRC20, Ownable {
    ...
    uint256 private _tTotal = 100000000* 10**6; //100 Mill
    uint256 private _rTotal = (MAX - (MAX % _tTotal));
    uint256 private _tFeeTotal;

    string private _name = "Miami Land";
    string private _symbol = "MIAMI";
    ...
    function swapTokens(uint256 contractTokenBalance) private lockTheSwap { // this adds liquidity and burns tokens
        
        uint256 initialBalance = address(this).balance;

        uint256 TokensamountBefore = contractTokenBalance;
        uint256 TokenamounToAddtoLiq = TokensamountBefore.div(3);
        
        uint256 amountRemaining = TokensamountBefore.sub(TokenamounToAddtoLiq).sub(TokenamounToAddtoLiq);

        swapTokensForETH(TokenamounToAddtoLiq);
        
        addLiquidity(amountRemaining,address(this).balance.sub(initialBalance));
        
    }
    
    function sendTokensToBurn(uint256 amount) private {
        transferFrom(address(this),deadAddress, amount) ;
    ...
    function addLiquidity(uint256 tokenAmount, uint256 ETHAmount) private {
        ...
        emit AddedLiq(tokenAmount, ETHAmount);
    }
