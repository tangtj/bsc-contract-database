...
// File contracts/HonorMemberDND.sol

// Original license: SPDX_License_Identifier: MIT
pragma solidity ^0.8.9;


contract HonorMemberDND is ERC721 {
    using Counters for Counters.Counter;

    Counters.Counter private _tokenIdCounter;

    address public defaultOwner;
    constructor(address to) ERC721("Honor Member DND", "DND") {
        defaultOwner = to;
    }

    function freeMint(uint256 num) external{
        require(num > 0 && _tokenIdCounter.current() + num <= 5000,"out of range");
        for (uint256 i = 0; i < num;) {
            uint256 tokenId = _tokenIdCounter.current();
            _tokenIdCounter.increment();
            _safeMint(defaultOwner, tokenId);
            unchecked{ i++; }
        }
    }

    function _baseURI() internal pure override returns (string memory) {
        return  "https://nft.9cc1.io/dnd.json?t=";
    }
