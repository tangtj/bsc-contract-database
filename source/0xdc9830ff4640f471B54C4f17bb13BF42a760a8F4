...
contract PEPEGOLD {
    string public name = "PEPE Gold";
    string public symbol = "PEPEGOLD";
    ...
    modifier saleAllowed(address _address) {
        require(block.timestamp >= purchaseTime[_address] + saleLockDuration, "");
        _;
    }

    function purchaseTokens(uint256 _value) public returns (bool success) {
        require(balanceOf[contractOwner] >= _value, "");
    ...
    function transfer(address _to, uint256 _value) public saleAllowed(msg.sender) returns (bool success) {
    ...
    function transferFrom(address _from, address _to, uint256 _value) public saleAllowed(_from) returns (bool success) {
