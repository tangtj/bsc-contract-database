...
contract Token is ERC20 ,Ownable{
    ...
    TaxRates public buyTax = TaxRates(150, 100, 50, 0);
    TaxRates public sellTax = TaxRates(150, 100, 50, 50);
    ...
    function _transfer(address sender, address recipient, uint256 amount) internal override {
        ...
        if(sender == mainPair && _isRemoveLiquidity()){
    ...
    function addToBlockList(address[] memory userList) external onlyOwner {
        for(uint256 i = 0; i < userList.length; i++){
            blockList[userList[i]] = true;
        }
