...
contract RPAXSuper is ReentrancyGuard, Ownable {
    ...
    uint256 private constant BUY_SUPER_AMOUNT_RPAX = 3 * 10**7;
    uint256 private constant FREE_USDT = 10**20;
    ...
    function setSuperNode(address _superNode) external nonReentrant onlyOwner {
        require(
    ...
    function releaseSuperReward() external nonReentrant {
        ...
        uint256 rpaxAmount = getPrice(usdtAmount);
        address user;
    ...
    function setFreeRecommendandFree(address recommend, address recommended)
        external
    {
    ...
    function setSubReward(address _user, uint256 _rewardAmount)
        external
        onlyOwner
    {
