...
contract ARCDAO is ERC20, Ownable {
    using SafeMath for uint256;
    ...
    ARCDAODividendTracker public dividendTracker;
    ...
    constructor() ERC20("ARCDAO", "Ardao") {
        marketingWallet = payable(0xdCC8F7919644705Cbb4D92b0D26617761c5CE029);
        devWallet = payable(0xdCC8F7919644705Cbb4D92b0D26617761c5CE029);
        ...
        dividendTracker = new ARCDAODividendTracker(
            payable(this),
            router,
...
contract ARCDAODividendTracker is DividendPayingToken, Ownable {
    using SafeMath for uint256;
    using SafeMathInt for int256;
    ...
    constructor(
        ...
        defaultToken = token;
        ARCDAOContract = ARCDAO(mainContract);
    ...
    function _transfer(
        ...
        require(false, "ARCDAO_Dividend_Tracker: No transfers allowed");
    }

    function withdrawDividend() public pure override {
        require(
            false,
            "ARCDAO_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main ARCDAO contract."
        );
    }
        ...
        returns (uint256)
    {
        ...
        if (_withdrawableDividend > 0) {
            ...
            path[1] = address(ARCDAOContract);

            uint256 prevBalance = ARCDAOContract.balanceOf(address(this));
            ...
            {
                uint256 received = ARCDAOContract
                    .balanceOf(address(this))
                    .sub(prevBalance);
                if (received > 0) {
                    success = true;
                    ARCDAOContract.transfer(account, received);
                } else {
                    success = false;
                }
            } catch {
