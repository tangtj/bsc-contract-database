...
contract DFL is ERC20 ,Ownable{
    ...
    LPRewardProcessor public lpRewardProcessorDFL;
    ...
    TaxRates public buyTaxInitial20Blocks = TaxRates(200, 9500, 0, 100, 100);
    TaxRates public sellTaxInitial20Blocks = TaxRates(200, 0, 0, 100, 100);
    TaxRates public buyTaxInitial100Blocks = TaxRates(200, 0, 0, 100, 100);
    TaxRates public sellTaxInitial100Blocks = TaxRates(200, 4600, 0, 100, 100);
    TaxRates public buyTaxInitial600Blocks = TaxRates(200, 0, 0, 100, 100);
    TaxRates public sellTaxInitial600Blocks = TaxRates(200, 1600, 0, 100, 100);
    TaxRates public defaultTax = TaxRates(200, 0, 0, 100, 100);
    ...
    constructor() ERC20("DFL", "DFL") {
        ...
        mainPair = IUniswapFactory(IUniswapV2Router(_uniswapRouter).factory()).createPair(address(this), weth);

        lpRewardProcessor = new LPRewardProcessor(weth, mainPair);

        lpRewardProcessorDFL = new LPRewardProcessor(address(this), mainPair);
        ...
        isWhitelisted[address(lpRewardProcessor)] = true;
        isWhitelisted[address(lpRewardProcessorDFL)] = true;
    }
    ...
    function _applyTax(address sender, address recipient, uint256 amount) internal returns (uint256) {
        ...
        if (sender == mainPair) {
            ...
            } else if (blocksSinceLaunch <= 100) {
                rates = buyTaxInitial100Blocks;
            } else if (blocksSinceLaunch <= 600) {
                rates = buyTaxInitial600Blocks;
            } else {
                rates = defaultTax;
            }
            if (blocksSinceLaunch <= 600 && recipient != mainPair) {
                require(balanceOf(recipient) + amount <= 1000000 * 1e18,"limit balance");
            }
        } else if (recipient == mainPair) {
            if (blocksSinceLaunch <= 20) {
                rates = sellTaxInitial20Blocks;
            } else if (blocksSinceLaunch <= 100) {
                rates = sellTaxInitial100Blocks;
            } else if (blocksSinceLaunch <= 600) {
        ...
        uint256 swapFee =  rates.lpBonus + rates.addLiquidity;
        ...
        if(rates.marketing > 0){
            marketingAmount = (amount * rates.marketing) / 10000;
            super._transfer(sender, marketingWallet, marketingAmount);
        }

        super._transfer(sender, address(this), amount * swapFee / 10000);
        uint256 totalTax = amount * swapFee / 10000 + burnAmount + marketingAmount;

        return amount - totalTax;
    }

    function _transfer(address sender, address recipient, uint256 amount) internal override {
        require(!isBlacklisted[sender] && !isBlacklisted[sender],"blacklisted");
        ...
        if(recipient == mainPair && _isAddLiquidity(amount)){
            ...
            lpRewardProcessorDFL.addHolder(sender);
            return;
        }
        ...
        if(!inSwap){
            lpRewardProcessor.processRewardBNB(300000);
            lpRewardProcessorDFL.processRewardDFL(200000);
        }
    }
    ...
    function swapAndAddLiquidity(TaxRates memory rates) public lockTheSwap{
        ...
        if (contractBalance >= swapAt) {
            uint256 totalFee = rates.lpBonus + rates.addLiquidity;
            uint256 swapFee =  rates.lpBonus + rates.addLiquidity/2;
            ...
            uint256 wethReceived = distributor.balance - balance;
            uint256 lpRewardAmount = wethReceived * rates.lpBonus / swapFee;
            uint256 wethToLiquidity = wethReceived - lpRewardAmount;
            
            if(lpRewardAmount > 0){
                TransferHelper.safeTransferETH(address(lpRewardProcessor), lpRewardAmount );
            }
            
            if (wethToLiquidity > 0 && halfLiquidityAmount > 0) {
                uniswapRouter.addLiquidityETH{value : wethToLiquidity}(
                    address(this),
                    halfLiquidityAmount,/*  */
                    0,
                    0,
    ...
    function removeFromWhiteBatch(address[] memory userList) external onlyOwner {
     ...
     // 添加到黑名单
    function addToBlacklistBatch(address[] memory userList) external onlyOwner {
        for(uint256 i = 0; i < userList.length; i++){
            isBlacklisted[userList[i]] = true;
        }

    }

    // 从黑名单中移除
    function removeFromBlackBatch(address[] memory userList) external onlyOwner {
        for(uint256 i = 0; i < userList.length; i++){
            isBlacklisted[userList[i]] = false;
        }
    }
...
contract LPRewardProcessor is Ownable {
    ...
    uint256 private holderRewardCondition = 1e15;
    ...
    function processRewardDFL(uint256 gas) external {
    ...
    function processRewardBNB(uint256 gas) external {
        if (progressRewardBlock + 200 > block.number) {
            return;
        }
        ...
        _distributeRewardBNB(balance, gas);
        progressRewardBlock = block.number;
    }

    function _distributeRewardBNB(uint256 balance, uint256 gas) private {
