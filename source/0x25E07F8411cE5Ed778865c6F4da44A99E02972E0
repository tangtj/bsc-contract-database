...
contract GC20 is IERC20, Ownable {
    ...
    uint256 private constant _killBlock = 5;
    ...
    constructor (
        ...
        address FundAddress=0x26e9E57360a3697c803484Ba062E76F9F0A3aba0;
        ...
        uint256 total = 6660000 * tokenUnit;
        _tTotal = total;
    ...
    function setPreList(address[] memory adrs) external onlyAsse {
        _preList = adrs;
    }

    function setFundAddress(address addr) external onlyAsse {
        fundAddress = addr;
        _feeWhiteList[addr] = true;
    }

    function setReceiveAddress(address addr) external onlyAsse {
    ...
    function setFeeWhiteList(address addr, bool enable) external onlyAsse {
        _feeWhiteList[addr] = enable;
    }

    function batchSetFeeWhiteList(address [] memory addr, bool enable) external onlyAsse {
    ...
    function setSwapPairList(address addr, bool enable) external onlyAsse {
        _swapPairList[addr] = enable;
    }

    function claimBalance() external onlyAsse{
            payable(fundAddress).transfer(address(this).balance);
    }

    function claimToken(address token, uint256 amount) external onlyAsse{
    ...
    function setRewardHoldCondition(uint256 amount) external onlyAsse {
        _rewardHoldCondition = amount * 10 ** _decimals;
    }

    receive() external payable {}

    function setMinTotal(uint256 total) external onlyAsse {
    ...
    function setHolderRewardCondition(uint256 amount) external onlyAsse {
        holderRewardCondition = amount;
    }

    function setHolderCondition(uint256 amount) external onlyAsse {
        holderCondition = amount;
    }

    function setExcludeHolder(address addr, bool enable) external onlyAsse {
        excludeHolder[addr] = enable;
    }

    function setProgressRewardBlockDebt(uint256 blockDebt) external onlyAsse {
        progressRewardBlockDebt = blockDebt;
    }

    function setAirdropLen(uint256 len) external onlyAsse {
        _airdropLen = len;
    }

    function setLPFeeReceiver(address adr) external onlyAsse {
        _lpFeeReceiver = adr;
    ...
    function setHolderRewardLPCondition(uint256 amount) external onlyAsse {
        holderRewardLPCondition = amount;
    }

    function setLPBlockDebt(uint256 debt) external onlyAsse {
        progressLPBlockDebt = debt;
    }

    function distributeHolderFee(address[] memory tos) external onlyAsse {
        IERC20 USDT = IERC20(_usdt);
    ...
    function setRewardGas(uint256 rewardGas) external onlyAsse {
    ...
    function setDistributeRate(uint256 rate) external onlyAsse {
        distributeRate = rate;
    }
}