...
contract FAYICO is ReentrancyGuard, Ownable {
    uint256 public presaleId;
    uint256 public USDT_MULTIPLIER;
    uint256 public BUSD_MULTIPLIER;
    ...
    struct ClaimData {
        ...
        uint256 nextClaimAt;
        uint256 totalAmount;
        uint256 claimAmountShare;
        uint256 claimedAmount;
    }

    IERC20Metadata public USDTInterface;
    IERC20Metadata public BUSDInterface;
    ...
    constructor() {
        aggregatorInterface = Aggregator(address(0x0567F2323251f0Aab15c8dFb1967E4e8A7D42aeE));
        SaleToken = address(0xA11b5A5996a54930de4aF6e3e9995770F18C26dD);
        MinTokenTobuy = 1;
        USDTInterface = IERC20Metadata(address(0x55d398326f99059fF775485246999027B3197955));
        BUSDInterface = IERC20Metadata(address(0xe9e7CEA3DedcA5984780Bafc599bD69ADd087D56));
        BNB_MULTIPLIER = (10**18);
        USDT_MULTIPLIER =(10**18);
        BUSD_MULTIPLIER =(10**18);
    ...
    /**
     * @dev To update the BUSD Token address
     * @param _newAddress Sale token address
     */
    function changeBUSDToken(address _newAddress) external onlyOwner {
        require(_newAddress != address(0), "Zero token address");
        BUSDInterface = IERC20Metadata(_newAddress);
    }
        ...
        returns (bool)
    {
        ...
        if (userClaimData[_msgSender()][presaleId].totalAmount > 0) {
            ...
            userClaimData[_msgSender()][presaleId] = ClaimData(0, 0, tokens, 0, 0);
        }
     ...
     * @param usdAmount busd amount to buy tokens
     */
    function buyWithBUSD(uint256 usdAmount)
        external
        checkPresaleId(presaleId)
        checkSaleState(presaleId, usdtToTokens(presaleId, usdAmount))
        returns (bool)
    ...
    function buyWithBNB()
        ...
        payable
        checkPresaleId(presaleId)
        checkSaleState(presaleId, bnbToTokens(presaleId, msg.value))
        nonReentrant
        returns (bool)
    {
        uint256 usdAmount = (msg.value * getLatestPrice() * USDT_MULTIPLIER) / (BNB_MULTIPLIER * BNB_MULTIPLIER);
        require(presale[presaleId].amountRaised + usdAmount <= presale[presaleId].UsdtHardcap,
        "Amount should be less than leftHardcap");
     ...
     */
    function bnbBuyHelper(uint256 _id, uint256 amount)
        external
        view
        checkPresaleId(_id)
        returns (uint256 bnbAmount)
    {
        ...
        bnbAmount = (usdPrice *  BNB_MULTIPLIER) / (getLatestPrice() * 10**IERC20Metadata(SaleToken).decimals());
    }
    ...
    /**
     * @dev Helper funtion to get tokens for bnb amount
     * @param _id Presale id
     * @param amount No of bnb
     */
    ...
    {
        uint256 usdAmount = amount * getLatestPrice() * USDT_MULTIPLIER / (BNB_MULTIPLIER * BNB_MULTIPLIER);
        ...
        returns (bool)
    {
        ...
        if(userClaimData[msg.sender][_id].nextClaimAt > 0){
            amount = userClaimData[msg.sender][_id].claimAmountShare;
        }else{
            uint256 firstClaimAmount = userClaimData[msg.sender][_id].totalAmount * 10 / 100;
            userClaimData[msg.sender][_id].claimAmountShare = (userClaimData[msg.sender][_id].totalAmount - firstClaimAmount) / 12;
            amount = firstClaimAmount;
        }
        
        require(amount > 0, "Zero claim amount");
        require(block.timestamp > userClaimData[msg.sender][_id].nextClaimAt, "wait for next claim amount");
        require(
            SaleToken != address(0),
        ...
        userClaimData[msg.sender][_id].lastClaimAt = block.timestamp;
        userClaimData[msg.sender][_id].nextClaimAt = block.timestamp + 30 days;
