...
library BuniCornLibrary {
    ...
    {
        require(tokenA != tokenB, "BUNILIB: IDENTICAL_ADDRESSES");
        (token0, token1) = tokenA < tokenB ? (tokenA, tokenB) : (tokenB, tokenA);
        require(address(token0) != address(0), "BUNILIB: ZERO_ADDRESS");
    }
        ...
        )
    {
        ...
        uint256 vReserve0;
        uint256 vReserve1;
        (reserve0, reserve1, vReserve0, vReserve1, feeInPrecision) = IBuniCornPool(pool).getTradeInfo();
    ...
    function getReserves(
        ...
        (IERC20 token0, ) = sortTokens(tokenA, tokenB);
        (uint256 reserve0, uint256 reserve1) = IBuniCornPool(pool).getReserves();
    ...
    function quote(
        ...
        require(amountA > 0, "BUNILIB: INSUFFICIENT_AMOUNT");
        require(reserveA > 0 && reserveB > 0, "BUNILIB: INSUFFICIENT_LIQUIDITY");
    ...
    function getAmountOut(
        ...
        require(amountIn > 0, "BUNILIB: INSUFFICIENT_INPUT_AMOUNT");
        require(reserveIn > 0 && reserveOut > 0, "BUNILIB: INSUFFICIENT_LIQUIDITY");
        ...
        require(reserveOut > amountOut, "BUNILIB: INSUFFICIENT_LIQUIDITY");
    }
    ...
    function getAmountIn(
        ...
        require(amountOut > 0, "BUNILIB: INSUFFICIENT_OUTPUT_AMOUNT");
        require(reserveIn > 0 && reserveOut > amountOut, "BUNILIB: INSUFFICIENT_LIQUIDITY");
