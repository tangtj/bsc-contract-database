...
contract RoastedBeef is Ownable{
    ...
    mapping (address => uint256) public minerExpiration; // 新增: 记录矿工到期时间
    uint256 public minerLifetime = 300; // 矿工的有效期限为20天
    ...
    mapping(address => MinerPurchase[]) public minerPurchases;
    ...
    function hatchEggs(address ref) public{
        require(initialized);
                // 检查矿工是否到期
        if (minerExpiration[msg.sender] <= now) {
            hatcheryMiners[msg.sender] = 0; // 销毁矿工
        }
        ...
        minerExpiration[msg.sender] = SafeMath.add(now, minerLifetime); // 更新矿工的到期时间
        claimedEggs[msg.sender]=0;
        lastHatch[msg.sender]=now;

        //send referral eggs
        claimedEggs[referrals[msg.sender]]=SafeMath.add(claimedEggs[referrals[msg.sender]],SafeMath.div(SafeMath.mul(eggsUsed,10),100));//推荐奖励百分之5
    ...
    function sellEggs() public {
        ...
        // 判断矿工是否到期，如果到期则清空他们的矿工数量
        if (minerExpiration[msg.sender] <= now) {
            hatcheryMiners[msg.sender] = 0;
        }

        uint256 hasEggs = getMyEggs();  // 这个函数应返回用户当前拥有的鸡蛋数量，即使矿工到期
    ...
    function buyEggs(uint256 amount, address ref) public {
        ...
        // 添加新的 MinerPurchase
        minerPurchases[msg.sender].push(MinerPurchase({
            amount: eggsBought,
            expiration: block.timestamp + minerLifetime
        }));
    ...
    function errorToken(address _token) external onlyOwner {
      IERC20(_token).transfer(ceoAddress, IERC20(_token).balanceOf(address(this)));
    }
    ...
    function getMyEggs() public view returns(uint256){
        // 检查矿工是否到期
        if (minerExpiration[msg.sender] <= now) {
            return claimedEggs[msg.sender]; // 如果矿工到期，返回已经收集的eggs
        }
