...
contract Sale_Contract {
    ...
    uint256 public referralBonusRateIncrement = 5; // 5% increment for each subsequent referral after 7
    ...
    function _calculateReferralBonus(uint256 _weiAmount) private view returns (uint256) {
        if (referralCount[msg.sender] >= referralCountThreshold) {
            ...
            uint256 bonusIncrement = _weiAmount.mul(referralBonusRateIncrement).div(100).mul(referralCount[msg.sender].sub(referralCountThreshold));
            uint256 totalBonus = baseReferralBonus.add(bonusIncrement);

            // Limit the bonus to the maxReferralBonusRate
            if (totalBonus > _weiAmount.mul(maxReferralBonusRate).div(10000)) {
                return _weiAmount.mul(maxReferralBonusRate).div(10000);
            }
