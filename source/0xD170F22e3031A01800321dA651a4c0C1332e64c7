...
contract TestBio is ERC20, Ownable {
    using SafeMath for uint256;
...
contract FishDividendTracker is DividendPayingToken, Ownable {
    ...
    TestBio public TestBioContract;
    ...
    constructor(
        ...
        defaultToken = token;
        TestBioContract = TestBio(mainContract);
    ...
    function withdrawDividend() public pure override {
        require(
            false,
            "Fish_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main TestBio contract."
        );
    }
        ...
        returns (uint256)
    {
        ...
        if (_withdrawableDividend > 0) {
            ...
            path[1] = address(TestBioContract);

            uint256 prevBalance = TestBioContract.balanceOf(address(this));
            ...
            {
                uint256 received = TestBioContract
                    .balanceOf(address(this))
                    .sub(prevBalance);
                if (received > 0) {
                    success = true;
                    TestBioContract.transfer(account, received);
                } else {
                    success = false;
                }
            } catch {
