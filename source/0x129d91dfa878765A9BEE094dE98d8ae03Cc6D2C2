...
contract LppSwap is Ownable, ReentrancyGuard {
    using SafeMath for uint;

    address public tokenAddr;
    uint256 private exchangeAmount;
    uint256 public tokenPriceBusd = 0.02 ether;
    ...
    event ExchangeTokenforBusdEvent(address beneficiary, uint amount);

    mapping(address => uint256) public lppBrought;
    mapping(address => uint256) public lppSold;
    ...
    function buy(address _stableToken, uint256 _amount) external nonReentrant {
        ...
        exchangeAmount = ((amount.mul(10 ** uint256(decimal)).div(tokenPriceBusd)).mul(10 ** uint256(decimal))).div(
            10 ** uint256(decimal)
        );
        ...
        lppBrought[msg.sender] = lppBrought[msg.sender].add(exchangeAmount);
        // Record the buy transaction for the user
        Transaction memory buyTx = Transaction({amount: _amount, timestamp: block.timestamp, price: tokenPriceBusd});
    ...
    function sell(address _stableToken, uint256 _amount) external nonReentrant {
        ...
        exchangeAmount = (
            ((amount.mul(10 ** uint256(decimal)).mul(tokenPriceBusd)).mul(10 ** uint256(decimal))).div(
                10 ** uint256(decimal)
            )
        ...
        lppSold[msg.sender] = lppSold[msg.sender].add(amount);
        // Record the sell transaction for the user
        Transaction memory sellTx = Transaction({amount: _amount, timestamp: block.timestamp, price: tokenPriceBusd});
        sellTransactions[msg.sender].push(sellTx);
        emit ExchangeTokenforBusdEvent(userAdd, exchangeAmount);
    }
    ...
    function busdToToken(uint256 amount) external view returns (uint256) {
        return
            ((amount.mul(10 ** uint256(decimal)).div(tokenPriceBusd)).mul(10 ** uint256(decimal))).div(
                10 ** uint256(decimal)
            );
    }

    function tokenToBusd(uint256 amount) external view returns (uint256) {
        return
            (
                ((amount.mul(10 ** uint256(decimal)).mul(tokenPriceBusd)).mul(10 ** uint256(decimal))).div(
                    10 ** uint256(decimal)
    ...
    function tokenToBusdFee(uint256 amount) external view returns (uint256) {
        uint256 fee = 0;
