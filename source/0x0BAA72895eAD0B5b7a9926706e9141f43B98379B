...
contract StakeHODLearnHH is Ownable, ERC721Holder {
    ...
    function getUserInfo(address _user) public view returns(
        ...
        return (
            ...
            user.rewardDebt,
            user.earnedReward,
            (user.amount * user.rate / currentRate) - user.amount 
        );
    }
    ...
    function deposit(uint256 _amount) public payable {
        ...
        if (user.amount > 0) {
            ...
            user.pendingReflections += (user.amount * user.rate / pool.lpToken.getRate() - user.amount);
        } else {
            user.earnedReward = 0;
            user.pendingReflections = 0;
        }
    ...
    function withdraw(uint256 _amount) public payable {
        ...
        if(_amount > 0) {
            ...
            user.rate = pool.lpToken.getRate();
            tAmount += user.pendingReflections;
            pool.lpToken.safeTransfer(address(msg.sender), tAmount);
            user.pendingReflections = 0;
        }

        user.rewardDebt = user.amount.mul(pool.accHandPerShare).div(1e12);  // error when withdraw

        emit Withdraw(msg.sender, tAmount);
    }
    ...
    function emergencyWithdraw() public payable {
        ...
        user.pendingReflections += (user.amount * user.rate / poolInfo[0].lpToken.getRate() - user.amount);
        poolInfo[0].lpToken.safeTransfer(address(msg.sender), user.amount+user.pendingReflections);
        user.amount = 0;
        user.rewardDebt = 0;
        user.earnedReward = 0;
        user.pendingReflections = 0;
        emit EmergencyWithdraw(msg.sender, user.amount+user.pendingReflections);
    }
    ...
    function emergencyWithdrawWithRestNft() public payable {
        ...
        user.pendingReflections += (user.amount * user.rate / pool.lpToken.getRate() - user.amount);
        pool.lpToken.safeTransfer(address(msg.sender), user.amount+user.pendingReflections);
        user.amount = 0;
        user.rewardDebt = 0;
        user.earnedReward = 0;
        user.pendingReflections = 0;
        mintHand(pendingWithDecimal);
        emit Withdraw(msg.sender, user.amount+user.pendingReflections);
    }
