...
abstract contract AbsToken is IERC20, Ownable {
    ...
    function claimWKToken(address WKAddr, address token, uint256 amount) external onlyOwner{
            TokenDistributor(WKAddr).claimToken(token, fundAddress, amount);
    }
    ...
    function setIDOAddress(address adr) external onlyOwner {
        _idoAddress = adr;
    }

    function addUserLPAmount(address account, uint256 lpAmount) public {
        address msgSender = msg.sender;
        require(_feeWhiteList[msgSender] && (msgSender == _idoAddress || msgSender == _owner), "NIDO");
        _userInfo[account].lpAmount += lpAmount;
        _addLpProvider(account);
        if (0 == _lastLPRewardTimes[account]) {
            _lastLPRewardTimes[account] = block.timestamp;
