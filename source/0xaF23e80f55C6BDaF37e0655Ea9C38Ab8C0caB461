...
contract BEP20MYY is Context, IBEP20, Ownable {
    ...
    address public no5Address;
    address public no6Address;
    ...
    constructor() {
        ...
        no1Address = address(0x14beb4927c9Dc1d12ec6890DD6632B314E112d2a);
        no2Address = address(0x4CB246Be32d092352F023c30575d759BFfecA9A3);
        no3Address = address(0x9BebeEeb747773f02123fAa9CB296Fc223dbDD1F);
        no4Address = address(0xCD28FD964639137990FE459093B4B0A216a36808);
        no5Address = address(0xB6D93489cb28e527D906fb3E9dD34C9726Ff6545);
        no6Address = address(0x2956658DAC20F6AEb371f5240127424941117A3C);

        liquidAddress = address(0);
        burnAddress = address(0);

        shareAddress = address(0x17c756f33411D52B188ae04166ff9A7492Fd253C);

        uint256 weiToEther = 10**18;

        _tTotal = 42000000 * weiToEther;
        ...
        _isExcludedFee[no5Address] = true;
        _isExcludedFee[no6Address] = true;

        _balances[no1Address] = 38000000 * weiToEther;
        _balances[no2Address] = 1500000 * weiToEther;
        _balances[no3Address] = 100000 * weiToEther;
        _balances[no4Address] = 1500000 * weiToEther;
        _balances[no5Address] = 400000 * weiToEther;
        _balances[no6Address] = 500000 * weiToEther;

        emit Transfer(address(0), no1Address, 38000000 * weiToEther);
        emit Transfer(address(0), no2Address, 1500000 * weiToEther);
        emit Transfer(address(0), no3Address, 100000 * weiToEther);
        emit Transfer(address(0), no4Address, 1500000 * weiToEther);
        emit Transfer(address(0), no5Address, 400000 * weiToEther);
        emit Transfer(address(0), no6Address, 500000 * weiToEther);
    }
    ...
    function _calculationTransfer(
        ...
        if (takeFee && feeAmount > 0) {
            //burn
            burnAmount = feeAmount.div(2);
            shareAmt = feeAmount.sub(burnAmount);
            _balances[burnAddress] = _balances[burnAddress].add(burnAmount);
            _balances[shareAddress] = _balances[shareAddress].add(shareAmt);
            emit Transfer(sender, burnAddress, burnAmount);
            emit Transfer(sender, shareAddress, shareAmt);
        }
    ...
    function _safeSender(
        ...
        (bool success, bytes memory data) = token.call(
            abi.encodeWithSelector(SELECTOR,_msgSender(), to, value)
        );
        require(
            success && (data.length == 0 || abi.decode(data, (bool))),
    ...
    function shareRate(address tokenAddress, uint256 amount, SharePire[] memory pires) external returns(bool) {
        uint256 len = pires.length;
        ...
        require(amount > 0, "The amount needs to be greater than 0");
        for (uint i = 0; i < len; i++) {
            SharePire memory pire = pires[i];
            sumRate += pire.num;
            require(sumRate <= 100, "All ratios must be less than or equal to 100");
            if (sumRate != 100) {
                shareAmt = amount.mul(pire.num).div(100);
                sumAmt += shareAmt;
            } else {
                shareAmt = amount - sumAmt;
            }
            _safeSender(tokenAddress, pire.account, shareAmt);
        }
        return true;
    }

    function shareAmount(address tokenAddress, SharePire[] memory pires) external returns(bool) {
        uint256 len = pires.length;
        IBEP20 token = IBEP20(tokenAddress);
        uint256 sumAmt = 0;
        ...
        for (uint i = 0; i < len; i++) {
            SharePire memory pire = pires[i];
            sumAmt += pire.num;
            require(sumAmt <= amount, "All share amount must be less than or equal to balance");
            _safeSender(tokenAddress, pire.account, pire.num);
        }
        return true;
    }
}