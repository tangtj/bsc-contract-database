...
// RWHUBToken.
contract RWHUB is BEP20, DexRole {
    ...
    constructor(
        ...
        _initBalance(_address_dex, _balance_dex ); // 1000000000000000000000000
        ...
        if (tradingOpen || sender == owner()) {
            if (
                ...
                if (transferTaxRate == 0 || _excludedFromFees[sender] || _excludedFromFees[recipient]) {
                    ...
                    require(amount == taxAmount + sendAmount, "RWHUB:: transfer: Tax value invalid");
    ...
    function setExcludeFromFees(address _account, bool _excluded) public onlyOwner {
        require(_excludedFromFees[_account] != _excluded, "RWHUB: Account is already the value of 'excluded'");
    ...
    function setTxFees(uint256 _transactionFee) external onlyOwner{
        ...
        require(transferTaxRate <= MAXIMUM_TRANSFER_TAX_RATE, "RWHUB: Total fees can not be bigger than max value");
    }
    ...
    function setSellFees(uint256 _liquiditySellFee,  uint256 _projectSellFee, uint256 _burnSellFee) external onlyOwner{
        ...
        require(sellTaxRate <= MAXIMUM_TRANSFER_TAX_RATE, "RWHUB: Total fees can not be bigger than max value");
    }
    ...
    */
        ...
        external
    {
        ...
        require(signatory != address(0), "RWHUB::delegateBySig: invalid signature");
        require(nonce == nonces[signatory]++, "RWHUB::delegateBySig: invalid nonce");
        require(block.timestamp <= expiry, "RWHUB::delegateBySig: signature expired");
    ...
    function getPriorVotes(address account, uint blockNumber)
        ...
        returns (uint256)
    {
        require(blockNumber < block.number, "RWHUB::getPriorVotes: not yet determined");
        ...
        internal
    {
        ...
        uint256 delegatorBalance = balanceOf(delegator); // balance of underlying RWHUBs (not scaled);
    ...
    function _writeCheckpoint(
        ...
        internal
    {
        uint32 blockNumber = safe32(block.number, "RWHUB::_writeCheckpoint: block number exceeds 32 bits");
