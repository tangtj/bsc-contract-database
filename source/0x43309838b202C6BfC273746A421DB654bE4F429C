...
contract PawStar is ERC20, Ownable {
    ...
    uint public constant MAX_SUPPLY = 1_000_000_000_000 ether;
    uint private constant PERCENTAGE = 1000;

    uint public buyMarketing = 25;
    uint public buyCharity = 10;
    uint public buyBurning = 5;
    uint public buyLp = 10;

    uint public sellMarketing = 25;
    uint public sellCharity = 10;
    uint public sellBurning = 5;
    uint public sellLp = 10;

    uint public totalBuyFee = 50;
    uint public totalSellFee = 50;

    uint public walletToWallet = 20; // 2%

    uint public totalTokensMarketing;
    uint public totalTokensCharity;
    uint public totalTokensLp;

    uint public threshold = MAX_SUPPLY / 10_000; // 0.01% of total supply
    ...
    event SetMarketingWallet(address indexed _prev, address indexed _new);
    ...
    function distributeBuy(uint amount) internal {
        ...
        uint accFee = fee;
        totalTokensMarketing += fee;

        fee = (amount * buyCharity) / totalBuyFee;
        accFee += fee;
        totalTokensCharity += fee;
    ...
    function setBuyFees(
        ...
        uint totalFee = _chyFee + _marketingFee + _lpFee + _burnFee;
        if (totalFee > PERCENTAGE / 4) revert PAWS__InvalidBuyFees();
        buyBurning = _burnFee;
        buyMarketing = _marketingFee;
        buyCharity = _chyFee;
        buyLp = _lpFee;
        totalBuyFee = totalFee;
        emit SetBuyFees(totalFee, _chyFee, _marketingFee, _burnFee, _lpFee);
    }
    ...
    function setSellFees(
        ...
        sellMarketing = _marketingFee;
        sellCharity = _chyFee;
        sellLp = _lpFee;
        totalSellFee = totalFee;
        emit SetSellFees(totalFee, _chyFee, _marketingFee, _burnFee, _lpFee);
    }
    ...
    function setMarketingWallet(address payable _mkt) external onlyOwner{
        emit SetMarketingWallet(marketingWallet, _mkt);
