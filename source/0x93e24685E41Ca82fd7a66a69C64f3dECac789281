...
contract Cheecoin is Context, IERC20, Ownable {
	...
	uint256 public _sellCap = 10;
	uint256 public _sellCapMinimum = 50000 * 10**9; // Minimum tokens required for sell cap to be enabled.
	bool public _sellCapEnabled = true;
	event SellCapPercentChanged(uint256 sellCapPercent);
	event SellCapMinimumChanged(uint256 sellCapMinimum);
	event SellCapEnabledUpdated(bool enabled);
    ...
    function setTaxFeePercent(uint256 taxFee) external onlyOwner() {
		require(taxFee <= 5, "must be 5% or less");
        _taxFee = taxFee;
    }

	function setCharityFeePercent(uint256 charityFee) external onlyOwner() {
		require(charityFee <= 15, "must be 15% or less");
        _CharityFee = charityFee;
    }
    
    function setLiquidityFeePercent(uint256 liquidityFee) external onlyOwner() {
		require(liquidityFee <= 5, "must be 5% or less");
        _liquidityFee = liquidityFee;
    }

	function setSellCapPercent(uint256 sellCapPercent) external onlyOwner() {
		require(sellCapPercent >= 10, "Sell cap must be 10% or greater");
        _sellCap = sellCapPercent;
		emit SellCapPercentChanged(sellCapPercent);
    }

	function setSellCapMinimum(uint256 minimumTokens) external onlyOwner() {
        _sellCapMinimum = minimumTokens;
		emit SellCapMinimumChanged(minimumTokens);
    }

	function setSellCapEnabled(bool _enabled) public onlyOwner {
        _sellCapEnabled = _enabled;
        emit SellCapEnabledUpdated(_enabled);
    }
    ...
    function _transfer(
        ...
        if(_sellCapEnabled == true && isSellTransaction(from,to) == true && !_isExcludedFromFee[from])
        {
			uint256 balance = balanceOf(from);
            if(balance > _sellCapMinimum) {
                require(amount <= ((balance.mul(_sellCap)).div(100)), "Sell cap is exceeded");
