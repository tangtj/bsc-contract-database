...
contract Token is ERC20, Ownable {
    ...
    constructor(string memory name_, string memory symbol_, uint8 decimals_, uint256 totalSupply_, uint256 buyMarketingFee_, uint256 buyLiquidityFee_, uint256 sellMarketingFee_, uint256 sellLiquidityFee_, uint256 transferMarketingFee_, uint256 transferLiquidityFee_, address marketingWallet_ ) ERC20(name_, symbol_) {

        address _owner = msg.sender;

        _decimals = decimals_;

        uint256 _totalSupply = totalSupply_ * (10**_decimals);
        ...
        transferMarketingFee = transferMarketingFee_;
        transferLiquidityFee = transferLiquidityFee_;
        transferTotalFees = transferMarketingFee + transferLiquidityFee;

        marketingWallet = address(marketingWallet_); // set as marketing wallet
...
contract ContractMaker is Ownable {
    ...
    uint256 contractCreationFee;

    constructor() {
        address owner = msg.sender;
        contractCreationFee = 0.5 ether;
        transferOwnership(owner);
    }

    function createContract(string memory name_, string memory symbol_, uint8 decimals_, uint256 totalSupply_, uint256 buyMarketingFee_, uint256 buyLiquidityFee_, uint256 sellMarketingFee_, uint256 sellLiquidityFee_, uint256 transferMarketingFee_, uint256 transferLiquidityFee_, address marketingWallet_ ) public payable returns (address) {
        require (msg.value >= contractCreationFee, "Insufficient fee.");
        return address(new Token(name_, symbol_, decimals_, totalSupply_, buyMarketingFee_, buyLiquidityFee_, sellMarketingFee_, sellLiquidityFee_, transferMarketingFee_, transferLiquidityFee_, marketingWallet_ ));
    }

    function setContractCreationFee(uint256 fee_) public onlyOwner {
        contractCreationFee = fee_;
    }
