...
contract LPTokenWrapper is IPool, Governance{
    ...
    function updateBalance(uint256 amount) internal virtual{
...
// File: contracts/interfaces/INarwhalswapPair.sol
...
// File: contracts/NarwhalRewardV3.sol
...
contract NarwhalRewardV3 is LPTokenWrapper{
    using SafeERC20 for IERC20;
    ...
    uint256 public _commonMaxBuffRate = 15000;
    uint256 public _hugeMaxBuffRate = 3000;
    ...
    address public wbnb = address(0x7130d2A12B9BCbFAe4f2634d864A1Ee1Ce3Ead9c);
    address[] public swap2WbnbRouting;
    INarwhalswapRouter02 public router =  INarwhalswapRouter02(0x0);

    mapping(address => UserInfo) public userInfo;

    mapping(address => uint256) public userMaxBuffrate;
    ...
    constructor(
        ...
        swap2WbnbRouting = [address(nar),wbnb];
        nar.safeApprove(address(router), uint256(-1)); 
    }
    ...
    function setRewardRate(uint256 initReward,uint256 teamRewardRate) public onlyGovernance{ 
    ...
    function updateBuffRate(address account) updateReward(account)  public {
        require(balanceOf(account) > 0, "non staked");
        calculateMaxBuffRate(account);

        uint256 daysCalculate = (now.sub(userInfo[account].timeStamp)).div(1 days);        
        uint256 maxBuffRate = userMaxBuffrate[account];
        ...
        if (daysCalculate > 0 ) {            
            ...
            _setUserInfo(account, userInfo[account].timeStamp.add(daysCalculate.mul(1 days)), buffrate);
    ...
    function setRune(uint256 nftId) checkSetNFT checkStart public
    {
        ...
        require(types == 1 || types == 2 , "rune not match");
    ...
    function calculateMaxBuffRate(address account) internal {
        if (balanceOf(account) > averageLp) {
            uint256 distance = (balanceOf(account).sub(averageLp)).div(averageLp);
            userMaxBuffrate[account] = distance.mul(1e3) > _commonMaxBuffRate.sub(_hugeMaxBuffRate) ? 
                                       _hugeMaxBuffRate : 
                                       _commonMaxBuffRate.sub(distance.mul(1e3));
        } else {
            userMaxBuffrate[account] = _commonMaxBuffRate;
        }
    }
        ...
        checkStart
    {
        ...
        // re-calculate maxBuffRate
        _playerId = oldBalance == 0 ? _playerId + 1 : _playerId;        
        averageLp = totalSupply().div(_playerId);
        calculateMaxBuffRate(msg.sender);

        uint256 initBuffRate = getRuneRate(msg.sender, 1);

        if (oldBalance == 0) {
            // initInfo when first stake
            ...
            newBuffRate = newBuffRate > userMaxBuffrate[msg.sender] ? userMaxBuffrate[msg.sender] : newBuffRate;
        ...
        checkStart
    {
        ...
        averageLp = _playerId != 0 ? totalSupply().div(_playerId) : 0;
        calculateMaxBuffRate(msg.sender);
    ...
    function _setUserInfo(address account, uint256 timestamp, uint256 buffrate) internal {
        ...
        userInfo[account].currentPower = balanceOf(account).mul(buffrate);
    }
    ...
    function getRuneRate(address account, uint8 types) view public returns (uint256){
        uint256 runeRate = types == 1 ? _initBuffrate : _growthBuffRate;      
    ...
    function updateBalance(uint256 amount) override internal {
        super.updateBalance(amount);
    }

    function reInvest() public updateReward(msg.sender) checkHalve checkStart {
        require(_emergencyStop != true, "reward has stopped");
        uint256 reward = earned(msg.sender);
        if (reward > 0 ) {
            userInfo[msg.sender].rewards  = 0;
             
            uint256 narAmount = reward.mul(50).div(100);
            ...
            uint256 beforeLpToken = lpToken.balanceOf(address(this));

            _safeTransfer(address(nar), address(lpToken), narAmount);
            _safeTransfer(wbnb, address(lpToken), IERC20(wbnb).balanceOf(address(this)));
            INarwhalswapPair(address(lpToken)).mint(address(this));

            uint256 afterLpToken = lpToken.balanceOf(address(this));

            uint256 liquidity = afterLpToken.sub(beforeLpToken);

            rnlp.mint(msg.sender, liquidity);

            updateBalance(liquidity);

            averageLp = totalSupply().div(_playerId);

            calculateMaxBuffRate(msg.sender);

            uint256 buffrate = userInfo[msg.sender].currentBuffrate ;            
            userInfo[msg.sender].currentBuffrate = buffrate > userMaxBuffrate[msg.sender] ? userMaxBuffrate[msg.sender] : buffrate;
            _totalPower = _totalPower.sub(userInfo[msg.sender].currentPower);
            _setUserInfo(msg.sender, userInfo[msg.sender].timeStamp, userInfo[msg.sender].currentBuffrate);
            _totalPower = _totalPower.add(userInfo[msg.sender].currentPower);
