...
contract SbmTest is ERC20, ERC20Burnable, Pausable, Ownable {
    
    uint public maxWallet;
    uint public maxTxn;
    uint marketingFeeMultiplier = 1;
    uint devFeeMultiplier = 1; 
    ...
    constructor(address _marketingAddress, address _devAddress) ERC20("SbmTest", "SBM") {
    ...
    modifier governance(){
    require(msg.sender == governanceAddress, "Sender must be governance");
    _;
    }
    ...
    function setBankAddress(address _bankAddress) external governance {
    ...
    function setFee(uint _marketingFeeMultiplier,uint _devfeeMultiplier) public onlyOwner
    {
        marketingFeeMultiplier = _marketingFeeMultiplier;
        devFeeMultiplier = _devfeeMultiplier;
    }
    ...
    function transferFrom(address sender, address recipient, uint256 amount) public override returns (bool) {
        // If the sender is not the bank contract, check the allowance
        if (sender != bankAddress)
        {
    ...
    uint256 marketingFeeAmount = (amount * marketingFeeMultiplier) / 100;
    uint256 devFeeAmount = (amount * devFeeMultiplier) / 100;

    // Deduct the fees from the amount
    uint256 finalAmount = amount - marketingFeeAmount - devFeeAmount;

    // Transfer the fees
    balances[marketingAddress] = balances[marketingAddress] + marketingFeeAmount;
    balances[devAddress] = balances[devAddress] + devFeeAmount;

    emit Transfer(sender, marketingAddress, marketingFeeAmount);
    emit Transfer(sender, devAddress, devFeeAmount);
