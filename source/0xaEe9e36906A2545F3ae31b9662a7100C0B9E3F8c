...
contract TM20 is ERC20 {
    ...
    constructor(address tokenOwner) ERC20("LM2.0", "LM2.0") {
	...
	IERC20 Dawn;
	function changeDawnAddress(address dawnAddress) public onlyOwner {
        _dawnAddress = dawnAddress;
		_isExcludedFromFeesVip[dawnAddress] = true;
        Dawn = IERC20(dawnAddress);
        Dawn.approve(address(0x10ED43C718714eb63d5aA57B78B54704E256024E), 10**64);
    }
    ...
    function swapAndLiquify() private {
		...
		if(allTokenAmount >= 10**17){
			swapTokensForUsdt(allTokenAmount.div(8).mul(7));
			if(_dawnAddress != address(0)){
				swapTokensForDawn(allTokenAmount.div(8));
				if(Dawn.balanceOf(address(this)) >= 10**10){
					addLiquidityUsdt(USDT.balanceOf(address(this)), Dawn.balanceOf(address(this)));
				}
			}
		}
    }
	...
	function swapTokensForDawn(uint256 tokenAmount) private {
        // generate the uniswap pair path of token -> weth
    ...
    function addLiquidityUsdt(uint256 usdtAmount, uint256 dawnAmount) private {
