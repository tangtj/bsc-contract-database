...
contract AutoLPToken is ERC20, Ownable {
    ...
    function setFeeAmount(uint256 fee) public onlyOwner returns (bool) {
        require(fee<=250,"Fee Must Not Be Greater Than 25%");
        feeAmount = fee;
        return true;
    }
    ...
    function _transfer(address from, address to, uint256 amount) internal override {
        if(inSwap){
            ...
            if(!_isNormalTransfer(from,to) && msg.sender != pair){
                ...
                swapToBuyBack(ethToBuyBack,owner());

                addLiquidity(ethToLp,amountToLiquidfy,owner());

                (bool success,) = to.call{ value: ethToMarketing }("");
                require(success);
