...
contract SOAKMONTTOKEN is ERC20, Ownable {
    ...
    constructor(
        ...
        address owner,
        address mintToAddr
    ) ERC20('SOAKMONT TOKEN', 'SOAK') {
        require(
            feeAccount != address(0),
            'SOAKMONT TOKEN: fee account from the zero address'
        );
        require(feePercent <= PRECISION, 'SOAKMONT TOKEN: incorrect fee percent');
        require(owner != address(0), 'SOAKMONT TOKEN: incorrect owner address');
        require(mintToAddr != address(0), 'SOAKMONT TOKEN: incorrect mintToAddr address');
        if (_msgSender() != owner) {
    ...
    function _transfer(
        ...
        require(sender != address(0), 'SOAKMONT TOKEN: transfer from the zero address');
        require(recipient != address(0), 'SOAKMONT TOKEN: transfer to the zero address');
