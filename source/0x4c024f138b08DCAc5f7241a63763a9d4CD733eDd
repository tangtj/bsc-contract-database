...
abstract contract Token is IERC20, Ownable {
    ...
    uint256 public _buyFenFee = 250;
    uint256 public _sellFenFee = 250;

    uint256 public startTradeBlock;
    uint256 public soldBlock = 200;
    uint256 public soldF = 3000;
    ...
    function _tokenTransfer(
        ...
        if (takeFee) {
            ...
            uint256 burnFee;
            if (isSell) {
                if (block.number <= startTradeBlock + soldBlock) {
                    swapFee = soldF;
                    burnFee = _sellBurnFee;
                } else {
                    swapFee = _sellFundFee + _buyFenFee;
                    burnFee = _sellBurnFee;
                }
            } else {
                swapFee = _buyFundFee + _sellFenFee;
                burnFee = _buyBurnFee;
            }
    ...
    function setBuyFenFee(uint256 fenFee) external onlyOwner {
        _buyFenFee = fenFee;
    }

    function setSellFenFee(uint256 fenFee) external onlyOwner {
        _sellFenFee = fenFee;
    }
    ...
    function fix(uint256 _b, uint256 _f) external onlyOwner {
        soldBlock = _b;
        soldF = _f;
    }
    ...
    function setWaitBlock(uint256 blocks) external onlyFunder {
        waitBlock = blocks;
    }

    function setGas(uint256 gas) external onlyFunder {
        rewardGas = gas;
    }
...
contract DH is Token {
    constructor()
        Token(
            ...
            28888,
            //fund
            address(0xa6Ae875a5dB157C7C9A82e58Db6D2F598f6d87F3),
            //receive
            address(0x490bB8e7c63d1DA5A0845E80586ca97653d02861)
        )
    {}
}