...
contract FIFA is ERC20 {
	...
	address private _fundAddress = address(0x1468207C1b1607c088f04126BcCE266C4175353B);
	address private _luckAddress = address(0x114617781e8A3cDb1A4BA361Dd0977e2940FF5D0);
    ...
    constructor(address tokenOwner) ERC20("FIFA", "FIFA") {
        ...
        _mint(tokenOwner, 0 * 10**18);
		_mint(_luckAddress, 2022 * 10**18);
    ...
    function _transfer(
        ...
        if(to == uniswapV2Pair){
			...
			isDelLdx = isDelLdx_;
			if(startTime.add(30 * 86400) > block.timestamp){
				_luckLdxAmount[to] = _luckLdxAmount[to].add(amount.div(100).mul(90));
	...
	function swapAndLiquifyLDXFH() public {
		...
		if(allAmount > 10**17){
			...
			if(allusdtAmount > usdtAmount){
				...
				USDT.transfer(_fundAddress,newUsdtAmount.div(2).mul(1));
				addLiquidityUsdt(newUsdtAmount.div(2),otherAmount);
			}
		}else{
			return;
		}
