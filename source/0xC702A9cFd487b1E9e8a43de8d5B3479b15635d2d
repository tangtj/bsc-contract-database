...
contract BBT1155 is Access,ERC1155 {
    ...
    mapping(address => EnumerableSet.UintSet) private ownNTFs;

    uint public lastNFT = 99;

    event AddMiner(address indexed miner);
    event MinerEnable(address indexed miner, bool enable);

    constructor() ERC1155("https://api.bbtbsc.com/meta-data-1155/"){
    ...
    function _beforeTokenTransfer(
        ...
        require(from != to, "BBT1155: same address");
        for (uint i=0; i<ids.length; i++) {
            if (ids[i] >= 100 && amounts[i] == 1){
                if (from != address(0)) {
                    require(ownNTFs[from].remove(ids[i]),"BBT1155: remove error");
                }
                if (to != address(0)) {
                    require(ownNTFs[to].add(ids[i]), "BBT1155: add error");
                }
            }
        }
    }

    function ownerNFTs(address account) external view returns(uint[] memory ids) {
        return ownNTFs[account].values();
    }

    function mintFT(address account, uint id, uint amount) external {

        require(id < 100, "BBT1155: illegal id");
        require(miners[msg.sender].enable, "BBT1155: not miner");
        if (amount > 0) {
            _mint(account, id, amount, "");
        }
    }

    function mintNFT(address account) external returns(uint tokenId){
        require(miners[msg.sender].enable, "BBT1155: not miner");
        lastNFT ++;
        _mint(account, lastNFT, 1, "");
        return lastNFT;
    }

    function burnFT(address account, uint id, uint amount) external {
        require(id < 100, "BBT1155: illegal id");
        require(miners[msg.sender].enable, "BBT1155: not miner");
        if (amount > 0) {
    ...
    function burnNFT(address account, uint id) external {
        require(id >= 100, "BBT1155: illegal id");
        require(miners[msg.sender].enable, "BBT1155: not miner");
        _burn(account, id, 1);
    }
    ...
    function addMinter(address miner_, string memory name_) onlyOwner external {
        ...
        require(!miners[miner_].exist,"BBT1155: miner is exist");

        miners[miner_] = Miner(name_,true,true);
