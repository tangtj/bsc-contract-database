...
contract NFTradeNFTToken is ERC721, AccessControl {
    ...
    constructor(string memory name, string memory symbol, address registry) public ERC721(name, symbol) {
        _setupRole(DEFAULT_ADMIN_ROLE, msg.sender);
        _setupRole(MINTER_ROLE, msg.sender);
        Registrar(registry).addCollection(address(this), name, symbol, msg.sender);
    }
    ...
    function mint(address receiver, string memory metadata) public onlyRole(MINTER_ROLE) returns (uint256) {
        _tokenIds.increment();
