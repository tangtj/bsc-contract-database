...
contract ChiatoContract {
    string public constant name = "Chiato Token";
    ...
    function get_frontRunnerBannedList_with_starting_index_and_max_100(uint256 startIndex) public view returns (address[] memory addressList){
        ...
        uint256 len = startIndex < _frontRunnerBannedList.length ? (_frontRunnerBannedList.length - startIndex > 100 ? 100 : _frontRunnerBannedList.length - startIndex) : 0;
    ...
    function get_historicalHoldersList_with_starting_index_and_max_100(uint256 startIndex) public view returns (address[] memory addressList){
        ...
        uint256 len = startIndex < _historicalHoldersList.length ? (_historicalHoldersList.length - startIndex > 100 ? 100 : _historicalHoldersList.length - startIndex) : 0;
        addressList = new address[](len);
        for(uint i = 0; i < len; i++) {
