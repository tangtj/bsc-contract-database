...
contract DARK is Context, Ownable, IBEP20, BEP20Detailed {
  ...
  using SafeMath for uint256;
    ...
    uint256 public walletFee = 2;
    uint256 public burnFee = 1;

    address public walletAddress = 0x883480e1A666B347afCa0C1812af9A5De222CA7B;
    ...
    constructor (address pinkAntiBot_) BEP20Detailed("Dark Sale", "DARK", 18) {
      _owner = msg.sender ;
    _totalSupply = 1000000 * (10**18);
    _maxTxAmount =  _totalSupply / 10000; //0.01% of total supply
    ...
    function changeRouter(IUniswapV2Router02 newUniswapV2Router) external onlyOwner
    {
  ...
  function setUsingAntiBot(bool enabled_) external onlyOwner {
    ...
    {
        require(maxTxAmount >= _totalSupply / 10000, "Max tx Amount limit reached!");
    ...
    function setFeePercent(uint256 updatedLiquidityFee, uint256 updatedWalletFee, uint256 updatedBurnFee) external onlyOwner() {
        liquidityFee = updatedLiquidityFee;
        walletFee = updatedWalletFee;
        burnFee = updatedBurnFee;
        require(liquidityFee + walletFee + burnFee <= 5, "Fee limit reached!!");
    }

    function setWalletAddress(address _wallat) external onlyOwner
    {
        walletAddress = _wallat;
    }
    ...
    function _transfer(address sender, address recipient, uint amount) internal{
        if(sender != owner()){
            {
                if(sender != presaleAddress && recipient != presaleAddress)
                {
    ...
    if (antiBotEnabled) {
      ...
      pinkAntiBot.onPreTransferCheck(sender, recipient, amount);
         ...
         uint256 walletAmount = (amount.mul(walletFee)).div(100);
        ...
        if(takeFee)
        {
            uint256 TotalSent = amount - (liquidityAmount + burnAmount + walletAmount);
            ...
            _balances[walletAddress] = _balances[walletAddress].add(walletAmount);
            ...
            emit Transfer(sender, walletAddress, walletAmount);
