...
contract bts_Presale {
    ...
    function buyTokensWithUsdt(uint256 _usdtAmount) public {
        require(usdt.approve(address(this), _usdtAmount), "Approval failed");
    ...
    function buyTokensWithBusd(uint256 _busdAmount) public {
        require(busd.approve(address(this), _busdAmount), "Approval failed");
    ...
    function withdrawBusd(uint256 amount) external {
        require(admin == msg.sender, "caller is not the owner");
        require(busd.transfer(admin, amount), "Transfer failed");
    }
