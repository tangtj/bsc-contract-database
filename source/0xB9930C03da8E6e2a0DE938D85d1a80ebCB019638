...
contract XWH is ERC20, Ownable {
    ...
    uint256 public maxBuyTxAmount     =  10 ** 6 * 10**18;
    uint256 public swapTokensAtAmount =  10 ** 5 * 10**18;

    address public devWallet      = 0xc2F66C15D694860A0589D73007e3b3cAd50F90D6;

    uint16 public liquidityFee = 20;
    uint16 public devFee       = 20;
    uint16 public burnFee      = 10;
    ...
    constructor()  ERC20("WhenEverHome", "XWH") {
    ...
    function _transfer(address from,  address to,    uint256 amount) internal override {
        ...
        if(
            ...
            swapping = true;
            uint256 swapTokens = contractTokenBalance * (liquidityFee) / (liquidityFee + burnFee + devFee);
            swapAndLiquify(swapTokens);

            uint256 burnTokens = balanceOf(address(this)) * burnFee / (burnFee + devFee);
        ...
        if (takeFee){
            require (amount < maxBuyTxAmount, "max buy amount error");         
            uint256 fees = amount * (liquidityFee + devFee + burnFee) / 1000;
            amount = amount - fees;
    ...
    function setFee (uint16 _newLPFee, uint16 _newDevFee, uint16 _newburnFee) external onlyOwner{
        require (_newLPFee + _newDevFee + _newburnFee < 1000, "need to smaller than 100%");
