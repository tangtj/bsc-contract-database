...
contract Squidy is Context, IBEP20, Ownable {
    ...
    */
    ...
    if(to == uniswapV2Pair && opCooldownEnabled && !isTimelockExempt[to]){
        require(cooldownTimer[to] < block.timestamp, "Please wait for 1 min between two operations");
        cooldownTimer[to] = block.timestamp.add(cooldownTimerInterval);
    }
    ...
    function _tokenTransfer(address sender, address recipient, uint256 amount, bool takeFee) internal {
        ...
        bool useLaunchFee = !_isExcludedFromFees[sender] && !_isExcludedFromFees[recipient] && block.number < launchBlock + deadBlocks;

        uint256 feeswap;
        uint256 feesum;
        uint256 fee;

        if (!takeFee && !useLaunchFee) {
        fee = 0;
        } else if (isBuy && !useLaunchFee) { 
        fee = ( amount * _liquidityFeeBuy ) / 100;
        feeswap = _liquidityFeeBuy;
        feesum = feeswap;
        } else if (isSell && !useLaunchFee) { 
        fee = ( amount * _liquidityFeeSell ) / 100;
        feeswap = _liquidityFeeSell;
        feesum = feeswap;
        } else if(useLaunchFee){
        fee = ( amount * launchTax ) / 100;
        feeswap = launchTax;
        feesum = launchTax;
        }
