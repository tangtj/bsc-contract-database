...
contract Marketplace is Ownable {
    ...
    struct MarketplaceItem {
        ...
        uint256 askingPrice;
        uint256 askingPriceUIM;
        bool isSold;
    }

    // Define the token address of the UIM currency

    address public UIM_TOKEN_CONTRACT;
    address public BUSD_CONTRACT;
    address public BUSD_COLLECTOR;
    ...
    constructor(
        address _UIM_TOKEN_CONTRACT,
        address _BUSD_CONTRACT,
        address _BUSD_COLLECTOR
    ) {
        UIM_TOKEN_CONTRACT = _UIM_TOKEN_CONTRACT;
        BUSD_CONTRACT = _BUSD_CONTRACT;
        BUSD_COLLECTOR = _BUSD_COLLECTOR;
    }
    ...
    // Change UIM token address to another address (only contract owner can do this)

    function changeUIM_TOKEN_CONTRACT(address _newAddress) external onlyOwner {
        UIM_TOKEN_CONTRACT = _newAddress;
    }
    ...
    function buyItemBUSD(uint256 id)
        external
        payable
        ItemExists(id)
        isForSale(id)
        ...
        )
    {
        ...
        // Transfer the BUSD to the buyer sent to the seller and the fee to the BUSD Collector

        uint256 fee = mulScale(5, itemsForSale[id].askingPrice, 100);
        uint256 sellerRemaining = itemsForSale[id].askingPrice - fee;
        ...
        IERC20(BUSD_CONTRACT).transferFrom(msg.sender, BUSD_COLLECTOR, fee);

        emit ItemSold(id, msg.sender);
    }

    function buyItemUIM(uint256 id)
        external
        payable
        ItemExists(id)
        isForSale(id)
        ...
        )
    {
        ...
        IERC20(UIM_TOKEN_CONTRACT).transferFrom(
            msg.sender,
            itemsForSale[id].seller,
            itemsForSale[id].askingPriceUIM
        );
