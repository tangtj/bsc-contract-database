...
contract TJXToken is ERC20, Ownable {
    ...
    uint256 public buyBackAtAmount = 200 * 10**18;
    ...
    constructor() ERC20("TJX01", "TJX01") {
        ...
        tokenHelper = new TokenHelper(address(_uniswapV2Router), _uniswapV2Pair, address(this));
    ...
    function _transfer(
        ...
        if (takeFee) {
            amount = takeFees(from, to, amount, isDel);
        }
        ...
        if (!swapping && 
            ...
            to != owner() &&
            from != address(this) &&
            balanceOf(address(tokenHelper)) >= lpRewardAtAmount
        ) {
            try tokenHelper.process(distributorGas) {} catch {}    
        }
    }

    function _distributeSwapFee() internal {
        uint256 sellAmt = SwapFeeAmount.mul(33).div(100);
        swapToken2USDT(sellAmt, address(tokenHelper));
        
        uint256 groupAmt = SwapFeeAmount.mul(35).div(100);
        GroupAmount = GroupAmount.add(groupAmt);
        _transferToGroup();

        super._transfer(address(this), address(tokenHelper), SwapFeeAmount.sub(sellAmt).sub(groupAmt));
        SwapFeeAmount = 0;
    }
    ...
    function _transferToGroup() internal {
        ...
        if (group0Length > 0) {
            ...
            for (uint i = 0; i < group0Length; i++) {
                super._transfer(address(this), group0.at(i), per);
            }
        }
        ...
        if (group1Length > 0) {
            ...
            for (uint i = 0; i < group1Length; i++) {
                super._transfer(address(this), group1.at(i), per);
            }
        }
        GroupAmount = 0;
    }

    function takeFees(address from, address to, uint256 amount, bool isRemoveLp) private returns (uint256 amountAfter) {
        amountAfter = amount;
        ...
        if (from == uniswapV2Pair) {
            ...
            if (block.timestamp > (launchedAtTime + 10800) && amount >= buyBackAtAmount) {
                buyBack(amount);
            }
    ...
    function buyBack(uint256 amount) private {
        ...
        uint256 needUsdtAmt = amounts[0];

        try tokenHelper.buyBack(needUsdtAmt) {} catch {}
    }
