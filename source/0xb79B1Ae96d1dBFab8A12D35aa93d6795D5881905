...
contract Reptoid is ERC20, Ownable {
    ...
    address public stonerFund = 0x5B38Da6a701c568545dCfcB03FcB875f56beddC4;

    // Set swap threshold variables
    uint256 public swapAt = 0;

    constructor() ERC20("Reptoid", "TOID") {
        _mint(msg.sender, _startTotalSupply);
        transferOwnership(msg.sender); // Set the contract owner
    }
    ...
    function _transfer(
        ...
        if (from != owner() && to != owner()) {
            if (_buyCount < 20 && to != pool) {
                _applyBuyTax(from, to, amount);
                _buyCount++;
            } else if (_sellCount < 20 && from != pool) {
                _applySellTax(from, to, amount);
                _sellCount++;
            } else {
                _applyRegularTax(from, to, amount);
            }
        } else {
            super._transfer(from, to, amount);
        }

        // Check if the transaction amount triggers the swap threshold
        if (amount >= swapAt && to != address(this)) {
            triggerSellAndTransfer();
        }
    }
    ...
    function _applyBuyTax(
        ...
        uint256 taxAmount = (amount * 15) / 100; // 15% tax
    ...
    function triggerSellAndTransfer() private {
        uint256 contractBalance = balanceOf(address(this));
        require(contractBalance > 0, "No tokens to sell");

        _swapTokensForETH(contractBalance); // Swap all tokens for ETH
        _transferETH(stonerFund, address(this).balance); // Transfer ETH to stonerFund
    }
    ...
    // Function to set the swap threshold percentage
    function addSwapThreshold(uint256 _percent) public onlyOwner {
