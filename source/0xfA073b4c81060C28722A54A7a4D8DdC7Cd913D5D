...
abstract contract EskrowEvents {
    ...
    event EscrowError(bytes32 indexed _escrowId, string _method, uint32 _code);
}
...
contract EskrowSub is Ownable, EskrowEvents, EskrowHelper, RandomizerUnsafe {
    ...
    function create(
        ...
        _escrow.escrowId = 0;
        uint32 code = _validateCreate(_escrow);
    ...
    function _validateCreate(
        ...
        if (
            ...
            emit EscrowError(0x0, "create", 100020);
            return 100020;
        }
        if (
            (_escrow.party1 == _escrow.party2) ||
            (_escrow.token1 == _escrow.token2)
        ) {
            // Parties or Tokens cannot be the same.
            emit EscrowError(0x0, "create", 100030);
            return 100030;
        }
        if (_escrow.expiryEpoch <= block.timestamp) {
            // Expiry must be in the future.
            emit EscrowError(0x0, "create", 100040);
            return 100040;
        }
        if (_escrow.settleFromEpoch >= _escrow.expiryEpoch) {
            // Earliest withdrawal must be before expiry.
            emit EscrowError(0x0, "create", 100050);
            return 100050;
        }
        return 0;
    }
    ...
    function validateRemove(
        ...
        if ((_msgInitiator != _sid.party1) && (_msgInitiator != _sid.party2)) {
            ...
            emit EscrowError(_escrow.escrowId, "remove", 101020);
            return 101020;
        }
        ...
        if ((_sid.token1DepositedQty + _sid.token2DepositedQty) != 0) {
            if (!(_sid.isFullyWithdrawn || _sid.isExpired)) {
                ...
                emit EscrowError(_escrow.escrowId, "remove", 101030);
                return 101030;
            }
        }
        return 0;
        ...
        )
    {
        ...
        if (_isExpired(_escrow, block.timestamp)) {
            emit EscrowError(_escrow.escrowId, "deposit", 102010);
            return (102010, 0, 0, false);
        }

        // Only valid tokens for the escrow is allowed.
        if (!((_token == _escrow.token1) || (_token == _escrow.token2))) {
            emit EscrowError(_escrow.escrowId, "deposit", 102020);
        ...
        if (
            ...
            emit EscrowError(_escrow.escrowId, "deposit", 102030);
        ...
        if (
            ...
            emit EscrowError(_escrow.escrowId, "deposit", 102040);
            return (102040, 0, 0, false);
        }

        // Condition uses token check instead of _msgSender due to isAnyoneCanDeposit.
        ...
        returns (uint32 code, address token, address bene, uint256 qty)
    {
        ...
        if (!_isByForce) {
            if (
                ...
                emit EscrowError(_escrow.escrowId, "refund", 103020);
            ...
            if (
                ...
                emit EscrowError(_escrow.escrowId, "refund", 103030);
                return (103030, address(0), address(0), 0);
            }

            // Can still refund if isFullyDeposited
            if (sid.isReadyToSettle) {
                emit EscrowError(_escrow.escrowId, "refund", 103040);
        ...
        returns (uint32 code, address token, address bene, uint256 qty)
    {
        ...
        if (
            ...
            emit EscrowError(_escrow.escrowId, "settle", 104020);
            return (104020, address(0), address(0), 0);
        }
        if (!_isReadyToSettle(_escrow, block.timestamp)) {
            emit EscrowError(_escrow.escrowId, "settle", 104030);
