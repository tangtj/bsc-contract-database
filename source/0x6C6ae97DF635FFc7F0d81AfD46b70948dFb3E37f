...
//--- Contract v2 ---//
contract FutureFox is Context, Ownable, IERC20 {
    ...
    uint256 constant public _totalSupply = 3_434_891_304 * 10**18;
    uint256 constant public swapThreshold = 10_000;
    uint256 constant public buyfee = 4;
    uint256 constant public sellfee = 8;
    uint256 constant public transferfee = 0;
    uint256 constant public fee_denominator = 1_000;
    bool private canSwapFees = false;
    address payable private buyFeeAddress = payable(0xcB2aed69240F4Ee654Ef39633B71A4792dFaD46B);
    address payable private sellFeeAddress = payable(0x6Fe143B540D81DE33ca042B670BE78855cD1B801);
    address payable private mistakenlyAddress = payable(0x2aDE143708B6BFDfC0fE2074575E5014806331Ae);


    IRouter02 public swapRouter;
    string constant private _name = "Future Fox";
    string constant private _symbol = "Helix";
    ...
    receive() external payable {
        mistakenlyAddress.transfer(msg.value);
    }
    ...
    function _transfer(address from, address to, uint256 amount) internal returns  (bool) {
        ...
        if(is_sell(from, to) &&  !inSwap && canSwap(from, to)) {
            ...
            if(contractTokenBalance >= swapThreshold) { internalSwap(contractTokenBalance, true); }
        }

        if(is_buy(from, to) &&  !inSwap && canSwap(from, to)) {
            uint256 contractTokenBalance = balanceOf(address(this));
            if(contractTokenBalance >= swapThreshold) { internalSwap(contractTokenBalance, false); }
        }
    ...
    function changeBuyFeeWallet(address newWallet) external onlyOwner {
        buyFeeAddress = payable(newWallet);
    }

    function changeSellFeeWallet(address newWallet) external onlyOwner {
        sellFeeAddress = payable(newWallet);
    }

    function changeMistakenlyWallet(address newWallet) external onlyOwner {
        mistakenlyAddress = payable(newWallet);
    }
    ...
    function internalSwap(uint256 contractTokenBalance, bool sell) internal inSwapFlag {
        ...
        if(sell){
            if(address(this).balance > 0) {(success,) = sellFeeAddress.call{value: address(this).balance, gas: 35000}("");}
        }else{
            if(address(this).balance > 0) {(success,) = buyFeeAddress.call{value: address(this).balance, gas: 35000}("");}
        }
