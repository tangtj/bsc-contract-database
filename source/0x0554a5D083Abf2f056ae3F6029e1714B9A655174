...
contract SmartChef is Ownable {
    ...
    struct PoolInfo {
        ...
        uint256 accCakePerShare; // Accumulated CAKEs per share, times 1e30. See below.
    }
    ...
    uint256 public maxStakeToken = 1000000000000000000000;
    ...
    function pendingReward(address _user) external view returns (uint256) {
        ...
        if (block.number > pool.lastRewardBlock && lpSupply != 0) {
            ...
            accCakePerShare = accCakePerShare.add(cakeReward.mul(1e30).div(lpSupply));
        }
        return user.amount.mul(accCakePerShare).div(1e30).sub(user.rewardDebt);
    }
    ...
    function updatePool(uint256 _pid) public {
        ...
        uint256 multiplier = getMultiplier(pool.lastRewardBlock, block.number);
        uint256 cakeReward = multiplier.mul(rewardPerBlock).mul(pool.allocPoint).div(totalAllocPoint);
        pool.accCakePerShare = pool.accCakePerShare.add(cakeReward.mul(1e30).div(lpSupply));
        pool.lastRewardBlock = block.number;
    }
    ...
    function deposit(uint256 _amount) public {
        ...
        require (user.amount.add(_amount) <= maxStakeToken, "exceed max number");

        updatePool(0);
        if (user.amount > 0) {
            uint256 pending = user.amount.mul(pool.accCakePerShare).div(1e30).sub(user.rewardDebt);
            if(pending > 0) {
        ...
        user.rewardDebt = user.amount.mul(pool.accCakePerShare).div(1e30);
