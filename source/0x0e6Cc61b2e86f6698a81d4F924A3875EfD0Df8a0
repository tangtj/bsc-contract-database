...
// File: contracts/corrected_new1_flattened.sol
...
contract PricePrediction {
    ...
    struct Bet {
        ...
        uint256 endPrice;   // New field to store the price at the end of the round
    ...
    function placeBet(Prediction _prediction) external payable {
        ...
        int256 endPrice = getPriceAt(getNextRoundStartTime());
        userBets[msg.sender].push(Bet({
            user: msg.sender,
            amount: msg.value,
            initialPrice: uint256(currentPrice),
            endPrice: uint256(endPrice),
    ...
    function claim() external {
        ...
        if (lastBet.prediction == Prediction.UP && lastBet.endPrice > lastBet.initialPrice) {
            lastBet.result = Result.WON;
            payable(msg.sender).transfer(lastBet.amount * 2);
        } else if (lastBet.prediction == Prediction.DOWN && lastBet.endPrice < lastBet.initialPrice) {
            lastBet.result = Result.WON;
            payable(msg.sender).transfer(lastBet.amount * 2);
        } else {
    ...
    function checkResult(address _user, uint256 round) external view returns (string memory) {
        ...
        bool won = false;
        if (specificBet.prediction == Prediction.UP && specificBet.endPrice > specificBet.initialPrice) {
            won = true;
        } else if (specificBet.prediction == Prediction.DOWN && specificBet.endPrice < specificBet.initialPrice) {
            won = true;
        }
    ...
    function getPriceAt(uint256 timestamp) internal view returns (int256) {
        (uint80 roundId, int256 price, , uint256 timeStamp, ) = priceFeed.latestRoundData();
        while(timeStamp > timestamp && roundId > 0) {
            roundId--;
