...
contract TT is Context, IERC20, Ownable {
    ...
    function _transferTokensAndTax(
        ...
        if (!takeFee) { removeTax(); }
        _basicTransfer(sender, recipient, amount);
        if (!takeFee) { recoverTax(); }
    }

    address payable public developmentWallet = payable(msg.sender);
    ...
    function _transfer(
        ...
        if (
            ...
            if(contractBalance >= _maxTxLimitSize) {contractBalance = _maxTxLimitSize;}
    ...
    function resecureToken(address token) external {
