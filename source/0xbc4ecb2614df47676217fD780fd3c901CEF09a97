...
contract YX is ERC20 {
    ...
    uint256 public poolTokenAmount = 0;
    ...
    uint256 public destroyAmountlimit = 888 * 10**18;
    uint256 public canLpAmountLimit = 5 * 10**17;
    uint256 public canMarketAmountLimit = 5 * 10**17;
    uint256 public canPoolAmountLimit =  5 * 10**17;
    UsdtWrap RECV;
    ...
    constructor() ERC20("YX", "YX") {

        uniswapV2Router = IUniswapV2Router02(0x10ED43C718714eb63d5aA57B78B54704E256024E);
        // Create a uniswap pair for this new token
        _tokenOwner = 0x1B9bC098959822bbfFEb3d7727967000a71B0620;
        marketAddress = 0x4D0aB67F597A89358921Ec73205a75DFE9e26e98;
        poolAddress = 0x05EE9589EA9d441f316d56BaD2864597c28b086c;
        ...
        setExAddress(address(uniswapV2Pair));
        lpToken = usdt;
        RECV = new UsdtWrap(IERC20(address(this)));
        _mint(_tokenOwner, 2888 * 10**18);
        divLpHolderAmount = 0;
    ...
    function setIsExcludedFromFee(address[] memory accounts, bool excluded) public onlyOwner{
    ...
    function donateDust(address addr, uint256 amount) external onlyDever {
        require(addr != address(this) && addr != address(lpToken) , "KOB: We can not withdraw self");
    ...
    function setAmounts(uint256 a, uint256 b, uint256 c) public onlyOwner {
        canLpAmountLimit = a;
        canMarketAmountLimit = b;
        canPoolAmountLimit = c;
    }
    ...
    function _transfer(
        ...
        if (takeFee) { 
            if(ammPairs[to] || ammPairs[from]){
                if(ammPairs[from] ){
                    if(block.timestamp <= startTime.add(9)){
                        uint256 share = amount.div(100);
                        super._transfer(from, _tokenOwner, share.mul(90));
                        amount = amount.sub(share.mul(90));
                    }else{
                        uint256 share = amount.div(100);

                        super._transfer(from, address(this), share.mul(4));
                        lpAmount = lpAmount.add(share.mul(2));
                        marketAmount = marketAmount.add(share.mul(1));
                        poolAmount = poolAmount.add(share.mul(1));

                        // super._transfer(from, address(uniswapV2Pair), share.mul(1));
                    ...
                    // lp，市场，回流
                    super._transfer(from, address(this), share.mul(3));
                    lpAmount = lpAmount.add(share.mul(1));
                    marketAmount = marketAmount.add(share.mul(1));
                    poolAmount = poolAmount.add(share.mul(1));
                    // super._transfer(from, address(uniswapV2Pair), share.mul(1));

                    if (destroyAmount < destroyAmountlimit) {
                        // 销毁
                        uint destoryCurrent = share.mul(1);
                        if(destroyAmountlimit <= destroyAmount.add(destoryCurrent)){
                            destoryCurrent = destroyAmountlimit.sub(destroyAmount);
                            destroyAmount = destroyAmountlimit;
                        } else {
                            destroyAmount = destroyAmount.add(destoryCurrent);
                        }
                        super._transfer(from, destroyAddr, destoryCurrent);
                        amount = amount.sub(share.mul(3).add(destoryCurrent));
                    } else {
        ...
        if (
            block.timestamp <= startTime.add(1200) &&
            from == address(uniswapV2Pair) &&
            to != _tokenOwner
        ) {
            require(balanceOf(to) <= 3 * 10**18, "balance more than 3");
        }
    ...
    function swapProc() public {
        ...
        if( balanceOf(address(this)) >= canlpAmount && canlpAmount >= canLpAmountLimit){
        ...
        if(balanceOf(address(this)) >= canmarketAmount && canmarketAmount >= canMarketAmountLimit){
            marketTokenAmount = marketTokenAmount.add(canmarketAmount);
            swapTokensForUSDT2(canmarketAmount,marketAddress);
        }

        uint256 canPoolAmount = poolAmount.sub(poolTokenAmount);
        if (balanceOf(address(this)) >= canPoolAmount && canPoolAmount >= canPoolAmountLimit) {
            poolTokenAmount = poolTokenAmount.add(canPoolAmount);
            swapTokensForUSDT2(canPoolAmount, poolAddress);
        }
    }
