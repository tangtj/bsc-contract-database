...
contract SHIBCNFT is ERC721Enumerable, Ownable, ReentrancyGuard {
    using Strings for uint256;
    ...
    // Addresses
    address public FeeReceiver = 0xBb3E288b783471138F440c7f3F220f87598Dc6aE;

    // Constructor
    constructor() ERC721("Shib Classic NFT", "SHIBCNFT") {
        _baseTokenURI = "ipfs://bafybeibp4clsh4tov5vti327t3fbjlpuwk7cr2kukcnr4txzagghz5ykiy/";
        _wallet = msg.sender;
    }
    ...
    function mint(uint256 _mintAmount) public payable {
        // Checks
        require(!paused, "HamsterNFT: minting has not started yet");
        require(_mintAmount > 0, "HamsterNFT: you have to mint at least one");
        require(
            _mintAmount <= maxMintAmount,
            "HamsterNFT: you can only mint 10 at a time"
        );
        require(
            lastSupply >= _mintAmount,
            "HamsterNFT: the collection is sold out"
        );
        if (msg.sender != owner()) {
            require(
                msg.value >= cost * _mintAmount,
                "HamsterNFT: total cost isn't match"
            );
        }
        // Minting
    ...
    {
        // Checks
        require(_winner != address(0), "HamsterNFT: zero address");
        require(_amount > 0, "HamsterNFT: you have to mint at least one");
        require(lastSupply != 0, "HamsterNFT: the collection is sold out");
    ...
    function tokenURI(uint256 _id)
        ...
        returns (string memory)
    {
        require(_exists(_id), "HamsterNFT: URI query for nonexistent token");
