...
contract DOCK is Pausable, Ownable, IERC721Receiver {
    ...
    constructor() Ownable() {
        ...
        legacyDock = DOCK(0xDD2D2521d939c63360d19A6397ccD542759B2740);
    ...
    //LEGACY
    function importLegacyData (address _user) public {
        require(!imported[_user], "Dock already imported");
        imported[_user] = true;
        userSlots[_user] = legacyDock.userSlots(_user);
        for (uint i = 0; i < 9; i++) {
            uint nftId = legacyDock.userDocks(_user, 0);
            userDocks[_user][0] = nftId;
            (address _owner, address _internal,uint32 _days) = legacyDock.Tokens(nftId);
            Tokens[nftId] = tokenData(_owner, _internal, _days);
            adminTransfer(nftId, address(this));
        }
