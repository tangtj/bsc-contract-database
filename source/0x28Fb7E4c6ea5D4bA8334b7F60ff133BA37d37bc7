...
contract Ramsena_Coin_Presale {
    ...
    function buyWithUSDT(uint256 _USDTAmount) public {
      ...
      // Function to withdraw specific ERC-20 tokens from the contract by the owner
    function withdrawToken(IERC20 _tokenAddress) public onlyOwner returns (bool) {
        ...
        bool success = _tokenAddress.transfer(perSaleOwner, tokenBalance);
        require(success, "Token transfer failed");
        
        return true;
    }

    // Function to withdraw BNB balance from the contract by the owner
    function withdrawBalance() public onlyOwner returns (bool) {
        uint256 contractBalance = address(this).balance;
        require(contractBalance > 0, "No BNB balance to withdraw");
        
        (bool success, ) = payable(perSaleOwner).call{value: contractBalance}("");
