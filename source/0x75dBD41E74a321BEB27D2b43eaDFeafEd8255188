...
contract AsicTestToken is ERC20, ERC20Burnable, Ownable, AccessControl, ERC20Permit {
    ...
    // Owner
    function disableDumpProtection() external onlyManager onlyOwner {
        dumpProtectionEnabled = false;
        emit DisableDumpProtection();
    }

    function removeSniperTax() external onlyManager onlyOwner {
        sniperTaxEnabled = false;
        emit SniperTaxRemoved();
    }

        function addSniperTax() external onlyManager onlyOwner {
        sniperTaxEnabled = true;
        emit SniperTaxAdd();
    }

    function enableTrading() external onlyManager onlyOwner {
        require(!tradingEnabled);
        tradingEnabled = true;
    ...
    function triggerSwapBack() external onlyManager onlyOwner {
        _swapBack();
        emit TriggerSwapBack();
    }

    function recoverBNB(string memory walletName) external onlyManager onlyOwner {
    ...
    function recoverBEP20(IERC20 token, address recipient) external onlyManager onlyOwner {
        uint256 amount = token.balanceOf(address(this));
        token.safeTransfer(recipient, amount);
        emit RecoverBEP20(address(token), amount);
    }

    function setIsWhitelisted(address account, bool value) external onlyManager onlyOwner {
        isWhitelisted[account] = value;
        emit SetWhitelisted(account, value);
    }

    function setIsCEX(address account, bool value) external onlyManager onlyOwner {
        isCEX[account] = value;
        emit SetCEX(account, value);
    }

    function setIsMarketMaker(address account, bool value) external onlyManager onlyOwner {
    ...
    function setTaxes(uint32 newBuyTax, uint32 newSellTax, uint32 newTransferTax) external onlyManager onlyOwner {
    ...
    function setSwapBackSettings(bool status,uint96 amount) external onlyManager onlyOwner {
        autoSwap = status;
        swapThreshold = amount * 10**18;
        emit SetSwapBackSettings(swapThreshold);
    }

    function setTransferGas(uint32 newGas) external onlyManager onlyOwner {
        require(newGas >= 25000 && newGas <= 50000);
        emit UpdateGasForProcessing(newGas,transferGas);
        transferGas = newGas;
    }

    function setWallets(Wallet memory wallet) external onlyManager onlyOwner {
