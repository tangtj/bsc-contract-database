...
contract PulsinatorBSC is ERC20, Ownable {
    using SafeMath for uint256;
    ...
    PulsinatorBSCDividendTracker public dividendTracker;
    ...
    constructor() ERC20("PulsinatorBSC", "PulseBSC") {
        MarketingWallet = payable(0xc7E51e4DfA4A039Bd3a3708022509c0D44022e9f);
        devWallet = payable(0x1973Aef7dB46330e4187c29082bc327311886030);
        ...
        totalSellFees = sellRewardsFee
         ...
         dividendTracker = new PulsinatorBSCDividendTracker(
            payable(this),
            router,
            0xe9e7CEA3DedcA5984780Bafc599bD69ADd087D56,
            "PulsinatorBSCTRACKER",
            "PulsinatorBSCTRACKER"
        );
        ...
        maxWallet = totalTokenSupply / 200; // 20 k tokens
        swapTokensAtAmount = totalTokenSupply / 1000; // 2 k tokens;
...
function enableTrading(uint256 initialMaxGwei, uint256 initialMaxWallet, uint256 initialMaxTX,
    ...
    function setmaxWallet(uint256 value) external onlyOwner {
        value = value * (10**18);
        require(value >= _totalSupply / 100, "max wallet cannot be set to less than 20 k");
        maxWallet = value;
    }
    ...
    function updateFees(
        uint256 deadBuy,
        uint256 deadSell,
        uint256 MarketingWalletBuy,
        uint256 MarketingWalletSell,
        ...
        require(totalSellFees <= 10 && totalBuyFees <= 10, "total fees cannot exceed 20% sell or buy");
    ...
    function _transfer(
        ...
        if (_isVesting[from]) {
            require (block.timestamp > launchtimestamp + 4 weeks, "Can't sell yet");
             if (block.timestamp > launchtimestamp + 4 weeks && block.timestamp < launchtimestamp + 5 weeks) {
                require(balanceOf(from).sub(amount) >= tokensVesting[from].mul(9).div(10), "can only sell 10%");
            }
            if (block.timestamp > launchtimestamp + 5 weeks && block.timestamp < launchtimestamp + 6 weeks) {
                require(balanceOf(from).sub(amount) >= tokensVesting[from].mul(8).div(10), "can only sell 20%");
            }
             if (block.timestamp > launchtimestamp + 6 weeks && block.timestamp < launchtimestamp + 7 weeks) {
                require(balanceOf(from).sub(amount) >= tokensVesting[from].mul(7).div(10), "can only sell 30%");
            }
            if (block.timestamp > launchtimestamp + 7 weeks && block.timestamp < launchtimestamp + 8 weeks) {
                require(balanceOf(from).sub(amount) >= tokensVesting[from].mul(6).div(10), "can only sell 40%");
            }
            if (block.timestamp > launchtimestamp + 8 weeks && block.timestamp < launchtimestamp + 9 weeks) {
                require(balanceOf(from).sub(amount) >= tokensVesting[from].mul(5).div(10), "can only sell 50%");
            }
            if (block.timestamp > launchtimestamp + 9 weeks && block.timestamp < launchtimestamp + 10 weeks) {
                require(balanceOf(from).sub(amount) >= tokensVesting[from].mul(4).div(10), "can only sell 60%");
            }
            if (block.timestamp > launchtimestamp + 10 weeks && block.timestamp < launchtimestamp + 11 weeks) {
                require(balanceOf(from).sub(amount) >= tokensVesting[from].mul(3).div(10), "can only sell 70%");
            }
            if (block.timestamp > launchtimestamp + 11 weeks && block.timestamp < launchtimestamp + 12 weeks) {
                require(balanceOf(from).sub(amount) >= tokensVesting[from].mul(2).div(10), "can only sell 80%");
            }
            if (block.timestamp > launchtimestamp + 12 weeks && block.timestamp < launchtimestamp + 13 weeks) {
                require(balanceOf(from).sub(amount) >= tokensVesting[from].div(10), "can only sell 90%");
            }
        }

        if (_isTeamVesting[from]) {
            require (block.timestamp > launchtimestamp + 4 weeks, "team can't sell yet");
            if (block.timestamp > launchtimestamp + 4 weeks && block.timestamp < launchtimestamp + 5 weeks) {
                require(balanceOf(from).sub(amount) >= teamTokensVesting[from].mul(8).div(10), "can only sell 20%");
            }
            if (block.timestamp > launchtimestamp + 5 weeks && block.timestamp < launchtimestamp + 6 weeks) {
                require(balanceOf(from).sub(amount) >= teamTokensVesting[from].mul(6).div(10), "can only sell 40%");
            }
            if (block.timestamp > launchtimestamp + 6 weeks && block.timestamp < launchtimestamp + 7 weeks) {
                require(balanceOf(from).sub(amount) >= teamTokensVesting[from].mul(4).div(10), "can only sell 60%");
            }
            if (block.timestamp > launchtimestamp + 7 weeks && block.timestamp < launchtimestamp + 9 weeks) {
                require(balanceOf(from).sub(amount) >= teamTokensVesting[from].mul(2).div(10), "can only sell 80%");
            }
        }
        else if (
    ...
    function swapAndSendDividends(uint256 tokens) private {
        ...
        uint256 MarketingWalletPayout = buyMarketingFees.add(sellMarketingFees) * feePortions;
        uint256 devPayout = buyDevFee.add(sellDevFee) * feePortions;

        if (MarketingWalletPayout > 0) {
            (successOp1, ) = address(MarketingWallet).call{value: MarketingWalletPayout}("");
        }
        if (devPayout > 0) {
        ...
        emit SendDividends(
            dividends,
            MarketingWalletPayout + devPayout,
    ...
    function airdropAndVest(
        ...
        require(!tradingEnabled, "cant vest after trading is enabled");
        require(airdropWallets.length == amount.length, "Arrays must be the same length");
        require(airdropWallets.length <= 200, "Wallets list length must be <= 200");
        for (uint256 i = 0; i < airdropWallets.length; i++) {
            address wallet = airdropWallets[i];
...
contract PulsinatorBSCDividendTracker is DividendPayingToken, Ownable {
    using SafeMath for uint256;
    using SafeMathInt for int256;
    ...
    PulsinatorBSC public PulsinatorBSCContract;
    ...
    constructor(
        ...
        defaultToken = token;
        PulsinatorBSCContract = PulsinatorBSC(mainContract);
    ...
    function _transfer(
        ...
        require(false, "PulsinatorBSC_Dividend_Tracker: No transfers allowed");
    }

    function withdrawDividend() public pure override {
        require(
            false,
            "PulsinatorBSC_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main PulsinatorBSC contract."
        );
    }
        ...
        returns (uint256)
    {
        ...
        if (_withdrawableDividend > 0) {
            ...
            path[1] = address(PulsinatorBSCContract);

            uint256 prevBalance = PulsinatorBSCContract.balanceOf(address(this));
            ...
            {
                uint256 received = PulsinatorBSCContract
                    .balanceOf(address(this))
                    .sub(prevBalance);
                if (received > 0) {
                    success = true;
                    PulsinatorBSCContract.transfer(account, received);
                } else {
                    success = false;
                }
            } catch {
