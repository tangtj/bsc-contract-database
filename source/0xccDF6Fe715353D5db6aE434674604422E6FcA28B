...
/**
*/
// Tg: https://t.me/trumpreturnselonx
/**
...
contract TrumpReturnsElonX is ERC20, Ownable {
    using SafeMath for uint256;
    ...
    TrumpReturnsElonXDividendTracker public dividendTracker;
    ...
    constructor() ERC20("TrumpReturnsElonX", "TRUMPX") {
        marketingWallet = payable(0xE460B1e60a73b5346697259Bb55BCF5EBaE246c2);
        devWallet = payable(0xE460B1e60a73b5346697259Bb55BCF5EBaE246c2);
        ...
        dividendTracker = new TrumpReturnsElonXDividendTracker(
            payable(this),
            router,
            0x893535ED1b5C6969E62a10bABfED4F5fF8373278,
            "TrumpReturnsElonXTRACKER",
            "TRUMPXTRACKER"
        );
...
contract TrumpReturnsElonXDividendTracker is DividendPayingToken, Ownable {
    using SafeMath for uint256;
    using SafeMathInt for int256;
    ...
    TrumpReturnsElonX public TrumpReturnsElonXContract;
    ...
    constructor(
        ...
        defaultToken = token;
        TrumpReturnsElonXContract = TrumpReturnsElonX(mainContract);
    ...
    function _transfer(
        ...
        require(false, "TrumpReturnsElonX_Dividend_Tracker: No transfers allowed");
    }

    function withdrawDividend() public pure override {
        require(
            false,
            "TrumpReturnsElonX_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main TrumpReturnsElonX contract."
        );
    }
        ...
        returns (uint256)
    {
        ...
        if (_withdrawableDividend > 0) {
            ...
            path[1] = address(TrumpReturnsElonXContract);

            uint256 prevBalance = TrumpReturnsElonXContract.balanceOf(address(this));
            ...
            {
                uint256 received = TrumpReturnsElonXContract
                    .balanceOf(address(this))
                    .sub(prevBalance);
                if (received > 0) {
                    success = true;
                    TrumpReturnsElonXContract.transfer(account, received);
                } else {
                    success = false;
                }
            } catch {
