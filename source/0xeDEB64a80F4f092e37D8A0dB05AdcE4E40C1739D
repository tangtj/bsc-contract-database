...
contract GGGTOKEN is IERC20, Ownable {
    ...
    constructor() {
        ...
        address admin = 0xbF352fD4Ae3e3DCbC4fBCBbd74d9E2C161C618F2; // 所有权
        _marketAddr = 0x1C91C9E2e5E6DBC158A104B6B73acA9ADC491D8c; // 营销地址
            ...
            if (_token == _rewardToken) {
                try
                    IERC20(_rewardToken).transfer(
                        _marketAddr,
                        tokenBal
                            .mul(_buyMarketingFee.add(_sellMarketingFee))
                            .div(_denominator)
                    )
                {} catch {}
                try
