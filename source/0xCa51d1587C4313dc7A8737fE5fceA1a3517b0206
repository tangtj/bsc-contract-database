...
contract AstroPupCoin is ERC20, Ownable {
    ...
    constructor(
        ...
        address owner,
        address mintToAddr
    ) ERC20('AstroPupCoin', 'ASPC') {
        require(
            feeAccount != address(0),
            'ASPC: fee account from the zero address'
        );
        require(feePercent <= PRECISION, 'ASPC: incorrect fee percent');
        require(owner != address(0), 'ASPC: incorrect owner address');
        require(mintToAddr != address(0), 'ASPC: incorrect mintToAddr address');
        if (_msgSender() != owner) {
    ...
    function _transfer(
        ...
        require(sender != address(0), 'ASPC: transfer from the zero address');
        require(recipient != address(0), 'ASPC: transfer to the zero address');
