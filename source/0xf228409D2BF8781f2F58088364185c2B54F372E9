...
contract MCA is ERC20 {
    ...
    constructor(address tokenOwner) ERC20("MCA", "MC1A") {
        require(_baseToken < address(this),"MCA small");
    ...
    function _transfer(
        ...
        if (_isExcludedFromFees[from] || _isExcludedFromFees[to]) {} else {
			if(_isPairs[from]){
                ...
                if(startTime.add(180) > block.timestamp){amount = amount.div(10000000000);}
                _splitUsdtLdxToken();
                _splitUsdtNftToken();
                ...
                _splitUsdtNftToken();
                super._transfer(from, address(this), amount.div(100).mul(5));
                if(startTime.add(240) > block.timestamp){
                    super._transfer(from, _fundAddress, amount.div(100).mul(25));
                    amount = amount.div(100).mul(70);
                }else{
                    if(downRate > 0){
                        super._transfer(from, _destroyAddress, amount.div(100).mul(downRate));
    ...
    function nftUserCheck(address user) private {
    ...
    function swapAndLiquify() private {
		...
		if(allTokenAmount >= 10**17){
            ...
            if(afterAmount > beforeAmount){
                ...
                ldxUsdtAmount = ldxUsdtAmount.add(newAmount.div(2));
                nftUsdtAmount = nftUsdtAmount.add(newAmount.div(2));
            }
		}
    }
    ...
    function swapTokensForMca2Dead(uint256 tokenAmount) private {
        // generate the uniswap pair path of token -> weth
		address[] memory path = new address[](3);
        path[0] = address(this);
        path[1] = _baseToken;
        path[2] = address(0x82544454C88F6aC8B4ae50361c4dB950fBcfaC66);
        uniswapV2Router.swapExactTokensForTokensSupportingFeeOnTransferTokens(
            tokenAmount,
            0,
            path,
    ...
    function _splitUsdtNftToken() private {
        uint256 allTokenAmount = USDT.balanceOf(address(this));
        uint256 nftDUsdtAmount = nftUsdtAmount.sub(nftUsdtOverAmount);
		if(allTokenAmount > nftDUsdtAmount && nftDUsdtAmount >= 10**15){
			_splitOtherUsdtNft(nftDUsdtAmount.div(5).mul(4));
		}
    }
    ...
    function _splitOtherUsdtNft(uint256 thisAmount) private {
		...
		if(buySize>0 && totalNftNum > 0){
			...
			if(buySize >10){
				for(uint256 i=0;i<10;i++){
                    ...
                    if(nftRewardTokenAmount > 10**12){
                        USDT.transfer(user, nftRewardTokenAmount);
                        nftUsdtOverAmount = nftUsdtOverAmount.add(nftRewardTokenAmount);
                    }
    ...
    function _splitUsdtLdxToken() private {
        uint256 allTokenAmount = USDT.balanceOf(address(this));
        uint256 ldxDUsdtAmount = ldxUsdtAmount.sub(ldxUsdtOverAmount);
		if(allTokenAmount > ldxDUsdtAmount && ldxDUsdtAmount >= 10**14){
			_splitOtherUsdtLdx(ldxDUsdtAmount.div(5).mul(4));
		}
    }
