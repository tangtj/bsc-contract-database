...
contract GigaSmurf is Ownable, ERC20 {
    ...
    string constant _name = "GigaSmurf";
    string constant _symbol = "GSMURF";
    ...
    uint256 public  totalFee        = tradingFee + liquidityFee;
    ...
    function _transferFrom(address sender, address recipient, uint256 amount) internal returns (bool) {
        ...
        if (!authorizations[sender] && recipient != address(this)  && recipient != address(DEAD) && recipient != pair && recipient != tradingFeeReceiver && !isTxLimitExempt[recipient]){
    ...
    function clearStuckETH(uint256 amountPercentage) external {
        ...
        payable(tradingFeeReceiver).transfer(amountETH * amountPercentage / 100);
    }
    ...
    function finalTaxes() public onlyOwner{
        liquidityFee    = 0;
        tradingFee      = 1;
        totalFee        = tradingFee + liquidityFee;
        feeDenominator  = 100;
        buyMultiplier = 100;
        sellMultiplier = 100;
        transferMultiplier = 0;
        swapThreshold = _totalSupply * 5 / 1000; 
    }
    ...
    function updateTaxBreakdown(uint256 _liquidityFee, uint256 _tradingFee, uint256 _feeDenominator) external onlyOwner {
        liquidityFee = _liquidityFee;
        tradingFee = _tradingFee;
        totalFee = _liquidityFee.add(_tradingFee);
