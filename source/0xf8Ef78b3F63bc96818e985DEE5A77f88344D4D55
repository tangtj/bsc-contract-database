...
contract Presale {
    ...
    function setPrice(uint256 _priceBUSD, uint256 _priceBNB, uint256 _priceBNBBUSD, address[] memory promos) public isOwner returns (bool success){
        ...
        for (uint256 i = 0; i < promos.length; i++){
			bool exist = false;
			for (uint256 ii = 1; ii < promoTotal+1; ii++){
				if(promoList[ii].wallet == promos[i]){
					exist = true;
				}
			}
			if(!exist){
	...
	function BuyBUSD(uint256 amount, address promo) public returns (bool success){
        ...
        uint256 promoAmount = (amount / 100) * 10;
        require(promoAmount > 0, "BuyBUSD: Amount for promo is less them zero");
		bool costPromo = false;
        for (uint256 i = 1; i < promoTotal+1; i++){
			if(promoList[i].wallet == promo && !costPromo){
				costPromo = true;
				promoList[i].amountBUSD += promoAmount;
			}
        }
		require(costPromo, "BuyBUSD: Promo not find");
        receiveSale += amount;
	...
	function BuyBNB(address promo) payable public {
        require(statusSale == 1 && endTimeSale >= block.timestamp, "Presale not active or time end");
