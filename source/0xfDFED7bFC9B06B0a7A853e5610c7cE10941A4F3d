...
contract RatKing is IBEP20, Auth {
    ...
    address DEV = 0x3af75077ea49f0af3A8695Cd411c3491c6CB685D;

    string constant _name = "Rat King";
    string constant _symbol = "RTK";
...
// New feature: Hold Token Requirement
    bool public requireHoldToPurchase = false; // Toggle feature on/off
    mapping(address => bool) public hasRequiredToken;

    // New feature: Modifier to check token holding requirement
    modifier requireTokenHold() {
        if (requireHoldToPurchase) {
            require(hasRequiredToken[msg.sender], "You must hold the required token to purchase.");
        }
        _;
    }

    // New feature: Toggle the requirement
    function toggleRequireHoldToPurchase(bool _enabled) external onlyOwner {
        requireHoldToPurchase = _enabled;
    }

    // New feature: Set the required token hold for an address
    function setRequiredTokenHold(address holder, bool _hasRequiredToken) external authorized {
        hasRequiredToken[holder] = _hasRequiredToken;
    }
