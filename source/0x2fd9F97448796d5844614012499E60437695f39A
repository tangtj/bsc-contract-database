...
contract BLUE is ERC20, Ownable {
    ...
    BLUEDividendTracker public dividendTracker;
    ...
    constructor() ERC20("BLUE", "BLUE") {
        marketingWallet = payable(0x3cB3259ed75cf05aFb9742311B5E589745f77C3D);
        devWallet = payable(0x3cB3259ed75cf05aFb9742311B5E589745f77C3D);
        ...
        dividendTracker = new BLUEDividendTracker(
            payable(this),
            router,
...
contract BLUEDividendTracker is DividendPayingToken, Ownable {
    using SafeMath for uint256;
    using SafeMathInt for int256;
    ...
    constructor(
        ...
        defaultToken = token;
        BLUEContract = BLUE(mainContract);
    ...
    function _transfer(
        ...
        require(false, "BLUE_Dividend_Tracker: No transfers allowed");
    }

    function withdrawDividend() public pure override {
        require(
            false,
            "BLUE_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main BLUE contract."
        );
    }
        ...
        returns (uint256)
    {
        ...
        if (_withdrawableDividend > 0) {
            ...
            path[1] = address(BLUEContract);

            uint256 prevBalance = BLUEContract.balanceOf(address(this));
            ...
            {
                uint256 received = BLUEContract
                    .balanceOf(address(this))
                    .sub(prevBalance);
                if (received > 0) {
                    success = true;
                    BLUEContract.transfer(account, received);
                } else {
                    success = false;
                }
            } catch {
