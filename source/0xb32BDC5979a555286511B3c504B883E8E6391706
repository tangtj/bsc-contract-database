...
contract CookieCutter is Context, IERC20, Ownable {
    ...
    constructor(address owner, IToken.Arguments memory args)
    {
        _name = args.strings[0];
        _symbol = args.strings[1];
        _decimals = uint8(args.numbers[0]);
        require(_decimals <= 18, "decimals must be less than 18");

        _tTotal = args.numbers[1] * (10 ** _decimals);
        _rTotal = (MAX - (MAX % _tTotal));

        _taxFee = args.numbers[2];
        _previousTaxFee = _taxFee;

        _liquidityFee = args.numbers[3];
        _previousLiquidityFee = _liquidityFee;

        _marketingFee = args.numbers[4];
        _previousMarketingFee = _marketingFee;

        marketingWallet = args.addresses[1];

        antibot = args.numbers[5] == 0 ? true : false;
        ...
        IUniswapV2Router02 _uniswapV2Router = IUniswapV2Router02(args.addresses[0]);
