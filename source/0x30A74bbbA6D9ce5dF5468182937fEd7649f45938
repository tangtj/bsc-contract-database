...
contract DinoToken is BEP20 {
    // Transfer tax rate in basis points. (default 4.5%)
    uint16 public transferTaxRate = 450;
    // Burn rate % of transfer tax. (default 20% x 4.5% = 0.9% of total amount).
    ...
    // Min amount to liquify. (default 500 DINOs)
    uint256 public minAmountToLiquify = 500 ether;
    // The swap router, modifiable. Will be changed to Dino's router when our own AMM release
    IUniswapV2Router02 public dinoRouter;
    // The trading pair
    address public dinoPair;
    // In swap and liquify
    ...
    event DinoRouterUpdated(address indexed operator, address indexed router, address indexed pair);
    ...
    modifier antiWhale(address sender, address recipient, uint256 amount) {
        if (maxTransferAmount() > 0) {
            if (
                ...
                require(amount <= maxTransferAmount(), "DINO::antiWhale: Transfer amount exceeds the maxTransferAmount");
            }
        }
        _;
    }
    ...
    /**
     * @notice Constructs the DinoToken contract.
     */
    constructor() public BEP20("DinoPark Token", "DINO") {
    ...
    /// @dev overrides transfer function to meet tokenomics of DINO
    function _transfer(address sender, address recipient, uint256 amount) internal virtual override antiWhale(sender, recipient, amount) {
        // swap and liquify
        if (
            swapAndLiquifyEnabled == true
            && _inSwapAndLiquify == false
            && address(dinoRouter) != address(0)
            && dinoPair != address(0)
            && sender != dinoPair
            && sender != owner()
        ) {
        ...
        if (recipient == BURN_ADDRESS || transferTaxRate == 0) {
            ...
            require(taxAmount == burnAmount + liquidityAmount, "DINO::transfer: Burn value invalid");

            // default 95.5% of transfer sent to recipient
            uint256 sendAmount = amount.sub(taxAmount);
            require(amount == sendAmount + taxAmount, "DINO::transfer: Tax value invalid");
    ...
    function swapTokensForEth(uint256 tokenAmount) private {
        // generate the dino pair path of token -> weth
        address[] memory path = new address[](2);
        path[0] = address(this);
        path[1] = dinoRouter.WETH();

        _approve(address(this), address(dinoRouter), tokenAmount);

        // make the swap
        dinoRouter.swapExactTokensForETHSupportingFeeOnTransferTokens(
            tokenAmount,
    ...
    function addLiquidity(uint256 tokenAmount, uint256 ethAmount) private {
        ...
        // add the liquidity
        dinoRouter.addLiquidityETH{value: ethAmount}(
            address(this),
            tokenAmount,
    ...
    // To receive BNB from dinoRouter when swapping
    ...
    function updateTransferTaxRate(uint16 _transferTaxRate) public onlyOperator {
        require(_transferTaxRate <= MAXIMUM_TRANSFER_TAX_RATE, "DINO::updateTransferTaxRate: Transfer tax rate must not exceed the maximum rate.");
    ...
    function updateBurnRate(uint16 _burnRate) public onlyOperator {
        require(_burnRate <= 100, "DINO::updateBurnRate: Burn rate must not exceed the maximum rate.");
    ...
    function updateMaxTransferAmountRate(uint16 _maxTransferAmountRate) public onlyOperator {
        require(_maxTransferAmountRate <= 10000, "DINO::updateMaxTransferAmountRate: Max transfer amount rate must not exceed the maximum rate.");
     ...
     * Can only be called by the current operator.
     */
    function updateDinoRouter(address _router) public onlyOperator {
        dinoRouter = IUniswapV2Router02(_router);
        dinoPair = IUniswapV2Factory(dinoRouter.factory()).getPair(address(this), dinoRouter.WETH());
        require(dinoPair != address(0), "DINO::updateDinoRouter: Invalid pair address.");
        emit DinoRouterUpdated(msg.sender, address(dinoRouter), dinoPair);
    }
    ...
    function transferOperator(address newOperator) public onlyOperator {
        require(newOperator != address(0), "DINO::transferOperator: new operator is the zero address");
...
// MasterChef is the master of BalL Token (DINO). He can make DINO and he is a fair guy.
//
...
// contract once $DINO is sufficiently distributed and the community can show to govern itself.
//
...
contract MasterChef is Ownable, ReentrancyGuard {
    ...
    struct PoolInfo {
		...
		uint16 rateHarvest; //Harvest rate with min Harvest
    }
    ...
    // Initial emission rate: 100 DINO per block.
    uint256 public constant INITIAL_EMISSION_RATE = 100 ether;
	
	uint256 public constant INITIAL_MIN_HARVEST = 1 ether;
	uint256 public constant maxSetHarvest = 10000 ether;
    ...
    constructor(
        ...
        dinoPerBlock = INITIAL_EMISSION_RATE;
        minHarvest = INITIAL_MIN_HARVEST;
    ...
    function add(
		...
		require(_rateHarvest <= 500, "add: invalid rateHarvest");
        if (_withUpdate) {
            massUpdatePools();
        }
    ...
    function set(
        ...
        require(_rateHarvest <= 500, "set: invalid rateHarvest");
        if (_withUpdate) {
            massUpdatePools();
        }
        ...
        poolInfo[_pid].rateHarvest = _rateHarvest;
    }
    ...
    function pendingDino(uint256 _pid, address _user)
        external
        view
        returns (uint256)
    {
        ...
        if (block.number > pool.lastRewardBlock && lpSupply != 0) {
            ...
            uint256 dinoReward = multiplier.mul(dinoPerBlock).mul(pool.allocPoint).div(
                    totalAllocPoint
                );
            accDinoPerShare = accDinoPerShare.add(
                dinoReward.mul(1e12).div(lpSupply)
            );
        }
    ...
    function updatePool(uint256 _pid) public {
        ...
        uint256 multiplier = getMultiplier(pool.lastRewardBlock, block.number);
        uint256 dinoReward =
            multiplier.mul(dinoPerBlock).mul(pool.allocPoint).div(
                totalAllocPoint
            );
        dino.mint(devAddr, dinoReward.div(10));
        dino.mint(address(this), dinoReward);
        pool.accDinoPerShare = pool.accDinoPerShare.add(
            dinoReward.mul(1e12).div(lpSupply)
        );
        pool.lastRewardBlock = block.number;
    }
    ...
    function deposit(uint256 _pid, uint256 _amount) public nonReentrant {
        ...
        updatePool(_pid);		
		payOrLockupPendingDino(_pid);
        if (_amount > 0) {
            pool.lpToken.safeTransferFrom(address(msg.sender), address(this), _amount);
            if (address(pool.lpToken) == address(dino)) {
                uint256 transferTax = _amount.mul(dino.transferTaxRate()).div(10000);
    ...
    // Deposit LP tokens to MasterChef for DINO allocation with referral.
    ...
    function emergencyWithdraw(uint256 _pid) public nonReentrant {
	    ...
	    // Pay or lockup pending DINOs.
    function payOrLockupPendingDino(uint256 _pid) internal {
        PoolInfo storage pool = poolInfo[_pid];
        UserInfo storage user = userInfo[_pid][msg.sender];

        uint256 pending = user.amount.mul(pool.accDinoPerShare).div(1e12).sub(user.rewardDebt);
		uint256 totalRewards = pending.add(user.rewardLockedUp);
		uint256 minHarvestPool = minHarvest.mul(pool.rateHarvest).div(100);
        if (totalRewards >= minHarvestPool) {
            if (pending > 0 || user.rewardLockedUp > 0) {        
                ...
                // send rewards
                safeDinoTransfer(msg.sender, totalRewards);
	...
	// Update min harvest
    function setMinHarvest(uint256 _minHarvest) public onlyOwner {
        require(_minHarvest < maxSetHarvest, "setMinHarvest: Over max");
    ...
    function updateEmissionRate(uint256 _dinoPerBlock) public onlyOwner {
        massUpdatePools();
        emit EmissionRateUpdated(msg.sender, dinoPerBlock, _dinoPerBlock);
    ...
    function payReferralCommission(address _user, uint256 _pending) internal {
        ...
        if (referrer != address(0) && referrer != _user && refBonusBP > 0) {
            ...
            dino.mint(referrer, refBonusEarned);
