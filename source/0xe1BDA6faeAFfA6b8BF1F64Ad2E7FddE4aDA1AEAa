...
contract BTF is Context, IERC20, Ownable {
    ...
    uint256 private fundFee = 3;
    uint256 private _previousFundFee = fundFee;
    uint256 private curbPercent = 50;
    uint256 private initBlock;
    uint256 private releaseDays = 0;

    address public fundAddress;
    address public burnAddress = 0x000000000000000000000000000000000000dEaD;

    uint256 private bonusFeeTotal = 0;
    uint256 private fundFeeTotal = 0;

    address public uniswapV2Pair;


    uint256 private bured_amount = 0;
    ...
    constructor(address _poolAddress,address _basePoolAddress,address _airdropAddress, address _fundAddress) {
        fundAddress = _fundAddress;
        poolAddress = _poolAddress;
        basePoolAddress = _basePoolAddress;
        airdropAddress = _airdropAddress;
        _mint(poolAddress,8000000000*1e18);
        _mint(basePoolAddress,1000000000*1e18);
        _mint(airdropAddress,1000000000*1e18);
 
        _isExcludedFromFee[address(this)] = true;
    }

    function mint(address to, uint256 amount) public onlyOwner {
        require((amount*1e18+_totalSupply) <= 10000000000*1e18,"more than 10 000 000 000");
        _mint(to, amount*1e18);
    }

    function setSwapPair(address pairAddress) public onlyOwner {
...
function _transfer(
        ...
        if(!_isExcludedFromFee[sender] && !_isExcludedFromFee[recipient] && is_fee) {
            _balances[fundAddress] = _balances[fundAddress].add(tFund);
            fundFeeTotal = fundFeeTotal.add(tFund);
            _balances[burnAddress] = _balances[burnAddress].add(tBurn);
            bured_amount = bured_amount.add(tBurn);
