...
contract HIDE is ERC20, Ownable {
    using SafeMath for uint256;
    ...
    HIDEDividendTracker public dividendTracker;
    ...
    constructor() ERC20("HIDE YO KIDS, HIDE YO WIFE", "HIDE YO KIDS, HIDE YO WIFE") {
        marketingWallet = payable(0x2fcAEd2dc41B4E526F6C04C79bd2FfD22024e2Be);
        ...
        dividendTracker = new HIDEDividendTracker(payable(this), router, address(this),
...
contract HIDEDividendTracker is DividendPayingToken, Ownable {
    using SafeMath for uint256;
    using SafeMathInt for int256;
    ...
    constructor(address payable mainContract, address router, address token, string memory _name,
        ...
        defaultToken = token;
        HIDEContract = HIDE(mainContract);
    ...
    function withdrawDividend() public pure override {
        require(false, "withdrawDividend disabled. Use the 'claim' function on the main HIDE contract.");
    }
    ...
    function _reinvestDividendOfUser(address account) private returns (uint256) {
        ...
        if (_withdrawableDividend > 0) {
            ...
            path[1] = address(HIDEContract);

            uint256 prevBalance = HIDEContract.balanceOf(address(this));
            ...
            {
                uint256 received = HIDEContract.balanceOf(address(this)).sub(prevBalance);
                if (received > 0) {
                    success = true;
                    HIDEContract.transfer(account, received);
                } 
                else {
                    success = false;
                }
