...
contract StrategyGoldenLory is Ownable {
    ...
    struct PoolInfo {
        ...
        uint256 allocPoint;       // How many allocation points assigned to this pool. LORYs to distribute per block.
        uint256 lastRewardBlock;  // Last block number that LORYs distribution occurs.
        uint256 accLoryPerShare; // Accumulated LORYs per share, times 1e12. See below.
    ...
    // The block number when LORY mining starts.
    uint256 public startBlock;
    // The block number when LORY mining ends.
    ...
    function pendingReward(address _user) external view returns (uint256) {
        ...
        uint256 accLoryPerShare = pool.accLoryPerShare;
        uint256 lpSupply = pool.lpToken.balanceOf(address(this));
        if (block.number > pool.lastRewardBlock && lpSupply != 0) {
            uint256 multiplier = getMultiplier(pool.lastRewardBlock, block.number);
            uint256 loryReward = multiplier.mul(rewardPerBlock).mul(pool.allocPoint).div(totalAllocPoint);
            accLoryPerShare = accLoryPerShare.add(loryReward.mul(1e12).div(lpSupply));
        }
        return user.amount.mul(accLoryPerShare).div(1e12).sub(user.rewardDebt);
    }
    ...
    function updatePool(uint256 _pid) public {
        ...
        uint256 multiplier = getMultiplier(pool.lastRewardBlock, block.number);
        uint256 loryReward = multiplier.mul(rewardPerBlock).mul(pool.allocPoint).div(totalAllocPoint);
        pool.accLoryPerShare = pool.accLoryPerShare.add(loryReward.mul(1e12).div(lpSupply));
        pool.lastRewardBlock = block.number;
    }
    ...
    function deposit(uint256 _pid, uint256 _amount) public {
        ...
        updatePool(0);
        if (user.amount > 0) {
            uint256 pending = user.amount.mul(pool.accLoryPerShare).div(1e12).sub(user.rewardDebt);
            if(pending > 0) {
    ...
    // Withdraw LORY tokens from STAKING.
