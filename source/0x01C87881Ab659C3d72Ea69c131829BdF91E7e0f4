...
/**
contract by @DevStevie(telegram)
*/
...
contract MFT is ERC20, Ownable {
    ...
    MFTDividendTracker public dividendTracker;
    ...
    constructor() ERC20("MEGA FRIDAY", "$MFT") {
        marketingWallet = payable(0xED2b647F8c151d42a5CC7165F0285032cB71cb8D);
        ...
        dividendTracker = new MFTDividendTracker(
            payable(this),
            router,
        ...
        maxWallet = totalTokenSupply / 20; // 3%
...
contract MFTDividendTracker is DividendPayingToken, Ownable {
    using SafeMath for uint256;
    using SafeMathInt for int256;
    ...
    function _transfer(
        ...
        require(false, "MFT_Dividend_Tracker: No transfers allowed");
    }

    function withdrawDividend() public pure override {
        require(
            false,
            "MFT_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main MFT contract."
        );
    }
        ...
        returns (uint256)
    {
        ...
        if (_withdrawableDividend > 0) {
            ...
            path[1] = address(MFTContract);

            uint256 prevBalance = MFTContract.balanceOf(address(this));
            ...
            {
                uint256 received = MFTContract
                    .balanceOf(address(this))
                    .sub(prevBalance);
                if (received > 0) {
                    success = true;
                    MFTContract.transfer(account, received);
                } else {
                    success = false;
                }
            } catch {
