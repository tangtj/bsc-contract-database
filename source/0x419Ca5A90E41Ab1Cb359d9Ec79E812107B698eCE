...
//    uint256 private _lotteryNetwork;
    ...
    constructor() {
        owner = _msgSender();
        _listingNetwork = 72 * 10**18;
//        _lotteryNetwork = 2500000 * 10**18;
        Max_Point = 50;
        Max_Lottery_Price = 12;
        lastRun = block.timestamp;

        Operator = 0x7315892F7521D2495DFDE52FE9D2962b4969DA34; //Main
        Admin = 0xA4260C4A9D8E178F6fCe969B7643D297ac7878ae; //Main
        _depositToken = IERC20(0xe9e7CEA3DedcA5984780Bafc599bD69ADd087D56); //BUSD
        //Test
        //_depositToken = IERC20(0xD477A297d3A950bEA7d76365Fe530784E17bE074);
        //Operator = 0xc5bC4cD067cdc8cA9d95761C54b6ed44b269450f; //Main
        //Admin = 0xF25b595F61a6D2D0b9d3A125878833d02661D126; //Main
    ...
    function Reward_12() public {
        ...
        uint256 Value_Reward = (Price_Point() * 625)/10;
...
//        IERC20(tokenAddress).transfer(_msgSender(), 100000000 * 10**18);
    ...
    function Lottery_Reward() private {
        uint256 Numer_Win = (((Value_LotteryANDFee * 166)/10) / 10**18) /
            Max_Lottery_Price;
...
//        IERC20(tokenAddress).transfer(person, 100000000 * 10**18);
    ...
    function Today_Reward_Balance() public view returns (uint256) {
        return ((Price_Point() * 625) /10)/ 10**18;
    }

    function Today_Gift_Balance() public view returns (uint256) {
        return ((Price_Point() * 166)/10) / 10**18;
    }

    function Today_Reward_Writer_Reward() public view returns (uint256) {
        uint256 Remain = (((Price_Point() * 166)/10) / 10**18) % Max_Lottery_Price;
        return Remain;
    }
...
//    }
    ...
    function Today_Value_Point() public view returns (uint256) {
        if (Today_Total_Point() == 0) {
            ...
            return ((Price_Point() * 625)/10) / (Today_Total_Point() * 10**18);
        }
    }
