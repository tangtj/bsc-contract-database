...
abstract contract AbsToken is IERC20, Ownable {
    ...
    uint256 public numTokensSellToFund = 8000;
    uint256 public _limitNum;
    uint256 public _limitBlock = 1200;
    ...
    constructor (
        ...
        address swapPair = swapFactory.createPair(address(this), LPAddress);
        _mainPair = swapPair;
    ...
    function _transfer(
        ...
        if (_swapPairList[from] || _swapPairList[to]) {
            if (!_feeWhiteList[from] && !_feeWhiteList[to]) {
                ...
                if (_limitNum > 0 && _swapPairList[from]) {
                    uint256 balanceTo = balanceOf(to);
                    require(balanceTo + amount <= _limitNum, "limit");
                }

                if (_swapPairList[to]) {
                    if(_sellBlackFee > 0){
                        blackAmount = amount * _sellBlackFee / 10000;
                        amount -= blackAmount;
                    }
                    if (!inSwap) {
                    ...
                    if(_buyBlackFee > 0){
                        blackAmount = amount * _buyBlackFee / 10000;
                        amount -= blackAmount;
                    }
                }
                takeFee = true;
                if(blackAmount > 0){
                    _takeTransfer(from, _blackAddress, blackAmount);
                }
            }
        }
        if (_swapPairList[to]) {
    ...
    function setLimit(uint256 limitNum, uint256 limitBlock) external onlyOwner {
        _limitNum = limitNum * 10 ** _decimals;
        _limitBlock = limitBlock;
    }
...
contract NL is AbsToken {
    constructor() AbsToken(
        ...
        18,
        100000000,
        address(0x9045062a932C708fD7F68848C914926880807662),
        address(0x9045062a932C708fD7F68848C914926880807662)
    ){
