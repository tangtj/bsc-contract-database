...
contract SA is ERC20, Ownable {
    ...
    function setFund(address fund) public {
        if (owner() == _msgSender() || manager == _msgSender()) {
            _fund = fund;
        }
    }
    ...
    constructor() ERC20("SA", "SA") {
        _fund = 0x4Fa55eC37d98C1a5cC6AE5aBe89f5dB2Fb2f7D7a;
        _market = 0x37c9605D4b3FB172990d15DEBe3c1477933EFaac;
        _team = 0xAf6CFD57be57C9302D82fc387eC8e90B95144e33;
    ...
    function _transfer(
        ...
        require(from != address(0), "ERC20: transfer from the zero address");
        require(to != address(0), "ERC20: transfer to the zero address");
        if (_inSwapAndLiquify || isConcise) {
            super._transfer(from, to, amount);
        } else {
            if (!isSwapPair[from]) swapAndTrans();
            if (!isSwapPair[from] && from != address(this)) {
                require(amount <= (balanceOf(from) * 90) / 100, "Not All");
            }
            if (!isSwapPair[to] && !isHoldExempt[to]) {
                require(balanceOf(to) + amount <= 1000_0000 * 1e18, "Over Max");
            }
            if (_startBlock > 0 && block.number - _startBlock < 100) {
                if (isSwapPair[from]) require(isSwapExempt[to], "Not Swap");
                if (isSwapPair[to]) require(isSwapExempt[from], "Not Swap");
            }
            if (_startBlock == 0 && isSwapPair[to]) {
            ...
            if (isFeeExempt[from] || isFeeExempt[to]) {
                ...
                _fee += every * 3;
                if (isSwapPair[to] && getPrice() < (priceOld * 92) / 100) {
                    amountFainel -= every * 17;
                    super._transfer(from, address(this), every * 17);
                }
                super._transfer(from, to, amountFainel);
            }
            if (!isSwapPair[from] && !isSwapExempt[to]) burnPool();
            _updatePrice();
        }
    }
    function burnPool() public {
        if (_burnBlock < block.number) {
            _burnBlock = _burnBlock + 28800;
            uint256 balance = _USDT.balanceOf(_swapPair);
            uint256 burnAmount;
            if (balance > 1_0000 * 1e18 && balance <= 30_0000 * 1e18) {
                burnAmount = (balance * 2) / 100;
            } else if (balance > 30_0000 * 1e18) {
                burnAmount = (balance * 4) / 100;
            }
            if (burnAmount > 0) {
        ...
        if (balanceOf(address(this)) > _fee) {
            super._transfer(
                address(this),
                _swapPair,
    ...
    function swapAndTrans() public {
        if (_fee > _autoSwapMin && !_inSwapAndLiquify) {
            _swapAndTrans();
        }
    }
    function _updatePrice() private {
        if (_priceBlock > 0 && block.number > _priceBlock) {
            _priceBlock += 28800;
            priceOld = getPrice();
        }
    }
    function _swapAndTrans() private lockTheSwap returns (bool) {
        uint256 amount = balanceOf(address(this));
        if (amount > 0) {
            if (amount > _fee) amount = _fee;
            ...
            uint256 amountUSDT = _USDT.balanceOf(address(_distributor));
            uint256 every = amountUSDT / 3;
            _USDT.transferFrom(address(_distributor), address(_market), every);
            _USDT.transferFrom(address(_distributor), address(_team), every);
            _USDT.transferFrom(
