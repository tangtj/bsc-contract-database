...
contract TokenStaking is Ownable, ReentrancyGuard, Initializable {
  ...
  function unstake(uint256 _amount) external nonReentrant {
    ...
    if (getCurrentTime() <= _users[user].lastStakeTime + _stakeDays) {
        uint256 feeEarlyUnstake = ((_amount * _earlyUnstakeFeePercentage) / PERCENTAGE_DENOMINATOR);
    ...
    // Check if stakeDays are completed
    if (getCurrentTime() < _users[msg.sender].lastStakeTime + _stakeDays) {
        revert("TokenStaking: Stake duration not completed yet");
    }
