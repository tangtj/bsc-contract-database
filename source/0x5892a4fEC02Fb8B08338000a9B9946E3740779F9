...
contract CRN is CfoTakeableV2, IERC20Metadata {
    using Address for address;
    ...
    string private constant _name = "CRN-DEX Token";
    string private constant _symbol = "CRN";
    uint256 private constant _totalSupply = 12000000 * 1e18;
    ...
    ISwapFactory public crnSwapFactory;
    ...
    constructor(
        address _initHolder,
        address _crnSwapFactory,
        address _processor
    ){
        DOMAIN_SEPARATOR = keccak256(abi.encode(DOMAIN_TYPEHASH, keccak256(bytes(_name)), ChainId.get(), address(this)));

        require(_crnSwapFactory != address(0),"crn swap factory can not be address 0");
        crnSwapFactory = ISwapFactory(_crnSwapFactory);
        require(_processor != address(0),"processor can not be address 0");
        feeTo = _processor;
    ...
    function isSwapPair(address pair) public view returns(bool){
        ...
        return crnSwapFactory.getPair(address(this), usdt) == pair || crnSwapFactory.getPair(address(this), wbnb) == pair ||
    ...
    function setCrnFactory (address _crnSwapFactory) external onlyOwner {
        crnSwapFactory = ISwapFactory(_crnSwapFactory);
    }
