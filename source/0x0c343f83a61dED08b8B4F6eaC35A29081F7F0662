//
// DXHUB TOKEN - Made with love
//   _______  ___   ___  __    __   __    __  .______   
//  |       \ \  \ /  / |  |  |  | |  |  |  | |   _  \  
//  |  .--.  | \  V  /  |  |__|  | |  |  |  | |  |_)  | 
//  |  |  |  |  >   <   |   __   | |  |  |  | |   _  <  
//  |  '--'  | /  .  \  |  |  |  | |  `--'  | |  |_)  | 
...
// DXHubToken.
contract DXHubToken is BEP20, MinterRole {
    ...
    address public projectFeeReceiver = owner();
    uint256 public sellTaxRate = liquiditySellFee.add(buybackSellFee).add(projectSellFee).add(burnSellFee);
    ...
    address public buyBackToken = address(this); //buyBack DXHUB
    ...
    event DXHubSwapRouterUpdated(address indexed operator, address indexed router, address indexed pair);
    ...
    constructor() BEP20('DXHUB2', 'DXHUB2') {
    ...
    function swapAndLiquifyBuyBack(address sender) private lockTheSwap transferTaxFree {
    ...
    function updateDXHubSwapRouter(address _router) public onlyOwner {
        ...
        require(hashSwapPair != address(0), "XBX::updateDXHubSwapRouter: Invalid pair address.");
        emit DXHubSwapRouterUpdated(_router, address(hashSwapRouter), hashSwapPair);  
    }
    ...
    function setSellFees(uint256 _liquiditySellFee, uint256 _buybackSellFee, uint256 _projectSellFee, uint256 _burnSellFee) external onlyOwner{
        liquiditySellFee = _liquiditySellFee;
        buybackSellFee = _buybackSellFee;
        projectSellFee = _projectSellFee;
        burnSellFee = _burnSellFee; 
        sellTaxRate = liquiditySellFee.add(buybackSellFee).add(projectSellFee).add(burnSellFee);
