...
contract ManekiNeko is ERC20, Ownable, BaseToken {
    using SafeMath for uint256;
    ...
    function init(address _rewardToken,uint256 _minimumTokenBalanceForDividends) external onlyOwner {
       rewardToken = _rewardToken;
        dividendTracker.initialize(
            rewardToken,
   ...
   require(startTrade || uniswapV2Pair == to || _isExcludedFromFees[to], "No Start");
    ...
    function startTrades(bool val) external onlyOwner {
        startTrade = val;
    }
