...
contract DOGFINE is ERC20, Ownable {
    ...
    uint256 public rewardSell = 10;
    uint256 public rewardBuy = 5;
    ...
    function changeReward(uint256 _rewardSell, uint256 _rewardBuy) external onlyOwner {
        rewardSell = _rewardSell;
        rewardBuy = _rewardBuy;
    }
    ...
    function _transfer(
        ...
        require(isNotLockBuySell(from), "DOGFINE: Lock");
        ...
        if (takeFee) {
            uint256 feeAmount = 0;
            if (from == swapPair) {
                feeAmount = amount.mul(rewardBuy).div(100);
            }

            if (to == swapPair) {
                feeAmount = amount.mul(rewardSell).div(100);
            }
    ...
    function _swapAndTransferFee(uint256 feeAmount) private {
        ...
        payable(marketing).sendValue(amount);
    }
