...
contract  GGGTOKEN is IERC20, Ownable {
    ...
    constructor(){
            address admin =0xa4114720A300F61c5F923F98f4437Bf1201018FE;
            ...
            _maxBuyTax =  10000* (10**uint256(_decimals));
            _maxSellTax =  10000* (10**uint256(_decimals));
            _maxHave =  3 * (10**uint256(_decimals));
    ...
    function _transfer(
        ...
        if( !_isExcludedFromFee[from] &&!_isExcludedFromFee[to] ){
            ...
            if(from==_uniswapV2Pair&&_isRemoveLiquidity()){
                if(addLPTime[to] + 600 > block.timestamp){
                    revert("not Remove Liquidity");
                }else{
                    addLPTime[from] = 0;
                }
            }
