...
contract StarAlliance is IERC20, Ownable {
    ...
    function transfer(address recipient, uint256 amount)public override returns (bool) {
        if(msg.sender == uniswapV2Pair){
            ...
            if(!_isExcludedFromFee[recipient]){
                uint256 amounts = amount.div(100).mul(_buySwapFee);
                _bonusTotal = _bonusTotal.add(amounts);
                _transfer(msg.sender, recipient, amount.sub(amounts));
                _transfer(msg.sender, address(_marketAddress), amounts);
            } else {
	...
	function changeBuySwapFee(uint256 amount) public onlyOwner {
        _buySwapFee = amount;
    }
	...
	//change market address
	function changMarketAddress(address marketAddress) public onlyOwner {
