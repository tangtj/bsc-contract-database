...
contract PricePrediction {
    ...
    function checkResult(address _user) external view returns (string memory) {
        require(userBets[_user].length > 0, "No bet history found for the user");
        Bet memory lastBet = userBets[_user][userBets[_user].length - 1];

        uint256 betRound = (lastBet.initialPrice - startTime) / ROUND_DURATION;
        uint256 userBetEndTime = startTime + (betRound + 1) * ROUND_DURATION;
        if (block.timestamp < userBetEndTime) {
            return "Waiting for the betting time to end";
        }

        if (lastBet.result == Result.PENDING) {
            int256 currentPrice = getCurrentPrice();
            bool won = false;
            if (lastBet.prediction == Prediction.UP && uint256(currentPrice) > lastBet.initialPrice) {
                won = true;
        ...
        } else if (lastBet.result == Result.WON) {
            return "You won!";
        } else {
            return "You lost!";
        }
