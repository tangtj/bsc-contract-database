...
contract SMBPool is Ownable, ReentrancyGuard {
    ...
    event SetStaking(bool stakable);

    function setStakable() public onlyOwner
    {
        canStake = !canStake;
        emit SetStaking(canStake);
    } 
    ...
    {
        require(canStake, "You can not stake in the pool at this time");
...
contract SMBPoolFactory is Ownable {
    ...
    function createPool(
        ...
        SMBPool(pool).initialize(token, reward, rewardPerBlock, startBlock, endBlock, _poolLimitPerUser, _admin);
