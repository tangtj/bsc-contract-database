...
contract OxyO2CommunityAndTeam is Ownable {
    uint256 public constant amount = 80000e18;
 ...
 BEP20 public ox2;
    ...
    function getremainingmonths(address member) public view returns (uint) {
         require(members[member].lastyear != 0, "Member does not exist");
        uint256 timestamp = block.timestamp;
        uint256 month;
        uint256 currentyear = DateTime.getYear(timestamp);
        uint256 currentmonth = DateTime.getMonth(timestamp);
        uint256 _lastmonth = members[member].lastmonth;
        uint256 _lastyear = members[member].lastyear;
        uint256 totalyears = currentyear - _lastyear;
    ...
    function coreReward() public {
        ...
        uint value = members[member].value;
        uint timestamp = block.timestamp;
        uint256 month = getremainingmonths(member);
        require(month > 0 , "timestamp exeeds");   
            uint256 ox2balance = ox2.balanceOf(address(this));
            require(ox2balance > 0 , "balance exeeds");
            if (ox2balance < value*month) {
