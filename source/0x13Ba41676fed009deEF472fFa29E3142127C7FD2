...
interface ICrowdsale {
    ...
    // Max. amount in BNB
    function maxAmountBnb() external view returns (uint256);
    ...
    function setMaxAmountBnb(uint256) external;

    function buyTokenBNB(uint256) external payable;

    function getBusdForBnb(uint256) external view returns (uint256[] memory amounts);
...
contract Crowdsale is ICrowdsale, Ownable {
    using SafeMath for uint256;

    uint256 private immutable _price;
    uint256 private _maxAmountUsd;
    uint256 private _maxAmountBnb;
    ...
    constructor(
        ...
        _price = 17; // 0.017 BUSD
        _maxAmountUsd = 50000 ether;
        _maxAmountBnb = 200 ether;
    ...
    function maxAmountBnb() public view virtual override returns (uint256) {
        return _maxAmountBnb;
    }
    ...
    function wsiAddress() public view virtual override returns (address) {
        return _wsiAddress;
    }
    ...
    function setMaxAmountBnb(uint256 value) public virtual override onlyOwner {
        _maxAmountBnb = value;
    }
        ...
        override
    {
        ...
        uint256 amountWsi = amountBusd.mul(1000).div(price());
    ...
    function _buyTokenErc20(uint256 amountUsd, address tokenAddress) private {
        ...
        require(token.transferFrom(msg.sender, address(this), amountUsd), "Crowdsale: token transfer failed");

        uint256 amountWsi = amountUsd.mul(1000).div(price());
    ...
    function _execute(address beneficiary, uint256 amountWsi) private {
        address walletAddress = _getVestingWalletAddress(beneficiary);

        _soldToken = _soldToken.add(amountWsi);
        emit TokenTransferred(walletAddress, amountWsi);
        emit TokenSold(beneficiary, amountWsi);

        IERC20 wsi = IERC20(wsiAddress());
        require(wsi.transfer(walletAddress, amountWsi), "Crowdsale: WSI transfer failed");
    }
    ...
    {
        IERC20 token = IERC20(tokenAddress);
        require(token.transfer(msg.sender, token.balanceOf(address(this))), "Crowdsale: token transfer failed");
    }

    function withdrawBNB() public virtual override onlyOwner {
        address payable sender = payable(msg.sender);
        (bool sent, bytes memory data) = sender.call{value: address(this).balance}("");
        require(sent, "Crowdsale: Failed to send Ether");
    }

    function transferTokenOwnership(address tokenAddress) public virtual override onlyOwner {
        Ownable wsi = Ownable(tokenAddress);
        wsi.transferOwnership(msg.sender);
    }
...
contract VestingManager is IVestingManager, Ownable {
    ...
    function startTimestamp() public view override virtual returns (uint256) {
        return _startTimestamp;
    }

    function duration() public view override virtual returns (uint256) {
        return _duration;
    }

    function setStartTimestamp(uint256 value) public override virtual onlyOwner {
        require(
            value > block.timestamp,
    ...
    function setDuration(uint256 value) public override virtual onlyOwner {
        _duration = value;
    }
}