/**
   ...
   snyperfund.com 
    ...
    A sustainable investment platform based on Binance Smart Chain (BSC) smart-contract technology.
        ...
        ┌───────────────────────────────────────────────────────────────────────┐
        │                                                                       │
        │   Telegram Public Group: https://t.me/snyperfund                      |
        │   Telegram Announcements: https://t.me/snyperfund_Ann                 |
        |   Twitter: https://twitter.com/snyperfund                             |
        └───────────────────────────────────────────────────────────────────────┘

    ~LAUNCH: 25TH JULY 4PM UTC

    ~ PLAN:
     Base Roi : 3% Daily
     +1%  Compounding Bonus
     
	 Base 4% Roi from 1st August 4pm utc
   
    
    
    ~ PLAN REQUIREMENTS: 
    min. 10 USDT, max. of 20k USDT
    ...
    24H  withdraw cooldown.
	24H  compound cooldown.
...
contract SnyperFundUSDT {
	using SafeMath for uint256;

	IERC20 public erctoken;
	address token = 0x55d398326f99059fF775485246999027B3197955; //USDT Bep20
    

	/** default percentages **/
	uint256 public PROJECT_FEE = 60;
    uint256 public DEV_FEE = 20;
	...
	uint256 public CUTOFF_STEP = 240 * 60 * 60;  //10d
	uint256 public WITHDRAW_COOLDOWN = 24 * 60 * 60;  //24h
	uint256 public COMPOUND_COOLDOWN = 24 * 60 * 60;  //24h
    uint256 public REINVEST_BONUS = 10; //1% Bonus on compounding
	uint256 public MAX_WITHDRAW = 10000 ether;  //10k
	uint256 public WALLET_LIMIT = 20000 ether;  //20k

    /** deposits after this timestamp gets additional percentages **/
    uint256 public PERCENTAGE_BONUS_STARTTIME = 1690905600; // From AUG 1ST 4pm UTC
	uint256 public PERCENTAGE_BONUS_PLAN_1 = 10; // +1% from Above time stamp
    ...
    uint public startTime = 1690300800 ; //  Jul 25th 4pm UTC 
	...
	constructor(address payable _dev1, address payable _dev2, address payable _mkt) {require(!isContract(_dev1) && !isContract(_dev2) && !isContract(_mkt));
        ...
        plans.push(Plan(100, 30, 30 ether, 20000 ether, 0, 0, 0, 0, true));
	}
