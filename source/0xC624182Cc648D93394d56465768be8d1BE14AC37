...
contract Token {
    ...
    string public name = "Chavo";
    string public symbol = "CHAVO";
    ...
    event TokensBlocked(address indexed account);
    event TokensUnblocked(address indexed account);
    event TokensBurned(address indexed account, uint256 amount);
    event FeeBuyChanged(uint256 newFee);
    event FeeSellChanged(uint256 newFee);
    event Transfer(address indexed from, address indexed to, uint256 value);
    event Approval(
    ...
    constructor() {
        _owner = 0xE84E9942Ac8c586351051Cb1cbFa8155DfA5F741;
    ...
    function setFeeBuy(uint256 newFee) external onlyOwner {
        feeBuy = newFee;
        emit FeeBuyChanged(newFee);
    }

    function setFeeSell(uint256 newFee) external onlyOwner {
        feeSell = newFee;
        emit FeeSellChanged(newFee);
    }

    function blockAddress(address account) external onlyOwner {
        require(account != address(0), "Invalid address");
        require(!_blockedAddresses[account], "Address is already blocked");
        _blockedAddresses[account] = true;
        emit TokensBlocked(account);
    }

    function unblockAddress(address account) external onlyOwner {
        require(account != address(0), "Invalid address");
        require(_blockedAddresses[account], "Address is not blocked");
        _blockedAddresses[account] = false;
        emit TokensUnblocked(account);
    }
    ...
    function isAddressBlocked(address account) public view returns (bool) {
    ...
    function mint(uint256 _amount) public returns (bool success) {
        require(msg.sender == _owner, "Operation unauthorised");
