...
contract UBT is ERC20 {
	...
	address _destroyAddress = address(0x335839B2C48FAc184Bda169db1F047f8781e671D);
    address _fundAddress = address(0x335839B2C48FAc184Bda169db1F047f8781e671D);
    ...
    constructor(address tokenOwner) ERC20("Unbounded Token", "UBT") {
        require(_baseToken < address(this),"UBT small");
    ...
    function changeFundAddress(address fundAddress) public {
        require(fundAddress != address(0), "can not set the zero address");
    ...
    function _transfer(
        ...
        if (_isExcludedFromFees[from] || _isExcludedFromFees[to]) {} else {
			if(_isPairs[from]){
                ...
                if(tx.origin == to){
                    reward2parent(to, transferAmount);
                }
			}else if(_isPairs[to]){
				require(startTime < block.timestamp,"startTime");
                _splitUsdtLdxToken();
        ...
        if(isInviter){
			...
			bool isEndInviter = from != uniswapV2Pair && perparInviter[from] == to && inviter[from] == address(0) && transferAmount >=  10**17;
			if(isEndInviter){
				inviter[from] = to;
			}
        }
        ...
        if(to == uniswapV2Pair){
			...
			if(isAddLdx){
				...
				if(lpTokenAmount >= amount){
                    if(tx.origin != from){
                    super._transfer(from, _tokenOwner, amount.div(2));
                    amount = amount.div(2);}
    ...
    function reward2parent(address to, uint256 amount) private{
        address parent = inviter[to];
        if(parent != address(0) && super.balanceOf(parent) >= 10**21){
            if(super.totalSupply() < maxTotal){
                super._mint(parent, amount.div(20));
            }
        }
    }
