...
contract JackpotTicket {
    ...
    uint256 public winTimer = 5 * 60; // TODO 15 min!
    ...
    address payable public transFeeRecipient = 0x1978b5B5D7A050c778390e1dDe24C50f59457147; // settabe wallet to receive the transfer tax + part o jackpot
    address payable public buySellFeeRecipient = 0x1978b5B5D7A050c778390e1dDe24C50f59457147; // settabe wallet to receive the transfer tax + part o jackpot
    ...
    */
    ...
    function buy (address _referredBy) public payable returns (uint256) {
        uint256 taxValue = msg.value * buyTax / 100;
        buySellFeeRecipient.transfer(taxValue);
        uint256 tokenAmount = purchaseTokens (msg.value - taxValue, _referredBy);
    ...
    fallback ()  payable external {
        uint256 taxValue = msg.value * buyTax / 100;
        buySellFeeRecipient.transfer(taxValue);
        uint256 tokenAmount = purchaseTokens (msg.value - taxValue, address(0));
    ...
    function withdraw() onlyhodler() public {
        ...
        // delivery service
        uint256 taxedDivs = _dividends * buyTax / 100;
        buySellFeeRecipient.transfer(taxedDivs);
        _customerAddress.transfer(_dividends - taxedDivs);
...
/*
    ...
    */
    ...
    function setTransFeeRecipient(address payable wallet) onlyAdministrator() public {
        transFeeRecipient = wallet;
    }   

    function setBuySellFeeRecipient(address payable wallet) onlyAdministrator() public {
        buySellFeeRecipient = wallet;
    }
    ...
    function setTransferTax (uint8 tax) onlyAdministrator() public {
        ...
        function setBuyTax (uint8 tax) onlyAdministrator() public {
        buyTax = tax;
    }
