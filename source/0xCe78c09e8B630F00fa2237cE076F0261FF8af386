...
contract FoxyMon is  BEP721Enumerable,MinterRole {
    using SafeMath for uint256;
     using SafeBEP20 for IBEP20;
    using Counters for Counters.Counter;
    
    IBEP20 public FoxyToken = IBEP20(address(0));
    IBEP20 public LpToken = IBEP20(address(0));
   uint256 public adoptprice=500e9;
 ...
 function initialize() public initializer {
         BEP721.initialize("Foxy Monsters","Foxymon");
  ...
  function setBlock(uint256 _setblock) public onlyOwner {
...
function setLPtoken(address _LPTokenaddr) public onlyOwner {
...
function SetLocktime(uint256 _lockedTime) public onlyOwner {
...
function SetDailyreward(uint256 _allreward) public onlyOwner {
    
    dailyreward=_allreward;
    
}

function Setblockreward(uint256 _mintperblock) public onlyOwner {
    
    mintperblock=_mintperblock;
    
}

function SetBurnt(address _burnaddr) public onlyOwner {
...
string private _baseTokenURI="https://apimon.foxynft.org/pet.php?id=";
...
function _baseURI() internal view virtual override returns (string memory) {
 ...
 uint nonce=0;
 uint public image=6;
 function randomImage() internal returns (uint) {
...
event EWithdrawLP(address ownerlp,uint256 totallp);
event EaddLP(uint256 petlp,uint256 totallp);
...
function AdoptPet() public {
 uint256   forburn = adoptprice; 
 ...
 function addPetLP(uint256 _userlp,uint256 _petid) public  {
      require(_userlp > 0, "Please input your LP");
      require(_isApprovedOrOwner(_msgSender(), _petid), "BEP721: transfer caller is not owner nor approved");

LpToken.safeTransferFrom(msg.sender,address(this),_userlp);
uint256     PetLP=PetView[_petid].PetLP;
poolreward();
if(PetLP >0) {
 ClaimReward(_petid);   
}
PetView[_petid].PetLP=PetLP.add(_userlp);
PetView[_petid].LPLock=block.timestamp.add(lockedTime);

emit EaddLP(_petid,_userlp);
 
  } 

function poolreward() private {
    uint256 blockNumber = block.number;
    if(blockNumber > lastblock){
   uint256 diffblock= blockNumber.sub(lastblock) ;
   uint256 poolrewards=diffblock.mul(mintperblock).mul(1e9);
   
   IBEP20(FoxyToken).mint(address(this),poolrewards);
...
function CalcReward(uint _petid) public view returns(uint256){
    ...
    uint256 PetLPPower= PetView[_petid].PetLP;
    uint256 dayonsec=86400;
    uint256   Petreward;
    if(PetLPPower==0){
       Petreward=0;
    }else{
    uint256 percentPower=PetLPPower.mul(10000).div(totalLP);
    uint256 rewardpersec=percentPower.mul(dailyreward).mul(1e9).div(dayonsec).div(10000);
    uint256 Staketime=  block.timestamp.sub(PetView[_petid].LastStake);
     Petreward=rewardpersec.mul(Staketime);
    }
    
    return(Petreward);
    
}
function CalcHolder(uint _petid) public view returns(uint256){
    uint256 totalLP=LpToken.balanceOf(address(this));
    uint256 PetLPPower= PetView[_petid].PetLP;
     uint256 percentPower;
    
    if(PetLPPower==0){
     percentPower=0;
    }else{
          percentPower=PetLPPower.mul(10000).div(totalLP);
    }
...
function ClaimReward(uint _petid) public  {
       require(_isApprovedOrOwner(_msgSender(), _petid), "BEP721: transfer caller is not owner nor approved");
      
        poolreward();
      uint256 tokenReward= CalcReward(_petid);
      FoxyToken.transfer(msg.sender,tokenReward);
      PetView[_petid].LastStake=block.timestamp;
      
      emit EClaim(msg.sender,tokenReward);
      
    }
    
    function WithdrawLP(uint _petid) public  {
       require(_isApprovedOrOwner(_msgSender(), _petid), "BEP721: transfer caller is not owner nor approved");
       uint256 OwnerpetLP= PetView[_petid].PetLP;
       uint256 LPLocked=PetView[_petid].LPLock;
       require(OwnerpetLP > 0, "Your PET LP IS ZERO");
       require(block.timestamp > LPLocked, "Your PET LP IS Locked");
     poolreward();
     ClaimReward(_petid);   
    LpToken.transfer(msg.sender,OwnerpetLP);
     OwnerpetLP= PetView[_petid].PetLP=0;
      PetView[_petid].LastStake=block.timestamp;
      
      emit EWithdrawLP(msg.sender,OwnerpetLP);
      
    }
    
     function emergenyWithdrawLP(uint _petid) public  {
       require(_isApprovedOrOwner(_msgSender(), _petid), "BEP721: transfer caller is not owner nor approved");
       uint256 OwnerpetLP= PetView[_petid].PetLP;
       uint256 LPLocked=PetView[_petid].LPLock;
       require(OwnerpetLP > 0, "Your PET LP IS ZERO");
