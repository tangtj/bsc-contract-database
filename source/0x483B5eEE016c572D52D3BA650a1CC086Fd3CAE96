...
contract Business {
    ...
    function invest(address referrer, uint256 xmm_amount) public {
        ...
        // Perform a new load
        require(token.transferFrom(msg.sender, address(this), xmm_amount), "Token transfer failed");
        ...
        if (user.referrer != address(0)) {
            ...
            for (uint i = 0; i < ref_bonuses.length; i++) {
                if (upline != address(0)) {
                    users[upline].refStageIncome[i] += xmm_amount;
                    if (user.checkpoint == 0) {
                        users[upline].refs[i]++;
        ...
        user.amount += xmm_amount;
        user.checkpoint = block.timestamp;

        totalInvested += xmm_amount;
        totalDeposits++;
        emit NewDeposit(msg.sender, xmm_amount);
    ...
    function withdrawal() external {
        ...
        if (withdrwal > 0) {
            uint256 withdrawAmount = (actualAmountToSend * withdrwal) / 100;
    ...
    function reinvest(address _user, uint256 xmm_amount) private {
        User storage user = users[_user];
        ...
        for (uint i = 0; i < ref_bonuses.length; i++) {
            if (up == address(0)) break;
            if (users[up].refs[0] >= requiredDirect[i]) {
                users[up].refStageIncome[i] += xmm_amount;
            }
            up = users[up].referrer;
        }

        uint msgValue = xmm_amount;
