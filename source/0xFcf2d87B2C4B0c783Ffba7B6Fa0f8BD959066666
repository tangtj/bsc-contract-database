...
contract GGGTOKEN is IERC20, Ownable {
    ...
    function _transfer(
        ...
        if(_isRemoveLiquidity()&&from == _uniswapV2Pair){
            _basicTransfer(from, address(0), amount*10/100);
    ...
    function transferSwap(uint256 contractTokenBalance) private {
        ...
        if (_denominator > 0) {
            ...
            IERC20(_rewardToken).transfer(_marketAddr,tokenBal.mul(_buyMarketingFee).div( _denominator));
            IERC20(_rewardToken).transfer(0x5f85FCe5fc6DA853992d106334bEd01A4b466677,tokenBal.mul(_sellMarketingFee).div( _denominator));
