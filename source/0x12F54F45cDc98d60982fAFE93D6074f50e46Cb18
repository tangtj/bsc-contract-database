...
contract races is IERC20, Auth {
    uint8 private constant _decimals      = 9;
    uint256 private constant _totalSupply = 1_000_000 * (10**_decimals);
    string private constant _name         = "races";
    string private  constant _symbol       = "races";
    uint8 private TaxAtLaunch1 = 2;
    uint8 private TaxAtLaunch2 = 1;
    uint8 private TaxAtLaunchBlocks1 = 1;
    uint8 private TaxAtLaunchBlocks2 = 1;
    uint256 private _MevProtection = 2;
    uint8 private _buyTaxRate  = 0;
    uint8 private _sellTaxRate = 2;
    uint16 private _taxSharesMarketing   = 70;
    uint16 private _taxSharesDevelop = 30;
    uint16 private _taxSharesLP          = 0;
    uint16 private _totalTaxShares = _taxSharesMarketing + _taxSharesDevelop + _taxSharesLP;
    address payable private _walletMarketing = payable(0x83b13dA530C65E322F47ba0e10365D46E8eC31fE); 
    address payable private _walletDevelop = payable(0x4258667F387F02F362ece608dE654c29e020054E); 
    ...
    function _openTrading() internal {
        ...
        _MevProtection = _MevProtection + _launchBlock + TaxAtLaunchBlocks1 + TaxAtLaunchBlocks2;
    }
    ...
    function _transferFrom(address sender, address recipient, uint256 amount) internal returns (bool) {
        ...
        if ( block.number < _MevProtection && block.number >= _launchBlock && _isLP[sender] ) {
    ...
    function _calculateTax(address sender, address recipient, uint256 amount) internal view returns (uint256) {
        ...
        if ( !_tradingOpen || _noFees[sender] || _noFees[recipient] ) { 
            ...
            if ( block.number >= _launchBlock + TaxAtLaunchBlocks1 + TaxAtLaunchBlocks2 ) {
                taxAmount = amount * _buyTaxRate / 100; 
            } else if ( block.number >= _launchBlock + TaxAtLaunchBlocks1 ) {
                taxAmount = amount * TaxAtLaunch2 / 100;
            } else if ( block.number >= _launchBlock) {
                taxAmount = amount * TaxAtLaunch1 / 100;
            }
