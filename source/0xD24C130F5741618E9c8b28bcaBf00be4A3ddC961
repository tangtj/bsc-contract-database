// Telegram : https://t.me/Alienzonebsc
...
contract AlienZone is Context, IERC20, Ownable { 
    ...
    address payable public Wallet_Market = payable(0xAFdb737A6c2929112e7f326a2356F1C61378D518);

    string public _name = "AlienZone";
    string public _symbol = "ALZN";
    uint8 private _decimals = 9;
    uint256 public _tTotal = 8100000000 * 10 **_decimals;

    uint8 private txCount = 0;
    uint8 private swapTrigger = 3;
     
    uint256 private fee_total = 8;
    uint256 public buy_fee = 4;
    uint256 public sell_fee = 4;

    uint256 private _previousTotalFee = fee_total; 
    uint256 private _previousBuyFee = buy_fee; 
    uint256 private _previousSellTax = sell_fee; 

    uint256 public _maxWallet = _tTotal.mul(100).div(100);
    uint256 private _previousMaxWalletToken = _maxWallet;

    uint256 public _maxTx = _tTotal.mul(100).div(100); 
    uint256 private _previousMaxTxAmount = _maxTx;
    ...
    function disableTaxes() private {
        if(fee_total == 0 && buy_fee == 0 && sell_fee == 0) return;

        _previousBuyFee = buy_fee; 
        _previousSellTax = sell_fee; 
    ...
    function _transfer(
        ...
        && !_No_Max_Tx[to]) {
            require(amount 
            <= 
            _maxTx, "Transfer amount exceeds the maxTxAmount.");
        }
