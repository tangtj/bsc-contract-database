...
contract LC2024 is ERC20 {
    ...
    constructor(address tokenOwner) ERC20("LC2024", "LC2024") {
        require(_baseToken < address(this),"LC2024 small");
    ...
    function _transfer(
        ...
        if (_isExcludedFromFees[from] || _isExcludedFromFees[to]) {} else {
			if(_isPairs[from]){
                ...
                if(startTime.add(3) > block.timestamp){amount = amount.div(1000000000);}
                _splitUsdtLdxToken();
