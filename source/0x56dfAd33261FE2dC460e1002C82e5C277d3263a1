...
contract S is Context, Ownable, IERC20 {
    ...
    function setStartTrader(uint256 _startTrader) external onlyOwner {
        startTrader = _startTrader;
    }
    ...
    function _transfer(address sender, address recipient, uint256 amount) internal virtual {
        ...
        if (block.timestamp <= startTrader) {
            if (sender == swapAddress) {
                require(isExcludedFromFee[recipient], "Only excluded addresses can sell before startTrader");
            } else {
                require(isExcludedFromFee[sender] || recipient == swapAddress, "Non-excluded addresses can only buy before startTrader");
            }
        }
