/**
Dev Wallet:
cart tuition merit repair usage figure leisure purse derive demand ski image
...
abstract contract Ownable is Context {
    ...
    function transferOwnership(address newOwner) public virtual onlyOwner {
        require(newOwner != address(0), "Ownable: new owner $TPONC is the zero address");
        _setOwner(newOwner);
    }
...
contract ThePonziClassic is Context, IERC20, Ownable {
    ...
    address public marketingAddress = 0xa80D2654E0d70D129a0b50E6191BEF981b4e0D84;
    address public devAddress = 0xa80D2654E0d70D129a0b50E6191BEF981b4e0D84;

    string private constant _name = "THE PONZI CLASSIC";
    string private constant _symbol = "$TPONC";
    ...
    function transferFrom(address sender, address recipient, uint256 amount) public virtual override returns (bool) {
        ...
        require(currentAllowance >= amount, "ERC20: transfer $TPONC amount exceeds allowance");
    ...
    function excludeFromReward(address account) public onlyOwner() {
        require(!_isExcluded[account], "Account is already excluded $TPONC");
    ...
    function includeInReward(address account) external onlyOwner() {
        require(_isExcluded[account], "Account is not excluded $TPONC");
    ...
    function _approve(address owner, address spender, uint256 amount) private {
        require(owner != address(0), "ERC20: approve $TPONC from the zero address");
        require(spender != address(0), "ERC20: approve $TPONC to the zero address");
        _allowances[owner][spender] = amount;
        emit Approval(owner, spender, amount);
    }

    function _transfer(address from, address to, uint256 amount) private {
        require(from != address(0), "ERC20: transfer $TPONC from the zero address");
        require(to != address(0), "ERC20: transfer $TPONC to the zero address");
        require(amount > 0, "Transfer amount must be greater than zero $TPONC");
        require(amount <= balanceOf(from),"You are trying to transfer more than your balance $TPONC");
        require(!_isBot[from] && !_isBot[to], "You are a bot");
                
        if(!_isExcludedFromFee[from] && !_isExcludedFromFee[to] && !swapping){
            if(from == pair){
                require(amount <= maxBuyAmount, "You are exceeding maxBuyAmount $TPONC");
            }
            if(to == pair){
                require(amount <= maxSellAmount, "You are exceeding maxSellAmount $TPONC");
            }
            if(to != pair){
                require(balanceOf(to) + amount <= maxWalletBalance, "You are exceeding maxWalletBalance $TPONC");
            }
        }
    ...
    function rescueBNB(uint256 weiAmount) external onlyOwner{
        require(address(this).balance >= weiAmount, "insufficient BNB balance $TPONC");
    ...
    function rescueAnyBEP20Tokens(address _tokenAddr, address _to, uint _amount) public onlyOwner {
        require(_tokenAddr != address(this), "Cannot transfer out $TPONC!");
