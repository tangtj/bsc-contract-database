...
contract MagicStaking is Ownable {
    ...
    IERC20 public magic;
    uint256 public magicPerBlock = uint256(5 ether).div(10000); 
    ...
    function setMagicToken(IERC20 _magic) external onlyOwner {
        require(address(magic) == address(0), 'Token already set!');
        magic = _magic;
    ...
    function pendingRewards(uint256 pid, address _user) external view returns (uint256) {
        ...
        if (block.number > pool.lastRewardBlock && depositedAmount != 0) {
            ...
            accMagicPerShare = accMagicPerShare.add(magicReward.mul(1e12).div(depositedAmount));
        }
        return user.amount.mul(accMagicPerShare).div(1e12).sub(user.rewardDebt).add(user.pendingRewards);
    }
    ...
    function updatePool(uint256 pid) internal {
        ...
        uint256 multiplier = block.number.sub(pool.lastRewardBlock);
        uint256 magicReward = multiplier.mul(magicPerBlock).mul(pool.allocPoint).div(totalAllocPoint);
        pool.rewardsAmount = pool.rewardsAmount.add(magicReward);
        pool.accMagicPerShare = pool.accMagicPerShare.add(magicReward.mul(1e12).div(depositedAmount));
        pool.lastRewardBlock = block.number;
    }
    ...
    function deposit(uint256 pid, uint256 amount) external {
        ...
        if (amount > 0) {
            magic.safeTransferFrom(address(msg.sender), address(this), amount);
    ...
    function withdraw(uint256 pid, uint256 amount) external {
        ...
        if (amount > 0) {
            magic.safeTransfer(address(msg.sender), amount);
    ...
    function claim(uint256 pid) public {
        ...
        if (pending > 0 || user.pendingRewards > 0) {
            user.pendingRewards = user.pendingRewards.add(pending);
            uint256 claimedAmount = safeMagicTransfer(msg.sender, user.pendingRewards, pid);
    ...
    function safeMagicTransfer(address to, uint256 amount, uint256 pid) internal returns (uint256) {
        PoolInfo memory pool = poolInfo[pid];
        if (amount > pool.rewardsAmount) {
            magic.transfer(to, pool.rewardsAmount);
            return pool.rewardsAmount;
        } else {
            magic.transfer(to, amount);
            return amount;
        }
    }
    
    function setMagicPerBlock(uint256 _magicPerBlock) external onlyOwner {
        require(_magicPerBlock > 0, "MAGIC per block should be greater than 0!");
