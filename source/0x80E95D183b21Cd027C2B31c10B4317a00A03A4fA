...
contract Astrobirdz is IERC20, IERC20Metadata, Pausable, ReentrancyGuard {
    ...
    address private _marketingAddress =
        0x8B2837fcaFAdD80d24C159dD910C201F06eC45Ce; // Wallet Address for Marketing Tax
    ...
    function setTax(uint256 _newTeamTax, uint256 _newMarketingTax)
        public
        whenNotPaused
        onlyOwner
    {
    ...
    function setTeamAddress(address _newAddress)
        public
        whenNotPaused
        onlyOwner
    {
 ...
 function getMaxTX() public view returns (uint256) {
    ...
    function _finalizeTransfer(
        ...
        if (swapEnabled && !swapping && takeFee &&  !(automatedMarketMakerPairs[from] || automatedMarketMakerPairs[to])) {
            swapping = true;
    ...
    function takeTaxes(address from, uint256 amount)
        internal
        returns (uint256)
    {
        bool success;
        ...
        _balances[address(this)] = (_balances[address(this)].add(teamTaxFee))
            .add(marketingTaxFee);
