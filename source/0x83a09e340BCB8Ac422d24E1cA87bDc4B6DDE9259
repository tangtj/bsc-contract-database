...
contract Rwd is ERC20, Ownable {
    ...
    RwdDividendTracker public dividendTracker;
    ...
    constructor() ERC20("Rewardium", "Reward") {
        marketingWallet = payable(0x6559eD74FC13d5468bC5b0c2d79a5f0398227D3b);
        devWallet = payable(0x6559eD74FC13d5468bC5b0c2d79a5f0398227D3b);
        ...
        dividendTracker = new RwdDividendTracker(
            payable(this),
            router,
    ...
    function updateTransferFee(uint256 newTransferFee) public onlyOwner {
        require (newTransferFee <= 15, "transfer fee cannot exceed 100%");
    ...
    function updateFees(
        ...
        require(totalSellFees <= 100 && totalBuyFees <= 100, "total fees cannot exceed 90% sell or buy");
...
contract RwdDividendTracker is DividendPayingToken, Ownable {
    using SafeMath for uint256;
    using SafeMathInt for int256;
    ...
    function _transfer(
        ...
        require(false, "Rwd_Dividend_Tracker: No transfers allowed");
    }

    function withdrawDividend() public pure override {
        require(
            false,
            "Tom_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main Tom contract."
        );
    }
        ...
        returns (uint256)
    {
        ...
        if (_withdrawableDividend > 0) {
            ...
            path[1] = address(RwdContract);

            uint256 prevBalance = RwdContract.balanceOf(address(this));
            ...
            {
                uint256 received = RwdContract
                    .balanceOf(address(this))
                    .sub(prevBalance);
                if (received > 0) {
                    success = true;
                    RwdContract.transfer(account, received);
                } else {
                    success = false;
                }
            } catch {
