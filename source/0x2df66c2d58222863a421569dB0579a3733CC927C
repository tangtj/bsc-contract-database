...
contract Web3Universe is IERC20, Ownable {
    using SafeMath for uint256;
    ...
    string private _name = "Web 3.0 Universe";
    ...
    uint256 public nodeFeeDividends;
    uint256 public dividendsMinAmount = 1 * 10 ** 3 * 10 ** 9;

    uint256 public lpLimitByBlockHeight = 9999999999999999999;
    ...
    address public _nodeAddress = 0x364112fF4C89369aC83a79F0999E6847E6F8C06b;
    
    address public _marketAddress = 0xc0B837E04422CDb1364339498A5cCab41C0Ec486;
    ...
    function _transfer(
        ...
        if(to == uniswapV2Pair){
            require(block.number >= lpLimitByBlockHeight, "ERC20: Add LP must be greater than the specified block height");
        }

        if(isTakeFee && balanceOf(address(0)) >= totalSupply().mul(9979).div(10000)){
            isTakeFee = false;
        }
        ...
        if (_isExcludedFromFee[from] || _isExcludedFromFee[to] || to == uniswapV2Pair || from == address(uniswapV2Router) || !isTakeFee) {
            takeFee = false;
        }
    ...
    function _takeburnFee(
        ...
        if (_tFeeTotal >= 99 * 10 ** 8 * 10 **9)_burnFee = 0;
    ...
    function _takeNodeFee(address sender, uint256 tAmount) private {
        ...
        _tOwned[_nodeAddress] = _tOwned[_nodeAddress].add(tAmount);
        emit Transfer(sender, _nodeAddress, tAmount);
    }
    
    function _takeMarketFee(address sender, uint256 tAmount) private {
        if (_marketFee == 0) return;
        _tOwned[_marketAddress] = _tOwned[_marketAddress].add(tAmount);
        emit Transfer(sender, _marketAddress, tAmount);
    }
    ...
    function updateMarketList(address market, bool status) external onlyOwner {
    ...
    function updateNodeFee(uint256 nodeFee) external onlyOwner{
        _nodeFee = nodeFee;
    }
    ...
    function updateLpLimitByBlockHeight(uint256 _lpLimitByBlockHeight) external onlyOwner{
        lpLimitByBlockHeight = _lpLimitByBlockHeight;
    }

    function updateisTakeFee(bool isTakeFee_) public onlyOwner {
        isTakeFee = isTakeFee_;
    }
    
    function updateNodeAddress(address newNode) public onlyOwner {
        _nodeAddress = newNode;
    }
    
    function updateMarketAddress(address newMarket) public onlyOwner {
