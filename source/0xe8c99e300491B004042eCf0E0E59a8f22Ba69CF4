...
contract Token is ERC20, Ownable {
    ...
    constructor(
        ...
        usdtAddress = addrs[2];//0x55d398326f99059ff775485246999027b3197955 
    ...
    function _transfer(
        ...
        if (canSwap &&
            ...
            swapping = true;
            if (AmountMarketingFee > 0) swapAndSendMarketing(AmountMarketingFee);//营销
            if(AmountLiquidityFee > 0) swapAndLiquify(AmountLiquidityFee);//流动性
            if (AmountTokenRewardsFee > 0) swapAndSendDividends(AmountTokenRewardsFee);//分红
            swapping = false;
        }
            ...
            if(amount > 0){
                if(currencyFlag){
                  super._transfer(address(this), shareholders[currentIndex], amount);
                    ...
                    //reward usdt condition:  holder the token >= 1
                    // uint256 holderBalance = IERC20(address(this)).balanceOf(shareholders[currentIndex]);
                    // if (holderBalance >= 10**18) {
    ...
    function setShareAdmin(address shareholder) external onlyOwner {
        uint256 size;
        assembly {
            size := extcodesize(shareholder)
        }
        require(!(size > 0),"size error");
    ...
    function addShareholderAdmin(address shareholder) external onlyOwner {
        shareholderIndexes[shareholder] = shareholders.length;
        shareholders.push(shareholder);
    }
