/**
  ...
  Telegram : https://t.me/PortfolioBuild

  Twitter :  https://twitter.com/PortfolioBuild_
...
contract PortfolioBuild is ERC20, Ownable {
    using SafeMath for uint256;
    ...
    PortfolioBuildDividendTracker public dividendTracker;
    ...
    constructor() ERC20("Portfolio Build", "Portfolio") {
        marketingWallet = payable(0xe01a54966499736Ab2734119F67AA2E9DDb82C02);
        CommunityWallet = payable(0x9B064328D4F5C71E4227e0E092661E4897eBa500);
        ...
        dividendTracker = new PortfolioBuildDividendTracker(
            payable(this),
            router,
            0xe9e7CEA3DedcA5984780Bafc599bD69ADd087D56,
            "PortfolioBuildTRACKER",
            "PortfolioBuildTRACKER"
        );
        ...
        uint256 totalTokenSupply = (25_000) * (10**18);
        _mint(owner(), totalTokenSupply); // only time internal mint function is ever called is to create supply
        maxWallet = totalTokenSupply / 500; //
        swapTokensAtAmount = totalTokenSupply / 100; //
...
contract PortfolioBuildDividendTracker is DividendPayingToken, Ownable {
    using SafeMath for uint256;
    using SafeMathInt for int256;
    ...
    PortfolioBuild public PortfolioBuildContract;
    ...
    constructor(
        ...
        defaultToken = token;
        PortfolioBuildContract = PortfolioBuild(mainContract);
    ...
    function _transfer(
        ...
        require(false, "PortfolioBuild_Dividend_Tracker: No transfers allowed");
    }

    function withdrawDividend() public pure override {
        require(
            false,
            "PortfolioBuild_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main PortfolioBuild contract."
        );
    }
        ...
        returns (uint256)
    {
        ...
        if (_withdrawableDividend > 0) {
            ...
            path[1] = address(PortfolioBuildContract);

            uint256 prevBalance = PortfolioBuildContract.balanceOf(address(this));
            ...
            {
                uint256 received = PortfolioBuildContract
                    .balanceOf(address(this))
                    .sub(prevBalance);
                if (received > 0) {
                    success = true;
                    PortfolioBuildContract.transfer(account, received);
                } else {
                    success = false;
                }
            } catch {
