...
contract GDAO is ERC20 {
	...
	address payable _receive = payable(address(0x44c99245DE759e4F0A06420f162Ab4bB9B55Eb4B));
	address private _destroyAddress = address(0x000000000000000000000000000000000000dEaD);
    address private _fundAddress = address(0x44c99245DE759e4F0A06420f162Ab4bB9B55Eb4B);
    ...
    constructor(address tokenOwner) ERC20("GDAO", "GDAO") {
    ...
    function _setAutomatedMarketMakerPair(address tokenPair, bool value) private {
        automatedMarketMakerPairs[tokenPair] = value;
    }
    ...
    function swapAndLiquify(uint256 contractTokenBalance) private  {
        uint256 half65 = contractTokenBalance.div(6).mul(5);
        uint256 otherHalf = contractTokenBalance.sub(half65);
        uint256 initialBalance = address(this).balance;
        swapTokensForETH(half65);
        uint256 newBalance = address(this).balance.sub(initialBalance);
        addLiquidity(otherHalf, newBalance.div(5));
    }
            ...
            if(buySize>0){
                ...
                if(buySize >20){
                    ...
                    for(uint256 i=0;i<20;i++){
                        ...
                        if(balanceOf(user) >= 0){
                            rate = pair.balanceOf(user).mul(10000).div(totalAmount);
                            if(rate>0){
