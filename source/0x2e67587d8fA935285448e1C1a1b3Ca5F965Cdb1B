...
// File: contracts/SUPRA/interfaces/spore/IShareRewardPool.sol
...
// File: contracts/SUPRA/strategies/spore/StrategysporeLP.sol
 ...
 * spore rewards from providing liquidity are farmed every few minutes, sold and split 50/50.
 ...
 */
contract StrategysporeLP is Ownable, Pausable {
     ...
     * {spore} - Token generated by staking our funds. In this case it's the spores token.
    ...
    address public constant shareRewardPool =
        address(0x7336F7df82e6Fc62A98C3a73E8d3C86a9BA574A1);
     ...
     * {sporeToWbnbRoute} - Route we take to get from {spore} into {wbnb}.
     * {wbnbToSUPRARoute} - Route we take to get from {wbnb} into {SUPRA}.
     * {sporeToLp0Route} - Route we take to get from {spore} into {lpToken0}.
     * {sporeToLp1Route} - Route we take to get from {spore} into {lpToken1}.
     */
    address[] public sporeToWbnbRoute = [spore, wbnb];
    address[] public sporeToLp0Route;
    address[] public sporeToLp1Route;
    ...
    constructor(address _lpPair, address _vault) public {
        ...
        if (lpToken0 == wbnb) {
            sporeToLp0Route = [spore, wbnb];
        } else if (lpToken0 != spore) {
            sporeToLp0Route = [spore, wbnb, lpToken0];
        }

        if (lpToken1 == wbnb) {
            sporeToLp1Route = [spore, wbnb];
        } else if (lpToken1 != spore) {
            sporeToLp1Route = [spore, wbnb, lpToken1];
        }

        IERC20(lpPair).safeApprove(shareRewardPool, uint256(-1));
        IERC20(spore).safeApprove(unirouter, uint256(-1));
     ...
     * It deposits {lpPair} in the shareRewardPool to farm {spore}
     */
    function deposit() public whenNotPaused {
        uint256 pairBal = IERC20(lpPair).balanceOf(address(this));

        if (pairBal > 0) {
            IShareRewardPool(shareRewardPool).stake(
                pairBal,
                "0x66616c6f70616d75636861"
            );
        }
    }
    ...
    function withdraw(uint256 _amount) external {
        ...
        if (pairBal < _amount) {
            IShareRewardPool(shareRewardPool).unstake(
                _amount.sub(pairBal),
                "0x66616c6f70616d75636861"
            );
     ...
     * 3. It swaps the {spore} token for {lpToken0} & {lpToken1}
    ...
    function chargeFees() internal {
        ...
        uint256 toWbnb =
            IERC20(spore).balanceOf(address(this)).mul(40).div(1000);
        ...
        uint256 callFee = wbnbBal.mul(25).div(100);
        IERC20(wbnb).safeTransfer(msg.sender, callFee);

        uint256 treasuryHalf = wbnbBal.mul(75).div(100);
        IERC20(wbnb).safeTransfer(treasury, treasuryHalf);
    }

    /**
     * @dev Swaps {spore} for {lpToken0}, {lpToken1} & {wbnb} using PancakeSwap.
     */
    function addLiquidity() internal {
        uint256 sporeHalf = IERC20(spore).balanceOf(address(this)).div(2);
    ...
    function balanceOfPool() public view returns (uint256) {
        uint256 _amount =
            IShareRewardPool(shareRewardPool).totalStakedFor(address(this));
        return _amount;
    }
    ...
    function pause() public onlyOwner {
        ...
        IERC20(spore).safeApprove(unirouter, 0);
