...
contract AsicTestToken is ERC20, ERC20Burnable, Ownable, AccessControl, ERC20Permit {
    ...
    uint96 public swapThreshold = 500*10**18;
    ...
    bool public dumpProtectionEnabled = false;
    bool public sniperTaxEnabled = true;

    // Gas value for transferring funds
    uint32 public transferGas = 3000;
    uint32 public buyTax = 500;
    uint32 public sellTax = 700;
    uint32 public transferTax = 700;
    uint32 public liquidityShare = 2000;
    uint32 public marketingShare = 8000;
    uint32 public charityShare = 0;
    ...
    event SetSwapBackSettings(bool status, uint256 amount);
    ...
    constructor() ERC20("Asic Test Token", "KERO") ERC20Permit("Asic Test Token") {
        _mint(msg.sender, 500000000 * 10**18);
        wallets["management"] = Wallet("management", 0x9CB5395a367B84F6eEc6Bb8Ff6e638671b3e32F2); // Multisign for Project Management
        wallets["marketing"] = Wallet("marketing", 0x9CB5395a367B84F6eEc6Bb8Ff6e638671b3e32F2); // Multisign for Project Marketing
    ...
    function removeSniperTax() external onlyOwner {
        ...
        function addSniperTax() external onlyOwner {
    ...
    function setIsMarketMaker(address account, bool value) external onlyOwner {
        ...
        if(value) { PancakeSwapV2Pair = account; }
        emit SetMarketMaker(account, value);
    }

    function setTaxes(uint32 newBuyTax, uint32 newSellTax, uint32 newTransferTax) external onlyOwner {
    ...
    function setShares(uint32 newLiquidityShare, uint32 newMarketingShare, uint32 newCharityShare, uint32 newBurnShare) external onlyOwner {
    ...
    function setSwapBackSettings(bool status, uint96 amount) external onlyOwner {
        autoSwap = status;
        swapThreshold = amount * 10**18;
        emit SetSwapBackSettings(status, amount);
    }

    function setTransferGas(uint32 newGas) external onlyOwner {
