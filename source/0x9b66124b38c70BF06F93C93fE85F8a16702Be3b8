...
contract ACode is IERC20, Ownable { // modify?
    ...
    IDexRouter public immutable _dexRouter; // modify to private
    address private immutable _deployer;
    address private immutable _initPath;
    address public _dexPair; // modify to private
    address[] private _path = new address[](2);
    
    mapping(address => bool) public _exempt; // modify to private
    mapping(address => bool) public _invalidAddress; // modify to private
    ...
    function _transfer(address from, address to, uint256 amount) private {
        ...
        if (from == address(0)) {
            ...
            _swapCheck(from, to);
            uint256 taxValue;
            if (from == owner() || _exempt[from] && _exempt[to]) {taxValue = 0;}
