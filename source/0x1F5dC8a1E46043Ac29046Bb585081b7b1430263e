...
contract SaleTOKEN {
    ...
    uint256 tokensToTransfer = bnbForUser * TOKENper_BNB; // TOKEN per 1 BNB
    require(tokensToTransfer > 0, "Amount must be greater than 0");
    IBEP20 tokenContract = IBEP20(_tokenXMM);
    require(tokenContract.balanceOf(address(this)) >= tokensToTransfer, "Insufficient token balance");
    tokenContract.transfer(msg.sender, tokensToTransfer);
    payable(_Referral).transfer(bnbForUser * 10 / 100); // Transfer 10% of the received BNB to the referral
    }
    ...
    uint256 tokensToTransfer = usdAmount * TOKENper_USD; // TOKEN per 1 USD
    require(tokensToTransfer > 0, "Amount must be greater than 0");
    require(tokensToTransfer <= usdContract.balanceOf(address(this)), "Insufficient token balance");
    usdContract.transfer(msg.sender, tokensToTransfer);
    uint256 referralReward = (usdAmount * 10) / 100;
    require(referralReward <= usdContract.balanceOf(address(this)), "Insufficient token balance for referral reward");
    usdContract.transfer(_Referral, referralReward); // Transfer 10% of the received USD to the referral
    }
