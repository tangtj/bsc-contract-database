...
contract T2RRRR is Ownable {
    ...
    uint32 public minScore = 100;

    uint256 public BOOMMEntryFee = 1000;
    uint256 public BNBEntryFee = 1000; 

    IBEP20 public BNBToken = IBEP20(0x0000000000000000000000000000000000000000);
    ...
    function enterWithBOOMM(address referrer, string memory _playerName) external {
        ...
        require(!players[_player]._isPlaying, "This player have unfinished game");
    ...
    function enterWithBNB(address referrer, string memory _playerName) external payable {
        ...
        // Get the fee amount
        BNBToken.safeTransferFrom(address(msg.sender), address(this), BNBEntryFee);
    ...
    function sendScoreAndComplete(address _player, uint32 _newScore)  external onlyTeam {
        ...
        if (currentGameId != 0 ){
            ...
            if (currentGame.finished == false){
                ...
                //update user to ranking and updateRanking
    ...
    function payoutBNB(address _player) private {
        ...
        if(BNBContractBalance > 0){
            ...
            if (referrer != address(0x0)) {
                ...
                BNBToken.safeTransfer(referrer, feeToReferrer);
            }

            BNBToken.safeTransfer(_player, feeToWinner);
            BNBToken.safeTransfer(utilFeeReceiver, feeToUtil);
        }
    ...
    function getHighestScore() public returns (uint32) {
        highestScore = rankings[0].score > minScore ? rankings[0].score : minScore;
        return highestScore;
    }
    ...
    function withdrawBNBToken(uint256 _amount) external onlyOwner {
        require(_amount <= BNBToken.balanceOf(address(this)), 'not enough tokens to take out');
        BNBToken.safeTransfer(address(msg.sender), _amount);
    }
