...
contract BFCToken is ERC20, ReentrancyGuard, Ownable {
    ...
    address public mintAddress =
        address(0x62C4779Eb0E01ebe0acF022E79aDf1eeC3d6935a); 
    ...
    function mint(address _to, uint256 _amount) external onlyOwner{
        require(MAX_MINT != totalMint,"Mint already Max");
        uint256 currentCanMint = MAX_MINT - totalMint;
        if(_amount > currentCanMint){
            _mint(_to,currentCanMint);
            totalMint = MAX_MINT;
        }else{
            _mint(_to,_amount);
