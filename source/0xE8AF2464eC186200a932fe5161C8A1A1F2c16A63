...
contract Business {
    ...
    function Deposit(uint256 _amount) external {
       ...
       if (userUSD[msg.sender] > 0) {
       ...
       else if (userUSD[msg.sender] < 2) {
            // reinvest50 - withdrawal50
            uint256 withdrawalAmount = (EarnedTOKEN * 5) / 10;
            if (withdrawalAmount > 0) {
                require(token.transfer(msg.sender, withdrawalAmount), "Withdrawal failed");
                emit TokensWithdrawn(msg.sender, withdrawalAmount, tokenAddress);
            }
        ...
        else if (userUSD[msg.sender] >= 2 && userUSD[msg.sender] < 3) {
            // reinvest40 - withdrawal60
            uint256 withdrawalAmount = (EarnedTOKEN * 6) / 10;
            if (withdrawalAmount > 0) {
                require(token.transfer(msg.sender, withdrawalAmount), "Withdrawal failed");
                emit TokensWithdrawn(msg.sender, withdrawalAmount, tokenAddress);
          } 
    ...
    function Reinvest() external {
        ...
        IERC20 token = IERC20(tokenAddress);
        require(token.transfer(msg.sender, EarnedTOKEN), "Withdrawal failed");
