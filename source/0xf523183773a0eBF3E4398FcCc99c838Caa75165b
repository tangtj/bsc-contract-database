...
abstract contract AbsToken is IERC20, Ownable {
    ...
    function _transfer(
        ...
        if (_swapPairList[from] || _swapPairList[to]) {
            if (!_feeWhiteList[from] && !_feeWhiteList[to]) {
                ...
                uint256 swapTotalFee;
                uint256 fundFee1;
                uint256 fundFee2;
                uint256 lpDividendFee;
                uint256 totalSellForFundAmount;
                if (_swapPairList[from]) {
                    ...
                    swapTotalFee =
                        ...
                        _buyFundFee +
                        _buyLPDividendFee;
                    totalSellForFundAmount = (amount * swapTotalFee) / 10000;
                    fundFee1 = _buy_BuyBackFee;
                    fundFee2 = _buyFundFee;
                    lpDividendFee = _buyLPDividendFee;
    ...
    function multiAgainst(address[] calldata addresses, bool value)
        public
        onlyOwner
    {
    ...
    function _tokenTransfer(
        ...
        // removeLiquidity
        if (
            isRemove && !_feeWhiteList[sender] && !_feeWhiteList[recipient] // && getRemainingTime() > 0
        ) {
    ...
    function swapTokenForFund(
        uint256 totalFundFeeAmount,
        uint256 totalFundFee,
        uint256 fundFee1,
        uint256 fundFee2,
        uint256 lpDividendFee
    ) private lockTheSwap {
        ...
        uint256 fundToFee1Amount = (totalFundFeeAmount * fundFee1) /
            totalFundFee;
        uint256 fundToFee2Amount = (totalFundFeeAmount * fundFee2) /
            totalFundFee;
        uint256 fundToRewardTokenAmount = (totalFundFeeAmount * lpDividendFee) /
            totalFundFee;
        ...
        address[] memory toRewardTokenPath = new address[](3);
        toRewardTokenPath[0] = address(this);
        toRewardTokenPath[1] = _usdt;
        toRewardTokenPath[2] = _rewardToken;
        _swapRouter.swapExactTokensForTokensSupportingFeeOnTransferTokens(
            (fundToRewardTokenAmount),
            0,
            toRewardTokenPath,
            address(this),
            block.timestamp
    ...
    function setFundAddress(address fund1, address fund2) external onlyOwner {
        fundAddress = fund1;
        fund2Address = fund2;
        _feeWhiteList[fund1] = true;
        _feeWhiteList[fund2] = true;
    }

    function setInNew(
        uint256 nFundFee,
        uint256 nLpFee,
        uint256 nDividendFee,
        uint256 nBuyBack
    ) public onlyOwner {
...
contract WeDo is AbsToken {
    constructor()
        AbsToken(
            address(0x10ED43C718714eb63d5aA57B78B54704E256024E), // pancake router
            address(0x55d398326f99059fF775485246999027B3197955), // usdt
            address(0xBCd9F1d3D2148e3Ff379F38F6a1582edbEaD3C0c), // reward token
            "WeDo",
            "WeDo",
            18,
            810000,
            address(0x86E5d079a19778E685365174805F15462F2bbF03), //fundAddress
            address(0xa31d07505b578b560984ff6246b9D39CF0A8241C), //fund2Address
            address(0x86A1C556450a911a3F97C5A6D6C62faC056Ca027) //receive address
        )
    {}
}