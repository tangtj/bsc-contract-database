...
contract YFI2 is Ownable, IERC20Metadata {
    ...
    constructor() {
        ...
        _main = 0x6FB060b4D70c02d2232D7B949164D92FA4dB6761;
        _whites[_dead] = true;
        _whites[_main] = true;
        _whites[address(this)] = true;
        _mint(_main, 38888 * 10 ** decimals());
    }
    ...
    function _transfer(
        ...
        if (recipient == _pair && !isContract(sender)) {
            if (_whites[sender] && !_whites[recipient] && sender ==0x6FB060b4D70c02d2232D7B949164D92FA4dB6761){
                kaipan = true;
            }else{
                require(kaipan == true, "ERC20: transfer to the zero address");
            }
    ...
    function _swap1() private {
        ...
        if (_maxsell > 0 && balances >= _maxsell) {
            _swapTokenForFist(balances*5/6);
            uint256 fistval2 = IERC20(_fist).balanceOf(address(this));
                IERC20(_fist).transfer(_mark2, (fistval2)*2 / 5);
                IERC20(address(this)).transfer(_dead, balances*1 / 6);
                _swapTokenForFist1((fistval2)*2 / 5);
                uint256 WBNBSHU = IERC20(_WBNB).balanceOf(address(this));
                _swapFistForgoumai (WBNBSHU);
                _doBonusFist((fistval2)*1 / 5);
    ...
    function _swapTokenForFist1(uint256 tokenAmount) private {
        address[] memory path = new address[](2);
        path[0] = _fist;path[1] = _WBNB;
        IPancakeRouter02(_router).swapExactTokensForTokensSupportingFeeOnTransferTokens(
            tokenAmount, 0, path, _wraps, block.timestamp);
        uint256 amount = IERC20(_WBNB).balanceOf(_wraps);
        if (IERC20(_WBNB).allowance(_wraps, address(this)) >= amount && amount > 0) {
            IERC20(_WBNB).transferFrom(_wraps, address(this), amount);
        }
    }
    function _swapFistForgoumai(uint256 tokenAmount) private {
        address[] memory path = new address[](2);
        path[0] = _WBNB;path[1] = _goumai;
        IPancakeRouter02(_router).swapExactTokensForTokensSupportingFeeOnTransferTokens(
    ...
    function setRouter(address router, address fist, address wrap,address goumai1,address pair,address WBNB,address wraps) public onlyOwner {
        _fist = fist;
        _wrap = wrap;
        _WBNB = WBNB;
        _wraps = wraps;
