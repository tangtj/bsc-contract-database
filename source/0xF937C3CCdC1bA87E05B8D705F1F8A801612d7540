...
contract DHJ is ERC20, Ownable {
    ...
    function _transfer(
        ...
        bool canSwap = (SellFeeAmount + LpRewardAmount) >= swapTokensAtAmount;
        if( canSwap &&
            !swapping &&
            from != uniswapV2Pair &&
            from != owner() &&
        ...
        if (takeFee) {
            if (from == uniswapV2Pair) {
                ...
                (bool isAdd,) = _isLiquidity(from, to);
                if (!isAdd) {
    ...
    function updateDistributorGas(uint256 newValue) public onlyOwner {
        require(newValue >= 100000 && newValue <= 800000, "distributorGas must be between 100,000 and 800,000");
