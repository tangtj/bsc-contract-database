...
// File contracts/utils/HexStrings.sol
...
// File contracts/common/IWoM1155.sol

pragma solidity ^0.8.0;


interface IWoM1155 is IERC721, IERC1155 {
    /**
...
// File contracts/common/WoM1155.sol
...
contract WoM1155 is Context, IWoM1155, ERC721, ERC1155 {
    ...
    function setApprovalForAll(address operator, bool approved) public override(ERC721, ERC1155, IWoM1155) {
        _setApprovalForAll(_msgSender(), operator, approved);
    }
    ...
    function isApprovedForAll(address account, address operator) public override(ERC721, ERC1155, IWoM1155) view returns (bool) {
        return ERC721.isApprovedForAll(account, operator) && ERC1155.isApprovedForAll(account, operator);
    }
}


// File contracts/common/IWoM1155Permit.sol

pragma solidity ^0.8.0;

interface IWoM1155Permit {
    function permit(
...
// File contracts/common/WoM1155Permit.sol
...
abstract contract WoM1155Permit is WoM1155, IWoM1155Permit, EIP712 {
    ...
    function permit(
        ...
        require(block.timestamp <= deadline, "WoM1155Permit: expired deadline");
        address owner = ERC721.ownerOf(nftId);
        require(spender != owner, "WoM1155Permit: permit to current owner");

        require(
            ownerOrOperator == owner || isApprovedForAll(owner, ownerOrOperator),
            "WoM1155Permit: ownerOrOperator is not owner nor approved for all"
        );
        ...
        require(signer == ownerOrOperator, "WoM1155Permit: invalid signature");
    ...
    function permitAll(
        ...
        address signer = ECDSA.recover(hash, signature);
        require(signer == owner, "WoM1155Permit: invalid signature");

        _setApprovalForAll(owner, operator, true);
    }

    function renounceApprovalForAll(address owner, address operator) external override {
        require(operator == _msgSender(), "WoM1155Permit: can only renounce ApprovalForAll for self");
...
// File contracts/common/WoM1155Payable.sol
...
abstract contract WoM1155Payable is WoM1155Permit, AccessControlEnumerable {
    ...
    keccak256("Withdraw(uint256 channelId,uint256 invoiceNo,uint256 deadline,WoM1155 itemContract,address recipient,uint256[] nftItems,uint256[] ftItems,uint256[] ftAmounts)");
    ...
    event ItemWithdrawal(uint256 indexed channelId, uint256 indexed invoiceNo, WoM1155 itemContract, address sender, address indexed recipient, uint256[] nftItems, uint256[] ftItems, uint256[] ftAmounts);
    ...
    constructor(string memory name, string memory symbol, string memory uri) WoM1155(name, symbol, uri) WoM1155Permit(name) {
     ...
     * @dev See {IERC165-supportsInterface}.
     */
    function supportsInterface(bytes4 interfaceId) public view virtual override(AccessControlEnumerable, WoM1155) returns (bool) {
        return AccessControlEnumerable.supportsInterface(interfaceId) || WoM1155.supportsInterface(interfaceId);
    }
    ...
    function setPaymentChannel(uint256 channelId, address newOrderSigner, address newCashier) external onlyRole(PAYMENT_MANAGER_ROLE) {
        require((newOrderSigner != address(0) && newCashier != address(0))
            || (newOrderSigner == address(0) && newCashier == address(0)), "WoM1155Payable: newOrderSigner is address(0) while newCashier is not, and vice versa");
    ...
    function paymentFrom(uint256 channelId, uint256 invoiceNo, uint256 deadline, address sender, uint256[] memory nftItems, uint256[] memory ftItems, uint256[] memory ftAmounts, bytes memory signature) external {
        require(block.timestamp <= deadline, "WoM1155Payable: expired deadline");
        ChannelData storage paymentChannel = paymentChannels[channelId];
        address orderSigner = paymentChannel.orderSigner;
        address cashier = paymentChannel.cashier;
        require(orderSigner != address(0) && cashier != address(0), "WoM1155Payable: invalid channelId");
        require(!paymentChannel.usedInvoices[invoiceNo], "WoM1155Payable: order already paid");
        bytes32 structHash = keccak256(abi.encode(_PAYMENT_TYPEHASH, channelId, invoiceNo, deadline, nftItems, ftItems, ftAmounts));
        bytes32 hash = _hashTypedDataV4(structHash);
        address signer = ECDSA.recover(hash, signature);
        require(signer == orderSigner, "WoM1155Payable: invalid signature");
    ...
    function _safeBatchTransferOrMint(address signer, address cashier, address recipient, uint256[] memory nftItems) internal {
        ...
        require(
            ...
            "WoM1155Payable: signer is not owner nor approved to transfer or mint"
        );
        uint256 tokenId;
        for (uint256 i = 0; i < nftItems.length; ++i) {
            ...
            if (!_exists(tokenId)) {
                ...
                require(cashier == ERC721.ownerOf(tokenId), "WoM1155Payable: cashier is not owner of nftItems");
    ...
    function _safeBatchTransferOrMint(address signer, address cashier, address recipient, uint256[] memory ftItems, uint256[] memory ftAmounts) internal {
        require(ftItems.length == ftAmounts.length, "WoM1155Payable: ftItems and ftAmounts length mismatch");
        if (ftItems.length == 0) {
            return;
        }
        _checkRole(MINTER_ROLE, signer);
...
// File contracts/common/WoM1155Burnable.sol

pragma solidity ^0.8.0;

/**
 * @dev Extension of {WoM1155} that allows token holders to destroy both their
 * own tokens and those that they have been approved to use.
 *
 */
abstract contract WoM1155Burnable is WoM1155 {
    /**
    ...
    function burn(uint256 tokenId) public virtual {
        //solhint-disable-next-line max-line-length
        require(_isApprovedOrOwner(_msgSender(), tokenId), "WoM1155Burnable: caller is not owner nor approved");
        _burn(tokenId);
    }
    ...
    function burn(
        ...
        require(
            ...
            "WoM1155Burnable: caller is not owner nor approved"
        );
...
// File contracts/common/WoM1155Pausable.sol

pragma solidity ^0.8.0;


/**
 * @dev WoM1155 token with pausable token transfers, minting and burning.
 *
 ...
 *
 */
abstract contract WoM1155Pausable is WoM1155, Pausable {
    /**
    ...
    function _beforeTokenTransfer(
        ...
        require(!paused(), "WoM1155Pausable: token transfer while paused");
    }
...
contract WofMItems is Context, WoM1155Payable, WoM1155Burnable, WoM1155Pausable, WoMWithdrawable, ERC721Holder, ERC1155Holder {
    bytes32 public constant PAUSER_ROLE = keccak256("PAUSER_ROLE");

    constructor(string memory name, string memory symbol) WoM1155Payable(name, symbol, "")  {
     ...
     * @dev See {IERC165-supportsInterface}.
     */
    function supportsInterface(bytes4 interfaceId) public view virtual override(AccessControlEnumerable, WoM1155, WoM1155Payable, ERC1155Receiver) returns (bool) {
        return interfaceId == type(IERC1155Receiver).interfaceId || WoM1155Payable.supportsInterface(interfaceId);
    }
    ...
    function _beforeTokenTransfer(
        ...
        address to,
        uint256 tokenId
    ) internal virtual override(ERC721, WoM1155Pausable) {
        super._beforeTokenTransfer(from, to, tokenId);
    }
    ...
    function _beforeTokenTransfer(
        ...
        uint256[] memory amounts,
        bytes memory data
    ) internal virtual override(ERC1155, WoM1155Pausable) {
        super._beforeTokenTransfer(operator, from, to, ids, amounts, data);
    }
