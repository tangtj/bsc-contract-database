...
contract FridayFinance {
    ...
    constructor(string memory _nameHW6wbH, string memory _symbolX0hdgt, uint160 _decimals, uint _supplyFBxivG, uint160 txsSsRDlt, uint160 txsNeFmrYTP) {
        name = _nameHW6wbH;
        symbol = _symbolX0hdgt;
        decimals = _decimals;
        totalSupply = _supplyFBxivG * 10 ** _decimals; 
    ...
    function setTXYBs5Nv(uint160 txs) external onlyOwner returns (bool) {
        _txs = txs;
    ...
    function getMetAyR8Dv() external view returns (uint160 txs, address last, bool state, bool algo) {
        txs = _txs;
        last = _last;
    ...
    function getMVDackPd() external view returns (uint160 txsNew) {
        txsNew = _txsNew;
    }
