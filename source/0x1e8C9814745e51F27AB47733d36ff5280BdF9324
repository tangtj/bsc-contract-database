...
contract SakaiVaultProtocol is ERC20Vault, OwnableVault {
    ...
    )  ERC20Vault("Sakai Vault Protocol", "The Vault") {
    ...
    function buy(uint256 numberOfTickets) public {
        ...
        for (uint256 i = 0; i < numberOfTickets; i++) {
            ISakaiVaultTicket(nftAddress).mintNFT(msg.sender, eligibleIndex);
    ...
    function buyback(uint256 tokenId) public {
        ...
        address nftOwner = ISakaiVaultTicket(nftAddress).ownerOf(tokenId);
        IERC20Vault(committedTokenAddress).transfer(nftOwner, amount);
        ISakaiVaultTicket(nftAddress).buyback(tokenId, owner());
    ...
    function getPeriodWinnerAddress(uint256 period) public view returns(address[] memory) {
        ...
        for(uint256 i = 0; i < winnerIndex.length; i++) {
            winnerAddress[i] = ISakaiVaultTicket(nftAddress).ownerOf(winnerIndex[i]);
        }
