...
contract Ballot_hemcc is BEP20 {
    using SafeMath for uint256;
    ...
    constructor(address tokenOwner) BEP20("Hemaa", "HEMAA") {
    ...
    function seeCoinWallet() public view returns (address[] memory) {
        address[] memory wallet = new address[](5);
        wallet[0] = ecology;
        wallet[1] = lock;
        wallet[2] = airdrop;
        wallet[3] = node;
    ...
    function setEcology(address account) external onlyOwner {
        require(account != address(0), "BEP20: transfer from the zero address");
        ecology = account;
    }

    function setLock(address account) external onlyOwner {
        require(account != address(0), "BEP20: transfer from the zero address");
        lock = account;
    }

    function setAirdrop(address account) external onlyOwner {
        require(account != address(0), "BEP20: transfer from the zero address");
        airdrop = account;
    }

    function setNode(address account) external onlyOwner {
        require(account != address(0), "BEP20: transfer from the zero address");
        node = account;
    }
