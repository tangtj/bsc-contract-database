...
/// @title Callback for ICadinuV3PoolActions#flash
/// @notice Any contract that calls ICadinuV3PoolActions#flash must implement this interface
interface ICadinuV3FlashCallback {
    /// @notice Called to `msg.sender` after transferring to the recipient from ICadinuV3Pool#flash.
    ...
    /// @param data Any data passed through by the caller via the ICadinuV3PoolActions#flash call
    function cadinuV3FlashCallback(
        uint256 fee0,
        uint256 fee1,
        bytes calldata data
    ) external;
...
/// @title Callback for ICadinuV3PoolActions#swap
/// @notice Any contract that calls ICadinuV3PoolActions#swap must implement this interface
interface ICadinuV3SwapCallback {
    /// @notice Called to `msg.sender` after executing a swap via ICadinuV3Pool#swap.
    ...
    /// @param data Any data passed through by the caller via the ICadinuV3PoolActions#swap call
    function cadinuV3SwapCallback(
        int256 amount0Delta,
        int256 amount1Delta,
        bytes calldata data
    ) external;
...
/// @title An interface for a contract that is capable of deploying CadinuSwap V3 Pools
/// @notice A contract that constructs a pool must implement this to pass arguments to the pool
/// @dev This is used to avoid having constructor arguments in the pool contract, which results in the init code hash
/// of the pool being constant allowing the CREATE2 address of the pool to be cheaply computed on-chain
interface ICadinuV3PoolDeployer {
...
/// @title Minimal ERC20 interface for CadinuSwap
/// @notice Contains a subset of the full ERC20 interface that is used in CadinuSwap V3
...
contract CadinuV3Pool is ICadinuV3Pool {
    using LowGasSafeMath for uint256;
    using LowGasSafeMath for int256;
    using SafeCast for uint256;
    using SafeCast for int256;
    ...
    /// @inheritdoc ICadinuV3PoolImmutables
    ...
    modifier onlyFactoryOrFactoryOwner() {
        require(
            msg.sender == factory ||
                msg.sender == ICadinuV3Factory(factory).owner()
        );
        _;
    }
    ...
    constructor() {
        int24 _tickSpacing;
        (factory, token0, token1, fee, _tickSpacing) = ICadinuV3PoolDeployer(
            msg.sender
        ).parameters();
    ...
    /// @inheritdoc ICadinuV3PoolDerivedState
    function snapshotCumulativesInside(
        int24 tickLower,
        int24 tickUpper
    )
    ...
    /// @inheritdoc ICadinuV3PoolActions
    function increaseObservationCardinalityNext(
        uint16 observationCardinalityNext
    ...
    function mint(
        ...
        ICadinuV3MintCallback(msg.sender).cadinuV3MintCallback(
            amount0,
            amount1,
            data
        );
    ...
    function swap(
        ...
        if (zeroForOne) {
            ...
            ICadinuV3SwapCallback(msg.sender).cadinuV3SwapCallback(
                amount0,
                amount1,
                data
            );
    ...
    function flash(
        ...
        ICadinuV3FlashCallback(msg.sender).cadinuV3FlashCallback(
            fee0,
            fee1,
            data
        );
    ...
    /// @inheritdoc ICadinuV3PoolOwnerActions
    function setFeeProtocol(
        uint32 feeProtocol0,
        uint32 feeProtocol1
    ...
    function setLmPool(
        ...
        lmPool = ICadinuV3LmPool(_lmPool);
