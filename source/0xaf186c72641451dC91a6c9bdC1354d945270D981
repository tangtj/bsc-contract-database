/**
TELEGRAM : https://t.me/BurningDesire_BSC
TWITTER : https://twitter.com/BurnDesireBSC
*/
...
contract BurningDesire is Context, IERC20, Ownable {
    ...
    string private constant _name = "BurningDesire";
    string private constant _symbol = "BD";
    uint256 private _tTotal = 1_000_000_000 * 10**_decimals;
    uint256 private _rTotal = (MAX - (MAX % _tTotal));
    
    uint256 public swapTokensAtAmount = 2_000_000 * 10**_decimals; // Should be between 0.03% to 0.2%  of the total supply
    // Max Transaction Amount
    uint256 public maxTxAmount = 30_000_000 * 10**_decimals; 
    // Max Wallet Amount
    uint256 public maxWalletBalance = 30_000_000 * 10**_decimals; 

    address public marketingAddress = 0x7F0933a26917585a3b16831670d857552FA6888A; // Marketing Fee Receiver
    address private constant burnWallet = 0x000000000000000000000000000000000000dEaD;
    ...
    struct Taxes {
      ...
      uint256 marketing;
      uint256 liquidity;
      uint256 burn; // New parameter for burn tax
    }

    Taxes public Buytaxes = Taxes(0,9,0,1);
    Taxes public sellTaxes = Taxes(0,15,0,5);
    ...
    function _getValues(uint256 tAmount, bool takeFee, bool isSell) private view returns (valuesFromGetValues memory to_return) {
        ...
        (to_return.rAmount, to_return.rTransferAmount, to_return.rRfi, to_return.rMarketing, to_return.rDev, to_return.rLiquidity, to_return.rBurn) = _getRValues(to_return, tAmount, takeFee, _getRate());
        return to_return;
    }
    ...
    function _getTValues(uint256 tAmount, bool takeFee, bool isSell) private view returns (valuesFromGetValues memory s) {
        ...
        s.tBurn = tAmount * temp.burn / 100; // Calculate burn amount
        s.tTransferAmount = tAmount-s.tRfi-s.tMarketing-s.tDev-s.tLiquidity-s.tBurn;
        return s;
    }

    function _getRValues(valuesFromGetValues memory s, uint256 tAmount, bool takeFee, uint256 currentRate) private pure returns (uint256 rAmount, uint256 rTransferAmount, uint256 rRfi,uint256 rMarketing, uint256 rDev, uint256 rLiquidity, uint256 rBurn) {
        ...
        rTransferAmount =  rAmount-rRfi-rMarketing-rDev-rLiquidity-rBurn;
        return (rAmount, rTransferAmount, rRfi,rMarketing,rDev,rLiquidity,rBurn);
    }
