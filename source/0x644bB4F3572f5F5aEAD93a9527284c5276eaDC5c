...
contract BFCToken is ERC20, ReentrancyGuard, Ownable {
    ...
    uint256 public constant MAX_MINT = 1 * 10 ** 22; //test
    uint256 public sonValidAmount = 5 * 10 ** 18; //test
    uint256 public constant DEL_TIME = 3600; //test
    uint256 public constant RECOMMEND_RATIO = 10;
    uint256 public constant MINT_RATIO = 10; //test
    uint256 private toUsdtFee = 20;
    ...
    address public fundAddress =
        address(0xe9A3334a595fA51B79564Af5078a453fa658460D);
    address public mintAddress =
        address(0x63dc154723fa9EA279c878917e26E49175BE923b);
    address public burnAddress =
        address(0xCb422fE4842b2FAE5453978dFdE3b69B4f38F466);

    constructor() ERC20("BFCToken", "BFC") { 
        uint256 total = 90000 * 10 ** decimals();
        _approve(
            address(this),
    ...
    function setExcludedFromFees(address excludedFromFees, bool _isExcludedFees)
        public
        onlyOwner
    {
        isExcludedFromFees[excludedFromFees] = _isExcludedFees;
    }
    ...
    function getUserBurnAmount(address user) public view returns (uint256) {
        ...
        if (_userTime > 0 && _userTime < endTime) {
            ...
            uint256 burnAmount = userBalance.mul(25).div(100).div(
                BLOCK_AMOUNT_OF_DAY
            ); // test
    ...
    function getUserMintAmount(address user) public view returns (uint256) {
        ...
        if (userTime > 0 && userTime < endTime) {
            ...
            uint256 mintAmount = tokenAmount
                ...
                .div(100); //test
            if (mintCount >= BLOCK_AMOUNT_OF_DAY)return mintAmount.mul(BLOCK_AMOUNT_OF_DAY);
    ...
    function _transfer(
        ...
        if (ammPairs[to]) {
            ...
            if (isAddLdx) {
                ...
                uint256 subAmount = amount.div(100).mul(10);
                super._transfer(address(uniswapV2PairUSDT), DESTROY, subAmount);
                _totalSupply = _totalSupply - subAmount;
