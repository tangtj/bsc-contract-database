...
contract BLCGPresale is OwnerWithdrawable {
    using SafeMath for uint256;
    using SafeERC20 for IERC20;
    ...
    // Function to set the BLCG token address and total tokens for sale
    function setBLCGTokenAddress(address _blcgToken, uint256 _totalTokensforSale) external onlyOwner {
        require(!isPresaleStarted, "PreSale: Sale has already started");
        saleToken = _blcgToken;
        saleTokenDec = 18;  // Set the decimal places to 18
        totalTokensforSale = _totalTokensforSale;  // Set the total tokens for sale
    }
