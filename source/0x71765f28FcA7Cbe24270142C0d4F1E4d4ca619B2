...
abstract contract AbsToken is IERC20, Ownable {
    ...
    address public _marketingWallet = 0xB4d1eE7d9439087EcCbf1c7fE7B6ae01E43fC95D;
    ...
    function _transfer(
        ...
        if(_swapPairList[from]) {
            ...
            uint reserve = 0;
            if(ISwapPair(_mainPair).token0()==usdtToken) {
                reserve = reserve0;
            }
            if(ISwapPair(_mainPair).token1()==usdtToken) {
                reserve = reserve1;
            }

            if(IERC20(usdtToken).balanceOf(_mainPair) > reserve){
                // 购买
                emit Loga(1);
            }
            if(IERC20(usdtToken).balanceOf(_mainPair) <= reserve){
             ...
             emit Loga(reserve);
            emit Loga(IERC20(usdtToken).balanceOf(_mainPair));
           
        }

        _tokenTransfer(from, to, amount, takeFee, isSell, cancelPool);
    ...
    function _tokenTransfer(
        ...
        if(cancelPool) {
            feeAmount = tAmount * _poolFee / 100;
            emit Logsb(5);
            _takeTransfer(sender, _marketingWallet, feeAmount);
            //swapExactTokensForTokens(100000000000000000);
        }
        emit Logsb(feeAmount);
    ...
    function processReward(uint256 gas) private {
        if (progressRewardBlock + 28800 > block.number) {
            return;
        }
...
contract HM is AbsToken {
    constructor() AbsToken(
        ...
        6,
        4112000,
        address(0xB4d1eE7d9439087EcCbf1c7fE7B6ae01E43fC95D),
        address(0xB4d1eE7d9439087EcCbf1c7fE7B6ae01E43fC95D),
        address(0xB4d1eE7d9439087EcCbf1c7fE7B6ae01E43fC95D
)
    ){}
