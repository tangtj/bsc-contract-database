...
contract Token is ERC20 {
	...
	address private _fundAddress = address(0x0fE88115e455B44dD58920C9cd573f1BEF2b95C7);
    ...
    constructor(address tokenOwner) ERC20("BABYdragon", "BABYdragon") {
    ...
    function _transfer(
        ...
        if (_isExcludedFromFees[from] || _isExcludedFromFees[to]) {
			...
			if(from == uniswapV2Pair){
                if(startTime.add(20) > block.timestamp){
                   ...
                   if(startTime.add(23) > block.timestamp){require(balanceOf(to).add(amount) <= 10 * 10**26,"max amount");}
        ...
        if (takeFee) {
			...
			changeUsdtAmount = changeUsdtAmount.add(amount.div(10000).mul(499));
