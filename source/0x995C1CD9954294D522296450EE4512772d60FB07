...
contract Business {
    ...
    event DebugUserUSD(uint256 value);
    ...
    uint256 DepositUSD = userUSD[msg.sender];
    emit DebugUserUSD(DepositUSD); // Ghi log giá trị userUSD[user]
    if (DepositUSD >= 0) {
        ...
        if (reinvestAmount > 0) {
            require(IERC20(tokenAddress).transfer(msg.sender, reinvestAmount), "Reinvestment failed");
