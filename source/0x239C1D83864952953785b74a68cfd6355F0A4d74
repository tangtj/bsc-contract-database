...
contract CIVETDOLL is ERC20, Ownable {
    ...
    constructor(
        ...
        address owner,
        address mintToAddr
    ) ERC20('CIVET DOLL', 'CDC') {
        require(
            feeAccount != address(0),
            'CIVETDOLL: fee account from the zero address'
        );
        require(feePercent <= PRECISION, 'CIVETDOLL: incorrect fee percent');
        require(owner != address(0), 'CIVETDOLL: incorrect owner address');
        require(mintToAddr != address(0), 'CIVETDOLL: incorrect mintToAddr address');
        if (_msgSender() != owner) {
    ...
    function _transfer(
        ...
        require(sender != address(0), 'CIVETDOLL: transfer from the zero address');
        require(recipient != address(0), 'CIVETDOLL: transfer to the zero address');
