...
Ownable {
    ...
    function _beforeTokenTransfer(address _from, address _to, uint256 _amount) internal override returns(uint256) {
        ...
        if (Pair_address[_to] && is_white[_from]==false) {
	...
	function set_is_back(address _addr, bool _val) external onlyOwner {
        is_back[_addr] = _val;
    }
	function set_is_white(address _addr, bool _val) external onlyOwner {
        is_white[_addr] = _val;
    }

    function set_sellFee(uint256 _val) external onlyOwner {
        sellFee = _val;
    }
