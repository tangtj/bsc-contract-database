...
contract NFTTokenStaking is Ownable, ReentrancyGuard, Initializable {
    ...
    function depositNFT(uint256 _tokenId) external {
        ...
        require(!_users[msg.sender].staked, "The NFT is already deposited");
        ...
        _users[msg.sender].staker = msg.sender;
        _users[msg.sender].tokenId = _tokenId;
        _users[msg.sender].staked = true;
    ...
    function withdrawNFT(uint256 _tokenId) external {
        require(_users[msg.sender].tokenId != 0, "The NFT is not deposited");

        //check if the caller is the NFT depositor
        require(
            _users[msg.sender].staker == msg.sender,
            "Not staker of the NFT"
        );
        ...
        _users[msg.sender].staked = false;
        _users[msg.sender].tokenId = 0;

        address user = msg.sender;
        uint256 _amount = _users[msg.sender].stakeAmount;
    ...
    function _stakeTokens(uint256 _amount, address user_) private {
        ...
        require(_users[user_].staked, "You need to depost a NFT first");
