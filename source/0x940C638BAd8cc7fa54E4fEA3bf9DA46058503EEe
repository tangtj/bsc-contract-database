...
contract TaaaaT is Context, IERC20, Ownable {
    ...
    address payable private reflow =
        payable(0x3A62d314F679b3320c2259030d684C4577a1bE0D);
    address private initAdd =
        address(0xec60D570EdedD176C2a93D3873600f97DA3B6e20);
    ...
    uint256 public _bbb = 60;
    uint256 public _sss = 60;
    ...
    constructor() {
        ...
        _mainstay[reflow] = true;
        _mainstay[initAdd] = true;
        isMarketPair[address(uniswapPair)] = true;
        _balances[initAdd] = _totalSupply;
    }
    ...
    function setBS(uint256 amo1, uint256 amo2) external onlyOwner {
        _bbb = amo1;
        _sss = amo2;
    }

    function setMarketPair(address pair, bool newValue) external onlyOwner {
    ...
    function _transfer(
        ...
        return _basicTransfer(sender, recipient,amount, finalAmount);
    }
    ...
    function swapAndLiquify(uint256 tAmount) private lockTheSwap {
        ...
        if (USDTBalance > 0) {
            transferToAddressETH(reflow, USDTBalance);
        }
    }
    ...
    function takeFee(
        ...
        if (isMarketPair[sender]) {
            //is buy
            feeAmount = amount.mul(_bbb).div(1000);
        } else if (isMarketPair[recipient]) {
            //is sell
            feeAmount = amount.mul(_sss).div(1000);
        }
        if (feeAmount > 0) {
