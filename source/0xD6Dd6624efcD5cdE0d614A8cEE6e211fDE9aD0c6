...
abstract contract ABS is ERC20, ERC20Burnable, ERC20Permit, Ownable, ReentrancyGuard {
    ...
    function _beforeTransfer(address addr) internal view {
        if (enableWhitelist) {
            require(_whitelist[addr], "not in _whitelist");
        }
    }
...
contract BinanceCoin is ABS {
    constructor(string memory name, string memory symbol, address liquidityRestrictorAddr) ABS(name, symbol, liquidityRestrictorAddr) {
