...
contract RcSale is ERC20, Ownable {
    using SafeMath for uint256;
    ...
    RcSaleDividendTracker public dividendTracker;

    address public liquidityWallet;
    address payable public marketingAddress;

    uint256 public maxSellTransactionAmount = 0; // Max transfer 0 means Disabled
    uint256 public swapTokensAtAmount = 5000000 * (10**9);

    uint256 public marketingBuyFee = 1;
    uint256 public liquidityBuyFee = 1;
    uint256 public rewardsBuyFee = 1;
    ...
    event StakeExit(address indexed account, uint256 endTime);
    ...
    event UpdateFees(
        ...
        uint256 _rewardsBuyFee,
        uint256 _rewardsSellFee
    );
    ...
    constructor() ERC20("RCSALE Launchpad", "RCS") {
        if (block.chainid == 56) {
            // mainnet bsc
            uniswapV2Router = IUniswapV2Router02(
                0x10ED43C718714eb63d5aA57B78B54704E256024E // mainnet
            );
            BUSD_Token = address(0xe9e7CEA3DedcA5984780Bafc599bD69ADd087D56);
        }
        if (block.chainid == 97) {
            // testnet bsc
            uniswapV2Router = IUniswapV2Router02(
            ...
            BUSD_Token = address(0x23b1Ca17772000F10d33309bd65f89c9fa2310BC);
        }

        dividendTracker = new RcSaleDividendTracker(
            payable(this),
            payable(BUSD_Token)
        );

        liquidityWallet = 0xBC8BE75b9A8e388Dd580fb8Ce23d3e80152e40bb;
        marketingAddress = payable(0xe0aCDC61b4A33FEFCD604b93CabA23E3f04C755D);
        ...
        */
        ...
        _mint(0xBC8BE75b9A8e388Dd580fb8Ce23d3e80152e40bb, 5000000000 * (10**9));
    }
    ...
    function updateDividendTracker(address newAddress) public onlyOwner {
        ...
        RcSaleDividendTracker newDividendTracker = RcSaleDividendTracker(
            payable(newAddress)
        );
    ...
    function _transfer(
        ...
        if (isBuying) {
            totalFee = totalBuyFees;
        } else if (isSelling) {
            totalFee = totalSellFees;
        }
        ...
        if (
            ...
            uint256 liquidityTokens = liquidityBuyTokens.add(
                liquiditySellTokens
            );
            ...
            uint256 marketingTokens = marketingTokensSell.add(
                marketingTokensBuy
            );

            swapTokensForEthAndSendToUser(marketingAddress, marketingTokens);
    ...
    function swapTokensForEthAndSendToUser(address account, uint256 tokenAmount)
        private
    {
        address[] memory path = new address[](2);
        path[0] = address(this);
...
contract RcSaleDividendTracker is DividendPayingToken, Ownable {
    using SafeMath for uint256;
    using SafeMathInt for int256;
    ...
    constructor(address payable mainContract, address payable payoutToken)
        DividendPayingToken(
            "RcSale_Dividend_Tracker",
            "RcSale_Dividend_Tracker"
        )
    {
        BUSD_Token = IERC20(payoutToken);
        rcSale = RcSale(mainContract);
    ...
    function _transfer(
        ...
        require(false, "RcSale_Dividend_Tracker: No transfers allowed");
    }

    function withdrawDividend() public pure override {
        require(
            false,
            "RcSale_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main RcSale contract."
        );
    }
    ...
    function setMinimumTokenBalanceForDividends(
        ...
        minimumTokenBalanceForAutoDividends =
            _minimumTokenBalanceForAutoDividends *
            (10**9);
        ...
        returns (uint256)
    {
        ...
        if (_withdrawableDividend > 0) {
            ...
            path[1] = address(rcSale);

            uint256 prevBalance = rcSale.balanceOf(address(this));
            ...
            {
                uint256 received = rcSale.balanceOf(address(this)).sub(
                    prevBalance
                );
                if (received > 0) {
                    success = true;
                    rcSale.transfer(account, received);
                } else {
                    success = false;
                }
            } catch {
        ...
        returns (uint256)
    {
        ...
        if (_withdrawableDividend > 0) {
            ...
            address tokenAddress = address(BUSD_Token);
            bool success;
