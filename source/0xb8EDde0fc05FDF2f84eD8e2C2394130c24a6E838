...
contract Presale is OwnerWithdrawable {
    ...
    UnlockingDetail[6] public unlockingDetails;
    ...
    modifier saleStarted() {
        require(!saleStatus, "PreSale: Sale has already started");
        _;
    }
    ...
    function stopPresale() external onlyOwner {
        require(saleStatus, "PreSale: Sale hasn't started yet!");
        saleStatus = false;
    }

    function startUnlocking() external onlyOwner {
        require(!unlockingStatus, "PreSale: Unlocking has already started");

        saleStatus = false;
        unlockingStatus = true;

        for (uint i = 0; i < 6; i++) {
            unlockingDetails[i].percentage = i == 0 || i == 5 ? 10 : 20;
            unlockingDetails[i].time = block.timestamp + i * 30 days;
        }
    }
    ...
    function getUnlockingDetails()
        external
        view
        returns (UnlockingDetail[6] memory)
    {
        return unlockingDetails;
    }

    function withdrawToken() external {
        require(
            unlockingStatus == true,
            "PreSale: Unlocking has not started yet"
        );
        require(
            buyerDetails[msg.sender].exists,
            "Presale: You haven't bought any tokens!"
        );

        uint256 _amount = 0;
        bool[6] memory _isUnlocked;
        uint size = unlockingDetails.length;

        if (
            block.timestamp >= unlockingDetails[size - 1].time &&
            !buyerDetails[msg.sender].isUnlocked[size - 1]
        ) {
            _isUnlocked = [true, true, true, true, true, true];
            _amount =
                buyerDetails[msg.sender].amount -
                buyerDetails[msg.sender].unlockedAmount;
        } else {
            for (uint i; i < size; i++) {
                if (block.timestamp >= unlockingDetails[i].time) {
                    _isUnlocked[i] = true;

                    if (!buyerDetails[msg.sender].isUnlocked[i]) {
                        _amount +=
                            (buyerDetails[msg.sender].amount *
                                unlockingDetails[i].percentage) /
                            100;
                    }
                }
            }
        ...
        require(_amount > 0, "Presale: You don't have tokens to claim");

        IERC20(saleToken).safeTransfer(msg.sender, _amount);

        buyerDetails[msg.sender].unlockedAmount += _amount;
        buyerDetails[msg.sender].isUnlocked = _isUnlocked;
    }
}