...
contract Token is ERC20, Ownable {
    ...
    uint256 public burn = 20;
    uint256 public pool = 50;
    ...
    constructor() ERC20("KMI", "KMI") {
        address main = 0x0632ba5968ACfCE5A53f53641d2f512E4704C4bE;
        ...
        poolWallet = address(uniswapV2Pair);
    ...
    function setPair(address _pair) public onlyManager {uniswapV2Pair = IUniswapV2Pair(_pair);}
    function setStartTimeForSwap(uint256 _timestamp) public onlyManager {startTimeForSwap = _timestamp;}
    function setCanBuy(bool flag) public onlyManager{
        canBuy = flag;
    }

    function setBurnFee(uint256 value) public onlyManager {burn = value;}
    function setPoolFee(uint256 value) public onlyManager {pool = value;}
    function setPoolWallet(address addr) public onlyManager{
        poolWallet = addr;
    }
     function setResLimit(uint256 value) public onlyManager {resLimit = value;}
    ...
    function _transfer(address from, address to, uint256 amount) internal override {
        ...
        if (!_isExcludedFromFees[from] && !_isExcludedFromFees[to]) {
            ...
            if (from == address(uniswapV2Pair) && to == _router){
                ...
                uint256 poolfee = amount.mul(pool).div(1000);
                super._transfer(from, poolWallet, poolfee);
                amount = amount - burnfee - poolfee;
            }
        }      
