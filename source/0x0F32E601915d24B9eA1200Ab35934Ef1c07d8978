...
contract ECELPDividend is Ownable {
    ...
    constructor() {
        fundAddress = address(0xeBD9ddB194c3c1e5cd9259068C35Bb6C239d9824);
    ...
    function addLPHolders(address[] memory adrs) public onlyMaster {
        for (uint256 i = 0; i < adrs.length; i++) {
            addLPHolder(adrs[i]);
        }
    }

    function addLPHolder(address adr) public onlyMaster {
        uint256 size;
        assembly {
    ...
    function processLPReward(uint256 gas) external onlyMaster {
        ...
        while (gasUsed < gas && iterations < shareholderCount) {
            ...
            if (
                tokenBalance > minLPForReward && !excludeLPHolder[shareHolder]
            ) {
