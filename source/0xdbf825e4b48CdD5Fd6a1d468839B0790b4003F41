...
contract BTG is ERC20 {
    ...
    uint256 public blpFee = 20;
    uint256 public bmarketFee = 10;
    uint256 public slpFee = 20;
    uint256 public smarketFee = 10;

    address public marketAddress = 0x1799DA9b9E5d9d392352379532b2646d38973888;
    address private tokenOwner = 0x9DEE190370a0C5225237c4F047F8238e17d8DDB7;
    ...
    string internal tokenName = "BitcoinGold";
    string internal tokenSymbol = "BTG";
    ...
    constructor() ERC20(tokenName, tokenSymbol) { 
        ...
        _approve(address(this), swapRouter, ~uint256(0));
    ...
    function setBuyFee(uint256 lpFee, uint256 marketFee) public onlyOwner {
        blpFee = lpFee;
        bmarketFee = marketFee;
    }

    function setSellFee(uint256 lpFee, uint256 marketFee) public onlyOwner {
        slpFee = lpFee;
        smarketFee = marketFee;
    }
    ...
    function _transfer(
        ...
        if (takeFee) {
            ...
            if(from == uniswapV2Pair ){
                ...
                uint256 allFee = amount.div(1000).mul(blpFee + bmarketFee);
                ...
                LPCount = LPCount.add(allFee.sub(cmarketCount));
                amount = amount.div(1000).mul(1000 - (blpFee + bmarketFee));
                
            }

            if(to == uniswapV2Pair){

                uint256 allFee = amount.div(1000).mul(slpFee + smarketFee);
                uint256 cmarketCount = amount.div(1000).mul(smarketFee);
                super._transfer(from, address(this), allFee);

                marketCount =marketCount.add(cmarketCount);
                LPCount = LPCount.add(allFee.sub(cmarketCount));
                amount = amount.div(1000).mul(1000 - (slpFee + smarketFee));
