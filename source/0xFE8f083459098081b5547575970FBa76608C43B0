/**
	...
	Telegram: https://t.me/DiamondHands11
	Max buy 2% max, hold 4%
	100% Liquidity burned, Ownership renounced
	
	There's also an anti-dump mechanic so holders get rewarded.
	
	Sell within 30 minutes   : 40% burn (30% Back to the Liquidity Pool, 10% Redistributed to Holders)
	Sell within 45 minutes   : 32% burn (24% Back to the Liquidity Pool, 8% Redistributed to Holders)
	Sell within 1 hour       : 24% burn (18% Back to the Liquidity Pool, 6%  Redistributed to Holders)
	Sell within 2 hours      : 16% burn (12% Back to the Liquidity Pool, 4%  Redistributed to Holders)
	...
	contract Diamond is Context, IBEP20, Ownable {
	    ...
	    uint256 public _maxWalletToken = 400 * 10**6 * 10**9; //
	
	    IPancakeRouter02 public immutable pcsV2Router;
	    address public immutable pcsV2Pair;
	
	    string private _name = "Diamond";
	    ...
	    function _getAntiDumpMultiplier() private view returns (uint256) {
	        ...
	        uint256 hour = 60 * 60;
	        uint256 minute = 60;
	        if (time_since_start < 30 * minute) {
	            return (5);
	        } else if (time_since_start < 45 * minute) {
	            return (4);
	        } else if (time_since_start < 1 * hour) {
	            return (3);
