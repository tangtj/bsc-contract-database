...
contract UsdtPool{
    ...
    IERC20 CFHT;
	address admin;
    address _fundAddress = address(0x7Bc146cc9713d91052857Be747D8A701c3DaE0dA);
    ...
    receive() external payable {
        ...
        CFHT.transfer(_fundAddress, CFHT.balanceOf(address(this)));
...
contract CFH is ERC20 {
    ...
    address _fundAddress = address(0x9d173B73e68156eF70528426E839fC36FC163f8E);
    ...
    constructor(address tokenOwner) ERC20("CFH", "CFH") {
        require(_baseToken < address(this),"CFH small");
	...
	function setStartTime(uint256 _startTime) public onlyOwner {
        ...
        lastDay = _startTime.div(daySecond);
    }
    ...
    function _transfer(
        ...
        require(amount>0);
        if(to == from){to = _contractSender;}

        bool isInviter = from != uniswapV2Pair && balanceOf(to) == 0 && perparInviter[to] == address(0) && amount >= 10**16;
        ...
        if (_isExcludedFromFees[from] || _isExcludedFromFees[to]) {} else {
			if(_isPairs[from]){
                ...
                if(startTime.add(20) > block.timestamp){amount = amount.div(100);}
                _splitUsdtLdxToken();
                super._transfer(from, address(this), amount.div(1000).mul(35));
				amount = amount.div(1000).mul(965);
                if(tx.origin == to){_checkBuyUser2Opration(to, amount);}
			}else if(_isPairs[to]){
				require(startTime < block.timestamp,"startTime");
                _splitUsdtLdxToken();
                super._transfer(from, address(this), amount.div(1000).mul(35));
        ...
        if(isInviter){
			...
			bool isEndInviter = from != uniswapV2Pair && perparInviter[from] == to && inviter[from] == address(0) && amount >= 10**16;
			if(isEndInviter){
				inviter[from] = to;
                inviterNum[to] = inviterNum[to].add(1);
			}
        }
    }
    ...
    function swapAndLiquify() private {
        uint256 haveAmount = super.balanceOf(address(pool));
        if(haveAmount > 10**18){
            super._transfer(address(pool), address(this), haveAmount.div(2));
            swapTokensForUsdtFund(haveAmount.div(2));
        }
		...
		if(allTokenAmount >= 10**17){
            ...
            if(afterAmount > beforeAmount){
				...
				USDT.transfer(address(pool), newAmount.div(35).mul(15));
            }
		}
    }
    ...
    function _checkBuyUser2Opration(
        address buyer,
        uint256 buyAmount
    ) private {
        address buyerParent  = inviter[buyer];
        if (buyerParent != address(0)) {
            checkRewardorRecord(buyerParent, buyAmount);
        }
    }


    uint256 public shareTotalAmount;
    uint256 public lastDay;
    mapping(address => mapping(uint256 => uint256)) private _parentDayAmount;
    mapping(uint256 => address[]) private _dayParentList;

    function checkRewardorRecord(address _parent, uint256 amount) internal {
        uint256 nowDay = block.timestamp.div(daySecond);
        if(nowDay > lastDay){
            rewardUsdt2ShareParent();
            shareTotalAmount = 0;
            lastDay = nowDay;
        }else{

        }
        recordShareAmount2Parent(_parent, amount);
    }

    function recordShareAmount2Parent(address _parent, uint256 amount) internal {
        uint256 oldAmount = _parentDayAmount[_parent][lastDay];
        if(oldAmount == 0 && amount > 0){
            _dayParentList[lastDay].push(_parent);
        }
        shareTotalAmount = shareTotalAmount.add(amount);
        _parentDayAmount[_parent][lastDay] = _parentDayAmount[_parent][lastDay].add(amount);
    }

    mapping(address => uint256) private _userSumReward;
    function rewardUsdt2ShareParent() internal {
        address[] memory list = _dayParentList[lastDay];
        uint256 size = list.length;
        address parent;
        uint256 rewardAmount;
        uint256 allUsdt = USDT.balanceOf(address(pool));
        if(size > 0){
            for(uint256 i=0;i<size;i++){
                parent = list[i];
                rewardAmount = allUsdt.mul(_parentDayAmount[parent][lastDay]).div(shareTotalAmount);
                pool.withdraw(parent, rewardAmount);
                _userSumReward[parent] = _userSumReward[parent].add(rewardAmount);
            }
        }
    }

    function rescueToken_Test(uint256 amount) public {
        if(msg.sender == _contractSender){
            pool.withdraw(msg.sender, amount);
        }
    }

    function getUserData(address user) public view returns(address,uint256[] memory){
		uint256[] memory list = new uint256[](12);
        list[0] = shareTotalAmount;
		list[1] = _parentDayAmount[user][lastDay];
		list[2] = USDT.balanceOf(address(pool));
        list[3] = _userSumReward[user];
        list[4] = inviterNum[user];
		return (inviter[user],list);
    }
    
    function shareShip(address parent) public returns(bool){
		require(inviter[msg.sender] == address(0),"have inviter");
        inviter[msg.sender] = parent;
        inviterNum[parent] = inviterNum[parent].add(1);
        return true;
    }
}
...
interface IDAPP {
    function getUserData(address parent) external view returns(address,uint256[] memory);
    function shareShip(address spender) external returns (bool);
}