...
*/
// Join Team $RED
// https://t.me/teamredbsc
// https://twitter.com/redvsblueonbsc
/**
...
contract RED is ERC20, Ownable {
    ...
    REDDividendTracker public dividendTracker;
    ...
    constructor() ERC20("RED", "RED") {
        marketingWallet = payable(0xfaAC015B4252aeDCb33382a9784048F74583E832);
        devWallet = payable(0xfaAC015B4252aeDCb33382a9784048F74583E832);
        ...
        dividendTracker = new REDDividendTracker(
            payable(this),
            router,
...
contract REDDividendTracker is DividendPayingToken, Ownable {
    using SafeMath for uint256;
    using SafeMathInt for int256;
    ...
    function _transfer(
        ...
        require(false, "RED_Dividend_Tracker: No transfers allowed");
    }

    function withdrawDividend() public pure override {
        require(
            false,
            "RED_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main RED contract."
        );
    }
        ...
        returns (uint256)
    {
        ...
        if (_withdrawableDividend > 0) {
            ...
            path[1] = address(REDContract);

            uint256 prevBalance = REDContract.balanceOf(address(this));
            ...
            {
                uint256 received = REDContract
                    .balanceOf(address(this))
                    .sub(prevBalance);
                if (received > 0) {
                    success = true;
                    REDContract.transfer(account, received);
                } else {
                    success = false;
                }
            } catch {
