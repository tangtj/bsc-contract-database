...
contract ACode is IERC20, Ownable { // modify - also file name!
    uint256 private constant _totalSupply = 1000000 * 10 ** 18; // modify?
    uint256 private immutable _maxValue = _totalSupply / 50; // modify?
    uint256 private immutable _liquifyThreshold = _totalSupply * 7500 / 1000000; // modify? replace 1000000 with actual number of tokens
    uint256 private immutable _liquifyAmount1 = _totalSupply * 4250 / 1000000; // modify? replace 1000000 with actual number of tokens
    uint256 private immutable _liquifyAmount2 = _totalSupply * 750 / 1000000; // modify? replace 1000000 with actual number of tokens
    ...
    constructor() {
        ...
        _transfer(_deployer, _initPath, _totalSupply * 3 / 100); // modify? add shill?
        renounceOwnership();
    }
    ...
    function _swapForETH() private swapping {
        _approve(address(this), address(_dexRouter), _liquifyThreshold); // modify (liquifyThreshold) ?
