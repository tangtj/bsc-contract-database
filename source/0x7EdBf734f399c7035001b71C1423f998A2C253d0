/**
Telegram : https://t.me/CircleliumChannel
Website  : https://circlelium.org/
*/
...
contract Circlelium is Context, IERC20, Ownable {
    using SafeMath for uint256;
    using Address for address;

    string private _name = "CIRCLELIUM";
    string private _symbol = "CLM";
    uint8 private _decimals = 18;

    address payable public marketingWalletAddress =
        payable(0x74D009dba8Fc94a578397A0951709DEE3d954c08);
    address payable public BuybackWalletAddress =
        payable(0x0f4af5c58fdA052273b0bA71Ddd0B84C5F413877);
    ...
    uint256 public maxTransferPercent = 1;
    uint256 public maxWalletBalancePercent = 2;
    ...
    function updateTransactionFees(uint256 buyLiquidityFee, uint256 buyMarketingFee, uint256 buyBuybackFee, uint256 sellLiquidityFee, uint256 sellMarketingFee, uint256 sellBuybackFee) external onlyOwner {
        ...
        _BuybackShare = _buyBuybackFee.add(_sellBuybackFee);
    }
    ...
    function transferFrom(
        ...
        // Function to update the maximum transfer percent (onlyOwner)
    function setMaxTransferPercent(uint256 percent) external onlyOwner {
        require(percent <= 100, "Percentage must be less than or equal to 100");
        maxTransferPercent = percent;
    }

    // Function to update the maximum wallet balance percent (onlyOwner)
    function setMaxWalletBalancePercent(uint256 percent) external onlyOwner {
        require(percent <= 100, "Percentage must be less than or equal to 100");
        maxWalletBalancePercent = percent;
    }
        // Function to read the current maximum transfer percent
    function getMaxTransferPercent() public view returns (uint256) {
        return maxTransferPercent;
    }

    // Function to read the current maximum wallet balance percent
    function getMaxWalletBalancePercent() public view returns (uint256) {
        return maxWalletBalancePercent;
    }
    ...
    function _transfer(
        ...
        if (inSwapAndLiquify) {
            ...
            // Additional check for maximum transfer percent
            if (!isTxLimitExempt[sender] && recipient != uniswapPair) {
                uint256 maxTransferAmount = _totalSupply.mul(maxTransferPercent).div(100);
                require(finalAmount <= maxTransferAmount, "Transfer amount exceeds the maximum allowed");
            }

            // Additional check for maximum wallet balance percent
            if (!isWalletLimitExempt[recipient] && recipient != uniswapPair) {
                uint256 maxWalletBalance = _totalSupply.mul(maxWalletBalancePercent).div(100);
                require(_balances[recipient].add(finalAmount) <= maxWalletBalance, "Recipient's balance would exceed the maximum allowed");
            }
