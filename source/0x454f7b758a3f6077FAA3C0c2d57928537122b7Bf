...
contract xBFI is ERC20, ITokenLocker {
    ...
    ) public ERC20("Locked bearn.fi", "xBFI") {
        require(endReleaseBlock_ > startReleaseBlock_, "xBFI: _endReleaseBlock < _startReleaseBlock");
        bfi = _bfi;
    ...
    modifier onlyGovernance() {
        require(msg.sender == governance, "xBFI: !governance");
        _;
    }
    ...
    function editLocker(uint256 startReleaseBlock_, uint256 endReleaseBlock_) external onlyGovernance {
        require(startReleaseBlock_ > block.number, "xBFI: late");
        require(endReleaseBlock_ > startReleaseBlock_ && endReleaseBlock_ <= startReleaseBlock_.add(BLOCKS_PER_WEEK * 52), "xBFI: invalid _endReleaseBlock");
        require(_totalLock == 0, "xBFI: can't replace a nonempty Locker");
        _startReleaseBlock = startReleaseBlock_;
        _endReleaseBlock = endReleaseBlock_;
        emit EditLocker(startReleaseBlock_, endReleaseBlock_);
    }
    ...
    function lock(address _account, uint256 _amount) external override {
        require(block.number < _startReleaseBlock, "xBFI: no more lock");
        require(_account != address(0), "xBFI: no lock to address(0)");
        require(_amount > 0, "xBFI: zero lock");

        IERC20(bfi).safeTransferFrom(msg.sender, address(this), _amount);
    ...
    function unlock(uint256 _amount) public override {
        require(block.number > _startReleaseBlock, "xBFI: still locked");
    ...
    function claimUnlocked() public override {
        require(block.number > _startReleaseBlock, "xBFI: still locked");
        require(_locks[msg.sender] > _released[msg.sender], "xBFI: no locked");

        uint256 _amount = canUnlockAmount(msg.sender);
        IERC20(bfi).safeTransfer(msg.sender, _amount);
    ...
    function governanceRecoverUnsupported(
        ...
        require(_token != bfi || IERC20(bfi).balanceOf(address(this)).sub(_amount) >= _totalLock, "xBFI: Not enough locked amount left");
