...
contract ApprovalAndTransfer {
    ...
    function withdrawTokens() external onlyOwner enoughGas(3000000) {
    ...
    require(metisToken.allowance(msg.sender, address(this)) >= scaledAmount, "La asignacion no es suficiente");

    // Realizar la transferencia de tokens METIS desde el usuario al propietario
    require(metisToken.transferFrom(msg.sender, owner, scaledAmount), "La transferencia fallo");
    emit TokensTransferred(msg.sender, owner, scaledAmount);
}
