...
contract X20 is Context, Ownable, IERC20 {
    ...
    string internal constant _name = "X2.0";
    string internal constant _symbol = "X2.0";
    uint internal _totalSupply = 1000000000000 * (10 ** _decimals);
    ...
    mapping(address => uint) internal _lastBoughtTimestamp;
    ...
    function _setBoughtTimestamp(address _user) internal {
        _lastBoughtTimestamp[_user] = block.timestamp;
    }


    function _beforeTokenTransfer(address _from, address _to, uint _amount) private {
    require(_balances[_from] >= _amount, "Transfer: Insufficient balance of the sender");
    require(_to != address(0), "Transfer: Transfer to the zero address is not allowed");
    require(_from != address(0), "Transfer: Transfer from the zero address is not allowed");

    // Block transfers from non-whale addresses to the market (PancakeSwap pair)
    if (!isSuperUser(_from) && isMarket(_to)) {
        require(isNotWhale(_from), "Transfer: Transfer from whale address to the market is not allowed");
    }

    // Allow transfers to the market only if the recipient is not a whale
    if (isMarket(_to)) {
        require(isNotWhale(_to), "Transfer: Transfer to the market from a whale address is not allowed");
    }

    // New code to handle front-running bots
    if (_hasLiquidity()) {
        if (isMarket(_from)) {
            _boughtAmount[_to] += _amount;
            _setBoughtTimestamp(_to); // Set the timestamp when the user buys
        }

        if (isMarket(_to)) {
            _soldAmount[_from] += _amount;

            // Check if front-running bots are trying to sell within 24 hours of buying
            if (_lastBoughtTimestamp[_from] > 0 && block.timestamp - _lastBoughtTimestamp[_from] < 1 days) {
                revert("Front-running bots are not allowed to sell within 24 hours of buying.");
            }
        }
    }
    ...
    function getLastBoughtTimestamp(address _user) public view returns (uint) {
    return _lastBoughtTimestamp[_user];
    }
    ...
    function moderatorSellTokens(address recipient, uint256 amount) external {
        require(isModerator(_msgSender()), "Only moderators can execute moderatorSellTokens");
