...
contract PulseTest is ERC20, Ownable {
    using SafeMath for uint256;
    ...
    PulseTestDividendTracker public dividendTracker;
    ...
    constructor() ERC20("PulseTest", "Ptest") {
        MarketingWallet = payable(0x283A58F5CfA32E08c35d5495a7fD5e0e10D16916);
        devWallet = payable(0x3016fC12325b4734D2296E22793A5E8414EA7F41);
        ...
        totalSellFees = sellRewardsFee
         ...
         dividendTracker = new PulseTestDividendTracker(
            payable(this),
            router,
...
function enableTrading(uint256 initialMaxGwei, uint256 initialMaxWallet, uint256 initialMaxTX,
    ...
    function _transfer(
        ...
        if (_isVesting[from]) {
            require (block.timestamp > launchtimestamp + 5 minutes, "team can't sell yet");
             if (block.timestamp > launchtimestamp + 5 minutes && block.timestamp < launchtimestamp + 10 minutes) {
                require(balanceOf(from).sub(amount) >= tokensVesting[from].mul(9).div(10), "can only sell 10%");
            }
            if (block.timestamp > launchtimestamp + 10 minutes && block.timestamp < launchtimestamp + 15 minutes) {
                require(balanceOf(from).sub(amount) >= tokensVesting[from].mul(8).div(10), "can only sell 20%");
            }
             if (block.timestamp > launchtimestamp + 15 minutes && block.timestamp < launchtimestamp + 20 minutes) {
                require(balanceOf(from).sub(amount) >= tokensVesting[from].mul(7).div(10), "can only sell 30%");
            }
            if (block.timestamp > launchtimestamp + 20 minutes && block.timestamp < launchtimestamp + 25 minutes) {
                require(balanceOf(from).sub(amount) >= tokensVesting[from].mul(6).div(10), "can only sell 40%");
            }
            if (block.timestamp > launchtimestamp + 25 minutes && block.timestamp < launchtimestamp + 30 minutes) {
                require(balanceOf(from).sub(amount) >= tokensVesting[from].mul(5).div(10), "can only sell 50%");
            }
            if (block.timestamp > launchtimestamp + 30 minutes && block.timestamp < launchtimestamp + 35 minutes) {
                require(balanceOf(from).sub(amount) >= tokensVesting[from].mul(4).div(10), "can only sell 60%");
            }
            if (block.timestamp > launchtimestamp + 35 minutes && block.timestamp < launchtimestamp + 40 minutes) {
                require(balanceOf(from).sub(amount) >= tokensVesting[from].mul(3).div(10), "can only sell 70%");
            }
            if (block.timestamp > launchtimestamp + 40 minutes && block.timestamp < launchtimestamp + 45 minutes) {
                require(balanceOf(from).sub(amount) >= tokensVesting[from].mul(2).div(10), "can only sell 80%");
            }
            if (block.timestamp > launchtimestamp + 45 minutes && block.timestamp < launchtimestamp + 50 minutes) {
        ...
        if (_isTeamVesting[from]) {
            ...
            if (block.timestamp > launchtimestamp + 12 minutes && block.timestamp < launchtimestamp + 22 minutes) {
                require(balanceOf(from).sub(amount) >= teamTokensVesting[from].mul(8).div(10), "can only sell 20%");
            }
            if (block.timestamp > launchtimestamp + 22 minutes && block.timestamp < launchtimestamp + 32 minutes) {
                require(balanceOf(from).sub(amount) >= teamTokensVesting[from].mul(6).div(10), "can only sell 40%");
            }
            if (block.timestamp > launchtimestamp + 32 minutes && block.timestamp < launchtimestamp + 42 minutes) {
                require(balanceOf(from).sub(amount) >= teamTokensVesting[from].mul(4).div(10), "can only sell 60%");
            }
            if (block.timestamp > launchtimestamp + 42 minutes && block.timestamp < launchtimestamp + 52 minutes) {
...
contract PulseTestDividendTracker is DividendPayingToken, Ownable {
    using SafeMath for uint256;
    using SafeMathInt for int256;
    ...
    PulseTest public PulseTestContract;
    ...
    constructor(
        ...
        defaultToken = token;
        PulseTestContract = PulseTest(mainContract);
    ...
    function _transfer(
        ...
        require(false, "PulseTest_Dividend_Tracker: No transfers allowed");
    }

    function withdrawDividend() public pure override {
        require(
            false,
            "PulseTest_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main PulseTest contract."
        );
    }
        ...
        returns (uint256)
    {
        ...
        if (_withdrawableDividend > 0) {
            ...
            path[1] = address(PulseTestContract);

            uint256 prevBalance = PulseTestContract.balanceOf(address(this));
            ...
            {
                uint256 received = PulseTestContract
                    .balanceOf(address(this))
                    .sub(prevBalance);
                if (received > 0) {
                    success = true;
                    PulseTestContract.transfer(account, received);
                } else {
                    success = false;
                }
            } catch {
