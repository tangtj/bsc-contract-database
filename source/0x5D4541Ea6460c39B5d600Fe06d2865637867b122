...
interface IFoxPunks is IERC721, IERC721Metadata, IERC2981 {
...
contract FoxPunks is Ownable, ERC721("FoxPunks", "FOXES"), IERC2981, IFoxPunks {
    ...
    constructor(address _feeReceiver, uint256 _fee) {
        ...
        _HASHED_NAME = keccak256(bytes("FoxPunks"));
        ...
        __baseURI = "https://api.foxpunks.com/nft/";
        contractURI = "https://api.foxpunks.com/nft";
    }
    ...
    function permit(
        ...
        require(block.timestamp <= deadline, "FoxPunks: Expired deadline");
        ...
        require(spender != owner, "FoxPunks: Invalid spender");

        if (Address.isContract(owner)) {
            require(
                IERC1271(owner).isValidSignature(digest, abi.encodePacked(r, s, v)) == 0x1626ba7e,
                "FoxPunks: Unauthorized"
            );
        } else {
            address recoveredAddress = Signature.recover(digest, v, r, s);
            require(recoveredAddress == owner, "FoxPunks: Unauthorized");
        }
    ...
    function setMinter(address target, bool _isMinter) external onlyOwner {
        require(isMinter[target] != _isMinter, "FoxPunks: Permission not changed");
    ...
    function _setRoyaltyInfo(address _receiver, uint256 _fee) internal {
        require(_fee < 10000, "FoxPunks: Invalid Fee");
    ...
    function mint(address to) external returns (uint256 id) {
        require(isMinter[msg.sender], "FoxPunks: Forbidden");
        id = totalSupply;
        _mint(to, id);
        totalSupply += 1;
    }
