...
contract BNBFactor{
	...
	function invest(address referrer, address refe,uint8 plan) public payable {
	...
	//	require(msg.value >= INVEST_MIN_AMOUNT);
       // require(plan < 4, "Invalid plan");
		...
		if (user.refe != address(0)) {
			address upline = user.refe;
			for (uint256 i = 0; i < 5; i++) {
				if (upline != address(0)) {
