...
contract LB is Owner {
	...
	address public CoinAddr = 0xF3cdfCB93E8D69b15FbA385A6fBc63825ca67640;
	...
	function Calculation(uint256 _GameId) external lock returns(bool) {
		...
		for(uint i=0;i<GameIdDataIdLength[_GameId];i++){
			...
			if(win_amount>0){
				AmountUserLists[tmp.user_addr]=AmountUserLists[tmp.user_addr].add(win_amount.sub(tmp.amount));
				AmountUserCoinLists[tmp.user_addr]=AmountUserCoinLists[tmp.user_addr].add(tmp.amount);
				
				
				all_win_amount=all_win_amount.add(win_amount.sub(tmp.amount));
			}else{
	...
	function set_admin_amount(address _AdminAddr,uint all_fee,uint all_amount,uint all_win_amount) internal returns(bool) {
		...
		AmountAdminLists[_AdminAddr]=AmountAdminLists[_AdminAddr].sub(all_win_amount); 
		AmountAdminCoinLists[_AdminAddr]=AmountAdminCoinLists[_AdminAddr].add(all_amount); 
