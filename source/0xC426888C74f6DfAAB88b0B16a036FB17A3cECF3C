...
contract TokenStaking is Ownable, ReentrancyGuard, Initializable {
  ...
  function unstake(uint256 _amount) external nonReentrant {
    ...
    if (stakePeriodOver) {
        ...
        // Ensure the user has enough tokens for unstaking
        require(_users[user].stakeAmount >= _amount + earlyUnstakeFee, "TokenStaking: not enough stake to unstake");

        // Update the user's staked amount considering the early unstake fee
        _users[user].stakeAmount -= (_amount + earlyUnstakeFee);

        // Emit event for the fee
        emit EarlyUnStakeFee(user, earlyUnstakeFee);

        // Transfer only the unstaked amount (without the fee) to the user
        IERC20(_tokenAddress).transfer(user, _amount - earlyUnstakeFee);
