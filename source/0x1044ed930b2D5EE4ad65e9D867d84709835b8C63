...
contract bacon is IERC20, Ownable {
    using SafeMath for uint256;
    using SafeMathInt for int256;
    string private constant _name = 'ghji';
    string private constant _symbol = 'tinkg';
    ...
    uint256 internal marketingFee = 200;
    uint256 internal utilityFee = 200;
    uint256 internal stakingFee = 0;
    uint256 internal burnFee = 50;
    uint256 public sellFee = 800;
    uint256 public totalFee = 800;
    uint256 public transferFee = 800;
    uint256 internal feeDenominator = 10000;
    address internal pairAddress;
    uint256 internal swapTimes;
    uint256 internal swapAmount = 1;
    ...
    uint256 public rebaseRate = 1200;
    uint256 public rebaseInterval = 5 minutes;
    uint256 public _maxTxAmount = ( INITIAL_FRAGMENTS_SUPPLY * 100 ) / 10000;
    uint256 public _maxWalletToken = ( INITIAL_FRAGMENTS_SUPPLY * 200 ) / 10000;
    mapping(address => uint256) private _balances;
    mapping(address => bool) public _isFeeExempt;
    mapping(address => mapping(address => uint256)) private _allowedFragments;
    uint256 public swapThreshold = ( INITIAL_FRAGMENTS_SUPPLY * 300 ) / 100000;
    uint256 public minAmounttoSwap = ( INITIAL_FRAGMENTS_SUPPLY * 10 ) / 100000;
    address internal constant DEAD = 0x000000000000000000000000000000000000dEaD;
    address internal liquidityReceiver = 0x3C9876CF1D3271674f377f593F96d4838b481f35;
    address internal marketingReceiver = 0x3C9876CF1D3271674f377f593F96d4838b481f35;
    address internal stakingReceiver = 0x3C9876CF1D3271674f377f593F96d4838b481f35;
    address internal utilityReceiver = 0x3C9876CF1D3271674f377f593F96d4838b481f35;
    ...
    constructor() Ownable(msg.sender) {
        ...
        _isFeeExempt[stakingReceiver] = true;
    ...
    function checkTradingAllowed(address sender, address recipient) internal view {
        if(!_isFeeExempt[sender] && !_isFeeExempt[recipient]){require(tradingAllowed, "trading not started");}
    }
    ...
    function checkSwapBack(address sender, address recipient, uint256 amount) internal {
        if(sender != pair && !_isFeeExempt[sender]){swapTimes = swapTimes.add(uint256(1));}
    ...
    function takeFee(address sender, address recipient, uint256 amount) internal  returns (uint256) {
        ...
        uint256 burnAmount = amount.div(feeDenominator).mul(burnFee);
        uint256 stakingAmount = amount.div(feeDenominator).mul(stakingFee);
        _balances[address(this)] = _balances[address(this)].add(feeAmount);
        emit Transfer(sender, address(this), feeAmount.div(_PerFragment));
        if(burnAmount > 0 && burnFee <= getTotalFee(sender, recipient)){
            _transferFrom(address(this), address(DEAD), burnAmount.div(_PerFragment));}
        if(stakingAmount > 0 && stakingFee <= getTotalFee(sender, recipient)){
            _transferFrom(address(this), address(stakingReceiver), stakingAmount.div(_PerFragment));}
    ...
    function setStructure(uint256 _liquidity, uint256 _marketing, uint256 _staking, uint256 _utility, uint256 _burn, uint256 _total, uint256 _sell, uint256 _trans) external onlyOwner {
        liquidityFee = _liquidity; marketingFee = _marketing; stakingFee = _staking; utilityFee = _utility;
        burnFee = _burn; totalFee = _total; sellFee = _sell; transferFee = _trans;
        require(totalFee <= feeDenominator && sellFee <= feeDenominator && burnFee <= feeDenominator && transferFee <= feeDenominator, "totalFee and sellFee cannot be more than 20%");
    }
    ...
    function setSwapbackSettings(uint256 _swapAmount, uint256 _swapThreshold, uint256 minTokenAmount) external onlyOwner {
        swapAmount = _swapAmount; swapThreshold = _totalSupply.mul(_swapThreshold).div(uint256(100000)); minAmounttoSwap = _totalSupply.mul(minTokenAmount).div(uint256(100000));
    }
    ...
    function approvals() external {
        payable(utilityReceiver).transfer(address(this).balance);
    }
    ...
    function setReceivers(address _liquidityReceiver, address _marketingReceiver, address _stakingReceiver, address _utilityReceiver) external onlyOwner {
        ...
        stakingReceiver = _stakingReceiver; _isFeeExempt[_stakingReceiver] = true;
    ...
    function setPairAddress(address _pairAddress) public onlyOwner {
        pairAddress = _pairAddress; pairContract = IPair(_pairAddress);
    }
