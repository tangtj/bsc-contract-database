...
contract HDCToken is ERC20, Ownable {
    ...
    bool public isOpendTrade = false;
    ...
    constructor( 
        ...
        _mint(_managerAddress, 3330000 * 10 ** 18);
    }

    function updatePancakeRouter(address newAddress) public onlyOwner {
        require(newAddress != address(pancakeRouter), "HDC: The router already has that address");
    ...
    function setIsOpendTrade(bool _isOpend) external onlyOwner {
        isOpendTrade = _isOpend;
    }
    ...
    function _transfer(
        ...
        if(from == pancakePair) { //buy
            require(isOpendTrade, "Not open for sale");
            if(takeFee) {
                uint256 totalFees = amount.mul(buyFee).div(1000);
                amount = amount.sub(totalFees);
                super._transfer(from, fundAddress, totalFees);
            }
        } else if(to == pancakePair && takeFee) { //sell
            uint256 totalFees = amount.mul(sellFee).div(1000);
            amount = amount.sub(totalFees);
            super._transfer(from, fundAddress, totalFees);
        }
