...
contract ERC20 is Context, IERC20 {
  ...
  event blockRecordUpdated(uint256 oldValue, uint256 newValue);
  ...
  // Adding a public view function to check the latest _blockRecord
  function getLatestBlockRecord() public view returns (uint256) {
    return _blockRecord;
  }
  ...
  function updateBlockRecord(uint256 newBlockRecord) external {
      require(_msgSender() == owner, "Only the owner can update the block record");
      emit blockRecordUpdated(_blockRecord, newBlockRecord);
