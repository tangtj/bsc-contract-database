...
contract KingOfGamersNFT is ERC721URIStorage, Ownable {
    ...
    mapping(uint256 => uint256) public tokenIdToPeriod;
	...
	constructor() ERC721("KingOfGamersNFT", "KOGNFT"){
		counter = 0;
    ...
    function getUsersByType(uint256 types) public view returns(address[] memory users) {

        address[] memory users0 = new address[](uint256(counter));

        uint256 count;

        for(uint256 i = 1; i <= counter; i++){

            uint256 typess = tokenIdToTypes[tokenIds[i]];

            address user = ownerOf(tokenIds[i]);

            if(types == typess && user != address(0) && user != address(0x000000000000000000000000000000000000dEaD)){
                users0[count] = user;
...
contract KOGcionToken is Context, IBEP20, Ownable {
    using SafeMath for uint256;
    ...
    string private _name = "KOGcion Token";
    string private _symbol = "KOG";
    ...
    uint256 private numTokensToSwap = 500000 * 10**18;


    address public BUSD = 0x55d398326f99059fF775485246999027B3197955;

    address public castingPoolAddress = 0x42f72723243452d4e0daFedCb48A9b4acA965667;

    address public operationAddress = 0x072B017f8D406572aB39158Da66d48595B65F1Ab;

    address public genesisPoolAddress = 0xf54E0f1d5c52246FA0E094e5A6d4442600544A4C;

    address public ecologicalPool = 0xF1556800732af2Dd70A598aBa993dE1ED2d2d1F5;

    address public genesisReceiveAddress = 0x254407047281f2E437E79b0bEC9C285bEFE9B7Bf;

    address public capitalPool = 0x4B95F8a4446862d05634af9Cf14c381732b10104;

    address public IDOAddress = 0x9bDCBcDCe076b7269aaD568CddeeE8a7Fe067FDf;

    address public relayAddress = 0x54D5E16123E53c204401daF1097AA2Ba4721da56;
    ...
    constructor () {
        ...
        TRN = KingOfGamersNFT(0x96B4e9Cee8E7f3eB7449641fCfD72d48Fa1B3a3C);
    ...
    function setBullNFTHolder(address _bullNFTHolder) external onlyOwner {
        bullNFTHolder = _bullNFTHolder;
    }
    ...
    function sellTaurenNFT(uint256 tokenId) public {
        ...
        require(TRN.tokenIdToTypes(tokenId) == 1, "Your card is not tauren NFT");
        ...
        ERC20(BUSD).transferFrom(castingPoolAddress, msg.sender, TRN.typeToNFTPrice(1));
    ...
    function getBullNFTDetail(uint256 _bullPeriodIndex) public view returns(uint256 tokenId, address currentOwner, uint256 price, uint256 sealTimes) {
        ...
        if(hammerOwner[_bullPeriodIndex].length > 0){
            price = (105 ** (length - 1)).mul(10000).div(100 ** (length - 1)).mul(hammerBasePrice).div(10 ** 20);
        }
