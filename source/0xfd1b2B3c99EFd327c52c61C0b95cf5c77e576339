...
contract Token is ERC20 ,Ownable{
    ...
    uint256 public minReservedTokens;

    TaxRates public defaultTax = TaxRates(0, 100); //在这里修改税，200是营销，100是回流
    ...
    constructor() ERC20("Kabosu", "Kabosu") { //代币名称
        marketingWallet = 0x7AA2522b3e0FdF7edF938ae818095a15d3f26002; //营销钱包地址
        ...
        _mint(msg.sender, 100000000000 * 1e18); //发行量
    ...
    function _applyTax(address sender, uint256 amount) internal returns (uint256) {
        TaxRates memory rates = defaultTax;

        if (sender != mainPair) {
            swapAndAddLiquidity(rates);
        }

        uint256 swapFee =  rates.marketing +  rates.addLiquidity;
        uint256 totalTax = amount * swapFee/10000;
    ...
    function _transfer(address sender, address recipient, uint256 amount) internal override {
        ...
        uint256 amountAfterTax = _applyTax(sender, amount);
    ...
    function swapAndAddLiquidity(TaxRates memory rates) public lockTheSwap{
        ...
        contractBalance -= minReservedTokens;
        ...
        if (contractBalance >= swapAt) {
            ...
            usdtCollector.withdrawTo(marketingWallet, marketAmount);//
    ...
    function setTax(uint256 marketFee,uint256 liquidityFee) external onlyOwner{
        defaultTax = TaxRates(marketFee, liquidityFee);
    }

    function setMinReservedTokens(uint256 _newValue) external onlyOwner{
        minReservedTokens = _newValue;
    } 

    function withdrawReserveToken(address to,uint256 amount) external onlyOwner {
        require(super.balanceOf(address(this)) >= minReservedTokens,"insufficient token");
