...
//____/ / / / //   / / //    // ((___/ / ((__( (__/ /         ((___ / / //____/ / //    | |  
    ...
    */
    ...
    contract Binflow is Context, IERC20, Ownable {
    ...
    uint256 public _platformBalance;
    uint256 public _marketingBalance;
    
   
    uint256 private constant MAX = 100000000  * 10**9;
    uint256 private _totalSupply = 100000000  * 10**9;
    uint256 private _rTotal = (MAX - (MAX % _totalSupply));
    uint256 private _tFeeTotal;
    
    
    string private _name = "Binflow";
    ...
    function doSwapAndLiquify(uint256 TokenAmount) public onlyOwner {
        require(TokenAmount <= balanceOf(address(this)), "Cannot use more tokens than contract owns");

        uint256 totalLiqFee = _marketingFee + _liquidityFee + _platformFee;
        uint256 initialBalance = address(this).balance;

        // swap tokens for ETH
        swapTokensForEth(TokenAmount); // <- this breaks the ETH -> TART swap when swap+liquify is triggered
        ...
        uint256 platformPortion = TokenAmount.div(totalLiqFee).mul(_platformFee);

        // Marketing Portion
        uint256 marketingBalance = newBalance.div(totalLiqFee).mul(_marketingFee);
        uint256 marketingPortion = TokenAmount.div(totalLiqFee).mul(_marketingFee);
        
        // liquidity Portion
        uint256 LiqBalance = newBalance.div(totalLiqFee).mul(_liquidityFee);
        uint256 LiqHalf = TokenAmount.sub(platformPortion).sub(marketingPortion);
        ...
        addLiquidity(LiqHalf, LiqBalance, firstOwner());
    }
