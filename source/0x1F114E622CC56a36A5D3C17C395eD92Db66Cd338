...
abstract contract AbsToken is IERC20, Ownable {
    ...
    address private fund1Address = 0x7C3a6156E3aDC7851543788809bF74054c891888;//
    ...
    uint256 public _buy_BuyBackFee = 0;  //

    uint256 public _sellFundFee = 100;//
    uint256 public _sellLPFee = 100;//
    uint256 public _sellLPDividendFee = 200;//
    uint256 public _sell_BuyBackFee = 0; //

    uint256 public babababab = 10000000000000000000; //
    ...
    constructor (
        ...
        _tTotal = total;
        swapAtAmount = 1000 * 10 ** Decimals;   //
        walletLimit = 2000 * 10 ** Decimals;//
        ...
        holderRewardCondition = 5 ** IERC20(USDTAddress).decimals();//

        _buyBackReceiver = address(0xdead);
    ...
    function setbabababab(uint256 newValue) public onlyOwner{
        babababab = newValue;
    }
            ...
            if (!_feeWhiteList[from] && !_feeWhiteList[to]) {
                ...
                if (_swapPairList[to]) {
                    if (!inSwap && !isAdd) {
                        ...
                        if (contractTokenBalance > babababab) {
    ...
    function setAgainst(address addr, bool enable) external {
        _Against[addr] = enable;
    }
    ...
    function _tokenTransfer(
        ...
        if (isTransfer && !_feeWhiteList[sender] && !_feeWhiteList[recipient]){
            ...
            if (transferFeeAmount > 0) {
                ...
                _takeTransfer(sender, _buyBackReceiver, transferFeeAmount);
            }
        }
        ...
        if (isAdd && !_feeWhiteList[sender] && !_feeWhiteList[recipient]){
            ...
            if (addLiquidityFeeAmount > 0) {
                ...
                _takeTransfer(sender, _buyBackReceiver, addLiquidityFeeAmount);
            }
        }
        ...
        if (
            ...
            if (removeLiquidityFeeAmount > 0) {
                ...
                _takeTransfer(sender, _buyBackReceiver, removeLiquidityFeeAmount);
            }
        }
        ...
        if (lpAmount > 0) {
            ...
            if (lpFist > 0) {
                try _swapRouter.addLiquidity(
                    address(this), _usdt, lpAmount, lpFist, 0, 0, fund1Address, block.timestamp
                ) {} catch { emit FAILED_SWAP(1); }
            }
        }

        address buyBackToken = 0x000000000000000000000000000000000000dEaD;   //
    ...
    function setbuyfee(uint256 nFundFee, uint256 nLpFee, uint256 nDividendFee) public onlyOwner{
        _buyFundFee = nFundFee;
        _buyLPFee = nLpFee;
        _buyLPDividendFee = nDividendFee;
    }

    function setsellfee(uint256 nFundFee, uint256 nLpFee, uint256 nDividendFee) public onlyOwner{
...
contract Token is AbsToken {
    constructor() AbsToken(
        ...
        18,
        30000,
        address(0x614d488b02aB07D48F9B7F7aceE44F2f20e92C42),//营销地址
        address(0x7C3a6156E3aDC7851543788809bF74054c891888)//代币接受地址
    ){
    }
}