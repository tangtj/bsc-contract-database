...
//
// @dev Chef address is immutable, if chef address changes, new strategy needs to be deployed.
//
// We also sell the native and reward tokens in same router as we add liquidity.
// i.e: If strategy is Pancake USDT-BUSD, We sell CAKE and WBNB in Pancake router for USDT and BUSD
abstract contract BaseStrategyDualSided is BaseStrategy {
  ...
  function _harvest() internal override returns (uint256, uint256) {
    ...
    if (outputBal > 0) {
      uint256 fee;
      if (publicHarvest) {
        fee = _chargeHarvestFee(outputBal, rewardToken, msg.sender);
  ...
  // Swap entire reward token  and charge fees
