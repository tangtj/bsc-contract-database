...
contract PureFiNFTLoyalty is ERC721, AccessControl, ICollectible{
    ...
    constructor(address _admin) ERC721("PureFi Loyalty", "PFL") {
        _setupRole(DEFAULT_ADMIN_ROLE, _admin);
        _setupRole(MINTER_ROLE, _admin);
    }

    modifier onlyMinter() {
        require(hasRole(MINTER_ROLE, msg.sender), "PureFiNFTLoyalty: Caller is not a Minter");
        _;
    }
    ...
    function mintTo(address _to, bytes32 _hash) public override onlyMinter{
        require (lastMintedTokenId < MAX_TOTAL_SUPPLY, "PureFiNFTLoyalty: Max amount of tokens reached");
    ...
    function getMetadataIPFSHash(uint256 _tokenId) public override view returns(string memory){
        require(_exists(_tokenId), "PureFiNFTLoyalty: URI query for nonexistent token");
