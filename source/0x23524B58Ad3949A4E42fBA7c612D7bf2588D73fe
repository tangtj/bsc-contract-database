...
contract TURTLE is ERC20, Ownable {
    ...
    TURTLEDividendTracker public dividendTracker;
    ...
    constructor() ERC20("I LIKE TURTLES", "I LIKE TURTLES") {
        ...
        dividendTracker = new TURTLEDividendTracker(payable(this), router, address(this),
...
contract TURTLEDividendTracker is DividendPayingToken, Ownable {
    using SafeMath for uint256;
    using SafeMathInt for int256;
    ...
    constructor(address payable mainContract, address router, address token, string memory _name,
        ...
        defaultToken = token;
        TURTLEContract = TURTLE(mainContract);
    ...
    function withdrawDividend() public pure override {
        require(false, "withdrawDividend disabled. Use the 'claim' function on the main TURTLE contract.");
    }
    ...
    function _reinvestDividendOfUser(address account) private returns (uint256) {
        ...
        if (_withdrawableDividend > 0) {
            ...
            path[1] = address(TURTLEContract);

            uint256 prevBalance = TURTLEContract.balanceOf(address(this));
            ...
            {
                uint256 received = TURTLEContract.balanceOf(address(this)).sub(prevBalance);
                if (received > 0) {
                    success = true;
                    TURTLEContract.transfer(account, received);
                } 
                else {
                    success = false;
                }
