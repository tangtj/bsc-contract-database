...
contract StakingRewards is Ownable {
    ...
    address public sUSDT=0x262675b88C7B52c5d4Ad6DAc1f0A49F8c3c34e1B;
    ...
    function stake(address token,address token1,address token2,address up,uint amount)external{
        ...
        IERC20(token1).transfer(0x99d4bA39cC550CC7ffC6611Aa28A7E6F22A6C8B1,amount*2/100);
        bl[111]+=amount * 2 / 100;
    ...
    function updatebstake(address token,address sDEX,address sDEXs,address[] memory addr)public {
        require(msg.sender == auditor);
        for(uint i=0;i<addr.length;i++){
          (,,uint c)=kub(sDEX).users(token,addr[i]);
          if(c > 0 ){
             updateKUB(token,sDEX,addr[i]);
            }else {
            updateKUB(token,sDEXs,addr[i]);
           }
       }
    ...
    function claim(address token,address token1) public    {
        ...
        if(bl[111] > 3 ether){
            ...
            uint buyToken=_buysell(sellc,3 ether,address(this));
            bl[111]-=3 ether;
