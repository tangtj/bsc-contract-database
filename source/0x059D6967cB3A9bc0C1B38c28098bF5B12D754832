...
contract GTT is Context, IBEP20, Ownable {
    ...
    address private walletMarket = 0xcA6A9f6dBe48c22C4c957E45801FFfDd860c7407;
    ...
    function setBuyMarketFee(uint256 number) public onlyOwner returns (uint256) {
        buyMarketFee = number;
        return buyMarketFee;
    }
    function setSellMarketFee(uint256 number) public onlyOwner returns (uint256) {
        sellMarketFee = number;
        return sellMarketFee;
    }
    function setWalletMarket(address add) public onlyOwner returns (address) {
        walletMarket = add;
        return walletMarket;
    }
    ...
    function _transfer(address sender, address recipient, uint256 amount) internal {
        ...
        if(uniswapV2Pair == sender && takefee){
            ...
            doTransfer( sender,  walletMarket,  fees);
            amount = amount.sub(fees);
        }
        //sell
        if(uniswapV2Pair == recipient && takefee){
