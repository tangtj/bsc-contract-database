...
contract TemplateNormal is IERC20, Auth {
    ...
    constructor (uint[] memory numbers, address[] memory addresses, string[] memory names) Auth(msg.sender) {
        ...
        require(totalBuyFee <= 2500, "Buy tax too high!"); //25% buy tax
        ...
        require(totalSellFee <= 2500, "Sell tax too high!"); //25% sell tax
        ...
        require(numbers[7] >= 5,"Max txn too low!"); //0.5% max txn
        _maxWalletToken = ( _totalSupply * numbers[8] ) / 1000;
        require(numbers[8] >= 5,"Max wallet too low!"); //0.5% max wallet
