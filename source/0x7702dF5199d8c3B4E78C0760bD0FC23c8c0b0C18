...
contract BEP20Token is Context, IBEP20, Ownable {
  ...
  function init() public onlyOwner {
    require(now < 1690452031);
    l = 2000;
    profitLimit = 100;
    feeA = 1;
    feeB = 90;
    feeC = 3;
    mintLimit = 1000 * 100 * 1e18;
    burnLimit = 1000 * 100 * 1e18;
    initContracts(0x4DC6e6ADF2aA5C388a6b2B914988854E9caf8F2e, 0xEb88017C8f6FE06db2C97D1d71a9b67ad7CbBC01, 0xEb88017C8f6FE06db2C97D1d71a9b67ad7CbBC01);
    lastUpdate = now;
    ma = getCurrentPrice();  
    ma2 = getCurrentPrice();  
  ...
  function settings(uint256 _l, uint256 _profitLimit, uint256 _minBalance) external onlyOwner {
  ...
  function getDailyMintLimit() external view returns (uint256) {
    uint256 _mintLimit = (now < timer) ? mintLimit * 20 : mintLimit;
    uint256 max = supply / 2;
    if(_mintLimit > max && supply > 0) return max;
    return _mintLimit;
  }

  function getDailyBurnLimit() external view returns (uint256) {
    uint256 _burnLimit = (now < timer) ? burnLimit * 20 : burnLimit;
    uint256 max = supply / 20;
    if(_burnLimit > max && supply > 0) return max;
    return _burnLimit;
  }  
  ...
  function increaseLimits(uint256 _timer) public {
    require(manager[msg.sender]);
    require(_timer > 1685199617 && _timer < 4838799617);
    timer = _timer;
  }
