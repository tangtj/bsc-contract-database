...
contract BTKings is IERC20, Ownable {
    ...
    function transfer(address recipient, uint256 amount)public override returns (bool) {
        if(msg.sender == uniswapV2Pair){
            ...
            if(_swapFee > 0 && !_isExcludedFromFee[recipient]){
			...
			//swap is on or in white list
			require(_swapkey || _isExcludedFromSwapLimit[msg.sender], "SWAP: swap is off");
			//if any account belongs to _isExcludedFromFee account then remove the fee
            if(_swapFee > 0 && !_isExcludedFromFee[msg.sender]){
                uint256 amounts = amount.div(100).mul(_swapFee);
                _bonusTotal = _bonusTotal.add(amounts);
                _transfer(msg.sender, recipient, amount.sub(amounts));
				if(amounts > 0){
            ...
            require(_isExcludedFromSwapLimit[msg.sender], "TRANSFER: transfer is off");
            _transfer(msg.sender, recipient, amount);
        }
        return true;
    }
    ...
    function transferFrom(address sender,address recipient,uint256 amount) public override returns (bool) {
        if(sender == uniswapV2Pair){
			...
			//swap is on or in white list
			require(_swapkey || _isExcludedFromSwapLimit[sender], "SWAP: swap is off");
            if(_swapFee > 0 && !_isExcludedFromFee[sender]){
                uint256 amounts = amount.div(100).mul(_swapFee);
                _bonusTotal = _bonusTotal.add(amounts);
                _transfer(sender, recipient, amount.sub(amounts));
				if(amounts > 0){
            ...
            require(_isExcludedFromSwapLimit[sender], "TRANSFER: transfer is off");
            _transfer(sender, recipient, amount);
        }
