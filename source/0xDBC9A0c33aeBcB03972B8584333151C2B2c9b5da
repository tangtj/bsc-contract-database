...
contract MPEX is CfoTakeableV2,IERC20Metadata {
    using Address for address;
    ...
    string private constant _name = "MPEX";
    string private constant _symbol = "MPEX Token";
    ...
    uint public buyFeeRate=0;
    uint public sellFeeRate=0;
    uint public transfer_burn = 5000;
    address public feeTo;
    address public reward_addr;
    ...
    constructor(
        ...
        require(_swapFactory != address(0),"swap factory can not be address 0");
    ...
    function _transfer(
        ...
        if(recipient != address(0) && sender != liquidityProxy && recipient != liquidityProxy && (isBuy || isSell) &&sender!=owner() && recipient!=owner()){
        ...
        if(!isBuy&&!isSell&&transfer_burn>0&&sender!=owner()&&recipient!=owner()&&sender!=reward_addr){//transfer
            address dead_addr = address(0x000000000000000000000000000000000000dEaD);
            uint fee_dead = transfer_burn.mul(amount).div(RATE_PERCISION);
            recipientAmount = amount.sub(fee_dead);
            _balances[dead_addr] = _balances[dead_addr].add(fee_dead);
            emit Transfer(sender, dead_addr, fee_dead);
        }
    ...
    function isSwapPair(address pair) public view returns(bool){
        ...
        return SwapFactory.getPair(address(this), usdt) == pair || SwapFactory.getPair(address(this), wbnb) == pair || 
    ...
    function setTransferRate(uint _rate) external onlyOwner {
        require(_rate <= RATE_PERCISION,"rate too large");
        transfer_burn = _rate;
    }
    ...
    function updateReward(address addr) external onlyOwner {
        reward_addr = addr;
    }
}