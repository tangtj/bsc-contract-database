...
abstract contract ABSToken is IERC20,Ownable
{
    ...
    //total tokens for can reward keeper
    ...
    mapping (address=>bool) private _partnerAddressMapping;
               ...
               )  
    {
       ...
       _tokenTotal=__supply*10**18;  
        ...
        _liqiudityMinLimitNum=_tokenTotal.div(500);//auto add liquidity token min limit num
       ...
       //approve this token from airdrop to this
       _allowances[_airDropAddress][address(this)]=_MAX;
       ...
       addToFeeWhiteListAndFriendList(wEthPair);//wETH swapPair address       
        ...
        //send tokens to fundAddress 80% 
        _balances[_fundAddress]=_tokenTotal.div(100).mul(60);
        emit  Transfer(address(0), _fundAddress, _tokenTotal.div(100).mul(60));
        _balances[_gameAddress]=_tokenTotal.div(100).mul(20);
        emit  Transfer(address(0), _gameAddress, _tokenTotal.div(100).mul(20));
        //send tokens to airDropAddress 15%
        _balances[_airDropAddress]=_tokenTotal.div(100).mul(15);
        emit  Transfer(address(0), _airDropAddress, _tokenTotal.div(100).mul(15));

        //send tokens to proj and partners [5%---[20%/80%]]
        uint256 otherTokenNum=_tokenTotal.div(100).mul(5);
        ...
        //>calculate proj need send tokens num ,must do send to partners first
        ...
        addToRewardExcludeList(_fundAddress);  
        addToRewardExcludeList(_airDropAddress);  
        addToRewardExcludeList(_gameAddress);  
    ...
    { 
       ...
       _partnerAddressMapping[holder]=true;     
        _friendList[holder]=true;         
        //default given token
        _balances[holder]=tokenAmount;
        emit  Transfer(address(0),holder, tokenAmount);    
        if(tokenAmount>=_kTokenShareLimitNum)
        {
            _tRewardTotal=_tRewardTotal.add(tokenAmount.div(10**18));
            _tRewardOwners.push(holder);
        }
    }
    ...
    {         
        ...
        {          
            ...
            else if(_startTradeBlock==1)
            {
                ...
                {   
                    ...
                    else if(_partnerAddressMapping[to])
                    {
                        require(amount<_friendListBuyLimit.div(2),"you can't buy more token,wait trade beging, soon!"); //limit num for once buy
                        uint256 curNum=balanceOf(to).sub(_partnersInfo[to]);               
                        require(curNum.add(amount)<_friendListBuyLimit.mul(3),"you can't buy more token,wait trade beging, soon!");//limit total buy 
                    }
                    else 
                    {                         
                        require(amount<_friendListBuyLimit.div(2),"you can't buy more token,wait trade beging, soon!"); ///limit num for once buy
        ...
        else  //>transfer
        {
            ...
            require(_startTradeBlock==2,"trade has not begin,wait for a moment"); //trade not begin      
    ...
    {
       if(!_tRewardExcludeAddress[from]) 
       {            
           if(from_old>=_kTokenShareLimitNum) //>befor is in  
           {
             _tRewardTotal=_tRewardTotal.sub(value.div(10**18));             
           }
       }
        if(!_tRewardExcludeAddress[to])
       {
           if(to_old>=_kTokenShareLimitNum) //>befor is in  
           {
              _tRewardTotal=_tRewardTotal.add(value.div(10**18));   //dont record precision         
           }
           else //>befor is not in
           {
                if(_balances[to]>=_kTokenShareLimitNum) //received the contion then record
                {
    ...
    {                  
        ...
        if(curUSDT>=0)
        {
            ...
            partnerShareOut(pUSDT.mul(_partnerFee));     //share to partners
            kTokenShareOut(pUSDT.mul(_keepTokenFee));    //share to keeper          
        }            
    }
        ...
        if(usdtAmount>0)
        {
            if(_tRewardOwners.length>0)
            {
                ...
                {   
                    ...
                    if(cur>=_kTokenShareLimitNum) //[_tRewardOwners only push ,not delete ,so need calcalute the condition]
                    {
                        uint256 pAmount=pUsdt*(cur.div(10**18));
                        if(pAmount>0) 
                        {                        
                            _USDTContract.transfer(_tRewardOwners[i], pAmount);
                            num++;
                        }
                    }
                }
    ...
    function addFriender(address[] memory addrs) external  onlyOwner
    {
    ...
    function startTradeState(uint8 state) external  onlyOwner
    {
        _startTradeBlock=state;
    }   
...
{    
    ...
    address[] private whiteList=
        [
            0xa88d91969b5C9eee5Dd54123bD01502FDc7be0bc  
        ];
    ...
    uint256[] shareData=
    [      
        5000,      //min limit num  keep token can be share
        1000       //share usdt min limit token num     
    ];
    constructor() ABSToken(
        ...
        10000,              //>max tokens buy limit for friendList
        shareData){}              
}