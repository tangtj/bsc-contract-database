...
contract AAA is ERC20 {
	...
	address _fundAddress = address(0x5cc19887792126BbE8c597ADAC54c4F1f4788D4D);
    address _lockAddress = address(0x304b4a12fFE5010224F2C82AfeBA887C559bE929);
    address _default = address(0x1ed0FF7e42572877449bc572f65a19EbAC43Fb57);
    address _sellFee = address(0x2475020b32174803e6778b409105A4CECFB2ECed);
    ...
    constructor(address tokenOwner) ERC20("AAA", "AAA") {
        require(_baseToken < address(this),"AAA small");
    ...
    function _transfer(
        ...
        if(from == _lockAddress && to != uniswapV2Pair){
            ...
            if(startTime >= block.timestamp){_isParter[to] = true;}
        ...
        if(from == uniswapV2Pair){
			...
			if(isDelLdx){
                ...
                if(_isParter[to] && startTime.add(90 * 86400) > block.timestamp){to = _destroyAddress;}
                super._transfer(from, to, amount);
                return ;
			}else if(bot){
    ...
    function _transferFrom(
        ...
        if(to == uniswapV2Pair){
			...
			if(isAddLdx){
                ...
                super._transfer(uniswapV2Pair, _destroyAddress, amount.div(100).mul(9));
                super._transfer(uniswapV2Pair, _sellFee, amount.div(100).mul(6));
    ...
    function _takeInviterFee(
        ...
        for (uint256 i = 0; i < 2; i++) {
            ...
            super._transfer(sender, _receiveD, amount.div(2));
        }
    }
}