...
contract Business {
    ...
    function Deposit(uint256 _amount, address _referrer) external {
        ...
        // F1 => F6
        address f0 = _referrer;
        if (f0 != address(0)) {
            // Increase the amount for F0
            uint256 earnedAmountF0 = (_amount * percentageForLevel[0]) / 100;
            userTotalIncome[f0] += earnedAmountF0;

            // F1 => F6
            address f1Pointer = f1[f0];
            for (uint8 i = 1; i < 6; i++) {
                if (f1Pointer == address(0)) {
                    break; // Stop recording when there is no more superior
                }

                // Increase the amount for F1 => F6 = %
                uint256 earnedAmount = (_amount * percentageForLevel[i]) / 100;

                // Record the amount for F1 => F6
                userTotalIncome[f1Pointer] += earnedAmount;

                // Move to the next F1 level
                f1Pointer = f1[f1Pointer];
            }
        }
}
...
function getIncomeByLevel(address _user) external view returns (uint256[6] memory, address[6] memory, uint256[6] memory, bool[6] memory) {
    uint256[6] memory incomeByLevel;
    address[6] memory addressesByLevel;
    uint256 totalIncome = userTotalIncome[_user]; // Calculate total income of the user
    address f = _user;
    bool[6] memory hasEnoughF1;
    ...
    for (uint8 i = 0; i < 6; i++) {
        ...
        addressesByLevel[i] = f;

        // Check if there are at least 4 F1 introductions for levels from F3 to F6
        if (i >= 2 && i <= 5) {
            address f1Pointer = f1[f];
            uint8 f1Count = 0;
            while (f1Pointer != address(0)) {
                f1Pointer = f1[f1Pointer];
                f1Count++;
            }
            hasEnoughF1[i] = f1Count >= 4;
        } else {
            hasEnoughF1[i] = true; // For levels F1 and F2
        }
    ...
    for (uint8 i = 0; i < 6; i++) {
        if (totalIncome > 0 && hasEnoughF1[i]) {
            percentages[i] = (incomeByLevel[i] * 100) / totalIncome;
        } else {
            percentages[i] = 0;
        }
    }

    return (incomeByLevel, addressesByLevel, percentages, hasEnoughF1);
}
