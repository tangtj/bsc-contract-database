...
// File: contracts/GoldenGoose/GoldenGoosev5.4.sol
...
contract InvestmentContract is AccessControl, Pausable {
    ...
    //SpreadLoss Variables
    uint256 public spreadLossTimestamp;
    uint256 public spreadLossAmount;
    uint256 public spreadLossTotalDeposits;
    
    //Unstake pause Variables
    bool public unstakeDisabled;
    ...
    event UnstakeDisabled(address wallet);
    event UnstakeEnabled(address wallet);
    ...
    function requestWithdrawal(uint256 amount) public  {
        require(!unstakeDisabled, "Unstake is disabled");
    ...
    function processMultipleWithdrawals(uint256[] memory requestIds) public onlyRole(OWNER_ROLE) {
        ...
        for(uint256 i = 0; i < requestIds.length; i++) {
            ...
            if(_withdrawalRequests[requestIds[i]].timestamp <= spreadLossTimestamp) {
                withdrawalAmount -= (withdrawalAmount * spreadLossAmount) / spreadLossTotalDeposits;
            }
    ...
    function spreadLoss(uint256 loss, uint256 batchSize) public onlyRole(OWNER_ROLE) {
        ...
        for (uint256 i = currentSpreadInvestorIndex; i < investors.length && processed < batchSize; i++) {
            ...
            userBalanceByAddress[investor].processing -= (userBalanceByAddress[investor].processing * loss) / totalDeposits;
            processed++;
        }
        ...
        if (currentSpreadInvestorIndex >= investors.length) {
            ...
            spreadLossTimestamp = block.timestamp;
            spreadLossAmount = loss;
            spreadLossTotalDeposits = totalDeposits;
    ...
    function getWithdrawalsRequest(uint256 count) external view returns(WithdrawalRequest[] memory) {
        ...
        for (uint i = 0; i < count; i++) {
            ...
            if( withdrawals[i].timestamp <= spreadLossTimestamp) {
                withdrawals[i].amount -= (withdrawals[i].amount * spreadLossAmount) / spreadLossTotalDeposits;
            }
        }
    ...
    function disableUnstake() public onlyRole(OWNER_ROLE) {
        require(!unstakeDisabled, "Unstake is disable");
        unstakeDisabled = true;
        emit UnstakeDisabled(msg.sender);
    }

    function enableUnstake() public onlyRole(OWNER_ROLE) {
        require(unstakeDisabled, "Unstake is enable");
        unstakeDisabled = false;
        emit UnstakeEnabled(msg.sender);
    }
