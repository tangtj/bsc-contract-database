    ...
    Ownable
{
    ...
    constructor() {
        ...
        rewardTokens.push(address(0x2859e4544C4bB03966803b044A93563Bd2D0DD4D)); // USDC
...
contract Evo is ERC20, Ownable {
    ...
    uint256 public operationsTransferFee;
    ...
    constructor() ERC20("Evo", "$EVO") {
        ...
        swapbackTreshold = (totalSupply * 5) / 10000; // 0.05% swapback trigger
        ...
        operationsTransferFee = 0;
        devTransferFee = 0;
        liquidityTransferFee = 0;
        totalTransferFees =
            marketingTransferFee +
            rewardsTransferFee +
            operationsTransferFee +
            liquidityTransferFee +
            devTransferFee;

        dividendTracker = new DividendTracker();

        mktWallet = address(0x50F988Ada753Cb16Cd5f700aFFe42E5D76433500); // set as marketing wallet
        operationsWallet = address(0x2dA8AA4B277300e7EE5E8270994d34ffdF51eA4C); // set as operations wallet
        devWallet = address(0x5E64efa28eEB99fBE9445a077333dC365Fa73718); // set as dev wallet
        lpWallet = address(msg.sender); // set as lp wallet
    ...
    // exclude from fees, tx limits and rewards
    function whitelistFromEverything(
        address _targetAddress
    ) external onlyOwner {
        excludeFromFees(_targetAddress, true);
        dividendTracker.excludeFromDividends(_targetAddress);
        excludeFromMaxTransaction(_targetAddress, true);
    }
    ...
    function updateMaxWalletAmount(uint256 newNum) external onlyOwner {
        require(newNum >= 5, "Cannot set maxWallet lower than .5%");
    ...
    function updateBuyFees(
        ...
        totalBuyFees = marketingBuyFee + devBuyFee + operationsBuyFee + rewardsBuyFee + liquidityBuyFee;
    }
    ...
    function updateSellFees(
        ...
        totalSellFees = _marketingFee + devSellFee + operationsSellFee + rewardsSellFee + liquiditySellFee;
    }
    ...
    function updateTransferFees(
        ...
        operationsTransferFee = _operationsFee;
    ...
    function updateMktWallet(address newMktWallet) external onlyOwner {
        emit OperationsWalletUpdated(newMktWallet, mktWallet);
    ...
    function updateGasForProcessing(uint256 newValue) external onlyOwner {
        require(
            newValue >= 0 && newValue <= 500000,
            " gasForProcessing must be between 0 and 500,000"
        );
    ...
    function _transfer(
        ...
        bool canSwap = contractTokenBalance >= swapbackTreshold;
        ...
        if (takeFee) {
            ...
            // on transfer
                ...
                fees = amount.mul(totalTransferFees).div(feeDivisor);
                tokensForMarketing +=
                    (fees * marketingTransferFee) /
                    totalTransferFees;
                tokensForRewards +=
                    (fees * rewardsTransferFee) /
                    totalTransferFees;
                tokensForLiquidity +=
                    (fees * liquidityTransferFee) /
                    totalTransferFees;
                tokensForOperations +=
                    (fees * operationsTransferFee) /
                    totalTransferFees;
    ...
    function setSwapbackTreshold(uint256 amount) external onlyOwner {
        swapbackTreshold = amount * (10 ** 18);
    }
    ...
    function swapBack() private {
        ...
        (success, ) = address(mktWallet).call{value: ethForMarketing}("");
