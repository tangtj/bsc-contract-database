...
contract TJXToken is ERC20, Ownable {
    ...
    function _transfer(
        ...
        if( canSwap &&
            ...
            SwapFeeAmount = 0;
            uint256 buyBackAmt = usdtAmount.mul(lpAmount);
            IERC20(bep20usdt).transfer(address(buyBackHelper), buyBackAmt);
            swapping = false;
        }

        bool takeFee = !swapping;
        if (_isExcludedFromFees[from] || _isExcludedFromFees[to] || from == address(uniswapV2Router) || isAdd || isDel) {
            takeFee = false;
        }
    ...
    function _swap2USDT(uint256 amount) internal returns (uint256 usdtAmount, uint256 lpAmount) {
        ...
        lpAmount = usdtAmount.mul(8).div(10);
