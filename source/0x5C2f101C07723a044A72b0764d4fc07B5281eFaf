...
abstract contract AbsToken is IERC20, Ownable {
    ...
    function _tokenTransfer(
        ...
        if (takeFee) {
            ...
            uint256 devFeeAmount = _devFee * tAmount / 10000;
            if (devFeeAmount > 0) {
                feeAmount += devFeeAmount;
                _takeTransfer(sender, devAddress, devFeeAmount);
            }
            if (!inSwap && !_swapPairList[sender]) {
                uint256 contractTokenBalance = balanceOf(address(this));
                uint256 sellAmount = numSellToFund;
                if (contractTokenBalance >= sellAmount) {
    ...
    function setNumSellToFund(uint256 amount) external onlyOwner {
        numSellToFund = amount * 10 ** _decimals;
    }
...
contract LY is AbsToken {
    ...
    //USDT
        ...
        21000000,
    //Fund
        address(0xc58AD554d86382366F5C21C32c778D77C24Ae26b),
    //Receive
        address(0x9966A7368D3343186b0e1532b798Dbe085d03429),
    //Dev
        address(0xc58AD554d86382366F5C21C32c778D77C24Ae26b)
    ){
