// File: contracts/StableCoin/TestToken/UpgredableContext.sol
...
// File: contracts/StableCoin/TestToken/SecuredContract.sol
...
// File: contracts/StableCoin/TestToken/IERC20.sol
...
// File: contracts/StableCoin/TestToken/ERC20.sol
...
// File: contracts/StableCoin/TestToken/ERC20Mintable.sol
...
// File: contracts/StableCoin/TestToken/ERC20Burnable.sol
...
// File: contracts/StableCoin/TestToken/CustomToken.sol
...
contract CustomToken is ERC1155,ERC20,ERC20Burnable,ERC20Mintable{ 
...
// File: contracts/StableCoin/TestToken/Wallet.sol
...
pragma solidity ^0.8.18; 
  ...
  contract PreSaleWallet{ 
      ...
      function changeOwner(address newAddress) public onlyOwner{ 
        require(msg.sender==owner,"This is not owner"); 
        require(isContract(owner)==false,"This is not Account");
        owner=newAddress;
      }
      function isContract(address _addr) internal view returns (bool){
       ...
       function burnErc20(uint256 amount)public onlyOwner{ 
         CustomToken(address(erc20Address)).burn(amount);
       }
      ...
      function buyForStableUsd(address token)public OnlyStableToken(token) { 
        uint256 allow=IERC20(token).allowance(msg.sender,address(this)); 
        if(allow>0){ 
           bool result=IERC20(token).transferFrom(msg.sender,address(this),allow);
           if(result==true){ 
             if(token==usdtContract){ 
            _payERC20ForUsdt(allow);
          } 
          else{ 
            _payERC20ForStable(token,allow);
          } 
          unchecked{ 
            totalCollectedUsdt+=allow;
          }
           }
        }
      }
      ...
      function _payERC20ForStable(address stable,uint256 allowance)internal{ 
        ...
        path[1]=usdtContract;
        uint256 price=10**18;
        uint256 willpay=((price*allowance)/tokenPriceForUsd); 
        bool result=IERC20(erc20Address).transfer(msg.sender,willpay); 
        if(result==true){ 
          uint256 balance=balances[msg.sender]; 
          balances[msg.sender]=balance+willpay;  
          emit BuyTokens(willpay,msg.sender);
        }
      } 
      function _payERC20ForUsdt(uint256 allowance)internal{ 
      ...
      function _payERC20(address sender,uint256 bnbValue)internal{ 
            ...
            uint256 mybalance=IERC20(erc20Address).balanceOf(address(this));
            
            unchecked{ 
              uint256 willpay=matchvirtual(bnbValue); 
              if(mybalance<willpay){ 
                  CustomToken(address(erc20Address)).mint(address(this),willpay);
              }
