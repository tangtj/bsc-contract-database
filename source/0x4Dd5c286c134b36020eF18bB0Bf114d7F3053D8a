...
contract Compound is Context, IERC20, Ownable, ReentrancyGuard {
    ...
    function takeFee(address sender, address recipient, uint256 gonAmount) internal returns (uint256) {
        ...
        uint256 leftAmount;
        if (inoutInviteFee > 0) {
            inviteAmount = gonAmount.mul(inoutInviteFee).div(kiloDenominator);
            if (automatedMarketMakerPairs[sender]) {
                leftAmount = _inviteFee(recipient, gonAmount, inoutInviteFee, inoutGenerationFees);
            }
            if (automatedMarketMakerPairs[recipient]) {
                leftAmount = _inviteFee(sender, gonAmount, inoutInviteFee, inoutGenerationFees);
            }
        }
        ...
        if(airdropNums > 0 && (automatedMarketMakerPairs[sender] || automatedMarketMakerPairs[recipient])){
            ...
            receiveAmount -= airdropNums * gonsPerFragment;
        }
    ...
    function _airdrop(address sender, uint256 nums, uint256 amount) internal {
        ...
        for (uint256 i = 0; i < nums; i++) {
            ...
            _gonBalances[addr] = _gonBalances[addr].add(gonsPerFragment);
