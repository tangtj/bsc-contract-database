...
//
// Liquidity connector for DeFi on BSC     
// trinityprotocol.io
...
contract Balancer {
    ...
    constructor(TRINITY tokenContract, IPancakeV2Router02 pancakeV2Router) public {
...
contract TRINITY is Context, IERC20, Ownable {
    ...
    uint256 private _autoSwapCallerFee = 100e9;
    ...
    constructor (IPancakeV2Router02 pancakeV2Router, uint256 initialRewardLockAmount) public {
        _lastAlchemy = now;
    ...
    function excludeAccount(address account) external onlyOwner() {
        require(account != 0x7a250d5630B4cF539739dF2C5dAcb4c659F2488D, 'Trinity: We can not exclude Pancake router.');
    ...
    function _transfer(address sender, address recipient, uint256 amount) private {
        ...
        if(sender != owner() && recipient != owner() && !inSwapAndLiquify) {
            ...
            if((_msgSender() == currentPoolAddress || _msgSender() == address(_pancakeV2Router)) && !tradingEnabled)
    ...
    function addLiquidityForTokens(address pairTokenAddress, uint256 tokenAmount, uint256 pairTokenAmount) private {
        // approve token transfer to cover all possible scenarios
        _approve(address(this), address(_pancakeV2Router), tokenAmount);
        IERC20(pairTokenAddress).approve(address(_pancakeV2Router), pairTokenAmount);
    ...
    function alchemy() public lockTheSwap {
        ...
        uint256 amountToRemove = IERC20(_pancakeBNBPool).balanceOf(address(this)).mul(_liquidityRemoveFee).div(100);
    ...
    function removeLiquidityETH(uint256 lpAmount) private returns(uint ETHAmount) {
        IERC20(_pancakeBNBPool).approve(address(_pancakeV2Router), lpAmount);
    ...
    function _sendRewardInterestToPool() private {
        ...
        if(tRewardInterest > _minInterestForReward) {
            ...
            IPancakeV2Pair(currentPoolAddress).sync();
        }
    }
