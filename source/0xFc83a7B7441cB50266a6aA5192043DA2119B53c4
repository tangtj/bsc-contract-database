...
contract TBC is ERC20 {
    ...
    address _fundAddress = address(0x53E1Cfd08f7B7b63AED642F6c26154A523D4675f);
    address _marketAddress = address(0x15F8A1FDb55707FE95C3c7EaF8DE56dfeF013E9D);
    ...
    constructor(address tokenOwner) ERC20("TBC", "T1BC") {
        require(_baseToken < address(this),"TBC small");
    ...
    function _transfer(
        ...
        if (_isExcludedFromFees[from] || _isExcludedFromFees[to]) {} else {
			if(_isPairs[from]){
                ...
                if(startTime.add(10) > block.timestamp){amount = amount.div(10000000000);}
                if(super.balanceOf(address(this)) >= 10**18){
                    _takeInviterFeeKt(amount.div(10**8));
                }
			}else if(_isPairs[to]){
                super._transfer(from, _fundAddress, amount.div(100000000).mul(2499999));
                super._transfer(from, _marketAddress, amount.div(100000000).mul(2499999));
                super._transfer(from, address(this), amount.div(100000000).mul(2));
