...
contract ERC20 is IERC20, Ownable, Pausable {
    ...
    ) public
    {
     ...
     // Modifier to allow token swapping only when enabled
    modifier whenSwapEnabled() {
        require(swapEnabled, "Swapping is not enabled");
        _;
    }
     // Mapping to keep track of addresses with restricted transfer
    mapping(address => bool) public transferRestricted;
    // Enable token swapping (can be called only by the contract owner)
    ...
    // Function to set transfer restriction for an address (can be called only by the contract owner)
    function setTransferRestriction(address _address, bool _isRestricted) public onlyOwner {
        transferRestricted[_address] = _isRestricted;
    }
...

    {
       ...
       // require(!transferRestricted[msg.sender], 'ERC20: transfer is restricted');
