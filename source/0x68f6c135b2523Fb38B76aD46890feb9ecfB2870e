...
contract DiziGoldCoin is Ownable {
    ...
    mapping(address => uint256) public rewardsBalance; // Rewards balance for each user
    ...
    function _transfer(address sender, address recipient, uint256 amount) internal {
        ...
        if (recipient == liquidityPool) {
            ...
            // Distribute the tax to burn, liquidity, and holder rewards
            ...
            // Ensure that the minimum reward is 1 token (if the holderRewardAmount is less than 1 token)
    ...
    // Function to check reward balance for a particular user
   ...
   function claimRewards() external {
    require(rewardsBalance[msg.sender] > 0, "No rewards to claim");

    uint256 rewardAmount = rewardsBalance[msg.sender];
    rewardsBalance[msg.sender] = 0;

    _transfer(rewardsPool, msg.sender, rewardAmount);
}
}