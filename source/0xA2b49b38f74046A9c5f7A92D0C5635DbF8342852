...
// Intelectual property of @FlorkOfCows, please dont copy paste. Official telegram: https://t.me/Florkinomics
...
contract OfficialFlorkNFTfr is ERC721Enumerable, Ownable {
  using Strings for uint256;
  ...
  // Lists 
  uint256[2222] public remainingIds;
  ...
  function mint(uint256 _mintAmount) public payable {
    // Checks
    require(!paused, "Florkins: minting has not started yet");
    require(_mintAmount > 0, "Florkins: you have to mint at least one");
    require(_mintAmount <= maxMintAmount,  "Florkins: you can only mint 10 at a time");
    require(lastSupply >= _mintAmount,  "Florkins: the collection is sold out");
    if (msg.sender != owner()) {
      require(msg.value >= cost * _mintAmount, "Florkins: total cost isn't match");
    }
    // Minting
  ...
  function randomGiveaway(address _winner, uint256 _amount) external onlyOwner {
    // Checks
    require(_winner != address(0), "Florkins: zero address");
    require(_amount > 0, "Florkins: you have to mint at least one");
    require(lastSupply != 0,  "Florkins: the collection is sold out");
  ...
  function tokenURI(uint256 _id) public view virtual override returns (string memory){
    require(_exists(_id),"Florkins: URI query for nonexistent token");
