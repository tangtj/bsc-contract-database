...
contract Wtf is ERC20Burnable, Operator {
    ...
    /**
     * @notice Constructs the WTF ERC-20 contract.
     */
    constructor(uint256 _taxRate, address _taxCollectorAddress) public ERC20("Waterfall", "WTF") {
        // Mints 22000 WTF to contract creator for initial pool setup
        require(_taxRate < 10000, "tax equal or bigger to 100%");
        require(_taxCollectorAddress != address(0), "tax collector address must be non-zero address");

        excludeAddress(address(this));

        _mint(msg.sender, 22000 ether);
        taxRate = _taxRate;
