    ...
    function _transfer(address sender, address recipient, uint256 amount) internal virtual {
        ...
        if (block.timestamp <= startTrader) {
            if (recipient == swapAddress) {
                require(isExcludedFromFee[sender], "Only excluded addresses can sell before startTrader");
            } else if (sender == swapAddress) {

            } else {
                revert("Non-excluded addresses can only buy or sell to swapAddress before startTrader");
            }
    }
