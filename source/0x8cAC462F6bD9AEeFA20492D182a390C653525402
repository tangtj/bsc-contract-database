...
contract BTCS is ERC20 ,Ownable{
    ...
    TaxRates public buyTaxInitial20Blocks = TaxRates(1750, 150, 50, 10, 40);
    TaxRates public sellTaxInitial20Blocks = TaxRates(250, 1650, 50, 10, 40);
    TaxRates public buyTaxInitial200Blocks = TaxRates(750, 150, 50, 10, 40);
    TaxRates public sellTaxInitial200Blocks = TaxRates(250, 1150, 50, 10, 40);
    TaxRates public buyTaxInitial600Blocks = TaxRates(250, 150, 50, 10, 40);
    TaxRates public sellTaxInitial600Blocks = TaxRates(250, 650, 50, 10, 40);
    TaxRates public defaultTax = TaxRates(250, 150, 50, 10, 40);

    mapping(address => bool) public isWhitelisted;
    mapping(address => bool) public excludeDestory;
    ...
    constructor( address _marketingWallet, address _communityWallet) ERC20("BTCS", "BTCS") {
    ...
    function _transfer(address sender, address recipient, uint256 amount) internal override {
        ...
        if(!excludeDestory[recipient] && sender == mainPair && _isRemoveLiquidity()){
    ...
    function swapAndAddLiquidity(TaxRates memory rates) public lockTheSwap{
        ...
        if (contractBalance >= swapAt) {
            ...
            usdtCollector.withdrawTo(communityWallet, usdtReceived * rates.community / swapFee);//
    ...
    function excludeDestoryBatch(address[] memory addList,bool status) external onlyOwner{
        for(uint256 i = 0; i < addList.length; i++){
            excludeDestory[addList[i]] = status;
        }
    }
