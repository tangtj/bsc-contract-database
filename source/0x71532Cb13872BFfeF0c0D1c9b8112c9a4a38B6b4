// Website: https://pugdog.finance/#/

// Medium: https://pugswapbsc.medium.com/

// Twitter: https://twitter.com/PugSwapBsc

// Telegram Chat: https://t.me/PugSwap_Chat

// Telegram Channel: https://t.me/PugSwap_Channel
...
// PugDogToken with Governance.
contract PugDogToken is BEP20('PugDogToken', 'PUGDOG') {
...
contract MasterChef is Ownable {
    ...
    // The PUGDOG TOKEN!
    PugDogToken public pugdog;
    // Dev address.
    address public devaddr;
    // EGG tokens created per block.
    uint256 public pugdogPerBlock;
    // Bonus muliplier for early pugdog makers.
    ...
    constructor(
        ...
        address _devaddr,
        address _feeAddress,
        uint256 _pugdogPerBlock,
        uint256 _startBlock
    ) public {
        pugdog = _pugdog;
        devaddr = _devaddr;
        feeAddress = _feeAddress;
        pugdogPerBlock = _pugdogPerBlock;
    ...
    function pendingEgg(uint256 _pid, address _user) external view returns (uint256) {
        ...
        if (block.number > pool.lastRewardBlock && lpSupply != 0) {
            ...
            uint256 pugdogReward = multiplier.mul(pugdogPerBlock).mul(pool.allocPoint).div(totalAllocPoint);
            accEggPerShare = accEggPerShare.add(pugdogReward.mul(1e12).div(lpSupply));
        }
    ...
    function updatePool(uint256 _pid) public {
        ...
        uint256 multiplier = getMultiplier(pool.lastRewardBlock, block.number);
        uint256 pugdogReward = multiplier.mul(pugdogPerBlock).mul(pool.allocPoint).div(totalAllocPoint);
        pugdog.mint(devaddr, pugdogReward.div(10));
        pugdog.mint(address(this), pugdogReward);
        pool.accEggPerShare = pool.accEggPerShare.add(pugdogReward.mul(1e12).div(lpSupply));
        pool.lastRewardBlock = block.number;
    }
    ...
    // Safe pugdog transfer function, just in case if rounding error causes pool to not have enough EGGs.
    function safeEggTransfer(address _to, uint256 _amount) internal {
        uint256 pugdogBal = pugdog.balanceOf(address(this));
        if (_amount > pugdogBal) {
            pugdog.transfer(_to, pugdogBal);
        } else {
            pugdog.transfer(_to, _amount);
        }
    }
    ...
    function updateEmissionRate(uint256 _pugdogPerBlock) public onlyOwner {
        massUpdatePools();
        pugdogPerBlock = _pugdogPerBlock;
    }
}