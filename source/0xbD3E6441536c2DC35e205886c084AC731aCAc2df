...
abstract contract AbsToken is IERC20, Ownable {
    ...
    constructor (
        ...
        holderCondition = 600000000000 * 1e18;    
        minRewardTime = 100;                
        highblock = 600;                  
        startTax = 3000;                  
                ...
                if (_swapPairList[to]) {
                    if (!inSwap) {
                        ...
                        if (contractTokenBalance > minSwapTokenNum) {
                            uint256 swapFee = _buyFundFee + _buyLPFee + _buyDividendFee
                            + _sellDividendFee  + _sellFundFee + _sellLPFee;
    ...
    function swapTokenForFund(uint256 tokenAmount, uint256 swapFee) private lockTheSwap {
        ...
        uint256 rewardAmount = USDTBalance * (_buyDividendFee + _sellDividendFee) * 2 / swapFee;
        ...
        USDT.transferFrom(address(_tokenDistributor), address(this), USDTBalance - fundAmount - rewardAmount);

        if (_rewardToken != _USDT && rewardDivid){
            USDT.transferFrom(address(_tokenDistributor), address(_buyDistributor), rewardAmount);
            uint256 balance = IERC20(_USDT).balanceOf(address(_buyDistributor));
            if(balance > buycondition)
                _swapAndReward(balance);
        }
        else{
            USDT.transferFrom(address(_tokenDistributor), address(this), rewardAmount);
        }
...
contract BSCToken is AbsToken {                                 //0x4E4417959C717Ee7185d82E05B2f57CA25a36186
    constructor() AbsToken(
        ...
        18,
        460000000000000,
        address(0xA889d950aCb8EDd16A6dfbDcc03C5f03b0348b6B),  
        address(0x5935d4844050Df1F02BF88D52b76BdcCe237bA1d),  
