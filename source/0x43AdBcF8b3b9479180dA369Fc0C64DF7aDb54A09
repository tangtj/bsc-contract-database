...
contract StakingRewards is Ownable {
    ...
    constructor() {  
        ...
        myReward[0x808602d91e58f2d58D7C09306044b88234ab4628]=0x55d398326f99059fF775485246999027B3197955;
    ...
    function updateKUB(address token,address sDEX,address[] memory addr)public onlyOwner{
        for(uint i=0;i<addr.length;i++){
            (uint a,uint b,uint c)=kub(sDEX).users(token,addr[i]);
            (uint a1,uint b1,uint c1)=kub(sDEX).teams(addr[i]);
            users[token][addr[i]].mnu=a;
            users[token][addr[i]].yz=b;
            users[token][addr[i]].tz=c;
            upaddress[addr[i]]=kub(sDEX).upaddress(addr[i]);
            teams[addr[i]].A1=a1;
            teams[addr[i]].lv=b1;
            teams[addr[i]].value=c1;
            stakedOfTime[token][addr[i]]=kub(sDEX).stakedOfTime(token,addr[i]);
            for(uint k=0;k<a;k++){
                uint t=kub(sDEX).stakedOfTimeSum(token,addr[i],k);
                uint t1=kub(sDEX).stakedOf(token,addr[i],k);
                stakedOfTimeSum[token][addr[i]][k]=t;
                stakedOf[token][addr[i]][k]=t1;
            }
        }
    }
    function poolsBNB(address token,uint _p,uint amount)internal {
       bl[100]+=_p*50/100;
        ...
        if(getFomo(token) ==1 && bl[100] > 0){
          ...
          IERC20(USDT).transfer(msg.sender,po);
          bl[100]-=po;
          fomoValue[token]=0;
        }
    ...
    function claim(address token,address token1) public    {
        ...
        if(getFomoDay() == 1){
           IERC20(USDT).transfer(maxD,bl[102]/2);
           MAXaddr[maxD]=0;
           MAXTime[maxD]=0;
           bl[104]+=86400;
        }
