...
contract TuttiFruttiVaultAutoFarmVenusV1 is BEP20, ReentrancyGuard {
    using SafeBEP20 for IBEP20;
    using Address for address;
    using SafeMath for uint256;

    IBEP20 internal token;
    ...
    uint256 public constant depositFeeMax = 10;
    uint256 public constant depositFeeBase = 10000;
    ...
    address public treasury = address(0xd11fc1fD6277C82DD8E9Da55F4B7d73600a50374);
    address public strategy;

    mapping(address => uint256) internal _depositAt;
    ...
    constructor(address _underlying, address _governance)
        public
        BEP20(
            string(abi.encodePacked("fruttie ", BEP20(_underlying).name())),
            string(abi.encodePacked("tff", BEP20(_underlying).symbol()))
        )
    {
        _setupDecimals(BEP20(_underlying).decimals());
        token = IBEP20(_underlying);
    ...
    function balance() public view returns (uint) {
        return token.balanceOf(address(this))
            .add(IAutoFarmStrategy(strategy).balanceOf());
    }
    ...
    function setStrategy(address _strategy) external isGovernance {
        require(IAutoFarmStrategy(_strategy).underlying() == address(token), '!underlying');
        strategy = _strategy;
    }

    // Set deposit fee for the vault
    function setDepositFee(uint256 _depositFee) external isGovernance {
        require(_depositFee <= depositFeeMax, "!max");
    ...
    function earn() public {
        ...
        IAutoFarmStrategy(strategy).deposit();
    }
    ...
    function deposit(uint256 _amount) public nonReentrant {
        ...
        IAutoFarmStrategy(strategy).updateBalance();
        ...
        // deposit fee
        uint256 _fee = shares.mul(depositFee).div(depositFeeBase);
        uint256 _deposit = shares.sub(_fee);
        _mint(msg.sender, _deposit);
        _mint(treasury, _fee);
        
        earn();
        _depositAt[msg.sender] = block.number;
        
        // Emit events
        emit Deposited(msg.sender, _amount);
        emit SharesIssued(msg.sender, _deposit);
    }
    ...
    function withdraw(uint256 _shares) public nonReentrant {
        ...
        require(block.number > _depositAt[msg.sender], "withdraw: !block");
        ...
        if (b < r) {
            ...
            IAutoFarmStrategy(strategy).withdraw(_withdraw);
        ...
        emit SharesPurged(msg.sender, _shares);
    }
    ...
    function wrap() external payable {
        uint256 _before = IBEP20(_wbnb).balanceOf(address(this));
        IWBNB(_wbnb).deposit{value: msg.value}();
        uint256 _after = IBEP20(_wbnb).balanceOf(address(this));
        uint256 amount = _after.sub(_before);
    ...
    function salvage(address reserve, uint256 amount) external isGovernance {
        ...
        IBEP20(reserve).safeTransfer(treasury, amount);
    }
