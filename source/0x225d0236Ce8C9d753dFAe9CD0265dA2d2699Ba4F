...
contract BGLToken is ERC20, Ownable {

	IUniswapV2Router02 public uniswapV2Router;
	address public immutable uniswapV2Pair;

	string private _name = "BGL Token";
	string private _symbol = "BGL";
	uint8 private _decimals = 18;

	BGLDividendTracker public dividendTracker;
	...
	constructor() public ERC20(_name, _symbol) {
		dividendTracker = new BGLDividendTracker();
	...
	function updateDividendTracker(address newAddress) public onlyOwner {
		require(newAddress != address(dividendTracker), "BGL: The dividend tracker already has that address");
		BGLDividendTracker newDividendTracker = BGLDividendTracker(payable(newAddress));
		require(newDividendTracker.owner() == address(this), "BGL: The new dividend tracker must be owned by the BGL token contract");
	...
	function setWallets(address newLiquidityWallet, address newMarketingWallet, address newGiveAwayWallet, address newLastManStandingWallet) public onlyOwner {
		if(liquidityWallet != newLiquidityWallet) {
			require(newLiquidityWallet != address(0), "BGL: The liquidityWallet cannot be 0");
			emit WalletChange('liquidityWallet', newLiquidityWallet, liquidityWallet);
			liquidityWallet = newLiquidityWallet;
		}
		if(marketingWallet != newMarketingWallet) {
			require(newMarketingWallet != address(0), "BGL: The marketingWallet cannot be 0");
			emit WalletChange('marketingWallet', newMarketingWallet, marketingWallet);
			marketingWallet = newMarketingWallet;
		}
		if(giveAwayWallet != newGiveAwayWallet) {
			require(newGiveAwayWallet != address(0), "BGL: The giveAwayWallet cannot be 0");
		...
		if(lastManStandingWallet != newLastManStandingWallet) {
			require(newLastManStandingWallet != address(0), "BGL: The lastManStandingWallet cannot be 0");
	...
	function claimBNBOverflow(uint256 amount) external onlyOwner {
		require(amount < address(this).balance, "BGL: Cannot send more than contract balance");
	...
	function _transfer(
		...
		_adjustTaxes(_isInLaunch, isBuyFromLp, isSelltoLp);
	...
	function _adjustTaxes(bool inLaunch, bool isBuyFromLp, bool isSelltoLp) private {
		uint256 blocksSinceLaunch = block.number - _launchBlockNumber;
		uint256 currentTimestamp = !isTradingEnabled && _tradingPausedTimestamp > _launchStartTimestamp  ? _tradingPausedTimestamp : _getNow();
		uint256 timeSinceLaunch = currentTimestamp - _launchStartTimestamp;
	...
	function _swapAndLiquify() private {
		...
		uint256 amountLiquidity = BNBBalanceAfterSwap * _liquidityFee / baseFee / 2;
		uint256 amountMarketing = BNBBalanceAfterSwap * _marketingFee / baseFee;
		uint256 amountGiveAway = BNBBalanceAfterSwap * _giveAwayFee / baseFee;
		uint256 amountLastManStanding = BNBBalanceAfterSwap * _lastManStandingFee / baseFee;
		uint256 amountHolders = BNBBalanceAfterSwap - (amountLiquidity + amountMarketing + amountGiveAway + amountLastManStanding);

		_payProjectWallets(amountMarketing, amountGiveAway, amountLastManStanding);

		if (amountToLiquify > 0) {
			_addLiquidity(amountToLiquify, amountLiquidity);
			emit SwapAndLiquify(amountToSwap, amountLiquidity);
		}

		(bool dividendSuccess,) = address(dividendTracker).call{value: amountHolders}("");
		if(dividendSuccess) {
			emit DividendsSent(amountHolders);
		}
	}
	function _payProjectWallets(uint256 amountMarketing, uint256 amountGiveAway, uint256 amountLastManStanding) private {
		uint256 initialBUSDBalance = BUSD.balanceOf(address(this));
		uint256 amountToSwap = amountMarketing + amountGiveAway + amountLastManStanding;

		_swapBNBForBUSD(amountToSwap);

		uint256 BUSDBalanceAfterSwap = BUSD.balanceOf(address(this)) - initialBUSDBalance;
		uint256 amountMarketingBUSD = BUSDBalanceAfterSwap * amountMarketing / amountToSwap;
		uint256 amountGiveAwayBUSD = BUSDBalanceAfterSwap * amountGiveAway / amountToSwap;
		uint256 amountLastManStandingBUSD = BUSDBalanceAfterSwap - (amountMarketingBUSD + amountGiveAwayBUSD);

		(bool success) = BUSD.transfer(marketingWallet, amountMarketingBUSD);
		(success) = BUSD.transfer(giveAwayWallet, amountGiveAwayBUSD);
		(success) = BUSD.transfer(lastManStandingWallet, amountLastManStandingBUSD);
	}
...
contract BGLDividendTracker is DividendPayingToken {
	using SafeMath for uint256;
	using SafeMathInt for int256;
