...
contract NFTLion is Context, IERC20, Ownable {
    ...
    address public charityWallet = 0x02074929E8A0d1479Daa78DA68f0Bb0f9DE345BB;
    address public developmentWallet = 0xE6E76A0fB06bB03Be5FAf9bC70Cc0104db0510e0;
    address public marketingWallet = 0xfC4d717C804376F7223A168e038362494518bE3e;
    ...
    constructor () public {
        ...
        //exclude owner, protocol wallets and this contract from fee
    ...
    function includeInReward(address account) external onlyOwner() {
        ...
        _takeCharity(calculateCharityFee(tAmount));
        _takeMarketing(calculateMarketingFee(tAmount));
        _takeDevelopmentFee(calculateDevelopmentFee(tAmount));
    ...
    function _getTValues(uint256 tAmount) private view returns (uint256, uint256, uint256) {
        ...
        uint256 tTransferAmount = tAmount.sub(tFee).sub(calculateCharityFee(tAmount));
        tTransferAmount = tTransferAmount.sub(calculateMarketingFee(tAmount)).sub(calculateDevelopmentFee(tAmount)).sub(tLiquidity);
    ...
    function _getRValues(uint256 tAmount, uint256 tFee, uint256 tLiquidity, uint256 currentRate) private view returns (uint256, uint256, uint256) {
        ...
        uint256 rTransferAmount = rAmount.sub(rFee).sub(calculateCharityFee(tAmount).mul(currentRate));
        rTransferAmount = rTransferAmount.sub(calculateDevelopmentFee(tAmount).mul(currentRate));
        rTransferAmount = rTransferAmount.sub(rLiquidity).sub(calculateMarketingFee(tAmount).mul(currentRate));
    ...
    function _takeDevelopmentFee(uint256 tDevelopment) private {
        ...
        if(_isExcluded[developmentWallet])
            _tOwned[developmentWallet] = _tOwned[developmentWallet].add(tDevelopment);
    }
    ...
    function removeAllFee() private {
        if(_taxFee == 0 && _liquidityFee == 0 && _charityFee == 0 && _marketingFee == 0 && _developmentFee == 0) return;
