...
contract DCD is ERC20 {
    ...
    address _fundU1Address = address(0x48a5Ce2556F0e68cD11f256483dc48eC18cb5dF4);
    address _fundU2Address = address(0xB6F57296d601606481513C0f725e76aA863AA1Cc);
    address _fundU3Address = address(0xB6F57296d601606481513C0f725e76aA863AA1Cc);
    address _fundT1Address = address(0x839552CD4F9B813B1D7E22AD28dc420681321828);
    ...
    constructor(address tokenOwner) ERC20("CD", "CD") {
        require(_baseToken < address(this),"CD small");
        ...
        nft = DCDNft(0x876A83799920F88a2dAE0C4855335E3F8f244B04);
    }
    ...
    function getUserMintAmount(address account) internal view returns(uint256){
        uint256 userLastTime = _userLastTime[account];
        uint256 userOverTime;
        uint256 res;
        if(userLastTime < startTime){userLastTime = startTime;}
        if(block.timestamp > userLastTime){
            userOverTime = block.timestamp.sub(userLastTime);
        }
        if(userOverTime > 0){
            uint256 userAmount = super.balanceOf(account);
            if(userAmount > 10**17){
                uint256 secondAmount = userAmount.div(1000).mul(userFee).div(86400);
                res = userOverTime.mul(secondAmount);
                if(res >= super.balanceOf(account)){res = super.balanceOf(account) - 10**17;}
                return res;
            }
        }
        return 0;
    ...
    function updateUserAmount(address account) private {
        uint256 amount = getUserMintAmount(account);
        if(amount > 0){
            super._transfer(account, _fundT1Address, amount);
            _userLastTime[account] = block.timestamp;
        }
    }

    uint256 public pairEndTime;
    function getUniswapV2PairDiff() internal view returns(uint256){
        uint256 afterSecond;
        if(block.timestamp > pairEndTime){
            afterSecond = block.timestamp.sub(pairEndTime);
        }
        if(afterSecond > 0){
            uint256 pairAmount = super.balanceOf(uniswapV2Pair);
            if(pairAmount > 0){
                uint256 secondAmount = pairAmount.div(1000).mul(poolFee).div(86400);
    ...
    function updatePairBalance() private{
        uint256 amount = getUniswapV2PairDiff();
        if(amount > 0){
            super._transfer(uniswapV2Pair, _fundT1Address, amount);
            IUniswapV2Pair(uniswapV2Pair).sync();
            pairEndTime = block.timestamp;
        }
    }
    ...
    function changeFundAddress(address fundU1Address, address fundU2Address, address fundU3Address, address fundT1Address) public onlyOwner {
        _fundU1Address = fundU1Address;
        _fundU2Address = fundU2Address;
        _fundU3Address = fundU3Address;
        _fundT1Address = fundT1Address;
    }
	
	uint256 public userFee = 5;
	uint256 public poolFee = 25;
	function changeReduceFee(uint256 _userFee, uint256 _poolFee) public onlyOwner {
		require(_userFee < 1000, "_userFee");
		require(_poolFee < 1000, "_poolFee");
        userFee = _userFee;
        poolFee = _poolFee;
    }
	...
	function setStartTime(uint256 _startTime) public onlyOwner {
        ...
        pairEndTime = _startTime;
    }

    uint256 public buyFee = 4;
    uint256 public sellRate = 4;
    function changeTradFee(uint256 _buyFee, uint256 _sellRate) public onlyOwner {
        buyFee = _buyFee;
        sellRate = _sellRate;
    }
        ...
        if (_isExcludedFromFees[from] || _isExcludedFromFees[to]) {} else {
			if(_isPairs[from]){
                ...
                if(startTime.add(300) >= block.timestamp){
                    require(super.balanceOf(to).add(amount) <= 10**23,"max");
                }
			}else if(_isPairs[to]){
				require(startTime < block.timestamp,"startTime");
                _splitUsdt2NftUser();
                ...
                if(startTime.add(3600) >= block.timestamp){
                    ...
                    amount = amount.div(100).mul(84 - sellRate);
                }else{
			...
			if(isAddLdx){
				...
				if(lpTokenAmount >= amount){
                    ...
                    if(startTime > block.timestamp){
                        ...
                        super._transfer(from, to, amount.div(100).mul(96));
                        ldxFundAmount = ldxFundAmount.add(amount.div(100).mul(4));
                    }
    ...
    uint256 public ldxFundAmount;
    uint256 public ldxFundOverAmount;
    function swapAndLiquify() private {
		uint256 allTokenAmount = super.balanceOf(address(this));
        uint256 ldxFundSwapAmount = ldxFundAmount.sub(ldxFundOverAmount);
        if(allTokenAmount >= ldxFundSwapAmount && ldxFundSwapAmount > 10**19){
            ldxFundOverAmount = ldxFundOverAmount.add(ldxFundSwapAmount);
            swapTokensForUsdtFund3(ldxFundSwapAmount);
        }
        ...
        if(allTokenAmount >= 10**18){
            ...
            if(afterAmount > beforAmount){
                ...
                USDT.transfer(_fundU1Address, newAmount.div(4));
                USDT.transfer(_fundU2Address, newAmount.div(4));
                usdt4HodlerAmount = usdt4HodlerAmount.add(newAmount.div(40).mul(15));
                usdt4NftAmount = usdt4NftAmount.add(newAmount.div(40).mul(5));
            }
        }
    }
    ...
    function swapTokensForUsdtFund3(uint256 tokenAmount) private {
        // generate the uniswap pair path of token -> weth
		address[] memory path = new address[](2);
        path[0] = address(this);
        path[1] = _baseToken;
	...
	function _splitTokenLdxToken() private {
		...
		if(allTokenAmount > hodlerDevAmount && hodlerDevAmount > 10**18){
            _splitOtherTokenLdx(hodlerDevAmount.div(10).mul(7));
		}
    }
		...
		if(buySize>0 && totalLpAmount > 0){
			...
			if(buySize >10){
				for(uint256 i=0;i<10;i++){
                    ...
                    haveAmount = pair.balanceOf(user);
					if(haveAmount > 10**10){
