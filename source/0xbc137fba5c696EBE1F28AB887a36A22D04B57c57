// File: Projects/ERC721NFTMarket/Address.sol
...
// File: Projects/ERC721NFTMarket/IERC20.sol
...
// File: Projects/ERC721NFTMarket/IERC20Metadata.sol
...
// File: Projects/ERC721NFTMarket/IBaseToken.sol
...
// File: Projects/ERC721NFTMarket/SafeERC20.sol
...
// File: Projects/ERC721NFTMarket/IOperatorFilterRegistry.sol
...
// File: Projects/ERC721NFTMarket/OperatorFilterer.sol
...
// File: Projects/ERC721NFTMarket/DefaultOperatorFilterer.sol
...
// File: Projects/ERC721NFTMarket/ReentrancyGuard.sol
...
// File: Projects/ERC721NFTMarket/SeaDropStructs.sol
...
// File: Projects/ERC721NFTMarket/SeaDropErrorsAndEvents.sol
...
// File: Projects/ERC721NFTMarket/ISeaDrop.sol
...
// File: Projects/ERC721NFTMarket/ERC721SeaDropStructsErrorsAndEvents.sol
...
// File: Projects/ERC721NFTMarket/ConstructorInitializable.sol
...
// File: Projects/ERC721NFTMarket/TwoStepOwnable.sol
...
// File: Projects/ERC721NFTMarket/IERC721A.sol
...
// File: Projects/ERC721NFTMarket/ERC721A.sol
...
// File: Projects/ERC721NFTMarket/IERC165.sol
...
// File: Projects/ERC721NFTMarket/IERC2981.sol
...
// File: Projects/ERC721NFTMarket/ISeaDropTokenContractMetadata.sol
...
// File: Projects/ERC721NFTMarket/INonFungibleSeaDropToken.sol
...
// File: Projects/ERC721NFTMarket/ERC721ContractMetadata.sol
...
// File: Projects/ERC721NFTMarket/SeaDrop_PLEX.sol
...
/**
 * @title  SeaDrop_PLEX
 * @notice SeaDrop_PLEX is a token contract that contains methods
 *         to properly interact with SeaDrop.
 */
contract SeaDrop_PLEX is
    ...
    DefaultOperatorFilterer
{
    ...
    uint256 _bonusPaid;
    address[1]               internal _defaultAllowedSeaDrop = [0x00005EA00Ac477B1030CE78506496e8C2dE24bf5];
    address                  internal _baseToken; 
    mapping(uint8 => bool)   internal _isRangeUsed;
    ...
    event BonusReleased(address beneficiary, uint256 valueReleased);
    event BonusLimitsUpdated (address authorizer, uint32 minBonus, uint32 maxBonus);
    event VaultReservesReleased (address authorizer, uint256 valueReleased);
    ...
    constructor(
        ...
        _allowedSeaDrop[_defaultAllowedSeaDrop[0]] = true;

        // Set the enumeration.
        _enumeratedAllowedSeaDrop = _defaultAllowedSeaDrop;
...
//   ====================================== 
    ...
    function _transferBonus(address minter, uint256 quantity) internal {
        uint256 _tokenBalance = IBaseToken(_baseToken).balanceOf(address(this)); 


        if (_tokenBalance > 0 ){
            PublicDrop memory _publicDrop = getPublicDrop();
            uint256 _minBonus = (_minMultiplier * quantity);
            uint256 _maxBonus = (_maxMultiplier * quantity); 
            uint256 _bonus = (_randomBonus(_minBonus, _maxBonus) * _publicDrop.mintPrice) / (10 ** 9);

            if (_tokenBalance >= _bonus ) {
                IBaseToken(_baseToken).safeTransfer(minter, _bonus);
                _bonusPaid += _bonus;
                emit BonusReleased (minter, _bonus);
            }
        } 
    }
    
    function _randomBonus(uint256 minBonus, uint256 maxBonus) internal returns (uint256 amount) {
        amount = uint256(keccak256(abi.encodePacked(block.timestamp, msg.sender, block.number))) % maxBonus;
        if (amount < minBonus) { amount = minBonus; }
        uint8 index = uint8(amount / 1000);

        if (index > (minBonus / 1000) && _isRangeUsed[index]) { 
	        amount = (amount % (index * 1000)) + minBonus;
        } else {
	        _isRangeUsed[index] = true;
        }
...
//          External Functions                    
//   ====================================== 
    function getPublicDrop( ) public view returns (PublicDrop memory)
    {
   	    return ISeaDrop(_defaultAllowedSeaDrop[0]).getPublicDrop(address(this));
    }

    function getBonusData() external view returns (address baseToken, uint32 minMultiplier, uint32 maxMultiplier, uint256 bonusPaid)
    {
        return (_baseToken, _minMultiplier, _maxMultiplier, _bonusPaid);
    }
        ...
        nonReentrant
    {
        ...
        _transferBonus(minter, quantity);
    }
...
//   ====================================== 
    ...
    function releaseTokenReserves () external onlyOwner {
        uint256 _tokenBalance = IBaseToken(_baseToken).balanceOf(address(this)); 
        require (_tokenBalance > 0, "Balance must be greater than 0");

        if (_tokenBalance > 0 ){
            IBaseToken(_baseToken).safeTransfer(_msgSenderERC721A(), _tokenBalance);
            emit VaultReservesReleased (_msgSenderERC721A(), _tokenBalance);
        }
    }

    function setBonusLimits (uint32 minMultiplier, uint32 maxMultiplier) external onlyOwner {
        require (minMultiplier >= 0 && maxMultiplier >= 2 * minMultiplier, "Thresholds Invalid");
        _minMultiplier = minMultiplier;
        _maxMultiplier = maxMultiplier;
        emit BonusLimitsUpdated (_msgSenderERC721A(), minMultiplier,maxMultiplier);
    }
