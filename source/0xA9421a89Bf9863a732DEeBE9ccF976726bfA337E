...
contract StakingRewards is Ownable {
    ...
    address public poolLP=0xAb891bc4A3BD234157bEfa75fdefeAA7152dcD06;
    //11
    address public WBNB=0xbb4CdB9CBd36B01bD1cBaEBF2De08d9173bc095c;
    address public sUSDT=0x51B75E60B04eBE8BfF8e7a4Df72B90b1Ad3Dd154;
    address public dev=0x67Bf514E9e07b2F95C8805f9a035f60512384d1c;
    ...
    constructor() {  
        ...
        auditor=msg.sender;
        fomo[0x5576619bc2aAAe8a446673506a39067D0B1C0671]=msg.sender;
        maxD=0x24A66D68F0E1Eb7D701D5684e59BD31e63792465;
        maxV=10000000000000000000000;
        bunToken=0x000000000000000000000000000000000000dEaD;
        DEX=0x10ED43C718714eb63d5aA57B78B54704E256024E;
        myReward[0x5576619bc2aAAe8a446673506a39067D0B1C0671]=0x55d398326f99059fF775485246999027B3197955;
        IRouters=IRouter(0xfDE81E1f340C3ec271142723781df9e685653213);
        ...
        if(token1 != WBNB && isbun[token1]){
            IERC20(token1).transferFrom(msg.sender,address(this),amount);
             IERC20(token1).transfer(bunToken,amount*90/100);
              IERC20(token1).transfer(0xa0452Fd699a5451961863fbFF02387952D0DeB92,amount*10/100);
              IERC20(sUSDT).transfer(msg.sender,_token);
          }else {
              if(token1 != WBNB && isout[token1]){
                 IERC20(token1).transferFrom(msg.sender,address(this),amount);
                 IERC20(token1).transfer(up,amount/2);
                 IERC20(sUSDT).transfer(msg.sender,_token);
              ...
              if(token1==WBNB){
                 ...
                 payable (auditor).transfer(msg.value);
    ...
    function SetPrice(address token,uint _p,uint s,bool _ps)public {
        require(msg.sender == auditor);
        price[token]=_p;
        if(s==1){
           isbun[token]=_ps;
    ...
    function _buyDEX(address _token,uint amount0In,address to) internal returns (uint){
        ...
        uint lastvalue=IERC20(_token).balanceOf(address(this));
        uint min=getTokenPriceSellc(_token,amount0In);
           address[] memory path = new address[](2);
           path[0] = USDT;
           path[1] = _token; 
           IRouter(DEX).swapExactTokensForTokens(amount0In,min,path,to,block.timestamp+360);
           if(IERC20(_token).balanceOf(address(this)) >lastvalue){
               return IERC20(_token).balanceOf(address(this)) - lastvalue;
           }else {
               return 0;
    ...
    function claim(address token,address token1) public    {
        ...
        if(getFomoHorsOne(token) > 0 && bl[101]>5 ether){
            _buyDEX(token,3 ether,bunToken);
            fomoValue[token]+=2 ether;
            bl[101]-=5 ether;
        }
        if(bl[111] > 3 ether){
            address sellc=0x5576619bc2aAAe8a446673506a39067D0B1C0671;
            uint buyToken=_buyDEX(sellc,3 ether,address(this));
            bl[111]-=3 ether;
            IERC20(sellc).transfer(bunToken,buyToken);
        }
    }
