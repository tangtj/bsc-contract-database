...
contract PGPNFT is ERC721Enumerable, Ownable {
    ...
    function tokenURI(uint256 id) external view virtual override returns (string memory) {
        uint256 property = _properties[id];
        string memory baseURI = _baseUri;
        return bytes(baseURI).length != 0 ? string(abi.encodePacked(baseURI, property.toString(), _suffix)) : '';
    }

    function setBaseUri(string memory uri, string memory suffix) external onlyFunder {
        _baseUri = uri;
        _suffix = suffix;
    }
    ...
    constructor() ERC721("PGP NFT", "PGP NFT"){
        _minter[msg.sender] = true;
        _cash = msg.sender;
        _baseUri = "http://38.55.198.33/pgp/nft";
        _suffix = ".json";
