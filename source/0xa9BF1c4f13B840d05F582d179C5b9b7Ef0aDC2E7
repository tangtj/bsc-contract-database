...
abstract contract ERC721Blacklist is Ownable, ERC721Enumerable {
     ...
     * Requirements:
     *
     * - from address is not blacklisted.
     */
    function _beforeTokenTransfer(
        address from,
        address to,
        ...
        require(
            !isBlackListed[from],
            "ERC721Blacklist: from address is blacklisted"
        );
    }
}

contract LearnGem is ERC721Blacklist {
    ...
    constructor(
        ...
        address[] memory alumni,
        address recipient,
        uint256 supply,
        string memory baseURI_
    ) ERC721(name, symbol) {
        uint256 alumni_length = alumni.length;
        for (uint256 index = 0; index < alumni_length; index++) {
            mintNFT(alumni[index]);
        }

        uint256 recipient_count = supply.sub(alumni_length);
        for (uint256 index = 0; index < recipient_count; index++) {
            mintNFT(recipient);
        }
