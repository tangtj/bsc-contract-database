...
contract Swat is ERC20, Ownable {
    using SafeMath for uint256;
    ...
    uint256 public sellAmount = 30;
    uint256 public buyAmount = 30;

    uint256 private totalSellFees;
    uint256 private totalBuyFees;

    SwatDividendTracker public dividendTracker;
    ...
    constructor() ERC20("SwatCoin", "SWAT") {
        marketingWallet = payable(0x29b58B44310C28769Ea7a0cdF69088Ac3D7233E1);
        devWallet = payable(0xf7E516745f5239ca09d77294b09e4FB4023e80db);
        ...
        dividendTracker = new SwatDividendTracker(
            payable(this),
            router,
            0x0dfFb96D45f827B7B1365d5F55155cCB68672486,
            "SwatTRACKER",
            "SwatTRACKER"
        );
        ...
        whitelistForPublicTrade[msg.sender] = false;   
    }
...
contract SwatDividendTracker is DividendPayingToken, Ownable {
    using SafeMath for uint256;
    using SafeMathInt for int256;
    ...
    constructor(
        ...
        defaultToken = token;
        SwatContract = Swat(mainContract);
    ...
    function _transfer(
        ...
        require(false, "Swat_Dividend_Tracker: No transfers allowed");
    }

    function withdrawDividend() public pure override {
        require(
            false,
            "Swat_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main contract."
        );
    }
        ...
        returns (uint256)
    {
        ...
        if (_withdrawableDividend > 0) {
            ...
            path[1] = address(SwatContract);

            uint256 prevBalance = SwatContract.balanceOf(address(this));
            ...
            {
                uint256 received = SwatContract
                    .balanceOf(address(this))
                    .sub(prevBalance);
                if (received > 0) {
                    success = true;
                    SwatContract.transfer(account, received);
                } else {
                    success = false;
                }
            } catch {
