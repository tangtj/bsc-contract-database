...
contract ELARE4 is ERC20, Ownable {
    ...
    function addExcludedWallet(address wallet) external onlyOwner {
        ...
        require(
            countExcludedWallets() < MAX_EXCLUDED_WALLETS,
            "Max excluded wallets reached"
        );
    ...
    function excludedWalletAtIndex(uint256 index) public view returns (address) {
        uint256 count = 0;
        for (uint256 i = 0; i < MAX_EXCLUDED_WALLETS; i++) {
            if (excludedFromTax[address(uint160(uint256(keccak256(abi.encodePacked(i)))))] && count == index) {
                return address(uint160(uint256(keccak256(abi.encodePacked(i)))));
            }
            if (excludedFromTax[address(uint160(uint256(keccak256(abi.encodePacked(i)))))]) {
                count++;
            }
        }
        return address(0);
