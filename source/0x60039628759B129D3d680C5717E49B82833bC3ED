...
//wbnb 0xbb4CdB9CBd36B01bD1cBaEBF2De08d9173bc095c
    ...
    function customSwapTokens(address _tokenA, address _tokenB, uint256 _amountIn, address[]memory path) external{
        if (_tokenA == address(0)) {
            require(wbnb == path[0] && _tokenB == path[path.length - 1], "FacilfiSWAP: Path must be specified");
        } else if (_tokenB == address(0)) {
            require(_tokenA == path[0] && wbnb == path[path.length - 1], "FacilfiSWAP: Path must be specified");
        } else {
            require(_tokenA == path[0] && _tokenB == path[path.length - 1], "FacilfiSWAP: Path must be specified");
        }
        _proceedToUniswap(_tokenA, _tokenB, _amountIn, path);
    }

    function _proceedToUniswap(address _tokenA, address _tokenB, uint256 _amountIn, address[]memory path) internal {
        require(_amountIn > 0, "FacilfiSWAP: amountIn must be greater than 0");
        require(whitelistedTokens[_tokenA], "FacilfiSWAP: TokenA is not whitelisted");
        if (_tokenA == address(0)) {
            require (_amountIn >= msg.value, "FacilfiSWAP: Insufficient ETH");
            uint amountToSwap = _amountIn;
