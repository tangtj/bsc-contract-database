...
contract SmartChef is Ownable, ReentrancyGuard {
    ...
    uint public depositFee = 400;
    uint public withdrawalFee = 1500;
  ...
  function canWithdraw( address _user) public view returns (bool) {
    ...
    function emergencyWithdraw() external nonReentrant {
        ...
        if (amountToTransfer > 0) {
            if(withdrawLockPeriod > 0) {
                uint fee = amountToTransfer.mul(400).div(10000);
    ...
    function updateStartAndEndBlocks(uint256 _startBlock, uint256 _bonusEndBlock) external onlyOwner {
        require(block.number < startBlock, "Dragon Pool has started");
