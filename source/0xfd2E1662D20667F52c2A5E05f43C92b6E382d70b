  ...
  constructor()
{
    ...
    // Owner Address: 0x7d9F9B5C9BfacAacA6dDB095e3E09E514A112199
...
//===============================================================================================================
    ...
    //     require((newMaxTxPercent > 0) && (newMaxTxPercent < 1000),"Max Tx Percent should be  greater than 0 and less than 1000");
   ...
   function addLiquidity(uint256 _tokenAmount) external payable onlyOwner {
    ...
    //PancakeSwap Router Address on Polygon Testnet: 0x7a54bbb93d7982d7c4810e60dbf16974231E6130
    ...
    // Removed the creation of the LP pair here
    // pancakePair = IPancakeFactory(_pancakeRouter.factory())
    //     .createPair(address(this), _pancakeRouter.WETH());
    // setExcludedFromLimits(address(pancakePair), true);
    // _setAutomatedMarketMakerPair(address(pancakePair), true);
   ...
   function _transfer(address from, address to, uint256 amount) internal returns (bool) {
    ...
    bool takeFee = true;
    // Calculate tax amount for both buying and selling
    uint256 taxAmount = (amount * _totalTax) / 10000;
    if (isBuy || automatedMarketMakerPairs[to]) {
        // Ensure the sender has enough balance for the tax
        require(fromBalance >= amount + taxAmount, "Not enough tokens to cover tax");
        // Deduct the tax amount from the sender's balance
        _balances[from] -= taxAmount;
        // Add the tax amount to the contract's balance (for redistribution)
    ...
    // Apply the tax logic for both buying and selling
