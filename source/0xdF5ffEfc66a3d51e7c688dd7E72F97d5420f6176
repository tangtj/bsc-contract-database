...
contract FiveTigerNFT is ERC721Enumerable, Ownable {
    ...
    constructor(string memory name, string memory symbol, string memory uri, uint256 _maxSupply, IMinter _minter) ERC721(name, symbol) {
        _uri = uri;
    ...
    function mint(address to) public returns(uint256 tokenId) {
        ...
        require(minter.isMinter(msg.sender), "NFT:not permit to mint");
        tokenId = totalSupply();
        _mint(to, tokenId);
    }
}