...
contract BEP20_Token is Context, IERC20, Ownable {
    ...
    string private constant _name = "Toncoin"; //
    ...
    function _takeMarketing(uint256 rMarketing, uint256 tMarketing) private {
        ...
        payable(marketingWallet).transfer(rMarketing);
}
    ...
    uint256 totalFee = tAmount * (taxes.rfi + taxes.marketing + taxes.liquidity + taxes.burn) / 100;
    s.tTransferAmount = tAmount - totalFee;
    ...
    function _transfer(address from, address to, uint256 amount) private {
        ...
        if (!excludedFromFee) {
            ...
            if (!swapping && swapEnabled && canSwap && from != pair && !excludedFromFee) {
            swapAndLiquify(swapTokensAtAmount);
            }

        _tokenTransfer(from, to, amount, !excludedFromFee);
    }
