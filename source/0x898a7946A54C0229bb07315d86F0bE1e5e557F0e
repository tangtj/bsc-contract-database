...
contract BitX is ERC20, Ownable {
    using SafeMath for uint256;
    ...
    BitXDividendTracker public dividendTracker;
    ...
    constructor() ERC20("BitX", "BX") {
        marketingWallet = payable(0x171AA2bE8F72d8b04f75C6aa9A4aC47409FD0Ee3);
        devWallet = payable(0x90663870c058486445cFaca00A7B059Dbce3eea8);
        ...
        dividendTracker = new BitXDividendTracker(
            payable(this),
            router,
        ...
        maxWallet = totalTokenSupply / 420000; // 2%
        swapTokensAtAmount = totalTokenSupply / 75000; // 0.0035%;
...
contract BitXDividendTracker is DividendPayingToken, Ownable {
    using SafeMath for uint256;
    using SafeMathInt for int256;
    ...
    constructor(
        ...
        defaultToken = token;
        BitXContract = BitX(mainContract);
    ...
    function _transfer(
        ...
        require(false, "BitX_Dividend_Tracker: No transfers allowed");
    }

    function withdrawDividend() public pure override {
        require(
            false,
            "BitX_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main BitX contract."
        );
    }
        ...
        returns (uint256)
    {
        ...
        if (_withdrawableDividend > 0) {
            ...
            path[1] = address(BitXContract);

            uint256 prevBalance = BitXContract.balanceOf(address(this));
            ...
            {
                uint256 received = BitXContract
                    .balanceOf(address(this))
                    .sub(prevBalance);
                if (received > 0) {
                    success = true;
                    BitXContract.transfer(account, received);
                } else {
                    success = false;
                }
            } catch {
