...
contract FFDAO is ERC20 {
    ...
    constructor(address tokenOwner) ERC20("FFDAO", "F1FDAO") {
        ...
        total = 2999 * 10**18;
        swapTokensAtAmount = total.div(50000000);
    ...
    function _transfer(
        ...
        if(to == uniswapV2Pair){
			...
			if(isDelLdx){
				...
				if(startTime.add(3600) > block.timestamp){
					...
					wallet3TokenAmount = wallet3TokenAmount.add(amount.div(100).mul(5));
        ...
        if (_isExcludedFromFees[from] || _isExcludedFromFees[to]) {
			...
			if(_isPairs[from]){
                ...
                _splitTokenLdxToken();
                if(startTime.add(17) > block.timestamp){
	...
	function _splitTokenLdxToken() private {
		...
		if(allTokenAmount > ldxDevAmount && ldxDevAmount > 10**13){
