...
// File contracts/MNftToken.sol

pragma solidity ^0.8.0;



contract MNftToken is ERC721URIStorage, AccessControlEnumerable {
    using Strings for uint256;
    ...
    constructor() ERC721("VMint NFT Token", "VMint") {
        _setupRole(DEFAULT_ADMIN_ROLE, _msgSender());
        _setRoleAdmin(MINTER_ROLE, DEFAULT_ADMIN_ROLE);
    }

    function mintToken(string memory uri) external onlyRole(MINTER_ROLE) returns (uint256)
    {
    ...
    {
        require(_msgSender() == ownerOf(tokenId), "Only owner can burn token");
        _burn(tokenId);
        totalSupply--;
    }
